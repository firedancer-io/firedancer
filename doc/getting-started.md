...
- Build Firedancer. E.g. From the directory where firedancer was
  ...
  build results will be in the relative directory `target`.  `make` has
  many powers; run `make help` for more info.  If building on a system
  with lots of isolated cores, see `contrib/make-j`.

- Reserve host resources for application usage.  E.g.:
    ```
    $ sudo target/gcc/bin/fd_shmem_cfg             \
      alloc 8 gigantic 0                           \
      alloc 8 gigantic 1                           \
      alloc 512 huge 0                             \
      alloc 512 huge 1
...
- Create an appropriately permissioned sandbox for managing shared
  memory data structures.  E.g.:
    ```
    $ sudo target/gcc/bin/fd_shmem_cfg init 0700 [USER] ""
    ```
