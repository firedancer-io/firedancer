...
#define FD_BANK_STATE_DEAD (0x00000008UL)       /* Dead.  We stopped replaying it before we could finish it (e.g. invalid block or pruned minority fork). */
...
{
    return bank->state & FD_BANK_STATE_DEAD; // $ Alert (returns value in [0, 8])
}
