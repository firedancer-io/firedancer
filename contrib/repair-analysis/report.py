...
def completion_times( fec_stats, shred_data, first_turbine, pdf ):
    ...
    # Calculate batch-level completion times (ref_tick granularity)
    # Aggregate FEC sets within each slot+ref_tick combination
    ...
    batch_stats['time_to_complete(ms)'] = batch_stats['time_to_complete'] / 1_000_000

    # Generate batch completion time distribution histogram
    fig = plt.figure(figsize=(12, 6))
    ...
    # Split batch statistics by execution phase
    ...
    # Display side-by-side comparison of FEC, batch, and slot-level timing statistics
    ...
    # Generate detailed batch completion timeline visualization for specific slot
    # Create horizontal interval chart showing ref_tick processing durations
    slot_interest = 100
    fig = plt.figure(figsize=(12, 6))
...
def turbine_shred_timeline(shreds_data, first_turbine, pdf):
    ...
    if slot_range > 1000:
        ...
        first_500_end = first_500_start + 500
        last_500_end = live_data['slot'].max()
        last_500_start = max(first_turbine, last_500_end - 499)
...
def slot_request_rate_analysis(shreds_data, first_turbine, pdf, include_after_turbine=False):
    ...
    print(f"Time to repair each slot: {avg_time_per_slot_ns / 1e6:.2f} milliseconds")
    ...
    # stats text
    avg_text = (f"Time to repair each slot: {avg_time_per_slot_ns / 1e6:.2f} ms\n"
...
def peer_stats_analysis(repair_requests, shreds_data, pdf):
    ...
    print("Generating RTT analysis page...")
    fig, axes = plt.subplots(2, 2, figsize=(24, 16))
    fig.suptitle('Round-Trip Time Analysis', fontsize=32)

    if not round_trip_times.empty:
        axes[0,0].hist(round_trip_times, bins=100, color='blue', alpha=0.7, edgecolor='black')
        ...
        axes[0,0].axvline(p25, color='lightgray', linestyle='--', linewidth=2, label=f'25th %ile: {p25:.1f}ms')
        axes[0,0].axvline(p50, color='gray', linestyle='--', linewidth=2, label=f'50th %ile: {p50:.1f}ms')
        axes[0,0].axvline(p75, color='darkgray', linestyle='--', linewidth=2, label=f'75th %ile: {p75:.1f}ms')
        axes[0,0].axvline(mean_val, color='black', linestyle='-', linewidth=2, label=f'Mean: {mean_val:.1f}ms')

        axes[0,0].set_title('RTT Distribution', fontsize=20)
        axes[0,0].set_xlabel('Round-trip Time (ms)', fontsize=16)
    ...
    if not rtt_filtered.empty:
        axes[0,1].hist(rtt_filtered, bins=75, color='lightblue', alpha=0.7, edgecolor='black')

        # Use the same percentile and mean lines from the full dataset (not recalculated from filtered data)
        if p25 < 200:
            axes[0,1].axvline(p25, color='lightgray', linestyle='--', linewidth=2, label=f'25th %ile: {p25:.1f}ms')
        if p50 < 200:
            axes[0,1].axvline(p50, color='gray', linestyle='--', linewidth=2, label=f'50th %ile: {p50:.1f}ms')
        if p75 < 200:
            axes[0,1].axvline(p75, color='darkgray', linestyle='--', linewidth=2, label=f'75th %ile: {p75:.1f}ms')
        if mean_val < 200:
            axes[0,1].axvline(mean_val, color='black', linestyle='-', linewidth=2, label=f'Mean: {mean_val:.1f}ms')

        axes[0,1].set_title('RTT Distribution (< 200ms)', fontsize=20)
        axes[0,1].set_xlabel('Round-trip Time (ms)', fontsize=16)
    ...
    if not median_rtt_per_peer.empty and not valid_rtt_data.empty:
        ...
        if top_16_rtt_data:
            bp1 = axes[1,0].boxplot(top_16_rtt_data, patch_artist=True, labels=top_16_labels, showfliers=False)
            for patch in bp1['boxes']:
                patch.set_facecolor('green')
                patch.set_alpha(0.7)

            axes[1,0].set_title('Top 16 Peers (Best Median RTT)', fontsize=20)
            axes[1,0].set_xlabel('Peer IP Address', fontsize=16)
            axes[1,0].set_ylabel('RTT Distribution (ms)', fontsize=16)
    ...
    if not median_rtt_per_peer.empty and not valid_rtt_data.empty:
        ...
        if bottom_16_rtt_data:
            bp2 = axes[1,1].boxplot(bottom_16_rtt_data, patch_artist=True, labels=bottom_16_labels, showfliers=False)
            for patch in bp2['boxes']:
                patch.set_facecolor('red')
                patch.set_alpha(0.7)

            axes[1,1].set_title('Bottom 16 Peers (Worst Median RTT)', fontsize=20)
            axes[1,1].set_xlabel('Peer IP Address', fontsize=16)
            axes[1,1].set_ylabel('RTT Distribution (ms)', fontsize=16)
    ...
    print("Generating hit rates and request counts page...")
    ...
    print("Generating combined analysis page...")
    fig, axes = plt.subplots(1, 2, figsize=(24, 12))
    ...
    # 1. scatterplot: request frequency vs hit rate
    ...
    # 2. scatterplot: request frequency vs RTT (vectorized filtering)
    if not peer_stats_combined.empty:
        ...
        if rtt_mask.any():
            ...
            axes[1].scatter(valid_requests, valid_rtt, alpha=0.6, s=60, color='darkblue')
...
def repair_efficiency_heatmap(repair_requests, shreds_data, snapshot_slot, first_turbine, pdf):
    ...
    uintmax_mask_resp = repair_period_responses['idx'] >= UINTMAX_THRESHOLD
    if uintmax_mask_resp.any():
        print(f"Replacing {uintmax_mask_resp.sum()} UINTMAX values in responses")
        for idx in repair_period_responses[uintmax_mask_resp].index:
            slot = repair_period_responses.loc[idx, 'slot']
            if slot in max_shred_per_slot:
                repair_period_responses.loc[idx, 'idx'] = max_shred_per_slot[slot]
            else:
                repair_period_responses.loc[idx, 'idx'] = 0
    ...
    print("Filtering for fully buffered slots...")
    valid_slots = []
    ...
    for slot in efficiency_data['slot'].unique():
        ...
        if expected_indices == actual_indices:
            ...
            print(f"Slot {slot}: skipped, has gaps in middle: {sorted(list(missing_indices))[:10]}...")

    print(f"Keeping {len(valid_slots)} fully buffered slots out of {efficiency_data['slot'].nunique()} total slots")
    ...
    print(f"Non-buffered slots (will show as empty columns): {len(non_buffered_slots)} slots")
    if len(non_buffered_slots) <= 20: # if not too many, show the list
        print(f"Non-buffered slot numbers: {non_buffered_slots}")
    else:
        print(f"Non-buffered slot examples: {non_buffered_slots[:10]}...{non_buffered_slots[-5:]}")
    ...
    # Show all slots between snapshot and first turbine (like slot request rate analysis)
    print(f"Processing all {len(slots)} slots from {min(slots)} to {max(slots)}")

    print(f"Processing {max_shred_idx + 1} shred indices for heatmap")
    ...
    print(f"Efficiency ratio statistics (buffered slots only):")
    print(f"  Full range: {efficiency_data_buffered['efficiency_ratio'].min():.2f} to {efficiency_data_buffered['efficiency_ratio'].max():.2f}")
    print(f"  95th percentile: {p95:.2f}")
    print(f"  99th percentile: {p99:.2f}")
    print(f"  Outlier threshold: {outlier_threshold:.2f}")
    print(f"  Found {len(outliers)} outlier shreds")
    print(f"Setting colormap scale from {min_efficiency_ratio:.2f} to {max_efficiency_ratio:.2f} (excluding outliers)")
    ...
    if len(outliers) > 0:
        ...
        print("-" * 85)
        for slot, row in outlier_summary.head(20).iterrows():  # Show top 20 worst slots
        ...
        if len(outlier_summary) > 20:
            print(f"... and {len(outlier_summary) - 20} more slots with outliers")
    else:
