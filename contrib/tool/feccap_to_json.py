...
def parse_feccap_file(file_path: str, data_limit: int = 100) -> Dict[str, Any]:
    ...
    for i in range(fec_count):
        ...
        # Extract slot and fec_set_idx from shred data header within chunk
        # Slot is at offset 0x41 (65 bytes) from start of shred header
        # fec_set_idx is at offset 0x4f (79 bytes) from start of shred header
        slot = struct.unpack('<Q', fec_msg_chunk_data[0x41:0x41+8])[0]
        fec_set_idx = struct.unpack('<I', fec_msg_chunk_data[0x4f:0x4f+4])[0]
        ...
        fec_record = OrderedDict([
            ('fec_message', OrderedDict([
                ('size', fec_msg_size),
                ('slot', slot),              # ulong slot from shred header
                ('fec_set_idx', fec_set_idx),  # uint fec_set_idx from shred header
