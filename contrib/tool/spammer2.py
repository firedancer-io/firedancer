...
def create_accounts_tx(funder, lamports, recent_blockhash, accs):
    acc_mint = Pubkey.from_string("FDTest1111111111111111111111111111111111111")

    tx = Transaction(recent_blockhash, None, funder.pubkey(), [set_compute_unit_price(5), set_compute_unit_limit(30000000)])
    for acc in accs:
        tx = tx.add(transfer(TransferParams(from_pubkey=funder.pubkey(), to_pubkey=acc.pubkey(), lamports=lamports)))
        #tx = tx.add(create_associated_token_account(payer=funder.pubkey(), owner=acc.pubkey(), mint=acc_mint ))
    tx.sign(funder)
    return tx
    # client.send_transaction(tx, funder, opts=TxOpts(skip_confirmation=True))

def mint_accounts_tx( funder, recent_blockhash, accs ):
    try:
        acc_mint = Pubkey.from_string("FDTest1111111111111111111111111111111111111")
        tx = Transaction(recent_blockhash, None, funder.pubkey(), [set_compute_unit_price(5), set_compute_unit_limit(30000000)])
        ...
        for acc in accs:
            ...
            tx = tx.add(mint_to(MintToParams( amount= 100, mint=acc_mint, dest=associated_token_address, mint_authority=funder.pubkey(), program_id=TOKEN_PROGRAM_ID)))
        tx.sign(funder)
        return tx
...
def create_accounts(funder, rpc, num_accs, lamports, seed, sock, tpus, txn_type):
    ...
    # txs = pqdm(acc_chunks, partial(mint_accounts_tx, funder, recent_blockhash), desc="mint to accounts 1", n_jobs=32)
    # send_round_of_txs(txs, sock, tpus)
    ...
    while len(remaining_accs) > 0:
        ...
        txs = pqdm(acc_chunks, partial(create_accounts_tx, funder, lamports, recent_blockhash), desc="fund accounts", n_jobs=32)
        send_round_of_txs(txs, sock, tpus)
        # txs = pqdm(acc_chunks, partial(mint_accounts_tx, funder, recent_blockhash), desc="mint to accounts 2", n_jobs=32)
        # send_round_of_txs(txs, sock, tpus)
...
def create_token(funder, recent_blockhash, sock, tpus):
    acc = Pubkey.from_string("FDTest1111111111111111111111111111111111111")
    tx = Transaction(recent_blockhash, None, funder.pubkey(), [set_compute_unit_price(10), set_compute_unit_limit(30000000)])
    tx = tx.add(transfer(TransferParams(from_pubkey=funder.pubkey(), to_pubkey=acc, lamports=100_000_000_000 )))

    params = InitializeMintParams(        
      decimals=9,
      freeze_authority=funder.pubkey(),
      mint=acc,
...
def gen_tx_system_transfer(recent_blockhash, key, acc, cu_price):
  ...
  tx = Transaction(recent_blockhash, None, acc, [set_compute_unit_price(cu_price), set_compute_unit_limit(300+300+150 * 1000), 
                                    transfer(TransferParams(from_pubkey=acc,to_pubkey=acc,lamports=1))] )
  ...
  # tx = tx.add( spl_transfer( SplTransferParams( amount=1,
...
def gen_txn_token_transfer( recent_blockhash, key, acc, cu_price):
  tx = Transaction(recent_blockhash,)
  return None
  ...
  while not stop_event.is_set():
    ...
    for i in range(len(keys)):
      if txn_type == TXN_TYPE_EMPTY:
        ...
        tx = gen_txn_token_transfer(recent_blockhash, keys[i], accs[i], cu_price)
      message = bytes(tx.to_solders())
      for tpu in tpus:
        sock.sendto(message, tpu)
...
def main():
  ...
  recent_blockhash = get_recent_blockhash(args.rpc)
  print("trying to create token")
  
  #create_token(funder, recent_blockhash, sock, tpus)
  #print("created token")
