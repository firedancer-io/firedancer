...
# It currently manages operations like:
# 1. Producing ledgers with mismatches (fetch, replay, minimize, upload)
# 2. Debugging ledgers with mismatches (solcap, execute, debug)
...
replay_mini_upload() {
  ...
  if [ "$REPETITIONS" = "multiple" ]; then
    multi_replay_mini_upload
  else
    single_replay_mini_upload 0
  fi
}
...
usage() {
  if [[ $1 == "setup" ]]; then
  ...
  elif [[ $1 == "replay-mini-upload" ]]; then
    echo -e "Usage: $0 replay-mini-upload \n\
                --network -n                    : Solana network to download the ledger from. Choose: mainnet|testnet|internal. \n\                
                --ledger -l                     : Directory where the initial ledger can be found. \n\
                --ledger-min -z                 : Directory where the minimized ledger should be placed. \n\
                --solana-build-dir -d           : Path to the solana|agave build directory. E.g. /home/fd_user/solana|agave/target/debug \n\
    ...
    echo -e "Usage: $0 conformance \n\
                --input -i                      : Path to the input file containing the mismatching transaction. \n\
                --target -t                     : Path to the target file containing the mismatching transaction. \n\
                --mode -m                       : Mode to run the conformance test. Choose: run-tests|exec|debug. \n\
                --directory -d                  : Path to the root directory containing firedancer, solana, solfuzz-agave and solana-conformance repos. \n\
                --index-max -x [Optional]       : Maximum index. Default: 600_000_000 \n\
                --pages -p [Optional]           : Number of pages. Default: 75 \n\
                --funk-pages -f [Optional]      : Number of funk pages. Default: 550"
    ...
    cat <<'EOF'
            SUBCOMMANDS:
                ...
                minify                                  : Get a minimized snapshot and minified rocksdb
                    --mode edge                         : Minimize around an epoch edge with some offset
                    --mode exact                        : Minimize around a specific [start_slot, end_slot]
                replay-mini-upload                      : Run fetch-recent, replay, minify and upload in sequence                                                          
                        --no-fetch                          : Run all the commands excluding fetch-recent. Pass in your desired ledger.
                        --repetitions once --mode exact     : Run the full cycle of commands once
                        --repetitions multiple --mode exact : Replay the entire ledger in multiple iterations.
                                                              ...
                                                              In the next cycle, this would begin from the slot after the last mismatched slot.                
                solcap                                  : Produce a diff between firedancer and solana labs solcaps
                conformance                             : Debug the mismatching transaction with debug, exec and run_tests mode.
...
parse_replay_mini_upload_options() {
  TEMP=$(getopt -o n:l:z:v:q:i:g:x:s:e:d:f:u:r: \
    --long network:,ledger:,ledger-min:,is-verify:,no-fetch,slots-in-epoch:,gigantic-pages:,index-max:,start-slot:,end-slot:,solana-build-dir:,firedancer-root-dir:,upload:,repetitions: \
    -- "$@")
...
parse_conformance_options() {
  TEMP=$(getopt -o i:t:d:x:p:f: --long input:,target:,directory:,index-max:,pages:,funk-pages: -- "$@")
  if [ $? != 0 ]; then
    echo "Incorrect options provided" >&2
    exit 1
  fi
  ...
  if [ -z "$REPO_ROOT" ] || [ -z "$INPUT_FILE" ]; then
    usage "conformance"
    exit 1
  fi

  if [ "$TARGET" != "agave" ] && [ "$TARGET" != "fd" ]; then
    echo "Invalid target: $TARGET. Choose: agave|fd."
    exit 1
  fi

  if [ "$MODE" != "debug" ] && [ "$MODE" != "exec" ] && [ "$MODE" != "run-tests" ]; then
    echo "Invalid mode: $MODE. Choose: debug|exec|run-tests."
    exit 1
  fi
...
replay-mini-upload)
  parse_replay_mini_upload_options "$@"
  echo -e "running cmd=replay-mini-upload with\n" \
    " network=$NETWORK,\n" \
    " mode=$MODE,\n" \
    " ledger=$LEDGER,\n" \
    " ledger-min=$LEDGER_MIN,\n" \
...
conformance)
  parse_conformance_options "$@"
  echo -e "running cmd=conformance with\n" \
