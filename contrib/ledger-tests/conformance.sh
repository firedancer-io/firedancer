...
agave)
    ...
    ;;
fd)
    TARGET_SHARED_OBJ_FILE="-t $txn_firedancer_repo/build/native/gcc/lib/libfd_exec_sol_compat.so"
    DEBUGGER="gdb"
    ;;
esac
...
if [[ "$MODE" == "run-tests" ]]; then
    ...
    HARNESS_TYPE="TxnHarness" solana-test-suite exec-instr $TARGET_SHARED_OBJ_FILE $INPUT_FILE
