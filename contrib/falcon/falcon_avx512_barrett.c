...
fq_mul_v( __m512i a, __m512i b ) {
  ...
  /* r in [0, 2Q); fold to [0, Q) via masked subtract.  See the same
   * comment in fq_mul_shoup_v (falcon_avx512.c). */
 ...
 * parallel-squeeze variant (see falcon_ref_ktp256.c).  The resulting
...
falcon_avx512_barrett_ktp256_crypto_sign_open( uint8_t       * m,  size_t * mlen,
  ...
  /* The KTP256 hash writes 16-bit coefficients; the AVX-512 NTT
  ...
  /* Always memmove and report msg_len -- like falcon_ref_ktp256, the
