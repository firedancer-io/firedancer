...
/* AVX-512-only turbopar hash: declared by falcon_avx512_common.h, but
 * we don't want to pull in that whole header here, so spell out the
 * declaration with the matching guard. */
#if defined(__AVX512F__) && defined(__AVX512BW__) && defined(__AVX512DQ__)
#define HAVE_AVX512 1
extern void fa512_hash_to_point_turbopar( uint16_t * out,
 ...
 * `falcon_x86_turbopar_crypto_sign_open`.  The hash is performed
 * inline based on `use_turbopar_hash`; everything else (parse, NTT
...
falcon_x86_verify_with_c( uint8_t       * m,  size_t * mlen,
  ...
  fq_t c[ N ];
#if HAVE_AVX512
  if( use_turbopar_hash ) {
    uint16_t c16[ N ];
    fa512_hash_to_point_turbopar( c16, sm + 2, NONCELEN + msg_len );
    for( int i=0; i<N; i++ ) c[ i ] = (fq_t)c16[ i ];
  } else
#else
  (void)use_turbopar_hash;
#endif
 ...
 * AVX-512. */
int
falcon_x86_turbopar_crypto_sign_open( uint8_t       * m,  size_t * mlen,
                                      uint8_t const * sm, size_t   smlen,
                                      uint8_t const * pk ) {
#if HAVE_AVX512
