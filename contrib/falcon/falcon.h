 ...
 *   - falcon_ref_ktp256_*       : same Pornin pipeline, but hash-to-point
 ...
 *   - falcon_x86_ktp256_*       : `falcon_x86` + parallel-squeeze hash.
 ...
 *                                   point of comparison.
 *   - falcon_avx512_barrett_ktp256_* : Barrett AVX-512 + parsq hash.
 *   - falcon_avx512_ktp256_*         : Shoup AVX-512 + parsq hash.
 *
...
/* Same AVX-512 NTT skeleton as `falcon_avx512_barrett`, but with
 * always-reduced add/sub (no lazy reduction across NTT passes, no off_q
 * tracking in the inverse).  Used to attribute the contribution of lazy
 * reduction vs the skeleton itself in the per-optimisation analysis. */
