...
/* An iterator goes invalid both at the end of a column family and on a
   read/checksum error.  Without asking, a damaged ledger yields a
   silently truncated capture and exit status 0. */

static void
iter_check_error( rocksdb_iterator_t * iter,
                  char const *         cf_name ) {
  char * err = NULL;
  rocksdb_iter_get_error( iter, &err );
  if( err ) {
    fprintf( stderr, "error: iterating column family \"%s\" failed: %s\n", cf_name, err );
    exit( 1 );
  }
}
  ...
  for(;;) {
    if( src->current_slot==UINT64_MAX ) {
      if( !rocksdb_iter_valid( src->root_iter ) ) {
        iter_check_error( src->root_iter, "root" );
    ...
    if( !src->data_slot_done ) {
      ...
      if( !rocksdb_iter_valid( iter ) ) {
        iter_check_error( iter, "data_shred" );
    ...
    if( !src->code_slot_done ) {
      ...
      if( !rocksdb_iter_valid( iter ) ) {
        iter_check_error( iter, "code_shred" );
        src->code_slot_done = 1;
        src->code_iter_done = 1;
        continue;
      }
