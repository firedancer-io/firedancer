...
pattern1 = r'ADD.?\(\s*"([^"]*)"\s*(?:,\s*([^)]*))?\s*\)'                           # ADD macros
pattern2 = r'if\(\s*(?:FD_(?:LIKELY|UNLIKELY)\(\s*)?!?(.*?)(?:\s*\)\s*\)|\s*\))'    # if statements for boolean flags
...
def parse_cmdline(flags_string):
    flags = []
    flag  = ()
    for line in flags_string.split('\n'):
        # ignore comments
        if line.startswith('#'):
            continue
        ...
        for word in sline.split(' '):
            ...
            if sword.startswith('--'):
                ...
                logging.error(f"unrecognized token while parsing - {sword}")
                sys.exit(1)
...
def build_flag_config_map(lines):
    for i in range(len(lines)):
        ...
        if match:
            ...
            if not config:
                # look for if statements in case it is a boolean flag
            ...
            if flag and flag not in flag_config_map and "firedancer" not in flag:
...
def update_flag_config_map(lines):
    for line in lines:
        ...
        if line.startswith("CFG_POP1") or line.startswith("CFG_POPARRAY1"):
            ...
            # if we have encountered this flag-config combo, update the config option
    ...
    flag_config_map["--snapshot-interval-slots"] = "snapshots.incremental_snapshot_interval_slots"
...
def build_dict_from_key(data, key, data_type, value):
    ...
    if len(parts) == 1:
        ...
        if data_type == "list":
            ...
            logging.error(f"value cannot be a dict - key={key}, value={value}")
            sys.exit(1)
        else:
            # cannot set a value more than once
            if parts[0] in data:
                logging.error(f"value cannot be updated - key={key}, value={value}")
                sys.exit(1)
            # only 1 argument for flags that are not `list` or `dict`
            elif len(value) != 1:
                logging.error(f"expected 1 value, found {len(value)} - key={key}, value={value}")
                sys.exit(1)
            else:
...
def main():
    ...
    if arguments.detect_mismatch:
        if hardcoded_flag_config_map != flag_config_map:
            logging.error("detected difference between config maps")
            # shows a mismatch in keys between the hardcoded and parsed maps
            if len(hardcoded_flag_config_map.keys()) > len(flag_config_map.keys()):
                logging.error(f"diff keys - {hardcoded_flag_config_map.keys()-flag_config_map.keys()}")
            else:
                logging.error(f"diff keys - {flag_config_map.keys()-hardcoded_flag_config_map.keys()}")
            # shows all key-value pairs in the parsed map
            if flag_config_map.keys() == hardcoded_flag_config_map.keys():
                logging.error("parsed map values")
                pprint(flag_config_map)
            sys.exit(1)

    if not os.path.exists(arguments.file_name):
        logging.error(f"file {arguments.file_name} does not exist")
        sys.exit(1)
    ...
    for (flag, *args) in flags_to_parse:
        ...
        if flag not in hardcoded_flag_config_map:
            logging.warning(f"cannot convert flag {flag}")
            continue
        # making sure there are not too many args
        if config_type_map[hardcoded_flag_config_map[flag]] not in ["dict", "list"] and len(args) != 1:
            logging.error(f"invalid arguments {args} for flag {flag}")
            sys.exit(1)
        # FD inverts flags with `--no` and `--disable`
        if config_type_map[hardcoded_flag_config_map[flag]] == "bool":
            if flag.startswith('--no') or flag.startswith('--disable'):
                args[0] = False
        # parse argument types
        if config_type_map[hardcoded_flag_config_map[flag]] == "int":
            args[0] = int(args[0])
        elif config_type_map[hardcoded_flag_config_map[flag]] == "float":
            args[0] = float(args[0])
        if flag == "--full-snapshot-interval-slots":
            seen_full = True
        if flag == "--snapshot-interval-slots":
            seen_incr = True
        build_dict_from_key(data, hardcoded_flag_config_map[flag], config_type_map[hardcoded_flag_config_map[flag]], args)
    ...
    if seen_incr and not seen_full:
        data["snapshots"]["full_snapshot_interval_slots"] = data["snapshots"]["incremental_snapshot_interval_slots"]
    ...
    if arguments.output_file:
        if os.path.exists(arguments.output_file):
            logging.error(f"file {arguments.output_file} already exists")
            sys.exit(1)
...
def argini():
    ...
    parser.add_argument("--systemd",         "-s", action="store_true",        help="Specify this if the file is a systemd unit file")
    parser.add_argument("--file-name",       "-f", required=True,  type=str,   help="Path to file containing Agave commandline")
    parser.add_argument("--output-file",     "-o", default="",                 help="Specify the path to the TOML file to create")
    parser.add_argument("--detect-mismatch", "-d", action="store_true",        help="Exit if there is a discrepancy between hardcoded and parsed flag maps")
