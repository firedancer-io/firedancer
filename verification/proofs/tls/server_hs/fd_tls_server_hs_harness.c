/* fd_tls_server_hs_harness symbolically executes all reachable server
...
harness( void ) {
  fd_tls_t tls_[1];
  fd_tls_t * tls = fd_tls_join( fd_tls_new( tls_ ) );
  ...
  memcpy( tls->alpn, "\xasolana-tpu", 12UL );
  tls->quic         = 1;
  ...
  fd_tls_delete( fd_tls_leave( tls ) );
}
