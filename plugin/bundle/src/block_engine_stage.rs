...
pub extern "C" fn plugin_bundle_poll(
    ...
    match executor.runtime.block_on(future) {
        ...
        Some(BundleOrPacket::Packet(packet, block_builder_pubkey, commission)) => {
...
fn produce_bundles(
    ...
    stream::unfold((url, domain_name, identity_pubkey), |(url, domain_name, identity_pubkey)| async move {
        let (mut client, auth_client, access_token, refresh_token) = match connect_auth(&url, &domain_name, &identity_pubkey).await {
            Ok(result) => result,
            Err(ProxyError::AuthenticationPermissionDenied) => {
                ...
                return Some((None, (url, domain_name, identity_pubkey)));
        ...
        let stream = stream.filter_map(|item| async {
            match item {
                ...
                (StreamSelector::Packet(packet), pubkey, commission) => {
                    Some(Ok(BundleOrPacket::Packet(packet, pubkey, commission)))
                }
        ...
        Some((Some(stream), (url, domain_name, identity_pubkey)))
