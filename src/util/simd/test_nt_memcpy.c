...
main( int     argc,
  ...
  do {
    ...
    /* make sure the memory is actually present, populate the TLB, etc.
       before we start timing. */
    memset( big_src, '\x1', BIG_SZ );   FD_COMPILER_UNPREDICTABLE( big_src );
    memset( big_dst, '\x2', BIG_SZ );   FD_COMPILER_UNPREDICTABLE( big_dst );
