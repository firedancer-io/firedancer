...
static inline ulong              /* removed ele_idx, not in slist and not in pool on return */
ref_slist_idx_pop_tail( void ) { /* assumes not empty */
  ulong ele_idx  = ref_slist_tail;
  ulong prev_idx = ref_ele[ ele_idx ].prev;
  ref_slist_tail = prev_idx;
  if( prev_idx==ELE_IDX_NULL ) ref_slist_head           = ELE_IDX_NULL;
  else                         ref_ele[ prev_idx ].next = ELE_IDX_NULL;
  return ele_idx;
}

static inline void                               /* ele_idx in slist before slist_idx on return */
ref_slist_idx_insert_before( ulong ele_idx,      /* assumes not in slist and not in pool */
                             ulong slist_idx ) { /* assumes in slist */
  ulong prev_idx = ref_ele[ slist_idx ].prev;

  ref_ele[ ele_idx ].prev = prev_idx;
  ref_ele[ ele_idx ].next = slist_idx;

  ref_ele[ slist_idx ].prev =ele_idx;
...
static inline void                      /* ele_idx not in slist and not in pool on return */
ref_slist_idx_remove( ulong ele_idx ) { /* assumes in slist */
  ulong prev_idx = ref_ele[ ele_idx ].prev;
  ulong next_idx = ref_ele[ ele_idx ].next;

  if( prev_idx==ELE_IDX_NULL ) ref_slist_head           = next_idx;
  else                         ref_ele[ prev_idx ].next = next_idx;

  if( next_idx==ELE_IDX_NULL ) ref_slist_tail           = prev_idx;
  else                         ref_ele[ next_idx ].prev = prev_idx;
}

static inline void                       /* ele_idx in slist where old_idx was, old_idx not in slist and not in pool on return */
ref_slist_idx_replace( ulong ele_idx,    /* assumes not in slist and not in pool */
                       ulong old_idx ) { /* assumes in slist */
