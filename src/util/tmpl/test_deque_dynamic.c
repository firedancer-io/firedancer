...
main( int     argc,
  ...
  FD_EXPECT_LOG_ERR( test_deque_new   ( NULL                     , TEST_DEQUE_MAX ) );
  FD_EXPECT_LOG_ERR( test_deque_new   ( (void*)((char*)scratch+1), TEST_DEQUE_MAX ) );
  FD_EXPECT_LOG_ERR( test_deque_delete( NULL ) );
  FD_EXPECT_LOG_ERR( test_deque_delete( (void*)((char*)scratch+1) ) );
  ...
  FD_EXPECT_LOG_ERR( test_deque_push_tail       ( deque, 42 ) );
  FD_EXPECT_LOG_ERR( test_deque_push_head       ( deque, 42 ) );
  FD_EXPECT_LOG_ERR( test_deque_push_head_nocopy( deque     ) );
  FD_EXPECT_LOG_ERR( test_deque_push_head_nocopy( deque     ) );
  FD_EXPECT_LOG_ERR( test_deque_insert_head     ( deque     ) );
  FD_EXPECT_LOG_ERR( test_deque_insert_tail     ( deque     ) );
  ...
  FD_TEST( test_deque_remove_all( deque )==deque );
  FD_EXPECT_LOG_ERR( test_deque_pop_head       ( deque ) );
  FD_EXPECT_LOG_ERR( test_deque_pop_tail       ( deque ) );
  FD_EXPECT_LOG_ERR( test_deque_pop_head_nocopy( deque ) );
  FD_EXPECT_LOG_ERR( test_deque_pop_tail_nocopy( deque ) );
  FD_EXPECT_LOG_ERR( test_deque_peek_tail      ( deque ) );
  FD_EXPECT_LOG_ERR( test_deque_peek_head      ( deque ) );
  FD_EXPECT_LOG_ERR( test_deque_peek_tail_const( deque ) );
  FD_EXPECT_LOG_ERR( test_deque_peek_head_const( deque ) );
  FD_EXPECT_LOG_ERR( test_deque_remove_head    ( deque ) );
  FD_EXPECT_LOG_ERR( test_deque_remove_tail    ( deque ) );
  ...
  iter = test_deque_iter_next( deque, iter );
  FD_EXPECT_LOG_ERR( test_deque_iter_ele_const( deque, iter ) );
  FD_EXPECT_LOG_ERR( test_deque_iter_ele( deque, iter ) );
  FD_TEST( test_deque_leave ( deque   )==shdeque         );
  FD_TEST( test_deque_delete( shdeque )==(void *)scratch );
  ...
  shdeque = test_deque_new ( scratch, 0UL ); FD_TEST( shdeque );
  deque   = test_deque_join( shdeque      ); FD_TEST( deque   );
  FD_EXPECT_LOG_ERR( test_deque_push_tail_wrap( deque, 23 )==deque );
  FD_EXPECT_LOG_ERR( test_deque_push_head_wrap( deque, 23 )==deque );
  FD_TEST( test_deque_leave ( deque   )==shdeque         );
  FD_TEST( test_deque_delete( shdeque )==(void *)scratch );
