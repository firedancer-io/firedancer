/* Generate prototypes, inlines and/or implementations for concurrent
   ...
   compilation unit:
     ...
     // a no-op.  Elements come from the free stack first (LIFO), then,
     // if POOL_LAZY, from the lazy region (ascending).  Regardless of
     // cnt this costs one CAS on the stack, plus one on the lazy region
     // if the stack ran short, plus one more on the stack if the lazy
     // region ran out mid call.  Acquired elements are independent: any
     // subset can be handed back with mypool_release.
     //
     // Running short is fatal: logs CRIT if the mypool was empty at some
     // point during the call while elements were still needed (the same
     // condition under which mypool_acquire returns NULL).  Size the
     // mypool such that this cannot happen.

     void mypool_acquire_batch( mypool_t * join, ulong cnt, myele_t * out[] );
...
POOL_STATIC void POOL_(acquire_batch)( POOL_(t) * join, ulong cnt, POOL_ELE_T * out[] );
...
# endif
  ...
  for(;;) {
    ...
    if( FD_UNLIKELY( lazy_empty ) ) {
      FD_LOG_CRIT(( "pool exhausted (requested %lu, acquired %lu, ele_max=%lu)", cnt, acq_cnt, ele_max ));
    }
