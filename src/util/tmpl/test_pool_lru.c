...
# endif
  ...
  FD_LOG_NOTICE(( "Testing LRU" ));

  for( ulong i=0UL; i<10000UL; i++ ) {
    ulong acquired0 = mypool_idx_acquire( pool );
    FD_TEST( mypool_idx_query_lru( pool )==acquired0 );
    ulong acquired1 = mypool_idx_acquire( pool );
    FD_TEST( mypool_idx_query_lru( pool )==acquired0 );
    ulong acquired2 = mypool_idx_acquire( pool );
    FD_TEST( mypool_idx_query_lru( pool )==acquired0 );
    ulong acquired3 = mypool_idx_acquire( pool );
    FD_TEST( mypool_idx_query_lru( pool )==acquired0 );

    mypool_idx_release( pool, acquired0 );
    FD_TEST( mypool_idx_query_lru( pool )==acquired1 );

    mypool_idx_release( pool, acquired2 );
    FD_TEST( mypool_idx_query_lru( pool )==acquired1 );

    mypool_idx_release( pool, acquired1 );
    FD_TEST( mypool_idx_query_lru( pool )==acquired3 );
    mypool_idx_release( pool, acquired3 );
  }
