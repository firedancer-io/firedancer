...
DEQUE_(delete)( void * shdeque ) {
  ...
  if( FD_UNLIKELY( !fd_ulong_is_aligned( (ulong)shdeque, DEQUE_(align)() ) ) ) FD_LOG_ERR(( "unaligned shmem" ));
#endif
  return shdeque;
}
...
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( max1==ULONG_MAX ) ) FD_LOG_ERR(( "cannot push_head_wrap when max is zero" ));
#endif
...
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( max1==ULONG_MAX ) ) FD_LOG_ERR(( "cannot push_tail_wrap when max is zero" ));
#endif
...
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( DEQUE_(empty)( deque ) ) ) FD_LOG_ERR(( "cannot peek on empty deque" ));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
...
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( DEQUE_(full)( deque ) ) ) FD_LOG_ERR(( "cannot insert into full deque" ));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  ulong max1  = hdr->max1;
  ulong cnt   = hdr->cnt;
...
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( DEQUE_(empty)( deque ) ) ) FD_LOG_ERR(( "cannot remove from empty deque" ));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  ulong max1  = hdr->max1;
  ulong cnt   = hdr->cnt;
...
DEQUE_(iter_ele)( DEQUE_T * deque, DEQUE_(iter_t) iter ) {
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( (iter.rem==0) | (iter.rem>hdr->cnt) ) ) FD_LOG_ERR(( "iter out of bounds" ));
#endif
