...
DEQUE_(delete)( void * shdeque ) {
  ...
  FD_DCHECK_CRIT( fd_ulong_is_aligned( (ulong)shdeque, DEQUE_(align)() ), "unaligned shmem" );
  return shdeque;
}
...
DEQUE_(push_head_wrap)( DEQUE_T * deque,
  ...
  FD_DCHECK_CRIT( max1!=ULONG_MAX, "cannot push_head_wrap when max is zero" );
...
DEQUE_(push_tail_wrap)( DEQUE_T * deque,
  ...
  FD_DCHECK_CRIT( max1!=ULONG_MAX, "cannot push_tail_wrap when max is zero" );
...
DEQUE_(peek_head)( DEQUE_T * deque ) {
  FD_DCHECK_CRIT( !DEQUE_(empty)( deque ), "cannot peek on empty deque" );
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
...
DEQUE_(insert_head)( DEQUE_T * deque ) {
  FD_DCHECK_CRIT( !DEQUE_(full)( deque ), "cannot insert into full deque" );
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  ulong max1  = hdr->max1;
  ulong cnt   = hdr->cnt;
  ulong start = hdr->start;
...
DEQUE_(remove_head)( DEQUE_T * deque ) {
  FD_DCHECK_CRIT( !DEQUE_(empty)( deque ), "cannot remove from empty deque" );
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  ulong max1  = hdr->max1;
  ulong cnt   = hdr->cnt;
  ulong start = hdr->start;
...
DEQUE_(iter_ele)( DEQUE_T * deque, DEQUE_(iter_t) iter ) {
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  FD_DCHECK_CRIT( (iter.rem>0) & (iter.rem<=hdr->cnt), "iter out of bounds" );
