...
PRQ_(new)( void * mem,
           ulong  max ) {
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( !mem | !fd_ulong_is_aligned( (ulong)mem, PRQ_(align)() ) ) ) FD_LOG_ERR(( "invalid mem" ));
#endif
...
PRQ_(insert)( PRQ_T *       heap,
  ...
  ulong hole = prq->cnt++;
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( hole==prq->max ) ) FD_LOG_ERR(( "prq full" ));
#endif
...
PRQ_(remove_min)( PRQ_T * heap ) {
  PRQ_(private_t) * prq = PRQ_(private_from_heap)( heap );
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( prq->cnt==0UL ) ) FD_LOG_ERR(( "prq empty" ));
#endif
  ulong cnt = --prq->cnt;
