...
#define FD_EXPECT_LOG_ERR( CALL ) do {                             \
  ...
  FD_EXPECT_LOG_ERR( map_new( (void*)((char*)mem+1), 5 ) );
  FD_EXPECT_LOG_ERR( map_new( mem,                  -1 ) );
  FD_EXPECT_LOG_ERR( map_new( mem,                  64 ) );

  map = map_new( mem, LG_SLOT_CNT ); FD_TEST( map );
  FD_EXPECT_LOG_ERR( map_insert  ( map, map_key_null() ) );
  FD_EXPECT_LOG_ERR( map_query   ( map, map_key_null(), 0UL ) );
#if MEMOIZE
