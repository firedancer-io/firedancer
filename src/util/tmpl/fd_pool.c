...
POOL_(idx)( POOL_T const * join,
            POOL_T const * ele ) {
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( !POOL_(ele_test)( join, ele ) ) ) FD_LOG_ERR(( "no such element" ));
#endif
...
POOL_(ele)( POOL_T *   join,
            ulong      idx ) {
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( !POOL_(idx_test)( join, idx ) ) ) FD_LOG_ERR(( "no such index" ));
#endif
...
POOL_(idx_acquire)( POOL_T * join ) {
  POOL_(private_t) * meta = POOL_(private_meta)( join );
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( !meta->free ) ) FD_LOG_ERR(( "pool is full" ));
#endif
...
POOL_(idx_release)( POOL_T * join,
  ...
  POOL_(private_t) * meta = POOL_(private_meta)( join );
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( (meta->max<=idx) | (idx==POOL_IDX_NULL) ) ) FD_LOG_ERR(( "invalid index" ));
#if POOL_SENTINEL
  if( FD_UNLIKELY( POOL_(idx_sentinel)( join )==idx ) ) FD_LOG_ERR(( "cannot releaes sentinel" ));
  if( FD_UNLIKELY( meta->free>=meta->max-1 ) ) FD_LOG_ERR(( "pool is empty" ));
#else
  if( FD_UNLIKELY( meta->free>=meta->max ) ) FD_LOG_ERR(( "pool is empty" ));
#endif
#endif
