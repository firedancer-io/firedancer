...
/* Enable handholding checks to catch **some** contract violations by
   the caller.  Note that due to little function local information (like
   ele_cnt) available handholding for dlists detects a relatively small
...
DLIST_(idx_peek_head)( DLIST_(t) const *   join,
  ...
  (void)pool;
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( DLIST_(is_empty)( join, pool ) ) ) FD_LOG_ERR(( "cannot peek on empty dlist" ));
#endif
...
DLIST_(idx_pop_head)( DLIST_(t) *   join,
                      DLIST_ELE_T * pool ) {
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( DLIST_(is_empty)( join, pool ) ) ) FD_LOG_ERR(( "cannot pop from empty dlist" ));
#endif
  DLIST_(private_t) * dlist = DLIST_(private)( join );
