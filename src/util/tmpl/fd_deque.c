...
DEQUE_(push_head)( DEQUE_T * deque,
  ...
  if ( DEQUE_(avail)( deque ) == 0 ) FD_LOG_CRIT(("push_head: deque is full"));
#endif
...
DEQUE_(push_tail)( DEQUE_T * deque,
                   DEQUE_T   ele ) {
#if FD_TMPL_USE_HANDHOLDING
  if ( DEQUE_(avail)( deque ) == 0 ) FD_LOG_CRIT(("push_tail: deque is full"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
...
#if FD_TMPL_USE_HANDHOLDING
  if ( DEQUE_(empty)( deque ) ) FD_LOG_CRIT(("push_head: deque is empty"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
...
#if FD_TMPL_USE_HANDHOLDING
  if ( DEQUE_(empty)( deque ) ) FD_LOG_CRIT(("pop_tail: deque is empty"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  hdr->end--;
...
DEQUE_(pop_idx_tail)( DEQUE_T * deque, ulong idx ) {
#if FD_TMPL_USE_HANDHOLDING
  if ( DEQUE_(empty)( deque ) ) FD_LOG_CRIT(("pop_idx_tail: deque is empty"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  ...
  for(;;) {
    idx++;
    if( hdr->start + idx >= hdr->end ) break;
    DEQUE_T * next = &hdr->deque[ DEQUE_(private_slot)( hdr->start + idx ) ];
    *cur = *next;
     cur =  next;
  }
  hdr->end--;
...
#if FD_TMPL_USE_HANDHOLDING
  if ( idx < DEQUE_(cnt)( deque ) ) FD_LOG_CRIT(("peek_index: %lu is not in deque range (%lu)", idx, DEQUE_(cnt)( deque )));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
...
#if FD_TMPL_USE_HANDHOLDING
  if ( idx < DEQUE_(cnt)( deque ) ) FD_LOG_CRIT(("peek_index_const: %lu is not in deque range (%lu)", idx, DEQUE_(cnt)( deque )));
#endif
  DEQUE_(private_t) const * hdr = DEQUE_(private_const_hdr_from_deque)( deque );
  return hdr->deque + DEQUE_(private_slot)( hdr->start + idx );
}
...
#if FD_TMPL_USE_HANDHOLDING
  if ( DEQUE_(avail)( deque ) == 0 ) FD_LOG_CRIT(("push_head_nocopy: deque is full"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  hdr->start--;
  return &hdr->deque[ DEQUE_(private_slot)( hdr->start ) ];
...
#if FD_TMPL_USE_HANDHOLDING
  if ( DEQUE_(avail)( deque ) == 0 ) FD_LOG_CRIT(("push_tail_nocopy: deque is full"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
...
#if FD_TMPL_USE_HANDHOLDING
  if ( DEQUE_(empty)( deque ) ) FD_LOG_CRIT(("pop_head_nocopy: deque is empty"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  DEQUE_T * ele = &hdr->deque[ DEQUE_(private_slot)( hdr->start ) ];
  hdr->start++;
...
#if FD_TMPL_USE_HANDHOLDING
  if ( DEQUE_(empty)( deque ) ) FD_LOG_CRIT(("pop_tail_nocopy: deque is empty"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  hdr->end--;
  return &hdr->deque[ DEQUE_(private_slot)( hdr->end ) ];
