...
DEQUE_(push_head)( DEQUE_T * deque,
  ...
  if ( FD_UNLIKELY( DEQUE_(avail)( deque ) == 0 ) )
    FD_LOG_CRIT(("push_head: deque is full"));
#endif
...
#if FD_TMPL_USE_HANDHOLDING
  if ( FD_UNLIKELY( DEQUE_(avail)( deque ) == 0 ) )
    FD_LOG_CRIT(("push_tail: deque is full"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
...
DEQUE_(pop_head)( DEQUE_T * deque ) {
  ...
  if ( FD_UNLIKELY( DEQUE_(empty)( deque ) ) )
    FD_LOG_CRIT(("push_head: deque is empty"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
...
#if FD_TMPL_USE_HANDHOLDING
  if ( FD_UNLIKELY( DEQUE_(empty)( deque ) ) )
    FD_LOG_CRIT(("pop_tail: deque is empty"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  hdr->end--;
...
#if FD_TMPL_USE_HANDHOLDING
  if ( FD_UNLIKELY( DEQUE_(empty)( deque ) ) )
    FD_LOG_CRIT(("pop_idx_tail: deque is empty"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
...
DEQUE_(peek_index)( DEQUE_T * deque, ulong idx ) {
  ...
  if ( FD_UNLIKELY( idx > DEQUE_(cnt)( deque ) ) )
    FD_LOG_CRIT(("peek_index: %lu is not in deque range (%lu)", idx, DEQUE_(cnt)( deque )));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
...
#if FD_TMPL_USE_HANDHOLDING
  if ( FD_UNLIKELY( idx > DEQUE_(cnt)( deque ) ) )
    FD_LOG_CRIT(("peek_index_const: %lu is not in deque range (%lu)", idx, DEQUE_(cnt)( deque )));
#endif
  DEQUE_(private_t) const * hdr = DEQUE_(private_const_hdr_from_deque)( deque );
  return hdr->deque + DEQUE_(private_slot)( hdr->start + idx );
}
...
#if FD_TMPL_USE_HANDHOLDING
  if ( FD_UNLIKELY( DEQUE_(avail)( deque ) == 0 ) )
    FD_LOG_CRIT(("push_head_nocopy: deque is full"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  hdr->start--;
  return &hdr->deque[ DEQUE_(private_slot)( hdr->start ) ];
...
#if FD_TMPL_USE_HANDHOLDING
  if ( FD_UNLIKELY( DEQUE_(avail)( deque ) == 0 ) )
    FD_LOG_CRIT(("push_tail_nocopy: deque is full"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
...
#if FD_TMPL_USE_HANDHOLDING
  if ( FD_UNLIKELY( DEQUE_(empty)( deque ) ) )
    FD_LOG_CRIT(("pop_head_nocopy: deque is empty"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  DEQUE_T * ele = &hdr->deque[ DEQUE_(private_slot)( hdr->start ) ];
  hdr->start++;
...
#if FD_TMPL_USE_HANDHOLDING
  if ( FD_UNLIKELY( DEQUE_(empty)( deque ) ) )
    FD_LOG_CRIT(("pop_tail_nocopy: deque is empty"));
#endif
  DEQUE_(private_t) * hdr = DEQUE_(private_hdr_from_deque)( deque );
  hdr->end--;
  return &hdr->deque[ DEQUE_(private_slot)( hdr->end ) ];
