...
SET_(new)( void * shmem,
           ulong  max ) {
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( !fd_ulong_is_aligned( (ulong)shmem, SET_(align)() ) ) ) FD_LOG_ERR(( "unaligned shmem" ));
  if( FD_UNLIKELY( (max<1UL) | (max>ULONG_MAX-63UL) ) ) FD_LOG_ERR(( "max out of bounds" ));
#endif
...
SET_(insert)( SET_(t) * set,
              ulong     idx ) {
#if FD_TMPL_USE_HANDHOLDING
  if( FD_UNLIKELY( !SET_(valid_idx)( set, idx ) ) ) FD_LOG_ERR(( "idx out of bounds" ));
#endif
