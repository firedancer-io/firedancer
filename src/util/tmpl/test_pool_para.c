...
batch( mypool_t * pool,
       ...
       myele_t ** out ) {
  out[ cnt ] = canary;
  mypool_acquire_batch( pool, cnt, out );
...
refill_tile_main( int     argc,
  ...
  (void)argc; (void)argv;
  refill_out[ 2 ] = canary;
  mypool_acquire_batch( refill_pool, 2UL, refill_out );
  FD_TEST( refill_out[ 2 ]==canary );
  return 0;
}
...
tile_main( int     argc,
  ...
  for( ulong iter_idx=0UL; iter_idx<iter_cnt; iter_idx++ ) {
    ...
    case 2: { /* acquire batch */
      ...
      if( FD_LIKELY( n ) ) {
        mypool_acquire_batch( pool, n, acq_ele+acq_cnt );
