...
inline ulong fd_tar_stream_footprint( void ) { return sizeof(struct fd_tar_stream); }

/*
  Initialize a fd_tar_stream data structure
*/
extern void fd_tar_stream_init(struct fd_tar_stream* self, fd_valloc_t valloc);

typedef void (*fd_tar_stream_callback)(void* arg, const char* name, void const * data, ulong datalen);

/*
  Process more tarball data. The callback is invoked on every complete file. Returns non-zero on end of tarball.
*/
extern int fd_tar_stream_moreData(struct fd_tar_stream* self, void const * data, ulong datalen, fd_tar_stream_callback cb, void* arg);
