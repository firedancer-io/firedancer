...
static void unshare_user( void ) {
  ...
  FD_TESTV( overflow_uid != 0 && overflow_gid != 0 );

  FD_TESTV( 0 == setresgid( overflow_gid, overflow_gid, overflow_gid ) );
  FD_TESTV( 0 == setresuid( overflow_uid, overflow_uid, overflow_uid ) );
...
void
fd_sandbox_private_unthreaded( void ) {
  unshare_user();
  setup_mountns();
  drop_capabilities();
...
void
fd_sandbox( uint close_fd_start,
            ushort allow_syscalls_sz,
            long * allow_syscalls ) {
