/* fd_util_libbpf: Support routines for libbpf.  Configures fd_log for
   libbpf and fixes error handling. */

#ifndef HEADER_fd_src_util_fd_util_h
#error "fd_util_libbpf.c must be included from fd_util.c"
#endif
...
fd_libbpf_print( enum libbpf_print_level level,
  ...
  if( FD_LIKELY( newline && newline[1]=='\0' ) )
    newline[0] = '\0';
