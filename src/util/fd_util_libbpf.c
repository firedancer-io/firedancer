...
/* fd_libbpf_private_boot: Configures the libbpf library including logging and
   error handling. Must be called on boot if this thread group calls
   any libbpf function during its lifetime. */
static void
fd_libbpf_private_boot( void ) {
  if( FD_UNLIKELY( 0!=libbpf_set_strict_mode( LIBBPF_STRICT_DIRECT_ERRS | LIBBPF_STRICT_CLEAN_PTRS ) ) )
    FD_LOG_ERR(( "libbpf_set_strict_mode(LIBBPF_STRICT_DIRECT_ERRS|LIBBPF_STRICT_CLEAN_PTRS) failed" ));
