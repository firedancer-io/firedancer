...
void fd_memusage_printout( const char * filename ) {
  ...
  fd_shmem_join_info_t * map = fd_shmem_private_map;
  fd_shmem_join_info_t * ptrs[FD_SHMEM_PRIVATE_MAP_SLOT_CNT];
  ulong ptrs_cnt = 0UL;
  for( ulong slot_idx=0UL; slot_idx<FD_SHMEM_PRIVATE_MAP_SLOT_CNT; slot_idx++ ) {
    ulong j0 = (ulong)map[slot_idx].shmem;
    if (!j0) continue;
    ptrs[ptrs_cnt++] = &map[slot_idx];
  }
  ...
  for( ulong ptr_idx=0UL; ptr_idx<ptrs_cnt; ptr_idx++ ) {
    fd_shmem_join_info_t * ptr = ptrs[ptr_idx];
