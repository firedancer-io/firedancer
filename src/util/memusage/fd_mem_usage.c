...
fd_mem_usage_get_handle( void const * mem, ulong usage, const char * descript, ... ) {
  ...
  if( fd_mem_usage_private_free != NULL ) {
    p = fd_mem_usage_private_free;
    fd_mem_usage_private_free = p->next;
  } else if( fd_mem_usage_pool_used < FD_MEM_USAGE_POOL_SIZE ) {
    p = &fd_mem_usage_pool[fd_mem_usage_pool_used++];
  } else {
    FD_LOG_CRIT(( "fd_mem_usage_get_handle: out of static memory" ));
    return NULL;
  }
...
fd_mem_usage_free_handle( fd_mem_usage_handle_t handle ) {
  ...
  p->next = fd_mem_usage_private_free;
  fd_mem_usage_private_free = p;
