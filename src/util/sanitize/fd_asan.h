...
/* If FD_HAS_ASAN is set:
   ...
   If FD_HAS_DEEPCLEAN is not set fd_asan_{poison,unpoison} just return addr,
   ...
   In order to satisfy this with exisiting allocators, when FD_HAS_DEEPCLEAN 
   ...
   behavior in fd_scratch with FD_HAS_DEEPCLEAN will not change the number of
