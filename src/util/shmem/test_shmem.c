...
main( int     argc,
    ...
    for( int i=0; i<65536; i++ ) {
      ...
      if( op ) { /* join */
        ...
        if( !ref_info[ idx ].ref_cnt ) { /* this join needs to map it */
          ...
          void * join = fd_shmem_join( name, mode, 0, NULL, NULL, NULL );
          FD_TEST( join );
          ...
          FD_TEST( fd_shmem_join( name, mode, 0, NULL, NULL, NULL )==join );
  ...
  for( ulong test_idx=0UL; test_idx<test_cnt; test_idx++ ) {
    ...
    memset( nop, fd_rng_int( rng ), sizeof(fd_shmem_join_info_t) ); *info = *nop;
    FD_TEST( fd_shmem_join( name, fd_rng_int( rng ) & 1, 0, NULL, NULL, info )==join ); ref->ref_cnt++;
    FD_TEST( !memcmp( info, ref, sizeof(fd_shmem_join_info_t) ) );
