...
fd_shmem_join_anonymous( char const * name,
  ...
  if( FD_UNLIKELY( join_info ) ) {
    FD_SHMEM_UNLOCK;
    FD_LOG_WARNING(( "(%s,%lx,%lx) memory already mapped as (%s,%lx,%lx)", name, a0, a1, join_info->name, (ulong)join_info->shmem, (ulong)join_info->shmem + join_info->page_cnt*join_info->page_sz ));
    return EINVAL;
  }
