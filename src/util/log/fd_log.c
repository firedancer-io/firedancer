...
fd_log_private_open_path( int          cmdline,
  ...
  if( fd_log_private_path[0]=='\0' ) {
    ...
    if( log_fileno==-1 ) {
      fd_log_private_fprintf_0( STDERR_FILENO, "uid %u %u gid %u %u", getuid(), geteuid(), getgid(), getegid() );
      fd_log_private_fprintf_0( STDERR_FILENO, "fopen failed (--log-path \"%s\") %d-%s; unable to boot\n", fd_log_private_path, errno, fd_io_strerror( errno ) );
      exit(1);
    }
    fd_log_private_fprintf_0( STDERR_FILENO, "Log at \"%s\"\n", fd_log_private_path );
  }
