...
main( int     argc,
  ...
  do {
    ...
    len=1UL; FD_TEST( fd_cstr_printf_check( NULL,      128UL, &len, "dummy" )==0         && len==0UL ); /* NULL cstr */
    len=1UL; FD_TEST( fd_cstr_printf      ( (void *)1,   0UL, &len, "dummy" )==(void *)1 && len==0UL ); /* zero sz */
    len=1UL; FD_TEST( fd_cstr_printf_check( (void *)1,   0UL, &len, "dummy" )==0         && len==0UL ); /* zero sz */
  } while(0);
...
#   endif
  ...
  for( ulong i=0UL, off=0UL; i<4; i++ ) {
    ulong frag_sz = strlen( ref_uint128_frag[i] );
    FD_TEST( off+frag_sz+1<=sizeof(ref_uint128) );
    fd_memcpy( ref_uint128+off, ref_uint128_frag[ i ], frag_sz+1 );
    off += frag_sz;
  }
...
# undef TEST_APPEND
  ...
  for( ulong i=0UL; i<=0x7f; i++ ) {
    char buf[ 2 ] = { 'H','H' };
    fd_cstr_fini( fd_cstr_append_utf8( fd_cstr_init( buf ), (uint)i ) );
    FD_TEST( buf[0]==(char)i && buf[1]=='\0' );
  }
  do {
    char buf[ 5 ];
#   define UTF8_TEST( rune, expected ) do {                               \
      fd_cstr_fini( fd_cstr_append_utf8( fd_cstr_init( buf ), (rune) ) ); \
      FD_TEST( strnlen( buf, sizeof(buf) )==sizeof(expected)-1 );         \
      FD_TEST( 0==memcmp( buf, (expected), sizeof(expected)-1 ) );        \
      FD_LOG_NOTICE(( "%s", buf )); \
    } while(0)

    UTF8_TEST(   0x16c9, "\xe1\x9b\x89"     );
    UTF8_TEST(     0x7f, "\x7f"             );
