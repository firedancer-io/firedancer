...
struct fd_quic_stream {
  ...
  uint64_t stream_id;                    /* all 1's indicates an unused stream object */
...
# define FD_QUIC_STREAM_FLAGS_MAX_STREAM_DATA (1u<<2u)
  ...
  /* tx_buf is a circular queue
     invariants
       ...
       */
  uint16_t tx_head; /* first unused byte of tx_buf */
  uint16_t tx_tail; /* first unacked (used) byte of tx_buf */
  uint16_t tx_sent; /* first unsent byte of tx_buf */
  uint64_t tx_offs; /* the offset of the (future) byte at tx_head */
  ...
  /* flow control */
  size_t   tx_max_stream_data; /* the limit on the number of bytes we are allowed to send
                                  to the peer on this stream
                                  this includes bytes implied by offsets that have not
                                  been received yet */
  size_t   tx_tot_data;        /* the total number of bytes transmitted on this stream */

  size_t   rx_max_stream_data; /* the limit on the number of bytes we allow the peer to 
                                  send to us */
  size_t   rx_tot_data;        /* the total number of bytes received on this stream */
