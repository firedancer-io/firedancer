...
fd_quic_handle_v1_initial( fd_quic_t *               quic,
  ...
  if( FD_UNLIKELY( !conn ) ) {
    if( quic->config.role==FD_QUIC_ROLE_SERVER ) {
      ...
      uint   arp_ifindex      = 0;
      uint   arp_host_dst_ip  = ntohl( dst_ip_addr );
...
fd_quic_send_arp( fd_quic_t *      quic,
  ...
  fd_ip_update_arp_table( ip, next_hop_ip_addr, ifindex );

  /* need ip_addr in host order */
  uint host_ip_addr = htonl( quic->config.net.ip_addr );
    ...
    if( FD_LIKELY( arp_rtn == FD_IP_SUCCESS ) ) {
      ...
      switch( arp_rtn ) {
        case FD_IP_PROBE_RQD:
          /* send ARP */
          fd_quic_send_arp( quic, arp_next_ip_addr, arp_ifindex );
        ...
        case FD_IP_NO_ROUTE:
          ...
          fd_quic_reschedule_conn( conn, (ulong)1e9 );
          return;
        default:
