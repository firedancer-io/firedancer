...
fd_quic_new( void * mem, fd_quic_config_t const * config ) {
  ...
  /* configure AIO */
  void *     shaio = fd_aio_new( &quic->aio_net_in, (void*)quic, fd_quic_aio_cb_receive );
  fd_aio_t * aio   = fd_aio_join( shaio );
  /* TODO we're assuming aio returned here is same as shaio
     if not this will break */

  if( FD_UNLIKELY( !aio ) ) {
    FD_LOG_ERR(( "Unable to create aio" ));
  }
...
fd_quic_delete( fd_quic_t * quic ) {
  ...
  fd_aio_leave ( &quic->aio_net_in );
  fd_aio_delete( &quic->aio_net_in );
}
...
fd_quic_tx_buffered( fd_quic_t * quic, fd_quic_conn_t * conn ) {
  ...
  fd_aio_pkt_info_t aio_buf = { conn->crypt_scratch, .buf_sz = (ushort)( cur_ptr - conn->crypt_scratch ) };
  ulong batch_idx = (ulong)-1;
  int aio_rc = fd_aio_send( quic->aio_net_out, &aio_buf, 1, &batch_idx );
