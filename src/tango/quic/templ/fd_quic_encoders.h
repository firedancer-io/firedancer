...
#define FD_TEMPL_ENCODE_IMPL_uint8(p,val) (                        \
    ( (p)[0] = (uchar)( (val) ) ) )
#define FD_TEMPL_ENCODE_IMPL_uint16(p,val) (                       \
    ( (p)[0] = (uchar)( (uint16_t)(val) >> (uint16_t)0x08 ) ),     \
    ( (p)[1] = (uchar)( (uint16_t)(val) >> (uint16_t)0x00 ) )  )
#define FD_TEMPL_ENCODE_IMPL_uint32(p,val) (                       \
    ( (p)[0] = (uchar)( (uint32_t)(val) >> (uint32_t)0x18 ) ) ,    \
    ( (p)[1] = (uchar)( (uint32_t)(val) >> (uint32_t)0x10 ) ) ,    \
    ( (p)[2] = (uchar)( (uint32_t)(val) >> (uint32_t)0x08 ) ) ,    \
    ( (p)[3] = (uchar)( (uint32_t)(val) >> (uint32_t)0x00 ) ) )
#define FD_TEMPL_ENCODE_IMPL_uint64(p,val) (                       \
    ( (p)[0] = (uchar)( (uint64_t)(val) >> (uint64_t)0x38 ) ) ,    \
    ( (p)[1] = (uchar)( (uint64_t)(val) >> (uint64_t)0x30 ) ) ,    \
    ( (p)[2] = (uchar)( (uint64_t)(val) >> (uint64_t)0x28 ) ) ,    \
    ( (p)[3] = (uchar)( (uint64_t)(val) >> (uint64_t)0x20 ) ) ,    \
    ( (p)[4] = (uchar)( (uint64_t)(val) >> (uint64_t)0x18 ) ) ,    \
    ( (p)[5] = (uchar)( (uint64_t)(val) >> (uint64_t)0x10 ) ) ,    \
    ( (p)[6] = (uchar)( (uint64_t)(val) >> (uint64_t)0x08 ) ) ,    \
    ( (p)[7] = (uchar)( (uint64_t)(val) >> (uint64_t)0x00 ) ) )
...
#define FD_TEMPL_MBR_ELEM(NAME,TYPE)                                        \
    buf += (cur_bit != 0);                                                  \
    cur_bit = 0;                                                            \
    if( buf + sizeof(fd_quic_##TYPE) > buf_end ) return FD_QUIC_PARSE_FAIL; \
...
#define FD_TEMPL_MBR_ELEM_VARINT(NAME,TYPE)                              \
    do{                                                                  \
      ...
      size_t tmp_sz = (size_t)( buf_end - buf );                         \
...
#define FD_TEMPL_MBR_ELEM_BITS(NAME,TYPE,BITS)                           \
    if( BITS + cur_bit > (size_t)( ( buf_end - buf ) * 8 ) ) {           \
      return FD_QUIC_PARSE_FAIL;                                         \
    }                                                                    \
...
#define FD_TEMPL_MBR_ELEM_VAR(NAME,BITS_MIN,BITS_MAX,LEN_NAME)           \
    ...
    if( tmp_len * 8 < BITS_MIN || tmp_len * 8 > BITS_MAX ) {             \
      FD_DEBUG( "buffer overflow encoding variable length field."        \
            "  field: " #NAME                                            \
            "  BITS_MIN: %lu"                                            \
            "  BITS_MAX: %lu"                                            \
            "  " #LEN_NAME ": %lu"                                       \
            ...
            (unsigned long)frame->LEN_NAME,                              \
            (unsigned long)( tmp_len * 8 ) );                            \
      return FD_QUIC_PARSE_FAIL;                                         \
    }                                                                    \
    memcpy( buf, frame->NAME, tmp_len );                                 \
    buf += tmp_len;
...
#define FD_TEMPL_MBR_ELEM_FIXED(NAME,TYPE,BYTES)                         \
    buf += (cur_bit != 0);                                               \
    cur_bit = 0;                                                         \
    if( BYTES > buf_end - buf ) return FD_QUIC_PARSE_FAIL;               \
    if( BYTES % sizeof( fd_quic_##TYPE ) ) return FD_QUIC_PARSE_FAIL;    \
