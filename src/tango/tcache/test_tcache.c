...
# define TEST(c) do if( FD_UNLIKELY( !(c) ) ) { FD_LOG_WARNING(( "FAIL: " #c )); return 1; } while(0)
  ...
  TEST( fd_tcache_align()==FD_TCACHE_ALIGN );
  TEST( !fd_tcache_footprint( ULONG_MAX, 4UL ) );
  TEST( !fd_tcache_footprint( 1UL, ULONG_MAX ) );
  TEST( fd_tcache_map_cnt_default( 0UL )== 0UL );
  TEST( fd_tcache_map_cnt_default( 1UL )== 8UL );
  TEST( fd_tcache_map_cnt_default( 2UL )== 8UL );
  TEST( fd_tcache_map_cnt_default( 3UL )==16UL );
  TEST( fd_tcache_map_cnt_default( 6UL )==16UL );
  TEST( fd_tcache_map_cnt_default( 7UL )==32UL );
  for( ulong rem=1000000UL; rem; rem-- ) {
    ...
    if( (!depth) || map_cnt<(depth+2UL) || !fd_ulong_is_pow2( map_cnt ) ) TEST( !footprint );
    else TEST( footprint==FD_TCACHE_FOOTPRINT( depth, map_cnt ) );
  }
  ...
  void *        mem     = fd_wksp_alloc_laddr( wksp, align, footprint ); TEST( mem );
  void *        _tcache = fd_tcache_new( mem, depth, map_cnt );          TEST( _tcache );
  fd_tcache_t * tcache  = fd_tcache_join( _tcache );                     TEST( tcache );
  ...
  TEST( fd_tcache_depth  ( tcache )==depth   );
  TEST( fd_tcache_map_cnt( tcache )==map_cnt );
  ulong * _oldest = fd_tcache_oldest_laddr( tcache ); TEST( _oldest );
  ulong * ring    = fd_tcache_ring_laddr  ( tcache ); TEST( ring    );
  ulong * map     = fd_tcache_map_laddr   ( tcache ); TEST( map     );
  ulong   oldest  = _oldest[0];                       TEST( !oldest );

  TEST( fd_tcache_tag_is_null( FD_TCACHE_TAG_NULL ) );
  TEST( fd_tcache_tag_is_null( ring[ oldest ]     ) );
  ...
  for( ulong seq=0UL; seq<depth; seq++ ) {
    ...
    TEST( !found );
    TEST( map_idx<map_cnt );
    TEST( fd_tcache_tag_is_null( map[ map_idx ] ) );
    ...
    TEST( found2 );
    TEST( map_idx2==map_idx );
    TEST( map[ map_idx ]==tag );
  }
  ...
  for( ulong seq=0UL; seq<depth; seq++ ) {
    ...
    FD_TCACHE_QUERY( found2, map_idx2, map, map_cnt, tag );
    TEST( !found2 );
    TEST( map_idx2<map_cnt ); /* remove might have shuffled map so no guarantee map_idx2 will match */
    TEST( fd_tcache_tag_is_null( map[ map_idx2 ] ) );
  }
  ...
  TEST( !fd_tcache_reset( ring, depth, map, map_cnt ) );
  ...
  oldest = fd_tcache_reset( ring, depth, map, map_cnt );      TEST( !oldest );
  ...
  ulong * bench_tag = (ulong *)fd_wksp_alloc_laddr( wksp, 0UL, bench_cnt*sizeof(ulong) ); TEST( bench_tag );
  ...
  TEST( fd_tcache_leave ( tcache  )==_tcache );
  TEST( fd_tcache_delete( _tcache )==mem     );
