...
fd_tcache_footprint( ulong depth,
  ...
  if( FD_UNLIKELY( (!depth) | (map_cnt<(depth+2UL)) | (!fd_ulong_is_pow2( map_cnt )) ) ) {
    FD_LOG_WARNING((" bad depth (%lu) and/or map_cnt (%lu)", depth, map_cnt ));
    return 0UL; /* Invalid depth / max_cnt */
  }
  ...
  if( FD_UNLIKELY( cnt>(ULONG_MAX/sizeof(ulong)) ) ) {
    FD_LOG_WARNING((" footprint too large (%lu)", cnt*sizeof(ulong) ));
    return 0UL; /* overflow */
  }
  cnt *= sizeof(ulong); /* no overflow */
  ulong footprint = fd_ulong_align_up( cnt, FD_TCACHE_ALIGN ); if( FD_UNLIKELY( footprint<cnt ) ) {
    FD_LOG_WARNING((" footprint too large (%lu)", cnt ));
    return 0UL; /* overflow */
  }
  return footprint;
}
