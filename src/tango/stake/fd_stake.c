...
fd_stake_footprint( ulong lg_slot_cnt ) {
  ...
  l = FD_LAYOUT_APPEND(
      l, fd_stake_staked_node_align(), fd_stake_staked_node_footprint( (int)lg_slot_cnt ) );
...
fd_stake_new( void * mem, ulong lg_slot_cnt ) {
  ...
  //         l, fd_stake_staked_node_align(), fd_stake_staked_node_footprint( (int)lg_slot_cnt ) ),
  //     (int)lg_slot_cnt );
  void * _staked_nodes = FD_SCRATCH_ALLOC_APPEND( l, fd_stake_staked_node_align(), fd_stake_staked_node_footprint( (int)lg_slot_cnt ) );
  // FD_LOG_NOTICE(("staked_nodes %p", _staked_nodes));
  // FD_LOG_NOTICE(("lg_slot_cnt %lu", lg_slot_cnt));
  stake->staked_nodes = fd_stake_staked_node_join(fd_stake_staked_node_new( _staked_nodes, (int)lg_slot_cnt ));
