...
fd_ip_arp_gen_arp_probe( uchar *   buf,
  ...
  arp->op             = 1;                     /* operation - request is 1 */

  fd_memcpy( arp->sender_hw_addr,    src_mac_addr, 6 ); /* sender hardware address */
  fd_memcpy( arp->sender_proto_addr, &net_ip_addr, 4 ); /* sender protocol (IPv4) address */

  fd_memset( arp->target_hw_addr,    0, 6 ); /* target hardware address - ignored for request */
  fd_memset( arp->target_proto_addr, 0, 4 ); /* target protocol (IPv4) address - ignored for request */
