/* TODO pare these down to only necessary */
#include <sys/types.h>
...
#define FD_LOG_ERR(x)     FD_LOG(x)
#define FD_LOG_WARNING(x) FD_LOG(x)
#define FD_LOG_NOTICE(x)  FD_LOG(x)
...
fd_nl_load_route_table( fd_nl_t *             nl,
  ...
  long sent = send( fd, &nl_request, sizeof(nl_request), 0 );
  if( sent == -1 ) {
  ...
  uchar __attribute__(( aligned(16) )) ibuf[4096]   = {0};
  ...
  while(1) {
    do {
      len = recv( fd, ibuf, sizeof(ibuf), 0 );
  ...
  while( NLMSG_OK(h, msglen) ) {
    ...
    if( h->nlmsg_type == NLMSG_ERROR ) {
      FD_LOG_WARNING(( "netlink returned data with error" ));
...
fd_nl_load_arp_table( fd_nl_t *           nl,
  ...
  //nl_request.ndm.ndm_state   = NUD_REACHABLE;
  ...
  while(1) {
    //uchar *     p     = ibuf;
    //uchar const p_end = ibuf + sizeof(ibuf);
    do {
      len = recv( fd, ibuf, sizeof(ibuf), 0 );
    } while( len < 0 && (errno == EINTR || errno == EAGAIN) );
  ...
  while( NLMSG_OK(h, msglen) ) {
    ...
    /* TODO can you spuriously get data from another process?? */
    //if( h->nlmsg_pid != our_pid ) {
    //  h = NLMSG_NEXT( h, msglen );
    //  continue;
    //}
