...
fd_xdp_new( void * mem, char const * intf, fd_xdp_config_t const * config ) {
  ...
  /* keep copy of config */
  fd_memcpy( &self.config, config, sizeof(*config) );
  ...
  self.umem.chunk_size = (uint)self.config.frame_size;
  self.umem.len        = self.config.frame_memory_size;
  self.umem.addr       = 0; // set later

  if( self.config.frame_memory == NULL ) {
    /* allocate aligned memory */
    posix_memalign( &self.owned_mem, (ulong)sysconf( _SC_PAGESIZE ), self.umem.len );
    if( !self.owned_mem ) {
  ...
  self.umem.addr = (ulong)self.config.frame_memory;
...
fd_xdp_rx_enqueue( fd_xdp_t * xdp, ulong * offset, ulong count ) {
  ...
  ulong * ring = fill->ring;
  ulong mask = fill->sz - 1;
...
fd_xdp_rx_enqueue2( fd_xdp_t * xdp, fd_xdp_frame_meta_t * meta, ulong count ) {
  ...
  ulong frame_mask = xdp->config.frame_size - 1u;
