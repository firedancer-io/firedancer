...
FD_TL fd_alloc_t * g_fd_ossl_alloc        = NULL;
FD_TL ulong        g_fd_ossl_alloc_errors = 0UL;
...
crypto_malloc( ulong        num,
  ...
  (void)file;
  (void)line;
  void * result = fd_alloc_malloc( g_fd_ossl_alloc, 8UL, num + 8UL );
  if( FD_UNLIKELY( !result ) ) {
    g_fd_ossl_alloc_errors++;
    return NULL;
  }
  *(ulong*)result = num;
...
crypto_free( void *       addr,
  ...
  fd_alloc_free( g_fd_ossl_alloc, (uchar*)addr - 8UL );
}
...
crypto_realloc( void *       addr,
  ...
  void * new = fd_alloc_malloc( g_fd_ossl_alloc, 8UL, num + 8UL );
