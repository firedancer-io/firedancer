...
fd_ibverbs_mock_qp_new( void * const mem,
  ...
  void * rx_cq_mem = FD_SCRATCH_ALLOC_APPEND( l, fd_ibv_wc_q_align(),     fd_ibv_wc_q_footprint     ( cq_depth     ) );
  void * tx_cq_mem = FD_SCRATCH_ALLOC_APPEND( l, fd_ibv_wc_q_align(),     fd_ibv_wc_q_footprint     ( cq_depth     ) );
  ...
  mock->rx_cq->wc_q = fd_ibv_wc_q_join( fd_ibv_wc_q_new( rx_cq_mem, cq_depth ) );
  mock->tx_cq->wc_q = fd_ibv_wc_q_join( fd_ibv_wc_q_new( tx_cq_mem, cq_depth ) );
  if( FD_UNLIKELY( !mock->rx_q || !mock->tx_q || !mock->rx_cq->wc_q || !mock->tx_cq->wc_q ) ) {
  ...
  for( ulong i=0UL; i<2UL; i++ ) {
    fd_ibverbs_mock_cq_t * mock_cq = i ? mock->tx_cq : mock->rx_cq;
    mock_cq->qp = mock;

    struct ibv_cq * cq = mock_cq->cq;
    cq->cq_context = mock_cq;
    cq->context    = ctx;

    struct ibv_cq_ex * cq_ex = mock_cq->cq_ex;
    cq_ex->cq_context    = mock_cq;
    cq_ex->context       = ctx;
  ...
  qp->qp_context = mock;
  qp->context    = ctx;
  qp->send_cq    = mock->tx_cq->cq;
  qp->recv_cq    = mock->rx_cq->cq;
...
fd_ibverbs_mock_qp_delete( fd_ibverbs_mock_qp_t * qp ) {
  ...
  fd_ibv_wc_q_delete     ( fd_ibv_wc_q_leave     ( qp->rx_cq->wc_q ) );
  fd_ibv_wc_q_delete     ( fd_ibv_wc_q_leave     ( qp->tx_cq->wc_q ) );
...
static void
fd_ibv_mock_cq_ex_pop( fd_ibverbs_mock_cq_t * mock_cq,
                       struct ibv_cq_ex *      cq_ex ) {
  FD_TEST( !fd_ibv_wc_q_empty( mock_cq->wc_q ) );
  struct ibv_wc * wc = fd_ibv_wc_q_pop_head_nocopy( mock_cq->wc_q );
  cq_ex->wr_id            = wc->wr_id;
  cq_ex->status           = wc->status;
  mock_cq->poll_opt.byte_len = wc->byte_len;
  mock_cq->poll_opt.opcode   = wc->opcode;
}
...
fd_ibv_mock_start_poll( struct ibv_cq_ex *        cq_ex,
  ...
  fd_ibverbs_mock_cq_t * mock_cq = cq_ex->cq_context;
  fd_ibverbs_mock_qp_t * mock = mock_cq->qp;
  FD_TEST( mock->magic==FD_IBVERBS_MOCK_QP_MAGIC );
  FD_TEST( !mock_cq->polling );
  FD_TEST( attr );
  if( FD_UNLIKELY( attr->comp_mask ) ) {
    FD_LOG_ERR(( "Sorry, ibv_mock doesn't support ibv_start_poll attr comp_mask" ));
  }
  if( fd_ibv_wc_q_empty( mock_cq->wc_q ) ) return ENOENT;
  mock_cq->polling = 1U;
  fd_ibv_mock_cq_ex_pop( mock_cq, cq_ex );
  return 0;
}
...
fd_ibv_mock_wc_read_opcode( struct ibv_cq_ex * cq_ex ) {
  fd_ibverbs_mock_cq_t * mock_cq = cq_ex->cq_context;
  FD_TEST( mock_cq->polling );
  return mock_cq->poll_opt.opcode;
}
...
fd_ibv_mock_wc_read_byte_len( struct ibv_cq_ex * cq_ex ) {
  fd_ibverbs_mock_cq_t * mock_cq = cq_ex->cq_context;
  FD_TEST( mock_cq->polling );
  return mock_cq->poll_opt.byte_len;
}
