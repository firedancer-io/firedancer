...
fd_xsk_setup_poll( fd_xsk_t *              xsk,
  ...
  if( FD_UNLIKELY( 0!=setsockopt( xsk->xsk_fd, SOL_SOCKET, SO_BUSY_POLL, &params->prefbusy_timebudget_micros, sizeof(uint) ) ) ) {
  ...
  if( FD_UNLIKELY( 0!=setsockopt( xsk->xsk_fd, SOL_SOCKET, SO_BUSY_POLL_BUDGET, &params->prefbusy_rx_budget, sizeof(uint) ) ) ) {
