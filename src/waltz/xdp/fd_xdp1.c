...
fd_xdp_gen_program( ulong          code_buf[ 512 ],
  ...
  *(code++) = FD_EBPF( mov64_imm, r5, 0                         );  // r5 = 0
  *(code++) = FD_EBPF( jeq_imm, r5, 0, LBL_PASS                 );  // goto LBL_PASS
  ...
  *(code++) = FD_EBPF_exit;                                           // return bpf_redirect_map(r1,r2,r3)
  ...
  for( ulong i=0UL; i<code_cnt; i++ ) {
    if( (code_buf[ i ] & 0x05)==0x05 ) {
      ...
      // case 6: jmp_target = lbl_drop;     break;
...
fd_xdp_install( uint           if_idx,
  ...
  ulong code_buf[ 512 ];
  ulong code_cnt = fd_xdp_gen_program( code_buf, xsk_map_fd, listen_ip4_addr, ports, ports_cnt, true);
