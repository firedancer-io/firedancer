...
def inject_gre_and_inner_ip(
    packet_data, inner_src_ip="10.0.0.1", inner_dst_ip="10.1.2.3"
):
...
def process_packet_files(
    ...
    for input_file in files:
        try:
            ...
            # Process packet
            modified_packet = inject_gre_and_inner_ip(
                packet_data, inner_src_ip, inner_dst_ip
            )
...
def main():
    ...
    parser.add_argument("--inner-src", default="10.1.2.3", help="Inner IPv4 source IP")
    parser.add_argument(
        "--inner-dst", default="10.0.0.2", help="Inner IPv4 destination IP"
    )

    args = parser.parse_args()

    process_packet_files(
        args.input_pattern, args.output_dir, args.inner_src, args.inner_dst
    )
