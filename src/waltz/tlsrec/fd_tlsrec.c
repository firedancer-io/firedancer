...
static fd_tls_cs_t const *
fd_tlsrec_cs( fd_tlsrec_conn_t const * conn ) {
...
static void
fd_tlsrec_update_traffic_secret( uchar                  secret[ static 48 ],
                                 uchar                  key[ static 32 ],
                                 uchar                  iv[ static 12 ],
                                 fd_tls_cs_t const *    cs ) {
  uchar next_secret[ 48 ];
  fd_hkdf_expand_label( next_secret, cs->hash_sz, secret, cs->hash_sz,
                        "traffic upd", 11UL, NULL, 0UL, cs->hmac_fn );
  fd_memcpy( secret, next_secret, cs->hash_sz );
  fd_hkdf_expand_label( key, cs->key_sz, secret, cs->hash_sz,
                        "key", 3UL, NULL, 0UL, cs->hmac_fn );
  fd_hkdf_expand_label( iv, 12UL, secret, cs->hash_sz,
                        "iv", 2UL, NULL, 0UL, cs->hmac_fn );
}
...
fd_tlsrec_peek_msg_sz( uchar const * buf, ulong buf_sz ) {
  ...
  ulong payload = fd_tls_u24_to_uint( hdr.length );
...
fd_tlsrec_post_hs_rx( fd_tlsrec_conn_t * conn, uchar const * msg, ulong msg_sz ) {
  if( msg[0]!=FD_TLS_MSG_KEY_UPDATE ) return FD_TLSREC_SUCCESS;
...
cb_secrets( void const * hs, void const * rx_secret, void const * tx_secret, uint level ) {
  ...
  fd_tls_cs_t const * cs   = fd_tlsrec_cs( conn );

  ulong ks = cs->key_sz, ss = cs->hash_sz;
  fd_tlsrec_keys_t * out = &conn->keys[ level==FD_TLS_LEVEL_APPLICATION ];
  out->key_sz = (uchar)ks;
  fd_memcpy( out->read_secret,  rx_secret, ss );
  fd_memcpy( out->write_secret, tx_secret, ss );
