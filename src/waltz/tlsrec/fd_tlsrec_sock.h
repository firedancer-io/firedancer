...
/* FD_TLSREC_SOCK_TX_RESERVE is the ciphertext headroom kept free for
   conn_rx output while a record is parked.  The largest post-handshake
   reply is a 27 byte KeyUpdate record.  FD_TLSREC_SOCK_RX_MTU is the
   most ciphertext read from the socket per call. */
...
#define FD_TLSREC_SOCK_RX_MTU     (16384UL)
