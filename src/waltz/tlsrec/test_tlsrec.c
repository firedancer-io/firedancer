...
test_tlsrec_seq_and_closed_update( fd_rng_t * rng ) {
  ...
  /* All write APIs fail closed rather than using a wrapped nonce. */
  for( int api=0; api<3; api++ ) {
    test_tlsrec_connect( &tls, cli, srv );
    cli->write_seq = ULONG_MAX;
    sz = sizeof(rec);
    int rc;
    if( api==0 ) rc = fd_tlsrec_conn_close( cli, rec, &sz );
    else if( api==1 ) rc = fd_tlsrec_conn_key_update( cli, rec, &sz, 0 );
    else {
      fd_tlsrec_slice_init( app_tx, (uchar *)data, sizeof(data) );
      rc = fd_tlsrec_conn_tx( cli, rec, &sz, app_tx );
      FD_TEST( fd_tlsrec_slice_sz(app_tx)==sizeof(data) );
    }
    FD_TEST( rc==FD_TLSREC_ERR_CRYPTO && !sz );
    FD_TEST( cli->write_seq==ULONG_MAX && cli->tx_closed && fd_tlsrec_conn_is_failed( cli ) );
    FD_TEST( cli->hs.base.reason==FD_TLS_REASON_REC_SEQ );
  }

  /* Exhaustion is also checked on reads, for both encrypted epochs. */
  for( uint key_idx=0U; key_idx<2U; key_idx++ ) {
    test_tlsrec_connect( &tls, cli, srv );
    if( !key_idx ) {
      cli->hs.base.state = FD_TLS_HS_WAIT_FINISHED;
      cli->tx_level = FD_TLS_LEVEL_HANDSHAKE;
    }
    cli->read_seq = srv->write_seq = ULONG_MAX-1UL;
    uchar cancel[] = { 1, FD_TLS_ALERT_USER_CANCELED };
    sz = test_tlsrec_send_raw_ex( srv, rec, cancel, sizeof(cancel), FD_TLS_REC_ALERT, 0UL, 0x0303, key_idx );
    fd_tlsrec_slice_init( tcp_rx, rec, sz );
    tcp_tx_sz = sizeof(tcp_tx); app_rx_sz = sizeof(app_rx);
    FD_TEST( fd_tlsrec_conn_rx( cli, tcp_rx, tcp_tx, &tcp_tx_sz, app_rx, &app_rx_sz )==FD_TLSREC_SUCCESS );
    FD_TEST( cli->read_seq==ULONG_MAX );
    sz = test_tlsrec_send_raw_ex( srv, rec, cancel, sizeof(cancel), FD_TLS_REC_ALERT, 0UL, 0x0303, key_idx );
    fd_tlsrec_slice_init( tcp_rx, rec, sz );
    tcp_tx_sz = sizeof(tcp_tx); app_rx_sz = sizeof(app_rx);
    FD_TEST( fd_tlsrec_conn_rx( cli, tcp_rx, tcp_tx, &tcp_tx_sz, app_rx, &app_rx_sz )==FD_TLSREC_ERR_PROTO );
    FD_TEST( cli->read_seq==ULONG_MAX && cli->hs.base.reason==FD_TLS_REASON_REC_SEQ );
    test_tlsrec_check_alert( cli, tcp_tx, tcp_tx_sz, FD_TLS_ALERT_INTERNAL_ERROR );
  }
}
