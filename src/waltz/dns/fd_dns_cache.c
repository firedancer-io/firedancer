...
fd_dns_cache_join( fd_dns_cache_t *      shcache,
  ...
  if( FD_UNLIKELY( !addr_pool ) ) {
    FD_LOG_CRIT(( "fd_dns_cache_addr_pool_join faile (memory corruption?)d" ));
  }
  fd_dns_cache_map_t * map = fd_dns_cache_map_join( ljoin->map, shmap, name_pool, fd_dns_cache_name_pool_max( name_pool ) );
  if( FD_UNLIKELY( !map ) ) {
    FD_LOG_CRIT(( "fd_dns_cache_map_join faile (memory corruption?)d" ));
  }
...
fd_dns_cache_put(
  ...
  if( FD_UNLIKELY( fd_dns_cache_map_txn_query( ljoin->map, &key, NULL, query )==FD_MAP_SUCCESS ) ) {
...
fd_dns_cache_remove(
  ...
  fd_dns_cache_map_query_t query[1] = {0};
  if( FD_LIKELY( fd_dns_cache_map_txn_query( ljoin->map, &key, NULL, query )==FD_MAP_SUCCESS ) ) {
    fd_dns_cache_remove_addresses( ljoin, query->ele );
    /* invalidates query */
    if( FD_UNLIKELY( fd_dns_cache_map_txn_remove( ljoin->map, &key, NULL, query )!=FD_MAP_SUCCESS ) ) {
...
fd_dns_cache_query_start(
  ...
  if( FD_UNLIKELY( fd_dns_cache_map_query_try( ljoin->map, &key, NULL, query->q )!=FD_MAP_SUCCESS ) ) return NULL;
