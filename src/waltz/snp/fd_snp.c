...
fd_snp_send( fd_snp_t *    snp,
  ...
  if( sz > 0 ) {
    return snp->cb.tx( snp->cb.ctx, buf, (ulong)sz, meta );
  }
  return 0;
}
...
fd_snp_process_packet( fd_snp_t *    snp,
  ...
  if( FD_UNLIKELY( type == SNP_TYPE_HS_CLIENT_INITIAL ) ) {
    ...
    for (uchar i = 0; i < hs->buffers_sz; i++) {
      ...
      if (sz > 0) {
        ...
        snp->cb.tx( snp->cb.ctx, scratch, (ulong)sz, meta );
      }
    }
  ...
  if( send_sz > 0 ) {
    snp->cb.tx( snp->cb.ctx, _buf, (ulong)send_sz, meta );
  }
}
