  ...
  for( ulong j=0; j<16; j++ ) {
    ...
    for( ulong k=0; k<20; k++ ) {
      ...
      */
      ...
      // if( network_packet[45]==0x1F ) {
      //   char msg[10] = { 0 }; memcpy( msg, network_packet+57, network_packet[55] );
      //   FD_LOG_NOTICE(( "%s processing packet %lu: %x %s", is_server?"server":"client", k, network_packet[45], msg ));
      // } else {
      //   FD_LOG_NOTICE(( "%s processing packet %lu: %x", is_server?"server":"client", k, network_packet[45] ));
      // }
      ...
      if( ctx->buf_cnt ) {
        ...
        // FD_LOG_NOTICE(( "Trace from network_packet %d", packet_sz ));
        trace[trace_cnt].data_sz = (ushort)packet_sz;
        memcpy( trace[trace_cnt].data, network_packet, trace[trace_cnt].data_sz );
      ...
      if( dupe_packet ) {
        packet_sz = trace[k].data_sz;
        memcpy( network_packet, trace[k].data, trace[k].data_sz );
        // FD_LOG_NOTICE(( "%s processing again packet %lu: %x", is_server?"server":"client", k, network_packet[45] ));
        packet_sz = fd_snp_process_packet( snp, network_packet, (ulong)packet_sz );
      }
    }
