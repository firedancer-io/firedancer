...
static void tx_callback(fd_stl_t* stl, stl_net_ctx_t* dst, const uchar* data, ulong data_sz);
static void rx_callback(fd_stl_t* stl, stl_net_ctx_t* src, const uchar* data, ulong data_sz);
...
// TX callback for STL - sends UDP packet
static void tx_callback(fd_stl_t* stl, stl_net_ctx_t* dst, const uchar* data, ulong data_sz) {
  (void)stl;
...
// RX callback for STL - processes received data
static void rx_callback(fd_stl_t* stl, stl_net_ctx_t* src, const uchar* data, ulong data_sz) {
  (void)stl; // Unused parameter
...
int main(int argc, char *argv[]) {
  ...
  uchar buf[1<<15];
  void* stl_mem = (void*)fd_ulong_align_up((ulong)buf, fd_stl_align());
  if (!stl_mem) {
      perror("Failed to allocate memory for STL");
      cleanup();
      return 1;
  }

  // Create and initialize STL
  fd_stl_t* stl = fd_stl_new(stl_mem, &limits);
  if (!stl) {
      fprintf(stderr, "Failed to initialize STL\n");
      cleanup();
      return 1;
  }

  // Set up callbacks
  stl->cb.tx = tx_callback;
  stl->cb.rx = rx_callback;

  if (is_server) {
    // Initialize server parameters
    fd_stl_s0_server_params_t server_params;
    memset(&server_params, 0, sizeof(server_params));
    stl->server_params = server_params;
  } else {
    // Initialize client parameters
    fd_stl_s0_client_params_t client_params;
    memset(&client_params, 0, sizeof(client_params));
    stl->client_params = client_params;

    printf("STL client initialized. Connecting to %s:%d\n", ip, port);
  }

  // Create network context for the server (used in client mode)
  stl_net_ctx_t server_ctx;
  if (!is_server) {
  ...
  while (running) {
    ...
    if (fds[1].revents & POLLIN) {
      ...
      if (recv_len > 0) {
        // Process the packet with STL
        fd_stl_process_packet(stl, recv_buffer, (ulong)recv_len,
    ...
    if (fds[0].revents & POLLIN) {
      ...
      if (!is_server) {
        char msg[16];
        snprintf(msg, sizeof(msg), "FD_STL_%c", c);
        FD_LOG_NOTICE(("Sending message: %s\n", msg));
        fd_stl_send(stl, &server_ctx, msg, strlen(msg));
      }
    }
  }
