...
test_h2_rbuf( fd_rng_t * rng ) {
  ...
  FD_TEST( fd_h2_rbuf_free_sz( rbuf )==32 );
  ...
  for( ulong iter=0UL; iter<10000000UL; iter++ ) {
    ...
    if( action & 1 ) {
      /* push */
      ulong blob_sz = fd_ulong_min( free_sz, 32 );
      ulong push_sz = fd_rng_ulong_roll( rng, blob_sz+1UL );
      ...
      if( action & 2 ) {
        ...
        FD_TEST( free_sz ? iov_cnt==2 : iov_cnt==0 );
        if( iov_cnt ) {
      ...
      if( action & (2+4+8) ) {
        /* gather */
        ulong sz0, sz1;
        uchar * b = fd_h2_rbuf_peek_head( rbuf, &sz0, &sz1 );
      ...
      } else if( action & (16+32) ) {
        /* pop */
        uchar * b = fd_h2_rbuf_pop( rbuf, pop_sz );
        for( ulong j=0UL; j<pop_sz; j++ ) {
          FD_TEST( shadow[ (shadow_cons++)%64 ]==b[j] );
        }
      } else {
        /* compact */
        fd_h2_rbuf_compact( rbuf );
        ulong sz0, sz1;
        uchar * b = fd_h2_rbuf_peek_head( rbuf, &sz0, &sz1 );
        FD_TEST( sz0>=pop_sz );
        for( ulong j=0UL; j<pop_sz; j++ ) {
    ...
    FD_TEST( fd_h2_rbuf_free_sz( rbuf )<=rbuf->frame_max );
    FD_TEST( fd_h2_rbuf_used_sz( rbuf )<=rbuf->frame_max );
