...
fd_h2_rx_window_update( fd_h2_conn_t *            conn,
  ...
  if( !stream_id ) {
    ...
    uint tx_wnd_new;
    if( FD_UNLIKELY( __builtin_uadd_overflow( tx_wnd, increment, &tx_wnd_new ) || tx_wnd_new>0x7fffffffU ) ) {
      fd_h2_conn_error( conn, FD_H2_ERR_FLOW_CONTROL );
      return 0;
    }
    ...
    uint tx_wnd_new;
    if( FD_UNLIKELY( __builtin_uadd_overflow( stream->tx_wnd, increment, &tx_wnd_new ) || tx_wnd_new>0x7fffffffU ) ) {
