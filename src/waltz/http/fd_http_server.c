...
fd_http_ws_compress_maybe( fd_http_server_t * http ) {
  /* we don't compress small frames */
  if( FD_LIKELY( http->stage_len <= 200 || http->stage_err ) ) return;
  ...
  if( FD_UNLIKELY( ZSTD_isError( compressed_sz ) ) ) {
    FD_LOG_WARNING(( "ZSTD_compressCCtx failed (%s)", ZSTD_getErrorName( compressed_sz ) ) );
    http->stage_err = 1;
    return;
  }
...
fd_http_server_ws_send( fd_http_server_t * http,
                        ulong              ws_conn_id ) {
  fd_http_ws_compress_maybe( http );
  ...
  if( FD_LIKELY( http->pollfds[ http->max_conns+ws_conn_id ].fd==-1 ) ) return 0;
  ...
  int compressed = conn->compress_websocket && http->stage_comp_len;
