...
test_fib4_hmap_capacity( fd_fib4_t * fib ) {
  ...
  for( ulong i = 0; i < hmap_max + table_max - 1; i++ ) {
...
main( int     argc,
  ...
  FD_TEST( fd_fib4_footprint( 16UL, 16UL, 4UL )<=sizeof(fib1_mem) );
  fd_fib4_t * fib_local = fd_fib4_join( fd_fib4_new( fib1_mem, 16UL, 16UL, 4UL, 123456UL ) );
  fd_fib4_t * fib_main  = fd_fib4_join( fd_fib4_new( fib2_mem, 16UL, 16UL, 4UL, 123456UL ) );
