...
dump_table( fd_netlink_t * netlink,
  ...
  ulong const route_peer_lock_cnt = 16UL;
  ulong const route_peer_seed     = 123456UL;
  FD_TEST( fd_fib4_footprint( route_max, route_peer_max, route_peer_lock_cnt  )<=sizeof(fib1_mem) );
  fd_fib4_t * fib = fd_fib4_join( fd_fib4_new( fib1_mem, route_max, route_peer_max, route_peer_lock_cnt, route_peer_seed ) );
