...
fd_fib4_footprint( ulong route_max,
                   ...
                   ulong route_peer_lock_cnt ) {
  if( route_max==0 || route_max>UINT_MAX ||
      route_peer_max==0 || route_peer_max>UINT_MAX ||
      route_peer_lock_cnt==0 || route_peer_lock_cnt>fd_fib4_hmap_lock_max() ) return 0UL;
  ulong elem_max       = fd_fib4_hmap_get_ele_max( route_peer_max   );
  ulong probe_max      = fd_fib4_hmap_get_probe_max( elem_max );
  ulong hmap_footprint = fd_fib4_hmap_footprint( elem_max, route_peer_lock_cnt, probe_max );
...
fd_fib4_new( void * mem,
  ...
  if( FD_UNLIKELY( route_peer_lock_cnt==0 || route_peer_lock_cnt>fd_fib4_hmap_lock_max() ) ) {
    FD_LOG_WARNING(( "invalid route_peer_lock_cnt" ));
    return NULL;
  }
  ...
  ulong hmap_footprint     = fd_fib4_hmap_footprint( hmap_elem_max, route_peer_lock_cnt, hmap_probe_max );
  FD_TEST( hmap_footprint );
  ...
  FD_TEST( fd_fib4_hmap_new( fib4_hmap_mem, hmap_elem_max, route_peer_lock_cnt, hmap_probe_max, route_peer_seed ) );
  ...
  fib4->hmap_lock_cnt    = route_peer_lock_cnt;
  fib4->hmap_seed        = route_peer_seed;
...
fd_fib4_clear( fd_fib4_t * fib4 ) {
  ...
  FD_TEST( fd_fib4_hmap_lock_range( hmap, 0, fd_fib4_hmap_lock_cnt( hmap ), FD_MAP_FLAG_BLOCKING, ignored )==FD_MAP_SUCCESS );

  ulong elem_max  = fd_fib4_hmap_get_ele_max( fib4->hmap_max );
  ulong probe_max = fd_fib4_hmap_get_probe_max( elem_max );

  FD_TEST( fd_fib4_hmap_new( fd_fib4_hmap_mem( fib4 ), elem_max, fib4->hmap_lock_cnt, probe_max, fib4->hmap_seed ) );
...
fd_fib4_insert( fd_fib4_t *     fib,
  ...
  if( ip4_dst!=0 && prefix==32 && hop[0].ip4_src!=0 ) {
...
fd_fib4_lookup( fd_fib4_t const * fib,
  ...
  if( fib->hmap_cnt>0 ) {
    ...
    if( find_err==FD_MAP_SUCCESS ) {
      ...
      if( FD_UNLIKELY( find_err==FD_MAP_ERR_AGAIN ) ) {
