...
/* allow us to call fd_quic_get_state */
...
client_fibre_fn( void * vp_arg ) {
  ...
  while( !client_done ) {
    ...
    if( !conn ) {
      ...
      conn = fd_quic_connect( quic, 0, 0, 0, 0 );
    ...
    if( stream && stream->stream_id == (ulong)-1 ) {
    ...
    if( rc == FD_QUIC_SUCCESS ) {
      ...
      FD_LOG_WARNING(( "send failed  stream: %p", (void*)stream ));
    }
  }
...
server_fibre_fn( void * vp_arg ) {
  ...
  /* run for another few minutes to ensure connections time out */
  ulong stop_time = now + (ulong)300e9;
  while( now < stop_time ) {
    fd_quic_service( quic );
    ulong next_wakeup = fd_quic_get_next_wakeup( quic );
    fd_fibre_wait_until( (long)fd_ulong_min( next_wakeup, stop_time ) );
  }

  /* when we're done, the maps should be empty */
  fd_quic_state_t * state = fd_quic_get_state( quic );

  /* verify connection and token maps are empty */
#define KEY_CNT( map_type, map ) map_type##_key_cnt( (map) )

  FD_TEST( KEY_CNT( fd_quic_conn_map,  state->conn_map  ) == 0 );
  FD_TEST( KEY_CNT( fd_quic_token_map, state->token_map ) == 0 );
}
...
main( int argc, char ** argv ) {
  ...
  client_quic->config.stateless_reset_seed = 123456789UL;
  ...
  fd_quic_netem_init( &mitm_client_to_server, 0.00f, 0.00f );
  fd_quic_netem_init( &mitm_server_to_client, 0.00f, 0.00f );
  ...
  FD_LOG_NOTICE(( "Installing default signal handlers" ));

  struct sigaction sa = {0};
  sa.sa_handler = SIG_DFL;
  if( sigaction( SIGTERM, &sa, NULL ) ) {
    FD_LOG_ERR(( "sigaction(SIGTERM) failed with error: %d %s", errno, strerror( errno ) ));
  }
  if( sigaction( SIGINT, &sa, NULL ) ) {
    FD_LOG_ERR(( "sigaction(SIGINT) failed with error: %d %s", errno, strerror( errno ) ));
  }
  if( sigaction( SIGPIPE, &sa, NULL ) ) {
    FD_LOG_ERR(( "sigaction(SIGPIPE) failed with error: %d %s", errno, strerror( errno ) ));
  }
  ...
  client_args_t client_args[1] = {{ .quic = client_quic }};
