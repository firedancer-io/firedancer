...
int done = 0;
fd_aio_pkt_info_t batch[1] = {{ (void *)ptr, (ushort)size }};
//TODO advance clock to get retries working
    while (k < 4000 && !done) {
      ...
      //hammer
      if (client_quic != NULL && server_quic != NULL) {
      ...
      case 0:
        ...
        // printf("Inside case 0\n");


        fd_quic_stream_t * stream = fd_quic_conn_new_stream(client_conn, FD_QUIC_TYPE_UNIDIR);
        if (stream == NULL) {
          return 1; 
        }
        fd_quic_stream_set_context( stream, meta );
        ...
        int rv = fd_quic_stream_send( stream, batch, 1 /* batch_sz */, 1 /* fin */ );
        if (rv == 1) {
          k++;
          // printf("k=%d\n",k);
