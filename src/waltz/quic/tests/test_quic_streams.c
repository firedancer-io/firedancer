...
my_stream_receive_cb(
  ...
  (void)cb_ctx; (void)conn;

  /* Derive expected payload */

  uchar payload_buf[ FD_TXN_MTU ];
  ulong buf_sz = fd_quic_stream_spam_gen( NULL, payload_buf, stream_id );

  FD_LOG_DEBUG(( "server rx stream data stream=%lu size=%lu offset=%lu",
        stream_id, data_sz, offset ));

  if( FD_UNLIKELY( fin && offset+data_sz != buf_sz ) ) {
    FD_LOG_ERR(( "data wrong size. expected: %lu, actual: %lu", buf_sz, offset+data_sz ));
  }
  if( FD_UNLIKELY( 0!=memcmp( data, payload_buf + offset, data_sz ) ) ) {
...
main( int     argc,
  ...
  fd_quic_stream_spam_t spammer[1] = {{ .gen_fn = fd_quic_stream_spam_gen }};
  ...
  ulong cum_sent_cnt = 0L;
  while( recvd < 10000 ) {
    ulong sent_cnt = fd_quic_stream_spam_service( client_conn, spammer );
