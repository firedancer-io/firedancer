...
void
my_stream_receive( fd_quic_stream_t * stream,
                      void *             ctx,
                      uchar const *      data,
                      ulong              data_sz,
                      ulong              offset,
  ...
  if ( stream->rx_tot_data < 5 ) {
    FD_TEST( data_sz == 1 );
    fd_quic_stream_set_rx_max_stream_data( stream, stream->rx_max_stream_data + data_sz );
  } else if ( stream->rx_max_stream_data < 12 ) {
    fd_quic_stream_set_rx_max_stream_data( stream, 12 );
  } else if ( stream->conn->rx_max_data < 12 ) {
...
main( int argc, char ** argv ) {
  ...
  server_quic->cb.stream_receive   = my_stream_receive;
