...
main( int     argc,
  ...
  long x; long y;
  FD_TEST( fd_clock_joint_read( _fd_tickcount, NULL, fd_log_wallclock_host, NULL, &x, &y, NULL )==FD_CLOCK_SUCCESS );
  FD_TEST( fd_clock_new( clock_shmem, (long)10e6, 0L, 0., 0., x, y, fd_tempo_tick_per_ns( NULL ) ) );
  FD_TEST( fd_clock_join( clock, clock_shmem, _fd_tickcount, NULL ) );
