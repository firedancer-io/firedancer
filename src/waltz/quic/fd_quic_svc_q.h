...
struct fd_quic_svc_timers_conn_meta {
  /* interp of idx is svc_type dependent
     For re-tx, is idx into prq buffer
     Else, is idx into backing pool */
...
struct __attribute__((aligned(32))) fd_quic_svc_event {
  ulong timeout;
  fd_quic_conn_t * conn;
  ulong prev;
  ulong next;
...
/* combined event and type for clean and concise return value
   Not used internally by svc_timers, just in interface */
struct fd_quic_svc_event_and_type {
  fd_quic_svc_event_t event;
  uint svc_type;
};
typedef struct fd_quic_svc_event_and_type fd_quic_svc_event_and_type_t;
...
/* fd_quic_svc_timers_next returns next event and type. If 'pop' is true,
   the event is popped from the queue.
   Returns {NULL, FD_QUIC_SVC_CNT} if the queue is empty.
