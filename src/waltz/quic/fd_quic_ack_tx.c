...
fd_quic_ack_pkt( fd_quic_ack_gen_t *      gen,
  ...
  if( ( enc_level == cached_ack->enc_level ) &
    ...
    FD_DEBUG( FD_LOG_DEBUG(( "gen=%p queue ACK for enc=%u pkt_num=%lu range=[%lu,%lu) seq=%u (merged)",
  ...
  FD_DEBUG( FD_LOG_DEBUG(( "gen=%p queue ACK for enc=%u pkt_num=%lu seq=%u",
...
fd_quic_gen_ack_frames( fd_quic_ack_gen_t *      gen,
  ...
  FD_DEBUG( FD_LOG_DEBUG(( "[ACK gen] elicited=%d instant=%d", gen->is_elicited, gen->is_instant )); )
  ...
  FD_DEBUG( FD_LOG_DEBUG(( "[ACK gen] next wakeup in %g ns", (double)(next_wakeup - now) )); )
  ...
  for( ; gen->tail != gen->head; gen->tail++ ) {
    ...
    FD_DEBUG( FD_LOG_DEBUG(( "gen=%p sending ACK enc=%u range=[%lu,%lu) seq=%u",
        (void *)gen, enc_level, ack->pkt_number.offset_lo, ack->pkt_number.offset_hi, gen->tail )); )
  }
  ...
  if( gen->head == gen->tail ) {
    ...
    FD_DEBUG( FD_LOG_DEBUG(( "Not all ACK frames were flushed" )); )
  }
