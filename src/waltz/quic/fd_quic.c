...
fd_quic_init( fd_quic_t * quic ) {
  ...
  state->svc_timers = (fd_quic_svc_timers_t *)( (ulong)quic + layout.svc_timers_off );
...
fd_quic_state_validate( fd_quic_t * quic ) {
  ...
  FD_TEST( fd_quic_svc_timers_validate( state->svc_timers ) );
...
fd_quic_service( fd_quic_t * quic ) {
  ...
  fd_quic_svc_event_and_type_t next = fd_quic_svc_timers_next( timers, now, 1 /* pop */);
  if( next.svc_type == FD_QUIC_SVC_CNT ) {
    return 0;
  }

  int cnt = 0;
  cnt = fd_quic_svc_poll( quic, next.event.conn, now );
  ...
  while( enc_level != ~0u ) {
    ...
    *pkt_meta = (fd_quic_pkt_meta_t){0};
    ...
    if( pkt_meta->flags ) {
      ...
      fd_quic_svc_schedule( state->svc_timers, conn, FD_QUIC_SVC_RETX, pkt_meta->expiry );
...
fd_quic_conn_free( fd_quic_t *      quic,
  ...
  /* remove connection from service queues */
  fd_quic_svc_cancel_all( state->svc_timers, conn );
...
fd_quic_get_next_wakeup( fd_quic_t * quic ) {
  ...
  fd_quic_svc_event_and_type_t next = fd_quic_svc_timers_next( state->svc_timers, now, 0 );
  if( FD_UNLIKELY( next.svc_type == FD_QUIC_SVC_CNT ) ) {
    return LONG_MAX;
  }
  return (ulong)next.event.timeout;
}
