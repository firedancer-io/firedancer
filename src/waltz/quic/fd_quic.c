...
fd_quic_limits_from_env( int  *   pargc,
  ...
  limits->inflight_frame_cnt = fd_env_strip_cmdline_ulong( pargc, pargv, "--quic-inflight-pkts", "QUIC_MAX_INFLIGHT_PKTS", 1280000UL );
...
/* fd_quic_gen_frame_store_pkt_meta returns 1 if successful, 0 if not */
static int
...
fd_quic_gen_handshake_frames( fd_quic_conn_t             * conn,
  ...
  if( offset_hi > offset_lo ) {
    ...
    conn->used_pkt_meta++;
    *pkt_meta = *pkt_meta_tmpl;
    pkt_meta->key.type            = FD_QUIC_PKT_META_TYPE_HS_DATA;
    pkt_meta->val.range.offset_lo = offset_lo;
    pkt_meta->val.range.offset_hi = offset_hi;
    fd_quic_pkt_meta_insert( &tracker->sent_pkt_metas[pkt_meta->enc_level], pkt_meta, tracker->pool );
  }
    ...
    if( cur_stream->upd_pkt_number >= pkt_num ) {
      ...
      if( FD_LIKELY( FD_QUIC_STREAM_ACTION( cur_stream ) ) ) {
        ...
        *frame_type_p = (uchar)frame_type;
        ...
        conn->used_pkt_meta++;
        *pkt_meta = *pkt_meta_tmpl;
        pkt_meta->key.type = FD_QUIC_PKT_META_TYPE_STREAM;
        pkt_meta->key.stream_id = cur_stream->stream_id;
        pkt_meta->val.range.offset_lo = stream_off;
        pkt_meta->val.range.offset_hi = stream_off + data_sz;
        fd_quic_pkt_meta_insert( &tracker->sent_pkt_metas[pkt_meta->enc_level], pkt_meta, tracker->pool );
      }
    }
