/* test_tls_web: live TLS v1.3 client handshake against public servers.
   ...
   Environment:
     ...
     FD_TLS_WEB_STRICT=1        treat unreachable targets as failures
     FD_TLS_WEB_FILTER=<substr> only probe hosts containing <substr> */
...
/* fd_tls_web_result describes the outcome of one probe.  verify_err
   is meaningful whenever verify_called is set, regardless of whether
   the handshake completed (a rejected chain aborts the handshake). */
...
struct fd_tls_web_result {
  ...
  ulong  chain_cert_cnt;    /* certs in the server's Certificate message */
  ulong  chain_rsa_mask;    /* bit i set: cert i (0=leaf) carries an RSA public key */
  int    chain_rsa_sig;     /* some cert in the chain is RSA-signed */
};

struct fd_tls_web_verify_ctx {
...
/* fd_tls_web_chain_scan records which certificates of a TLS 1.3
   Certificate message body (RFC 8446 Section 4.4.2) carry RSA keys or
   RSA signatures.  It only looks for the PKCS#1 OIDs, which is enough
   to explain a verify failure: fd_x509 has no RSA support at all. */

static int
fd_tls_web_has_oid( uchar const * p, ulong sz, uchar const * oid, ulong oid_sz ) {
  for( ulong i=0UL; i+oid_sz<=sz; i++ ) if( !memcmp( p+i, oid, oid_sz ) ) return 1;
  return 0;
}

static void
fd_tls_web_chain_scan( struct fd_tls_web_verify_ctx * ctx,
                       uchar const *                  msg,
                       ulong                          msg_sz ) {
  static uchar const oid_rsa_key[] = { 0x06,0x09,0x2a,0x86,0x48,0x86,0xf7,0x0d,0x01,0x01,0x01 }; /* rsaEncryption */
  static uchar const oid_rsa_sig[] = { 0x06,0x09,0x2a,0x86,0x48,0x86,0xf7,0x0d,0x01,0x01 };      /* pkcs-1 prefix: sha*WithRSA, RSASSA-PSS */

  if( msg_sz<4UL ) return;
  ulong ctx_len = msg[0];
  if( msg_sz<1UL+ctx_len+3UL ) return;
  uchar const * p   = msg+1UL+ctx_len;
  ulong list_len    = ((ulong)p[0]<<16) | ((ulong)p[1]<<8) | p[2];
  p += 3UL;
  uchar const * end = p + fd_ulong_min( list_len, (ulong)( msg+msg_sz-p ) );

  while( end-p>=3L ) {
    ulong cert_len = ((ulong)p[0]<<16) | ((ulong)p[1]<<8) | p[2];
    p += 3UL;
    if( (ulong)(end-p)<cert_len ) break;
    if( ctx->cert_cnt<64UL ) {
      if( fd_tls_web_has_oid( p, cert_len, oid_rsa_key, sizeof(oid_rsa_key) ) ) ctx->rsa_mask |= 1UL<<ctx->cert_cnt;
      if( fd_tls_web_has_oid( p, cert_len, oid_rsa_sig, sizeof(oid_rsa_sig) ) ) ctx->rsa_sig = 1;
    }
    ctx->cert_cnt++;
    p += cert_len;
    if( end-p<2L ) break;
    ulong ext_len = ((ulong)p[0]<<8) | p[1];
    p += 2UL + ext_len;
  }
}

static int
fd_tls_web_verify( void *        _ctx,
                   uchar const * cert_chain,
                   ulong         cert_chain_sz,
                   void const *  handshake ) {
  (void)handshake;
  struct fd_tls_web_verify_ctx * ctx = _ctx;
  ctx->called = 1;
  ctx->err    = fd_x509_verify_tls_cert_msg(
      cert_chain, cert_chain_sz, ctx->ca_store,
      ctx->host, strlen( ctx->host ), fd_x509_unix_now_seconds() );
  if( ctx->err!=FD_X509_VERIFY_OK ) fd_tls_web_chain_scan( ctx, cert_chain, cert_chain_sz );
  return ctx->err!=FD_X509_VERIFY_OK;
}
...
fd_tls_web_send_all( int           fd,
  ...
  while( off<sz ) {
    ...
    if( FD_UNLIKELY( sent<0 && errno!=EAGAIN && errno!=EWOULDBLOCK && errno!=EINTR ) ) return errno;
...
fd_tls_web_probe( struct fd_tls_web_conn *   c,
  ...
  struct fd_tls_web_verify_ctx verify_ctx = { .ca_store = ca_store, .host = host };
  ...
  tls.cert_verify_fn  = fd_tls_web_verify;
  tls.cert_verify_ctx = &verify_ctx;
  ...
  result->verify_called     = verify_ctx.called;
  result->verify_err        = verify_ctx.err;
  result->chain_cert_cnt    = verify_ctx.cert_cnt;
  result->chain_rsa_mask    = verify_ctx.rsa_mask;
  result->chain_rsa_sig     = verify_ctx.rsa_sig;
...
fd_tls_web_why( struct fd_tls_web_result const * r,
  ...
  if( r->verify_called && r->verify_err!=FD_X509_VERIFY_OK ) {
    if( r->chain_rsa_mask & 1UL ) {
      fd_cstr_printf( buf, buf_sz, NULL, "leaf certificate has an RSA key; fd_x509 does not support RSA" );
    } else if( r->chain_rsa_mask ) {
      ulong depth = (ulong)fd_ulong_find_lsb( r->chain_rsa_mask );
      fd_cstr_printf( buf, buf_sz, NULL, "EC leaf, but CA cert %lu of %lu in the chain has an RSA key; fd_x509 does not support RSA",
                      depth, r->chain_cert_cnt );
    } else if( r->chain_rsa_sig ) {
      fd_cstr_printf( buf, buf_sz, NULL, "chain uses an RSA signature; fd_x509 does not support RSA" );
    } else {
  ...
  if( r->hs_ok && !r->verify_called ) {
    fd_cstr_printf( buf, buf_sz, NULL, "handshake ok but cert_verify_fn never ran" );
    return buf;
  }
  ...
  case FD_TLS_REASON_CERT_PARSE:
    /* fd_tls rejects the leaf before cert_verify_fn runs when its key
...
main( int     argc,
  ...
  int          strict = !!getenv( "FD_TLS_WEB_STRICT" );
  ...
  for( ulong i=0UL; i<sizeof(hosts)/sizeof(hosts[0]); i++ ) {
    ...
    if( outcome==FD_TLS_WEB_UNREACHABLE ) {
      ...
      pass = ( outcome==FD_TLS_WEB_OK );
      if( pass ) n_ok++; else n_fail++;
    }

    char why[ 192 ];
    FD_LOG_NOTICE(( "%-30s %-11s key=%u/%lu alpn=%d %s%s",
