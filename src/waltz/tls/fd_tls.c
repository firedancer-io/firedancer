...
fd_tls_handle_cert_verify( fd_tls_estate_base_t *    hs,
  ...
  case FD_TLS_SIGNATURE_ECDSA_SECP256R1_SHA256: {
    ...
    int sig_err = fd_secp256r1_verify_accept_low_s( sign_msg, 130UL, raw_sig, compressed_pk, sha256 );
...
fd_tls_client_handshake( fd_tls_t const *      client,
  ...
  case FD_TLS_HS_CONNECTED: {
    ...
    return fd_tls_alert( &handshake->base, FD_TLS_ALERT_UNEXPECTED_MESSAGE, FD_TLS_REASON_POST_HS_MSG );
  }
  default:
...
fd_tls_reason_cstr( uint reason ) {
  ...
  case FD_TLS_REASON_CERT_VERIFY:
    return "certificate chain rejected by verify callback";
