...
main( int argc, char ** argv ) {
  ...
  uchar * shmem2 = (uchar*)fd_wksp_alloc_laddr(
                     wksp,
                     fd_qos_align(),
                     fd_qos_footprint( 1<<LG_ENTRIES ),
                     1UL );
  ...
  FD_LOG_NOTICE(( "shmem: %p  shmem2: %p  equal? %d", (void*)shmem, (void*)shmem2, shmem == shmem2 ));
  ...
  ulong qos_offs = (ulong)qos_mem - (ulong)shmem;
  ...
  /* get reference to qos via other mapping of the workspace */
  /* this tests differing vm mappings in different tiles */
  void * alt_qos_laddr = (void*)( (ulong)shmem2 + (ulong)qos_offs );
  fd_qos_t * alt_qos = fd_qos_join( alt_qos_laddr );
