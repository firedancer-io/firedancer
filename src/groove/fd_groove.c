...
fd_groove_upsert_account( fd_groove_t *       groove,
  ...
  if( FD_UNLIKELY( !used ) ) {
    ...
    FD_LOG_WARNING(( "inserted account into groove which did not exist %s", FD_BASE58_ENC_32_ALLOCA( pubkey ) ));
    return;
  }
  ...
  if ( FD_UNLIKELY(val_size != data_len) ) {
    ...
    FD_LOG_WARNING(( "updated account in groove with different size %s", FD_BASE58_ENC_32_ALLOCA( pubkey ) ));
  ...
  FD_LOG_WARNING(( "updated account in groove with same size %s", FD_BASE58_ENC_32_ALLOCA( pubkey ) ));
...
fd_groove_upsert_account_from_snapshot( fd_groove_t *                   groove,
  ...
  if( FD_UNLIKELY( used ) ) {
    ...
    if( FD_UNLIKELY( meta->dlen != hdr->meta.data_len ) ) {
      ...
      fd_groove_meta_map_publish( query );
      FD_LOG_WARNING(( "updated account in groove %s", FD_BASE58_ENC_32_ALLOCA( pubkey ) ));
      return new_region;
    }
  }
  ...
  FD_LOG_WARNING(( "inserted account into groove %s", FD_BASE58_ENC_32_ALLOCA( pubkey ) ));
  return new_region;
}
