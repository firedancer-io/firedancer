...
#     endif
    ...
    if( !mem ) {
      ...
      if( FD_UNLIKELY( !mem ) )
        FD_LOG_WARNING(( "fd_groove_data_alloc(align=%lu,sz=%lu,tag=%016lx failed (%i-%s)",
                         align, sz, tag, err, fd_groove_strerror( err ) ));
...
main( int     argc,
  ...
  ulong        volume_cnt = fd_env_strip_cmdline_ulong( &argc, &argv, "--volume-cnt", NULL,             3UL );
