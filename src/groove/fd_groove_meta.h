...
/* The groove key metadata contains a 64-bit wide bitfield used to hold
   irregularly sized key metadata compactly.
     ...
     bits[ 1:15] 13 -> -       (available for additional use)
     bits[16:39] 24 -> val_sz  (num bytes for key's val)
     bits[40:63] 24 -> val_max (max bytes for key's val, 0<=val_sz<=val_max)

   fd_groove_meta_bits pack the components used, val_sz and val_max into
   this bitfield.  used treats 0/non-zero as 0/1.
   val_sz and val_max are assumed in [0,2^24).

   fd_grove_meta_bits_{used,val_sz,val_max} unpack this field
   from the bitfield.  used returns a value in [0,1].
...
fd_groove_meta_bits( int   used,
                     ulong val_sz,
                     ulong val_max ) {
  return ((ulong)!!used) | (val_sz<<16) | (val_max<<40);
}
