...
test_basic( void ) {
  ...
  ag_vote_new_notar( &v, 0UL, &h, &sk, 0UL );
  ...
  FD_TEST( ag_vote_check_sig( &v, &pk ) );

  ag_vote_new_notar_fallback( &v, 1UL, &h, &sk, 2UL );
  FD_TEST( v.kind==AG_VOTE_TYPE_NOTAR_FALLBACK );
  FD_TEST( ag_vote_block_hash( &v )!=NULL );
  FD_TEST( ag_vote_check_sig( &v, &pk ) );

  ag_vote_new_skip( &v, 3UL, &sk, 0UL );
  FD_TEST( v.kind==AG_VOTE_TYPE_SKIP );
  FD_TEST( ag_vote_block_hash( &v )==NULL );
  FD_TEST( ag_vote_check_sig( &v, &pk ) );

  ag_vote_new_skip_fallback( &v, 3UL, &sk, 0UL );
  FD_TEST( v.kind==AG_VOTE_TYPE_SKIP_FALLBACK );
  FD_TEST( ag_vote_block_hash( &v )==NULL );

  ag_vote_new_final( &v, 4UL, &sk, 0UL );
...
test_payload_distinct( void ) {
  ...
  ulong sa = ag_vote_payload_bytes_to_sign( a, AG_VOTE_TYPE_NOTAR,          7UL, &h );
  ulong sb = ag_vote_payload_bytes_to_sign( b, AG_VOTE_TYPE_NOTAR_FALLBACK, 7UL, &h );
  FD_TEST( sa==sb );
  FD_TEST( memcmp( a, b, sa )!=0 );

  ulong sn = ag_vote_payload_bytes_to_sign( a, AG_VOTE_TYPE_NOTAR, 7UL, &h );
  ulong sk = ag_vote_payload_bytes_to_sign( b, AG_VOTE_TYPE_SKIP,  7UL, NULL );
  FD_TEST( sn==4UL+8UL+32UL );
  FD_TEST( sk==4UL+8UL );

  ulong s0 = ag_vote_payload_bytes_to_sign( a, AG_VOTE_TYPE_SKIP, 7UL, NULL );
  ulong s1 = ag_vote_payload_bytes_to_sign( b, AG_VOTE_TYPE_SKIP, 8UL, NULL );
  FD_TEST( s0==s1 );
...
check_wire( ag_vote_t const * v, ag_aggsig_pk_t const * pk ) {
  ...
  ulong n = ag_vote_serialize( v, out, sizeof(out) );
  FD_TEST( n>0UL );
  ...
  FD_TEST( FD_LOAD( uint,  out+off )==v->kind         ); off += 4UL; /* Vote variant tag       */
...
test_serialize( void ) {
  ...
  ag_vote_t v;
  ag_vote_new_notar         ( &v, 12345UL, &h, &sk, 7UL     ); check_wire( &v, &pk );
  ag_vote_new_notar_fallback( &v, 99UL,    &h, &sk, 65535UL ); check_wire( &v, &pk ); /* max u16 rank */
  ag_vote_new_skip          ( &v, 42UL,        &sk, 3UL     ); check_wire( &v, &pk );
  ag_vote_new_skip_fallback ( &v, 42UL,        &sk, 3UL     ); check_wire( &v, &pk );
  ag_vote_new_final         ( &v, 7UL,         &sk, 1UL     ); check_wire( &v, &pk );

  /* buffer too small -> 0 */
  uchar small[ 8 ];
  FD_TEST( ag_vote_serialize( &v, small, sizeof(small) )==0UL );
