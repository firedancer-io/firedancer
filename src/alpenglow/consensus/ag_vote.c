...
ag_vote_payload_bytes_to_sign( uchar *           out,
                               ...
                               fd_hash_t const * h ) {
  ulong o = 0UL;
  FD_STORE( uint,  out+o, kind ); o += 4UL;
...
sign_payload( ag_aggsig_sig_t *      sig,
  ...
  ulong sz = ag_vote_payload_bytes_to_sign( buf, kind, slot, h );
...
ag_notar_vote_new( ag_notar_vote_t *      out,
  ...
  sign_payload( &out->sig, AG_VOTE_TYPE_NOTAR, slot, h, sk );
}
...
ag_notar_fallback_vote_new( ag_notar_fallback_vote_t * out,
                            ...
                            ushort                     signer ) {
  out->slot = slot; out->block_hash = *h; out->signer = signer;
  sign_payload( &out->sig, AG_VOTE_TYPE_NOTAR_FALLBACK, slot, h, sk );
}
...
ag_skip_vote_new( ag_skip_vote_t *       out,
  ...
  sign_payload( &out->sig, AG_VOTE_TYPE_SKIP, slot, NULL, sk );
}
...
ag_skip_fallback_vote_new( ag_skip_fallback_vote_t * out,
                           ...
                           ushort                    signer ) {
  out->slot = slot; out->signer = signer;
  sign_payload( &out->sig, AG_VOTE_TYPE_SKIP_FALLBACK, slot, NULL, sk );
}
...
ag_final_vote_new( ag_final_vote_t *      out,
                   ...
                   ushort                 signer ) {
  out->slot = slot; out->signer = signer;
  sign_payload( &out->sig, AG_VOTE_TYPE_FINAL, slot, NULL, sk );
}
...
ag_vote_new_notar( ag_vote_t *            out,
  ...
  ag_notar_vote_new( &out->inner.notar, slot, h, sk, signer );
}
...
ag_vote_new_notar_fallback( ag_vote_t *            out,
  ...
  ag_notar_fallback_vote_new( &out->inner.notar_fallback, slot, h, sk, signer );
}
...
ag_vote_new_skip( ag_vote_t *            out,
  ...
  ag_skip_vote_new( &out->inner.skip, slot, sk, signer );
}
...
ag_vote_new_skip_fallback( ag_vote_t *            out,
  ...
  ag_skip_fallback_vote_new( &out->inner.skip_fallback, slot, sk, signer );
}
...
ag_vote_new_final( ag_vote_t *            out,
  ...
  ag_final_vote_new( &out->inner.final_, slot, sk, signer );
}
...
ag_vote_check_sig( ag_vote_t const *      self,
  ...
  case AG_VOTE_TYPE_NOTAR_FALLBACK:
    sz  = ag_vote_payload_bytes_to_sign( buf, AG_VOTE_TYPE_NOTAR_FALLBACK, self->inner.notar_fallback.slot, &self->inner.notar_fallback.block_hash );
    sig = &self->inner.notar_fallback.sig; break;
  case AG_VOTE_TYPE_SKIP:
    sz  = ag_vote_payload_bytes_to_sign( buf, AG_VOTE_TYPE_SKIP, self->inner.skip.slot, NULL );
    sig = &self->inner.skip.sig; break;
  case AG_VOTE_TYPE_SKIP_FALLBACK:
    sz  = ag_vote_payload_bytes_to_sign( buf, AG_VOTE_TYPE_SKIP_FALLBACK, self->inner.skip_fallback.slot, NULL );
