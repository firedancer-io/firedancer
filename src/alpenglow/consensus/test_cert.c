...
/* ag_block_final_cert_se produces the bytes a remote validator parses
   out of a block footer, so it has to be the exact inverse of _de.  Run
   real certs through se then de and check they come back identical --
   including the signature, which _se has to re-compress out of the
   uncompressed form the in-memory aggregate holds. */
...
test_footer_se( void ) {
  ...
  create_signers( n );
  void * em; ag_epoch_info_t * e = make_epoch( n, &em );
  fd_hash_t h; memset( h.uc, 0x37, sizeof(fd_hash_t) );

  uchar buf[ 1024 ];
  uchar out[ AG_BLOCK_FINAL_CERT_SERIALIZED_MAX ];

  for( int slow=0; slow<2; slow++ ) {
    /* Build the cert pair the way a peer's footer would deliver it, so
       the aggregates hold real (decompressed) signatures. */
    ulong off = 0UL;
    FD_STORE( ulong, buf, 7UL ); off += 8UL;
    memcpy( buf+off, h.uc, sizeof(fd_hash_t) ); off += sizeof(fd_hash_t);
    off += put_aggregate( buf+off, n, slow ? 7UL : 9UL );
    buf[ off++ ] = (uchar)slow;
    if( slow ) off += put_aggregate( buf+off, n, 7UL );

    ag_cert_t certs[ 2 ]; ulong cert_cnt;
    FD_TEST( ag_block_final_cert_de( certs, &cert_cnt, buf, off )==AG_CERT_DE_SUCCESS );
    FD_TEST( ag_block_final_cert_decompress( certs, cert_cnt )==AG_CERT_DE_SUCCESS );
    FD_TEST( cert_cnt==(slow ? 2UL : 1UL) );

    ulong out_sz = ag_block_final_cert_se( certs, cert_cnt, out, sizeof(out) );
    FD_TEST( out_sz==off );
    FD_TEST( !memcmp( out, buf, out_sz ) );
    FD_TEST( out_sz<=AG_BLOCK_FINAL_CERT_SERIALIZED_MAX );

    /* And the round trip is stable through a second parse. */
    ag_cert_t rt[ 2 ]; ulong rt_cnt;
    FD_TEST( ag_block_final_cert_de( rt, &rt_cnt, out, out_sz )==AG_CERT_DE_SUCCESS );
    FD_TEST( ag_block_final_cert_decompress( rt, rt_cnt )==AG_CERT_DE_SUCCESS );
    FD_TEST( rt_cnt==cert_cnt );
    for( ulong i=0UL; i<rt_cnt; i++ ) FD_TEST( rt[i].kind==certs[i].kind );
    FD_TEST( ag_cert_check_threshold( &rt[0], e ) );

    /* A buffer one byte short must fail rather than truncate. */
    FD_TEST( !ag_block_final_cert_se( certs, cert_cnt, out, out_sz-1UL ) );
  }

  /* Shapes _de never produces are refused rather than mis-serialized. */
  ag_cert_t bogus[ 2 ]; memset( bogus, 0, sizeof(bogus) );
  bogus[0].kind = AG_CERT_TYPE_NOTAR;
  FD_TEST( !ag_block_final_cert_se( bogus, 1UL, out, sizeof(out) ) );
  FD_TEST( !ag_block_final_cert_se( bogus, 0UL, out, sizeof(out) ) );
  FD_TEST( !ag_block_final_cert_se( bogus, 3UL, out, sizeof(out) ) );
