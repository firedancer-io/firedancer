...
static void
add_vote_helper( fd_slot_state_t * ss, fd_vote_t const * vote, fd_epoch_info_t const * ei, out_t * t ) {
...
test_safe_to_notar( fd_wksp_t * wksp ) {
  ...
  fd_vote_t notar_vote; fd_vote_new_notar( &notar_vote, slot, &hash, &g_sk[1], 1UL );
  ...
  fd_vote_t skip_vote; fd_vote_new_skip( &skip_vote, slot, &g_sk[0], 0UL );
...
test_slashable_skip_and_notarize( fd_wksp_t * wksp ) {
  ...
  fd_vote_t s1; fd_vote_new_skip( &s1, slot, &g_sk[1], 1UL );
  ...
  fd_vote_t n2; fd_vote_new_notar( &n2, slot, &hash, &g_sk[2], 2UL );
  add_vote_helper( ss, &n2, ei, &t );
  fd_vote_t skip_vote; fd_vote_new_skip( &skip_vote, slot, &g_sk[2], 2UL );
...
test_slashable_notar_different_hash( fd_wksp_t * wksp ) {
  ...
  fd_vote_t notar_a; fd_vote_new_notar( &notar_a, slot, &hash_a, &g_sk[1], 1UL );
  add_vote_helper( ss, &notar_a, ei, &t );

  /* notarizing a different hash for the same slot is slashable */
  fd_vote_t notar_b; fd_vote_new_notar( &notar_b, slot, &hash_b, &g_sk[1], 1UL );
...
test_slashable_skip_and_finalize( fd_wksp_t * wksp ) {
  ...
  fd_vote_t f1; fd_vote_new_final( &f1, slot, &g_sk[1], 1UL );
  ...
  fd_vote_t sf1; fd_vote_new_skip_fallback( &sf1, slot, &g_sk[1], 1UL );
  o = fd_slot_state_check_slashable_offence( ss, &sf1 );
  FD_TEST( o.kind==FD_SLASHABLE_SKIP_AND_FINALIZE && o.validator==1UL && o.slot==slot );

  /* skip first, then finalize is slashable */
  fd_vote_t s2; fd_vote_new_skip( &s2, slot, &g_sk[2], 2UL );
  add_vote_helper( ss, &s2, ei, &t );
  fd_vote_t f2; fd_vote_new_final( &f2, slot, &g_sk[2], 2UL );
  ...
  fd_vote_t sf3; fd_vote_new_skip_fallback( &sf3, slot, &g_sk[3], 3UL );
  add_vote_helper( ss, &sf3, ei, &t );
  fd_vote_t f3; fd_vote_new_final( &f3, slot, &g_sk[3], 3UL );
...
test_slashable_notar_fallback_and_finalize( fd_wksp_t * wksp ) {
  ...
  fd_vote_t f1; fd_vote_new_final( &f1, slot, &g_sk[1], 1UL );
  add_vote_helper( ss, &f1, ei, &t );
  fd_vote_t nf1; fd_vote_new_notar_fallback( &nf1, slot, &hash, &g_sk[1], 1UL );
  ...
  fd_vote_t nf2; fd_vote_new_notar_fallback( &nf2, slot, &hash, &g_sk[2], 2UL );
...
test_should_ignore_duplicate_votes( fd_wksp_t * wksp ) {
  ...
  fd_vote_t v1n; fd_vote_new_notar( &v1n, slot, &hash, &g_sk[1], 1UL );
...
test_count_finalize_creates_cert_at_quorum( fd_wksp_t * wksp ) {
  ...
  fd_vote_t fv4; fd_vote_new_final( &fv4, slot, &g_sk[4], 4UL );
  ...
  fd_vote_t fv5; fd_vote_new_final( &fv5, slot, &g_sk[5], 5UL );
...
test_count_notar_fallback_creates_cert_at_quorum( fd_wksp_t * wksp ) {
  ...
  fd_vote_t nf3; fd_vote_new_notar_fallback( &nf3, slot, &hash, &g_sk[3], 3UL );
  ...
  fd_vote_t nf4; fd_vote_new_notar_fallback( &nf4, slot, &hash, &g_sk[4], 4UL );
  ...
  fd_slot_state_add_cert( ss, &t.certs[0] );
  fd_vote_t nf5; fd_vote_new_notar_fallback( &nf5, slot, &hash, &g_sk[5], 5UL );
