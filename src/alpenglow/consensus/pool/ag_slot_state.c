...
ag_slot_state_add_vote( ag_slot_state_t * slot_state,
  ...
  case AG_VOTE_TYPE_NOTAR: {
    ...
    /* The signature has to be in the aggregate BEFORE the stake is
       counted, because counting is what builds the certificates and a
       certificate is built out of this very aggregate.  Counting first
       means that when a vote is the one that crosses a threshold, the
       cert is built from an aggregate that does not yet contain it: on
       the FIRST vote for a block hash the aggregate does not exist at
       all and ag_notar_fallback_cert_from_aggs trips
       FD_TEST( notar_agg || nf_agg ).

       A multi-validator cluster hides this -- the first vote is nowhere
       near quorum, and by the time one crosses it the map is populated
       -- but a single validator holding all the stake crosses every
       threshold on its own first vote.  The skip / skip-fallback /
       finalize cases below always did it in this order. */
  ...
  case AG_VOTE_TYPE_NOTAR_FALLBACK: {
    ...
    /* Same ordering requirement as AG_VOTE_TYPE_NOTAR above. */
