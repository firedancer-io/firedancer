...
ag_votor_new( void *                 shmem,
  ...
  FD_TEST( root_block_id );  /* unlike ag_pool_new, no NULL "root unknown" form */
  votor->sign                    = sign;
  votor->sign_ctx                = sign_ctx;
  votor->highest_final_cert_slot = 0UL;

  /* Bootstrap the root.  Everything at or below it is taken as decided,
     so the first real block chains off it: a mid-window slot matches its
     parent against the root's voted_notar, and a window-start slot
     against the parent_ready entry.

     The root is NOT always (0, all-zero).  The Rust reference names the
     genesis block Hash::default(), but Firedancer's genesis block id is
     whatever replay invented for it (fd_rng_secure, see
     fd_replay_tile.c ctx->initial_block_id), and after a snapshot boot
     the root is the snapshot slot.  Seeding zeros here means no block
     ever matches its parent, so nothing is ever notarized, no timeout is
     ever armed, and consensus never starts. */
  {
    ag_votor_slot_state_t * g = slot_state_mut( votor, root_slot );
    g->voted                = 1;
    g->voted_notar          = *root_block_id;
    g->block_notarized      = *root_block_id;
    g->retired              = 1;

    ag_block_id_t root_parent = { .slot = root_slot, .hash = *root_block_id };
    parent_ready_insert( votor, root_slot, &root_parent );
  }

  set_timeouts( votor, root_slot );
