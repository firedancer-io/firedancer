...
ag_pool_new( void *                      mem,
  ...
  /* Seed with the root the caller gave us whenever there IS one.  This
     used to branch on `root_slot==0`, which records the all-zero hash as
     slot 0's notar fallback -- correct only under the Rust reference's
     Hash::default() genesis convention.  Firedancer's genesis block id
     is a real (random) hash (fd_replay_tile.c ctx->initial_block_id), so
     the default seeding made slot 0's readiness unmatchable and nothing
     was ever notarized.

     A NULL hash still means "no root known yet" -- the votor tile builds
     a throwaway pool in unprivileged_init before the first slot
     completes -- and that is what the default seeding is for. */
  if( FD_LIKELY( root_block_hash ) ) ag_parent_ready_tracker_seed_root( pr, root_slot, root_block_hash );
  else                               ag_parent_ready_tracker_default  ( pr );
...
prune( ag_pool_t * self ) {
  ...
  /* Shed s2n_waiting_parent_cert entries below the root too.  An entry
     means "when parent P is notarized, tell child C", and it is only
     released by add_valid_cert when a NOTAR / NOTAR_FALLBACK cert for
     that exact P arrives.  Once P is below first_unpruned no such cert
     can still be admitted -- P's slot_state has just been dropped above
     -- so the entry is unreachable and would sit in the pool forever.

     Without this the pool leaks one element per slot in the ordinary
     case: ag_pool_add_block runs when replay finishes a block, which
     under alpenglow is well after the parent was certified, so
     add_valid_cert's removal has already run and found nothing and the
     insert below it is dead on arrival.  The pool is blockid_max
     (= max_live_slots) deep, so a validator dies of
     FD_TEST( s2n_waiting_parent_cert_pool_free(...) ) after roughly
     that many slots -- observed at slot 2049 with max_live_slots 2048. */
  for(;;) {
    int                              found = 0;
    ag_block_id_t                    drop  = {0};
    s2n_waiting_parent_cert_map_t *  m     = self->s2n_waiting_parent_cert_map;
    s2n_waiting_parent_cert_pool_t * p     = self->s2n_waiting_parent_cert_pool;
    for( s2n_waiting_parent_cert_map_iter_t it = s2n_waiting_parent_cert_map_iter_init( m, p );
         !s2n_waiting_parent_cert_map_iter_done( it, m, p );
         it = s2n_waiting_parent_cert_map_iter_next( it, m, p ) ) {
      s2n_waiting_parent_cert_t const * e = s2n_waiting_parent_cert_map_iter_ele_const( it, m, p );
      if( e->parent.slot < first_unpruned ) { drop = e->parent; found = 1; break; }
    }
    if( !found ) break;
    s2n_waiting_parent_cert_t * e = s2n_waiting_parent_cert_map_ele_remove( self->s2n_waiting_parent_cert_map, &drop, NULL, self->s2n_waiting_parent_cert_pool );
    FD_TEST( e );
    s2n_waiting_parent_cert_pool_ele_release( self->s2n_waiting_parent_cert_pool, e );
  }
