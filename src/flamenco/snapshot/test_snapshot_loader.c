...
int main( int     argc,
  ...
  fd_funk_t funk[1];
  FD_TEST( fd_funk_join( funk, fd_funk_new( fd_wksp_alloc_laddr( wksp, fd_funk_align(), fd_funk_footprint( txn_max, rec_max ), funk_tag ), funk_tag, funk_seed, txn_max, rec_max ) ) );
  ...
  fd_tpool_t * tpool = fd_tpool_init( _tpool, worker_cnt, 0UL );
  if( tpool == NULL ) {
  ...
  for( ulong i=1UL; i<worker_cnt; i++) {
    if( fd_tpool_worker_push( tpool, i ) == NULL ) {
