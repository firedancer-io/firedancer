...
static void
fd_event_runtime_stake_delegation_entry_emit( fd_bank_t const * bank,
                                              ulong             bank_seq,
                                              int               kind,
...
void
fd_event_runtime_stake_delegation_snapshot_emit( fd_bank_t const * bank,
                                                 uchar const *     stake_account,
                                                 uchar const *     vote_account,
                                                 ulong             stake,
                                                 ulong             activation_epoch,
                                                 ...
                                                 ulong             credits_observed ) {
  if( FD_LIKELY( !fd_event_tl ) ) return;
  fd_event_runtime_stake_delegation_entry_emit( bank, 0UL, FD_EVENT_RUNTIME_STAKE_DELEGATION_KIND_SNAPSHOT,
...
fd_event_runtime_stake_delegation_payout_emit( fd_bank_t const * bank,
                                               ...
                                               ulong             credits_observed ) {
  if( FD_LIKELY( !fd_event_tl ) ) return;
  fd_event_runtime_stake_delegation_entry_emit( bank, bank->bank_seq, FD_EVENT_RUNTIME_STAKE_DELEGATION_KIND_REWARD,
                                                stake_account, vote_account, stake,
                                                activation_epoch, deactivation_epoch, credits_observed );
}
...
fd_event_runtime_rooted_emit( fd_bank_t const *                          bank,
                              ...
                              fd_new_votes_delta_stats_t const *         new_votes_delta_stats ) {
  if( FD_LIKELY( !fd_event_tl ) ) return;
  ...
  ev.stake_delegations_cnt     = fd_stake_delegations_cnt( stake_delegations );
  ...
  ev.new_votes_inserts         = new_votes_delta_stats->inserts;
  ev.new_votes_removes         = new_votes_delta_stats->removes;
