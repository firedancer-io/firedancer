...
fd_event_runtime_epoch_vat_burn( ulong burn_per_vote_account ) {
  if( FD_LIKELY( !fd_event_tl ) ) return;
  fd_event_runtime_epoch_summary.vat_burn_per_vote_account = burn_per_vote_account;
}

void
fd_event_runtime_epoch_feature( uchar const * feature_id ) {
  if( FD_LIKELY( !fd_event_tl ) ) return;
  if( FD_UNLIKELY( fd_event_runtime_epoch_summary.feature_cnt>=16UL ) ) {
    FD_LOG_WARNING(( "runtime_epoch feature_activations overflow (cap 16), dropping activation" ));
    return;
  }
...
fd_event_runtime_epoch_emit( fd_bank_t const * bank ) {
  ...
  ev.stake_history_sysvar = fd_event_runtime_epoch_summary.stake_history;

  ev.epoch_schedule_sysvar.slots_per_epoch             = bank->f.epoch_schedule.slots_per_epoch;
  ev.epoch_schedule_sysvar.leader_schedule_slot_offset = bank->f.epoch_schedule.leader_schedule_slot_offset;
  ev.epoch_schedule_sysvar.warmup                      = !!bank->f.epoch_schedule.warmup;
  ev.epoch_schedule_sysvar.first_normal_epoch          = bank->f.epoch_schedule.first_normal_epoch;
  ev.epoch_schedule_sysvar.first_normal_slot           = bank->f.epoch_schedule.first_normal_slot;
  ...
  ev.epoch_rewards_sysvar = fd_event_runtime_epoch_summary.epoch_rewards;

  ev.vat_burn_per_vote_account = fd_event_runtime_epoch_summary.vat_burn_per_vote_account;
...
fd_event_runtime_rooted_emit( fd_bank_t const *                          bank,
  ...
  ev.effective_stake           = totals.effective;
  ev.activating_stake          = totals.activating;
  ev.deactivating_stake        = totals.deactivating;
...
FD_STATIC_ASSERT( sizeof (fd_event_runtime_slot_diffs_t)<=FD_EVENT_RUNTIME_SLOT_DIFFS_FOOTPRINT, event_slot_diffs_fit );
FD_STATIC_ASSERT( alignof(fd_event_runtime_slot_diffs_t)<=8UL,                                   event_slot_diffs_align );

static FD_TL uchar * fd_event_runtime_slot_diffs_mem;
static FD_TL ulong   fd_event_runtime_slot_diffs_max;

void
fd_event_runtime_slot_diffs_init( void * mem,
                                  ulong  bank_max ) {
  fd_memset( mem, 0, bank_max*FD_EVENT_RUNTIME_SLOT_DIFFS_FOOTPRINT );
  fd_event_runtime_slot_diffs_mem = mem;
  fd_event_runtime_slot_diffs_max = bank_max;
}

static inline fd_event_runtime_slot_diffs_t *
fd_event_runtime_slot_diffs_at( ulong bank_idx ) {
  if( FD_UNLIKELY( !fd_event_runtime_slot_diffs_mem || bank_idx>=fd_event_runtime_slot_diffs_max ) ) return NULL;
  return fd_type_pun( fd_event_runtime_slot_diffs_mem + bank_idx*FD_EVENT_RUNTIME_SLOT_DIFFS_FOOTPRINT );
}

void
fd_event_runtime_slot_diffs_reset( ulong bank_idx ) {
  fd_event_runtime_slot_diffs_t * diffs = fd_event_runtime_slot_diffs_at( bank_idx );
  if( FD_UNLIKELY( !diffs ) ) return;
  diffs->sysvar_cnt = 0UL;
  diffs->other_cnt  = 0UL;
}
...
fd_event_runtime_block_account( fd_bank_t *   bank,
  ...
  /* Coalesce repeated writes to one account into a single diff: first
...
fd_event_runtime_block_emit( fd_bank_t const *             bank,
  ...
  static FD_TL fd_event_runtime_block_t ev;
  ...
  fd_blake3_hash( lthash->bytes, FD_LTHASH_LEN_BYTES, ev.accounts_lt_hash_checksum );
  ...
  fd_runtime_fee_split( execution_fees, priority_fees, &fees_burned, &leader_fee_reward );

  ev.execution_fees    = execution_fees;
  ev.priority_fees     = priority_fees;
  ev.tips              = tips;
  ev.fees_burned       = fees_burned;
  ev.leader_fee_reward = leader_fee_reward;
  ...
  ev.clock_sysvar.slot                  = clock->slot;
  ev.clock_sysvar.epoch_start_timestamp = clock->epoch_start_timestamp;
  ev.clock_sysvar.epoch                 = clock->epoch;
  ev.clock_sysvar.leader_schedule_epoch = clock->leader_schedule_epoch;
  ev.clock_sysvar.unix_timestamp        = clock->unix_timestamp;
