...
/* Emit a runtime_stake_delegation remove event for a non-transaction
   removal (e.g. an epoch-boundary prune).  Only the stake account is set
   (no signature, index_in_slot = UInt64 max).  No-op when the tile has
   no event link. */
