...
#define REQUIRE(c)                      \
  ...
  */
  ...
  do
  {
    fd_stake_config_t config[1] = {{.config_keys_len = 0,
  ...
  genesis->accounts_len += feature_cnt;
...
ulong fd_genesis_blob_size(fd_genesis_options_t const *options)
{
  ...
  sz += 8;
  // accounts
#define ACCT_OVERHEAD 89UL
  // faucet
  sz += ACCT_OVERHEAD;
  // identity
  sz += ACCT_OVERHEAD;
  // vote
  sz += ACCT_OVERHEAD + FD_VOTE_STATE_V3_SZ;
  // stake
  sz += ACCT_OVERHEAD + FD_STAKE_STATE_V2_SZ;
  // stake_config
  sz += ACCT_OVERHEAD + 10UL;
  // funded accounts
  sz += funded_cnt * (ACCT_OVERHEAD);
  // feature gate accounts
  sz += feature_cnt * (ACCT_OVERHEAD + 9UL);
#undef ACCT_OVERHEAD
  ...
  // poh_config
  sz += 9;
  sz += (options->hashes_per_tick ? 9 : 1);
  sz += 8;
  sz += 4;
