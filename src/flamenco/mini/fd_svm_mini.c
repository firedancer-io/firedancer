...
create_view( fd_svm_mini_t * mini,
  ...
  view->mini          = mini;
  view->bank          = bank;
  view->runtime_stack = NULL;
  fd_accdb_mini_join_user    ( mini->accdb_mini,     view->accdb     );
  fd_progcache_mini_join_user( mini->progcache_mini, view->progcache );
  fd_funk_txn_xid_set_root( &view->xid );
...
fd_svm_mini_create( fd_svm_mini_limits_t const * user_limits,
  ...
  if( !limits.max_fork_nodes     ) limits.max_fork_nodes     = fd_svm_mini_limits_default.max_fork_nodes;
  ...
  banks_mem = aligned_alloc( fd_banks_align(), fd_banks_footprint( limits.max_fork_nodes, limits.max_live_slots ) );
  if( FD_UNLIKELY( !banks_mem ) ) goto oom;
  svm->banks = fd_banks_join( fd_banks_new( banks_mem, limits.max_fork_nodes, limits.max_live_slots, 0, 1UL ) );
  ...
  fd_funk_txn_xid_t xid_base; fd_funk_txn_xid_set_root( &xid_base );
  fd_funk_txn_xid_t xid_new = { .ul={ root_slot, ULONG_MAX } };
  fd_accdb_attach_child        ( svm->accdb_admin,     &xid_base, &xid_new );
  fd_accdb_advance_root        ( svm->accdb_admin,                &xid_new );
  fd_progcache_txn_attach_child( svm->progcache_admin, &xid_base, &xid_new );
  fd_progcache_txn_advance_root( svm->progcache_admin,            &xid_new );

  /* Create reference */

  fd_svm_view_t * root_view = create_view( svm, root_bank );
  if( FD_UNLIKELY( !root_view ) ) goto oom;
  root_view->xid = xid_new;
...
fd_svm_mini_join_root( fd_svm_mini_t * svm ) {
  fd_bank_t * root_bank = fd_banks_root( svm->banks );
  return create_view( svm, root_bank );
}
...
fd_svm_view_fork( fd_svm_view_t * view,
  ...
  /* Create account database transaction */
...
fd_svm_view_freeze( fd_svm_view_t * view ) {
  fd_banks_t * banks = view->mini->banks;
  fd_bank_t *  bank  = view->bank;
...
fd_svm_view_advance_root( fd_svm_view_t * view ) {
  ...
  fd_svm_mini_t * mini  = view->mini;
  fd_banks_t *    banks = mini->banks;
  fd_bank_t *     bank  = view->bank;
  if( FD_UNLIKELY( bank->refcnt!=1UL ) ) {
    FD_LOG_CRIT(( "attempted to root SVM bank while there are still references" ));
  }
  ulong advanceable_idx;
  if( FD_UNLIKELY( !fd_banks_advance_root_prepare( banks, bank->idx, &advanceable_idx ) ) ) {
    FD_LOG_CRIT(( "invalid 'advance root' request" ));
  }

  /* Advance account database */

  fd_progcache_txn_advance_root( view->mini->progcache_admin, &view->xid );
  fd_accdb_advance_root        ( view->mini->accdb_admin,     &view->xid );

  /* Advance root */

  fd_banks_advance_root( banks, bank->idx );
}
