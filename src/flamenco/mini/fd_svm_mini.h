...
struct fd_svm_mini_limits {
  ...
  ulong max_fork_nodes;      /* max number of fork graph nodes */
...
/* fd_svm_view_advance_root advances an svm_mini's root slot to node
   ...
   the old root.  Asserts that the node is marked as frozen.  This does
   *not* free the rooted view.  It is the caller's responsibility to
   call svm_view_delete. */
