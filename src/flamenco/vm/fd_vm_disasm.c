...
int
fd_vm_disasm_instr( uchar const *              text,
                    ulong                      text_cnt,
                    ulong                      pc,
                    fd_sbpf_syscalls_t const * syscalls,
                    char *                     out,
  ...
  case FD_SBPF_OPCODE_CLASS_LD: {
    ...
    fd_sbpf_instr_t i1 = fd_sbpf_instr( FD_LOAD( ulong, text+8UL ) );
...
int
fd_vm_disasm_program( uchar const *              text,
                      ulong                      text_cnt,
                      fd_sbpf_syscalls_t const * syscalls,
                      char *                     out,
                      ulong                      out_max,
  ...
  for( ulong i=0UL; i<text_cnt; i++ ) {
    ...
    OUT_PRINTF( "    " );
    int err = fd_vm_disasm_instr( text+8UL*i, text_cnt-i, i, syscalls, out, out_max, _out_len );
