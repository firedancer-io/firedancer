...
# undef TEST
  ...
  FD_TEST( fd_vm_trace_event_exe( NULL, reg[0UL] & 0xffffUL, reg[1UL] & 0xffffUL, reg[2UL], reg+3UL, text, 2UL, 6UL, 4UL, 0UL, 0UL )==FD_VM_ERR_INVAL );
  ...
  for(;;) {
    ...
    case 0: { /* exe */
      ...
      text[0] = fd_rng_ulong( rng );
      text[1] = fd_rng_ulong( rng );
      int err = fd_vm_trace_event_exe( trace, reg[0UL] & 0xffffUL, reg[1UL] & 0xffffUL, reg[2UL], reg+3UL, text, 2UL, 7UL, 4UL, 0UL, 0UL );
