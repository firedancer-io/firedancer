...
fd_vm_syscall_sol_blake3( /**/            void *  _ctx,
  ...
  for( ulong i=0UL; i<slices_cnt; i++ ) {
    ...
    ulong cost = fd_ulong_max( vm_compute_budget.mem_op_base_cost,
                               fd_ulong_sat_mul( vm_compute_budget.sha256_byte_cost, slices[i].len ) / 2UL );
    int err = fd_vm_consume_compute( ctx, cost );
    if( FD_UNLIKELY( err ) ) return err;

    fd_blake3_append( blake, slice, slices[i].len );
  }
...
fd_vm_syscall_sol_keccak256( /**/            void *  _ctx,
  ...
  if( FD_LIKELY( slices_cnt ) ) {
    ...
    for( ulong i=0UL; i<slices_cnt; i++ ) {
      ...
      fd_keccak256_append( keccak, slice, slices[i].len );
    }
  }
