...
/* SIMD-0529 operation cost, i.e. what is charged on top of the flat
   FD_BIG_MOD_EXP_BASE_CU. */
static ulong
fd_big_mod_exp_operation_cost( uchar const * exp,
                               ulong         exp_len,
                               ulong         base_len,
                               ulong         mod_len ) {
  ...
  return (complexity + FD_BIG_MOD_EXP_CU_DIVISOR - 1UL) / FD_BIG_MOD_EXP_CU_DIVISOR;
}
...
fd_vm_syscall_sol_big_mod_exp( /**/  void *  _vm,
  ...
  /* Charge the flat base cost up front, before any translation that could
     fail without being paid for. */
  FD_VM_CU_UPDATE( vm, FD_BIG_MOD_EXP_BASE_CU );

  /* Read the params record (48B, 8-byte aligned). */
  ...
  /* Length validation rejects only operands over the maximum; mod_len==0 is
  ...
  /* Translate the exponent and charge before translating the base and modulus. */
  uchar const * exp = exp_len ? FD_VM_MEM_HADDR_LD( vm, params->exponent, FD_VM_ALIGN_RUST_U8, exp_len ) : (uchar const *)"";

  FD_VM_CU_UPDATE( vm, fd_big_mod_exp_operation_cost( exp, exp_len, base_len, mod_len ) );

  uchar const * base = base_len ? FD_VM_MEM_HADDR_LD( vm, params->base,    FD_VM_ALIGN_RUST_U8, base_len ) : (uchar const *)"";
  uchar const * mod  = mod_len  ? FD_VM_MEM_HADDR_LD( vm, params->modulus, FD_VM_ALIGN_RUST_U8, mod_len  ) : (uchar const *)"";

  /* An empty, zero, one or even modulus fails here, having been charged. */
