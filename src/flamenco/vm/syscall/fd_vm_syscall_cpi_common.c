...
VM_SYSCALL_CPI_UPDATE_CALLER_ACC_FUNC( fd_vm_t *                          vm,
  ...
  if( !vm->direct_mapping ) {
    ...
    *(caller_account->lamports) = fd_txn_account_get_lamports( callee_acc );
    ...
    fd_memcpy( caller_acc_data, fd_txn_account_get_acc_data( callee_acc ), updated_data_len );
  } else { /* Direct mapping enabled */
    ...
    *(caller_account->lamports) = fd_txn_account_get_lamports( callee_acc );
    ...
    if( original_len && vm->input_mem_regions[ acc_region_idx ].haddr!=(ulong)fd_txn_account_get_acc_data_mut( callee_acc ) ) {
      vm->input_mem_regions[ acc_region_idx ].haddr = (ulong)fd_txn_account_get_acc_data_mut( callee_acc );
      zero_all_mapped_spare_capacity = 1;
    }
    ...
    if( FD_UNLIKELY( spare_len ) ) {
      if( fd_txn_account_get_data_len( callee_acc )>spare_len ) {
        memset( fd_txn_account_get_acc_data_mut( callee_acc ) + fd_txn_account_get_data_len( callee_acc ) - spare_len, 0, spare_len );
      }
    }
    ...
    if( realloc_bytes_used && !vm->is_deprecated ) {
      ...
      uchar * from_slice = fd_txn_account_get_acc_data_mut( callee_acc ) + original_len;
