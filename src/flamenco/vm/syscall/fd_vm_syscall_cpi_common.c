...
VM_SYSCALL_CPI_INSTRUCTION_TO_INSTR_FUNC( fd_vm_t * vm,
  ...
  fd_pubkey_t * txn_accs = vm->instr_ctx->txn_ctx->account_keys;
...
VM_SYCALL_CPI_UPDATE_CALLEE_ACC_FUNC( fd_vm_t *                          vm,
  ...
  /* resize to 0 bytes */
  fd_txn_account_resize( callee_acc.acct, 0 );
  ...
  if( vm->direct_mapping ) {
    for( ulong i=0UL; i<vm->instr_ctx->instr->acct_cnt; i++ ) {
      ...
      if( fd_instr_acc_is_writable_idx( vm->instr_ctx->instr, i ) ) {
        ...
        fd_guarded_borrowed_account_t caller_acc;
        err = fd_exec_instr_ctx_try_borrow_account( vm->instr_ctx, i, &caller_acc );
        if( FD_UNLIKELY( err ) ) return err;

        uchar is_writable = !!fd_borrowed_account_can_data_be_changed( &caller_acc, &err );
