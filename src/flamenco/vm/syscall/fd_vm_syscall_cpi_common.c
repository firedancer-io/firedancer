...
VM_SYSCALL_CPI_TRANSLATE_AND_UPDATE_ACCOUNTS_FUNC(
  ...
  for( ulong i=0UL; i<instruction_accounts_cnt; i++ ) {
    ...
    for( ushort j=0; j<account_infos_length && !found; j++ ) {
      ...
      /* https://github.com/anza-xyz/agave/blob/v3.0.1/syscalls/src/cpi.rs#L205-L210 */
      ...
      /* https://github.com/anza-xyz/agave/blob/v3.0.1/syscalls/src/cpi.rs#L214-L221 */
      if( FD_LIKELY( vm->stricter_abi_and_runtime_constraints ) ) {
        #ifdef VM_SYSCALL_CPI_ACC_INFO_DATA_LEN_VADDR
      ...
      /* https://github.com/anza-xyz/agave/blob/v3.0.1/syscalls/src/cpi.rs#L222-L223 */
      ...
      /* TODO: check that serialized_data and serialized_data_len are set correctly */
      ...
      if( vm->stricter_abi_and_runtime_constraints && vm->direct_mapping ) {
        /* https://github.com/anza-xyz/agave/blob/v3.0.1/syscalls/src/cpi.rs#L97-L99 */
        caller_account->serialized_data     = NULL; /* TODO: make sure we handle this corretly later */
        ...
        /* Skip translation checks here
           https://github.com/anza-xyz/agave/blob/v3.0.1/syscalls/src/cpi.rs#L99-L115 */
        ...
        caller_account->serialized_data_len = data_vaddr_len; /* FIXME?? */
      } else {
...
VM_SYSCALL_CPI_UPDATE_CALLER_ACC_FUNC( fd_vm_t *                          vm,
  ...
  *(caller_account->lamports) = fd_txn_account_get_lamports( callee_acc );
  ...
  /* Update the caller account data with the value from the callee
     https://github.com/anza-xyz/agave/blob/v3.0.1/syscalls/src/cpi.rs#L1194-L1267 */
  VM_SYSCALL_CPI_ACC_INFO_DATA( vm, caller_acc_info, caller_acc_data );
  (void)caller_acc_data_len;

  /* https://github.com/anza-xyz/agave/blob/v3.0.1/syscalls/src/cpi.rs#L1194-L1195 */
  ulong prev_len = *caller_account->ref_to_len_in_vm.translated;
  ...
  /* https://github.com/anza-xyz/agave/blob/v3.0.1/syscalls/src/cpi.rs#L1218-L1252 */
  if( prev_len != post_len ) {
    /* https://github.com/anza-xyz/agave/blob/v3.0.1/syscalls/src/cpi.rs#L1221-L1239 */
    /* FIXME: we need to zero out the memory that is no longer being used if the account shrunk in size */
    caller_acc_data = FD_VM_MEM_SLICE_HADDR_ST( vm, caller_acc_data_vm_addr, alignof(uchar), post_len );
    /* FIXME: handle special case where stricter_abi_and_runtime_constraints is true and direct_mapping is true (logic from get_serialized_data) */

    /* https://github.com/anza-xyz/agave/blob/v3.0.1/syscalls/src/cpi.rs#L1240-L1241 */
    ulong * ref_to_len = caller_account->ref_to_len_in_vm.translated;
    *ref_to_len        = post_len;

    /* https://github.com/anza-xyz/agave/blob/v3.0.1/syscalls/src/cpi.rs#L1243-L1251 */
  ...
  if( !(vm->stricter_abi_and_runtime_constraints && vm->direct_mapping) ) {
    fd_memcpy( caller_acc_data, fd_txn_account_get_data( callee_acc ), post_len );
  }

  /* Update the caller account region
     https://github.com/anza-xyz/agave/blob/v3.0.1/syscalls/src/cpi.rs#L1150-L1166 */
  if( vm->stricter_abi_and_runtime_constraints && address_space_reserved_for_account > 0UL ) {
    ...
    region->region_sz   = (uint)fd_borrowed_account_get_data_len( &borrowed_callee_acc );

    /* https://github.com/anza-xyz/agave/blob/master/program-runtime/src/serialization.rs#L28-L34 */
    int err;
    int is_writable = fd_borrowed_account_can_data_be_changed( &borrowed_callee_acc, &err );
    if( err ) {
      FD_VM_ERR_FOR_LOG_INSTR( vm, FD_EXECUTOR_INSTR_ERR_INVALID_ARG );
      return FD_EXECUTOR_INSTR_ERR_INVALID_ARG;
    }
    region->is_writable = (uchar)is_writable;
  }
