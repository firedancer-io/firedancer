...
VM_SYSCALL_CPI_INSTRUCTION_TO_INSTR_FUNC( fd_vm_t *                         vm,
  ...
  int program_id_idx = fd_exec_txn_ctx_find_index_of_account( vm->instr_ctx->txn_out, program_id );
  ...
  for( ushort i=0; i<VM_SYSCALL_CPI_INSTR_ACCS_LEN( cpi_instr ); i++ ) {
    ...
    int idx_in_txn    = fd_exec_txn_ctx_find_index_of_account( vm->instr_ctx->txn_out, pubkey );
...
VM_SYSCALL_CPI_ENTRYPOINT( void *  _vm,
  ...
  int err_exec = fd_execute_instr( vm->instr_ctx->runtime, vm->instr_ctx->bank, vm->instr_ctx->txn_in, vm->instr_ctx->txn_out, vm->instr_ctx->txn_ctx, instruction_to_execute );
