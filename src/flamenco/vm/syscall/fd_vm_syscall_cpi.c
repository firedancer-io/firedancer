...
fd_vm_prepare_instruction( fd_instr_info_t *        callee_instr,
  ...
  for( ulong i = 0; i < deduplicated_instruction_accounts_cnt; i++ ) {
    ...
    if( FD_UNLIKELY( instruction_account->is_writable && !fd_borrowed_account_is_writable( &borrowed_caller_acct ) ) ) {
      FD_BASE58_ENCODE_32_BYTES( fd_borrowed_account_get_pubkey( &borrowed_caller_acct )->uc, id_b58 );
    ...
    if ( FD_UNLIKELY( instruction_account->is_signer && !( fd_borrowed_account_is_signer( &borrowed_caller_acct ) || fd_vm_syscall_cpi_is_signer( fd_borrowed_account_get_pubkey( &borrowed_caller_acct ), signers, signers_cnt) ) ) ) {
      FD_BASE58_ENCODE_32_BYTES( fd_borrowed_account_get_pubkey( &borrowed_caller_acct )->uc, id_b58 );
