...
fd_vm_prepare_instruction( fd_instr_info_t *        callee_instr,
  ...
  for( ulong i=0UL; i<callee_instr->acct_cnt; i++ ) {
    ...
    if( index_in_transaction == USHORT_MAX ) {
      ...
      fd_log_collector_msg( instr_ctx, "Instruction references an unknown account", 42UL );
  ...
  int err = fd_exec_txn_ctx_get_key_of_account_at_index(instr_ctx->txn_ctx, callee_instr->program_id, &callee_program_id_pubkey );
  if( FD_UNLIKELY( err ) ) {
