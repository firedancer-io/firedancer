...
fd_vm_prepare_instruction( fd_instr_info_t const *  caller_instr,
  ...
  if( FD_LIKELY( FD_FEATURE_ACTIVE( instr_ctx->txn_ctx->slot_ctx, lift_cpi_caller_restriction ) ) ) {
    ...
    if( FD_UNLIKELY( !FD_FEATURE_ACTIVE( instr_ctx->txn_ctx->slot_ctx, remove_accounts_executable_flag_checks ) ) ) {
