  ...
  if( FD_LIKELY( trace_entry != NULL ) ) {
    ...
    if( ( result_meta_haddr->data_len == trace_entry->instr_info->data_sz ) &&
      ...
      /* Get the instruction info's program id pubkey. Note we have to use the transaction context's
         get_key_of_account_at_index function because we don't have access to the instruction context.
      ...
      int err = fd_exec_txn_ctx_get_key_of_account_at_index( vm->instr_ctx->txn_ctx, instr_info->program_id, &isntr_info_program_id_key );
      if( FD_UNLIKELY( err ) ) {
        FD_LOG_WARNING(("unable to get last program id in get processed sibling syscall!"));
        FD_VM_ERR_FOR_LOG_INSTR( vm, err );
        return err;
      }
