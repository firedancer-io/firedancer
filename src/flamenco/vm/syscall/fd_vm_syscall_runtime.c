...
fd_vm_syscall_sol_get_clock_sysvar( /**/            void *  _vm,
  ...
  FD_VM_TRANSLATE_MUT_N( vm, 1UL, &var_query, NULL, NULL, NULL, NULL );
...
fd_vm_syscall_sol_get_return_data( /**/            void *  _vm,
  ...
  if( FD_LIKELY( length ) ) {
    ...
    FD_VM_TRANSLATE_MUT_N( vm, 2UL, &return_data_query, &program_id_query, NULL, NULL, NULL );
...
fd_vm_syscall_sol_get_processed_sibling_instruction(
  ...
  if( FD_LIKELY( found_instruction_context != NULL ) ) {
    ...
    FD_VM_TRANSLATE_MUT_N( vm, 1UL, &result_header_query, NULL, NULL, NULL, NULL );
    ...
    if( result_header->data_len==found_instruction_context->instr_info->data_sz &&
        result_header->accounts_len==found_instruction_context->instr_info->acct_cnt ) {
      ...
      FD_VM_TRANSLATE_MUT_N( vm, 4UL, &program_id_query, &data_query, &accounts_query, &result_header_query, NULL );
      ...
      int err = fd_exec_instr_ctx_get_last_program_key( vm->instr_ctx,
