...
fd_vm_syscall_sol_get_epoch_stake( /**/            void *  _vm,
  ...
  fd_vote_states_t const *    vote_states    = fd_bank_vote_states_locking_query( vm->instr_ctx->bank );
  fd_vote_state_ele_t const * vote_state_ele = fd_vote_states_query_const( vote_states, vote_address );
  *_ret = vote_state_ele ? vote_state_ele->stake_t_1 : 0UL;
  fd_bank_vote_states_end_locking_query( vm->instr_ctx->bank );
