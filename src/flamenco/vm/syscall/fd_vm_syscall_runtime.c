...
fd_vm_syscall_sol_get_epoch_stake( /**/            void *  _vm,
  ...
  fd_vote_states_t const * vote_states = fd_bank_vote_states_locking_query( vm->instr_ctx->txn_ctx->bank );
  *_ret = fd_query_pubkey_stake( vote_address, vote_states );
  fd_bank_vote_states_end_locking_query( vm->instr_ctx->txn_ctx->bank );
