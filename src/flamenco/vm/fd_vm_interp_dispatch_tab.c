...
BRANCH_PRE_CODE
  if ( (ulong)(pc + (int)instr.imm + 1L) < ctx->text_cnt ) {
    ...
    cond_fault = 0;
    fd_vm_stack_push( ctx, (ulong)pc, &register_file[6] ); // FIXME: stack overflow fault
    pc += (int)instr.imm;
  } else {
...
BRANCH_PRE_CODE
{
  ...
  // FIXME: DO STACK STUFF correctly: move this r10 manipulation on success.
  register_file[10] += 0x2000; // FIXME: MAGIC NUMBER
  cond_fault = fd_vm_stack_push( ctx, (ulong)pc, &register_file[6] ); // FIXME: stack overflow fault
...
BRANCH_PRE_CODE
  ...
  if( FD_UNLIKELY( fd_vm_stack_is_empty( ctx ) ) ) {
    if( due_insn_cnt > previous_instruction_meter ) goto interp_fault;
    goto JT_RET_LOC;
  }
  fd_vm_stack_pop( ctx, (ulong *)&pc, &register_file[6] );
BRANCH_POST_CODE
JT_CASE_END
