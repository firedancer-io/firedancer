...
fd_vote_timestamps_footprint( ulong max_live_slots,
  ...
  l = FD_LAYOUT_APPEND( l, alignof(snapshot_ele_t),               sizeof(snapshot_ele_t)*max_vote_accs*max_snaps );
...
fd_vote_timestamps_new( void * shmem,
  ...
  for( uchar i=0; i<max_snaps; i++ ) {
    snapshot_key_ele_t * key = snapshot_key_pool_ele_acquire( snapshot_keys_pool_mem );
    ...
    snapshot_key_pool_ele_release( snapshot_keys_pool_mem, key );
  }
...
fd_vote_timestamps_attach_child( fd_vote_timestamps_t * vote_ts,
  ...
  if( FD_UNLIKELY( parent_fork_idx==USHORT_MAX ) ) {
    FD_CRIT( vote_ts->root_idx==USHORT_MAX, "Root already exists" );
    child->parent_idx = USHORT_MAX;
    vote_ts->root_idx = child_idx;
  } else {
