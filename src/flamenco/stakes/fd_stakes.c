...
deserialize_and_update_vote_account( fd_exec_slot_ctx_t *                slot_ctx,
  ...
  if( FD_UNLIKELY( fd_txn_account_init_from_funk_readonly( vote_account, vote_account_pubkey, slot_ctx->funk, slot_ctx->funk_txn ) ) ) {
...
accumulate_stake_cache_delegations_tpool( void  *tpool,
  ...
  FD_SPAD_FRAME_BEGIN( spad ) {
    for( fd_delegation_pair_t_mapnode_t * n =  delegations_roots[worker_idx];
      ...
      int rc = fd_txn_account_init_from_funk_readonly( acc, &n->elem.account, slot_ctx->funk, slot_ctx->funk_txn );
      if( FD_UNLIKELY( rc!=FD_FUNK_ACC_MGR_SUCCESS || acc->const_meta->info.lamports==0UL ) ) {
        continue;
      }
