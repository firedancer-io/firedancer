...
get_vote_credits( uchar const *        account_data,
  ...
  ulong credits_cnt;
  fd_vote_epoch_credits_t const * vote_epoch_credits = fd_vote_account_epoch_credits( account_data, account_data_len, &credits_cnt );
  FD_TEST( vote_epoch_credits );
  FD_TEST( credits_cnt<=FD_EPOCH_CREDITS_MAX );
  epoch_credits->cnt = (uchar)credits_cnt;
