...
static fd_acc_lamports_t get_minimum_delegation( fd_exec_slot_ctx_t const * slot_ctx ) {
...
int authorized_check_signers(fd_exec_instr_ctx_t* ctx, uchar const * instr_acc_idxs, fd_pubkey_t * txn_accs, fd_pubkey_t * staker) {
...
static int
deactivate(fd_stake_state_t* stake_state, fd_pubkey_t const * stake_acc, fd_exec_instr_ctx_t* ctx, ulong epoch) {
  ...
  int result = write_stake_state(ctx->slot_ctx, stake_acc, stake_state, 0);
...
static int
validate_delegated_amount( fd_exec_instr_ctx_t* ctx, fd_acc_lamports_t account_lamports, fd_acc_lamports_t rent_exempt_reserve, fd_acc_lamports_t * stake_amount) {
  *stake_amount = fd_ulong_sat_sub( account_lamports, rent_exempt_reserve);
  if (*stake_amount < get_minimum_delegation(ctx->slot_ctx)) {
...
write_stake_config( fd_exec_slot_ctx_t *      slot_ctx,
  ...
  uchar *             acc_data = NULL;
  int err = fd_acc_mgr_modify_old( slot_ctx->acc_mgr, slot_ctx->funk_txn, acc_key, 1, data_sz, NULL, NULL, &acc_meta, &acc_data );
  FD_TEST( !err );
...
// TODO: needs to be some sort of "db context"
int
read_stake_config( fd_exec_slot_ctx_t * slot_ctx,
                   fd_stake_config_t * result ) {

  int read_result = 0;
  uchar const * acc_rec = fd_acc_mgr_view_raw( slot_ctx->acc_mgr, slot_ctx->funk_txn, &fd_solana_stake_program_config_id, NULL, &read_result );
...
static int validate_split_amount(
    ...
    int err = fd_acc_mgr_view_old( ctx.acc_mgr, ctx.funk_txn, source_acc, NULL, &metadata_source, NULL );
    FD_TEST( !err );
    ...
    err = fd_acc_mgr_view_old( ctx.acc_mgr, ctx.funk_txn, dest_acc, NULL, &metadata_dest, NULL );
    FD_TEST( !err );
    ...
    read_stake_state( ctx.slot_ctx, metadata_source, &source_state );
    ...
    *source_remaining_balance = source_lamports - lamports;
    ...
    if (FD_FEATURE_ACTIVE(ctx.slot_ctx, stake_split_uses_rent_sysvar)) {
      *destination_rent_exempt_reserve = fd_rent_exempt_minimum_balance(ctx.slot_ctx, destination_data_len);
    } else {
    ...
    if (
      !FD_FEATURE_ACTIVE(ctx.slot_ctx, clean_up_delegation_errors) &&
...
static int merge_delegation_stake_and_credits_observed( fd_exec_slot_ctx_t * slot_ctx, fd_stake_state_t* stake_state, fd_acc_lamports_t absorbed_lamports, fd_acc_lamports_t absorbed_credits_observed) {
  if (FD_FEATURE_ACTIVE(slot_ctx, stake_merge_with_unmatched_credits_observed)) {
...
static int new_warmup_cooldown_rate_epoch( fd_exec_instr_ctx_t* ctx, ulong * result ) {
    if (FD_FEATURE_ACTIVE(ctx->slot_ctx, reduce_stake_warmup_cooldown)) {
      fd_epoch_schedule_t epoch_schedule;
      fd_sysvar_epoch_schedule_read(ctx->slot_ctx, &epoch_schedule);
      ulong slot = ctx->epoch_ctx->features.reduce_stake_warmup_cooldown;
...
static int get_if_mergeable( fd_exec_instr_ctx_t* ctx, fd_stake_state_t* stake_state, fd_sol_sysvar_clock_t clock, fd_stake_history_t history, fd_merge_kind_t* merge_kind) {
...
int authorize(fd_exec_instr_ctx_t ctx,
  ...
  if ( fd_stake_authorize_is_staker( stake_authorize ) ) {
    ...
    int write_result = write_stake_state(ctx.slot_ctx, stake_acc, &stake_state, 0);
...
int fd_executor_stake_program_execute_instruction( fd_exec_instr_ctx_t ctx ) {
  ...
  int read_result = fd_acc_mgr_view_old( ctx.acc_mgr, ctx.funk_txn, stake_acc, &stake_acc_ro, &stake_acc_meta, &stake_acc_data );
  if( FD_UNLIKELY( read_result!=FD_ACC_MGR_SUCCESS ))
    return read_result;
  if( FD_UNLIKELY( 0!=memcmp( stake_acc_meta->info.owner, fd_solana_stake_program_id.key, sizeof(fd_pubkey_t) ) ) )
  ...
  if ( fd_stake_instruction_is_initialize( &instruction ) ) {
    ...
    if ( memcmp( &txn_accs[instr_acc_idxs[1]], fd_sysvar_rent_id.key, sizeof(fd_pubkey_t) ) != 0 ) {
    ...
    ulong minimum_rent_exempt_balance = fd_rent_exempt_minimum_balance( ctx.slot_ctx, stake_acc_meta->dlen );
    ulong minimum_balance;
    if ( FD_FEATURE_ACTIVE(ctx.slot_ctx, stake_allow_zero_undelegated_amount) ) {
      minimum_balance = minimum_rent_exempt_balance;
    } else {
      minimum_balance = get_minimum_delegation(ctx.slot_ctx) + minimum_rent_exempt_balance;
    }
    ...
    fd_stake_state_t stake_state;
    read_stake_state( ctx.slot_ctx, stake_acc_meta, &stake_state );
    ...
    fd_sol_sysvar_clock_t clock;
    bool require_custodian_for_locked_stake_authorize = FD_FEATURE_ACTIVE(ctx.slot_ctx, require_custodian_for_locked_stake_authorize);
    fd_pubkey_t * custodian = NULL;

    if ( require_custodian_for_locked_stake_authorize ) {
      if ( memcmp( &txn_accs[instr_acc_idxs[1]], fd_sysvar_clock_id.key, sizeof(fd_pubkey_t) ) != 0 ) {
        return FD_EXECUTOR_INSTR_ERR_INVALID_ARG;
      }
    ...
    if ( memcmp( &txn_accs[instr_acc_idxs[2]], fd_sysvar_clock_id.key, sizeof(fd_pubkey_t) ) != 0 ) {
      return FD_EXECUTOR_INSTR_ERR_INVALID_ARG;
    }
    fd_sol_sysvar_clock_t clock;
    fd_sysvar_clock_read( ctx.slot_ctx, &clock );
    ...
    if ( memcmp( &txn_accs[instr_acc_idxs[3]], fd_sysvar_stake_history_id.key, sizeof(fd_pubkey_t) ) != 0 ) {
      return FD_EXECUTOR_INSTR_ERR_INVALID_ARG;
    }
    fd_stake_history_t history;
    fd_sysvar_stake_history_read( ctx.slot_ctx, &history );

    /* Check that Instruction Account 4 is the Stake Config Program account */
    if ( memcmp( &txn_accs[instr_acc_idxs[4]], fd_solana_stake_program_config_id.key, sizeof(fd_pubkey_t) ) != 0 ) {
      return FD_EXECUTOR_INSTR_ERR_INVALID_ARG;
    }
    fd_stake_config_t stake_config;
    int result = read_stake_config( ctx.slot_ctx, &stake_config );
    if ( FD_UNLIKELY(result != FD_EXECUTOR_INSTR_SUCCESS) ) {
      return FD_EXECUTOR_INSTR_ERR_INVALID_ARG;
    }
    ...
    int err = fd_acc_mgr_view_old( ctx.acc_mgr, ctx.funk_txn, vote_acc, NULL, &vote_acc_meta, &vote_acc_data );
    ...
    if( memcmp( vote_acc_meta->info.owner, fd_solana_vote_program_id.key, sizeof(fd_pubkey_t) ) != 0 )
    ...
    if ( fd_stake_state_is_initialized( &stake_state ) ) {
      ...
      */
      ...
      if (stake_activating_and_deactivating( &stake_state.inner.stake.stake.delegation, clock.epoch, &history, new_activation_epoch).effective != 0) {
        ushort stake_lamports_ok = ( FD_FEATURE_ACTIVE( ctx.slot_ctx, stake_redelegate_instruction ) ) ? stake_amount >= stake_state.inner.stake.stake.delegation.stake : 1;
    ...
    result = write_stake_state( ctx.slot_ctx, stake_acc, &stake_state, 0 );
    if ( result != FD_EXECUTOR_INSTR_SUCCESS ) {
      FD_LOG_WARNING(( "failed to write stake account state: %d", result ));
      return result;
    }
    ...
    FD_LOG_INFO(( "stake_split_uses_rent_sysvar=%ld",            FD_FEATURE_ACTIVE( ctx.slot_ctx, stake_split_uses_rent_sysvar            ) ));
    FD_LOG_INFO(( "stake_allow_zero_undelegated_amount=%ld",     FD_FEATURE_ACTIVE( ctx.slot_ctx, stake_allow_zero_undelegated_amount     ) ));
    FD_LOG_INFO(( "clean_up_delegation_errors=%ld",              FD_FEATURE_ACTIVE( ctx.slot_ctx, clean_up_delegation_errors              ) ));
    FD_LOG_INFO(( "stake_raise_minimum_delegation_to_1_sol=%ld", FD_FEATURE_ACTIVE( ctx.slot_ctx, stake_raise_minimum_delegation_to_1_sol ) ));
  ...
  // https://github.com/firedancer-io/solana/blob/56bd357f0dfdb841b27c4a346a58134428173f42/programs/stake/src/stake_instruction.rs#L192
    ...
    int read_result = fd_acc_mgr_view_old( ctx.acc_mgr, ctx.funk_txn, split_acc, NULL, &split_metadata, NULL );
    if( FD_UNLIKELY( read_result != FD_ACC_MGR_SUCCESS ) ) {
      FD_LOG_DEBUG(( "failed to read split account metadata" ));
      return read_result;
    }
    if( memcmp( split_metadata->info.owner, fd_solana_stake_program_id.key, sizeof(fd_pubkey_t) ) != 0 ) {
      return FD_EXECUTOR_INSTR_ERR_INCORRECT_PROGRAM_ID;
    }
    ...
    read_stake_state( ctx.slot_ctx, split_metadata, &split_state );  /* unnecessary view */
    ...
    if ( fd_stake_state_is_stake( &stake_state ) ) {
      ...
      fd_acc_lamports_t minimum_delegation = FD_FEATURE_ACTIVE(ctx.slot_ctx, stake_raise_minimum_delegation_to_1_sol) ? MINIMUM_DELEGATION_SOL * LAMPORTS_PER_SOL: MINIMUM_STAKE_DELEGATION;
      ...
      if (source_remaining_balance == 0) {
        ...
        if ( FD_FEATURE_ACTIVE(ctx.slot_ctx, clean_up_delegation_errors) && stake_state.inner.stake.stake.delegation.stake < fd_ulong_sat_add(minimum_delegation, lamports)) {
      ...
      if (FD_FEATURE_ACTIVE(ctx.slot_ctx, clean_up_delegation_errors) && split_stake_amount < minimum_delegation) {
        ctx.txn_ctx->custom_err = 12;
        return FD_EXECUTOR_INSTR_ERR_CUSTOM_ERR; // (StakeError::InsufficientDelegation.into());
      }
      ...
      write_stake_state( ctx.slot_ctx, split_acc, &split_state, 1 );
      write_stake_state( ctx.slot_ctx, stake_acc, &stake_state, 0 );
      ...
      fd_acc_lamports_t additional_required_lamports = FD_FEATURE_ACTIVE(ctx.slot_ctx, stake_allow_zero_undelegated_amount) ? 0 : ( FD_FEATURE_ACTIVE(ctx.slot_ctx, stake_raise_minimum_delegation_to_1_sol) ? MINIMUM_DELEGATION_SOL * LAMPORTS_PER_SOL : MINIMUM_STAKE_DELEGATION);
      ...
      int result = write_stake_state( ctx.slot_ctx, split_acc, &split_state, 1 );
      if ( result != FD_EXECUTOR_INSTR_SUCCESS ) {
    ...
    if (instr_acc_idxs[0] != instr_acc_idxs[1]) {
      ...
      FD_TEST( 0==fd_acc_mgr_modify_old( ctx.acc_mgr, ctx.funk_txn, split_acc, 0, 0UL, NULL, NULL, &split_metadata_rw, NULL ));
      FD_TEST( 0==fd_acc_mgr_modify_old( ctx.acc_mgr, ctx.funk_txn, stake_acc, 0, 0UL, NULL, NULL, &stake_metadata_rw, NULL ));
    ...
    fd_sysvar_stake_history_read( ctx.slot_ctx, &stake_history );
    ...
    read_result = fd_acc_mgr_view_old( ctx.acc_mgr, ctx.funk_txn, to_acc, &to_acc_rec_ro, NULL, NULL );
    ...
    int write_result = fd_acc_mgr_modify_old( ctx.acc_mgr, ctx.funk_txn, to_acc, 1, 0UL, NULL, NULL, &to_acc_metadata, &to_acc_data );
    ...
    write_result = fd_acc_mgr_modify_old( ctx.acc_mgr, ctx.funk_txn, stake_acc, 0, 0UL, stake_acc_ro, NULL, &stake_acc_metadata_rw, NULL );
    FD_TEST( !write_result );
    ...
    fd_stake_state_t stake_state;
    int result = read_stake_state( ctx.slot_ctx, stake_acc_meta, &stake_state );
    if (result != FD_EXECUTOR_INSTR_SUCCESS) {
      return result;
    }
    ...
    int read_result = fd_acc_mgr_view_old( ctx.acc_mgr, ctx.funk_txn, source_acc, &source_rec_ro, &source_metadata, NULL );
    if( FD_UNLIKELY( read_result != FD_ACC_MGR_SUCCESS ) ) {
      FD_LOG_WARNING(( "failed to read split account metadata" ));
      return read_result;
    }
    if ( memcmp( &source_metadata->info.owner, fd_solana_stake_program_id.key, sizeof(fd_pubkey_t) ) != 0 ) {
      return FD_EXECUTOR_INSTR_ERR_INCORRECT_PROGRAM_ID;
    }
    ...
    /* Read the current State State from the Stake account */
    fd_stake_state_t source_state;
    read_stake_state( ctx.slot_ctx, source_metadata, &source_state );
    ...
    if (source_merge_kind.is_active_stake && stake_merge_kind.is_active_stake) {
      ...
      if (!FD_FEATURE_ACTIVE(ctx.slot_ctx, stake_merge_with_unmatched_credits_observed) && stake_state.inner.stake.stake.credits_observed != source_state.inner.stake.stake.credits_observed) {
    ...
    if (stake_merge_kind.discriminant == MERGE_KIND_INACTIVE && source_merge_kind.discriminant == MERGE_KIND_INACTIVE) {
      ...
      merge_delegation_stake_and_credits_observed(ctx.slot_ctx, &stake_state, src_lamports, source_state.inner.stake.stake.credits_observed);
    } else if (stake_merge_kind.discriminant == MERGE_KIND_FULLY_ACTIVE && source_merge_kind.discriminant == MERGE_KIND_FULLY_ACTIVE) {
      is_some_merge = 1;
      merge_delegation_stake_and_credits_observed(ctx.slot_ctx, &stake_state, source_state.inner.stake.stake.delegation.stake, source_state.inner.stake.stake.credits_observed);
    } else {
      ctx.txn_ctx->custom_err = 6;
      return FD_EXECUTOR_INSTR_ERR_CUSTOM_ERR; // Err(StakeError::MergeMismatch.into())
    }
    ...
    int write_result = fd_acc_mgr_modify_old( ctx.acc_mgr, ctx.funk_txn, source_acc, 0, 0UL, source_rec_ro, NULL, &source_metadata_rw, &source_data_rw );
    FD_TEST( !write_result );
        write_result = fd_acc_mgr_modify_old( ctx.acc_mgr, ctx.funk_txn, stake_acc,  0, 0UL, stake_acc_ro,  NULL, &stake_metadata_rw,  &stake_data_rw );
    FD_TEST( !write_result );
    ...
    if ( FD_FEATURE_ACTIVE( ctx.slot_ctx, vote_stake_checked_instructions ) ) {
      /* https://github.com/solana-labs/solana/blob/8f2c8b8388a495d2728909e30460aa40dcc5d733/programs/stake/src/stake_instruction.rs#L43 */
    ...
    if( !FD_FEATURE_ACTIVE( ctx.slot_ctx, add_get_minimum_delegation_instruction_to_stake_program ) ) {
      return FD_EXECUTOR_INSTR_ERR_INVALID_INSTR_DATA;
    }
    ...
    if( !FD_FEATURE_ACTIVE( ctx.slot_ctx, stake_deactivate_delinquent_instruction ) ) {
      return FD_EXECUTOR_INSTR_ERR_INVALID_INSTR_DATA;
    }
    ...
    FD_TEST( 0==fd_acc_mgr_view_old( ctx.acc_mgr, ctx.funk_txn, delinquent_vote_acc, NULL, &delinquent_vote_acc_meta, &delinquent_vote_acc_data ) );
    if ( memcmp( &delinquent_vote_acc_meta->info.owner, fd_solana_vote_program_id.key, sizeof(fd_pubkey_t) ) != 0 ) {
      return FD_EXECUTOR_INSTR_ERR_INCORRECT_PROGRAM_ID;
    }
    ...
    FD_TEST( 0==fd_acc_mgr_view_old( ctx.acc_mgr, ctx.funk_txn, reference_vote_acc, NULL, &reference_vote_acc_meta, &reference_vote_acc_data ) );
    if ( memcmp( &reference_vote_acc_meta->info.owner, fd_solana_vote_program_id.key, sizeof(fd_pubkey_t) ) != 0 ) {
      return FD_EXECUTOR_INSTR_ERR_INCORRECT_PROGRAM_ID;
    }

    fd_stake_state_t stake_state;
    result = read_stake_state( ctx.slot_ctx, stake_acc_meta, &stake_state );
    if (result != FD_EXECUTOR_INSTR_SUCCESS) {
    ...
    if ( memcmp( &txn_accs[instr_acc_idxs[3]], fd_solana_stake_program_config_id.key, sizeof(fd_pubkey_t) ) != 0 ) {
      return FD_EXECUTOR_INSTR_ERR_INVALID_ARG;
    }
    fd_stake_config_t stake_config;
    result = read_stake_config( ctx.slot_ctx, &stake_config );
    if ( FD_UNLIKELY(result != FD_EXECUTOR_INSTR_SUCCESS) ) {
      return FD_EXECUTOR_INSTR_ERR_INVALID_ARG;
    }
    ...
    result = fd_acc_mgr_view_old( ctx.acc_mgr, ctx.funk_txn, uninitialized_stake_acc, &uninitialized_stake_acc_rec_ro, &uninitialized_stake_acc_metadata, NULL );
    if ( FD_UNLIKELY( result != FD_ACC_MGR_SUCCESS ) ) {
      FD_LOG_WARNING(( "failed to read account metadata" ));
      return FD_EXECUTOR_INSTR_ERR_MISSING_ACC;
    }
    if( memcmp( &uninitialized_stake_acc_metadata->info.owner, fd_solana_stake_program_id.key, sizeof(fd_pubkey_t) ) != 0 ) {
      return FD_EXECUTOR_INSTR_ERR_INCORRECT_PROGRAM_ID;
    }
    ...
    result = read_stake_state( ctx.slot_ctx, uninitialized_stake_acc_metadata, &uninitialized_stake_state );
    if ( FD_UNLIKELY(result != FD_EXECUTOR_INSTR_SUCCESS) ) {
      return result;
    }
    ...
    FD_TEST( 0==fd_acc_mgr_view_old( ctx.acc_mgr, ctx.funk_txn, vote_acc, NULL, &vote_meta2, &vote_data ) );
    if( memcmp( &vote_meta2->info.owner, fd_solana_vote_program_id.key, sizeof(fd_pubkey_t) ) != 0 ) {
      return FD_EXECUTOR_INSTR_ERR_INCORRECT_PROGRAM_ID;
    }
    ...
    fd_sol_sysvar_clock_t clock;
    result = fd_sysvar_clock_read( ctx.slot_ctx, &clock );
    if( FD_UNLIKELY( result != FD_EXECUTOR_INSTR_SUCCESS ) ) {
      return result;
    }

    fd_stake_history_t history;
    result = fd_sysvar_stake_history_read( ctx.slot_ctx, &history);
    if (result != FD_EXECUTOR_INSTR_SUCCESS)
      return result;
    ...
    FD_TEST( 0==fd_acc_mgr_modify_old( ctx.acc_mgr, ctx.funk_txn, uninitialized_stake_acc, 0, 0UL, NULL, NULL, &uninitialized_stake_acc_metadata_rw, NULL ) );
    ...
    result = fd_acc_mgr_modify_old( ctx.acc_mgr, ctx.funk_txn, stake_acc, 0, 0UL, stake_acc_ro, NULL, &stake_acc_metadata, NULL );
    if ( FD_UNLIKELY( result != FD_ACC_MGR_SUCCESS ) ) {
    ...
    write_stake_state(ctx.slot_ctx, uninitialized_stake_acc, &uninitialized_stake_state, 1);
  }
  else {
