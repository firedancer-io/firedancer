...
fd_stake_delegations_apply_fork_delta( ulong                      epoch,
  ...
  for( fork_map_iter_t iter = fork_map_iter_init( fork_map, delta_pool );
    ...
    if( FD_LIKELY( !stake_delegation->is_tombstone ) ) {
      ...
      /* No root element to tag if the root pool couldn't take the
         delegation; the fallback tier resolves it from the accounts
         database at the boundary and hands out an untagged copy. */
