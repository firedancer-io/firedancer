...
fd_stake_delegations_pubkeys( fd_stake_delegations_t const * stake_delegations ) {
  return fd_type_pun( (uchar *)stake_delegations + stake_delegations->stake_pubkeys_offset_ );
}
...
fd_stake_delegations_new( void *               mem,
  ...
  if( FD_UNLIKELY( !stake_pubkeys ) ) {
    FD_LOG_WARNING(( "NULL stake_pubkeys" ));
    return NULL;
  }
  ...
  stake_delegations->stake_pubkeys_offset_    = (long)(ulong)stake_pubkeys - (long)(ulong)stake_delegations;
...
fd_stake_delegations_reset( fd_stake_delegations_t * stake_delegations ) {
  fd_stake_pubkeys_lock( fd_stake_delegations_pubkeys( stake_delegations ) );
  ...
  fd_stake_pubkeys_reset( fd_stake_delegations_pubkeys( stake_delegations ) );
  stake_delegations->effective_stake    = 0UL;
  stake_delegations->activating_stake   = 0UL;
  stake_delegations->deactivating_stake = 0UL;
  stake_delegations->pool_idx_wmk_      = 0UL;
  stake_delegations->fp_warmed_awarded  = 0;
  fd_stake_pubkeys_unlock( fd_stake_delegations_pubkeys( stake_delegations ) );
}
...
fd_stake_delegations_set_stake_totals( fd_stake_delegations_t * stake_delegations,
  ...
  fd_stake_pubkeys_t * stake_pubkeys = fd_stake_delegations_pubkeys( stake_delegations );
  fd_stake_pubkeys_lock( stake_pubkeys );
  stake_delegations->effective_stake    = effective_stake;
  stake_delegations->activating_stake   = activating_stake;
  stake_delegations->deactivating_stake = deactivating_stake;
...
root_update( fd_stake_delegations_t * stake_delegations,
  ...
  if( FD_LIKELY( !stake_delegation ) ) {
    if( FD_UNLIKELY( !root_pool_free( pool ) ) ) {
      fd_stake_pubkeys_fallback_enter( fd_stake_delegations_pubkeys( stake_delegations ), stake_account );
      return NULL;
    }
    ...
    stake_delegation->pubkey_idx = fd_stake_pubkeys_acquire( fd_stake_delegations_pubkeys( stake_delegations ), stake_account );
  }
...
fd_stake_delegations_prune_inactive_root( fd_stake_delegations_t *   stake_delegations,
  ...
  for( ulong idx=0UL; idx<stake_delegations->pool_idx_wmk_; idx++ ) {
    ...
    uint        pubkey_idx    = delegation->pubkey_idx;
    root_map_idx_remove( map, &stake_account, (uint)idx, pool );
    delegation->in_use = 0;
    root_pool_idx_release( pool, (uint)idx );
    fd_stake_pubkeys_release( stake_pubkeys, pubkey_idx );
  }
...
refresh_remove( root_map_t *            map,
  ...
  fd_stake_pubkeys_refresh_remove( stake_pubkeys, pubkey_idx );
}
...
#define BATCH 64UL
  ...
  for(;;) {
    ulong batch_n = 0UL;
    while( batch_n<BATCH ) {
      uint idx = fd_stake_pubkeys_iter_next( stake_pubkeys, &cursor );
      if( FD_UNLIKELY( idx==UINT_MAX ) ) break;
      pubkeys[ batch_n ]  = fd_stake_pubkeys_query( stake_pubkeys, idx )->uc;
      writable[ batch_n ] = 0;
      ref_idx[ batch_n ]  = idx;
      batch_n++;
    }
    if( FD_UNLIKELY( !batch_n ) ) break;
    ...
    for( ulong j=0UL; j<batch_n; j++ ) {
      ...
      if( FD_UNLIKELY( !stake || stake->stake_type!=FD_STAKE_STATE_STAKE ) ) {
        refresh_remove( map, pool, stake_pubkeys, stake_account, ref_idx[ j ] );
        continue;
      }
...
#undef BATCH
  ...
  fd_stake_pubkeys_refresh_fini( stake_pubkeys, root_pool_used( pool ) );
  fd_stake_pubkeys_unlock( stake_pubkeys );
}
...
fd_stake_delegations_fallback( fd_stake_delegations_t const * stake_delegations ) {
  return fd_stake_pubkeys_fallback( fd_stake_delegations_pubkeys( stake_delegations ) );
}
...
fd_stake_delegations_evict_fork( fd_stake_delegations_t * stake_delegations,
  ...
  while( !fork_map_iter_done( iter, fork_map, delta_pool ) ) {
    ...
    fd_stake_pubkeys_release( fd_stake_delegations_pubkeys( stake_delegations ), ele->pubkey_idx );
  ...
  for( fork_map_iter_t iter = fork_map_iter_init( fork_map, delta_pool );
    ...
    if( FD_LIKELY( !stake_delegation->is_tombstone ) ) {
      ...
      if( FD_LIKELY( old_delegation ) ) {
        fd_stake_history_entry_t old_entry     = fd_stake_delegation_activation_status( old_delegation, epoch, stake_history, warmup_cooldown_rate_epoch, use_fixed_point_stake_math );
      ...
      fd_stake_history_entry_t new_acc = fd_stake_delegation_activation_status( stake_delegation, epoch, stake_history, warmup_cooldown_rate_epoch, use_fixed_point_stake_math );
      ...
      if( FD_LIKELY( delegation_idx!=UINT_MAX ) ) {
        ...
        uint                    pubkey_idx      = old_delegation->pubkey_idx;
        fd_stake_history_entry_t old_entry = fd_stake_delegation_activation_status( old_delegation, epoch, stake_history, warmup_cooldown_rate_epoch, use_fixed_point_stake_math );
        stake_delegations->effective_stake    -= old_entry.effective;
        stake_delegations->activating_stake   -= old_entry.activating;
        stake_delegations->deactivating_stake -= old_entry.deactivating;
        ...
        fd_stake_pubkeys_release( fd_stake_delegations_pubkeys( stake_delegations ), pubkey_idx );
      }
    }
  }
...
fd_stake_delegations_iter_advance_fallback( fd_stake_delegations_iter_t * iter ) {
  ...
  for(;;) {
    ...
    ulong batch_n = 0UL;
    while( batch_n<FD_STAKE_DELEGATIONS_ITER_BATCH ) {
      uint idx = fd_stake_pubkeys_iter_next( stake_pubkeys, &iter->scan_idx );
      if( FD_UNLIKELY( idx==UINT_MAX ) ) break;
      pubkeys [ batch_n ] = fd_stake_pubkeys_query( stake_pubkeys, idx )->uc;
      writable[ batch_n ] = 0;
      pool_idx[ batch_n ] = (ulong)idx;
      batch_n++;
    }
    ...
    for( ulong j=0UL; j<batch_n; j++ ) {
      ...
      ele->pubkey_idx           = (uint)pool_idx[ j ];
...
fd_stake_delegations_iter_init( fd_stake_delegations_iter_t *  iter,
  ...
  iter->batch_idx         = 0UL;
  fd_stake_pubkeys_t * stake_pubkeys = fd_stake_delegations_pubkeys( stake_delegations );
  iter->fallback          = fd_stake_pubkeys_fallback( stake_pubkeys );
...
fd_stake_delegations_mark_delta( fd_stake_delegations_t *   stake_delegations,
  ...
  for( fork_map_iter_t iter = fork_map_iter_init( fork_map, delta_pool );
    ...
    if( FD_UNLIKELY( !base_delegation ) ) {
      if( FD_UNLIKELY( !root_pool_free( root_pool ) ) ) {
        /* No room to project this delta into the root for the duration of
           ...
           tier. */
        fd_stake_pubkeys_fallback_enter( fd_stake_delegations_pubkeys( stake_delegations ), &delta_delegation->stake_account );
        continue;
      }
      ...
      base_delegation->pubkey_idx      = delta_delegation->pubkey_idx;
    ...
    if( FD_LIKELY( !delta_delegation->is_tombstone ) ) {
      fd_stake_history_entry_t new_acc = fd_stake_delegation_activation_status( delta_delegation, epoch, stake_history, warmup_cooldown_rate_epoch, use_fixed_point_stake_math );
      stake_delegations->effective_stake    += new_acc.effective;
      stake_delegations->activating_stake   += new_acc.activating;
      stake_delegations->deactivating_stake += new_acc.deactivating;
    }
...
fd_stake_delegations_unmark_delta( fd_stake_delegations_t *   stake_delegations,
  ...
  for( fork_map_iter_t iter = fork_map_iter_init( fork_map, delta_pool );
    ...
    if( FD_UNLIKELY( base_delegation->dne_in_root )) {
      if( FD_LIKELY( !delta_delegation->is_tombstone ) ) {
        fd_stake_history_entry_t acc = fd_stake_delegation_activation_status( delta_delegation, epoch, stake_history, warmup_cooldown_rate_epoch, use_fixed_point_stake_math );
      ...
      fd_stake_history_entry_t acc = fd_stake_delegation_activation_status( base_delegation, epoch, stake_history, warmup_cooldown_rate_epoch, use_fixed_point_stake_math );
      stake_delegations->effective_stake    += acc.effective;
      stake_delegations->activating_stake   += acc.activating;
      stake_delegations->deactivating_stake += acc.deactivating;
    }
