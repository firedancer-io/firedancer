...
fd_stake_delegations_prune_inactive_root( fd_stake_delegations_t *   stake_delegations,
  ...
  for( ulong idx=0UL; idx<stake_delegations->pool_idx_wmk_; idx++ ) {
    ...
    pubkey_ref_release( stake_delegations, &stake_account );
  }
...
refresh_remove( root_map_t *                  map,
                ...
                pubkey_map_t *                ref_map,
                fd_stake_delegation_ref_t *   ref_pool,
                fd_pubkey_t const *           stake_account,
                ulong                         ref_idx ) {
  ...
  ref_pool[ ref_idx ].refcnt = 0U;
  pubkey_pool_ele_release( ref_pool, &ref_pool[ ref_idx ] );
}
  ...
  while( i<wmk ) {
    ...
    for( ulong j=0UL; j<batch_n; j++ ) {
      ...
      if( FD_UNLIKELY( !stake || stake->stake_type!=FD_STAKE_STATE_STAKE ) ) {
        refresh_remove( map, pool, ref_map, ref_pool, stake_account, ref_idx[ j ] );
        continue;
      }
  ...
  for(;;) {
    ...
    if( FD_UNLIKELY( stake_delegations->disk_pubkey_used_ ) ) {
      while( batch_n<FD_STAKE_DELEGATIONS_ITER_BATCH && iter->disk_scan_idx<stake_delegations->disk_slot_cnt_ ) {
        ...
        disk_ref_t const * rec = fd_type_pun_const( iter->chunk + chunk_idx*sizeof(disk_ref_t) );
        if( FD_UNLIKELY( disk_ref_is_live( stake_delegations, rec ) ) ) {
          iter->batch[ batch_n ].stake_account = rec->stake_account;
          pubkeys [ batch_n ] = iter->batch[ batch_n ].stake_account.uc;
          writable[ batch_n ] = 0;
    ...
    for( ulong j=0UL; j<batch_n; j++ ) {
      ...
      ele->is_tombstone         = 0;
      ele->warmup_cooldown_rate = iter->warmup_cooldown_rate;
      ele->in_use               = 1;
...
fd_stake_delegations_iter_init( fd_stake_delegations_iter_t *  iter,
  ...
  if( FD_UNLIKELY( iter->fallback ) ) {
    ...
    iter->warmup_cooldown_rate       = fd_stake_warmup_cooldown_rate( epoch, warmup_cooldown_rate_epoch );
