...
void *fd_pubkey_node_generate( void *mem, void **alloc_mem, fd_rng_t * rng ) {
  fd_pubkey_node_t *self = (fd_pubkey_node_t *) mem;
  *alloc_mem = (uchar *) *alloc_mem + sizeof(fd_pubkey_node_t);
  fd_pubkey_node_new(mem);
  fd_pubkey_generate( &self->pubkey, alloc_mem, rng );
  return mem;
}

void *fd_rent_fresh_accounts_partition_generate( void *mem, void **alloc_mem, fd_rng_t * rng ) {
  fd_rent_fresh_accounts_partition_t *self = (fd_rent_fresh_accounts_partition_t *) mem;
  *alloc_mem = (uchar *) *alloc_mem + sizeof(fd_rent_fresh_accounts_partition_t);
  fd_rent_fresh_accounts_partition_new(mem);
  self->partition = fd_rng_ulong( rng );
  ulong accounts_len = fd_rng_ulong( rng ) % 8;
  ...
  for( ulong i=0; i < accounts_len; i++ ) {
    ...
    fd_pubkey_node_generate( &node->elem, alloc_mem, rng );
...
void *fd_rent_fresh_accounts_generate( void *mem, void **alloc_mem, fd_rng_t * rng ) {
  ...
  *alloc_mem = (uchar *) *alloc_mem + sizeof(fd_rent_fresh_accounts_t);
  ...
  ulong partitions_len = fd_rng_ulong( rng ) % 8;
  ...
  for( ulong i=0; i < partitions_len; i++ ) {
    ...
    fd_rent_fresh_accounts_partition_generate( &node->elem, alloc_mem, rng );
