...
/* Encoded Size: Dynamic */
struct __attribute__((aligned(8UL))) fd_rent_dirty_accounts_partition {
  ulong partition;
  ulong accounts_len;
  fd_pubkey_t * accounts;
};
typedef struct fd_rent_dirty_accounts_partition fd_rent_dirty_accounts_partition_t;
#define FD_RENT_DIRTY_ACCOUNTS_PARTITION_FOOTPRINT sizeof(fd_rent_dirty_accounts_partition_t)
#define FD_RENT_DIRTY_ACCOUNTS_PARTITION_ALIGN (8UL)

struct __attribute__((aligned(8UL))) fd_rent_dirty_accounts_partition_global {
  ulong partition;
  ulong accounts_len;
  ulong accounts_gaddr;
};
typedef struct fd_rent_dirty_accounts_partition_global fd_rent_dirty_accounts_partition_global_t;
#define FD_RENT_DIRTY_ACCOUNTS_PARTITION_GLOBAL_FOOTPRINT sizeof(fd_rent_dirty_accounts_partition_global_t)
#define FD_RENT_DIRTY_ACCOUNTS_PARTITION_GLOBAL_ALIGN (8UL)

typedef struct fd_rent_dirty_accounts_partition_t_mapnode fd_rent_dirty_accounts_partition_t_mapnode_t;
#define REDBLK_T fd_rent_dirty_accounts_partition_t_mapnode_t
#define REDBLK_NAME fd_rent_dirty_accounts_partition_t_map
#define REDBLK_IMPL_STYLE 1
#include "../../util/tmpl/fd_redblack.c"
struct fd_rent_dirty_accounts_partition_t_mapnode {
    fd_rent_dirty_accounts_partition_t elem;
    ulong redblack_parent;
    ulong redblack_left;
    ulong redblack_right;
    int redblack_color;
};
static inline fd_rent_dirty_accounts_partition_t_mapnode_t *
fd_rent_dirty_accounts_partition_t_map_join_new( void * * alloc_mem, ulong len ) {
  if( FD_UNLIKELY( 0 == len ) ) len = 1; // prevent underflow
  *alloc_mem = (void*)fd_ulong_align_up( (ulong)*alloc_mem, fd_rent_dirty_accounts_partition_t_map_align() );
  void * map_mem = *alloc_mem;
  *alloc_mem = (uchar *)*alloc_mem + fd_rent_dirty_accounts_partition_t_map_footprint( len );
  return fd_rent_dirty_accounts_partition_t_map_join( fd_rent_dirty_accounts_partition_t_map_new( map_mem, len ) );
}
/* Encoded Size: Dynamic */
struct __attribute__((aligned(8UL))) fd_rent_dirty_accounts {
  ulong total_count;
  fd_rent_dirty_accounts_partition_t_mapnode_t * partitions_pool;
  fd_rent_dirty_accounts_partition_t_mapnode_t * partitions_root;
};
typedef struct fd_rent_dirty_accounts fd_rent_dirty_accounts_t;
#define FD_RENT_DIRTY_ACCOUNTS_FOOTPRINT sizeof(fd_rent_dirty_accounts_t)
#define FD_RENT_DIRTY_ACCOUNTS_ALIGN (8UL)

struct __attribute__((aligned(8UL))) fd_rent_dirty_accounts_global {
  ulong total_count;
  ulong partitions_pool_gaddr;
  ulong partitions_root_gaddr;
};
typedef struct fd_rent_dirty_accounts_global fd_rent_dirty_accounts_global_t;
#define FD_RENT_DIRTY_ACCOUNTS_GLOBAL_FOOTPRINT sizeof(fd_rent_dirty_accounts_global_t)
#define FD_RENT_DIRTY_ACCOUNTS_GLOBAL_ALIGN (8UL)
...
ulong fd_rent_dirty_accounts_partition_footprint( void );
ulong fd_rent_dirty_accounts_partition_align( void );
...
ulong fd_rent_dirty_accounts_footprint( void );
ulong fd_rent_dirty_accounts_align( void );
