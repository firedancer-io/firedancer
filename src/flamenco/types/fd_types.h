...
typedef struct fd_pubkey_t_mapnode fd_pubkey_t_mapnode_t;
#define REDBLK_T fd_pubkey_t_mapnode_t
#define REDBLK_NAME fd_pubkey_t_map
#define REDBLK_IMPL_STYLE 1
#include "../../util/tmpl/fd_redblack.c"
struct fd_pubkey_t_mapnode {
    fd_pubkey_t elem;
    ulong redblack_parent;
    ulong redblack_left;
    ulong redblack_right;
...
static inline fd_pubkey_t_mapnode_t *
fd_pubkey_t_map_join_new( void * * alloc_mem, ulong len ) {
  if( FD_UNLIKELY( 0 == len ) ) len = 1; // prevent underflow
  *alloc_mem = (void*)fd_ulong_align_up( (ulong)*alloc_mem, fd_pubkey_t_map_align() );
  void * map_mem = *alloc_mem;
  *alloc_mem = (uchar *)*alloc_mem + fd_pubkey_t_map_footprint( len );
  return fd_pubkey_t_map_join( fd_pubkey_t_map_new( map_mem, len ) );
}
/* Encoded Size: Dynamic */
struct __attribute__((aligned(8UL))) fd_rent_fresh_accounts_partition {
  ulong partition;
  fd_pubkey_t_mapnode_t * accounts_pool;
  fd_pubkey_t_mapnode_t * accounts_root;
};
