...
test_get_prunes_insufficient_upserts( void ) {
  ...
  fd_prune_finder_get_prunes( pf, 100UL, origins, 1UL, &out_prunes, &out_prunes_len );
...
test_get_prunes_tiebreaker( void ) {
  ...
  FD_TEST( out_prunes_len == 1UL );
  FD_TEST( !memcmp( out_prunes[0].relayer_pubkey.uc, relayers[FD_PRUNE_MIN_INGRESS_NODES].pubkey, 32UL ) );
  free( bytes );
}
...
test_get_prunes_min_ingress_stake( void ) {
  ...
  fd_prune_finder_get_prunes( pf, 250001UL, origins, 1UL, &out_prunes, &out_prunes_len );
  FD_TEST( out_prunes_len == 1UL );
  FD_TEST( !memcmp( out_prunes[0].relayer_pubkey.uc, out_stake_thresh.pubkey, 32UL ) );
  FD_TEST( out_prunes[0].prune_len == 1UL );
  free( bytes );
}
