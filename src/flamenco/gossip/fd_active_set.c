...
#define POOL_NAME bucket_entry_pool
...
#define DLIST_NAME  bucket_entry_last_hit
#define DLIST_ELE_T fd_active_set_bucket_entry_t
...
struct fd_active_set_bucket_private {
  fd_active_set_bucket_entry_t * pool;
  bucket_entry_last_hit_t *      last_hit;
...
#define BUCKET_ENTRY_IDX_SENTINEL (UCHAR_MAX)
...
struct peer_meta {
  ulong stake;
  uchar bucket_idx[FD_ACTIVE_SET_STAKE_BUCKETS]; /* bucket_idx[i] == BUCKET_ENTRY_IDX_SENTINEL if not in bucket i */
};
...
fd_active_set_footprint( void ) {
  ...
  for( ulong i=0UL; i<FD_ACTIVE_SET_STAKE_BUCKETS; i++ ) {
    ...
    l = FD_LAYOUT_APPEND( l, bucket_entry_pool_align(),       bucket_entry_pool_footprint( FD_ACTIVE_SET_PEERS_PER_BUCKET ) );
    l = FD_LAYOUT_APPEND( l, bucket_entry_last_hit_align(),   bucket_entry_last_hit_footprint() );
...
fd_active_set_new( void *     shmem,
  ...
  for( ulong i=0UL; i<FD_ACTIVE_SET_STAKE_BUCKETS; i++ ) {
    ...
    void * _bucket_pool      = FD_SCRATCH_ALLOC_APPEND( l, bucket_entry_pool_align(),       bucket_entry_pool_footprint( FD_ACTIVE_SET_PEERS_PER_BUCKET ) );
    void * _bucket_last_hit  = FD_SCRATCH_ALLOC_APPEND( l, bucket_entry_last_hit_align(),   bucket_entry_last_hit_footprint() );
    ...
    bucket->pool = bucket_entry_pool_join( bucket_entry_pool_new( _bucket_pool, FD_ACTIVE_SET_PEERS_PER_BUCKET ) );
    FD_TEST( bucket->pool );

    bucket->last_hit = bucket_entry_last_hit_join( bucket_entry_last_hit_new( _bucket_last_hit ) );
    FD_TEST( bucket->last_hit );
    ...
    for( ulong i=0UL; i<FD_ACTIVE_SET_PEERS_PER_BUCKET; i++ ) {
      fd_active_set_bucket_entry_t * peer = bucket_entry_pool_ele( bucket->pool, i );
...
fd_active_set_nodes( fd_active_set_t *          active_set,
  ...
  ulong out_idx = 0UL;
  bucket_insert_dlist_iter_t it = bucket_insert_dlist_iter_fwd_init( bucket->insert_dlist, bucket->pool );
  for( ; !bucket_insert_dlist_iter_done        ( it, bucket->insert_dlist, bucket->pool );
         it = bucket_insert_dlist_iter_fwd_next( it, bucket->insert_dlist, bucket->pool ) ) {

    ulong                          idx  = bucket_insert_dlist_iter_idx( it, bucket->insert_dlist, bucket->pool );
    fd_active_set_bucket_entry_t * peer = bucket_entry_pool_ele( bucket->pool, idx );
    ...
    bucket_entry_last_hit_idx_remove   ( bucket->last_hit, idx, bucket->pool );
    bucket_entry_last_hit_idx_push_tail( bucket->last_hit, idx, bucket->pool );
  }
  return out_idx;
}
...
fd_active_set_prune( fd_active_set_t * active_set,
  ...
  for( ; !bucket_insert_dlist_iter_done        ( it, bucket->insert_dlist, bucket->pool );
    ...
    ulong                          i    = bucket_insert_dlist_iter_idx( it, bucket->insert_dlist, bucket->pool );
    fd_active_set_bucket_entry_t * peer = bucket_entry_pool_ele( bucket->pool, i );
...
fd_active_set_rotate( fd_active_set_t *            active_set,
  ...
  if( FD_LIKELY( !bucket_entry_pool_free( bucket->pool ) ) ) {
    replace_idx = bucket_insert_dlist_idx_pop_head( bucket->insert_dlist, bucket->pool );
    replace     = bucket_entry_pool_ele( bucket->pool, replace_idx );

    ulong crds_idx = replace->entry_idx;

    out_maybe_flush->txbuild  = replace->push_state;
    out_maybe_flush->crds_idx = crds_idx;
    bucket_entry_last_hit_idx_remove( bucket->last_hit, replace_idx, bucket->pool );
    ...
    replace_idx = bucket_entry_pool_idx_acquire( bucket->pool );
    replace     = bucket_entry_pool_ele( bucket->pool, replace_idx );
  ...
  peer_meta_t * e         = &active_set->peer_metas[ crds_idx ];
  e->bucket_idx[ bucket_idx ] = (uchar)replace_idx;
  ...
  bucket_insert_dlist_idx_push_tail  ( bucket->insert_dlist, replace_idx, bucket->pool );
  bucket_entry_last_hit_idx_push_tail( bucket->last_hit,     replace_idx, bucket->pool );
}
...
fd_active_set_peer_remove( fd_active_set_t *          active_set,
  ...
  for( ulong j=0UL; j<FD_ACTIVE_SET_STAKE_BUCKETS; j++ ) {
    ...
    fd_active_set_bucket_entry_t * entry  = bucket_entry_pool_ele( bucket->pool, e->bucket_idx[j] );
    ...
    bucket_insert_dlist_idx_remove  ( bucket->insert_dlist, e->bucket_idx[j], bucket->pool );
    bucket_entry_last_hit_idx_remove( bucket->last_hit,     e->bucket_idx[j], bucket->pool );
    bucket_entry_pool_idx_release   ( bucket->pool,         e->bucket_idx[j] );
...
fd_active_set_flush_stale_advance( fd_active_set_t *            active_set,
  ...
  for( ulong i=0UL; i<FD_ACTIVE_SET_STAKE_BUCKETS; i++ ) {
    fd_active_set_bucket_t * bucket = active_set->buckets[i];
    if( FD_LIKELY( bucket_entry_last_hit_is_empty( bucket->last_hit, bucket->pool ) ) ) continue;

    fd_active_set_bucket_entry_t * peer = bucket_entry_last_hit_ele_peek_head( bucket->last_hit, bucket->pool );
    if( FD_LIKELY( peer->last_hit.wallclock_nanos>=stale_if_before ) ) continue;
    maybe_flush->txbuild  = peer->push_state;
    maybe_flush->crds_idx = peer->entry_idx;
    peer->last_hit.wallclock_nanos = now;
    bucket_entry_last_hit_ele_pop_head ( bucket->last_hit, bucket->pool );
    bucket_entry_last_hit_ele_push_tail( bucket->last_hit, peer, bucket->pool );
    return 1;
  }
