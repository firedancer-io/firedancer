...
test_basic( void ) {
  ...
  fd_ip4_port_t random_address[1];
  ...
  FD_TEST( fd_ping_tracker_pop_request( ping_tracker, now+seconds(12), &out_pubkey, &out_address, &out_token ) );
  FD_TEST( !fd_ping_tracker_active( ping_tracker, random_pubkey, 0UL, random_address, now+seconds(12) ) );
  FD_TEST( !fd_ping_tracker_pop_request( ping_tracker, now+seconds(13), &out_pubkey, &out_address, &out_token ) );
  FD_TEST( fd_ping_tracker_pop_request( ping_tracker, now+seconds(14), &out_pubkey, &out_address, &out_token ) );
  FD_TEST( !fd_ping_tracker_pop_request( ping_tracker, now+seconds(15), &out_pubkey, &out_address, &out_token ) );
...
test_juggle( void ) {
  ...
  uchar random_pubkey1[ 32UL ] = { 0 };
  for( ulong i=0UL; i<32UL; i++ ) random_pubkey1[ i ] = fd_rng_uchar( rng );

  fd_ip4_port_t random_address1[1];
  random_address1->addr = fd_rng_uint( rng );
  random_address1->port = fd_rng_ushort( rng );

  uchar random_pubkey2[ 32UL ] = { 0 };
  for( ulong i=0UL; i<32UL; i++ ) random_pubkey2[ i ] = fd_rng_uchar( rng );
  ...
  /* Different address */
  fd_ping_tracker_register( ping_tracker, random_pubkey1, 0UL, random_address2, valid_pong_token, now+seconds(3) );
  FD_TEST( !fd_ping_tracker_active( ping_tracker, random_pubkey1, 0UL, random_address1, now+seconds(3) ) );

  /* Wrong token */
  uchar wrong_pong_token[ 32UL ] = { 0 };
  fd_ping_tracker_register( ping_tracker, random_pubkey1, 0UL, random_address1, wrong_pong_token, now+seconds(3) );
  FD_TEST( !fd_ping_tracker_active( ping_tracker, random_pubkey1, 0UL, random_address1, now+seconds(3) ) );

  /* Correct token */
  fd_ping_tracker_register( ping_tracker, random_pubkey1, 0UL, random_address1, valid_pong_token, now+seconds(3) );
  FD_TEST( fd_ping_tracker_active( ping_tracker, random_pubkey1, 0UL, random_address1, now+seconds(3) ) );
...
test_change_address( void ) {
  ...
  fd_ping_tracker_register( ping_tracker, random_pubkey, 0UL, random_address, valid_pong_token, now+seconds(2) );
...
test_random( void ) {
  ...
  for( ulong i=0UL; i<100000UL; i++ ) {
    ...
    if( opt==0UL ) {
      uchar random_pubkey[ 32UL ] = { 0 };
      for( ulong j=0UL; j<32UL; j++ ) random_pubkey[ j ] = fd_rng_uchar( rng );
