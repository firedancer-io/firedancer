...
test_basic( void ) {
  ...
  /* Two tracked peers */
  FD_TEST( change_ctx->invoke_cnt==2 );
  ...
  FD_TEST( out_address->port==p.address.port );
  FD_TEST( change_ctx->invoke_cnt==2 );
  fd_ping_tracker_advance( ping_tracker, now+seconds(20) );
...
test_register( void ) {
  ...
  fd_ping_tracker_advance( ping_tracker, now+seconds(1) );
  ...
  fd_ip4_port_t const * out_address;
  uchar const *         out_token;
  fd_ping_tracker_advance( ping_tracker, now+seconds(2) );
  ...
  /* Different address */
  fd_ping_tracker_advance( ping_tracker, now+seconds(3) );
  fd_ping_tracker_register( ping_tracker, p1.pubkey, p2.address, valid_pong_token, now+seconds(3) );
  ...
  fd_ping_tracker_advance( ping_tracker, now+seconds(3) );
  fd_ping_tracker_register( ping_tracker, p1.pubkey, p1.address, wrong_pong_token, now+seconds(3) );
  FD_TEST( change_ctx->invoke_cnt==1UL );

  /* Unknown peer */
  fd_ping_tracker_register( ping_tracker, p2.pubkey, p2.address, valid_pong_token, now+seconds(3) );
  FD_TEST( change_ctx->invoke_cnt==1UL );

  /* Correct token */
  fd_ping_tracker_register( ping_tracker, p1.pubkey, p1.address, valid_pong_token, now+seconds(3) );
  ...
  fd_ping_tracker_advance( ping_tracker, now+seconds(4) );
  fd_ping_tracker_register( ping_tracker, p1.pubkey, p1.address, valid_pong_token, now+seconds(4) );
  FD_TEST( change_ctx->invoke_cnt==2UL );
...
test_change_address( void ) {
  ...
  fd_ping_tracker_advance( ping_tracker, now+seconds(2) );
  fd_ping_tracker_register( ping_tracker, p.pubkey, p.address, valid_pong_token, now+seconds(2) );
  FD_TEST( change_ctx->invoke_cnt==2UL );
...
test_invalid_transitions( void ) {
  ...
  fd_ping_tracker_advance( ping_tracker, now+seconds(18*60+4) );
  ...
  fd_ping_tracker_advance( ping_tracker, now+seconds(20*60+3) );
  ...
  /* pop */
  fd_ping_tracker_advance( ping_tracker, now+seconds(20*60+4) );
  ...
  fd_ping_tracker_advance( ping_tracker, now+seconds(20*60+24) );
  FD_TEST( !fd_ping_tracker_pop_request( ping_tracker, now+seconds(20*60+24), &out_pubkey, &out_address, &out_token ) );
  fd_ping_tracker_advance( ping_tracker, now+seconds(20*60+25) );
