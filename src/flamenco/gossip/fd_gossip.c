...
dump_gossip_msg( uchar const * buf, ulong len, ulong cnt ) {
  char filename[100];
  sprintf(filename, "/home/rsivakumaran/scratch/gossip/gossip_epoch_msg_dump/%lu.bin", cnt);
...
fd_gossip_recv(fd_gossip_t * glob, const fd_gossip_peer_addr_t * from, fd_gossip_msg_t * gmsg) {
  ...
  case fd_gossip_msg_enum_pull_resp: {
    fd_gossip_pull_resp_t * pull_resp = &gmsg->inner.pull_resp;
    fd_gossip_recv_crds_array( glob, NULL, pull_resp->crds, pull_resp->crds_len, FD_GOSSIP_ROUTE_PULL_RESP );
    break;
  }
  case fd_gossip_msg_enum_push_msg: {
    fd_gossip_push_msg_t * push_msg = &gmsg->inner.push_msg;
    fd_gossip_recv_crds_array( glob, from, push_msg->crds, push_msg->crds_len, FD_GOSSIP_ROUTE_PUSH );
    break;
  }
