...
fd_gossip_pull_request_encode_ctx_init( uchar *                               payload,
  ...
  out_view->contact_info->value_off = CUR_OFFSET; /* Offset to the start of contact info in payload */
...
fd_gossip_pull_request_encode_bloom_bits( fd_gossip_view_pull_request_t       * view,
  ...
  ulong const num_bits_set_offset = view->bloom_bits_offset + view->bloom_len*sizeof(ulong) + sizeof(ulong); /* after bloom bits cnt */
...
fd_gossip_contact_info_encode( fd_contact_info_t const *     contact_info,
  ...
  if( opt_encoded_sz ) {
    *opt_encoded_sz = BYTES_CONSUMED; /* Return the size of the encoded contact info */
  }
  return 0;
}
