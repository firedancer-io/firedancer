/* Owned by CRDS table, tightly coupled with contact info side table.
  ...
  Each sampler needs a different weight scoring implementation. This co
    - defines weight scoring implementations for each sampler.
    - defines macros to {insert, remove, update} all samplers in one go
    - defines a functions to sample each sampler

  Modifications to the samplers are edge driven and infrequent.

  A sampler undergoes an {insert, remove} event whenever:
    - a peer's contact info is inserted/removed in the CRDS table
    - a peer transitions between active/inactive in the ping tracker
        - insert when peer becomes active
        - remove when peer becomes inactive

  A sampler undergoes an update event whenever:
    - a peer's stake changes (contact info is updated in the CRDS table) */
...
typedef struct peer_wsampler peer_wsampler_t;

#define PREV_PEER_WEIGHT( ps, idx ) \
  ( (idx) ? (ps)->cumul_weight[(idx)-1] : 0UL )
int
peer_wsampler_init( peer_wsampler_t * ps ) {
...
int
peer_wsampler_upd( peer_wsampler_t * ps,
                   ulong             weight,
                   ulong             idx,
                   ulong             ele_cnt ) {
  if( FD_UNLIKELY( !ps ) ) return -1;
...
int
peer_wsampler_rem( peer_wsampler_t * ps,
                   ulong             idx,
                   ulong             ele_cnt ) {
...
struct crds_samplers {
  peer_wsampler_t   pr_sampler[1];
  peer_wsampler_t   bucket_samplers[25];
...
ulong
peer_wsampler_peer_score( fd_crds_entry_t * peer,
                          long              now ) {
...
ulong
peer_wsampler_bucket_score( fd_crds_entry_t * peer,
                            ulong             bucket ) {
