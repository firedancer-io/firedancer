...
unfresh( fd_crds_t * crds,
  ...
  while( !crds_contact_info_fresh_list_is_empty( crds->contact_info.dlist, crds->pool ) ) {
    ...
    crds_samplers_upd_peer( crds->samplers, head, head->contact_info.sampler_idx, now );
  }
}
    ...
    if( FD_UNLIKELY( incumbent->key.tag==FD_GOSSIP_VALUE_CONTACT_INFO ) ) {
      ...
      if( FD_LIKELY( !is_from_me ) ) {
        if( FD_UNLIKELY( candidate->stake!=incumbent->stake ) ) {
          ...
          crds_samplers_upd_peer( crds->samplers, candidate, incumbent->contact_info.sampler_idx, now );
        } else {
          crds_samplers_swap_peer( crds->samplers, candidate, incumbent->contact_info.sampler_idx );
        }
      }
