...
fd_crds_insert( fd_crds_t *                         crds,
  ...
  if( FD_LIKELY( replace ) ) {
    if( FD_UNLIKELY( upsert_check_result==FD_CRDS_UPSERT_CHECK_UNDETERMINED ) ) {
      ...
      int res = memcmp( candidate->value_hash, replace->value_hash, 32UL );
      if( res<0 ) {
        /* replace wins */
    ...
    if( FD_UNLIKELY( replace->key.tag==FD_CRDS_TAG_CONTACT_INFO ) ) {
      ...
      if( FD_LIKELY( !is_from_me ) ) {
        if( FD_UNLIKELY( candidate->stake!=replace->stake ) ) {
          ...
          crds_samplers_upd_peer( crds->samplers, candidate, replace->contact_info.sampler_idx, now );
        } else {
          crds_samplers_swap_peer( crds->samplers, candidate, replace->contact_info.sampler_idx );
        }
      }
      ...
      if( FD_LIKELY( crds->metrics ) ) {
        if( replace->stake ) {
          crds->metrics->staked_peer_cnt--;
          crds->metrics->visible_stake -= replace->stake;
        } else {
          crds->metrics->unstaked_peer_cnt--;
        }
      }
