...
test_ctx_init( test_ctx_t * tc,
  ...
  ulong link_fp   = 2UL * ( fd_mcache_footprint( LINK_DEPTH, LINK_MTU )
                          + fd_dcache_footprint( fd_dcache_req_data_sz( LINK_MTU, LINK_DEPTH, 1UL, 1 ), 0UL ) );
  ulong page_cnt  = (gossip_fp + link_fp) / FD_SHMEM_NORMAL_PAGE_SZ + 4096UL /* for wksp metadata */;
  ulong cpu_idx   = 0UL;
