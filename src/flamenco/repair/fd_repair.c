...
fd_repair_new ( void * shmem, ulong seed ) {
  ...
  void        * signpool = FD_SCRATCH_ALLOC_APPEND( l, fd_repair_pending_sign_req_pool_align(), fd_repair_pending_sign_req_pool_footprint( FD_REPAIR_PENDING_SIGN_REQ_MAX ) );
  void        * signmap  = FD_SCRATCH_ALLOC_APPEND( l, fd_repair_pending_sign_req_map_align(),  fd_repair_pending_sign_req_map_footprint( FD_REPAIR_PENDING_SIGN_REQ_MAX ) );
  ...
  repair->pending_sign_pool = fd_repair_pending_sign_req_pool_join( fd_repair_pending_sign_req_pool_new( signpool, FD_REPAIR_PENDING_SIGN_REQ_MAX       ) );
  repair->pending_sign_map  = fd_repair_pending_sign_req_map_join ( fd_repair_pending_sign_req_map_new ( signmap,  FD_REPAIR_PENDING_SIGN_REQ_MAX, seed ) );
  repair->last_decay = 0;
  repair->last_print = 0;
  repair->next_nonce = 0;
...
fd_repair_add_active_peer( fd_repair_t * glob, fd_ip4_port_t const * addr, fd_pubkey_t const * id ) {
  ...
  if (val == NULL) {
    ...
    /* This memory could get read by the catchup tool, so keep a valid
       state of the peer list at all times.*/
...
fd_repair_inflight_remove( fd_repair_t * glob,
  ...
  if( inflight_req ) {
    ...
    if( FD_LIKELY( active_elem ) ) {
      ...
      if( FD_UNLIKELY( active_elem->first_resp_ts == 0 ) ) active_elem->first_resp_ts = now;
      active_elem->last_resp_ts   = now;
