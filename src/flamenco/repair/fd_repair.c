...
fd_actives_shuffle( fd_repair_t * repair ) {
  ...
  FD_SCRATCH_SCOPE_BEGIN {
    ...
    if ( FD_UNLIKELY(( total_stake == ULONG_MAX )) ) {
      FD_LOG_WARNING(( "saturated total stake - need to change total_stake to support higher values" ));
    }
    ...
    if( leftovers_cnt ) {
      ...
      for( ulong i = 0UL; i < 64 && tot_cnt < FD_REPAIR_STICKY_MAX && tot_cnt < fd_active_table_key_cnt( repair->actives ); ++i ) {

        /* Repeatedly try to find a random peer we haven't selected yet, until we find one */
        for( ulong j = 0UL; j < fd_active_table_key_cnt( repair->actives ); j++ ) {
