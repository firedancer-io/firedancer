...
fd_stake_rewards_footprint( ulong max_stake_accounts,
  ...
  l = FD_LAYOUT_APPEND( l, alignof(fork_info_t),      fd_ulong_sat_mul( max_fork_width, sizeof(fork_info_t) ) );
...
fd_stake_rewards_new( void * shmem,
  ...
  if( FD_UNLIKELY( !max_fork_width || max_fork_width>FD_BANKS_MAX_BANKS ) ) {
    FD_LOG_WARNING(( "max_fork_width must be in [1,%lu]", FD_BANKS_MAX_BANKS ));
    return NULL;
  }
  ulong partition_ele_cnt = fd_ulong_sat_mul( max_fork_width, max_stake_accounts );
  ...
  void *               fork_info_mem   = FD_SCRATCH_ALLOC_APPEND( l, alignof(fork_info_t),     fd_ulong_sat_mul( max_fork_width, sizeof(fork_info_t) ) );
  ...
  stake_rewards->max_fork_width     = max_fork_width;
...
fd_stake_rewards_clear( fd_stake_rewards_t * stake_rewards ) {
  ...
  for( ulong i=0UL; i<stake_rewards->max_fork_width; i++ ) get_fork_info( stake_rewards, (ushort)i )->refcnt = 0UL;
  stake_rewards->epoch = ULONG_MAX;
}
...
fd_stake_rewards_window_lo( fd_stake_rewards_t const * stake_rewards,
                            ushort                     fork_idx ) {
  return get_fork_info( stake_rewards, fork_idx )->win_lo;
}

uint
fd_stake_rewards_window_hi( fd_stake_rewards_t const * stake_rewards,
                            ushort                     fork_idx ) {
  return get_fork_info( stake_rewards, fork_idx )->win_hi;
}
