...
calculate_reward_points_partitioned( fd_exec_slot_ctx_t *       slot_ctx,
  ...
  fd_tpool_exec_all_batch( tpool, 0UL, fd_tpool_worker_cnt( tpool ), calculate_points_tpool, temp_info->stake_infos, &task_args, NULL, 1UL, 0UL, temp_info->stake_infos_len );
...
FD_SCRATCH_SCOPE_BEGIN {
  ...
  fd_vote_reward_t_mapnode_t * vote_reward_map_pool = fd_vote_reward_t_map_join( fd_vote_reward_t_map_new( fd_valloc_malloc( fd_scratch_virtual(), // TODO: SPAD
...
calculate_stake_vote_rewards( fd_exec_slot_ctx_t *                       slot_ctx,
  ...
  result->stake_reward_calculation.pool = fd_stake_reward_pool_join( fd_stake_reward_pool_new( fd_valloc_malloc( valloc,
  ...
  result->vote_reward_map_pool = fd_vote_reward_t_map_join( fd_vote_reward_t_map_new( fd_valloc_malloc( valloc,
                                                                                                        fd_vote_reward_t_map_align(),
  ...
  fd_tpool_exec_all_batch( tpool, 0UL, fd_tpool_worker_cnt( tpool ), calculate_stake_vote_rewards_account_tpool, temp_info, &task_args, NULL, 1UL, 0UL, temp_info->stake_infos_len );
}
...
calculate_validator_rewards( fd_exec_slot_ctx_t *                      slot_ctx,
  ...
  calculate_reward_points_partitioned( slot_ctx, stake_history, rewards, &result->point_value, tpool, temp_info );
...
hash_rewards_into_partitions( fd_exec_slot_ctx_t *                        slot_ctx,
  ...
  result->partitioned_stake_rewards.partitions     = fd_valloc_malloc( valloc,
...
calculate_rewards_for_partitioning( fd_exec_slot_ctx_t                   * slot_ctx,
  ...
  calculate_validator_rewards( slot_ctx, prev_epoch, rewards.validator_rewards, validator_result, temp_info, tpool, valloc );
...
calculate_rewards_and_distribute_vote_rewards( fd_exec_slot_ctx_t *                                        slot_ctx,
  ...
  calculate_rewards_for_partitioning( slot_ctx, prev_epoch, parent_blockhash, rewards_calc_result, temp_info, tpool, valloc );
...
distribute_epoch_reward_to_stake_acc( fd_exec_slot_ctx_t * slot_ctx,
  ...
  if( fd_stake_get_state(stake_acc_rec, valloc, stake_state) != 0 ) {
...
static void
set_epoch_reward_status_inactive( fd_exec_slot_ctx_t * slot_ctx, fd_valloc_t valloc ) {
...
distribute_epoch_rewards_in_partition( fd_stake_reward_dlist_t * partition,
  ...
  for( fd_stake_reward_dlist_iter_t iter = fd_stake_reward_dlist_iter_fwd_init( partition, pool );
    ...
    if( distribute_epoch_reward_to_stake_acc( slot_ctx,
                                              ...
                                              stake_reward->credits_observed, valloc ) == 0 ) {
...
void
fd_distribute_partitioned_epoch_rewards( fd_exec_slot_ctx_t * slot_ctx, fd_valloc_t valloc ) {
  ...
  if( fd_ulong_sat_add( height, 1UL ) >= distribution_end_exclusive ) {
    set_epoch_reward_status_inactive( slot_ctx, valloc );
...
fd_update_rewards( fd_exec_slot_ctx_t * slot_ctx,
  ...
  calculate_rewards_and_distribute_vote_rewards( slot_ctx, parent_epoch, parent_blockhash, rewards_result, temp_info, tpool, valloc );
...
FD_SCRATCH_SCOPE_BEGIN {
  ...
  if ( FD_UNLIKELY( epoch_rewards->active ) ) {
    ...
    fd_accumulate_stake_infos( slot_ctx, stakes, stake_history, new_warmup_cooldown_rate_epoch, &_accumulator, &epoch_info, tpool, spads, spads_cnt, valloc );
    fd_refresh_vote_accounts( slot_ctx, stake_history, new_warmup_cooldown_rate_epoch, tpool, spads, spads_cnt, &epoch_info );
    ...
    calculate_stake_vote_rewards( slot_ctx, stake_history, rewarded_epoch, &point_value, calculate_stake_vote_rewards_result, &epoch_info, tpool, valloc );
