...
fd_multi_epoch_leaders_stake_msg_init( fd_multi_epoch_leaders_t * mleaders,
  ...
  mleaders->scratch->epoch          = hdr->epoch;
  mleaders->scratch->start_slot     = hdr->start_slot;
  mleaders->scratch->slot_cnt       = hdr->slot_cnt;
  mleaders->scratch->staked_cnt     = hdr->staked_cnt;
  mleaders->scratch->excluded_stake = hdr->excluded_stake;

  fd_memcpy( mleaders->stake_weight, hdr+1UL, sizeof(fd_stake_weight_t)*hdr->staked_cnt );
}
...
fd_multi_epoch_leaders_get_leader_for_slot( fd_multi_epoch_leaders_t const * mleaders,
                                            ulong                            slot ) {
  fd_pubkey_t const * even_leader_option = fd_ptr_if( mleaders->init_done[0], fd_epoch_leaders_get( mleaders->lsched[ 0 ], slot ), NULL );
  fd_pubkey_t const * odd_leader_option  = fd_ptr_if( mleaders->init_done[1], fd_epoch_leaders_get( mleaders->lsched[ 1 ], slot ), NULL );
