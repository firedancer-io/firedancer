...
/* The epoch info message also carries one compressed BLS voting pubkey
   ...
   ignore the trailing array.  FD_EPOCH_INFO_BLS_PUBKEY_SZ must equal
   FD_BLS_PUBKEY_COMPRESSED_SZ (asserted at the producer).  The array is
   provisioned for the Alpenglow voter cap, not the (much larger)
   leader-schedule stake-weight bound. */
