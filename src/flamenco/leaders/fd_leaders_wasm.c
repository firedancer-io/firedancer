...
static uchar __attribute__((aligned(FD_EPOCH_LEADERS_ALIGN))) shmem[ FD_EPOCH_LEADERS_FOOTPRINT(MAX_PUB_CNT, MAX_SLOTS_CNT) ];
...
fd_epoch_leaders_wasm_init( ulong         epoch,
   ...
   //  fd_stakes_sort_inplace( pubkeys_stakes, pub_cnt );
    ...
    fd_epoch_leaders_t * _lsched = fd_epoch_leaders_join( fd_epoch_leaders_new( shmem, epoch, slot0, slot_cnt, pub_cnt, pubkeys_stakes, excluded_stake ) );
...
fd_epoch_leaders_wasm_get_sched_cnt() {
   return ( (fd_epoch_leaders_t *)shmem )->sched_cnt;
}

uint *
fd_epoch_leaders_wasm_get_sched() {
   return ( (fd_epoch_leaders_t *)shmem )->sched;
}