...
main( int     argc,
  ...
  for( ulong i=0UL; i<432000UL; i++ ) {
    ...
    FD_TEST( !memcmp( fd_epoch_leaders_get_vote( leaders, slot0+i ), &stakes[leaders_idx[i]].vote_key, 32UL ) );
  }
     ...
     scheduled identity alone. */
  {
    ...
    FD_TEST( leaders_buf == fd_epoch_leaders_new( leaders_buf, 1UL, 0UL, test_slot_cnt, 3UL, shared_stakes ) );
    ...
    for( ulong slot=0UL; slot<test_slot_cnt; slot++ ) {
      ...
      for( ulong i=0UL; i<3UL; i++ ) {
        if( !memcmp( vote->uc, shared_stakes[i].vote_key.uc, 32UL ) ) {
          entry = i;
          break;
        }
      }
