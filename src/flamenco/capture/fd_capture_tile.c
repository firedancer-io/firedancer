...
typedef struct fd_capctx_tile_ctx {
  ulong tile_idx;
...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  l = FD_LAYOUT_APPEND( l, alignof(fd_capctx_tile_ctx_t), sizeof(fd_capctx_tile_ctx_t) );
...
static inline void
after_credit( fd_capctx_tile_ctx_t * ctx,
  ...
  fd_capctx_buf_t * capctx_buf = ctx->capctx_buf;
...
privileged_init( fd_topo_t *      topo,
  ...
  fd_capctx_tile_ctx_t * ctx = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_capctx_tile_ctx_t), sizeof(fd_capctx_tile_ctx_t) );
  ...
  ctx->file = fopen( "capture.solcap", "w+" );
  FD_TEST( ctx->file );
...
#define STEM_CALLBACK_CONTEXT_TYPE  fd_capctx_tile_ctx_t
#define STEM_CALLBACK_CONTEXT_ALIGN alignof(fd_capctx_tile_ctx_t)

#define STEM_CALLBACK_AFTER_CREDIT    after_credit

#include "../../disco/stem/fd_stem.c"

fd_topo_run_tile_t fd_tile_capctx = {
  .name                     = "capctx",
