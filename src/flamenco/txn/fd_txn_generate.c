...
fd_txn_base_generate( uchar out_txn_meta[ static FD_TXN_MAX_SZ ],
  ...
  *write_ptr = (uchar)txn_meta->acct_addr_cnt;
  ...
  if( signers_write_sz ) memcpy( write_ptr, accounts->signers_w, signers_write_sz );
  write_ptr += signers_write_sz;

  if( signers_read_sz ) memcpy( write_ptr, accounts->signers_r, signers_read_sz );
  write_ptr += signers_read_sz;

  if( non_signers_write_sz ) memcpy( write_ptr, accounts->non_signers_w, non_signers_write_sz );
  write_ptr += non_signers_write_sz;

  if( non_signers_read_sz ) memcpy( write_ptr, accounts->non_signers_r, non_signers_read_sz );
