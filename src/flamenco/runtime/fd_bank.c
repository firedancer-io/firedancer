...
fd_bank_t const *
fd_banks_publish( fd_banks_t * banks, fd_hash_t * block_id ) {
...
fd_bank_t *
fd_banks_rekey_root_bank( fd_banks_t * banks, fd_hash_t * block_id ) {
  ...
  bank = fd_banks_map_ele_remove( fd_banks_get_bank_map( banks ), &bank->block_id_, NULL, fd_banks_get_bank_pool( banks ) );
  if( FD_UNLIKELY( !bank ) ) {
...
fd_banks_rekey_bank_by_block_id( fd_banks_t * banks,
  ...
  fd_bank_t * bank_ = fd_banks_map_ele_remove( bank_map, &bank->block_id_, NULL, bank_pool );
  if( FD_UNLIKELY( !bank_ ) ) {
    FD_LOG_CRIT(( "invariant violation: failed to remove bank from map" ));
  }

  bank_->block_id_ = *new_block_id;

  fd_banks_map_ele_insert( bank_map, bank_, bank_pool );
