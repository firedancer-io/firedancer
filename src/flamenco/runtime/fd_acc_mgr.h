...
#define FD_FUNK_ACC_MGR_SUCCESS             (0)
#define FD_FUNK_ACC_MGR_ERR_UNKNOWN_ACCOUNT (-1)
#define FD_FUNK_ACC_MGR_ERR_WRITE_FAILED    (-2)
#define FD_FUNK_ACC_MGR_ERR_READ_FAILED     (-3)
#define FD_FUNK_ACC_MGR_ERR_WRONG_MAGIC     (-4)
...
/* fd_funk_get_account_meta_readonly requests a read-only handle to account data.
...
/* fd_funk_get_account_meta_mutable requests a writable handle to an account.
   ...
   - Returns pointer to mutable account metadata and data analogous to
     fd_funk_acc_mgr_get_acc_meta_readonly.
   - IMPORTANT:  Return value may point to the same memory region as a
     previous calls to fd_funk_acc_mgr_get_acc_meta_readonly or fd_acc_mgr_modify_raw do,
...
/* fd_funk_account_strerror converts an fd_acc_mgr error code into a human
