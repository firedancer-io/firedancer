...
fd_txncache_max_txnpages( ulong max_active_slots,
  ...
  /* The pool must hold the worst case set of simultaneously live
     ...
     FD_TXNCACHE_MAX_BLOCKHASH_DISTANCE further root advances, so up to
     max_active_slots forks' insertions are live at once.  The worst
     case is the boot transient: the snapshot root (holding the full
     ...
     indistinguishably; see FD_PACK_MAX_TXNCACHE_TXN_PER_SLOT).  Live
     inserts are 1x, so each non-snapshot fork holds at most
     max_txn_per_slot/2 entries.

     We count pages, not transactions, so pages might be wasted.  The
     maximum page wastage occurs when every blockcache has a partially
     full tail page, adding max_active_slots-1 pages beyond the global
...
fd_txncache_shmem_footprint( ulong max_live_slots,
  ...
  ulong resident_pages = fd_ulong_min( 1UL+(cache_footprint-1UL)/sizeof(fd_txncache_txnpage_t), _max_txnpages );
  ...
  l = FD_LAYOUT_APPEND( l, alignof(fd_txncache_page_meta_t), (resident_pages<_max_txnpages ? _max_txnpages : 0UL)*sizeof(fd_txncache_page_meta_t) );
  l = FD_LAYOUT_APPEND( l, alignof(ulong),                   (resident_pages<_max_txnpages ? resident_pages : 0UL)*sizeof(ulong)                  );
...
fd_txncache_shmem_new( void * shmem,
  ...
  if( FD_UNLIKELY( !cache_footprint ) ) return NULL;
  ...
  ulong bounded       = resident_pages<_max_txnpages;
  ...
  void * _frame_owner = FD_SCRATCH_ALLOC_APPEND( l, alignof(ulong),                   (bounded ? resident_pages : 0UL)*sizeof(ulong)                  );
  memset( _page_meta, 0, (bounded ? _max_txnpages : 0UL)*sizeof(fd_txncache_page_meta_t) );
  memset( _frame_owner, 0xFF, (bounded ? resident_pages : 0UL)*sizeof(ulong) );
  ...
  for( ulong i=0UL; i<max_active_slots; i++ ) {
    descends_set_t * descends_set = descends_set_join( descends_set_new( (uchar *)_blockcache_descends + i*_descends_footprint, max_active_slots ) );
