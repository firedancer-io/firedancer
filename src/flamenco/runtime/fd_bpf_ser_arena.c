...
bundle_laddr( fd_bpf_ser_arena_t * arena,
              ulong                idx ) {
  return (uchar *)arena + sizeof(fd_bpf_ser_arena_t) + idx*FD_BPF_SER_ARENA_BUNDLE_FOOTPRINT;
}
...
fd_bpf_ser_arena_release( fd_bpf_ser_arena_t * arena,
  ...
  ulong idx = off/FD_BPF_SER_ARENA_BUNDLE_FOOTPRINT;
  if( FD_UNLIKELY( (off%FD_BPF_SER_ARENA_BUNDLE_FOOTPRINT) || idx>=arena->bundle_cnt ) ) FD_LOG_CRIT(( "bad bundle" ));
