...
get_program_data_address( fd_pubkey_t const * program_addr ) {
  ...
  fd_pubkey_t   out;
  uint          custom_err;
  fd_pubkey_find_program_address( &fd_solana_bpf_loader_upgradeable_program_id, 1UL, &seed, &seed_sz, &out, NULL, &custom_err );
  return out;
}
...
target_builtin_new_checked( target_builtin_t *   target_builtin,
  ...
  case FD_CORE_BPF_MIGRATION_TARGET_BUILTIN:
    if( FD_UNLIKELY( !tmp_account_read( program_account, slot_ctx, program_address, spad ) ) ) {
...
source_buffer_new_checked( tmp_account_t *            acc,
  ...
  if( FD_UNLIKELY( !tmp_account_read( acc, slot_ctx, pubkey, spad ) ) ) {
...
new_target_program_account( tmp_account_t *            acc,
  ...
  if( FD_UNLIKELY( !rent ) ) FD_LOG_ERR(( "fd_bank_rent_query failed" ));
...
new_target_program_data_account( tmp_account_t *       acc,
  ...
  ulong        lamports = fd_rent_exempt_minimum_balance( fd_bank_rent_query( slot_ctx->bank ), space );
...
void
migrate_builtin_to_core_bpf1( fd_exec_slot_ctx_t *                   slot_ctx,
  ...
  ulong capitalization = fd_bank_capitalization_get( slot_ctx->bank );
  int cap_ok = 1;
  ...
  /* Write back accounts */
  tmp_account_store( new_target_program,      slot_ctx );
  tmp_account_store( new_target_program_data, slot_ctx );
  tmp_account_t empty; tmp_account_new( &empty, spad, 0UL );
  empty.addr = source->addr;
  tmp_account_store( &empty, slot_ctx );
...
fd_migrate_builtin_to_core_bpf( fd_exec_slot_ctx_t *                   slot_ctx,
                                ...
                                fd_spad_t *                            spad ) {
  fd_spad_push( spad );
  migrate_builtin_to_core_bpf1( slot_ctx, config->builtin_program_id, config, spad );
  fd_spad_pop( spad );
}
