...
fd_slot_params_t const FD_SLOT_PARAMS_400MS = {
  .ns_per_slot                                                = (uint128)400000000UL,
  ...
  .max_data_shreds_per_slot                                   = 32768U,
  .max_code_shreds_per_slot                                   = 32768U,
...
fd_slot_params_t const FD_SLOT_PARAMS_350MS = {
  .ns_per_slot                                                = (uint128)350000000UL,
  ...
  .max_data_shreds_per_slot                                   = 28672U,
  .max_code_shreds_per_slot                                   = 28672U,
...
fd_slot_params_t const FD_SLOT_PARAMS_300MS = {
  .ns_per_slot                                                = (uint128)300000000UL,
  ...
  .max_data_shreds_per_slot                                   = 24576U,
  .max_code_shreds_per_slot                                   = 24576U,
...
fd_slot_params_t const FD_SLOT_PARAMS_250MS = {
  .ns_per_slot                                                = (uint128)250000000UL,
  ...
  .max_data_shreds_per_slot                                   = 20480U,
  .max_code_shreds_per_slot                                   = 20480U,
...
fd_slot_params_t const FD_SLOT_PARAMS_200MS = {
  .ns_per_slot                                                = (uint128)200000000UL,
  ...
  .max_data_shreds_per_slot                                   = 16384U,
  .max_code_shreds_per_slot                                   = 16384U,
...
fd_slot_params_at_slot( fd_features_t const *       features,
  ...
  for( ulong i=0UL; i<FD_SLOT_TIME_GATE_CNT; i++ ) {
    ...
    if( gate->params->ns_per_slot < result.ns_per_slot &&
        fd_slot_params_feature_effective_slot( features, epoch_schedule, gate ) <= slot ) {
          result = *gate->params;
    }
  }
  return result;
...
fd_slot_params_next_transition( fd_features_t const *       features,
                                fd_epoch_schedule_t const * epoch_schedule,
                                ulong                       slot,
                                uint128                     current_ns_per_slot ) {
  ulong next = ULONG_MAX;
  for( ulong i=0UL; i<FD_SLOT_TIME_GATE_CNT; i++ ) {
    fd_slot_time_gate_t const * gate = &fd_slot_time_gates[ i ];
    if( gate->params->ns_per_slot >= current_ns_per_slot ) continue; /* redundant */
...
fd_slot_params_slot_range_duration_ns( fd_features_t const *       features,
  ...
  while( curr_slot<end_slot ) {
    ...
    ulong            seg_end   = next<end_slot ? next : end_slot;
                     ns        = fd_uint128_sat_add( ns, fd_uint128_sat_mul( (uint128)( seg_end-curr_slot ), params.ns_per_slot ) );
                     curr_slot = seg_end;
  }
