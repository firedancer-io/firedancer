...
struct fd_block_map {
  ...
  /* Data
   ...
   data_gaddr points to an allocated memory region of data_sz that
   contains a concatenation of the block itself, shreds metadata
   (fd_block_shred_t) and microblocks metadata (fd_block_micro_t).

   shred_gaddr, shred_cnt, micro_gaddr, micro_cnt are additional fields
   to support iterating by shred or microblock.  Additionally, callers
   can  iterate transactions within microblocks.
...
/* fd_blockstore_shred_complete returns 1 if the blockstore has received
   all shreds for slot, otherwise 0. */

static inline int
fd_blockstore_shred_complete( fd_blockstore_t * blockstore, ulong slot ) {
  fd_block_map_t * block_map_entry = fd_blockstore_block_map_query( blockstore, slot );
  if ( FD_UNLIKELY( !block_map_entry ) ) return 0;
  return block_map_entry->consumed_idx != FD_SHRED_IDX_NULL &&
