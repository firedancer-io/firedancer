...
/* fd_banks_mark_bank_dead marks the current bank (and all of its
   ...
   it is set to the number of banks newly marked dead.  The caller is
   still responsible for handling the behavior of the dead bank
   correctly.  After a bank is marked dead, the caller should never
   increment the reference count on the bank. */
