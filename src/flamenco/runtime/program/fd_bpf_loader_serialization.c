    ...
    if( FD_UNLIKELY( acc_idx_seen[acc_idx] ) ) {
      ...
      if( !stricter_abi_and_runtime_constraints ) {
        ...
        if( fd_borrowed_account_can_data_be_resized( &view_acc, pre_len, &can_data_be_resized_err ) &&
          ...
          if( FD_UNLIKELY( set_data_err ) ) {
            FD_BASE58_ENCODE_32_BYTES( view_acc.pubkey->uc, key_b58 );
            FD_LOG_WARNING(( "set_data_err: %d, key: %s pre %lu post %lu", set_data_err, key_b58, pre_len, post_len ));
            FD_LOG_HEXDUMP_WARNING(( "pre", fd_borrowed_account_get_data( &view_acc ), pre_len ));
            FD_LOG_HEXDUMP_WARNING(( "post", post_data, post_len ));
            return set_data_err;
          }
