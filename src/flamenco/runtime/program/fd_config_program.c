...
# define ACC_IDX_CONFIG ((uchar)0)
  ...
  config_account_key = fd_borrowed_account_get_pubkey( &config_acc_rec );
  ...
  for( ulong i=0UL; i<key_list->keys_len; i++ ) {
    ...
    if( 0!=memcmp( signer, config_account_key, sizeof(fd_pubkey_t) ) ) {
      ...
      if( FD_UNLIKELY( 0!=memcmp( fd_borrowed_account_get_pubkey( &signer_account ), signer, sizeof(fd_pubkey_t) ) ) ) {
