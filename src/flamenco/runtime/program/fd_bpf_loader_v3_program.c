...
execute( fd_exec_instr_ctx_t * instr_ctx, fd_sbpf_validated_program_t * prog ) {
  ...
  FD_LOG_NOTICE(( "Program consumed %lu of %lu compute units; start=%lu end=%lu",
                  before_instr_cus - instr_ctx->txn_ctx->compute_meter, before_instr_cus, before_instr_cus, instr_ctx->txn_ctx->compute_meter ));
