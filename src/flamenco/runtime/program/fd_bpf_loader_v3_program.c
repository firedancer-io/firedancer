...
fd_bpf_loader_v3_program_write_state( fd_exec_instr_ctx_t *               instr_ctx,
  ...
  /* The Agave client implementation of this function parallels set_state() 
     function which calls get_data_mut() which further calls 
     can_data_be_changed(). */
  int err;
...
common_close_account( fd_pubkey_t * authority_address,
  ...
  err = fd_bpf_loader_v3_program_write_state( instr_ctx, 0UL, close_account, state );
  if( FD_UNLIKELY( err!=FD_BINCODE_SUCCESS ) ) {
...
process_loader_upgradeable_instruction( fd_exec_instr_ctx_t * instr_ctx ) {
  ...
  switch( instruction.discriminant ) {
    ...
    case fd_bpf_upgradeable_loader_program_instruction_enum_initialize_buffer: {
      ...
      err = fd_bpf_loader_v3_program_write_state( instr_ctx, 0UL, buffer, &buffer_state );
      if( FD_UNLIKELY( err!=FD_BINCODE_SUCCESS ) ) {
    ...
    case fd_bpf_upgradeable_loader_program_instruction_enum_deploy_with_max_data_len: {
      ...
      err = fd_bpf_loader_v3_program_write_state( instr_ctx, 1UL, programdata, &programdata_loader_state );
      if( FD_UNLIKELY( err!=FD_BINCODE_SUCCESS ) ) {
    ...
    case fd_bpf_upgradeable_loader_program_instruction_enum_upgrade: {
      ...
      err = fd_bpf_loader_v3_program_write_state( instr_ctx, 0UL, programdata, &programdata_state );
      if( FD_UNLIKELY( err!=FD_BINCODE_SUCCESS ) ) {
        FD_LOG_WARNING(( "Bpf state write for programdata account failed" ));
        return err;
      }
      ...
      if( FD_UNLIKELY( !fd_account_set_data_length( instr_ctx, 2UL, PROGRAMDATA_METADATA_SIZE, &err ) ) ) {
        return err;
      }
      break;
    }
    ...
    case fd_bpf_upgradeable_loader_program_instruction_enum_set_authority: {
      ...
      if( fd_bpf_upgradeable_loader_state_is_buffer( &account_state ) ) {
        ...
        err = fd_bpf_loader_v3_program_write_state( instr_ctx, 0UL, account, &account_state );
        if( FD_UNLIKELY( err!=FD_BINCODE_SUCCESS ) ) {
    ...
    case fd_bpf_upgradeable_loader_program_instruction_enum_close: {
      ...
      if( FD_UNLIKELY( !fd_account_set_data_length( instr_ctx, 0UL, SIZE_OF_UNINITIALIZED, &err ) ) ) {
        return err;
      }
    ...
    case fd_bpf_upgradeable_loader_program_instruction_enum_extend_program: {
      ...
      err = fd_bpf_loader_v3_program_write_state( instr_ctx, 0UL, programdata_account, &programdata_state );
      if( FD_UNLIKELY( err!=FD_BINCODE_SUCCESS ) ) {
