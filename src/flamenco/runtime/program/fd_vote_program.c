...
set_vote_account_state( fd_borrowed_account_t *     vote_account,
  ...
  fd_rent_t const rent               = fd_sysvar_rent_read_nofail( ctx->sysvar_cache );
...
update_commission( fd_borrowed_account_t *     vote_account,
  ...
  if( FD_FEATURE_ACTIVE_BANK( ctx->txn_ctx->bank, allow_commission_decrease_at_any_time ) ) {
  ...
  if( FD_LIKELY( enforce_commission_update_rule &&
                 FD_FEATURE_ACTIVE_BANK( ctx->txn_ctx->bank, commission_updates_only_allowed_in_first_half_of_epoch ) ) ) {
...
fd_vote_program_execute( fd_exec_instr_ctx_t * ctx ) {
  ...
  case fd_vote_instruction_enum_update_commission: {
    ...
    fd_epoch_schedule_t const * epoch_schedule = fd_sysvar_epoch_schedule_read( ctx->sysvar_cache, &epoch_schedule_ );
  ...
  case fd_vote_instruction_enum_vote_switch: {
    if( FD_FEATURE_ACTIVE_BANK( ctx->txn_ctx->bank, deprecate_legacy_vote_ixs ) &&
        FD_FEATURE_ACTIVE_BANK( ctx->txn_ctx->bank, enable_tower_sync_ix ) ) {
      return FD_EXECUTOR_INSTR_ERR_INVALID_INSTR_DATA;
    }
    ...
    if( FD_UNLIKELY( !fd_sysvar_slot_hashes_is_valid( ctx->sysvar_cache ) ) ) {
      return FD_EXECUTOR_INSTR_ERR_UNSUPPORTED_SYSVAR;
    }
    ...
    fd_slot_hash_t const * slot_hashes = fd_sysvar_slot_hashes_join_const( ctx->sysvar_cache ); /* guaranteed to succeed */
    rc = process_vote_with_account( &me, slot_hashes, clock, vote, signers, ctx );
    fd_sysvar_slot_hashes_leave_const( ctx->sysvar_cache, slot_hashes );
  ...
  case fd_vote_instruction_enum_update_vote_state_switch: {
    ...
    if( FD_LIKELY( !fd_sysvar_slot_hashes_is_valid( ctx->sysvar_cache ) ) ) {
      return FD_EXECUTOR_INSTR_ERR_UNSUPPORTED_SYSVAR;
    }
    ...
    fd_slot_hash_t const * slot_hashes = fd_sysvar_slot_hashes_join_const( ctx->sysvar_cache );
  ...
  case fd_vote_instruction_enum_tower_sync_switch: {
    if( !FD_FEATURE_ACTIVE_BANK( ctx->txn_ctx->bank, enable_tower_sync_ix ) ) {
      return FD_EXECUTOR_INSTR_ERR_INVALID_INSTR_DATA;
    }
  ...
  case fd_vote_instruction_enum_withdraw: {
    ...
    fd_rent_t rent_;
    fd_rent_t const * rent_sysvar = fd_sysvar_rent_read( ctx->sysvar_cache, &rent_ );
    if( FD_UNLIKELY( !rent_sysvar ) )
      return FD_EXECUTOR_INSTR_ERR_UNSUPPORTED_SYSVAR;
    fd_sol_sysvar_clock_t clock_;
    fd_sol_sysvar_clock_t const * clock_sysvar = fd_sysvar_clock_read( ctx->sysvar_cache, &clock_ );
