...
same. This is especially important given the Vote Program's implications for consensus.
...
Henceforth, thou shalt abide by the transliteration commandments:
  ...
  - All modules shall be structured similarly, and prefixed in C accordingly.
    - e.g. `VoteState::new` in Rust becomes `new` under comment subheading `impl VoteState` in C.
    - It is possible the name is overloaded. For example, there is both `new` in `impl Vote` and
`impl VoteState`. In those situations, prefix with the namespace e.g. `vote_new` vs.
`vote_state_new`.
...
vote_state_update_commission( fd_borrowed_account_t * vote_account,
                              uchar                   commission,
                              fd_pubkey_t const *     signers[static SIGNERS_MAX],
                              fd_exec_instr_ctx_t     ctx /* feature set */ );
...
static void
vote_state_process_next_vote_slot( fd_vote_state_t * self,
...
vote_state1_14_11_from_vote_state( fd_vote_state_t *         vote_state,
                                   fd_exec_instr_ctx_t *     ctx,
                                   fd_vote_state_1_14_11_t * vote_state1_14_11 /* out */ );
...
static int
decode_compact_update( fd_exec_instr_ctx_t              ctx,
...
fd_executor_vote_program_execute_instruction( fd_exec_instr_ctx_t ctx ) {
  ...
  case fd_vote_instruction_enum_update_commission: {
    ...
    if( FD_LIKELY( FD_FEATURE_ACTIVE( ctx.slot_ctx,
      ...
      if( FD_UNLIKELY( !vote_state_is_commission_update_allowed( clock.slot, &epoch_schedule ) ) ) {
  ...
  case fd_vote_instruction_enum_vote_switch: {
    ...
    rc = fd_sysvar_slot_hashes_read( ctx.slot_ctx, &slot_hashes );
...
vote_state_check_update_vote_state_slots_are_valid( fd_vote_state_t *        vote_state,
  ...
  FD_LOG_WARNING(
      ( "Slot hashes index %lu Slot Hash %32J Vote hash %32J",
        slot_hashes_index,
        deq_fd_slot_hash_t_peek_index_const( slot_hashes->hashes, slot_hashes_index )->hash.key,
        vote_state_update->hash.key ) );
...
vote_state_process_new_vote_state( fd_vote_state_t *   vote_state,
    ...
    if( FD_LIKELY( previous_vote ) ) {
      if( FD_UNLIKELY( previous_vote->lockout.slot >= vote->lockout.slot ) ) {
        ...
        return FD_EXECUTOR_INSTR_ERR_CUSTOM_ERR;
                              ...
                              lockout_last_locked_out_slot( &previous_vote->lockout ) ) ) {
  ...
  while( current_vote_state_index < deq_fd_landed_vote_t_cnt( vote_state->votes ) &&
    ...
    if( FD_LIKELY( current_vote->lockout.slot < new_vote->lockout.slot ) ) {
      ...
      current_vote_state_index = fd_ulong_checked_add_expect(current_vote_state_index, 1, "`current_vote_state_index` is bounded by `MAX_LOCKOUT_HISTORY` when slot is less than proposed");
      ...
      new_vote->latency = deq_fd_landed_vote_t_peek_index(vote_state->votes, current_vote_state_index)->latency;

      current_vote_state_index = fd_ulong_checked_add_expect(current_vote_state_index, 1, "`current_vote_state_index` is bounded by `MAX_LOCKOUT_HISTORY` when slot is equal to proposed");
      new_vote_state_index = fd_ulong_checked_add_expect(new_vote_state_index, 1, "`new_vote_state_index` is bounded by `MAX_LOCKOUT_HISTORY` when slot is equal to proposed");
    } else {
      new_vote_state_index = fd_ulong_checked_add_expect(new_vote_state_index, 1, "`new_vote_state_index` is bounded by `MAX_LOCKOUT_HISTORY` when slot is greater than proposed");
    }
  }
  ...
  if( ( !both_none && ( vote_state->root_slot.is_some != new_root.is_some ||
    ...
    vote_state_increment_credits( vote_state, epoch, earned_credits );
  }
...
vote_state_withdraw(
  ...
  rc = fd_instr_borrowed_account_modify(
      &ctx, vote_account->pubkey, 0, 0 /* TODO min_data_sz */, &vote_account );
  if( FD_UNLIKELY( rc != FD_PROGRAM_OK ) ) return rc;
  ...
  rc = fd_instr_borrowed_account_modify(
      &ctx, &txn_accs[instr_acc_idxs[to_account_index]], 0, 0 /* TODO min_data_sz */, &to_account );
  if( FD_UNLIKELY( rc != FD_PROGRAM_OK ) ) return rc;
...
vote_state_set_vote_account_state( fd_borrowed_account_t * vote_account,
  ...
  if( FD_FEATURE_ACTIVE( ctx->slot_ctx, vote_state_add_vote_latency ) ) {
    ...
    if( FD_UNLIKELY(
            ( vote_account_get_data_len( vote_account ) < vsz ) &&
            ( !vote_account_is_rent_exempt_at_data_length( vote_account, vsz, *ctx ) ||
              ( vote_account_set_data_length( vote_account, vsz, *ctx ) != FD_PROGRAM_OK ) ) ) ) {
...
vote_state_process_vote_unfiltered( fd_vote_state_t *   vote_state,
  ...
  for( deq_ulong_iter_t iter = deq_ulong_iter_init( vote_slots );
    ...
    vote_state_process_next_vote_slot( vote_state, *ele, epoch, current_slot );
  }
  return FD_PROGRAM_OK;
}
...
vote_state_initialize_account( fd_borrowed_account_t *       vote_account,
  ...
  if( FD_UNLIKELY( vote_account_data_len !=
                   vote_state_versions_vote_state_size_of(
                       FD_FEATURE_ACTIVE( ctx.slot_ctx, vote_state_add_vote_latency ) ) ) ) {
...
vote_state_process_vote_with_account( fd_borrowed_account_t *       vote_account,
  ...
  rc = vote_state_process_vote( &vote_state, vote, slot_hashes, clock->epoch, clock->slot, ctx );
  if( FD_UNLIKELY( rc != FD_PROGRAM_OK ) ) return rc;
...
vote_state_do_process_vote_state_update( fd_vote_state_t *        vote_state,
  ...
  rc = vote_state_process_new_vote_state( vote_state,
                                          landed_votes,
                                          vote_state_update->root,
...
vote_state_process_next_vote_slot( fd_vote_state_t * self,
  ...
  if( FD_UNLIKELY( deq_fd_landed_vote_t_cnt( self->votes ) == MAX_LOCKOUT_HISTORY ) ) {
    ...
    vote_state_increment_credits( self, epoch, credits );
  }
...
vote_state1_14_11_from_vote_state( fd_vote_state_t *         vote_state,
  ...
  if( NULL != vote_state->votes ) {
    vote_state1_14_11->votes = deq_fd_vote_lockout_t_alloc( ctx->valloc );
...
vote_account_set_data_length( fd_borrowed_account_t * self,
  ...
  if( FD_UNLIKELY( vote_account_get_data_len( self ) == new_length ) ) return FD_PROGRAM_OK;

  rc = fd_account_touch( &ctx, self->const_meta, self->pubkey, &rc );
  if( FD_UNLIKELY( rc != FD_PROGRAM_OK ) ) return rc;

  // Tell funk we have a new length...
  rc = fd_instr_borrowed_account_modify( &ctx, self->pubkey, 0, new_length, &self );
  switch( rc ) {
  case FD_ACC_MGR_SUCCESS:
    return FD_PROGRAM_OK;
  case FD_ACC_MGR_ERR_UNKNOWN_ACCOUNT:
...
set_state( fd_borrowed_account_t *     self,
  ...
  int rc = fd_instr_borrowed_account_modify( &ctx, self->pubkey, 0, serialized_sz, &self );
  if( FD_UNLIKELY( rc != FD_PROGRAM_OK ) ) return rc;
