...
from_vote_state_1_14_11( fd_vote_state_t *         vote_state,
  ...
  if( vote_state->votes ) {
    ulong   vote_cnt  = deq_fd_landed_vote_t_cnt( vote_state->votes );
    uchar * deque_mem = fd_spad_alloc( spad, deq_fd_vote_lockout_t_align(),
                                       deq_fd_vote_lockout_t_footprint( vote_cnt ) );
    vote_state_1_14_11->votes = deq_fd_vote_lockout_t_join( deq_fd_vote_lockout_t_new( deque_mem, vote_cnt ) );
...
do_process_vote_state_update( fd_vote_state_t *           vote_state,
  ...
  ulong              vote_cnt     = deq_fd_vote_lockout_t_cnt( vote_state_update->lockouts );
  uchar *            deque_mem    = fd_spad_alloc( ctx->txn_ctx->spad,
                                                   deq_fd_landed_vote_t_align(),
                                                   deq_fd_landed_vote_t_footprint( vote_cnt ) );
  fd_landed_vote_t * landed_votes = deq_fd_landed_vote_t_join( deq_fd_landed_vote_t_new( deque_mem, vote_cnt ) );
