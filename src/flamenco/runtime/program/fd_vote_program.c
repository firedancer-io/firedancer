...
set_vote_account_state( fd_borrowed_account_t *     vote_account,
  ...
  if( FD_FEATURE_ACTIVE( ctx->slot_ctx, vote_state_add_vote_latency ) ) {
    ...
    int resize_rent_exempt = fd_rent_exempt_minimum_balance( &epoch_bank->rent, vsz ) <= vote_account->acct->const_meta->info.lamports;
...
withdraw( fd_exec_instr_ctx_t const *   ctx,
  ...
  rc = fd_exec_instr_ctx_try_borrow_account( ctx, to_account_index, &to );
  if( FD_UNLIKELY( rc ) ) return rc;
...
fd_vote_program_execute( fd_exec_instr_ctx_t * ctx ) {
  ...
  rc = fd_exec_instr_ctx_try_borrow_account( ctx, 0, &me );
  if( FD_UNLIKELY( rc ) ) return rc;
