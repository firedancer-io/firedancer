...
check_program_account( fd_exec_instr_ctx_t *         instr_ctx,
  ...
  *err = FD_EXECUTOR_INSTR_SUCCESS;
  ...
  void const * data = fd_txn_account_get_data    ( program->acct );
  ulong        dlen = fd_txn_account_get_data_len( program->acct );
  fd_loader_v4_state_t const * state = fd_loader_v4_get_state( data, dlen, err );
  if( FD_UNLIKELY( *err ) ) {
    return NULL;
  }
