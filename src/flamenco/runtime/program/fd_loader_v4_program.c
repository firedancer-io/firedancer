...
check_program_account( fd_exec_instr_ctx_t *         instr_ctx,
  ...
  fd_exec_result_t res = fd_loader_v4_get_state( program, state );
...
fd_loader_v4_program_instruction_write( fd_exec_instr_ctx_t *                            instr_ctx,
  ...
  FD_BORROWED_ACCOUNT_TRY_BORROW_IDX( instr_ctx, 0UL, program ) {
    ...
    res = check_program_account( instr_ctx, program, authority_address, &state );
    if( FD_UNLIKELY( res.err ) ) {
      return res;
    }
    ...
    uchar * data = NULL;
    ulong dlen = 0UL;
    res = fd_account_get_data_mut( instr_ctx, 0UL, &data, &dlen );
    if( FD_UNLIKELY( res.err ) ) {
      return res;
    }
...
fd_loader_v4_program_instruction_truncate( fd_exec_instr_ctx_t *                               instr_ctx,
  ...
  FD_BORROWED_ACCOUNT_TRY_BORROW_IDX( instr_ctx, 0UL, program ) {
    ...
    if( FD_UNLIKELY( program->const_meta->info.lamports<required_lamports ) ) {
      ...
      FD_BORROWED_ACCOUNT_TRY_BORROW_IDX( instr_ctx, 2UL, recipient ) {
        ...
        ulong lamports_to_receive = fd_ulong_sat_sub( program->const_meta->info.lamports,
                                                      required_lamports );
        res = fd_account_checked_sub_lamports( instr_ctx, 0UL, lamports_to_receive );
        if( FD_UNLIKELY( res.err ) ) {
          return res;
        }
        res = fd_account_checked_add_lamports( instr_ctx, 2UL, lamports_to_receive );
        if( FD_UNLIKELY( res.err ) ) {
          return res;
        }
    ...
    if( new_size==0UL ) {
      ...
      res = fd_account_set_data_length( instr_ctx, 0UL, 0UL );
      if( FD_UNLIKELY( res.err ) ) {
        return res;
      }
    } else {
      /* https://github.com/anza-xyz/agave/blob/v2.1.4/programs/loader-v4/src/lib.rs#L197-L199 */
      res = fd_account_set_data_length( instr_ctx, 0UL, new_program_dlen );
      if( FD_UNLIKELY( res.err ) ) {
        return res;
      }
      ...
      if( is_initialization ) {
        ...
        res = fd_loader_v4_set_state( instr_ctx, 0UL, &state );
        if( FD_UNLIKELY( res.err ) ) {
          return res;
        }
      }
...
fd_loader_v4_program_instruction_deploy( fd_exec_instr_ctx_t * instr_ctx ) {
  ...
  FD_BORROWED_ACCOUNT_TRY_BORROW_IDX( instr_ctx, buffer_idx, buffer ) {
    ...
    if( source_program_present ) {
      ...
      res = check_program_account( instr_ctx, buffer, authority_address, &source_state );
      if( FD_UNLIKELY( res.err ) ) {
        return res;
      }
    ...
    if( source_program_present ) {
      ...
      FD_BORROWED_ACCOUNT_TRY_BORROW_IDX( instr_ctx, 0UL, program ) {
        ...
        res = fd_account_set_data_from_slice( instr_ctx, 0UL, buffer->const_data, buffer->const_meta->dlen );
        if( FD_UNLIKELY( res.err ) ) {
          return res;
        }

        /* https://github.com/anza-xyz/agave/blob/v2.1.4/programs/loader-v4/src/lib.rs#L300 */
        res = fd_account_set_data_length( instr_ctx, 2UL, 0UL );
        if( FD_UNLIKELY( res.err ) ) {
          return res;
        }

        /* https://github.com/anza-xyz/agave/blob/v2.1.4/programs/loader-v4/src/lib.rs#L301 */
        res = fd_account_checked_sub_lamports( instr_ctx, 2UL, transfer_lamports );
        if( FD_UNLIKELY( res.err ) ) {
          return res;
        }

        /* https://github.com/anza-xyz/agave/blob/v2.1.4/programs/loader-v4/src/lib.rs#L302 */
        res = fd_account_checked_add_lamports( instr_ctx, 0UL, transfer_lamports );
        if( FD_UNLIKELY( res.err ) ) {
          return res;
        }
      } FD_BORROWED_ACCOUNT_DROP( program );
  ...
  FD_BORROWED_ACCOUNT_TRY_BORROW_IDX( instr_ctx, 0UL, program ) {
    ...
    res = fd_loader_v4_set_state( instr_ctx, 0UL, &program_state );
    if( FD_UNLIKELY( res.err ) ) {
      return res;
    }
...
fd_loader_v4_program_instruction_finalize( fd_exec_instr_ctx_t * instr_ctx ) {
  ...
  FD_BORROWED_ACCOUNT_TRY_BORROW_IDX( instr_ctx, 2UL, next_version ) {
    ...
    res = fd_loader_v4_get_state( next_version, &state_of_next_version );
    if( FD_UNLIKELY( res.err ) ) {
      return res;
    }
...
fd_loader_v4_program_execute( fd_exec_instr_ctx_t * instr_ctx ) {
  ...
  FD_SPAD_FRAME_BEGIN( instr_ctx->txn_ctx->spad ) {
    ...
    fd_exec_result_t res = FD_EXECUTOR_INSTR_SUCCESS;
    if( !memcmp( program_id, fd_solana_bpf_loader_v4_program_id.key, sizeof(fd_pubkey_t) ) ) {
      ...
      switch( instruction.discriminant ) {
        case fd_loader_v4_program_instruction_enum_write: {
          ...
          res = fd_loader_v4_program_instruction_write( instr_ctx, &instruction.inner.write );
          break;
        }
        case fd_loader_v4_program_instruction_enum_truncate: {
          ...
          res = fd_loader_v4_program_instruction_truncate( instr_ctx, &instruction.inner.truncate );
          break;
        }
        case fd_loader_v4_program_instruction_enum_deploy: {
          ...
          res = fd_loader_v4_program_instruction_deploy( instr_ctx );
          break;
        }
        case fd_loader_v4_program_instruction_enum_retract: {
          ...
          res = fd_loader_v4_program_instruction_retract( instr_ctx );
          break;
        }
        case fd_loader_v4_program_instruction_enum_transfer_authority: {
          ...
          res = fd_loader_v4_program_instruction_transfer_authority( instr_ctx );
          break;
        }
        case fd_loader_v4_program_instruction_enum_finalize: {
          ...
          res = fd_loader_v4_program_instruction_finalize( instr_ctx );
          break;
        }
      }
    } else {
      ...
      int err = fd_txn_borrowed_account_view_idx( instr_ctx->txn_ctx, instr_ctx->instr->program_id, &program );
      if( FD_UNLIKELY( err ) ) {
        return fd_instr_err( err );
      }
      ...
      fd_loader_v4_state_t state = {0};
      res = fd_loader_v4_get_state( program, &state );
      if( FD_UNLIKELY( res.err ) ) {
        return res;
      }
      ...
      res = fd_bpf_execute( instr_ctx, prog, 0 );
    }
