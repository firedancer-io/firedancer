...
fd_builtin_programs_init( fd_bank_t *               bank,
  ...
  for( ulong i=0UL; i<fd_num_builtins(); i++ ) {
    ...
    if( bank->data->fields.slot==0UL && builtins[i].enable_feature_offset==NO_ENABLE_FEATURE_ID && !fd_builtin_is_bpf( accdb, xid, builtins[i].pubkey ) ) {
      fd_write_builtin_account( bank, accdb, xid, capture_ctx, *builtins[i].pubkey, builtins[i].data, strlen( builtins[i].data ) );
    } else if( builtins[i].core_bpf_migration_config && FD_FEATURE_ACTIVE_OFFSET( bank->data->fields.slot, &bank->data->fields.features, builtins[i].core_bpf_migration_config->enable_feature_offset ) ) {
      continue;
    } else if( builtins[i].enable_feature_offset!=NO_ENABLE_FEATURE_ID && !FD_FEATURE_ACTIVE_OFFSET( bank->data->fields.slot, &bank->data->fields.features, builtins[i].enable_feature_offset ) ) {
      continue;
    } else {
      fd_write_builtin_account( bank, accdb, xid, capture_ctx, *builtins[i].pubkey, builtins[i].data, strlen(builtins[i].data) );
    }
  ...
  for( ulong i=0; i<MIGRATING_BUILTINS_COUNT; i++ ) {
    ...
    if( !memcmp( pubkey->uc, config->builtin_program_id->key, sizeof(fd_pubkey_t) ) ) {
      if( config->enable_feature_offset!=NO_ENABLE_FEATURE_ID &&
        FD_FEATURE_ACTIVE_OFFSET( bank->data->fields.slot, &bank->data->fields.features, config->enable_feature_offset ) ) {
