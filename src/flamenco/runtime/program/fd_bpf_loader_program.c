...
fd_deploy_program( fd_exec_instr_ctx_t * instr_ctx,
  ...
  ulong deploy_slot = instr_ctx->bank->data->fields.slot+1UL;

  fd_vm_syscall_register_slot( syscalls,
                               deploy_slot,
                               &instr_ctx->bank->data->fields.features,
                               1 );

  /* Load executable */
  fd_sbpf_elf_info_t elf_info[ 1UL ];
  fd_prog_versions_t versions = fd_prog_versions( &instr_ctx->bank->data->fields.features, deploy_slot );
...
fd_bpf_execute( fd_exec_instr_ctx_t *      instr_ctx,
  ...
  fd_vm_syscall_register_slot( syscalls,
                               instr_ctx->bank->data->fields.slot,
                               &instr_ctx->bank->data->fields.features,
                               0 );
  ...
  int dump_syscall_to_pb = instr_ctx->runtime->log.dump_proto_ctx &&
                           instr_ctx->bank->data->fields.slot>=instr_ctx->runtime->log.dump_proto_ctx->dump_proto_start_slot &&
...
fd_bpf_loader_program_execute( fd_exec_instr_ctx_t * ctx ) {
  ...
  if( !memcmp( metadata->owner, &fd_solana_bpf_loader_upgradeable_program_id, sizeof(fd_pubkey_t) ) ) {
    ...
    if( FD_UNLIKELY( program_data_slot>=ctx->bank->data->fields.slot ) ) {
  ...
  fd_funk_txn_xid_t xid = { .ul = { ctx->bank->data->fields.slot, ctx->bank->data->idx } };
  fd_progcache_rec_t const * cache_entry =
