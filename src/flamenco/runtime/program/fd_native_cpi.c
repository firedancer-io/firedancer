...
fd_native_cpi_execute_system_program_instruction( fd_exec_instr_ctx_t * ctx,
  ...
  for ( ushort j = 0; j < acct_metas_len; j++ ) {
    ...
    for ( ushort k = 0; k < ctx->instr->acct_cnt; k++ ) {
      /* TODO check if agave checks for errrors here! */
      ushort idx_in_txn = ctx->instr->accounts[k].index_in_transaction;
      if ( memcmp( acct_meta->pubkey, ctx->txn_ctx->account_keys[idx_in_txn].uc, sizeof(fd_pubkey_t) ) == 0 ) {
        ...
        instr_info->accounts[j] = fd_instruction_account_init( idx_in_txn,
                                                               k,
                                                               j,
