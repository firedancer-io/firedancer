...
#define ACC_IDX_OWNER (2UL)
  ...
  fd_guarded_borrowed_account_t * owner_acc = NULL;
  err = fd_exec_instr_ctx_try_borrow_account( ctx, ACC_IDX_OWNER, owner_acc );
  if( FD_UNLIKELY( err ) ) {
    return err;
  }
    ...
    fd_memcpy( owner_pubkey, owner_acc->acct->pubkey, sizeof(fd_pubkey_t) );
  ...
  {
    fd_guarded_borrowed_account_t * proof_acc = NULL;
    err = fd_exec_instr_ctx_try_borrow_account( ctx, ACC_IDX_PROOF, proof_acc );
    if( FD_UNLIKELY( err ) ) {
      return err;
    }
    fd_memcpy( proof_pubkey, proof_acc->acct->pubkey, sizeof(fd_pubkey_t) );
    fd_borrowed_account_drop( proof_acc );
  }

  {
    fd_guarded_borrowed_account_t * dest_acc = NULL;
    err = fd_exec_instr_ctx_try_borrow_account( ctx, ACC_IDX_DEST, dest_acc );
    if( FD_UNLIKELY( err ) ) {
      return err;
    }
    fd_memcpy( dest_pubkey, dest_acc->acct->pubkey, sizeof(fd_pubkey_t) );
  ...
  if( FD_UNLIKELY( err ) ) {
    ...
    if( FD_UNLIKELY( proof_acc->acct->const_meta->dlen < sizeof(fd_zksdk_proof_ctx_state_meta_t) ) ) {
      return FD_EXECUTOR_INSTR_ERR_INVALID_ACC_DATA;
    }
    fd_zksdk_proof_ctx_state_meta_t const * proof_ctx_state_meta = fd_type_pun_const( proof_acc->acct->const_data );
  ...
  if( FD_UNLIKELY( err ) ) {
    ...
    err = fd_borrowed_account_checked_add_lamports( dest_acc, proof_acc->acct->const_meta->info.lamports );
    if( FD_UNLIKELY( err ) ) {
      return err;
    }
...
#define MAX_SZ (sizeof(fd_zksdk_range_proof_u256_proof_t)+sizeof(fd_zksdk_batched_range_proof_context_t))
  ...
  if( ctx->instr->data_sz == 5UL ) {
    ...
    fd_guarded_borrowed_account_t * proof_data_acc = NULL;
    err = fd_exec_instr_ctx_try_borrow_account( ctx, 0UL, proof_data_acc );
    if( FD_UNLIKELY( err ) ) {
      return err;
    }
  ...
  if( instr_acc_cnt > accessed_accounts ) {
    ...
    fd_guarded_borrowed_account_t * _acc = NULL;
    err = fd_exec_instr_ctx_try_borrow_account( ctx, accessed_accounts+1, _acc );
    if( FD_UNLIKELY( err ) ) {
      return err;
    }
    ...
    fd_guarded_borrowed_account_t * proof_context_acc = NULL;
    err = fd_exec_instr_ctx_try_borrow_account( ctx, accessed_accounts, proof_context_acc );
    if( FD_UNLIKELY( err ) ) {
      return err;
    }

      /* https://github.com/anza-xyz/agave/blob/v2.0.1/programs/zk-elgamal-proof/src/lib.rs#L103-L105 */
      if( FD_UNLIKELY( !fd_memeq( proof_context_acc->acct->const_meta->info.owner, &fd_solana_zk_elgamal_proof_program_id, sizeof(fd_pubkey_t) ) ) ) {
        return FD_EXECUTOR_INSTR_ERR_INVALID_ACC_OWNER;
      }

      /* https://github.com/anza-xyz/agave/blob/v2.0.1/programs/zk-elgamal-proof/src/lib.rs#L107-L112 */
      if( FD_UNLIKELY( proof_context_acc->acct->const_meta->dlen >= CTX_HEAD_SZ && proof_context_acc->acct->const_data[32] != 0 ) ) {
      ...
      if( FD_UNLIKELY( proof_context_acc->acct->const_meta->dlen != context_data_sx ) ) {
        return FD_EXECUTOR_INSTR_ERR_INVALID_ACC_DATA;
      }
