...
deploy_env_init( deploy_env_t * env,
  ...
  ulong banks_footprint = fd_banks_footprint( 1UL, 1UL, 2048UL, 2048UL, 32768UL, 4UL<<20 );
  void * banks_mem = fd_wksp_alloc_laddr( wksp, fd_banks_align(), banks_footprint, tag++ );
  FD_TEST( banks_mem );
  env->banks = fd_banks_join( fd_banks_new( banks_mem, FD_STAKE_DELEGATIONS_FD, 1UL, 1UL, 2048UL, 32768UL, 2048UL, 0, 42UL, 4UL<<20 ) );
  FD_TEST( env->banks );
