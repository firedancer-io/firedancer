...
get_state( fd_borrowed_account_t const * self,
  ...
  bincode_ctx.valloc  = valloc; /* No real allocation from this valloc. */
...
initialize( fd_exec_instr_ctx_t const *   ctx,
  ...
  do {
    int rc = get_state( stake_account, fd_runtime_spad_virtual( ctx->txn_ctx->spad ), &stake_state );
    if( FD_UNLIKELY( rc ) ) return rc;
  } while(0);
...
authorize( fd_exec_instr_ctx_t const *   ctx,
  ...
  rc = get_state( stake_account, fd_runtime_spad_virtual( ctx->txn_ctx->spad ), &stake_state );
  if( FD_UNLIKELY( rc ) ) return rc;
  switch( stake_state.discriminant ) {
...
delegate( fd_exec_instr_ctx_t const *   ctx,
  ...
  fd_valloc_t valloc = fd_runtime_spad_virtual( ctx->txn_ctx->spad );
  ...
  vote_get_state_rc = fd_vote_get_state( vote_account, valloc, &vote_state );
  ...
  case fd_stake_state_v2_enum_initialized: {
    ...
    fd_vote_convert_to_current( &vote_state, valloc ); // FIXME
    fd_stake_t stake =
  ...
  case fd_stake_state_v2_enum_stake: {
    ...
    if( FD_UNLIKELY( vote_get_state_rc ) ) return vote_get_state_rc;
    fd_vote_convert_to_current( &vote_state, valloc );
...
deactivate( fd_exec_instr_ctx_t const *   ctx,
  ...
  rc                        = get_state( stake_account, fd_runtime_spad_virtual( ctx->txn_ctx->spad ), &state );
  if( FD_UNLIKELY( rc ) ) return rc;
...
split( fd_exec_instr_ctx_t const * ctx,
  ...
  rc = get_state( split, fd_runtime_spad_virtual( ctx->txn_ctx->spad ), &split_get_state );
  if( FD_UNLIKELY( rc ) ) return rc;
...
merge( fd_exec_instr_ctx_t *         ctx, // not const to log
  ...
  rc = get_state( stake_account, fd_runtime_spad_virtual( ctx->txn_ctx->spad ), &stake_account_state );
  if( FD_UNLIKELY( rc ) ) return rc;
  ...
  rc = get_state( source_account, fd_runtime_spad_virtual( ctx->txn_ctx->spad ), &source_account_state );
  if( FD_UNLIKELY( rc ) ) return rc;
...
move_stake_or_lamports_shared_checks( fd_exec_instr_ctx_t *   invoke_context, // not const to log
    ...
    fd_stake_state_v2_t source_account_state = {0};
    rc = get_state( source_account, fd_runtime_spad_virtual( invoke_context->txn_ctx->spad ), &source_account_state );
    if( FD_UNLIKELY( rc ) ) return rc;
    ...
    rc = get_state( destination_account, fd_runtime_spad_virtual( invoke_context->txn_ctx->spad ), &destination_account_state );
    if( FD_UNLIKELY( rc ) ) return rc;
...
deactivate_delinquent( fd_exec_instr_ctx_t *   ctx,
  ...
  rc = fd_vote_get_state( delinquent_vote_account, valloc, &delinquent_vote_state_versioned );
  if( FD_UNLIKELY( rc ) ) return rc;
  fd_vote_convert_to_current( &delinquent_vote_state_versioned, valloc );
  ...
  rc = fd_vote_get_state( reference_vote_account, valloc, &reference_vote_state_versioned );
  if( FD_UNLIKELY( rc ) ) return rc;
  fd_vote_convert_to_current( &reference_vote_state_versioned, valloc );
...
fd_stake_program_execute( fd_exec_instr_ctx_t * ctx ) {
  ...
  fd_valloc_t valloc = fd_runtime_spad_virtual( spad );
