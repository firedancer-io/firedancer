...
/* The worst case stake instruction footprint can happen with one of the
   dynamically sized instruction types.

   Both these types have a dynamic size based on the size of the seed.
   authorize_with_seed
   authorize_checked_with_seed

   A loose, but reaosonable bound is that the seed takes up the entire
   FD_TXN_MTU to express.

   So, the worst case footprint is the sum of the size of the
   instruction and the transaction MTU. */

#define FD_STAKE_INSTR_FOOTPRINT (FD_TXN_MTU + sizeof(fd_stake_instruction_t))
...
validate_split_amount( fd_exec_instr_ctx_t const * invoke_context,
  ...
  fd_guarded_borrowed_account_t source_account = {0};
  ...
  fd_guarded_borrowed_account_t destination_account = {0};
  ...
  fd_rent_t rent_;
  fd_rent_t const * rent = fd_sysvar_cache_rent_read( invoke_context->sysvar_cache, &rent_ );
...
authorized_check( fd_stake_authorized_t const * self,
  ...
  case fd_stake_authorize_enum_staker:
    if( FD_LIKELY( fd_signers_contains( signers, signers_cnt, &self->staker ) ) ) {
      return 0;
    }
  ...
  case fd_stake_authorize_enum_withdrawer:
    if( FD_LIKELY( fd_signers_contains( signers, signers_cnt, &self->withdrawer ) ) ) {
      return 0;
    }
    return FD_EXECUTOR_INSTR_ERR_MISSING_REQUIRED_SIGNATURE;
  default:
...
authorized_authorize( fd_stake_authorized_t *                  self,
  ...
  case fd_stake_authorize_enum_staker:
    if( FD_UNLIKELY( !fd_signers_contains( signers, signers_cnt, &self->staker ) &&
                      !fd_signers_contains( signers, signers_cnt, &self->withdrawer ) ) ) {
      return FD_EXECUTOR_INSTR_ERR_MISSING_REQUIRED_SIGNATURE;
    }
  ...
  case fd_stake_authorize_enum_withdrawer:
    if( FD_LIKELY( lockup_custodian_args ) ) {
      ...
      if( lockup_is_in_force( lockup, clock, NULL ) ) {
        ...
        if( !custodian ) { // FIXME FD_LIKELY
          *custom_err = FD_STAKE_ERR_CUSTODIAN_MISSING;
          ...
          if( FD_UNLIKELY( !fd_signers_contains( signers, signers_cnt, custodian ) ) ) {
      ...
      rc = authorized_check( self, signers, signers_cnt, *stake_authorize );
...
set_lockup_meta( fd_stake_meta_t *             self,
  ...
  if( lockup_is_in_force( &self->lockup, clock, NULL ) ) {
    if( !fd_signers_contains( signers, signers_cnt, &self->lockup.custodian ) ) {
      return FD_EXECUTOR_INSTR_ERR_MISSING_REQUIRED_SIGNATURE;
    }
  } else if( !fd_signers_contains( signers, signers_cnt, &self->authorized.withdrawer ) ) {
    return FD_EXECUTOR_INSTR_ERR_MISSING_REQUIRED_SIGNATURE;
  }
...
get_if_mergeable( fd_exec_instr_ctx_t *         invoke_context, // not const to log
  ...
  case fd_stake_state_v2_enum_stake: {
    ...
    fd_epoch_schedule_t const * epoch_schedule = fd_sysvar_cache_epoch_schedule_read( invoke_context->sysvar_cache, epoch_schedule_ );
...
get_stake_status( fd_exec_instr_ctx_t const *    invoke_context,
  ...
  if( FD_UNLIKELY( !fd_sysvar_cache_stake_history_is_valid( sysvar_cache ) ) ) {
    return FD_EXECUTOR_INSTR_ERR_UNSUPPORTED_SYSVAR;
  }

  fd_epoch_schedule_t epoch_schedule_[1];
  fd_epoch_schedule_t const * epoch_schedule = fd_sysvar_cache_epoch_schedule_read( sysvar_cache, epoch_schedule_ );
...
initialize( fd_borrowed_account_t *       stake_account,
  ...
  do {
    int rc = get_state( stake_account->meta, &stake_state );
    if( FD_UNLIKELY( rc ) ) return rc;
  } while(0);
...
authorize( fd_borrowed_account_t *       stake_account,
  ...
  rc = get_state( stake_account->meta, &stake_state );
  if( FD_UNLIKELY( rc ) ) return rc;
  switch( stake_state.discriminant ) {
...
delegate( fd_exec_instr_ctx_t const *   ctx,
  ...
  vote_state = fd_vote_get_state( vote_account.meta, ctx->runtime->stake_program.delegate.vote_state_mem );
  ...
  fd_guarded_borrowed_account_t stake_account = {0};
  ...
  case fd_stake_state_v2_enum_initialized: {
    ...
    rc = authorized_check( &meta.authorized, signers, signers_cnt, STAKE_AUTHORIZE_STAKER );
    if( FD_UNLIKELY( rc ) ) return rc;
...
deactivate( fd_borrowed_account_t *       stake_account,
  ...
  rc                        = get_state( stake_account->meta, &state );
  if( FD_UNLIKELY( rc ) ) return rc;
...
set_lockup( fd_borrowed_account_t *       stake_account,
  ...
  case fd_stake_state_v2_enum_initialized: {
    ...
    rc                     = set_lockup_meta( meta, lockup, signers, signers_cnt, clock );
    if( FD_UNLIKELY( rc ) ) return rc;
    return set_state( stake_account, &state );
  }
...
split( fd_exec_instr_ctx_t const * ctx,
  ...
  fd_guarded_borrowed_account_t split = {0};
  ...
  rc = get_state( split.meta, &split_get_state );
  if( FD_UNLIKELY( rc ) ) return rc;
  ...
  case fd_stake_state_v2_enum_stake: {
    ...
    int is_active = status.effective>0;
    ...
    if( validated_split_info.source_remaining_balance==0 ) {
      ...
      if( FD_UNLIKELY( fd_ulong_sat_sub( stake->delegation.stake, lamports ) <
                        minimum_delegation ) ) {
        ctx->txn_out->err.custom_err = FD_STAKE_ERR_INSUFFICIENT_DELEGATION;
        return FD_EXECUTOR_INSTR_ERR_CUSTOM_ERR;
      }
  ...
  case fd_stake_state_v2_enum_uninitialized: {
    ...
    if( FD_UNLIKELY( !fd_signers_contains( signers, signers_cnt, stake_pubkey ) ) ) {
...
merge( fd_exec_instr_ctx_t *         ctx, // not const to log
  ...
  rc = authorized_check( &meta( &stake_merge_kind )->authorized, signers, signers_cnt, STAKE_AUTHORIZE_STAKER );
  if( FD_UNLIKELY( rc ) )
    return rc;

  fd_stake_state_v2_t source_account_state = {0};
  rc = get_state( source_account.meta, &source_account_state );
  if( FD_UNLIKELY( rc ) ) return rc;
...
move_stake_or_lamports_shared_checks( fd_exec_instr_ctx_t *   invoke_context, // not const to log
    ...
    fd_sol_sysvar_clock_t clock_;
    fd_sol_sysvar_clock_t const * clock = fd_sysvar_cache_clock_read( invoke_context->sysvar_cache, &clock_ );
    if( FD_UNLIKELY( !clock ) ) return FD_EXECUTOR_INSTR_ERR_UNSUPPORTED_SYSVAR;

    fd_stake_history_t const * stake_history = fd_sysvar_cache_stake_history_join_const( invoke_context->sysvar_cache );
    ...
    rc = get_if_mergeable( invoke_context,
                         ...
                         stake_history,
                         source_merge_kind,
                         &invoke_context->txn_out->err.custom_err );
    if( FD_UNLIKELY( rc ) ) return rc;

    // https://github.com/anza-xyz/agave/blob/cdff19c7807b006dd63429114fb1d9573bf74172/programs/stake/src/stake_state.rs#L191
    rc = authorized_check( &meta( source_merge_kind )->authorized, signers, 1UL, STAKE_AUTHORIZE_STAKER );
    if( FD_UNLIKELY( rc ) ) return rc;
    ...
    rc = get_state( destination_account->meta, &destination_account_state );
    if( FD_UNLIKELY( rc ) ) return rc;
...
withdraw( fd_exec_instr_ctx_t const *   ctx,
  ...
  case fd_stake_state_v2_enum_stake: {
    ...
    rc = authorized_check( &meta->authorized, signers, 1UL, STAKE_AUTHORIZE_WITHDRAWER );
    if( FD_UNLIKELY( rc ) ) return rc;
  ...
  case fd_stake_state_v2_enum_uninitialized: {
    ...
    if( FD_UNLIKELY( !fd_signers_contains( signers, 1UL, stake_account.pubkey ) ) ) {
      return FD_EXECUTOR_INSTR_ERR_MISSING_REQUIRED_SIGNATURE;
    }
  ...
  if( FD_UNLIKELY( lockup_is_in_force( &lockup, clock, custodian_pubkey ) ) ) {
    ctx->txn_out->err.custom_err = FD_STAKE_ERR_LOCKUP_IN_FORCE;
    return FD_EXECUTOR_INSTR_ERR_CUSTOM_ERR;
  };
...
deactivate_delinquent( fd_exec_instr_ctx_t *   ctx,
  ...
  fd_guarded_borrowed_account_t delinquent_vote_account = {0};
  ...
  if( !acceptable_reference_epoch_credits( reference_vote_state.epoch_credits, current_epoch ) ) {
    ctx->txn_out->err.custom_err = FD_STAKE_ERR_INSUFFICIENT_REFERENCE_VOTES;
    return FD_EXECUTOR_INSTR_ERR_CUSTOM_ERR;
  }
...
fd_stake_program_execute( fd_exec_instr_ctx_t * ctx ) {
  ...
  uchar instr_mem[ FD_STAKE_INSTR_FOOTPRINT ] __attribute__((aligned(alignof(fd_stake_instruction_t))));
  ...
  int epoch_rewards_active = 0;
  {
    ...
    if( fd_sysvar_cache_epoch_rewards_read( ctx->sysvar_cache, epoch_rewards ) ) {
  ...
  if( epoch_rewards_active && instruction->discriminant!=fd_stake_instruction_enum_get_minimum_delegation ) {
    ctx->txn_out->err.custom_err = FD_STAKE_ERR_EPOCH_REWARDS_ACTIVE;
    return FD_EXECUTOR_INSTR_ERR_CUSTOM_ERR;
  }
  ...
  case fd_stake_instruction_enum_delegate_stake: {
    ...
    if( FD_UNLIKELY( rc ) ) return rc;
    if( FD_UNLIKELY( !fd_sysvar_cache_stake_history_is_valid( ctx->sysvar_cache ) ) ) {
      return FD_EXECUTOR_INSTR_ERR_UNSUPPORTED_SYSVAR;
    }
    ...
    fd_stake_history_t const * stake_history = fd_sysvar_cache_stake_history_join_const( ctx->sysvar_cache );
    rc = delegate( ctx,
                   0,
                   1,
                   clock,
                   ...
                   signers,
                   signers_cnt );
    fd_sysvar_cache_stake_history_leave_const( ctx->sysvar_cache, stake_history );
    break;
  }
  ...
  case fd_stake_instruction_enum_split: {
    ...
    rc = split( ctx, 0, lamports, 1, signers, signers_cnt );
    break;
  }
  ...
  case fd_stake_instruction_enum_merge: {
    ...
    fd_stake_history_t const * stake_history = fd_sysvar_cache_stake_history_join_const( ctx->sysvar_cache );
    rc = merge( ctx, 0, 1, clock, stake_history, signers, signers_cnt );
    fd_sysvar_cache_stake_history_leave_const( ctx->sysvar_cache, stake_history );
    break;
  }
  ...
  case fd_stake_instruction_enum_deactivate: {
    ...
    rc = deactivate( &me, clock, signers, signers_cnt, &ctx->txn_out->err.custom_err );
  ...
  case fd_stake_instruction_enum_set_lockup: {
    ...
    rc = set_lockup( &me, lockup, signers, signers_cnt, clock );
    break;
  }
  ...
  case fd_stake_instruction_enum_deactivate_delinquent: {
    ...
    rc = deactivate_delinquent( ctx, &me, 1, 2, clock->epoch, &ctx->txn_out->err.custom_err );
    break;
  }
