...
get_state( fd_txn_account_t const * self,
  ...
  fd_bincode_decode_ctx_t bincode_ctx = {
    .data    = fd_txn_account_get_acc_data( self ),
    .dataend = fd_txn_account_get_acc_data( self ) + fd_txn_account_get_data_len( self ),
  };
...
write_stake_config( fd_exec_slot_ctx_t * slot_ctx, fd_stake_config_t const * stake_config ) {
  ...
  ctx3.data    = fd_txn_account_get_acc_data_mut( rec );
  ctx3.dataend = fd_txn_account_get_acc_data_mut( rec ) + data_sz;
...
fd_store_stake_delegation( fd_txn_account_t *   stake_account,
  ...
  int is_uninit = 1;
  if( fd_txn_account_get_data_len( stake_account )>=4 ) {
    uint prefix = FD_LOAD( uint, fd_txn_account_get_acc_data( stake_account ) );
