...
fd_bpf_get_executable_program_content_for_upgradeable_loader( fd_exec_slot_ctx_t *    slot_ctx,
  ...
  if( fd_txn_account_init_from_funk_readonly( programdata_acc,
                                              programdata_address,
                                              slot_ctx->funk,
                                              slot_ctx->funk_txn ) != FD_FUNK_ACC_MGR_SUCCESS ) {
    return -1;
  }
...
fd_bpf_scan_task( void * tpool,
  ...
  if( fd_txn_account_init_from_funk_readonly( exec_rec, pubkey, slot_ctx->funk, slot_ctx->funk_txn ) != FD_FUNK_ACC_MGR_SUCCESS ) {
    return;
  }
...
fd_bpf_check_and_create_bpf_program_cache_entry( fd_exec_slot_ctx_t * slot_ctx,
                                                 ...
                                                 fd_spad_t *          runtime_spad ) {
  FD_TXN_ACCOUNT_DECL( exec_rec );
  if( fd_txn_account_init_from_funk_readonly( exec_rec, pubkey, slot_ctx->funk, funk_txn ) != FD_FUNK_ACC_MGR_SUCCESS ) {
    return -1;
  }
