...
fd_bpf_get_executable_program_content_for_v4_loader( fd_txn_account_t const * program_acc,
  ...
  *program_data_len = fd_txn_account_get_data_len( program_acc ) - LOADER_V4_PROGRAM_DATA_OFFSET;
  return fd_txn_account_get_acc_data( program_acc ) + LOADER_V4_PROGRAM_DATA_OFFSET;
}
...
fd_bpf_get_executable_program_content_for_upgradeable_loader( fd_funk_t const *        funk,
  ...
  fd_bpf_upgradeable_loader_state_t * program_account_state =
    fd_bincode_decode_spad(
      ...
      fd_txn_account_get_acc_data( program_acc ),
  ...
  fd_bincode_decode_ctx_t ctx_programdata = {
    .data    = fd_txn_account_get_acc_data( programdata_acc ),
    .dataend = fd_txn_account_get_acc_data( programdata_acc ) + fd_txn_account_get_data_len( programdata_acc ),
  };
  ...
  *program_data_len = fd_txn_account_get_data_len( programdata_acc ) - PROGRAMDATA_METADATA_SIZE;
  return fd_txn_account_get_acc_data( programdata_acc ) + PROGRAMDATA_METADATA_SIZE;
}
...
fd_bpf_get_executable_program_content_for_v1_v2_loaders( fd_txn_account_t const * program_acc,
  ...
  *program_data_len = fd_txn_account_get_data_len( program_acc );
  return fd_txn_account_get_acc_data( program_acc );
}
...
fd_bpf_check_and_create_bpf_program_cache_entry( fd_exec_slot_ctx_t * slot_ctx,
  ...
  if( !fd_executor_pubkey_is_bpf_loader( fd_txn_account_get_owner( exec_rec ) ) ) {
    return -1;
  }
