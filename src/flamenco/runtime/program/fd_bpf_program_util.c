...
fd_sbpf_validated_program_footprint( ulong rodata_sz ) {
  return sizeof(fd_sbpf_validated_program_t)+rodata_sz;
}
...
fd_acc_mgr_cache_key( fd_pubkey_t const * pubkey ) {
  ...
  id.c[ FD_FUNK_REC_KEY_FOOTPRINT - 1 ] = FD_ACC_MGR_ELF_CACHE_TYPE;
...
fd_bpf_create_bpf_program_cache_entry( fd_exec_slot_ctx_t * slot_ctx,
  ...
  FD_SCRATCH_SCOPE_BEGIN {
    ...
    fd_funk_rec_t * rec = fd_funk_rec_write_prepare( funk, funk_txn, &id, fd_sbpf_validated_program_footprint( elf_info.rodata_footprint), 1, NULL, &funk_err );
    ...
    uchar * rodata = val + sizeof(fd_sbpf_validated_program_t);
...
fd_bpf_scan_and_create_bpf_program_cache_entry( fd_exec_slot_ctx_t * slot_ctx ) {
  fd_funk_t * funk = slot_ctx->acc_mgr->funk;
  ulong cnt = 0;
  for (fd_funk_rec_t const *rec = fd_funk_txn_first_rec( funk, NULL );
        NULL != rec;
    ...
    if( fd_acc_mgr_view( slot_ctx->acc_mgr, slot_ctx->funk_txn, program_pubkey, exec_rec ) != FD_ACC_MGR_SUCCESS ) {
      continue;
    }
    ...
    if( fd_executor_bpf_upgradeable_loader_program_is_executable_program_account( slot_ctx, program_pubkey ) == 0
      ...
      if( fd_bpf_create_bpf_program_cache_entry( slot_ctx, program_pubkey ) != 0 ) {
        FD_LOG_DEBUG(( "failed to load program %32J", program_pubkey->key ));
        continue;
      }
    } else { 
      continue;
  ...
  FD_LOG_WARNING(( "loaded program cache: %lu", cnt));
