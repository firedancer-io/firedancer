...
# define ACC_IDX_PAYER     (2UL)
  ...
  do {
    fd_slot_hash_t const * slot_hash = fd_sysvar_slot_hashes_join_const( ctx->sysvar_cache );
...
# define ACC_IDX_PAYER     (2UL)
  ...
  fd_rent_t rent[1];
  if( FD_UNLIKELY( !fd_sysvar_rent_read( ctx->sysvar_cache, rent ) ) ) {
    FD_LOG_ERR(( "fd_sysvar_rent_read failed" ));
  }
...
# define ACC_IDX_RECIPIENT (2UL)
  ...
  fd_sol_sysvar_clock_t clock_;
  fd_sol_sysvar_clock_t * clock = fd_sysvar_clock_read( ctx->sysvar_cache, &clock_ );
  if( FD_UNLIKELY( !clock ) ) {
    return FD_EXECUTOR_INSTR_ERR_UNSUPPORTED_SYSVAR;
  }
