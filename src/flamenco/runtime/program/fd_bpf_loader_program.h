...
#define FD_BPF_UPGRADEABLE_LOADER_INSTR_INITIALIZE_BUFFER        (0U)
#define FD_BPF_UPGRADEABLE_LOADER_INSTR_WRITE                    (1U)
#define FD_BPF_UPGRADEABLE_LOADER_INSTR_DEPLOY_WITH_MAX_DATA_LEN (2U)
#define FD_BPF_UPGRADEABLE_LOADER_INSTR_UPGRADE                  (3U)
#define FD_BPF_UPGRADEABLE_LOADER_INSTR_SET_AUTHORITY            (4U)
#define FD_BPF_UPGRADEABLE_LOADER_INSTR_CLOSE                    (5U)
#define FD_BPF_UPGRADEABLE_LOADER_INSTR_EXTEND_PROGRAM           (6U)
#define FD_BPF_UPGRADEABLE_LOADER_INSTR_SET_AUTHORITY_CHECKED    (7U)
#define FD_BPF_UPGRADEABLE_LOADER_INSTR_MIGRATE                  (8U)
#define FD_BPF_UPGRADEABLE_LOADER_INSTR_EXTEND_PROGRAM_CHECKED   (9U)
...
enum {
  fd_bpf_upgradeable_loader_program_instruction_enum_initialize_buffer        = FD_BPF_UPGRADEABLE_LOADER_INSTR_INITIALIZE_BUFFER,
  fd_bpf_upgradeable_loader_program_instruction_enum_write                    = FD_BPF_UPGRADEABLE_LOADER_INSTR_WRITE,
  fd_bpf_upgradeable_loader_program_instruction_enum_deploy_with_max_data_len = FD_BPF_UPGRADEABLE_LOADER_INSTR_DEPLOY_WITH_MAX_DATA_LEN,
  fd_bpf_upgradeable_loader_program_instruction_enum_upgrade                  = FD_BPF_UPGRADEABLE_LOADER_INSTR_UPGRADE,
  fd_bpf_upgradeable_loader_program_instruction_enum_set_authority            = FD_BPF_UPGRADEABLE_LOADER_INSTR_SET_AUTHORITY,
  fd_bpf_upgradeable_loader_program_instruction_enum_close                    = FD_BPF_UPGRADEABLE_LOADER_INSTR_CLOSE,
  fd_bpf_upgradeable_loader_program_instruction_enum_extend_program           = FD_BPF_UPGRADEABLE_LOADER_INSTR_EXTEND_PROGRAM,
  fd_bpf_upgradeable_loader_program_instruction_enum_set_authority_checked    = FD_BPF_UPGRADEABLE_LOADER_INSTR_SET_AUTHORITY_CHECKED,
  fd_bpf_upgradeable_loader_program_instruction_enum_migrate                  = FD_BPF_UPGRADEABLE_LOADER_INSTR_MIGRATE,
  fd_bpf_upgradeable_loader_program_instruction_enum_extend_program_checked   = FD_BPF_UPGRADEABLE_LOADER_INSTR_EXTEND_PROGRAM_CHECKED
};
...
#define FD_BPF_UPGRADEABLE_LOADER_STATE_UNINITIALIZED (0U)
#define FD_BPF_UPGRADEABLE_LOADER_STATE_BUFFER        (1U)
#define FD_BPF_UPGRADEABLE_LOADER_STATE_PROGRAM       (2U)
#define FD_BPF_UPGRADEABLE_LOADER_STATE_PROGRAM_DATA  (3U)

enum {
  fd_bpf_upgradeable_loader_state_enum_uninitialized = FD_BPF_UPGRADEABLE_LOADER_STATE_UNINITIALIZED,
  fd_bpf_upgradeable_loader_state_enum_buffer        = FD_BPF_UPGRADEABLE_LOADER_STATE_BUFFER,
  fd_bpf_upgradeable_loader_state_enum_program       = FD_BPF_UPGRADEABLE_LOADER_STATE_PROGRAM,
  fd_bpf_upgradeable_loader_state_enum_program_data  = FD_BPF_UPGRADEABLE_LOADER_STATE_PROGRAM_DATA
};
...
static inline uchar
fd_bpf_upgradeable_loader_state_is_uninitialized( fd_bpf_upgradeable_loader_state_t const * self ) {
  return (uchar)( self->discriminant==FD_BPF_UPGRADEABLE_LOADER_STATE_UNINITIALIZED );
}

static inline uchar
fd_bpf_upgradeable_loader_state_is_buffer( fd_bpf_upgradeable_loader_state_t const * self ) {
  return (uchar)( self->discriminant==FD_BPF_UPGRADEABLE_LOADER_STATE_BUFFER );
}

static inline uchar
fd_bpf_upgradeable_loader_state_is_program( fd_bpf_upgradeable_loader_state_t const * self ) {
  return (uchar)( self->discriminant==FD_BPF_UPGRADEABLE_LOADER_STATE_PROGRAM );
}

static inline uchar
fd_bpf_upgradeable_loader_state_is_program_data( fd_bpf_upgradeable_loader_state_t const * self ) {
  return (uchar)( self->discriminant==FD_BPF_UPGRADEABLE_LOADER_STATE_PROGRAM_DATA );
}
...
static inline int
fd_bpf_upgradeable_loader_program_instruction_decode( fd_bpf_upgradeable_loader_program_instruction_t * out,
                                                      uchar const *                                     data,
                                                      ulong                                             data_sz ) {
...
# define CURSOR          (_payload+_i)
  ...
  case FD_BPF_UPGRADEABLE_LOADER_INSTR_WRITE: {
    fd_bpf_upgradeable_loader_program_instruction_write_t * w = &out->inner.write;
  ...
  case FD_BPF_UPGRADEABLE_LOADER_INSTR_DEPLOY_WITH_MAX_DATA_LEN: {
    fd_bpf_upgradeable_loader_program_instruction_deploy_with_max_data_len_t * d =
        &out->inner.deploy_with_max_data_len;
    CHECK_LEFT( 8UL );
    d->max_data_len = FD_LOAD( ulong, CURSOR ); INC( 8UL );
    return 0;
  }

  case FD_BPF_UPGRADEABLE_LOADER_INSTR_EXTEND_PROGRAM: {
    fd_bpf_upgradeable_loader_program_instruction_extend_program_t * e = &out->inner.extend_program;
    CHECK_LEFT( 4UL );
    e->additional_bytes = FD_LOAD( uint, CURSOR ); INC( 4UL );
    return 0;
  }

  case FD_BPF_UPGRADEABLE_LOADER_INSTR_EXTEND_PROGRAM_CHECKED: {
    fd_bpf_upgradeable_loader_program_instruction_extend_program_checked_t * e =
        &out->inner.extend_program_checked;
    CHECK_LEFT( 4UL );
    e->additional_bytes = FD_LOAD( uint, CURSOR ); INC( 4UL );
    return 0;
  }

  case FD_BPF_UPGRADEABLE_LOADER_INSTR_INITIALIZE_BUFFER:
  case FD_BPF_UPGRADEABLE_LOADER_INSTR_UPGRADE:
  case FD_BPF_UPGRADEABLE_LOADER_INSTR_SET_AUTHORITY:
  case FD_BPF_UPGRADEABLE_LOADER_INSTR_CLOSE:
  case FD_BPF_UPGRADEABLE_LOADER_INSTR_SET_AUTHORITY_CHECKED:
  case FD_BPF_UPGRADEABLE_LOADER_INSTR_MIGRATE:
    return 0;
...
static inline int
fd_bpf_upgradeable_loader_program_instruction_encode( fd_bpf_upgradeable_loader_program_instruction_t const * in,
                                                      uchar *                                                 buf,
                                                      ulong                                                   bufsz,
                                                      ulong *                                                 out_sz ) {
...
# define CURSOR          (_payload+_i)
  ...
  case FD_BPF_UPGRADEABLE_LOADER_INSTR_WRITE: {
    fd_bpf_upgradeable_loader_program_instruction_write_t const * w = &in->inner.write;
  ...
  case FD_BPF_UPGRADEABLE_LOADER_INSTR_DEPLOY_WITH_MAX_DATA_LEN: {
    fd_bpf_upgradeable_loader_program_instruction_deploy_with_max_data_len_t const * d =
        &in->inner.deploy_with_max_data_len;
    CHECK_LEFT( 8UL );
  ...
  case FD_BPF_UPGRADEABLE_LOADER_INSTR_EXTEND_PROGRAM: {
    fd_bpf_upgradeable_loader_program_instruction_extend_program_t const * e =
        &in->inner.extend_program;
    CHECK_LEFT( 4UL );
  ...
  case FD_BPF_UPGRADEABLE_LOADER_INSTR_EXTEND_PROGRAM_CHECKED: {
    fd_bpf_upgradeable_loader_program_instruction_extend_program_checked_t const * e =
        &in->inner.extend_program_checked;
    CHECK_LEFT( 4UL );
    FD_STORE( uint, CURSOR, e->additional_bytes ); INC( 4UL );
    break;
  }
...
static inline ulong
fd_bpf_upgradeable_loader_program_instruction_size( fd_bpf_upgradeable_loader_program_instruction_t const * in ) {
  switch( in->discriminant ) {
    case FD_BPF_UPGRADEABLE_LOADER_INSTR_WRITE:                    return 4UL + 4UL + 8UL + in->inner.write.bytes_len;
    case FD_BPF_UPGRADEABLE_LOADER_INSTR_DEPLOY_WITH_MAX_DATA_LEN: return 4UL + 8UL;
    case FD_BPF_UPGRADEABLE_LOADER_INSTR_EXTEND_PROGRAM:           return 4UL + 4UL;
    case FD_BPF_UPGRADEABLE_LOADER_INSTR_EXTEND_PROGRAM_CHECKED:   return 4UL + 4UL;
    case FD_BPF_UPGRADEABLE_LOADER_INSTR_INITIALIZE_BUFFER:
    case FD_BPF_UPGRADEABLE_LOADER_INSTR_UPGRADE:
    case FD_BPF_UPGRADEABLE_LOADER_INSTR_SET_AUTHORITY:
    case FD_BPF_UPGRADEABLE_LOADER_INSTR_CLOSE:
    case FD_BPF_UPGRADEABLE_LOADER_INSTR_SET_AUTHORITY_CHECKED:
    case FD_BPF_UPGRADEABLE_LOADER_INSTR_MIGRATE:                  return 4UL;
    default:                                                       return 0UL;
  }
}
...
static inline int
fd_bpf_upgradeable_loader_state_decode( fd_bpf_upgradeable_loader_state_t * out,
                                        uchar const *                       data,
                                        ulong                               data_sz ) {
  uchar const * _payload    = data;
  ulong const   _payload_sz = data_sz;
...
# define CURSOR          (_payload+_i)
  ...
  case FD_BPF_UPGRADEABLE_LOADER_STATE_UNINITIALIZED:
    return 0;

  case FD_BPF_UPGRADEABLE_LOADER_STATE_BUFFER: {
    fd_bpf_upgradeable_loader_state_buffer_t * b = &out->inner.buffer;
  ...
  case FD_BPF_UPGRADEABLE_LOADER_STATE_PROGRAM: {
    fd_bpf_upgradeable_loader_state_program_t * p = &out->inner.program;
    CHECK_LEFT( 32UL );
    fd_memcpy( p->programdata_address.key, CURSOR, 32UL ); INC( 32UL );
    return 0;
  }

  case FD_BPF_UPGRADEABLE_LOADER_STATE_PROGRAM_DATA: {
    fd_bpf_upgradeable_loader_state_program_data_t * pd = &out->inner.program_data;
...
static inline int
fd_bpf_upgradeable_loader_state_encode( fd_bpf_upgradeable_loader_state_t const * in,
                                        uchar *                                   buf,
                                        ulong                                     bufsz,
                                        ulong *                                   out_sz ) {
  uchar * const _payload    = buf;
...
# define CURSOR          (_payload+_i)
  ...
  case FD_BPF_UPGRADEABLE_LOADER_STATE_BUFFER: {
    fd_bpf_upgradeable_loader_state_buffer_t const * b = &in->inner.buffer;
  ...
  case FD_BPF_UPGRADEABLE_LOADER_STATE_PROGRAM: {
    fd_bpf_upgradeable_loader_state_program_t const * p = &in->inner.program;
    CHECK_LEFT( 32UL );
  ...
  case FD_BPF_UPGRADEABLE_LOADER_STATE_PROGRAM_DATA: {
    fd_bpf_upgradeable_loader_state_program_data_t const * pd = &in->inner.program_data;
...
static inline ulong
fd_bpf_upgradeable_loader_state_size( fd_bpf_upgradeable_loader_state_t const * in ) {
  switch( in->discriminant ) {
    case FD_BPF_UPGRADEABLE_LOADER_STATE_UNINITIALIZED: return 4UL;
    case FD_BPF_UPGRADEABLE_LOADER_STATE_BUFFER:        return 4UL + 1UL + ( in->inner.buffer.has_authority_address ? 32UL : 0UL );
    case FD_BPF_UPGRADEABLE_LOADER_STATE_PROGRAM:       return 4UL + 32UL;
    case FD_BPF_UPGRADEABLE_LOADER_STATE_PROGRAM_DATA:  return 4UL + 8UL + 1UL + ( in->inner.program_data.has_upgrade_authority_address ? 32UL : 0UL );
    default:                                            return 0UL;
  }
}
