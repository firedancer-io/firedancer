...
fd_acc_mgr_view_raw( fd_acc_mgr_t *         acc_mgr,
  ...
  if( FD_UNLIKELY( fd_funkier_rec_query_test( query ) != FD_FUNKIER_SUCCESS ) ) {
    FD_LOG_ERR(( "fd_funkier_rec_query_test failed for pubkey: %s", FD_BASE58_ENC_32_ALLOCA( pubkey->uc ) ));
  }
