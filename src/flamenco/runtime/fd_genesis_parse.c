...
fd_genesis_parse( uchar const * payload,
  ...
  for( ulong i=0UL; i<genesis->builtin_len; i++ ) {
    ...
    if( FD_UNLIKELY( genesis->builtin_off[ i ] + sizeof(fd_genesis_account_t) + account->meta.dlen > FD_GENESIS_MAX_MESSAGE_SIZE ) ) {
      FD_LOG_WARNING(( "genesis builtin account data length %lu exceeds supported max size %lu", genesis->builtin_off[ i ] + sizeof(fd_genesis_account_t) + account->meta.dlen, FD_GENESIS_MAX_MESSAGE_SIZE ));
      return NULL;
    }
    CHECK_LEFT( account->meta.dlen );
    uchar * data = FD_SCRATCH_ALLOC_APPEND( l, alignof(uchar), account->meta.dlen );
