...
replay( global_state_t * state,
  ...
  fd_bpf_scan_and_create_bpf_program_cache_entry( state->slot_ctx );
  ...
  for ( ulong slot = state->slot_ctx->slot_bank.slot+1; slot < state->end_slot; ++slot ) {
    ...
    if (blk == NULL) {
      FD_LOG_DEBUG(("failed to read slot %ld", slot));
      continue;
    }
    ...
    if ( justverify ) {
      ...
      ulong blk_txn_cnt = 0;
      FD_TEST( fd_runtime_block_eval_tpool( state->slot_ctx, state->capture_ctx, val, sz, tpool, max_workers, state->scheduler, &blk_txn_cnt ) == FD_RUNTIME_EXECUTE_SUCCESS );
      txn_cnt += blk_txn_cnt;
      slot_cnt++;
      ...
      if ( FD_UNLIKELY( !expected ) ) {
        ...
        fd_print_account_hashes( state->slot_ctx, tpool, max_workers );
...
main( int     argc,
  ...
  state.scheduler           = fd_env_strip_cmdline_ulong( &argc, &argv, "--scheduler",   NULL, 2    );
