  ...
  for( ulong i = 0; i < slot_deltas->slot_deltas_len; i++ ) {
    ...
    for( ulong j = 0; j < slot_delta->slot_delta_vec_len; j++ ) {
      ...
      /* This adjustment in offset has to be after we insert all the
         ...
         slices once again. */
      fd_txncache_set_txnhash_offset( ctx->status_cache, blockhash->uc, pair->value.txn_idx );
      fd_txncache_set_nonce_txnhash_offset( ctx->status_cache, blockhash->uc, pair->value.txn_idx );
    }
  }
