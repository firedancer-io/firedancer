...
fd_exec_instr_ctx_get_key_of_account_at_index( fd_exec_instr_ctx_t const * ctx,
  ...
  ushort idx_in_txn;
  int err = fd_exec_instr_ctx_get_index_of_instr_account_in_transaction( ctx, idx_in_instr, &idx_in_txn );
...
fd_exec_instr_ctx_get_last_program_key( fd_exec_instr_ctx_t const * ctx,
                                        fd_pubkey_t const * *       key ) {
  return fd_exec_txn_ctx_get_key_of_account_at_index( ctx->txn_ctx, ctx->instr->program_id, key );
}
...
fd_exec_instr_ctx_try_borrow_account( fd_exec_instr_ctx_t const * ctx,
  ...
  int err = fd_exec_txn_ctx_get_account_at_index( ctx->txn_ctx, idx_in_txn, &txn_account, NULL );
  if( FD_UNLIKELY( err ) ) {
  ...
  fd_borrowed_account_init( account, txn_account, ctx, (ushort)idx_in_instr );
...
fd_exec_instr_ctx_get_signers( fd_exec_instr_ctx_t const * ctx,
  ...
  for( uchar i = 0; i < ctx->instr->acct_cnt && j < FD_TXN_SIG_MAX; i++ )
    if( fd_instr_acc_is_signer_idx( ctx->instr, i ) ) {
      ushort idx_in_txn = ctx->instr->accounts[i].index_in_transaction;
      int err = fd_exec_txn_ctx_get_key_of_account_at_index( ctx->txn_ctx, idx_in_txn, &signers[j++]);
      if( FD_UNLIKELY( err ) ) return err;
    }
  return FD_EXECUTOR_INSTR_SUCCESS;
}
