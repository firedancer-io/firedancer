...
#define FD_GET_KEY_OF_ACCOUNT_AT_INDEX_DEFAULT_ERR_CHECK( ctx, idx, pubkey ) do { \
   int err = fd_exec_instr_ctx_get_key_of_account_at_index( ctx, idx, pubkey );   \
   if( FD_UNLIKELY( err ) ) return err;                                           \
} while (0)
...
/* Although fd_signers_contains does not take an instruction context,
   it is included here for completeness of signer helper functions 
   
   Loop conditions could be optimized to allow for unroll/vectorize */

static inline int
fd_signers_contains( fd_pubkey_t const * signers[FD_TXN_SIG_MAX],
                     fd_pubkey_t const * pubkey ) {
