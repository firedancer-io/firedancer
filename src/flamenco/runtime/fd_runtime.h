...
/* Bincode valloc footprint over the execution of a single transaction.
   ...
   programs, including possible CPIs from native programs.  Moreover,
   even if we were to figure out a precise upper bound right now, the
   upper bound could change in the future.  Even if the possible paths
   remain steady at the Solana protocol level, the footprint is subject
   to change when we change our implementation.
