...
fd_blockstore_new( void * shmem,
  ...
  fd_block_t * block_map = fd_block_map_join( block_map_shmap );
  if( FD_UNLIKELY( !block_map_shmap ) ) {
...
fd_blockstore_delete( void * shblockstore ) {
  ...
  fd_wksp_free_laddr( fd_block_map_delete( fd_wksp_laddr_fast( wksp, blockstore->block_map_gaddr ) ) );
...
/* Index the fd_block_micro_t offsets into the block data region and
   also allocate and index the transactions. */

static void
index_micros_and_txns( fd_blockstore_t * blockstore, ulong slot, fd_block_data_t * block ) {
...
#define MAX_TXNS ( 1 << 18 )
  ...
  fd_block_micro_t * micros     = fd_wksp_laddr_fast( fd_blockstore_wksp( blockstore ), block->micro_gaddr );
  ulong              micros_cnt = block->micro_cnt;
  ...
  while( blockoff < sz ) {
    ...
    for( ulong mblk = 0; mblk < mcount; ++mblk ) {
      if( FD_UNLIKELY( blockoff + sizeof( fd_microblock_hdr_t ) > sz ) ) {
...
fd_blockstore_slot_remove( fd_blockstore_t * blockstore, ulong slot ) {
  FD_LOG_DEBUG(( "[%s] %lu", __func__, slot ));

  fd_block_t * block = fd_block_map_remove( fd_blockstore_block_map( blockstore ), &slot );
  fd_block_meta_t * block_meta = &block->meta;
  if( FD_UNLIKELY( !block_meta ) ) {
    FD_LOG_WARNING(( "[%s] %lu not found.", __func__, slot ));
    return;
  }

  /* It is not safe to remove a replaying block. */

  if( FD_UNLIKELY( fd_uchar_extract_bit( block_meta->flags, FD_BLOCK_FLAG_REPLAYING ) ) ) {
    FD_LOG_WARNING(( "[%s] %lu is replaying. not removing.", __func__, slot ));
    return;
  }

  /* Unlink slot from its parent. */

  fd_block_meta_t * parent_block_meta =
      fd_blockstore_block_meta_query( blockstore, block_meta->parent_slot );
  if( FD_UNLIKELY( parent_block_meta ) ) { /* optimize for not found (min slot w/ no parent) */

    if ( FD_LIKELY( parent_block_meta->child_slot_cnt == 1 ) ) { /* optimize for one child */

      /* If there's only one child, remove it. */

      parent_block_meta->child_slots[0] = FD_SLOT_NULL;
      parent_block_meta->child_slot_cnt = 0;

    } else {

      /* If there is more than one child, find the child and swap-remove
         with the last element. */

      for( ulong i = 0; i < parent_block_meta->child_slot_cnt; i++ ) {
        if( FD_LIKELY( parent_block_meta->child_slots[i] == slot ) ) {
          parent_block_meta->child_slots[i] =
              parent_block_meta->child_slots[--parent_block_meta->child_slot_cnt];
        }
      }
    }
  }
  ...
  fd_block_data_t * block_data = &block->data;

  if( FD_UNLIKELY( !block_data->data_gaddr ) ) {
    return;
  }

  fd_wksp_t *  wksp  = fd_blockstore_wksp( blockstore );
  fd_alloc_t * alloc = fd_blockstore_alloc( blockstore );
  uchar *      data  = fd_wksp_laddr_fast( wksp, block_data->data_gaddr );

  if( FD_LIKELY( block_data->txn_gaddr ) ) {
    fd_blockstore_txn_map_t * txn_map = fd_wksp_laddr_fast( wksp, blockstore->txn_map_gaddr );
    fd_block_txn_ref_t *      txns    = fd_wksp_laddr_fast( wksp, block_data->txn_gaddr );
    for( ulong j = 0; j < block_data->txn_cnt; ++j ) {
      fd_blockstore_txn_key_t sig;
  ...
  if( FD_LIKELY( block_data->txn_meta_gaddr ) ) {
    ulong mgaddr = block_data->txn_meta_gaddr;
    while( mgaddr ) {
...
fd_blockstore_publish( fd_blockstore_t * blockstore, ulong smr ) {
  ...
  if( FD_UNLIKELY( !fd_blockstore_block_meta_query( blockstore, smr ) ) ) {
  ...
  while( !fd_blockstore_slot_deque_empty( q ) ) {
    ...
    for( ulong i = 0; i < block_meta->child_slot_cnt; i++ ) {
      if( FD_LIKELY( block_meta->child_slots[i] != smr ) ) {
        fd_blockstore_slot_deque_push_tail( q, block_meta->child_slots[i] );
      }
    }

    if( !fd_uchar_extract_bit( block_meta->flags, FD_BLOCK_FLAG_FINALIZED ) ) {
...
deshred( fd_blockstore_t * blockstore, ulong slot ) {
  ...
  ulong shred_cnt = block->meta.complete_idx + 1;
  ulong micro_cnt = FD_LOAD( ulong, fd_blockstore_shred_query( blockstore, slot, 0 ) );

  /* Set the timestamp. */

  block->meta.ts              = fd_log_wallclock();

  /* Get the size of the data region by summing shred payload sizes. */
  ...
  for( uint i = 0; i < shred_cnt; i++ ) {
    ...
    if( FD_UNLIKELY( !query ) ) {
      FD_LOG_ERR(( "[%s] missing shred (%lu, %u) while deshredding", __func__, slot, i ));
    }
    data_sz += fd_shred_payload_sz( &query->hdr );
  }
  if ( FD_UNLIKELY( data_sz < sizeof(ulong) ) ) {
    FD_LOG_ERR(( "[%s] block data_sz too small for slot %lu", __func__, slot ));
  }

  /* Get the size of the fd_block_shred_t region using shred_cnt. */

  ulong shred_sz = shred_cnt * sizeof(fd_block_shred_t);

  /* Get the size of the fd_block_micro_t region using micro_cnt, which is the first 8 bytes of the block. */

  ulong micro_sz  = micro_cnt * sizeof(fd_block_micro_t);

  /* Allocate the memory region. */

  uchar * mem = fd_alloc_malloc( alloc, fd_blockstore_align(), data_sz + shred_sz + micro_sz );
  if( FD_UNLIKELY( !mem ) ) {
    FD_LOG_ERR(( "[%s] failed to allocate mem for slot %lu", __func__, slot ));
  }
  block->data.data_gaddr  = fd_wksp_gaddr_fast( wksp, mem );
  block->data.data_sz     = data_sz;
  block->data.shred_gaddr = fd_wksp_gaddr_fast( wksp, mem + data_sz);
  block->data.shred_cnt   = shred_cnt;
  block->data.micro_gaddr = fd_wksp_gaddr_fast( wksp, mem + data_sz + shred_sz );
  block->data.micro_cnt   = micro_cnt;

  /* Copy in the shred payloads to the data region and shred headers
     into the fd_block_shred_t region. */

  fd_deshredder_t    deshredder = { 0 };
  fd_shred_t const * deshredder_shreds[1]  = { 0 };
  fd_deshredder_init( &deshredder, mem, data_sz, deshredder_shreds, 0 );
  long  rc  = -FD_SHRED_EPIPE;
  ulong off = 0;
  fd_block_shred_t * block_shreds = fd_wksp_laddr_fast( wksp, block->data.shred_gaddr );
  for( uint i = 0; i < shred_cnt; i++ ) {
    fd_shred_key_t   key       = { .slot = slot, .idx = i };
    fd_buf_shred_t * buf_shred = fd_buf_shred_map_ele_remove( shred_map, &key, NULL, shred_pool );
    if( FD_UNLIKELY( !buf_shred ) ) {
      FD_LOG_ERR(( "[%s] missing shred idx %u during deshred. slot %lu.", __func__, i, slot ));
    }

    fd_shred_t const * shred = fd_blockstore_shred_query( blockstore, slot, i );
    deshredder.shreds        = &shred;
    deshredder.shred_cnt     = 1;
    rc                       = fd_deshredder_next( &deshredder );
    FD_TEST( rc >= 0 );

    block_shreds[i].hdr = *shred;
    block_shreds[i].off = off;

    FD_TEST( !memcmp( &block_shreds[i].hdr, shred, sizeof(fd_shred_t) ) );
    FD_TEST( !memcmp( mem + block_shreds[i].off,
                      fd_shred_data_payload( shred ),
                      fd_shred_payload_sz( shred ) ) );

    off += fd_shred_payload_sz( shred );
    fd_buf_shred_pool_ele_release( shred_pool, buf_shred );
  }

  /* Check the deshredder for errors. */

  if( FD_UNLIKELY( rc < 0 || deshredder.result != FD_SHRED_ESLOT ) ) {
    FD_LOG_WARNING(( "[%s] deshredding slot %lu failed. deshredder rc: %ld, result: %ld", __func__, slot, rc, deshredder.result ));
    fd_alloc_free( alloc, mem );
    fd_blockstore_slot_remove( blockstore, slot );
    for( uint i = 0; i < shred_cnt; i++ ) {
      fd_shred_key_t key = { .slot = slot, .idx = i };
      fd_buf_shred_pool_ele_release( shred_pool, fd_buf_shred_map_ele_remove( shred_map, &key, NULL, shred_pool ) );
    }
    return FD_BLOCKSTORE_ERR_DESHRED_INVALID;
  }

  index_micros_and_txns( blockstore, slot, &block->data );

  fd_block_micro_t *    micros     = fd_wksp_laddr_fast( fd_blockstore_wksp( blockstore ), block->data.micro_gaddr );
  fd_microblock_hdr_t * last_micro = (fd_microblock_hdr_t *)( mem + micros[block->data.micro_cnt - 1].off );
  memcpy( &block->meta.block_hash, last_micro->hash, sizeof(fd_hash_t) );
  return FD_BLOCKSTORE_OK;
}
...
fd_blockstore_shred_insert( fd_blockstore_t * blockstore, fd_shred_t const * shred ) {
  ...
  if( FD_UNLIKELY( !fd_buf_shred_pool_free( shred_pool ) ) ) {
    FD_LOG_ERR(( "[%s] fd_buf_shred_pool is full. halting.", __func__ ));
  }
  ...
  fd_block_t * block_map = fd_blockstore_block_map( blockstore );
  fd_block_t * block = fd_block_map_query( block_map, &slot, NULL );
  if( FD_UNLIKELY( !block ) ) {
    ...
    block = fd_block_map_insert( block_map, &slot );
    if( FD_UNLIKELY( !block ) ) return FD_BLOCKSTORE_ERR_SLOT_FULL;

    /* Set the map key. */

    block->slot = block->slot;

    /* Initialize the block_meta. Note some fields are initialized
       to dummy values because we do not have all the necessary metadata
       yet. */

    fd_block_meta_t * block_meta = &block->meta;

    block_meta->parent_slot = shred->slot - shred->data.parent_off;
    memset( block_meta->child_slots,
            UCHAR_MAX,
            FD_BLOCKSTORE_CHILD_SLOT_MAX * sizeof(ulong) );
    block_meta->child_slot_cnt = 0;

    block_meta->block_height   = 0;
    block_meta->block_hash     = ( fd_hash_t ){ 0 };
    block_meta->bank_hash      = ( fd_hash_t ){ 0 };
    block_meta->flags          = 0;
    block_meta->ts             = 0;
    block_meta->reference_tick = (uchar)( (int)shred->data.flags &
                                               (int)FD_SHRED_DATA_REF_TICK_MASK );
    block_meta->consumed_idx   = FD_SHRED_IDX_NULL;
    block_meta->received_idx   = shred->idx + 1;
    block_meta->complete_idx   = FD_SHRED_IDX_NULL;

    /* Initialize block data to 0. */

    block->data.data_gaddr = 0;
  }

  fd_block_meta_t * block_meta = &block->meta;
  FD_LOG_DEBUG(( "slot_meta->consumed_idx: %u, shred->slot: %lu, slot_meta->received_idx: %u, "
                 "shred->idx: %u, shred->complete_idx: %u",
                 block_meta->consumed_idx,
                 shred->slot,
                 block_meta->received_idx,
                 shred->idx,
                 block_meta->complete_idx ));

  /* Update shred windowing metadata: consumed, received, shred_cnt */

  while( fd_blockstore_shred_query( blockstore, shred->slot, (uint)( block_meta->consumed_idx + 1U ) ) ) {
    block_meta->consumed_idx++;
  }
  block_meta->received_idx = fd_uint_max( block_meta->received_idx, shred->idx + 1 );
  if( shred->data.flags & FD_SHRED_DATA_FLAG_SLOT_COMPLETE ) block_meta->complete_idx = shred->idx;
  ...
  if( FD_LIKELY( parent_block_meta ) ) {
    int found = 0;
    for( ulong i = 0; i < parent_block_meta->child_slot_cnt; i++ ) {
      if( FD_LIKELY( parent_block_meta->child_slots[i] == slot ) ) {
        found = 1;
    ...
    if( FD_UNLIKELY( !found ) ) {
      if( parent_block_meta->child_slot_cnt == FD_BLOCKSTORE_CHILD_SLOT_MAX ) {
        FD_LOG_ERR(( "failed to add slot %lu to parent %lu's children. exceeding child slot max",
                     slot,
                     block_meta->parent_slot ));
      }
      parent_block_meta->child_slots[parent_block_meta->child_slot_cnt++] = slot;
    }
  }

  if( FD_LIKELY( block_meta->consumed_idx == FD_SHRED_IDX_NULL ||
                 block_meta->consumed_idx != block_meta->complete_idx ) ) {
    return FD_BLOCKSTORE_OK;
  }

  /* Received all shreds, so try to assemble a block. */
  FD_LOG_DEBUG(( "[%s] received all shreds for slot %lu - now building a block", __func__, shred->slot ));
...
fd_blockstore_shred_query( fd_blockstore_t * blockstore, ulong slot, uint shred_idx ) {
  ...
  fd_buf_shred_t *     query      = fd_buf_shred_map_ele_query( shred_map, &key, NULL, shred_pool );
...
fd_blockstore_shred_query_volatile( fd_blockstore_t * blockstore,
  ...
  fd_block_t * block = fd_blockstore_block_query( blockstore, slot );
  if( FD_UNLIKELY( !block || block->data.data_gaddr == 0 ) ) return -1;
  if( shred_idx > block->meta.complete_idx ) return -1;
  fd_wksp_t * wksp = fd_blockstore_wksp( blockstore );
  fd_block_shred_t * shreds = fd_wksp_laddr_fast( wksp, block->data.shred_gaddr );
  ulong sz = fd_shred_payload_sz( &shreds[shred_idx].hdr );
  memcpy( buf, &shreds[shred_idx].hdr, FD_SHRED_DATA_HEADER_SZ );
  fd_memcpy( (uchar*)buf + FD_SHRED_DATA_HEADER_SZ, (uchar*)fd_wksp_laddr_fast( wksp, block->data.data_gaddr ) + shreds[shred_idx].off, sz );
...
fd_blockstore_parent_slot_query( fd_blockstore_t * blockstore, ulong slot ) {
  fd_block_meta_t * query = fd_blockstore_block_meta_query( blockstore, slot );
...
int
fd_blockstore_block_data_query_volatile( fd_blockstore_t * blockstore, ulong slot, fd_block_meta_t * block_meta_out, fd_valloc_t alloc, uchar ** block_data_out, ulong * block_data_out_sz ) {
  ...
  fd_wksp_t * wksp = fd_blockstore_wksp( blockstore );
  fd_block_t const * block_map = fd_wksp_laddr_fast( wksp, blockstore->block_map_gaddr );
  for(;;) {
    uint seqnum;
    if( FD_UNLIKELY( fd_readwrite_start_concur_read( &blockstore->lock, &seqnum ) ) ) continue;

    fd_block_t const * query = fd_block_map_query_safe( block_map, &slot, NULL );
    if( FD_UNLIKELY( !query ) ) return FD_BLOCKSTORE_ERR_SLOT_MISSING;
    memcpy( block_meta_out, query, sizeof(fd_block_meta_t) );
    ulong blk_gaddr = query->data.data_gaddr;
    ...
    ulong blk_data_gaddr = query->data.data_gaddr;
    if( FD_UNLIKELY( !blk_data_gaddr ) ) return FD_BLOCKSTORE_ERR_SLOT_MISSING;
    ulong sz = *block_data_out_sz = query->data.data_sz;
...
fd_blockstore_block_meta_query_volatile( fd_blockstore_t * blockstore, ulong slot, fd_block_meta_t * block_meta_out ) {
  ...
  for(;;) {
    ...
    fd_block_t const * query = fd_block_map_query_safe( block_map, &slot, NULL );
    if( FD_UNLIKELY( !query ) ) return FD_BLOCKSTORE_ERR_SLOT_MISSING;
    memcpy( block_meta_out, &query->meta, sizeof(fd_block_meta_t) );
    ulong blk_gaddr = query->data.data_gaddr;
    if( FD_UNLIKELY( !blk_gaddr ) ) return FD_BLOCKSTORE_ERR_SLOT_MISSING;
...
fd_blockstore_txn_query_volatile( fd_blockstore_t * blockstore, uchar const sig[FD_ED25519_SIG_SZ], fd_blockstore_txn_map_t * txn_out, long * blk_ts, uchar * blk_flags, uchar txn_data_out[FD_TXN_MTU] ) {
  ...
  for(;;) {
    ...
    fd_block_t const * query = fd_block_map_query_safe( block_map, &txn_out->slot, NULL );
    ...
    if( blk_ts ) *blk_ts = query->meta.ts;
    if( blk_flags ) *blk_flags = query->meta.flags;
    ulong ptr = query->data.data_gaddr;
    ulong sz = query->data.data_sz;
...
fd_blockstore_log_block_status( fd_blockstore_t * blockstore, ulong around_slot ) {
  for( ulong i = around_slot - 5; i < around_slot + 20; ++i ) {
    fd_block_meta_t * block_meta = fd_blockstore_block_meta_query( blockstore, around_slot );
...
fd_blockstore_log_mem_usage( fd_blockstore_t * blockstore ) {
  ...
  fd_block_t * block_map = fd_blockstore_block_map( blockstore );
  ulong block_map_cnt = fd_block_map_key_cnt( block_map );
  ulong block_map_max = fd_block_map_key_max( block_map );
  FD_LOG_NOTICE(( "slot map footprint: %s (%lu entries used out of %lu, %lu%%)",
                  fd_smart_size( fd_block_map_footprint( block_map_max ), tmp1, sizeof(tmp1) ),
                  block_map_cnt,
                  block_map_max,
                  (100U*block_map_cnt)/block_map_max ));
  fd_blockstore_txn_map_t * txn_map = fd_blockstore_txn_map( blockstore );
  ...
  while( !fd_blockstore_slot_deque_empty( q ) ) {
    ...
    fd_block_data_t * block_data = fd_blockstore_block_data_query( blockstore, curr );
    if( FD_UNLIKELY( !block_data || !block_data->data_gaddr ) ) continue;
    if( block_data->data_gaddr ) {
      block_cnt++;
      data_tot += block_data->data_sz;
      data_max = fd_ulong_max( data_max, block_data->data_sz );
    }
...
fd_blockstore_init( fd_blockstore_t * blockstore, ulong slot ) {
  ...
  fd_block_t *      block      = fd_block_map_insert( fd_blockstore_block_map( blockstore ), &slot );
  fd_block_meta_t * block_meta = &block->meta;
  fd_block_data_t * block_data = &block->data;

  block_meta->parent_slot     = FD_SLOT_NULL;
  for( ulong i = 0; i < FD_BLOCKSTORE_CHILD_SLOT_MAX; i++ ) {
    block_meta->child_slots[i] = FD_SLOT_NULL;
  }
  block_meta->child_slot_cnt  = 0;
  ...
  block_meta->block_hash      = ( fd_hash_t ){ 0 };
  block_meta->bank_hash       = ( fd_hash_t ){ 0 };
  block_meta->merkle_hash     = ( fd_hash_t ){ 0 };
  block_meta->flags           = fd_uchar_set_bit(
                                     fd_uchar_set_bit(
                                     fd_uchar_set_bit(
                                     fd_uchar_set_bit( block_meta->flags,
                                       /* x1111x1x */
                                       FD_BLOCK_FLAG_PROCESSED ),
                                       FD_BLOCK_FLAG_EQVOCSAFE ),
                                       FD_BLOCK_FLAG_CONFIRMED ),
                                       FD_BLOCK_FLAG_FINALIZED );
  block_meta->reference_tick  = 0;
  block_meta->ts              = 0;

  block_meta->consumed_idx    = 0;
  block_meta->received_idx    = 0;
  block_meta->complete_idx    = 0;
  ...
  uchar * mem = fd_alloc_malloc( fd_blockstore_alloc( blockstore ),
                                 fd_blockstore_align(),
                                 sizeof(ulong) );

  /* This fake block has no data, shred or microblock region, and the
     first 8 bytes (the micro cnt) are 0. */

  memset( mem, 0, sizeof(ulong) );

  block_data->data_gaddr  = fd_wksp_gaddr_fast( fd_blockstore_wksp( blockstore ), mem );
  block_data->data_sz     = sizeof(ulong);
  block_data->shred_gaddr = sizeof(ulong);
  block_data->shred_cnt   = 0;
  block_data->micro_gaddr = sizeof(ulong);
  block_data->micro_cnt   = 0;
}
