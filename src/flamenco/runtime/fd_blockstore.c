...
deshred( fd_blockstore_t * blockstore, ulong slot ) {
  ...
  FD_LOG_NOTICE(("deshred TIME 1: %ld ms", (fd_log_wallclock() - time) / 1000 ));
  ...
  FD_LOG_NOTICE(("deshred TIME 2: %ld ms", (fd_log_wallclock() - time) / 1000 ));
  time = fd_log_wallclock();
  ...
  FD_LOG_NOTICE(("deshred TIME 3: %ld ms", (fd_log_wallclock() - time) / 1000 ));
  time = fd_log_wallclock();
  ...
  FD_LOG_NOTICE(("deshred TIME 4: %ld ms", (fd_log_wallclock() - time) / 1000 ));
  time = fd_log_wallclock();

  fd_blockstore_scan_block( blockstore, slot, block );

  FD_LOG_NOTICE(("deshred TIME 5 (scan_block): %ld ms", (fd_log_wallclock() - time) / 1000 ));
  time = fd_log_wallclock();
  ...
  FD_LOG_NOTICE(("deshred TIME 6: %ld ms", (fd_log_wallclock() - time) / 1000 ));
  time = fd_log_wallclock();
...
fd_blockstore_block_meta_test( fd_blockstore_t * blockstore, ulong slot ) {
  ...
  for(;;) {
    ...
    loop_ct++;
    if( loop_ct > 3 ) FD_LOG_WARNING(( "[%s] loop_ct > 3", __func__ ));
  }
}
