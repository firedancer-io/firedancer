...
fd_blockstore_slot_remove( fd_blockstore_t * blockstore, ulong slot ) {
  ...
  if( FD_UNLIKELY( parent_block_map_entry ) ) { /* optimize for not found (min slot w/ no parent) */

    if ( FD_LIKELY( parent_block_map_entry->child_slot_cnt == 1 ) ) { /* optimize for one child */

      /* If there's only one child, remove it. */

      parent_block_map_entry->child_slots[0] = FD_SLOT_NULL;
      parent_block_map_entry->child_slot_cnt = 0;
  ...
  if( FD_UNLIKELY( !fd_blockstore_shred_complete( blockstore, slot ) ) ) {
  ...
  /* Remove slot's txns from the txn_map */

  uchar * data = fd_wksp_laddr_fast( fd_blockstore_wksp( blockstore ), block_map_entry->data_gaddr );
  ulong   sz   = block_map_entry->data_sz;
  ulong   off  = sizeof( ulong );
  for( ulong mblk = 0; mblk < block_map_entry->micro_cnt; ++mblk ) {
    fd_microblock_hdr_t * hdr = (fd_microblock_hdr_t *)( (const uchar *)data + off );
    off += sizeof( fd_microblock_hdr_t );
    for( ulong txn_idx = 0; txn_idx < hdr->txn_cnt; txn_idx++ ) {
      uchar         txn_out[FD_TXN_MAX_SZ];
      uchar const * raw    = (uchar const *)data + off;
      ulong         pay_sz = 0;
      fd_txn_parse_core( (uchar const *)raw,
                         fd_ulong_min( sz - off, FD_TXN_MTU ),
                         txn_out,
                         NULL,
                         &pay_sz );
      fd_txn_t const * txn = (fd_txn_t const *)txn_out;
      ...
      fd_blockstore_txn_key_t const * sigs =
          (fd_blockstore_txn_key_t const *)( (ulong)raw + (ulong)txn->signature_off );
      for( ulong j = 0; j < txn->signature_cnt; j++ ) {
        fd_blockstore_txn_map_t * txn_map_entry =
            fd_blockstore_txn_map_remove( fd_blockstore_txn_map( blockstore ), sigs + j );
  ...
  ulong mgaddr = block_map_entry->txn_meta_gaddr;
  while( mgaddr ) {
...
deshred( fd_blockstore_t * blockstore, ulong slot ) {
  ...
  fd_block_map_t * block = fd_blockstore_block_map_query( blockstore, slot );

  ulong shred_cnt = block->complete_idx + 1;
  ...
  block->data_gaddr  = fd_wksp_gaddr_fast( wksp, mem );
  block->data_sz     = data_sz;
  block->shred_gaddr = fd_wksp_gaddr_fast( wksp, mem + data_sz);
  block->shred_cnt   = shred_cnt;
  block->micro_gaddr = fd_wksp_gaddr_fast( wksp, mem + data_sz + shred_sz );
  block->micro_cnt   = micro_cnt;

  /* Copy in the shred payloads. */

  fd_deshredder_t    deshredder = { 0 };
  fd_shred_t const * shreds[1]  = { 0 };
  fd_deshredder_init( &deshredder, mem, data_sz, shreds, 0 );
  ...
  for( uint i = 0; i < shred_cnt; i++ ) {
    ...
    fd_block_shred_t * shreds = fd_wksp_laddr_fast( wksp, block->shred_gaddr );
    shreds[i].hdr             = *shred;
    shreds[i].off             = off;

    FD_TEST( !memcmp( &shreds[i].hdr, shred, sizeof(fd_shred_t) ) );
    FD_TEST( !memcmp( mem + shreds[i].off,
  ...
  /* For every microblock, index the transactions in the transaction map. */

  off = sizeof(ulong); /* Skip the microblock count. */
  for( ulong i = 0; i < micro_cnt; ++i ) {
    if( FD_UNLIKELY( off + sizeof( fd_microblock_hdr_t ) > data_sz ) ) {
      FD_LOG_WARNING(( "[%s] deshredding slot %lu failed. incomplete microblock header.", __func__, slot ));
      return FD_BLOCKSTORE_ERR_DESHRED_INVALID;
    }
    fd_microblock_hdr_t * hdr = (fd_microblock_hdr_t *)mem + off;
    off += sizeof(fd_microblock_hdr_t);

    for( ulong txn_idx = 0; txn_idx < hdr->txn_cnt; txn_idx++ ) {
      uchar         txn_out[FD_TXN_MAX_SZ];
      uchar const * raw    = (uchar const *)mem + off;
      ulong         pay_sz = 0;
      ulong         txn_sz = fd_txn_parse_core( (uchar const *)raw,
                                        fd_ulong_min( block->data_sz - off, FD_TXN_MTU ),
                                        txn_out,
                                        NULL,
                                        &pay_sz );
      if( txn_sz == 0 || txn_sz > FD_TXN_MTU ) {
        FD_LOG_ERR(( "failed to parse transaction %lu in microblock %lu in slot %lu. txn size: %lu",txn_idx,i,slot,txn_sz ));
      }
      fd_txn_t const * txn = (fd_txn_t const *)txn_out;

      if( pay_sz == 0UL ) {
        FD_LOG_ERR(( "failed to parse transaction %lu in microblock %lu in slot %lu", txn_idx, i, slot ));
      }
  ...
  fd_microblock_hdr_t * last_micro = (fd_microblock_hdr_t *)( mem + micros[block->micro_cnt - 1].off );
  memcpy( &block->block_hash, last_micro->hash, sizeof(fd_hash_t) );
  return FD_BLOCKSTORE_OK;
}
...
fd_blockstore_shred_query_volatile( fd_blockstore_t * blockstore,
  ...
  if( FD_UNLIKELY( !block_map_entry || block_map_entry->data_gaddr == 0 ) ) return -1;
  if( shred_idx > block_map_entry->complete_idx ) return -1;
  fd_wksp_t * wksp = fd_blockstore_wksp( blockstore );
  fd_block_shred_t * shreds = fd_wksp_laddr_fast( wksp, block_map_entry->shred_gaddr );
  ulong sz = fd_shred_payload_sz( &shreds[shred_idx].hdr );
  memcpy( buf, &shreds[shred_idx].hdr, FD_SHRED_DATA_HEADER_SZ );
  fd_memcpy( (uchar*)buf + FD_SHRED_DATA_HEADER_SZ, (uchar*)fd_wksp_laddr_fast( wksp, block_map_entry->data_gaddr ) + shreds[shred_idx].off, sz );
...
fd_blockstore_block_query( fd_blockstore_t * blockstore, ulong slot ) {
  ...
  if( FD_UNLIKELY( !query || query->data_gaddr == 0 ) ) return NULL;
  return fd_wksp_laddr_fast( fd_blockstore_wksp( blockstore ), query->data_gaddr );
}
...
fd_blockstore_block_data_query_volatile( fd_blockstore_t * blockstore, ulong slot, fd_block_map_t * block_map_entry_out, fd_valloc_t alloc, uchar ** block_data_out, ulong * block_data_out_sz ) {
  ...
  for(;;) {
    ...
    ulong blk_gaddr = query->data_gaddr;
    ...
    ulong blk_data_gaddr = query->data_gaddr;
    if( FD_UNLIKELY( !blk_data_gaddr ) ) return FD_BLOCKSTORE_ERR_SLOT_MISSING;
    ulong sz = *block_data_out_sz = query->data_sz;
...
fd_blockstore_txn_query_volatile( fd_blockstore_t * blockstore, uchar const sig[FD_ED25519_SIG_SZ], fd_blockstore_txn_map_t * txn_out, long * blk_ts, uchar * blk_flags, uchar txn_data_out[FD_TXN_MTU] ) {
  ...
  for(;;) {
    ...
    ulong ptr = query->data_gaddr;
    ulong sz = query->data_sz;
...
fd_blockstore_log_mem_usage( fd_blockstore_t * blockstore ) {
  ...
  while( !fd_blockstore_slot_deque_empty( q ) ) {
    ...
    if( FD_UNLIKELY( !block_map_entry || !block_map_entry->data_gaddr ) ) continue;
    if( block_map_entry->data_gaddr ) {
      block_cnt++;
      data_tot += block_map_entry->data_sz;
      data_max = fd_ulong_max( data_max, block_map_entry->data_sz );
    }
...
fd_blockstore_init( fd_blockstore_t * blockstore, ulong slot ) {
  ...
  block_map_entry->data_gaddr  = fd_wksp_gaddr_fast( fd_blockstore_wksp( blockstore ), mem );
  block_map_entry->data_sz     = sizeof(ulong);
  block_map_entry->shred_gaddr = sizeof(ulong);
  block_map_entry->shred_cnt   = 0;
  block_map_entry->micro_gaddr = sizeof(ulong);
  block_map_entry->micro_cnt   = 0;
}
