...
void fd_enable_everything(struct fd_features *f) {
  fd_memset(f, 0, sizeof(*f));
  f->secp256k1_program_enabled = 1; // secp256k1 program
  f->spl_token_v2_multisig_fix = 1; // spl-token multisig fix
  f->no_overflow_rent_distribution = 1; // no overflow rent distribution
  ...
  f->full_inflation_mainnet_certusoneenable = 1; // full inflation enabled by Certus One
  ...
  f->check_init_vote_data = 1; // check initialized Vote data
  f->require_custodian_for_locked_stake_authorize = 1; // require custodian to authorize withdrawer change for locked stake
  f->vote_stake_checked_instructions = 1; // vote/state program checked instructions #18345
  f->system_transfer_zero_check = 1; // perform all checks for transfers of 0 lamports
  f->spl_token_v2_set_authority_fix = 1; // spl-token set_authority fix
  f->demote_program_write_locks = 1; // demote program write locks to readonly, except when upgradeable loader present #19593 #20265
  f->send_to_tpu_vote_port = 1; // send votes to the tpu vote port
  f->reduce_required_deploy_balance = 1; // reduce required payer balance for program deploys
  f->verify_tx_signatures_len = 1; // prohibit extra transaction signatures
  f->stake_program_advance_activating_credits_observed = 1; // Enable advancing credits observed for activation epoch #19309
  f->stake_merge_with_unmatched_credits_observed = 1; // allow merging active stakes with unmatched credits_observed #18985
  f->secp256k1_recover_syscall_enabled = 1; // secp256k1_recover syscall
  f->rent_for_sysvars = 1; // collect rent from accounts owned by sysvars
  f->optimize_epoch_boundary_updates = 1; // optimize epoch boundary updates
  f->dedupe_config_program_signers = 1; // dedupe config program signers
  f->libsecp256k1_0_5_upgrade_enabled = 1; // upgrade libsecp256k1 to v0.5.0
  f->stakes_remove_delegation_if_inactive = 1; // remove delegations from stakes cache when inactive
  f->add_compute_budget_program = 1; // Add compute_budget_program
  f->reject_non_rent_exempt_vote_withdraws = 1; // fail vote withdraw instructions which leave the account non-rent-exempt
  f->evict_invalid_stakes_cache_entries = 1; // evict invalid stakes cache entries on epoch boundaries
  f->spl_token_v3_3_0_release = 1; // spl-token v3.3.0 release
  f->remove_native_loader = 1; // remove support for the native loader
  f->ed25519_program_enabled = 1; // enable builtin ed25519 signature verify program
  f->sol_log_data_syscall_enabled = 1; // enable sol_log_data syscall
  f->return_data_syscall_enabled = 1; // enable sol_{set,get}_return_data syscall
  f->spl_associated_token_account_v1_0_4 = 1; // SPL Associated Token Account Program release version 1.0.4, tied to token 3.3.0 #22648
  f->leave_nonce_on_success = 1; // leave nonce as is on success
  f->require_rent_exempt_accounts = 1; // require all new transaction accounts with data to be rent-exempt
  f->do_support_realloc = 1; // support account data reallocation
  f->add_get_processed_sibling_instruction_syscall = 1; // add add_get_processed_sibling_instruction_syscall
  f->tx_wide_compute_cap = 1; // transaction wide compute cap
  f->requestable_heap_size = 1; // Requestable heap frame size
  f->warp_timestamp_with_a_vengeance = 1; // warp timestamp again, adjust bounding to 150% slow #25666
  f->nonce_must_be_writable = 1; // nonce must be writable
  f->reject_empty_instruction_without_program = 1; // fail instructions which have native_loader as program_id directly
  f->add_shred_type_to_shred_seed = 1; // add shred-type to shred seed #25556
  f->fixed_memcpy_nonoverlapping_check = 1; // use correct check for nonoverlapping regions in memcpy syscall
  f->nonce_must_be_advanceable = 1; // durable nonces must be advanceable
  f->enable_durable_nonce = 1; // enable durable nonce #25744
  f->separate_nonce_from_blockhash = 1; // separate durable nonce and blockhash domains #25744
  f->nonce_must_be_authorized = 1; // nonce must be authorized
  f->update_syscall_base_costs = 1; // update syscall base costs
  f->vote_withdraw_authority_may_change_authorized_voter = 1; // vote account withdraw authority may change the authorized voter #22521
  f->disable_bpf_deprecated_load_instructions = 1; // disable ldabs* and ldind* SBF instructions
  f->disable_bpf_unresolved_symbols_at_runtime = 1; // disable reporting of unresolved SBF symbols at runtime
  f->executables_incur_cpi_data_cost = 1; // Executables incur CPI data costs
  f->max_tx_account_locks = 1; // enforce max number of locked accounts per transaction
  f->quick_bail_on_panic = 1; // quick bail on panic
  f->default_units_per_instruction = 1; // Default max tx-wide compute units calculated per instruction
  f->record_instruction_in_transaction_context_push = 1; // move the CPI stack overflow check to the end of push
  f->add_set_compute_unit_price_ix = 1; // add compute budget ix for setting a compute unit price
  f->limit_secp256k1_recovery_id = 1; // limit secp256k1 recovery id
  f->check_physical_overlapping = 1; // check physical overlapping regions
  f->prevent_calling_precompiles_as_programs = 1; // prevent calling precompiles as programs
  f->spl_associated_token_account_v1_1_0 = 1; // SPL Associated Token Account Program version 1.1.0 release #24741
  f->spl_token_v3_4_0 = 1; // SPL Token Program version 3.4.0 release #24740
  f->disable_fee_calculator = 1; // deprecate fee calculator
  f->vote_authorize_with_seed = 1; // An instruction you can use to change a vote accounts authority when the current authority is a derived key #25860
  ...
  f->preserve_rent_epoch_for_rent_exempt_accounts = 1; // preserve rent epoch for rent exempt accounts #26479
  f->filter_votes_outside_slot_hashes = 1; // filter vote slots older than the slot hashes history
  ...
  f->add_get_minimum_delegation_instruction_to_stake_program = 1; // add GetMinimumDelegation instruction to stake program
  f->stake_allow_zero_undelegated_amount = 1; // Allow zero-lamport undelegated amount for initialized stakes #24670
  f->credits_auto_rewind = 1; // Auto rewind stake's credits_observed if (accidental) vote recreation is detected #22546
  f->libsecp256k1_fail_on_bad_count2 = 1; // fail libsec256k1_verify if count appears wrong
  f->vote_state_update_root_fix = 1; // fix root in vote state updates #27361
  f->move_serialized_len_ptr_in_cpi = 1; // cpi ignore serialized_len_ptr #29592
  f->stake_split_uses_rent_sysvar = 1; // stake split instruction uses rent sysvar
  f->on_load_preserve_rent_epoch_for_rent_exempt_accounts = 1; // on bank load account, do not try to fix up rent_epoch #28541
  f->disable_rehash_for_rent_epoch = 1; // on accounts hash calculation, do not try to rehash accounts #28934
  f->fix_recent_blockhashes = 1; // stop adding hashes for skipped slots to recent blockhashes
  f->cap_bpf_program_instruction_accounts = 1; // enforce max number of accounts per bpf program instruction #26628
  f->update_rewards_from_cached_accounts = 1; // update rewards from cached accounts
  f->use_default_units_in_fee_calculation = 1; // use default units per instruction in fee calculation #26785
  f->disable_builtin_loader_ownership_chains = 1; // disable builtin loader ownership chains #29956
  f->check_slice_translation_size = 1; // check size when translating slices
  f->disable_fees_sysvar = 1; // disable fees sysvar
  f->stop_truncating_strings_in_syscalls = 1; // Stop truncating strings in syscalls #31029
  f->incremental_snapshot_only_incremental_hash_calculation = 1; // only hash accounts in incremental snapshot during incremental snapshot creation #26799
  f->remove_bpf_loader_incorrect_program_id = 1; // stop incorrectly throwing IncorrectProgramId in bpf_loader #30747
  f->apply_cost_tracker_during_replay = 1; // apply cost tracker to blocks during replay #29595
  f->stake_redelegate_instruction = 1; // enable the redelegate stake instruction #26294
  f->reject_callx_r10 = 1; // Reject bpf callx r10 instructions
  f->update_hashes_per_tick = 1; // Update desired hashes per tick on epoch boundary
  f->epoch_accounts_hash = 1; // enable epoch accounts hash calculation #27539
  f->checked_arithmetic_in_fee_validation = 1; // checked arithmetic in fee validation #31273
  f->simplify_writable_program_account_check = 1; // Simplify checks performed for writable upgradeable program accounts #30559
  f->set_exempt_rent_epoch_max = 1; // set rent epoch to Epoch::MAX for rent-exempt accounts #28683
  f->enable_bpf_loader_set_authority_checked_ix = 1; // enable bpf upgradeable loader SetAuthorityChecked instruction #28424
  f->disable_deploy_of_alloc_free_syscall = 1; // disable new deployments of deprecated sol_alloc_free_ syscall
  f->enable_early_verification_of_account_modifications = 1; // enable early verification of account modifications #25899
  f->vote_state_add_vote_latency = 1; // replace Lockout with LandedVote (including vote latency) in vote state #31264
  f->curve25519_syscall_enabled = 1; // enable curve25519 syscalls
  f->error_on_syscall_bpf_function_hash_collisions = 1; // error on bpf function hash collisions
  f->drop_merkle_shreds = 1; // drop merkle shreds #29711
  f->compact_vote_state_updates = 1; // Compact vote state updates to lower block size
  ...
  f->native_programs_consume_cu = 1; // Native program should consume compute units #30620
  f->increase_tx_account_lock_limit = 1; // increase tx account lock limit to 128 #27241
  f->bpf_account_data_direct_mapping = 1; // use memory regions to map account data into the rbpf vm instead of copying the data
  f->cap_accounts_data_allocations_per_transaction = 1; // cap accounts data allocations per transaction #27375
  f->stake_raise_minimum_delegation_to_1_sol = 1; // Raise minimum stake delegation to 1.0 SOL #24357
  f->enable_alt_bn128_syscall = 1; // add alt_bn128 syscalls #27961
  f->remove_congestion_multiplier_from_fee_calculation = 1; // Remove congestion multiplier from transaction fee calculation #29881
  f->disable_cpi_setting_executable_and_rent_epoch = 1; // disable setting is_executable and_rent_epoch in CPI #26987
  f->clean_up_delegation_errors = 1; // Return InsufficientDelegation instead of InsufficientFunds or InsufficientStake where applicable #31206
  f->round_up_heap_size = 1; // round up heap size when calculating heap cost #30679
  f->skip_rent_rewrites = 1; // skip rewriting rent exempt accounts during rent collection #26491
  f->switch_to_new_elf_parser = 1; // switch to new ELF parser #30497
  f->vote_state_update_credit_per_dequeue = 1; // Calculate vote credits for VoteStateUpdate per vote dequeue to match credit awards for Vote instruction
  f->enable_turbine_fanout_experiments = 1; // enable turbine fanout experiments #29393
  f->full_inflation_devnet_and_testnet = 1; // full inflation on devnet and testnet
  f->cap_transaction_accounts_data_size = 1; // cap transaction accounts data size up to a limit #27839
  f->enable_big_mod_exp_syscall = 1; // add big_mod_exp syscall #28503
  f->stake_minimum_delegation_for_rewards = 1; // stakes must be at least the minimum delegation to earn rewards
  f->stop_sibling_instruction_search_at_parent = 1; // stop the search in get_processed_sibling_instruction when the parent instruction is reached #27289
  f->include_loaded_accounts_data_size_in_fee_calculation = 1; // include transaction loaded accounts data size in base fee calculation #30657
  f->remove_deprecated_request_unit_ix = 1; // remove support for RequestUnitsDeprecated instruction #27500
  f->relax_authority_signer_check_for_lookup_table_creation = 1; // relax authority signer check for lookup table creation #27205
  f->prevent_rent_paying_rent_recipients = 1; // prevent recipients of rent rewards from ending in rent-paying state #30151
  f->allow_votes_to_directly_update_vote_state = 1; // enable direct vote state update
  f->add_set_tx_loaded_accounts_data_size_instruction = 1; // add compute budget instruction for setting account data size per transaction #30366
  f->loosen_cpi_size_restriction = 1; // loosen cpi size restrictions #26641
  f->limit_max_instruction_trace_length = 1; // limit max instruction trace length #27939
  f->delay_visibility_of_program_deployment = 1; // delay visibility of program upgrades #30085
  f->disable_turbine_fanout_experiments = 1; // disable turbine fanout experiments #29393
  f->enable_partitioned_epoch_reward = 1; // enable partitioned rewards at epoch boundary #32166
  f->blake3_syscall_enabled = 1; // blake3 syscall
  f->last_restart_slot_sysvar = 1; // enable new sysvar last_restart_slot
  f->enable_request_heap_frame_ix = 1; // Enable transaction to request heap frame using compute budget instruction #30076
  f->keep_merkle_shreds = 1; // keep merkle shreds #29711
  f->enable_program_redeployment_cooldown = 1; // enable program redeployment cooldown #29135
  f->account_hash_ignore_slot = 1; // ignore slot when calculating an account hash #28420
  f->cap_accounts_data_len = 1; // cap the accounts data len
  f->commission_updates_only_allowed_in_first_half_of_epoch = 1; // validator commission updates are only allowed in the first half of an epoch #29362
  f->cap_accounts_data_size_per_block = 1; // cap the accounts data size per block #25517
