...
struct fd_txncache_private_nonce_txn {
  uint  slot;                            /* Slot where the transaction was executed.  A transaction might be in
                                            ...
                                            different forks.  The same slot will not appear multiple times
                                            however. */
  uint  slotcache_next;                  /* Pointer to the next element in the per-slot linked list containing
                                            this entry from the nonce txn pool. */
  uint  slotcache_prev;                  /* Pointer to the previous element in the per-slot linked list
                                            containing this entry from the nonce txn pool.  Used for updating
                                            linked lists when we reprobe the nonce txn hash table. */
...
struct fd_txncache_private_nonce_slotcache {
  ...
  uint  head;           /* The head of the linked list of nonce txns for this slot.  As we add
                           nonce transactions to the list, the head pointer is updated to the
                           new item, and the new item is pointed to the previous head. */
};
...
struct __attribute__((aligned(FD_TXNCACHE_ALIGN))) fd_txncache_private {
  ...
  /* Cache line aligned. */
  ulong  nonce_blockcache_max_slot;
  ulong  lowest_observed_slot;
  ...
  ulong  nonce_slotcache_off;  /* The cache of nonce transactions by slot, so we can quickly purge
                                  ...
                                  table that maps a slot number to a linked list of nonce transactions
                                  for that slot. */
...
fd_txncache_get_blockcache( fd_txncache_t const * tc ) {
...
fd_txncache_footprint( ulong max_rooted_slots,
  ...
  if( FD_UNLIKELY( max_nonce_txns>UINT_MAX ) ) return 0UL;
...
fd_txncache_new( void * shmem,
  ...
  if( FD_UNLIKELY( max_nonce_txns>UINT_MAX ) ) return NULL;
  ...
  FD_LOG_INFO(( "txncache footprint: root_slots %lu, blockcache %lu, txnpages_free %lu, txnpages %lu, nonce_slotcache %lu, nonce_txns %lu, nonce_blockcache %lu total %lu",
                txncache->blockcache_off-txncache->root_slots_off,
                txncache->txnpages_free_off-txncache->blockcache_off,
                txncache->txnpages_off-txncache->txnpages_free_off,
                txncache->nonce_slotcache_off-txncache->txnpages_off,
                txncache->nonce_txns_off-txncache->nonce_slotcache_off,
                txncache->nonce_blockcache_off-txncache->nonce_txns_off,
                _end-(ulong)_nonce_blockcache,
                _end-(ulong)txncache ));
  ...
  fd_txncache_private_nonce_slotcache_t * nonce_slotcache = (fd_txncache_private_nonce_slotcache_t *)_nonce_slotcache;
  for( ulong i=0UL; i<max_live_slots; i++ ) {
    blockcache[ i ].max_slot  = FD_TXNCACHE_ENTRY_FREE;
    nonce_slotcache[ i ].slot = FD_TXNCACHE_ENTRY_FREE;
  }
...
fd_txncache_print_state( fd_txncache_t const * tc,
                         char const *          prefix ) {
  FD_LOG_INFO(( "%s: rooted %lu/%lu slots, txnpages %u/%u free, blockcache pop curr %u peak %u/%lu, nonce slotcache pop curr %u peak %u/%lu, nonce slotcache per slot pop peak %u/%lu, query present %lu absent %lu",
                prefix,
...
fd_txncache_remove_blockcache_idx( fd_txncache_t * tc,
  ...
  /* Free pages. */
  uint * pages = fd_txncache_get_blockcache_pages( tc, blockcache+idx );
  memcpy( txnpages_free+tc->txnpages_free_cnt, pages, blockcache[ idx ].pages_cnt*sizeof(uint) );
  ...
  for(;;) {
    ...
    for(;;) {
      ...
      ulong hash  = fd_txncache_key_hash( blockcache[ idx ].blockhash );
...
fd_txncache_remove_nonce_txn_idx( fd_txncache_t * tc,
  ...
  if( FD_UNLIKELY( nonce_txns[ idx ].slotcache_prev!=UINT_MAX ) ) {
    FD_LOG_CRIT(( "invariant violation: nonce txn %u has prev %u", idx, nonce_txns[ idx ].slotcache_prev ));
  }
  if( FD_LIKELY( nonce_txns[ idx ].slotcache_next!=UINT_MAX ) ) {
    nonce_txns[ nonce_txns[ idx ].slotcache_next ].slotcache_prev = UINT_MAX;
  }
  /* Remove: make a hole at idx. */
  nonce_txns[ idx ].slot           = FD_TXNCACHE_ENTRY_FREE_UINT;
  nonce_txns[ idx ].slotcache_next = UINT_MAX;
  nonce_txns[ idx ].slotcache_prev = UINT_MAX;
  ...
  for(;;) {
    ...
    for(;;) {
      ...
      ulong start = nonce_txns[ idx ].slot%tc->nonce_txns_max;
      if( !(((hole<start) & (start<=idx)) | ((hole>idx) & ((hole<start) | (start<=idx)))) ) break;
    }

    nonce_txns[ hole ] = nonce_txns[ idx ];
    /* Repair the linked list. */
    if( FD_LIKELY( nonce_txns[ hole ].slotcache_prev!=UINT_MAX ) ) {
      nonce_txns[ nonce_txns[ hole ].slotcache_prev ].slotcache_next = hole;
    }
    if( FD_LIKELY( nonce_txns[ hole ].slotcache_next!=UINT_MAX ) ) {
      nonce_txns[ nonce_txns[ hole ].slotcache_next ].slotcache_prev = hole;
    }
...
fd_txncache_remove_nonce_slotcache_idx( fd_txncache_t * tc,
  ...
  while( nonce_txn_idx!=UINT_MAX ) {
    nonce_txn_idx = nonce_txns[ nonce_txn_idx ].slotcache_next;
    fd_txncache_remove_nonce_txn_idx( tc, nonce_txn_idx, &nonce_txn_idx );
    nonce_slotcache[ idx ].nonce_slot_pop--;
  }
  if( FD_UNLIKELY( nonce_slotcache[ idx ].nonce_slot_pop!=0U ) ) {
    FD_LOG_CRIT(( "invariant violation: nonce slotcache %lu has nonce_slot_pop %u after purging", idx, nonce_slotcache[ idx ].nonce_slot_pop ));
  }
...
fd_txncache_find_blockhash( fd_txncache_t *                     tc,
  ...
  fd_txncache_private_blockcache_t * tc_blockcache = fd_txncache_get_blockcache( tc );
...
fd_txncache_ensure_nonce_blockcache( fd_txncache_t *                            tc,
  ...
  for(;;) {
    ...
    if( FD_LIKELY( FD_TXNCACHE_ENTRY_FREE_UINT==FD_ATOMIC_CAS( &((*out_blockcache)->max_slot), FD_TXNCACHE_ENTRY_FREE_UINT, FD_TXNCACHE_ENTRY_XBUSY_UINT ) ) ) {
      memcpy( (*out_blockcache)->blockhash, blockhash, 32UL );
      (*out_blockcache)->txnhash_offset = 0UL;
      //FIXME gauge for how many unique nonce blockhashes we have
      // FD_ATOMIC_FETCH_AND_ADD( &tc->block_pop, 1U );
      FD_COMPILER_MFENCE();
...
fd_txncache_ensure_new_nonce_txn( fd_txncache_t *                          tc,
  ...
  for(;;) {
    ...
    if( FD_UNLIKELY( nonce_txn_find==FD_TXNCACHE_FIND_FULL ) ) FD_LOG_CRIT(( "invariant violation: nonce txn table full" ));
    if( FD_UNLIKELY( nonce_txn_find==FD_TXNCACHE_FIND_FOUND ) ) FD_LOG_CRIT(( "invariant violation: nonce txn table found existing nonce txn" ));
...
fd_txncache_ensure_txnpage( fd_txncache_t *                    tc,
  ...
  if( FD_UNLIKELY( page_cnt==tc->txnpages_per_blockhash_max ) ) return NULL;
  if( FD_LIKELY( FD_ATOMIC_CAS( &pages[ page_cnt ], UINT_MAX, UINT_MAX-1UL )==UINT_MAX ) ) {
...
fd_txncache_insert_regular_txn_do( fd_txncache_t *                    tc,
  ...
  for(;;) {
    ...
    /* We have txn->key_sz-txnhash_offset bytes available for the key
       hash function.  However, we shouldn't feed more than what we
       actually store, which is 20 bytes. */
    ulong txnhash = fd_txncache_key_hash( txn->txnhash+txnhash_offset );
...
fd_txncache_insert_nonce_txn_do( fd_txncache_t *                          tc,
  ...
  for(;;) {
    uint head = nonce_slotcache->head;
    ...
    if( FD_LIKELY( FD_ATOMIC_CAS( &nonce_slotcache->head, head, nonce_txn_idx )==head ) ) {
      if( FD_LIKELY( head!=UINT_MAX ) ) {
        nonce_txns[ head ].slotcache_prev = nonce_txn_idx;
      }
  ...
  if( FD_UNLIKELY( nonce_blockcache && ( txn->flags&FD_TXNCACHE_FLAG_SNAPSHOT || tc->root_slots_cnt==0UL || nonce_blockcache->max_slot>=root_slots[ 0UL ] ) ) ) {
    for(;;) {
      uint max_slot     = nonce_blockcache->max_slot;
    ...
    for(;;) {
      ulong max_slot = tc->nonce_blockcache_max_slot;
...
fd_txncache_insert_regular_txn( fd_txncache_t *              tc,
  ...
  for(;;) {
    ...
    if( FD_LIKELY( txn->slot>=lowest_observed_slot ) ) break;
    if( FD_LIKELY( FD_ATOMIC_CAS( &tc->lowest_observed_slot, lowest_observed_slot, txn->slot )==lowest_observed_slot ) ) break;
    FD_SPIN_PAUSE();
  }
