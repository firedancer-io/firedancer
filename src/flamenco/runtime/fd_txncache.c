  ...
  for(;;) {
    ...
    if( FD_LIKELY( nonce_txns[ hole ].slotcache_next!=UINT_MAX ) ) {
      if( FD_UNLIKELY( nonce_txns[ nonce_txns[ hole ].slotcache_next ].slotcache_prev_is_head ) ) {
        FD_LOG_CRIT(( "invariant violation: nonce txn %u has next %u with prev %u being head", hole, nonce_txns[ hole ].slotcache_next, (uint)nonce_txns[ nonce_txns[ hole ].slotcache_next ].slotcache_prev ));
      }
...
fd_txncache_purge_slot( fd_txncache_t * tc,
  ...
  for( ulong i=0UL; i<tc->live_slots_max; /* manually advance i */ ) {
    ...
    if( FD_LIKELY( !( tc->root_slots_cnt==0UL || tc->nonce_blockcache_deactivated_slot_delta==ULONG_MAX ) ) ) {
      /* For slots loaded from an Agave snapshot, we insert all transactions as nonce transactions.  This makes the nonce slot population count inflated.  So we don't track these slots into the max count
       */
...
fd_txncache_query_batch( fd_txncache_t *             tc,
  ...
  for( ulong i=0UL; i<queries_cnt; i++ ) {
    ...
    if( FD_LIKELY( query->flags&FD_TXNCACHE_FLAG_REGULAR_TXN ) ) {
      if( FD_UNLIKELY( tc->root_slots_cnt==0UL || tc->nonce_blockcache_deactivated_slot_delta==ULONG_MAX ) ) {
        /* The nonce blockcache initialized from the snapshot hasn't
           been deactivated.  This implies that the regular transaction
           could be in the nonce txn cache.  So check there too, before
           we check the main txn cache. */
        ulong * root_slots     = fd_txncache_get_root_slots( tc );
        ulong   txnhash_offset = 0UL;
        fd_txncache_private_nonce_blockcache_t * nonce_blockcache;
        ...
        if( FD_LIKELY( result==FD_TXNCACHE_FIND_FOUND && ( tc->root_slots_cnt==0UL || nonce_blockcache->max_slot>=root_slots[ 0UL ] ) ) ) {
          ...
          /* The query into the nonce txn cache is gated behind the
             expiration check of the nonce blockcache entry, because if
             the nonce blockcache entry is expired, then the regular txn
             can't possibly be in the nonce txn cache.  In contrast, if
             it were an actual query for nonce transaction, then we have
             to query the nonce txn cache, regardless of whether the
             nonce blockcache entry is expired or not.  It's just that
             when the nonce blockcache entry is expired, we assume a
             txnhash offset of 0. */
          fd_txncache_private_nonce_txn_t * nonce_txn;
...
fd_txncache_set_nonce_txnhash_offset( fd_txncache_t * tc,
  ...
  if( FD_UNLIKELY( blockcache->txnhash_offset!=0U && blockcache->txnhash_offset!=txnhash_offset ) ) {
    FD_LOG_CRIT(( "non-zero txnhash_offset %u being reset to a different value %lu this indicates that the snapshot is busted or not being parsed correctly", blockcache->txnhash_offset, txnhash_offset ));
  }
