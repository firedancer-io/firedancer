...
struct blockcache {
  ...
  descends_set_t * descends; /* Each fork can descend from other forks in the txncache, and this bit vector contains one
                                ...
                                the bit at index F in descends[] os set. */
};
...
fd_txncache_new( void *                ljoin,
  ...
  if( FD_UNLIKELY( !_descends_footprint ) ) FD_LOG_CRIT(( "invalid descends_set params" ));
  ...
  for( ulong i=0UL; i<shmem->active_slots_max; i++ ) {
    ...
    ltc->blockcache_pool[ i ].descends = descends_set_join( descends_set_new( (uchar *)_blockcache_descends + i*_descends_footprint, max_active_slots ) );
