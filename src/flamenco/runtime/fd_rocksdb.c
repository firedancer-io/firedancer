...
fd_rocksdb_copy_over_txn_status_range( fd_rocksdb_t *    src,
  ...
  for ( ulong slot = start_slot; slot <= end_slot; ++slot ) {
    ...
    if( FD_LIKELY( block_entry && fd_blockstore_shreds_complete( blockstore, slot, NULL ) ) ) {
...
fd_rocksdb_import_block_blockstore( fd_rocksdb_t *    db,
  ...
  *((ulong *) &k[8]) = fd_ulong_bswap(start_idx);
  ...
  if( FD_LIKELY( block_map_entry && fd_blockstore_shreds_complete( blockstore, slot, NULL ) ) ) {
    size_t vallen = 0;
    char * err = NULL;
    char * res = rocksdb_get_cf(
      db->db,
  ...
  if( txnstatus && FD_LIKELY( block_map_entry && fd_blockstore_shreds_complete( blockstore, slot, NULL ) ) ) {
