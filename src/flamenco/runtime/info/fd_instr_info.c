...
{
  uchar is_program = 0;
  for( ushort j=0; j<txn_ctx->txn_descriptor->instr_cnt; j++ ) {
  ...
  uchar bpf_upgradeable_in_txn = 0;
  for( ushort j=0; j<txn_ctx->accounts_cnt; j++ ) {
    const fd_pubkey_t * acc = &txn_ctx->accounts[j];
    if( !memcmp( acc->uc, fd_solana_bpf_loader_upgradeable_program_id.key, sizeof(fd_pubkey_t) ) ) {
