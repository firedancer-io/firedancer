...
/* Mirrors Agave's slot_params.rs cost-limit test and the bank-level
   ...
   Firedancer stateless fd_slot_params API.  The baseline is fixed at
...
params_eq( fd_slot_params_t const * a, fd_slot_params_t const * b ) {
  return a->ns_per_slot                                              ==b->ns_per_slot
      ...
      && a->max_data_shreds_per_slot                                 ==b->max_data_shreds_per_slot
      && a->max_code_shreds_per_slot                                 ==b->max_code_shreds_per_slot
...
/* Asserts the regime effective at `slot` (built from `feat`/`es` over the
   fixed 400ms baseline) equals `exp`. */
...
test_out_of_order_range( void ) {
  ...
  FD_TEST( fd_slot_params_slot_range_duration_ns( &f, &es,  0UL, 160UL )==(uint128)32UL*ns400 + (uint128)128UL*ns200 ); /* [0,160), 64 interior */
  FD_TEST( fd_slot_params_slot_range_duration_ns( &f, &es, 40UL, 100UL )==(uint128)60UL*ns200 );                        /* [40,100) inside 200ms, 64 interior */
  ...
  FD_TEST( fd_slot_params_slot_range_duration_years( &f, &es, 32UL,  66UL )==(double)34UL/spy200 );                       /* [32,66), pure 200ms, 64 interior */
  FD_TEST( fd_slot_params_slot_range_duration_years( &f, &es,  0UL,  66UL )==(double)32UL/spy400 + (double)34UL/spy200 ); /* [0,66), crosses 32 and 64 */
  FD_TEST( fd_slot_params_slot_range_duration_years( &f, &es, 40UL, 100UL )==(double)60UL/spy200 );                       /* [40,100), 64 interior, single segment */

  /* Sanity: ranges that don't straddle slot 64 already agree. */
  FD_TEST( fd_slot_params_slot_range_duration_years( &f, &es, 32UL,  64UL )==(double)32UL/spy200 ); /* boundary at the range end */
  FD_TEST( fd_slot_params_slot_range_duration_years( &f, &es,  0UL,  32UL )==(double)32UL/spy400 ); /* pre-reduction segment */
}
...
test_range_integration( void ) {
  ...
  FD_TEST( fd_slot_params_slot_range_duration_ns   ( &f, &es, 5UL, 4UL )==(uint128)0 ); /* start>end */
  FD_TEST( fd_slot_params_slot_range_duration_years( &f, &es, 5UL, 5UL )==0.0         ); /* start>=end */

  /* No reduction: range reduces to a constant baseline. */
