...
struct fd_txncache_single_txn {
  uint  fork_next; /* Pointer to the next element on the same fork, used to prune transactions belonging to minority forks. */
  uint  fork_prev; /* Doubly linked for txnpage compaction. */
...
struct fd_txncache_blockcache_shmem {
  ...
  uint txn_head;         /* Pointer to the head of a doubly linked list of transactions that landed on this fork.
