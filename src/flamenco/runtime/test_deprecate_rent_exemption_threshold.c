...
add_bank_stake_delegation_entry( test_env_t *        env,
                                 fd_pubkey_t const * stake_account,
                                 fd_pubkey_t const * vote_account ) {
  fd_stake_delegations_delta_t * stake_delegations_delta = fd_type_pun( (uchar *)env->banks->data + env->banks->data->stake_delegations_delta_offset );
...
advance_to_slot( test_env_t * env,
                 ulong        target_slot ) {
  ulong current_slot = env->bank->data->fields.slot;
  int rent_modified = 0;
