...
static int
sysvar_obj_recover( fd_sysvar_cache_t *     cache,
  ...
  if( FD_UNLIKELY( pos->decode_footprint( &ctx, &obj_sz )!=FD_BINCODE_SUCCESS ) ) {
    if( log_fails ) {
      FD_LOG_DEBUG(( "Failed to decode sysvar %s with data_sz=%lu: decode failed",
                    FD_BASE58_ENC_32_ALLOCA( key ), data_sz ));
    }
    return EINVAL;
  }
  if( FD_UNLIKELY( obj_sz > pos->obj_max ) ) {
    if( log_fails ) {
      FD_LOG_WARNING(( "Failed to restore sysvar %s: obj_sz=%lu exceeds max=%u",
                      FD_BASE58_ENC_32_ALLOCA( key ), obj_sz, pos->obj_max ));
    }
    return ENOMEM;
  }
  ...
  FD_LOG_DEBUG(( "Restored sysvar %s (data_sz=%lu obj_sz=%lu)",
                 FD_BASE58_ENC_32_ALLOCA( key ), data_sz, obj_sz ));
  return 0;
}
...
sysvar_data_fill( fd_sysvar_cache_t *  cache,
  ...
  if( FD_UNLIKELY( rec->vt->get_lamports( rec )==0 ) ) return 0;
  ...
  if( FD_UNLIKELY( data_sz > pos->data_max ) ) {
    if( log_fails ) {
      FD_LOG_WARNING(( "Failed to restore sysvar %s: data_sz=%lu exceeds max=%u",
                      FD_BASE58_ENC_32_ALLOCA( key ), data_sz, pos->data_max ));
    }
    return ENOMEM;
  }
  ...
  return sysvar_obj_recover( cache, desc, pos, key, data, data_sz, log_fails );
}
...
fd_sysvar_account_update( fd_exec_slot_ctx_t * slot_ctx,
  ...
  if( FD_UNLIKELY( __builtin_usubl_overflow( lamports_after, lamports_before, &lamports_minted ) ) ) {
    FD_LOG_CRIT(( "fd_sysvar_account_update: lamports overflowed: address=%s lamports_before=%lu lamports_after=%lu",
                  FD_BASE58_ENC_32_ALLOCA( address ), lamports_before, lamports_after ));
  }
...
fd_sysvar_cache_data_modify_commit(
  ...
  /* Recover object cache entry from data cache entry */
  int err = sysvar_obj_recover( cache, desc, pos, key, data, sz, 1 );
