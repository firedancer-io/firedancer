...
/* fd_sysvar_desc_write_{lock,unlock} {acquires,releases} a write
   lock on a sysvar cache entry. */

static inline void
fd_sysvar_desc_write_lock( fd_sysvar_desc_t * desc ) {
  for(;;) {
...
static inline void
fd_sysvar_cache_desc_write_unlock( fd_sysvar_desc_t * desc ) {
  for(;;) {
    uint const state_old = FD_VOLATILE_CONST( desc->flags );
    if( FD_UNLIKELY( !fd_sysvar_cache_flags_writer_cnt( state_old ) ) ) {
