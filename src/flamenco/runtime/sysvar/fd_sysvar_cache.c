...
fd_sysvar_cache_data_modify_prepare(
  ...
  ulong _dummy[1];
  if( !opt_sz     ) opt_sz     = _dummy;
  if( !opt_sz_max ) opt_sz_max = _dummy;
  ...
  fd_sysvar_desc_t *      desc = &cache->desc      [ entry->desc_idx ];
  fd_sysvar_pos_t const * pos  = &fd_sysvar_pos_tbl[ entry->desc_idx ];
  desc->flags &= ~FD_SYSVAR_FLAG_VALID;
  desc->flags |= FD_SYSVAR_FLAG_WRITE_LOCK;
  *opt_sz      = desc->data_sz;
  *opt_sz_max  = fd_sysvar_pos_tbl[ entry->desc_idx ].data_max;
  return (uchar *)cache + pos->data_off;
}
...
#define SIMPLE_SYSVAR_READ( name, name2, typet )                       \
  typet *                                                              \
  fd_sysvar_##name##_read( fd_sysvar_cache_t const * cache,            \
                           typet *                   out ) {           \
...
fd_sysvar_slot_hashes_join(
  ...
  fd_sysvar_cache_t * cache = fd_bank_sysvar_cache_modify( slot_ctx->bank );
  if( FD_UNLIKELY( !fd_sysvar_slot_hashes_is_valid( cache ) ) ) return NULL;
...
fd_sysvar_slot_history_join(
  ...
  fd_sysvar_cache_t * cache = fd_bank_sysvar_cache_modify( slot_ctx->bank );
  if( FD_UNLIKELY( !fd_sysvar_slot_history_is_valid( cache ) ) ) return NULL;
  FD_VOLATILE( cache->desc[ idx ].flags ) = FD_SYSVAR_FLAG_WRITE_LOCK; /* FIXME consider CAS */
  cache->desc[ idx ].data_sz = 0U;
  return (void *)cache->obj_slot_history;
}
...
fd_sysvar_stake_history_join(
  ...
  fd_sysvar_cache_t * cache = fd_bank_sysvar_cache_modify( slot_ctx->bank );
  if( FD_UNLIKELY( !fd_sysvar_stake_history_is_valid( cache ) ) ) return NULL;
  FD_VOLATILE( cache->desc[ idx ].flags ) = FD_SYSVAR_FLAG_WRITE_LOCK; /* FIXME consider CAS */
  cache->desc[ idx ].data_sz = 0U;
  return (void *)cache->obj_stake_history;
}
