...
test_sysvar_stake_history_update( fd_wksp_t * wksp ) {
  ...
  FD_TEST( fd_sysvar_stake_history_is_valid( env->sysvar_cache )==0 );
  ...
  fd_sysvar_epoch_schedule_write( env->slot_ctx, &schedule );
  ...
  fd_sysvar_stake_history_update( env->slot_ctx, &entry0 );
  FD_TEST( fd_sysvar_stake_history_is_valid( env->sysvar_cache )==1 );
