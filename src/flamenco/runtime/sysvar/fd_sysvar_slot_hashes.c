...
fd_sysvar_slot_hashes_write( fd_bank_t *               bank,
  ...
  fd_sysvar_account_update( bank, funk, xid, capture_ctx, &fd_sysvar_slot_hashes_id, enc, FD_SYSVAR_SLOT_HASHES_BINCODE_SZ );
}
...
fd_sysvar_slot_hashes_update( fd_bank_t *               bank,
                              ...
                              fd_funk_txn_xid_t const * xid,
                              fd_capture_ctx_t *        capture_ctx ) {
  uchar __attribute__((aligned(alignof(fd_slot_hashes_global_t)))) slot_hashes_mem[FD_SYSVAR_SLOT_HASHES_FOOTPRINT];
