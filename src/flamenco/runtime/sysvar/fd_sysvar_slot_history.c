...
fd_sysvar_slot_history_update( fd_exec_slot_ctx_t * slot_ctx, fd_spad_t * runtime_spad ) {
  ...
  fd_bincode_decode_ctx_t ctx = {
    .data    = fd_txn_account_get_acc_data( rec ),
    .dataend = fd_txn_account_get_acc_data( rec ) + fd_txn_account_get_data_len( rec )
  };
  ...
  ulong sz = fd_ulong_max( fd_txn_account_get_data_len( rec ), slot_history_min_account_size );
  ...
  fd_bincode_encode_ctx_t e_ctx = {
    .data    = fd_txn_account_get_acc_data_mut( rec ),
    .dataend = fd_txn_account_get_acc_data_mut( rec )+sz,
  };
  ...
  fd_txn_account_set_lamports( rec, fd_rent_exempt_minimum_balance( rent, sz ) );
