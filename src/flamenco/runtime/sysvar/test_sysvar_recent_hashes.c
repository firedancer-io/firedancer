...
test_sysvar_recent_hashes_init( fd_wksp_t * wksp ) {
  ...
  FD_TEST( fd_sysvar_recent_hashes_is_valid( env->sysvar_cache )==0 );
  ...
  fd_sysvar_rent_write( env->slot_ctx, &rent );

  /* Create an empty recent hashes sysvar */
  fd_sysvar_recent_hashes_init( env->slot_ctx );
  FD_TEST( fd_sysvar_recent_hashes_is_valid( env->sysvar_cache )==1 );
  {
    ulong sz = 0UL;
