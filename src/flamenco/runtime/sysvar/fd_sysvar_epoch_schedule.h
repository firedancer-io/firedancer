...
/* fd_epoch_schedule_derive populates an epoch schedule from a
   user-provided epoch length and leader-schedule slot offset.  Returns
   schedule on success, NULL if epoch_len is outside [FD_EPOCH_LEN_MIN,
   FD_EPOCH_LEN_MAX] (logs a warning). */
