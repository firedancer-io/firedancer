...
test_sysvar_cache_empty( void ) {
  ...
  /* Test is_valid */
  FD_TEST( !fd_sysvar_clock_is_valid            ( cache1 ) );
  FD_TEST( !fd_sysvar_epoch_rewards_is_valid    ( cache1 ) );
  FD_TEST( !fd_sysvar_epoch_schedule_is_valid   ( cache1 ) );
  FD_TEST( !fd_sysvar_last_restart_slot_is_valid( cache1 ) );
  FD_TEST( !fd_sysvar_recent_hashes_is_valid    ( cache1 ) );
  FD_TEST( !fd_sysvar_rent_is_valid             ( cache1 ) );
  FD_TEST( !fd_sysvar_slot_hashes_is_valid      ( cache1 ) );
  FD_TEST( !fd_sysvar_slot_history_is_valid     ( cache1 ) );
  FD_TEST( !fd_sysvar_stake_history_is_valid    ( cache1 ) );
  ...
  FD_TEST( !fd_sysvar_clock_read( cache1, &clock ) );
  fd_sysvar_epoch_rewards_t epoch_rewards;
  FD_TEST( !fd_sysvar_epoch_rewards_read( cache1, &epoch_rewards ) );
  fd_epoch_schedule_t epoch_schedule;
  FD_TEST( !fd_sysvar_epoch_schedule_read( cache1, &epoch_schedule ) );
  fd_sol_sysvar_last_restart_slot_t last_restart_slot;
  FD_TEST( !fd_sysvar_last_restart_slot_read( cache1, &last_restart_slot ) );
  fd_rent_t rent;
  FD_TEST( !fd_sysvar_rent_read( cache1, &rent ) );

  /* Test sysvar join accessors */
  FD_TEST( !fd_sysvar_slot_history_join_const ( cache1 ) );
  FD_TEST( !fd_sysvar_slot_hashes_join_const  ( cache1 ) );
  FD_TEST( !fd_sysvar_stake_history_join_const( cache1 ) );
...
test_sysvar_cache_read( void ) {
  ...
  FD_TEST( fd_sysvar_clock_is_valid( cache )==0 );
  ...
  FD_TEST( fd_sysvar_clock_is_valid( cache )==1 );
  ...
  fd_sol_sysvar_clock_t clock_copy = fd_sysvar_clock_read_nofail( cache );
...
static void
test_sysvar_cache_modify_prepare( void ) {
  fd_exec_slot_ctx_t slot_ctx = {0};
  fd_bank_t bank = {0};
  slot_ctx.bank = &bank;
  FD_TEST( fd_sysvar_cache_join( fd_sysvar_cache_new( bank.sysvar_cache ) ) );
  fd_sysvar_cache_t * cache = fd_bank_sysvar_cache_modify( slot_ctx.bank );
  for( ulong i=0UL; i<FD_SYSVAR_CACHE_ENTRY_CNT; i++ ) {
    fd_pubkey_t const *     key  = &fd_sysvar_key_tbl[ i ];
    fd_sysvar_pos_t const * pos  = &fd_sysvar_pos_tbl[ i ];
    fd_sysvar_desc_t *      desc = &cache->desc      [ i ];
    void *                  data = (uchar *)cache + pos->data_off;

    desc->data_sz = 2U;
    desc->flags   = FD_SYSVAR_FLAG_VALID;
    FD_TEST( fd_sysvar_cache_data_modify_prepare( &slot_ctx, key, NULL, NULL )==data );
    FD_TEST( desc->flags==FD_SYSVAR_FLAG_WRITE_LOCK );

    desc->data_sz = 2U;
    desc->flags   = FD_SYSVAR_FLAG_VALID;
    ulong sz = 234;
    FD_TEST( fd_sysvar_cache_data_modify_prepare( &slot_ctx, key, &sz, NULL )==data );
    FD_TEST( desc->flags==FD_SYSVAR_FLAG_WRITE_LOCK );
    FD_TEST( sz==2UL );

    desc->flags   = FD_SYSVAR_FLAG_VALID;
    ulong sz_max = 0;
    FD_TEST( fd_sysvar_cache_data_modify_prepare( &slot_ctx, key, NULL, &sz_max )==data );
    FD_TEST( desc->flags==FD_SYSVAR_FLAG_WRITE_LOCK );
    FD_TEST( sz_max==pos->data_max );
  }
}
...
test_sysvar_cache( void ) {
  ...
  test_sysvar_cache_modify_prepare();
}
