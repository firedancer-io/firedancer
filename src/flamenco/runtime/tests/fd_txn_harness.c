...
fd_solfuzz_pb_txn_ctx_create( fd_solfuzz_runner_t *              runner,
  ...
  /* Setup vote states dummy account */
  fd_vote_states_t * vote_states_prev = fd_vote_states_join( fd_vote_states_new( fd_bank_vote_states_prev_modify( runner->bank ), FD_RUNTIME_MAX_WRITABLE_ACCOUNTS_PER_TRANSACTION, 999UL ) );
  if( FD_UNLIKELY( !vote_states_prev ) ) {
    return NULL;
  }

  /* Setup vote states dummy account */
  fd_vote_states_t * vote_states_prev_prev = fd_vote_states_join( fd_vote_states_new( fd_bank_vote_states_prev_prev_modify( runner->bank ), FD_RUNTIME_MAX_WRITABLE_ACCOUNTS_PER_TRANSACTION, 999UL ) );
