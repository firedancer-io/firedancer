...
find_reward_partition( fd_stake_rewards_t *      stake_rewards,
  ...
  for( uint p=0U; p<num_partitions; p++ ) {
    for( fd_stake_rewards_iter_init( stake_rewards, fork_idx, p );
      ...
      ulong       lamports;
      ulong       credits;
      fd_stake_rewards_iter_ele( stake_rewards, &cur, &lamports, &credits );
...
/* Rewrites the stake account's credits_observed, keeping the stake
   delegations index in sync with it. */
...
patch_stake_credits_observed( fd_svm_mini_t *     mini,
  ...
  FD_TEST( ss_orig && ss_orig->stake_type==FD_STAKE_STATE_STAKE );
  fd_stake_state_t ss_new = *ss_orig;
  ss_new.stake.stake.credits_observed = credits_observed;
  uchar owner_copy[32]; memcpy( owner_copy, acc.owner, 32 );
  ulong lamports_copy = acc.lamports;
  int   exec_copy     = acc.executable;
  fd_accdb_unread_one( mini->runtime->accdb, &acc );
...
test_credit_rewind_force_update( fd_svm_mini_t * mini ) {
  ...
  params->slots_per_epoch    = TEST_SLOTS_PER_EPOCH;
  params->root_slot          = TEST_ROOT_SLOT;
  /* Two validators: the second one earns points, so the global points
     denominator stays non-zero and the reward calculation is not
     skipped.  The first one is the rewind case under test. */
  params->mock_validator_cnt = 2UL;
  ulong root_idx = fd_svm_mini_reset( mini, params );
  ...
  fd_pubkey_t identity_key, vote_key, stake_key;
  mock_validator_keys_idx( params->hash_seed, 0UL, &identity_key, &vote_key, &stake_key );
  fd_pubkey_t other_identity_key, other_vote_key, other_stake_key;
  mock_validator_keys_idx( params->hash_seed, 1UL, &other_identity_key, &other_vote_key, &other_stake_key );

  patch_vote_account( mini, root_idx, &vote_key,       0, 0UL, 5UL, 0UL );
  patch_vote_account( mini, root_idx, &other_vote_key, 0, 0UL, 5UL, 0UL );

  patch_stake_credits_observed( mini, root_idx, &stake_key, &vote_key, 10UL );
...
/* A Tower epoch whose global points denominator comes out zero is
   skipped in its entirety: no stake account is touched at the epoch
   boundary, and recalculating the rewards from the EpochRewards sysvar
   -- what a bank restored from a snapshot taken during the distribution
   interval does -- must not touch one either.  Otherwise the restored
   bank would force credits_observed forward on stake accounts the nodes
   that stayed up never rewrote, and diverge in the bank hash. */

static void
test_zero_points_skips_rewards( fd_svm_mini_t * mini ) {
  fd_svm_mini_params_t params[1];
  fd_svm_mini_params_default( params );
  params->slots_per_epoch    = TEST_SLOTS_PER_EPOCH;
  params->root_slot          = TEST_ROOT_SLOT;
  ...
  /* The only staker has observed more credits than its vote account
     holds, so it earns no points and the global denominator is zero. */
  patch_vote_account( mini, root_idx, &vote_key, 0, 0UL, 5UL, 0UL );
  patch_stake_credits_observed( mini, root_idx, &stake_key, &vote_key, 10UL );
  ...
  ulong              stake_lam_before = read_lamports( mini, root_fk, &stake_key );
  fd_stake_t         stake_before     = read_stake   ( mini, root_fk, &stake_key );
  FD_TEST( stake_before.credits_observed==10UL );

  ulong              epoch_idx  = fd_svm_mini_attach_child( mini, root_idx, TEST_EPOCH_BOUNDARY );
  fd_bank_t *        epoch_bank = fd_svm_mini_bank( mini, epoch_idx );
  fd_accdb_fork_id_t epoch_fk   = fd_svm_mini_fork_id( mini, epoch_idx );

  /* The sysvar is active for the distribution interval, but records a
     zero rewards budget and a zero points denominator. */
  fd_sysvar_epoch_rewards_t er[1];
  FD_TEST( fd_sysvar_epoch_rewards_read( mini->runtime->accdb, epoch_fk, er ) );
  FD_TEST( er->active );
  FD_TEST( er->total_rewards   ==0UL );
  ...
  FD_TEST( fd_stake_rewards_total_rewards(
      stake_rewards, epoch_bank->stake_rewards_fork_id )==0UL );
  FD_TEST( find_reward_partition(
      stake_rewards, epoch_bank->stake_rewards_fork_id, &stake_key, partition_cnt )==UINT_MAX );
  ...
  FD_LOG_NOTICE(( "test_zero_points_skips_rewards: PASSED" ));
}
...
test_hash_rewards_into_partitions( void ) {
  ...
  uchar fork_idx = fd_stake_rewards_init( sr, &blockhash, 100UL, num_partitions, 0UL );
  ...
  for( uint p=0U; p<num_partitions; p++ ) {
    for( fd_stake_rewards_iter_init( sr, fork_idx, p );
         ...
         fd_stake_rewards_iter_next( sr ) ) {
      fd_pubkey_t pubkey; ulong lamports, credits_observed;
      fd_stake_rewards_iter_ele( sr, &pubkey, &lamports, &credits_observed );
      total_count++;
...
/* Drain one partition of a sealed fork, checking every entry against
   the i -> (lamports=i+1, credits=i) encoding used by the tests below
   and marking it in seen.  Returns the number of entries. */

static ulong
drain_partition( fd_stake_rewards_t * sr,
                 uchar                fork_idx,
                 uint                 partition_idx,
                 ulong                reward_cnt,
                 uchar *              seen,
                 ulong *              sum ) {
  ulong cnt = 0UL;
  for( fd_stake_rewards_iter_init( sr, fork_idx, partition_idx );
       !fd_stake_rewards_iter_done( sr );
       fd_stake_rewards_iter_next( sr ) ) {
    fd_pubkey_t pubkey; ulong lamports, credits_observed;
    fd_stake_rewards_iter_ele( sr, &pubkey, &lamports, &credits_observed );
...
static void
insert_encoded( fd_stake_rewards_t * sr,
                uchar                fork_idx,
                ulong                reward_cnt ) {
...
/* When the rewards of an epoch do not all fit in the staging buffer,
   every full buffer is flushed to the fork's overflow area on disk and
   the remainder stays in an in-memory buffer.  Reading the partitions
   back must yield every reward exactly once, in the partition it hashes
   to, with nothing recomputed. */

static void
test_hash_rewards_overflow( void ) {
  ulong const capacity       = 64UL;
  ulong const reward_cnt     = 500UL; /* seven flushes and a partial image */
  uint  const num_partitions = 8U;
  ...
  /* Reference partitioning: the same inserts with everything resident. */
  uchar ref_partition[ 500 ];
  {
    ulong ref_footprint = fd_stake_rewards_footprint( reward_cnt, 1UL );
    void * ref_mem = aligned_alloc( fd_stake_rewards_align(), ref_footprint );
    FD_TEST( ref_mem );
    fd_stake_rewards_t * ref = fd_stake_rewards_join( fd_stake_rewards_new( ref_mem, reward_cnt, 1UL ) );
    uchar ref_fork = fd_stake_rewards_init( ref, &blockhash, 100UL, num_partitions, reward_cnt );
    insert_encoded( ref, ref_fork, reward_cnt );
    fd_stake_rewards_fini( ref, ref_fork );
    for( uint p=0U; p<num_partitions; p++ ) {
      for( fd_stake_rewards_iter_init( ref, ref_fork, p );
           !fd_stake_rewards_iter_done( ref );
           fd_stake_rewards_iter_next( ref ) ) {
        fd_pubkey_t pubkey; ulong lamports, credits_observed;
        fd_stake_rewards_iter_ele( ref, &pubkey, &lamports, &credits_observed );
        ref_partition[ FD_LOAD( ulong, pubkey.key ) ] = (uchar)p;
      }
    }
    free( ref_mem );
  }

  uchar fork_idx = fd_stake_rewards_init( sr, &blockhash, 100UL, num_partitions, reward_cnt );
  ...
  for( uint p=0U; p<num_partitions; p++ ) {
    for( fd_stake_rewards_iter_init( sr, fork_idx, p );
      ...
      ulong i = FD_LOAD( ulong, pubkey.key );
      FD_TEST( i<reward_cnt );
      FD_TEST( ref_partition[ i ]==p );
    }
    total_count += drain_partition( sr, fork_idx, p, reward_cnt, seen, &total_sum );
  }
  FD_TEST( total_count==reward_cnt );
  FD_TEST( total_sum==reward_cnt*(reward_cnt+1UL)/2UL );

  /* A second pass reads the same saved partitions again. */
  memset( seen, 0, sizeof(seen) );
  total_count = 0UL;
  total_sum   = 0UL;
  for( uint p=0U; p<num_partitions; p++ ) total_count += drain_partition( sr, fork_idx, p, reward_cnt, seen, &total_sum );
  FD_TEST( total_count==reward_cnt );

  /* Purging returns the extents; the next fork can reuse them. */
  fd_stake_rewards_purge( sr, fork_idx );
  fork_idx = fd_stake_rewards_init( sr, &blockhash, 200UL, 1U, reward_cnt );
  insert_encoded( sr, fork_idx, reward_cnt );
  fd_stake_rewards_fini( sr, fork_idx );
  memset( seen, 0, sizeof(seen) );
  total_sum = 0UL;
  FD_TEST( drain_partition( sr, fork_idx, 0U, reward_cnt, seen, &total_sum )==reward_cnt );

  free( mem );

  FD_LOG_NOTICE(( "test_hash_rewards_overflow: PASSED" ));
}

/* An exact multiple of the capacity leaves a full image behind the
   flushes (the buffer is only flushed to make room for the next
   insert), and a fork that announces more rewards than it inserts never
   touches the disk. */

static void
test_hash_rewards_overflow_edges( void ) {
  ulong const capacity       = 16UL;
  uint  const num_partitions = 4U;

  ulong footprint = fd_stake_rewards_footprint( capacity, 2UL );
  void * mem = aligned_alloc( fd_stake_rewards_align(), footprint );
  FD_TEST( mem );

  fd_stake_rewards_t * sr = fd_stake_rewards_join(
      fd_stake_rewards_new( mem, capacity, 2UL ) );
  FD_TEST( sr );

  fd_hash_t blockhash = {{ 0 }};
  memset( blockhash.hash, 0x5C, sizeof(blockhash.hash) );

  ulong reward_cnt = 3UL*capacity;
  uchar fork_idx = fd_stake_rewards_init( sr, &blockhash, 100UL, num_partitions, reward_cnt );
  insert_encoded( sr, fork_idx, reward_cnt );
  fd_stake_rewards_fini( sr, fork_idx );
  ...
  /* Over-announced, under-inserted: uses only in-memory storage. */
  uchar small_fork = fd_stake_rewards_init( sr, &blockhash, 100UL, num_partitions, 10UL*capacity );
  insert_encoded( sr, small_fork, capacity/2UL );
  fd_stake_rewards_fini( sr, small_fork );
  memset( seen, 0, sizeof(seen) );
  total_count = 0UL;
  for( uint p=0U; p<num_partitions; p++ ) total_count += drain_partition( sr, small_fork, p, capacity/2UL, seen, &total_sum );
  FD_TEST( total_count==capacity/2UL );
...
test_hash_rewards_spill( void ) {
  ulong const capacity       = 32UL;
  ulong const reward_cnt     = 20UL;
  uint  const num_partitions = 3U;
  ulong const max_forks      = 6UL;

  ulong footprint = fd_stake_rewards_footprint( capacity, max_forks );
  void * mem = aligned_alloc( fd_stake_rewards_align(), footprint );
  FD_TEST( mem );

  fd_stake_rewards_t * sr = fd_stake_rewards_join(
      fd_stake_rewards_new( mem, capacity, max_forks ) );
  FD_TEST( sr );

  fd_hash_t blockhash = {{ 0 }};
  uchar     fork[ 4 ];
  for( ulong f=0UL; f<3UL; f++ ) {
    memset( blockhash.hash, (int)(0x10UL+f), sizeof(blockhash.hash) );
    fork[f] = fd_stake_rewards_init( sr, &blockhash, 100UL+f, num_partitions, reward_cnt );
    insert_encoded( sr, fork[f], reward_cnt );
    fd_stake_rewards_fini( sr, fork[f] );
  }

  for( ulong f=0UL; f<3UL; f++ ) {
    uchar seen[ 20 ] = { 0 };
    ulong cnt = 0UL, sum = 0UL;
    for( uint p=0U; p<num_partitions; p++ ) cnt += drain_partition( sr, fork[f], p, reward_cnt, seen, &sum );
  ...
  fork[3] = fd_stake_rewards_init( sr, &blockhash, 104UL, num_partitions, reward_cnt );
  insert_encoded( sr, fork[3], reward_cnt );
  fd_stake_rewards_fini( sr, fork[3] );
  ...
  for( ulong f=0UL; f<max_forks; f++ ) {
    memset( blockhash.hash, (int)(0x30UL+f), sizeof(blockhash.hash) );
    all[f] = fd_stake_rewards_init( sr, &blockhash, 200UL+f, num_partitions, reward_cnt );
    insert_encoded( sr, all[f], reward_cnt );
    fd_stake_rewards_fini( sr, all[f] );
  }
  for( ulong f=0UL; f<max_forks; f++ ) {
    uchar seen[ 20 ] = { 0 };
    ulong cnt = 0UL, sum = 0UL;
  ...
  FD_LOG_NOTICE(( "test_hash_rewards_spill: PASSED" ));
}

/* A fork with both an overflow area and a spilled image reads each
   partition from the two disk runs. */

static void
test_hash_rewards_overflow_and_spill( void ) {
  ulong const capacity       = 32UL;
  ulong const reward_cnt     = 100UL;
  uint  const num_partitions = 5U;

  ulong footprint = fd_stake_rewards_footprint( capacity, 4UL );
  void * mem = aligned_alloc( fd_stake_rewards_align(), footprint );
  FD_TEST( mem );

  fd_stake_rewards_t * sr = fd_stake_rewards_join(
      fd_stake_rewards_new( mem, capacity, 4UL ) );
  FD_TEST( sr );

  /* Two overflowing forks, then a small third one whose seal evicts the
     first. */
  fd_hash_t blockhash = {{ 0 }};
  uchar     fork[ 3 ];
  ulong     cnts[ 3 ] = { reward_cnt, reward_cnt, 10UL };
  for( ulong f=0UL; f<3UL; f++ ) {
    memset( blockhash.hash, (int)(0x40UL+f), sizeof(blockhash.hash) );
    fork[f] = fd_stake_rewards_init( sr, &blockhash, 100UL+f, num_partitions, cnts[f] );
  ...
  for( ulong f=0UL; f<3UL; f++ ) {
    uchar seen[ 100 ] = { 0 };
    ulong cnt = 0UL, sum = 0UL;
    for( uint p=0U; p<num_partitions; p++ ) cnt += drain_partition( sr, fork[f], p, cnts[f], seen, &sum );
    FD_TEST( cnt==cnts[f] );
    FD_TEST( sum==cnts[f]*(cnts[f]+1UL)/2UL );
  }

  free( mem );

  FD_LOG_NOTICE(( "test_hash_rewards_overflow_and_spill: PASSED" ));
}
...
test_hash_rewards_pubkeys_across_forks( void ) {
  ...
  for( ulong fork=0UL; fork<max_forks; fork++ ) {
    ...
    fork_idx[fork] = fd_stake_rewards_init( sr, &blockhash, 100UL + fork, 2U, 0UL );
  ...
  fd_stake_rewards_fini( sr, fork_idx[max_forks-1UL] ); /* earlier forks sealed implicitly by the next init */
  ...
  fd_hash_t blockhash = {{ 0 }};
  uchar next_epoch_fork_idx = fd_stake_rewards_init( sr, &blockhash, 200UL, 1U, 0UL );
  for( ulong i=0UL; i<max_accs; i++ ) {
    fd_pubkey_t pubkey = {{ 0 }};
  ...
  blockhash.ul[0] = 1UL;
  uchar next_epoch_second_fork_idx = fd_stake_rewards_init( sr, &blockhash, 201UL, 1U, 0UL );
  for( ulong i=0UL; i<max_accs; i++ ) {
    fd_pubkey_t pubkey = {{ 0 }};
...
test_hash_rewards_purge_first_fork( void ) {
  ...
  uchar first_fork_idx = fd_stake_rewards_init( sr, &blockhash, 100UL, 1U, 0UL );
  ...
  blockhash.ul[0] = 1UL;
  uchar second_fork_idx = fd_stake_rewards_init( sr, &blockhash, 101UL, 1U, 0UL );
  pubkey.ul[0] = 2UL;
  ...
  fd_stake_rewards_iter_ele( sr, &actual_pubkey, &lamports, &credits_observed );
...
test_update_reward_history_in_partition_empty( fd_svm_mini_t * mini ) {
  ...
  ulong starting_block_height = child_bank->f.block_height;
  uchar empty_fork_idx = init_stake_rewards( child_bank, &blockhash, starting_block_height, 1U );
  fd_stake_rewards_fini( fd_bank_stake_rewards_modify( child_bank ), empty_fork_idx );
...
main( int     argc,
  ...
  test_zero_points_skips_rewards( mini );
  ...
  test_hash_rewards_overflow();
  test_hash_rewards_overflow_edges();
  test_hash_rewards_spill();
  test_hash_rewards_overflow_and_spill();
