...
dump_account_state( fd_txn_account_t const *    txn_account,
    ...
    fd_memcpy(output_account->data->bytes, fd_txn_account_get_acc_data( txn_account ), fd_txn_account_get_data_len( txn_account ) );
...
dump_lut_account_and_contained_accounts(  fd_exec_slot_ctx_t const *     slot_ctx,
  ...
  fd_pubkey_t * lookup_addrs = (fd_pubkey_t *)&fd_txn_account_get_acc_data( alut_account )[FD_LOOKUP_TABLE_META_SIZE];
...
create_txn_context_protobuf_from_txn( fd_exec_test_txn_context_t * txn_context_msg,
  ...
  */
  ...
  for( ulong i = 0; i < txn_descriptor->addr_table_lookup_cnt; ++i ) {
    ...
    fd_acct_addr_t * lookup_addrs  = (fd_acct_addr_t *)&fd_txn_account_get_acc_data( txn_account )[FD_LOOKUP_TABLE_META_SIZE];
