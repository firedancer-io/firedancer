...
FD_SPAD_FRAME_BEGIN( vm->instr_ctx->txn_ctx->spad ) {
  ...
  fd_txn_t const * txn_descriptor = TXN( &vm->instr_ctx->txn_ctx->txn );
  memcpy( signature, (uchar const *)vm->instr_ctx->txn_ctx->txn.payload + txn_descriptor->signature_off, sizeof(fd_ed25519_sig_t) );
  char encoded_signature[FD_BASE58_ENCODED_64_SZ];
...
FD_SPAD_FRAME_BEGIN( txn_ctx->spad ) {
  ...
  fd_ed25519_sig_t signature;
  fd_txn_t const * txn_descriptor = TXN( &txn_ctx->txn );
  memcpy( signature, (uchar const *)txn_ctx->txn.payload + txn_descriptor->signature_off, sizeof(fd_ed25519_sig_t) );
  char encoded_signature[FD_BASE58_ENCODED_64_SZ];
  fd_base58_encode_64( signature, NULL, encoded_signature );
