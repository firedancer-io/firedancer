...
static uchar
dump_account_if_not_already_dumped( fd_accdb_t *                accdb,
...
static void
dump_executable_account_if_exists( fd_accdb_t *                      accdb,
                                   fd_accdb_fork_id_t                fork_id,
  ...
  dump_account_if_not_already_dumped( accdb, fork_id, programdata_acc, spad, out_account_states, out_account_states_count, NULL );
}

static void
dump_sanitized_transaction( fd_accdb_t *                           accdb,
                            fd_accdb_fork_id_t                     fork_id,
...
static void
add_account_and_programdata_to_dumped_accounts( fd_accdb_t *                accdb,
                                                fd_accdb_fork_id_t          fork_id,
  ...
  /* Add the current account to the dumped accounts set. We can save
     ...
     ourselves a call to accdb. */
  uchar ret = add_account_to_dumped_accounts( dumped_accounts, pubkey );
  if( ret==0 ) return;

  /* Read the account from accdb to see if its a program account and if
     it needs to be dumped. */
  fd_acc_t program_account = fd_accdb_read_one( accdb, fork_id, pubkey->uc );
  if( FD_UNLIKELY( !program_account.lamports ) ) {
    fd_accdb_unread_one( accdb, &program_account );
    return;
  }

  /* Return if its not owned by the v3 loader */
  if( FD_LIKELY( !fd_pubkey_eq( (fd_pubkey_t*)program_account.owner, &fd_solana_bpf_loader_upgradeable_program_id ) ) ) {
    fd_accdb_unread_one( accdb, &program_account );
    return;
  }
...
static void
add_lut_accounts_to_dumped_accounts( fd_accdb_t *                accdb,
                                     fd_accdb_fork_id_t          fork_id,
                                     fd_dump_account_key_set_t * dumped_accounts,
                                     fd_pubkey_t const *         pubkey ) {
  ...
  if( FD_UNLIKELY( !lut_account.lamports ) ) {
    fd_accdb_unread_one( accdb, &lut_account );
    return;
  }

  /* Decode the ALUT account and dump all pubkeys within the lookup
     table. */
  if( lut_account.data_len<FD_LOOKUP_TABLE_META_SIZE || (lut_account.data_len&0x1fUL) ) {
    fd_accdb_unread_one( accdb, &lut_account );
    return;
  }

  /* Copy the addresses out and release before nested acquires. */
  fd_pubkey_t lookup_addrs[ 256 ]; /* LOOKUP_TABLE_MAX_ADDRESSES */
  ulong       lookup_addrs_cnt = fd_ulong_min( (lut_account.data_len-FD_LOOKUP_TABLE_META_SIZE)>>5UL, 256UL );
  fd_memcpy( lookup_addrs, lut_account.data+FD_LOOKUP_TABLE_META_SIZE, lookup_addrs_cnt*sizeof(fd_pubkey_t) );
  fd_accdb_unread_one( accdb, &lut_account );

  for( ulong i=0UL; i<lookup_addrs_cnt; i++ ) {
    fd_pubkey_t const * referenced_pubkey = &lookup_addrs[i];
    add_account_and_programdata_to_dumped_accounts( accdb, fork_id, dumped_accounts, referenced_pubkey );
  }
}
...
create_block_context_protobuf_from_block( fd_block_dump_ctx_t * dump_ctx,
  ...
  /* We should use the bank fields from the parent slot in order to
     capture the block context from before the current block was
     executed, since dumping is happening in the block finalize step. */
  ...
  ulong  stake_delegations_fork_id_cnt = fd_banks_stake_delegations_fork_ids( banks, parent_bank, stake_delegations_fork_ids );
  ...
  for( ulong i=0UL; i<dump_ctx->txns_to_dump_cnt; i++ ) {
    ...
    dump_sanitized_transaction( accdb, parent_bank->accdb_fork_id, txn_descriptor, txn_ptr->payload, spad, &block_context->txns[i] );
    ...
    for( ushort l=0; l<txn_descriptor->acct_addr_cnt; l++ ) {
      ...
      add_account_and_programdata_to_dumped_accounts( accdb, parent_bank->accdb_fork_id, dumped_accounts, account_key );
    }
    ...
    for( ushort l=0; l<txn_descriptor->addr_table_lookup_cnt; l++ ) {
      ...
      add_lut_accounts_to_dumped_accounts( accdb, parent_bank->accdb_fork_id, dumped_accounts, lut_key );
    }
  }
  ...
  for( fd_dump_account_key_iter_t * iter = fd_dump_account_key_iter_init( dumped_accounts_iter_, dumped_accounts );
    ...
    fd_acc_t acc = fd_accdb_read_one( accdb, parent_bank->accdb_fork_id, pubkey->uc );
...
create_txn_context_protobuf_from_txn( fd_exec_test_txn_context_t * txn_context_msg,
  ...
  for( ulong i = 0; i < txn_descriptor->addr_table_lookup_cnt; ++i ) {
    ...
    /* Copy the addresses out and release before nested acquires. */
    fd_pubkey_t lookup_addrs[ 256 ]; /* LOOKUP_TABLE_MAX_ADDRESSES */
    ulong       lookup_addrs_cnt = fd_ulong_min( (acc.data_len - FD_LOOKUP_TABLE_META_SIZE) / sizeof(fd_pubkey_t), 256UL );
  ...
  for( uint i=0U; i<accounts_dumped_so_far; i++ ) {
    ...
    dump_executable_account_if_exists( runtime->accdb, bank->accdb_fork_id, maybe_program_account, spad, txn_context_msg->account_shared_data, &txn_context_msg->account_shared_data_count );
  }
  ...
  dump_sanitized_transaction( runtime->accdb, bank->accdb_fork_id, txn_descriptor, txn_payload, spad, sanitized_transaction );
...
create_instr_context_protobuf_from_instructions( fd_exec_test_instr_context_t * instr_context,
  ...
  for( ulong i = 0; i < txn_out->accounts.executable_cnt; i++ ) {
    ...
    if( !account_exists ) {
      fd_exec_test_acct_state_t * output_account = &instr_context->accounts[instr_context->accounts_count++];
      dump_account_state( ro, output_account, spad );
    }
  }
...
static void
write_account_to_result1( uchar const *               pubkey,
                          ulong                       lamports,
                          uchar const *               owner,
                          ulong                       data_len,
                          uchar const *               _data,
