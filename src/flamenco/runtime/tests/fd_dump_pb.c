...
add_account_and_programdata_to_dumped_accounts( fd_funk_t *                   funk,
  ...
  FD_TXN_ACCOUNT_DECL( program_account );
...
add_lut_accounts_to_dumped_accounts( fd_funk_t *                   funk,
  ...
  FD_TXN_ACCOUNT_DECL( lut_account );
...
create_block_context_protobuf_from_block( fd_block_dump_ctx_t * dump_ctx,
  ...
  fd_funk_txn_xid_t              parent_xid     = { .ul = { fd_bank_slot_get( parent_bank ), fd_bank_slot_get( parent_bank ) } };
  ...
  block_context->slot_ctx.slot                      = fd_bank_slot_get( parent_bank );
  // HACK FOR NOW: block height gets incremented in process_new_epoch, so we should dump block height + 1
  block_context->slot_ctx.block_height              = fd_bank_block_height_get( parent_bank ) + 1UL;
  fd_memcpy( block_context->slot_ctx.poh, fd_bank_poh_query( parent_bank ), sizeof(fd_pubkey_t) );
  fd_memcpy( block_context->slot_ctx.parent_bank_hash, fd_bank_bank_hash_query( parent_bank ), sizeof(fd_pubkey_t) );
  block_context->slot_ctx.prev_slot                 = fd_bank_parent_slot_get( parent_bank );
...
create_txn_context_protobuf_from_txn( fd_exec_test_txn_context_t * txn_context_msg,
  ...
  for( ulong i = 0; i < txn_ctx->accounts_cnt; ++i ) {
    ...
    if( !is_builtin_account( &txn_ctx->account_keys[i] ) ) {
  ...
  for( ulong i = 0; i < num_sysvar_entries; i++ ) {
    ...
    for( ulong j = 0; j < txn_ctx->accounts_cnt; j++ ) {
      if ( 0 == memcmp( txn_ctx->account_keys[j].key, fd_dump_sysvar_ids[i], sizeof(fd_pubkey_t) ) ) {
        account_exists = true;
        break;
      }
    }
...
fd_dump_instr_to_protobuf( fd_exec_txn_ctx_t * txn_ctx,
  ...
  FD_SPAD_FRAME_BEGIN( txn_ctx->spad ) {
    ...
    if (pb_encode(&stream, FD_EXEC_TEST_INSTR_CONTEXT_FIELDS, &instr_context)) {
      ...
      sprintf( output_filepath, "%s/instr-%s-%hu.instrctx", txn_ctx->capture_ctx->dump_proto_output_dir, encoded_signature, instruction_idx );
...
fd_dump_txn_to_protobuf( fd_exec_txn_ctx_t * txn_ctx, fd_spad_t * spad ) {
  FD_SPAD_FRAME_BEGIN( spad ) {
    ...
    if( pb_encode( &stream, FD_EXEC_TEST_TXN_CONTEXT_FIELDS, &txn_context_msg ) ) {
      char output_filepath[1024];
      sprintf( output_filepath, "%s/txn-%s.txnctx", txn_ctx->capture_ctx->dump_proto_output_dir, encoded_signature );
      FILE * file = fopen(output_filepath, "wb");
      if( file ) {
        fwrite( out, 1, stream.bytes_written, file );
        fclose( file );
...
fd_dump_block_to_protobuf( fd_block_dump_ctx_t *     dump_ctx,
  ...
  if( pb_encode( &stream, FD_EXEC_TEST_BLOCK_CONTEXT_FIELDS, &dump_ctx->block_context ) ) {
    char output_filepath[1024];
    sprintf( output_filepath, "%s/block-%lu.blockctx", capture_ctx->dump_proto_output_dir, fd_bank_slot_get( bank ) );
    FILE * file = fopen(output_filepath, "wb");
    if( file ) {
      fwrite( out, 1, stream.bytes_written, file );
      fclose( file );
