...
dump_lut_account_and_contained_accounts(  fd_exec_slot_ctx_t const *     slot_ctx,
  ...
  for( ulong i=0UL; i<lookup_addrs_cnt; i++ ) {
    ...
    dump_account_if_not_already_dumped( slot_ctx->accdb, &slot_ctx->funk_txn_xid, referenced_pubkey, spad, out_account_states, out_account_states_count, NULL );
  }
}

static void
dump_executable_account_if_exists( fd_accdb_client_t *               accdb,
  ...
  dump_account_if_not_already_dumped( accdb, funk_txn_xid, programdata_acc, spad, out_account_states, out_account_states_count, NULL );
}
...
dump_vote_accounts( fd_exec_slot_ctx_t const *        slot_ctx,
  ...
  for( fd_vote_states_iter_t * iter = fd_vote_states_iter_init( iter_, vote_states ); !fd_vote_states_iter_done( iter ); fd_vote_states_iter_next( iter ) ) {
    ...
    dump_account_if_not_already_dumped( slot_ctx->accdb, &slot_ctx->funk_txn_xid, &vote_state->vote_account, spad, out_acct_states, out_acct_states_cnt, NULL );
  }
...
static void
dump_sanitized_transaction( fd_accdb_client_t *                    accdb,
                            fd_funk_txn_xid_t const *              funk_txn_xid,
  ...
  if( !message->is_legacy ) {
    ...
    for( ulong i = 0; i < txn_descriptor->addr_table_lookup_cnt; ++i ) {
      ...
      int err = fd_txn_account_init_from_funk_readonly( addr_lut_rec, alut_key, accdb, funk_txn_xid );
...
create_block_context_protobuf_from_block( fd_exec_test_block_context_t * block_context,
  ...
  for( ulong i=0UL; i<num_sysvar_entries; i++ ) {
    dump_account_if_not_already_dumped( slot_ctx->accdb, &slot_ctx->funk_txn_xid, &fd_relevant_sysvar_ids[i], spad, block_context->acct_states, &block_context->acct_states_count, NULL );
  }

  for( ulong i=0UL; i<num_loaded_builtins; i++ ) {
    dump_account_if_not_already_dumped( slot_ctx->accdb, &slot_ctx->funk_txn_xid, &loaded_builtins[i], spad, block_context->acct_states, &block_context->acct_states_count, NULL );
  }
  ...
  for( fd_stake_delegations_iter_t * iter = fd_stake_delegations_iter_init( iter_, stake_delegations );
    ...
    dump_account_if_not_already_dumped( slot_ctx->accdb, &slot_ctx->funk_txn_xid, &stake_delegation->stake_account, spad, block_context->acct_states, &block_context->acct_states_count, NULL );
  }
  ...
  for( fd_vote_states_iter_t * iter = fd_vote_states_iter_init( vote_iter_, vote_states ); !fd_vote_states_iter_done( iter ); fd_vote_states_iter_next( iter ) ) {
    fd_vote_state_ele_t const * vote_state = fd_vote_states_iter_ele( iter );
    dump_account_if_not_already_dumped( slot_ctx->accdb, &slot_ctx->funk_txn_xid, &vote_state->vote_account, spad, block_context->acct_states, &block_context->acct_states_count, NULL );
  }
  ...
  for( ulong i=0UL; i<block_info->microblock_batch_cnt; i++ ) {
    ...
    for( ulong j=0UL; j<microblock_batch->microblock_cnt; j++ ) {
      ...
      for( ulong k=0UL; k<txn_cnt; k++ ) {
        ...
        dump_sanitized_transaction( slot_ctx->accdb, &slot_ctx->funk_txn_xid, txn_descriptor, txn_ptr->payload, spad, &block_context->txns[block_context->txns_count++] );
        ...
        for( ushort l=0; l<txn_descriptor->acct_addr_cnt; l++ ) {
          ...
          dump_account_if_not_already_dumped( slot_ctx->accdb, &slot_ctx->funk_txn_xid, account_key, spad, block_context->acct_states, &block_context->acct_states_count, NULL );
        }
        ...
        for( ulong l=0; l<dumped_accounts; l++ ) {
          ...
          dump_executable_account_if_exists( slot_ctx->accdb, &slot_ctx->funk_txn_xid, maybe_program_account, spad, block_context->acct_states, &block_context->acct_states_count );
        }
      }
    }
  }
...
create_txn_context_protobuf_from_txn( fd_exec_test_txn_context_t * txn_context_msg,
  ...
  */
  ...
  for( ulong i = 0; i < txn_ctx->accounts_cnt; ++i ) {
    ...
    int ret = fd_txn_account_init_from_funk_readonly( txn_account, &txn_ctx->account_keys[i], txn_ctx->accdb, &txn_ctx->funk_txn_xid );
    if( FD_UNLIKELY( ret ) ) {
      continue;
    }
  ...
  for( ulong i = 0; i < txn_descriptor->addr_table_lookup_cnt; ++i ) {
    ...
    int ret = fd_txn_account_init_from_funk_readonly( txn_account, alut_key, txn_ctx->accdb, &txn_ctx->funk_txn_xid );
    ...
    for( ulong j=0; j<addr_lut->writable_cnt; j++ ) {
      ...
      ret = fd_txn_account_init_from_funk_readonly( referenced_account, referenced_addr, txn_ctx->accdb, &txn_ctx->funk_txn_xid );
      if( FD_UNLIKELY( ret ) ) continue;
  ...
  for( uint i=0U; i<accounts_dumped_so_far; i++ ) {
    ...
    dump_executable_account_if_exists( txn_ctx->accdb, &txn_ctx->funk_txn_xid, maybe_program_account, spad, txn_context_msg->account_shared_data, &txn_context_msg->account_shared_data_count );
  }

  /* Dump sysvars */
  for( ulong i = 0; i < num_sysvar_entries; i++ ) {
    FD_TXN_ACCOUNT_DECL( txn_account );
    int ret = fd_txn_account_init_from_funk_readonly( txn_account, &fd_relevant_sysvar_ids[i], txn_ctx->accdb, &txn_ctx->funk_txn_xid );
  ...
  dump_sanitized_transaction( txn_ctx->accdb, &txn_ctx->funk_txn_xid, txn_descriptor, txn_payload, spad, sanitized_transaction );
...
create_instr_context_protobuf_from_instructions( fd_exec_test_instr_context_t * instr_context,
  ...
  for( ulong i = 0; i < txn_ctx->executable_cnt; i++ ) {
    FD_TXN_ACCOUNT_DECL( txn_account );
    int ret = fd_txn_account_init_from_funk_readonly( txn_account, txn_ctx->executable_accounts[i].pubkey, txn_ctx->accdb, &txn_ctx->funk_txn_xid );
    if( ret != FD_ACC_MGR_SUCCESS ) {
      continue;
    }
    // Make sure the account doesn't exist in the output accounts yet
...
FD_SPAD_FRAME_BEGIN( txn_ctx->spad ) {
  ...
  uchar const * program_data     = fd_program_cache_get_account_programdata( txn_ctx->accdb,
