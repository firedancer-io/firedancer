...
fd_exec_test_instr_context_create( fd_exec_instr_test_runner_t *        runner,
  ...
  for( ulong j=0UL; j < test_ctx->instr_accounts_count; j++ ) {
    ...
    info->accounts[j].index_in_transaction = (ushort) index;
    info->accounts[j].index_in_caller      = (ushort) j;
    info->accounts[j].index_in_callee      = (ushort) j;
    info->accounts[j].is_writable          = test_ctx->instr_accounts[j].is_writable;
    info->accounts[j].is_signer            = test_ctx->instr_accounts[j].is_signer;
...
fd_exec_txn_test_run( fd_exec_instr_test_runner_t * runner, // Runner only contains funk instance, so we can borrow instr test runner
  ...
  FD_SPAD_FRAME_BEGIN( runner->spad ) {
    ...
    /* Capture borrowed accounts */
    for( ushort j=0UL; j < txn_ctx->accounts_cnt; j++ ) {
