...
fd_solfuzz_pb_bundle_ctx_create( fd_solfuzz_runner_t *                 runner,
  ...
  runner->bank->stake_delegations_fork_id = fd_stake_delegations_new_fork( stake_delegations, USHORT_MAX );
  ...
  for( ulong i=0UL; bundle_prep_ok && i<txn_cnt; i++ ) {
    ...
    if( exec_res!=FD_RUNTIME_EXECUTE_SUCCESS ) {
      ...
      if( is_bundle ) {
        ...
        txn_outs[i].err.is_committable = 0;
        fd_runtime_cancel_txn( runtime, NULL, NULL, &txn_outs[i] );
      }
      break;
    }
    ...
    if( !is_bundle ) fd_runtime_commit_txn( runtime, runner->bank, NULL, &txn_outs[i] );
  }

  if( is_bundle && !saw_exec_err ) {
    for( ulong i=0UL; i<txn_cnt; i++ ) {
      fd_runtime_commit_txn( runtime, runner->bank, NULL, &txn_outs[i] );
    }
  }
