...
fd_solfuzz_block_run( fd_solfuzz_runner_t * runner,
  ...
  FD_SPAD_FRAME_BEGIN( runner->spad ) {
    ...
    if( FD_LIKELY( leaders ) ) {
      ...
      /* Get epoch schedule for better fallback values */
      fd_epoch_schedule_t epoch_schedule;
      fd_epoch_schedule_t * schedule = fd_sysvar_epoch_schedule_read( runner->funk, &xid, &epoch_schedule );

      ulong fallback_epoch = leaders->epoch ? leaders->epoch : 1UL;

      effects->leader_schedule = (fd_exec_test_leader_schedule_effects_t) {
        .leaders_epoch = fallback_epoch,
        .leaders_slot0 =  leaders->slot0 ? leaders->slot0 :
          (schedule ? fd_epoch_slot0( schedule, fallback_epoch ) : 0UL),
        .leaders_slot_cnt = leaders->slot_cnt ? leaders->slot_cnt :
          (schedule ? fd_epoch_slot_cnt( schedule, fallback_epoch ) : FD_RUNTIME_SLOTS_PER_EPOCH),
        .leader_pub_cnt = leaders->pub_cnt,
        .leaders_sched_cnt = leaders->sched_cnt*4,
      };
