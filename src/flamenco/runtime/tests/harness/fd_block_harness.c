...
fd_runtime_fuzz_block_refresh_vote_accounts( fd_vote_accounts_pair_global_t_mapnode_t * vote_accounts_pool,
                                             ...
                                             fd_stakes_slim_t *                         stake_delegations ) {
  for( ulong i=0UL; i<stake_delegations->stake_accounts_cnt; i++ ) {
    fd_stake_account_slim_t * node = &stake_delegations->stake_accounts[i];
    fd_pubkey_t * voter_pubkey = &node->delegation.voter_pubkey;
    ulong         stake        = node->delegation.stake;
...
fd_runtime_fuzz_block_register_stake_delegation( fd_exec_slot_ctx_t * slot_ctx,
 ...
 FD_TXN_ACCOUNT_DECL( acc );
  ...
  /* Nothing to do if the account already exists in the cache */
  if( fd_stake_accounts_search( stake_delegations, pubkey ) ) {
    return;
  }

  /* At this point, the node is new and needs to be inserted into the
     stakes cache. */
  fd_stake_account_slim_t * new_entry = fd_stake_accounts_add( stake_delegations, pubkey );
  new_entry->delegation = stake_state.inner.stake.stake.delegation;
}
...
fd_runtime_fuzz_block_ctx_create( fd_runtime_fuzz_runner_t *           runner,
  ...
  fd_stakes_slim_t * stake_delegations = fd_bank_stakes_locking_modify( slot_ctx->bank );
  ...
  for( ushort i=0; i<test_ctx->acct_states_count; i++ ) {
    ...
    fd_runtime_fuzz_block_register_stake_delegation( slot_ctx, stake_delegations, &pubkey );
  }
  ...
  stake_delegations->epoch = fd_slot_to_epoch( epoch_schedule, test_ctx->slot_ctx.prev_slot, NULL );
  ...
  vote_accounts = fd_bank_next_epoch_stakes_locking_modify( slot_ctx->bank );
