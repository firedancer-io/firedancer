...
fd_hash_bank( fd_exec_slot_ctx_t * slot_ctx,
  ...
  uchar lhash1[32];
  fd_lthash_hash((fd_lthash_value_t *) slot_ctx->slot_bank.lthash.lthash, lhash1);
  ...
  FD_LOG_NOTICE(( "\n\n[Replay]\n"
                  ...
                  FD_LTHASH_ENC_32_ALLOCA( (fd_lthash_value_t *)fd_type_pun_const( slot_ctx->account_delta_hash.hash ) ),
                  FD_BASE58_ENC_32_ALLOCA( lhash1 ),
                  slot_ctx->signature_cnt,
...
fd_accounts_sorted_subrange( fd_exec_slot_ctx_t * slot_ctx, uint range_idx, uint range_cnt, ulong * num_pairs_out, fd_lthash_value_t *lthash_values, ulong n0
  ...
  for( ulong i = num_iter_accounts; i; --i ) {
    ...
    if( FD_FEATURE_ACTIVE( slot_ctx, account_hash_ignore_slot ) )
      fd_hash_account_v1( (uchar *) hash, &new_lthash_value, metadata, rec->pair.key->uc, fd_account_get_data(metadata) );
    else
      fd_hash_account_v0( (uchar *) hash, &new_lthash_value, metadata, rec->pair.key->uc, fd_account_get_data(metadata), metadata->slot );
