...
fd_instr_stack_push( fd_exec_txn_ctx_t *     txn_ctx,
  ...
  int err = fd_exec_txn_ctx_get_key_of_account_at_index( txn_ctx, instr->program_id, &program_id_pubkey );
  if( FD_UNLIKELY( err ) ) {
    return FD_EXECUTOR_INSTR_ERR_UNSUPPORTED_PROGRAM_ID;
  }

  /* TODO: really agave just checks if it can get the program key so this should not be needed */
  if( FD_UNLIKELY( !memcmp( program_id_pubkey->key, fd_solana_native_loader_id.key, sizeof(fd_pubkey_t)) ) ) {
    return FD_EXECUTOR_INSTR_ERR_UNSUPPORTED_PROGRAM_ID;
  }
