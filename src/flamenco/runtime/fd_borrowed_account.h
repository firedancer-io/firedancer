...
int
fd_borrowed_account_set_state( fd_borrowed_account_t * borrowed_acct, uchar * bytes, ulong size );

/* Operators */

/* fd_borrowed_account_checked_{add,sub}_lamports mirros Agave function
   solana_sdk::transaction_context::BorrowedAccount::checked_{add/sub}_lamports.
...
fd_borrowed_account_checked_add_lamports( fd_borrowed_account_t * borrowed_acct,
  ...
  int err = fd_ulong_checked_add( borrowed_acct->acct->const_meta->info.lamports, lamports, &balance_post );
  if( FD_UNLIKELY( err ) ) {
...
fd_borrowed_account_checked_sub_lamports( fd_borrowed_account_t * borrowed_acct,
                                 ulong                   lamports ) {
  ulong balance_post = 0UL;
  int err = fd_ulong_checked_sub( borrowed_acct->acct->const_meta->info.lamports, lamports, &balance_post );
  if( FD_UNLIKELY( err ) ) {
    return FD_EXECUTOR_INSTR_ERR_ARITHMETIC_OVERFLOW;
  }
...
fd_borrowed_account_is_owned_by_current_program( fd_borrowed_account_t const * borrowed_acct ) {
  return 0==memcmp( borrowed_acct->instr_ctx->instr->program_id_pubkey.key, borrowed_acct->acct->const_meta->info.owner, sizeof(fd_pubkey_t) );
}
