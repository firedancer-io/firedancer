...
fd_runtime_commit_txn( fd_runtime_t * runtime,
  ...
  /* Bundle accounts are acquired and released once for the whole bundle
     (fd_runtime_prepare_bundle_accounts / fd_runtime_fini_bundle), so
     per-txn release is skipped here.  The commit bit set above is
     honored by the single bundle-level release. */
...
fd_runtime_cancel_txn( fd_runtime_t * runtime,
  ...
  /* See fd_runtime_commit_txn: bundle accounts are released once at the
     bundle level, not per-txn. */
  if( FD_LIKELY( !txn_out->accounts.is_bundle ) ) {
    for( ushort i=0; i<txn_out->accounts.cnt; i++ ) {
      if( FD_LIKELY( !txn_out->accounts.account_acquired[ i ] ) ) continue;
      fd_accdb_release( runtime->accdb, 1UL, txn_out->accounts.account[ i ] );
...
fd_runtime_fini_bundle( fd_runtime_t * runtime ) {
  if( FD_LIKELY( runtime->accounts.account_cnt ) ) {
    fd_accdb_release( runtime->accdb, runtime->accounts.account_cnt, runtime->accounts.account );
  }
  if( runtime->accounts.executable_cnt ) {
...
# define FD_BUNDLE_ACCT_MAX (FD_PACK_MAX_TXN_PER_BUNDLE*MAX_TX_ACCOUNT_LOCKS)
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    if( FD_UNLIKELY( err!=FD_RUNTIME_EXECUTE_SUCCESS ) ) return FD_RUNTIME_TXN_ERR_ADDRESS_LOOKUP_TABLE_NOT_FOUND;
