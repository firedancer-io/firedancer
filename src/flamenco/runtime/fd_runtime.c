...
fd_runtime_run_incinerator( fd_exec_slot_ctx_t * slot_ctx ) {
  int db_err = fd_runtime_account_write( slot_ctx, &fd_sysvar_incinerator_id, NULL, NULL, 0UL );
  if( FD_UNLIKELY( db_err!=FD_ACCDB_SUCCESS ) ) FD_LOG_ERR(( "Failed to run incinerator: database error (%i-%s)", db_err, fd_accdb_strerror( db_err ) ));
}
...
fd_runtime_leader_payout( fd_exec_slot_ctx_t * slot_ctx,
  ...
  int db_err = FD_RUNTIME_ACCOUNT_UPDATE_BEGIN( slot_ctx, leader.uc, leader_ref, 0UL ) {
...
fd_runtime_load_txn_address_lookup_tables(
  ...
  for( ulong i = 0UL; i < txn->addr_table_lookup_cnt; i++ ) {
    ...
    int db_err = FD_ACCDB_READ_BEGIN( accdb, addr_lut_acc, addr_lut_rec ) {

      /* https://github.com/anza-xyz/agave/blob/368ea563c423b0a85cc317891187e15c9a321521/accounts-db/src/accounts.rs#L96-L114 */
      if( FD_UNLIKELY( memcmp( fd_accdb_ref_owner( addr_lut_rec ), fd_solana_address_lookup_table_program_id.key, sizeof(fd_pubkey_t) ) ) ) {
      ...
      if( FD_UNLIKELY( fd_accdb_ref_data_sz( addr_lut_rec ) < FD_LOOKUP_TABLE_META_SIZE ) ) {
      ...
      fd_address_lookup_table_state_t addr_lookup_table_state[1];
      if( FD_UNLIKELY( !fd_bincode_decode_static(
          address_lookup_table_state, &addr_lookup_table_state,
          fd_accdb_ref_data_const( addr_lut_rec ),
          FD_LOOKUP_TABLE_META_SIZE, /* bound checked above */
          NULL ) ) ) {
        return FD_RUNTIME_TXN_ERR_INVALID_ADDRESS_LOOKUP_TABLE_DATA;
      }
      ...
      if( FD_UNLIKELY( (fd_accdb_ref_data_sz( addr_lut_rec ) - FD_LOOKUP_TABLE_META_SIZE) & 0x1fUL ) ) {
        return FD_RUNTIME_TXN_ERR_INVALID_ADDRESS_LOOKUP_TABLE_DATA;
      }

      /* https://github.com/anza-xyz/agave/blob/368ea563c423b0a85cc317891187e15c9a321521/accounts-db/src/accounts.rs#L101-L112 */
      fd_acct_addr_t * lookup_addrs     = (fd_acct_addr_t *)( (ulong)fd_accdb_ref_data_const( addr_lut_rec )+ FD_LOOKUP_TABLE_META_SIZE );
      ulong            lookup_addrs_cnt = (fd_accdb_ref_data_sz( addr_lut_rec ) - FD_LOOKUP_TABLE_META_SIZE) >> 5UL; // = (dlen - 56) / 32
      ...
      int err = fd_get_active_addresses_len(
          &addr_lookup_table_state->inner.lookup_table,
          slot,
          hashes,
          lookup_addrs_cnt,
    ...
    if( FD_UNLIKELY( db_err!=FD_ACCDB_SUCCESS ) ) {
      if( FD_UNLIKELY( db_err!=FD_ACCDB_ERR_KEY ) ) {
        FD_LOG_ERR(( "Failed to read address lookup table account: database error (%i-%s)", db_err, fd_accdb_strerror( db_err ) ));
      }
...
fd_feature_activate( fd_exec_slot_ctx_t *    slot_ctx,
  ...
  FD_RUNTIME_ACCOUNT_UPDATE_BEGIN( slot_ctx, acct, rec, 9UL ) {
    ...
    fd_feature_t * feature = fd_bincode_decode_static(
        feature, feature_,
        fd_accdb_ref_data_const( rec->ro ),
        fd_accdb_ref_data_sz   ( rec->ro ),
        NULL );
...
fd_runtime_update_program_cache( fd_exec_slot_ctx_t * slot_ctx,
  ...
  if( txn_descriptor->transaction_version==FD_TXN_V0 ) {
    ...
    fd_slot_hashes_global_t const * slot_hashes_global = fd_sysvar_slot_hashes_read( slot_ctx->accdb, runtime_spad );
...
fd_runtime_init_bank_from_genesis( fd_exec_slot_ctx_t *        slot_ctx,
  ...
  fd_stake_history_t * stake_history = fd_sysvar_stake_history_read( slot_ctx->accdb, runtime_spad );
