...
static int
recover_clock( fd_bank_t * bank, fd_spad_t * runtime_spad ) {
  ...
  for( fd_vote_accounts_pair_global_t_mapnode_t * n = fd_vote_accounts_pair_global_t_map_minimum(vote_accounts_pool, vote_accounts_root);
    ...
    if( slot != 0 || n->elem.stake != 0 ) {
      fd_vote_record_timestamp_vote_with_slot( &n->elem.key, timestamp, slot, bank );
    }
  } FD_SPAD_FRAME_END;
  }
...
fd_bank_t *
fd_runtime_recover_bank( fd_banks_t *                        banks,
                         fd_solana_manifest_global_t const * manifest,
                         fd_spad_t *                         runtime_spad ) {

  fd_bank_t * bank = fd_banks_clone_from_parent( banks, manifest->bank.slot, 0UL );
  if( FD_UNLIKELY( !bank ) ) {
  ...
  fd_block_hash_queue_global_t * bhq = fd_bank_block_hash_queue_modify( bank );
  ...
  fd_bank_bank_hash_set( bank, old_bank->hash );
  ...
  *fee_rate_governor = old_bank->fee_rate_governor;
  ...
  fd_bank_capitalization_set( bank, old_bank->capitalization );
  ...
  fd_bank_transaction_count_set( bank, old_bank->transaction_count );

  /* Parent Signature Count */

  fd_bank_parent_signature_cnt_set( bank, old_bank->signature_count );

  /* Tick Height */

  fd_bank_tick_height_set( bank, old_bank->tick_height );

  /* Max Tick Height */

  fd_bank_max_tick_height_set( bank, old_bank->max_tick_height );

  /* Hashes Per Tick */

  ulong * hashes_per_tick = fd_versioned_bank_hashes_per_tick_join( old_bank );
  fd_bank_hashes_per_tick_set( bank, !!hashes_per_tick ? *hashes_per_tick : 0UL );

  /* NS Per Slot */

  fd_bank_ns_per_slot_set( bank, old_bank->ns_per_slot );

  /* Ticks Per Slot */

  fd_bank_ticks_per_slot_set( bank, old_bank->ticks_per_slot );

  /* Genesis Creation Time */

  fd_bank_genesis_creation_time_set( bank, old_bank->genesis_creation_time );

  /* Slots Per Year */

  fd_bank_slots_per_year_set( bank, old_bank->slots_per_year );

  /* Inflation */

  fd_bank_inflation_set( bank, old_bank->inflation );

  /* Block Height */

  fd_bank_block_height_set( bank, old_bank->block_height );

  /* Epoch Account Hash */

  fd_hash_t * epoch_account_hash = fd_solana_manifest_epoch_account_hash_join( manifest );
  if( !!epoch_account_hash ) {
    fd_bank_epoch_account_hash_set( bank, *epoch_account_hash );
  } else {
    fd_bank_epoch_account_hash_set( bank, (fd_hash_t){0} );
  }

  /* Prev Slot */

  fd_bank_prev_slot_set( bank, old_bank->parent_slot );

  /* Execution Fees */

  fd_bank_execution_fees_set( bank, old_bank->collector_fees );
  ...
  if( last_hash ) {
    fd_bank_poh_set( bank, *last_hash );
  }

  /* Prev Bank Hash */

  fd_bank_prev_bank_hash_set( bank, old_bank->parent_hash );
  ...
  recover_clock( bank, runtime_spad );
  ...
  do {
    ...
    for( ulong i=0UL; i<manifest->versioned_epoch_stakes_len; i++ ) {
      ...
      if( versioned_epoch_stakes[i].epoch == epoch+1UL ) {
        ...
        fd_bank_total_epoch_stake_set( bank, versioned_epoch_stakes[i].val.inner.Current.total_stake );
      }
    }

    fd_bank_use_prev_epoch_stake_set( bank, epoch + 2UL );
    ...
    fd_bank_epoch_stakes_end_locking_modify( bank );

    /* Move next EpochStakes */

    fd_vote_accounts_global_t * next_epoch_stakes = fd_bank_next_epoch_stakes_locking_modify( bank );
    ...
    fd_bank_next_epoch_stakes_end_locking_modify( bank );

  } while(0);

  fd_slot_lthash_t * lthash = fd_bank_lthash_modify( bank );
...
fd_txncache_t *
fd_runtime_recover_status_cache( fd_txncache_t *         status_cache,
  ...
  for( ulong i = 0; i < slot_deltas->slot_deltas_len; i++ ) {
    ...
    if( slot_delta->is_root ) {
      fd_txncache_register_root_slot( status_cache, slot );
    }
    for( ulong j = 0; j < slot_delta->slot_delta_vec_len; j++ ) {
  ...
  fd_txncache_insert_batch( status_cache, insert_vals, num_entries );
  ...
  for( ulong i = 0; i < slot_deltas->slot_deltas_len; i++ ) {
    ...
    for( ulong j = 0; j < slot_delta->slot_delta_vec_len; j++ ) {
      fd_status_pair_t * pair      = &slot_delta->slot_delta_vec[j];
      fd_hash_t *        blockhash = &pair->hash;
      fd_txncache_set_txnhash_offset( status_cache, slot, blockhash->uc, pair->value.txn_idx );
    }
  }
...
block_finalize_tpool_wrapper( void * para_arg_1,
  ...
  for( ulong worker_idx=1UL; worker_idx<worker_cnt; worker_idx++ ) {
    ...
    fd_tpool_exec( tpool, worker_idx, fd_account_hash_task,
                   ...
                   &task_data->lthash_values[worker_idx], NULL, 0UL,
...
fd_runtime_prepare_txns_start( fd_bank_t *                  bank,
  ...
  for( ulong txn_idx = 0UL; txn_idx < txn_cnt; txn_idx++ ) {
    ...
    int err = fd_execute_txn_prepare_start( bank,
                                            funk,
                                            funk_txn,
                                            txn_ctx,
                                            txn_descriptor,
...
fd_runtime_prepare_and_execute_txn( fd_bank_t *                  bank,
  ...
  res = fd_execute_txn_prepare_start( bank, funk, funk_txn, txn_ctx, txn_descriptor, &raw_txn );
  if( FD_UNLIKELY( res ) ) {
    txn->flags = 0U;
    return -1;
  }
...
fd_runtime_prepare_execute_finalize_txn_task( void * tpool,
  ...
  fd_bank_t *                  bank         = (fd_bank_t *)tpool;
  ...
  fd_funk_t *                  funk         = (fd_funk_t *)stride;
  fd_funk_txn_t *              funk_txn     = (fd_funk_txn_t *)l0;
  ...
  fd_runtime_finalize_txn( funk, funk_txn, task_info, task_info->txn_ctx->spad, bank );
}
  ...
  while( curr_exec_idx<txn_cnt ) {
    ...
    for( ulong worker_idx=1UL; worker_idx<exec_spad_cnt; worker_idx++ ) {
      ...
      fd_tpool_exec( tpool, worker_idx, fd_runtime_prepare_execute_finalize_txn_task,
                     bank, (ulong)capture_ctx, (ulong)task_infos[curr_exec_idx].txn,
                     &task_infos[ curr_exec_idx ], exec_spads[ worker_idx ], (ulong)funk,
                     (ulong)funk_txn, 0UL, 0UL, 0UL, 0UL, 0UL);
    ...
    if( cost_tracker_opt!=NULL ) {
      for( ulong i=exec_idx_start; i<curr_exec_idx; i++ ) {
        ...
        if( FD_UNLIKELY( res ) ) {
          FD_LOG_WARNING(( "Block cost limits exceeded for slot %lu", bank->slot ));
          break;
        }
      }
    }
...
static void
fd_update_stake_delegations( fd_bank_t *       bank,
...
fd_migrate_builtin_to_core_bpf( fd_bank_t *         bank,
  ...
  uchar program_exists = ( fd_txn_account_init_from_funk_readonly( target_program_account, builtin_program_id, funk, funk_txn )==FD_ACC_MGR_SUCCESS );
  if( !stateless ) {
  ...
  if( FD_UNLIKELY( fd_txn_account_init_from_funk_readonly( program_data_account, target_program_data_address, funk, funk_txn )==FD_ACC_MGR_SUCCESS ) ) {
  ...
  fd_funk_txn_t * parent_txn = funk_txn;
  fd_funk_txn_xid_t migration_xid = fd_funk_generate_xid();
  fd_funk_txn_start_write( funk );
  funk_txn = fd_funk_txn_prepare( funk, funk_txn, &migration_xid, 0UL );
  ...
  err = fd_txn_account_init_from_funk_mutable( new_target_program_account, builtin_program_id, funk, funk_txn, stateless, SIZE_OF_PROGRAM );
  if( FD_UNLIKELY( err ) ) {
  ...
  new_target_program_account->vt->set_slot( new_target_program_account, bank->slot );
  ...
  fd_txn_account_mutable_fini( new_target_program_account, funk, funk_txn );
  ...
  new_target_program_data_account->vt->set_slot( new_target_program_data_account, bank->slot );
  ...
  fd_txn_account_mutable_fini( new_target_program_data_account, funk, funk_txn );
  ...
  err = fd_directly_invoke_loader_v3_deploy(
      bank,
      funk,
      funk_txn,
  ...
  fd_txn_account_mutable_fini( source_buffer_account, funk, funk_txn );
  ...
  fd_funk_txn_start_write( funk );
  fd_funk_txn_publish_into_parent( funk, funk_txn, 1 );
  fd_funk_txn_end_write( funk );
  funk_txn = parent_txn;
  return;
...
fail:
  ...
  fd_funk_txn_start_write( funk );
  fd_funk_txn_cancel( funk, funk_txn, 0UL );
  fd_funk_txn_end_write( funk );
  funk_txn = parent_txn;
}
...
fd_apply_builtin_program_feature_transitions( fd_bank_t *     bank,
  ...
  for( ulong i=0UL; i<fd_num_builtins(); i++ ) {
    ...
    if( builtins[i].core_bpf_migration_config && FD_FEATURE_ACTIVE_OFFSET( bank->slot, fd_bank_features_get( bank ), builtins[i].core_bpf_migration_config->enable_feature_offset ) ) {
    ...
    if( builtins[i].enable_feature_offset!=NO_ENABLE_FEATURE_ID && FD_FEATURE_JUST_ACTIVATED_OFFSET( bank, builtins[i].enable_feature_offset ) ) {
      ...
      fd_write_builtin_account( bank, funk, funk_txn, *builtins[i].pubkey, builtins[i].data, strlen(builtins[i].data) );
    }
  }
  ...
  for( ulong i=0UL; i<fd_num_stateless_builtins(); i++ ) {
    if( stateless_builtins[i].core_bpf_migration_config && FD_FEATURE_ACTIVE_OFFSET( bank->slot, fd_bank_features_get( bank ), stateless_builtins[i].core_bpf_migration_config->enable_feature_offset ) ) {
      FD_LOG_NOTICE(( "Migrating stateless builtin program %s to core BPF", FD_BASE58_ENC_32_ALLOCA( stateless_builtins[i].pubkey->key ) ));
      fd_migrate_builtin_to_core_bpf( bank,
                                      funk,
                                      funk_txn,
  ...
  for( ulong i=0UL; i<fd_num_precompiles(); i++ ) {
    if( FD_FEATURE_JUST_ACTIVATED_OFFSET( bank, precompiles[i].feature_offset ) ) {
      fd_write_builtin_account( bank, funk, funk_txn, *precompiles[i].pubkey, "", 0 );
    }
  }
...
fd_feature_activate( fd_features_t *         features,
  ...
  int err = fd_txn_account_init_from_funk_readonly( acct_rec, (fd_pubkey_t*)acct, funk, funk_txn );
  if( FD_UNLIKELY( err != FD_ACC_MGR_SUCCESS ) ) {
    return;
  }
  ...
  if( feature->has_activated_at ) {
    ...
    err = fd_txn_account_init_from_funk_mutable( modify_acct_rec, (fd_pubkey_t *)acct, funk, funk_txn, 0, 0UL );
    if( FD_UNLIKELY( err != FD_ACC_MGR_SUCCESS ) ) {
      return;
    }

    feature->has_activated_at = 1;
    feature->activated_at     = bank->slot;
    fd_bincode_encode_ctx_t encode_ctx = {
    ...
    fd_txn_account_mutable_fini( modify_acct_rec, funk, funk_txn );
  }
...
fd_features_activate( fd_bank_t *     bank,
  ...
  for( fd_feature_id_t const * id = fd_feature_iter_init();
    ...
    fd_feature_activate( features, bank, funk, funk_txn, id, id->id.key, runtime_spad );
  }
}

uint
fd_runtime_is_epoch_boundary( fd_bank_t * bank, ulong curr_slot, ulong prev_slot ) {
  ulong slot_idx;
...
fd_runtime_process_new_epoch( fd_bank_t *     bank,
  ...
  ulong                       slot;
  fd_epoch_schedule_t const * epoch_schedule = fd_bank_epoch_schedule_query( bank );
  ulong                       epoch          = fd_slot_to_epoch( epoch_schedule, bank->slot, &slot );

  /* Activate new features
     https://github.com/anza-xyz/agave/blob/v2.1.0/runtime/src/bank.rs#L6587-L6598 */
  fd_features_activate( bank, funk, funk_txn, runtime_spad );
  fd_features_restore( bank, funk, funk_txn, runtime_spad );
  ...
  int     is_some                       = fd_new_warmup_cooldown_rate_epoch( bank->slot,
                                                                             funk,
                                                                             funk_txn,
                                                                             runtime_spad,
  ...
  fd_block_hash_queue_global_t const * bhq              = fd_bank_block_hash_queue_query( bank );
...
static void
fd_runtime_init_bank_from_genesis( fd_bank_t *                 bank,
  ...
  fd_block_hash_queue_global_t *      block_hash_queue = fd_bank_block_hash_queue_modify( bank );
  ...
  fd_bank_max_tick_height_set( bank, genesis_block->ticks_per_slot * (bank->slot + 1) );
  ...
  fd_stakes_global_t * stakes_global = fd_bank_stakes_locking_modify( bank );
...
fd_runtime_process_genesis_block( fd_bank_t *        bank,
  ...
  fd_runtime_init_program( bank, funk, funk_txn, runtime_spad );
  ...
  fd_hash_t * bank_hash = fd_bank_bank_hash_modify( bank );
  int result = fd_update_hash_bank_tpool(
      bank,
      funk,
      funk_txn,
      capture_ctx,
...
fd_runtime_read_genesis( fd_bank_t *          bank,
  ...
  fd_hash_t * genesis_hash_bm = fd_bank_genesis_hash_modify( bank );
  ...
  if( !is_snapshot ) {
    ...
    xid.ul[1] = 0UL;
    xid.ul[0] = 0UL;
    *funk_txn_out = fd_funk_txn_prepare( funk, NULL, &xid, 1 );
    fd_funk_txn_end_write( funk );

    fd_runtime_init_bank_from_genesis( bank,
        genesis_block,
        &genesis_hash,
        runtime_spad );
    ...
    for( ulong i=0; i<genesis_block->accounts_len; i++ ) {
      ...
      fd_txn_account_mutable_fini( rec, funk, *funk_txn_out );
    }
    ...
    fd_features_restore( bank, funk, *funk_txn_out, runtime_spad );

    int err = fd_runtime_process_genesis_block(
        bank,
        funk,
        *funk_txn_out,
        capture_ctx,
