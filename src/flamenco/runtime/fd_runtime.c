...
update_next_leaders( fd_bank_t *          bank,
  ...
  ulong epoch    = fd_slot_to_epoch ( epoch_schedule, bank->data->fields.slot, NULL ) + 1UL;
...
fd_runtime_update_leaders( fd_bank_t *          bank,
  ...
  ulong epoch    = fd_slot_to_epoch ( epoch_schedule, bank->data->fields.slot, NULL );
  ulong slot0    = fd_epoch_slot0   ( epoch_schedule, epoch );
  ulong slot_cnt = fd_epoch_slot_cnt( epoch_schedule, epoch );
...
fd_runtime_settle_fees( fd_bank_t *               bank,
  ...
  ulong execution_fees = bank->data->execution_fees;
  ulong priority_fees  = bank->data->priority_fees;
  ...
  fd_pubkey_t const * leader = fd_epoch_leaders_get( leaders, bank->data->fields.slot );
  if( FD_UNLIKELY( !leader ) ) FD_LOG_CRIT(( "fd_epoch_leaders_get(%lu) returned NULL", bank->data->fields.slot ));
  ...
  if( FD_UNLIKELY( !fd_runtime_validate_fee_collector( bank, rw->ro, fees ) ) ) {  /* validation failed */
    ...
    FD_LOG_INFO(( "slot %lu has an invalid fee collector, burning fee reward (%lu lamports)", bank->data->fields.slot, fees ));
  } else {
...
fd_runtime_freeze( fd_bank_t *         bank,
  ...
  fd_funk_txn_xid_t const xid = { .ul = { bank->data->fields.slot, bank->data->idx } };

  if( FD_LIKELY( bank->data->fields.slot != 0UL ) ) {
  ...
  /* jito collects a 3% fee at the end of the block + 3% fee at
     distribution time. */
  ulong tips_pre_comission = bank->data->tips;
  bank->data->tips = (tips_pre_comission - (tips_pre_comission * 6UL / 100UL));
...
fd_apply_builtin_program_feature_transitions( fd_bank_t *               bank,
  ...
  for( ulong i=0UL; i<fd_num_builtins(); i++ ) {
    ...
    if( builtins[i].core_bpf_migration_config && FD_FEATURE_ACTIVE_OFFSET( bank->data->fields.slot, &bank->data->fields.features, builtins[i].core_bpf_migration_config->enable_feature_offset ) ) {
  ...
  for( ulong i=0UL; i<fd_num_stateless_builtins(); i++ ) {
    if( stateless_builtins[i].core_bpf_migration_config && FD_FEATURE_ACTIVE_OFFSET( bank->data->fields.slot, &bank->data->fields.features, stateless_builtins[i].core_bpf_migration_config->enable_feature_offset ) ) {
...
fd_feature_activate( fd_bank_t *               bank,
  ...
  if( feature.is_active ) {
    ...
    FD_LOG_DEBUG(( "feature %s not activated at slot %lu, activating", addr_b58, bank->data->fields.slot ));
    fd_accdb_rw_t rw[1];
    ...
    feature.is_active       = 1;
    feature.activation_slot = bank->data->fields.slot;
...
fd_runtime_process_new_epoch( fd_banks_t *              banks,
  ...
  */
  ...
  FD_LOG_NOTICE(( "starting epoch %lu at slot %lu took %.6f seconds", bank->data->fields.epoch, bank->data->fields.slot, (double)(end - start) / 1e9 ));
}
...
fd_runtime_block_pre_execute_process_new_epoch( fd_banks_t *              banks,
  ...
  if( FD_LIKELY( slot != 0UL ) ) {
    ...
    ulong prev_epoch = fd_slot_to_epoch( epoch_schedule, bank->data->parent_slot, NULL );
    ulong slot_idx;
...
fd_runtime_block_sysvar_update_pre_execute( fd_bank_t *               bank,
  ...
  fd_runtime_new_fee_rate_governor_derived( bank, bank->data->parent_signature_cnt );

  fd_epoch_schedule_t const * epoch_schedule = &bank->data->fields.epoch_schedule;
  ulong                       parent_epoch   = fd_slot_to_epoch( epoch_schedule, bank->data->parent_slot, NULL );
  fd_sysvar_clock_update( bank, accdb, xid, capture_ctx, runtime_stack, &parent_epoch );

  // It has to go into the current txn previous info but is not in slot 0
  if( bank->data->fields.slot != 0 ) {
...
fd_runtime_block_execute_prepare( fd_banks_t *         banks,
  ...
  if( FD_LIKELY( bank->data->fields.slot ) ) {
    fd_cost_tracker_t * cost_tracker = fd_bank_cost_tracker_modify( bank );
    FD_TEST( cost_tracker );
    fd_cost_tracker_init( cost_tracker, &bank->data->fields.features, bank->data->fields.slot );
  }
...
fd_runtime_update_bank_hash( fd_bank_t *        bank,
  ...
  fd_hashes_hash_bank(
      lthash,
      &bank->data->fields.bank_hash,
      (fd_hash_t *)&bank->data->fields.poh,
      bank->data->signature_count,
      new_bank_hash );
  ...
  if( capture_ctx && capture_ctx->capture_solcap &&
    ...
    fd_capture_link_write_bank_preimage(
      ...
      (fd_hash_t *)&bank->data->prev_bank_hash,
...
fd_runtime_commit_txn( fd_runtime_t * runtime,
  ...
  if( FD_UNLIKELY( txn_out->err.txn_err ) ) {
    ...
    if( txn_out->details.tips>0UL )
      FD_ATOMIC_FETCH_AND_ADD( &bank->data->tips, txn_out->details.tips );
    ...
    ulong current_slot = bank->data->fields.slot;
  ...
  FD_ATOMIC_FETCH_AND_ADD( &bank->data->transaction_count, 1UL );
  FD_ATOMIC_FETCH_AND_ADD( &bank->data->execution_fees,    txn_out->details.execution_fee );
  FD_ATOMIC_FETCH_AND_ADD( &bank->data->priority_fees,     txn_out->details.priority_fee );
  FD_ATOMIC_FETCH_AND_ADD( &bank->data->signature_count,   txn_out->details.signature_count );

  if( !txn_out->details.is_simple_vote ) {
    FD_ATOMIC_FETCH_AND_ADD( &bank->data->nonvote_txn_count, 1 );
    if( FD_UNLIKELY( txn_out->err.exec_err ) ) {
      FD_ATOMIC_FETCH_AND_ADD( &bank->data->nonvote_failed_txn_count, 1 );
    }
  }

  if( FD_UNLIKELY( txn_out->err.exec_err ) ) {
    FD_ATOMIC_FETCH_AND_ADD( &bank->data->failed_txn_count, 1 );
  }

  FD_ATOMIC_FETCH_AND_ADD( &bank->data->total_compute_units_used, txn_out->details.compute_budget.compute_unit_limit - txn_out->details.compute_budget.compute_meter );
...
# if FD_HAS_FLATCC
  uchar dump_txn = !!( runtime->log.dump_proto_ctx &&
                       bank->data->fields.slot >= runtime->log.dump_proto_ctx->dump_proto_start_slot &&
...
fd_runtime_init_bank_from_genesis( fd_banks_t *              banks,
  ...
  bank->data->parent_slot = ULONG_MAX;
  ...
  bank->data->fields.max_tick_height = genesis->poh.ticks_per_slot * (bank->data->fields.slot + 1);
...
fd_runtime_process_genesis_block( fd_bank_t *               bank,
  ...
  fd_hash_t const * prev_bank_hash = &bank->data->prev_bank_hash;
...
fd_runtime_account_is_writable_idx( fd_txn_in_t const *  txn_in,
  ...
  return fd_runtime_account_is_writable_idx_flat( bank->data->fields.slot,
                                                  idx,
