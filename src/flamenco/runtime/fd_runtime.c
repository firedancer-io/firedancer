...
fd_runtime_run_incinerator( fd_exec_slot_ctx_t * slot_ctx ) {
  ...
  ulong new_capitalization = fd_ulong_sat_sub( fd_bank_capitalization_get( slot_ctx->bank ), rec->vt->get_lamports( rec ) );
  fd_bank_capitalization_set( slot_ctx->bank, new_capitalization );
...
fd_runtime_use_multi_epoch_collection( fd_exec_slot_ctx_t const * slot_ctx, ulong slot ) {
  ...
  ulong slot_count_in_two_day = fd_runtime_slot_count_in_two_day( fd_bank_ticks_per_slot_get( slot_ctx->bank ) );
...
fd_runtime_update_rent_epoch( fd_exec_slot_ctx_t * slot_ctx ) {
  if( FD_FEATURE_ACTIVE_BM( slot_ctx->bank, disable_partitioned_rent_collection ) ) {
    return;
  }
...
fd_runtime_freeze( fd_exec_slot_ctx_t * slot_ctx, fd_spad_t * runtime_spad ) {
  ...
  if( FD_FEATURE_ACTIVE_BM( slot_ctx->bank, reward_full_priority_fee ) ) {
  ...
  if( FD_LIKELY( fees ) ) {
    ...
    do {
      ...
      if ( FD_LIKELY( FD_FEATURE_ACTIVE_BM( slot_ctx->bank, validate_fee_collector_account ) ) ) {
        ulong _burn;
...
fd_runtime_process_txns_in_microblock_stream( fd_exec_slot_ctx_t * slot_ctx,
  ...
  while( curr_exec_idx<txn_cnt ) {
    ...
    if( cost_tracker_opt!=NULL && FD_FEATURE_ACTIVE_BM( slot_ctx->bank, apply_cost_tracker_during_replay ) ) {
  ...
  for( fd_funk_txn_t * txn = slot_ctx->funk_txn; txn; txn = fd_funk_txn_parent(txn, txnpool) ) {
    if( ++depth == (FD_RUNTIME_OFFLINE_NUM_ROOT_BLOCKS - 1 ) ) {
      ...
      if( txn->xid.ul[0] >= fd_bank_eah_start_slot_get( slot_ctx->bank ) ) {
        if( !FD_FEATURE_ACTIVE_BM( slot_ctx->bank, accounts_lt_hash ) ) {
          do_eah = 1;
        }
...
fd_runtime_block_execute_tpool( fd_exec_slot_ctx_t *    slot_ctx,
  ...
  if( FD_FEATURE_ACTIVE_BM( slot_ctx->bank, apply_cost_tracker_during_replay ) ) {
