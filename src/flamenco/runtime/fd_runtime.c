...
fd_apply_builtin_program_feature_transitions( fd_bank_t *               bank,
  ...
  for( ulong i=0UL; i<fd_num_builtins(); i++ ) {
    ...
    if( builtins[i].core_bpf_migration_config && FD_FEATURE_ACTIVE_OFFSET( fd_bank_slot_get( bank ), &bank->data->f.features, builtins[i].core_bpf_migration_config->enable_feature_offset ) ) {
  ...
  for( ulong i=0UL; i<fd_num_stateless_builtins(); i++ ) {
    if( stateless_builtins[i].core_bpf_migration_config && FD_FEATURE_ACTIVE_OFFSET( fd_bank_slot_get( bank ), &bank->data->f.features, stateless_builtins[i].core_bpf_migration_config->enable_feature_offset ) ) {
...
fd_runtime_process_new_epoch( fd_banks_t *              banks,
  ...
  */
  ...
  FD_LOG_NOTICE(( "starting epoch %lu at slot %lu took %.6f seconds", bank->data->f.epoch, fd_bank_slot_get( bank ), (double)(end - start) / 1e9 ));
}
...
fd_runtime_block_execute_prepare( fd_banks_t *         banks,
  ...
  if( FD_LIKELY( fd_bank_slot_get( bank ) ) ) {
    ...
    fd_cost_tracker_init( cost_tracker, &bank->data->f.features, fd_bank_slot_get( bank ) );
  }
