...
fd_accdb_reset( fd_accdb_t * accdb ) {
  ...
  /* Shared metrics (preserve accounts_capacity). */
  ulong accounts_capacity = shmem->shmetrics->accounts_capacity;
  memset( shmem->shmetrics, 0, sizeof(fd_accdb_shmem_metrics_t) );
  shmem->shmetrics->accounts_capacity = accounts_capacity;
  ...
  /* Tell T2 (background thread) to clear its stale deferred fork
     chain.  T2's deferred_fork_head/tail now reference recycled pool
     elements; T2 must discard them before processing any future
     advance_root or purge command. */
...
fd_accdb_snapshot_save_whead( fd_accdb_t *                   accdb,
  ...
  out->whead_val          = accdb->shmem->whead[ 0 ].val;
  out->has_partition      = accdb->shmem->has_partition[ 0 ];
  out->partition_max      = accdb->shmem->partition_max;
  out->disk_current_bytes = accdb->shmem->shmetrics->disk_current_bytes;

  if( accdb->shmem->has_partition[ 0 ] ) {
    ulong idx = packed_partition_idx( &accdb->shmem->whead[ 0 ] );
    fd_accdb_partition_t * part = partition_pool_ele( accdb->partition_pool, idx );
    out->savepoint_bytes_freed = part->bytes_freed;
  } else {
