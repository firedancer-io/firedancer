...
fiber_probe_exec( void * _ctx ) {
  ...
  ulong len = 0xdeadUL;
  int   r   = fd_accdb_probe_pd_this_fork( f->accdb, f->probe.fork_id, f->probe.pubkey, &pd, &len, NULL );
  FD_TEST( pd==0 || pd==1 );
  FD_TEST( r==0 || r==1 );
...
test_probe_vs_pd_commit( void ) {
  ...
  for( ulong i=0UL; i<ITER_DEFAULT; i++ ) {
    ...
    int   pd  = 0;
    ulong len = ULONG_MAX;
    FD_TEST( fd_accdb_probe_pd_this_fork( ctl, a, key, &pd, &len, NULL )==1 );
    FD_TEST( pd==1 );
...
test_pd_parent_read_vs_child_write( void ) {
  ...
  for( ulong i=0UL; i<ITER_DEFAULT; i++ ) {
    ...
    int   pd  = 0;
    ulong len = ULONG_MAX;
    FD_TEST( fd_accdb_probe_pd_this_fork( ctl, c, key_d, &pd, &len, NULL )==1 );
