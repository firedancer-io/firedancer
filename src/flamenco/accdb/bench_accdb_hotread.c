...
bench_setup( int * out_fd,
  ...
  *out_fd = fd;
  ...
  ulong shmem_fp = fd_accdb_shmem_footprint( max_accounts, 0UL, max_live_slots, max_account_writes_per_slot, partition_cnt, cache_fp, 640UL, 1UL, 0UL );
  FD_TEST( shmem_fp );
  ...
  fd_accdb_shmem_t * shmem = fd_accdb_shmem_join(
      fd_accdb_shmem_new( shmem_mem, max_accounts, 0UL, max_live_slots,
                          ...
                          partition_sz, cache_fp, 640UL, 0, 42UL, 1UL, 0UL ) );
  FD_TEST( shmem );
