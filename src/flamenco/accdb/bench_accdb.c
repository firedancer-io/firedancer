...
bench_setup( int * out_fd,
  ...
  *out_fd = fd;
  ...
  ulong shmem_fp = fd_accdb_shmem_footprint( max_accounts, 0UL, max_live_slots, max_account_writes_per_slot, partition_cnt, cache_fp, BENCH_CACHE_MIN_RESERVED, 1UL, 0UL );
  FD_TEST( shmem_fp );
...
static void
bench_write_one( fd_accdb_t *       accdb,
...
/* Helper: read a single account via acquire/release.  Returns 1 if
   found. */
static int
bench_read_one( fd_accdb_t *       accdb,
                fd_accdb_fork_id_t fork_id,
                uchar const *      pubkey,
                ulong *            out_lamports,
                uchar *            out_data,
...
bench_write( ulong   account_cnt,
  ...
  for( ulong i=0UL; i<account_cnt; i++ ) {
    ...
    bench_write_one( accdb, fork, pubkey, i+1UL,
...
bench_read( ulong   account_cnt,
  ...
  for( ulong i=0UL; i<account_cnt; i++ ) {
    ...
    if( bench_read_one( accdb, fork, pubkey, &lamports,
...
bench_replay( ulong   slot_cnt,
  ...
  for( ulong s=0UL; s<slot_cnt; s++ ) {
    ...
    for( ulong r=0UL; r<reads_per_slot; r++ ) {
      ...
      make_pubkey( pubkey, idx );
      if( bench_read_one( accdb, cur, pubkey, &lamports,
                          rdata, &data_len, owner ) ) {
        total_read_hits++;
      }
      total_reads++;
    ...
    for( ulong w=0UL; w<writes_per_slot; w++ ) {
      ...
      ulong sz = sample_account_size( rng );
      if( sz>BENCH_MAX_DATA_SZ ) sz = BENCH_MAX_DATA_SZ;
      bench_write_one( accdb, cur, pubkey, (s*writes_per_slot + w)+1UL,
                       sz ? data_buf : NULL, sz, dummy_owner );
    ...
    if( FD_LIKELY( s>0UL ) ) {
      ...
      { int b = 0; fd_accdb_background( accdb, &b ); }
    }
...
bench_mixed( ulong   base_cnt,
  ...
  for( ulong i=0UL; i<op_cnt; i++ ) {
    ...
    make_pubkey( pubkey, idx );
    if( coin < read_pct ) {
      bench_read_one( accdb, fork, pubkey, &lamports,
                      rdata, &data_len, owner );
      reads++;
    } else {
      ulong sz = sample_account_size( rng );
