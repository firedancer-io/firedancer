...
fd_accdb_cache_class_cnt( ulong   cache_footprint,
  ...
  static const ulong ws_target[ FD_ACCDB_CACHE_CLASS_CNT ] = {
    ...
    13000UL,  /* class 1: p99 ~10.4K */
     ...
     1800UL,  /* class 4: p99  ~1.0K, needs headroom for pre-evict to keep up */
      ...
      512UL,  /* class 6: p99   ~212, 2.4x; was 704 */
      448UL,  /* class 7: p99   ~179; 192 staging (MIN_RESERVED) + 256
                 retention = 1.4x p99 read set; was 544 */
  };
  ...
  /* min_reserved is a runtime parameter; guard the Phase-1 cost
     multiplication against overflow before it wraps and makes the
     "budget too small" check below misbehave (a wrapped small value
     would pass the check and yield bogus class counts). */
  if( FD_UNLIKELY( min_reserved>ULONG_MAX/slot_sz_sum ) ) {
    FD_LOG_WARNING(( "cache_min_reserved %lu too large (overflows minimum cost)", min_reserved ));
    for( ulong c=0UL; c<FD_ACCDB_CACHE_CLASS_CNT; c++ ) class_cnt[ c ] = 0UL;
    return 0;
  }

  ulong minimum_cost = min_reserved * slot_sz_sum;
