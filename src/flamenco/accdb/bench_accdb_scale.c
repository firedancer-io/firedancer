...
main( int     argc,
  ...
  FD_LOG_NOTICE(( "  %7s %12s %12s %10s %10s %10s", "threads", "txn/s", "ns/txn", "ns/acc", "scaling", "backouts" ));
  for( ulong s=0UL; s<sweep_cnt; s++ ) {
    ...
    ulong failed = 0UL;
    for( ulong t=0UL; t<T; t++ ) failed += fd_accdb_metrics( joins[ t ] )->acquire_failed;
    double ns_per_txn = (double)busy_ns/(double)txn_cnt;
    if( !s ) single = ns_per_txn;
    FD_LOG_NOTICE(( "  %7lu %12.0f %12.0f %10.0f %9.2fx %10lu", T,
