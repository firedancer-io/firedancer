...
fd_accdb_shmem_footprint( ulong max_accounts,
  ...
  l = FD_LAYOUT_APPEND( l, alignof(uint),            max_accounts*sizeof(uint)                                );
...
fd_accdb_shmem_new( void * shmem,
  ...
  void * _deferred_acc_buf    = FD_SCRATCH_ALLOC_APPEND( l, alignof(uint),            max_accounts*sizeof(uint)                             );
  ...
  accdb->deferred_acc_buf_max = max_accounts;
