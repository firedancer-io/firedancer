...
#define FD_ACCDB_OFF_TOMBSTONE (FD_ACCDB_OFF_MASK-1UL)        /* zero-lamport account, index only */

static inline int
fd_accdb_offset_is_disk( ulong offset ) {
  return offset<FD_ACCDB_OFF_TOMBSTONE;
}
...
/* The `size` field in fd_accdb_disk_meta_t (named executable_size in
   ...
   The three in-memory-only flag bits exist only in the index, never on
   disk:
   ...
   Zero-lamport accounts are represented as index-only tombstones.  They
   have offset FD_ACCDB_OFF_TOMBSTONE, no cache binding, and canonical
   zero data length / executable metadata.  The offset is the sole type
   tag; no executable_size bit is reserved for tombstones.
