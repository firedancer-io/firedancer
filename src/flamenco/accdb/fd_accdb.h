...
/* fd_accdb_probe_pd_this_fork checks whether the newest version of
   ...
   returns 1, sets *out_pd_write to that version's pd_write flag, sets
   *out_data_len to its data length, and (if out_lamports!=NULL) sets
   *out_lamports to its lamports.  Otherwise returns 0, sets
   *out_pd_write to 0, and leaves *out_data_len / *out_lamports
   untouched.

   The pd_write gate does not consider lamports: a programdata closed
   this slot is a lamports==0 tombstone that must still report
   pd_write=1.  out_lamports is provided separately so loaded-account-
   size accounting can apply Agave's "zero-lamport account is
   nonexistent" rule to the current-fork version (the parent-fork copy
   the loader reads may still be funded).  Pass NULL when only the gate
   is needed.
