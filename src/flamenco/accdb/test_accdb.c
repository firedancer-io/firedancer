...
/* test_revert_whead: revert_whead releases partitions and restores
   disk_current_bytes.  Uses standard 1 GiB partitions (no compaction
   risk with small writes), but writes enough data to allocate at least
   one partition in each phase (full snapshot and incremental). */
static void
test_revert_whead( void ) {
  ...
  /* Full-snapshot load: write 5 accounts with 1 KiB data each.
     This allocates at least one partition. */
  ...
  for( ulong i=0UL; i<5UL; i++ ) {
    ...
    fd_accdb_snapshot_write_one( accdb, SENTINEL, snap_pks[ i ],
                                 10UL, (i+1UL)*100UL, 1024UL, 0, &replaced );
  }
  ...
  /* Incremental snapshot load: write 5 more accounts. */
  fd_accdb_snapshot_load_begin( accdb );
  ...
  for( ulong i=0UL; i<5UL; i++ ) {
    ...
    fd_accdb_snapshot_write_one( accdb, incr_fork, incr_pks[ i ],
                                 20UL, (i+1UL)*1000UL, 1024UL, 0, &replaced );
  }
  fd_accdb_snapshot_load_end( accdb );
