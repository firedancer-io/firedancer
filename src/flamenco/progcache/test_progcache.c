...
test_invalidate( fd_wksp_t * wksp ) {
  ...
  test_env_txn_prepare( env, &fork_a, &fork_b );
  fd_progcache_rec_t const * rec2 = fd_progcache_invalidate( env->progcache, &fork_b, &key, fork_b.ul[0], 1UL );
  FD_TEST( rec2!=rec );
  FD_TEST( !rec2->executable );
...
test_invalidate_nonexistent( fd_wksp_t * wksp ) {
  ...
  fd_funk_rec_key_t key = test_key( 1UL );
  fd_progcache_rec_t const * rec = fd_progcache_invalidate( env->progcache, &fork_a, &key, fork_a.ul[0], 1UL );
  FD_TEST( rec );
  FD_TEST( !rec->executable );
...
test_invalidate_dup( fd_wksp_t * wksp ) {
  ...
  fd_funk_txn_xid_t fork_c = { .ul = { 3UL, 2UL } };
  test_env_txn_prepare( env, &fork_b, &fork_c );
  fd_progcache_rec_t const * rec3 = fd_progcache_invalidate( env->progcache, &fork_c, &key, fork_c.ul[0], 1UL );
...
test_publish_gc( fd_wksp_t * wksp ) {
  ...
  fd_funk_txn_xid_t fork_b = { .ul = { 2UL, 1UL } };
  test_env_txn_prepare( env, &fork_a, &fork_b );
  fd_progcache_rec_t const * rec_b = fd_progcache_invalidate( env->progcache, &fork_b, &key, fork_b.ul[0], 1UL );
  FD_TEST( rec_b );
