...
#define FD_PROG_HEAD_LOCKED ULONG_MAX
...
static long
evict_one( fd_progcache_t *     cache,
           fd_progcache_rec_t * rec ) {
  return fd_prog_delete_rec( cache->join, rec );
}
  ...
  while( rec_rem>0L || heap_rem>0L ) {
    ...
    if( exists ) {
      ...
      if( visited ) {
        ...
        long res = evict_one( cache, rec0+head );
        if( res>=0L ) {
          rec_rem--;
          heap_rem -= res;
        }
