...
int
fd_voter_txn_parse( uchar             txn[static FD_TXN_MTU],
                    ulong             txn_sz,
                    fd_voter_vote_t * voter_votes_out ) {
  ...
  ushort                  ix_data_sz = parsed_txn->instr[0].data_sz;
  uchar *                 ix_data    = txn + parsed_txn->instr[0].data_off;
  ...
  if( FD_UNLIKELY( vote_instruction->discriminant != discriminants ||
