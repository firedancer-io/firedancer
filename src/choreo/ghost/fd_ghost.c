...
fd_ghost_new( void * shmem, ulong ele_max, ulong seed ) {
  ...
  void *       hash  = FD_SCRATCH_ALLOC_APPEND( l, fd_ghost_map_hash_align(), fd_ghost_map_hash_footprint( ele_max ) );
  void *       slot  = FD_SCRATCH_ALLOC_APPEND( l, fd_ghost_map_slot_align(), fd_ghost_map_slot_footprint( ele_max ) );
  ...
  ghost->map_hash_gaddr = fd_wksp_gaddr_fast( wksp, fd_ghost_map_hash_join( fd_ghost_map_hash_new( hash, ele_max, seed ) ) );
  ghost->map_slot_gaddr = fd_wksp_gaddr_fast( wksp, fd_ghost_map_slot_join( fd_ghost_map_slot_new( slot, ele_max, seed ) ) );
...
maps_insert( fd_ghost_t * ghost, fd_ghost_ele_t * ele ) {
  fd_ghost_map_hash_t * maph = fd_ghost_map_hash( ghost );
  fd_ghost_map_slot_t * maps = fd_ghost_map_slot( ghost );
  fd_ghost_ele_t      * pool = fd_ghost_pool( ghost );

  FD_TEST( fd_ghost_map_hash_ele_insert( maph, ele, pool ) ); /* cannot fail */
  if( FD_LIKELY( fd_ghost_map_slot_ele_query( maps, &ele->slot, NULL, pool ) == NULL ) ) {
   fd_ghost_map_slot_ele_insert( maps, ele, pool ); /* cannot fail */
  }
}

/* Removes all occurrences of `block_id` from the maps. */
static fd_ghost_ele_t *
maps_remove( fd_ghost_t * ghost, fd_hash_t * block_id ) {
  fd_ghost_map_hash_t * maph = fd_ghost_map_hash( ghost );
  fd_ghost_map_slot_t * maps = fd_ghost_map_slot( ghost );
  fd_ghost_ele_t      * pool = fd_ghost_pool( ghost );

  fd_ghost_ele_t * ele = fd_ghost_map_hash_ele_remove( maph, block_id, NULL, pool );
  if( FD_LIKELY( ele ) ) {
    fd_ghost_ele_t * eles = fd_ghost_map_slot_ele_query( maps, &ele->slot, NULL, pool );
    if( FD_LIKELY( eles && memcmp( &ele->key, block_id, sizeof(fd_hash_t) ) == 0 ) ) {
      fd_ghost_map_slot_ele_remove( maps, &ele->slot, NULL, pool );
    }
  }
  return ele;
}
...
void
fd_ghost_init( fd_ghost_t * ghost, ulong root_slot, fd_hash_t * block_id ) {
  ...
  FD_TEST( fd_ghost_root( ghost ) == fd_ghost_query( ghost, block_id  ) );
...
fd_ghost_verify( fd_ghost_t const * ghost ) {
  ...
  ulong                       null = fd_ghost_pool_idx_null( pool );
  fd_ghost_map_hash_t const * maph = fd_ghost_map_hash_const( ghost );

  /* Check every ele that exists in pool exists in map. */

  if( fd_ghost_map_hash_verify( maph, fd_ghost_pool_max( pool ), pool ) ) return -1;
...
fd_ghost_replay_vote( fd_ghost_t * ghost, fd_voter_t * voter, fd_hash_t const * vote_bid ) {
  ...
  fd_ghost_ele_t * ele = fd_ghost_query( ghost, &vote.block_id );
...
fd_ghost_rooted_vote( fd_ghost_t * ghost, fd_voter_t * voter, ulong root ) {
  ...
  fd_ghost_ele_t * ele = fd_ghost_query( ghost, fd_ghost_block_id( ghost, root ) );
# if FD_GHOST_USE_HANDHOLDING
...
fd_ghost_publish( fd_ghost_t * ghost, fd_hash_t * block_id ) {
  ...
  fd_ghost_ele_t * newr = fd_ghost_query( ghost, block_id );

# if FD_GHOST_USE_HANDHOLDING
  if( FD_UNLIKELY( !newr                                                  ) ) { FD_LOG_WARNING(( "[%s] publish block_id %s not found",      __func__, FD_BASE58_ENC_32_ALLOCA(block_id) )); return NULL; }
...
int
fd_ghost_is_ancestor( fd_ghost_t const * ghost, fd_hash_t const * ancestor, fd_hash_t const * block_id ) {
  fd_ghost_ele_t const * root = fd_ghost_root_const( ghost );
  fd_ghost_ele_t const * curr = fd_ghost_query_const( ghost, block_id );
...
# if FD_GHOST_USE_HANDHOLDING
  ...
  if( FD_UNLIKELY( !curr                  ) ) { FD_LOG_WARNING(( "[%s] block_id %s not in ghost.",       __func__, FD_BASE58_ENC_32_ALLOCA(block_id) )); return 0; }
# endif
