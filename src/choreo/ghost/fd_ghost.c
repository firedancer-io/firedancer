...
/* Removes all occurrences of `hash_id` from the maps. */
static fd_ghost_ele_t *
maps_remove( fd_ghost_t * ghost, fd_hash_t * hash_id ) {
  fd_ghost_hash_map_t * maph = fd_ghost_hash_map( ghost );
  fd_ghost_slot_map_t * maps = fd_ghost_slot_map( ghost );
  fd_ghost_ele_t      * pool = fd_ghost_pool( ghost );

  fd_ghost_ele_t * ele = fd_ghost_hash_map_ele_remove( maph, hash_id, NULL, pool );
  if( FD_LIKELY( ele ) ) {
    fd_ghost_ele_t * eles = fd_ghost_slot_map_ele_query( maps, &ele->slot, NULL, pool );
    if( FD_LIKELY( eles && memcmp( &ele->key, hash_id, sizeof(fd_hash_t) ) == 0 ) ) {
...
void
fd_ghost_init( fd_ghost_t * ghost, ulong root_slot, fd_hash_t * hash_id ) {
  ...
  FD_TEST( fd_ghost_root( ghost ) == fd_ghost_query( ghost, hash_id  ) );
...
fd_ghost_ele_t *
fd_ghost_insert( fd_ghost_t * ghost, fd_hash_t const * parent_hash, ulong slot, fd_hash_t const * hash_id ) {
  VER_INC;
  FD_LOG_NOTICE(( "[%s] slot: %lu, %s. parent: %s.", __func__, slot, FD_BASE58_ENC_32_ALLOCA(hash_id), FD_BASE58_ENC_32_ALLOCA(parent_hash) ));
...
# if FD_GHOST_USE_HANDHOLDING
  if( FD_UNLIKELY( fd_ghost_query( ghost, hash_id ) ) ) { FD_LOG_WARNING(( "[%s] hash_id %s already in ghost.", __func__, FD_BASE58_ENC_32_ALLOCA(hash_id)  )); return NULL; }
  if( FD_UNLIKELY( !parent                      ) ) { FD_LOG_WARNING(( "[%s] missing `parent_id` %s.",       __func__, FD_BASE58_ENC_32_ALLOCA(parent_hash) )); return NULL; }
...
fd_ghost_replay_vote( fd_ghost_t * ghost, fd_voter_t * voter, fd_hash_t const * vote_bid ) {
  ...
  FD_LOG_DEBUG(( "[%s] %s replay vote %s, %lu, last: %lu", __func__, FD_BASE58_ENC_32_ALLOCA(&voter->key), FD_BASE58_ENC_32_ALLOCA(vote_bid), slot, vote.slot ));
  ...
  if( FD_UNLIKELY( memcmp( &vote.hash, vote_bid, sizeof(fd_hash_t) ) == 0 ) ) return;
  ...
  voter->replay_vote.hash = *vote_bid; /* update the cached replay vote hash_id on voter */
}
...
fd_ghost_rooted_vote( fd_ghost_t * ghost, fd_voter_t * voter, ulong root ) {
  ...
  fd_ghost_ele_t * ele = fd_ghost_query( ghost, fd_ghost_hash_id( ghost, root ) );
# if FD_GHOST_USE_HANDHOLDING
...
fd_ghost_publish( fd_ghost_t * ghost, fd_hash_t const * hash_id ) {
  ...
  fd_ghost_ele_t * newr = fd_ghost_query( ghost, hash_id );

# if FD_GHOST_USE_HANDHOLDING
  if( FD_UNLIKELY( !newr                                                  ) ) { FD_LOG_WARNING(( "[%s] publish hash_id %s not found",      __func__, FD_BASE58_ENC_32_ALLOCA(hash_id) )); return NULL; }
...
int
fd_ghost_is_ancestor( fd_ghost_t const * ghost, fd_hash_t const * ancestor, fd_hash_t const * hash_id ) {
  fd_ghost_ele_t const * root = fd_ghost_root_const( ghost );
  fd_ghost_ele_t const * curr = fd_ghost_query_const( ghost, hash_id );
...
# if FD_GHOST_USE_HANDHOLDING
  ...
  if( FD_UNLIKELY( !curr                  ) ) { FD_LOG_WARNING(( "[%s] hash_id %s not in ghost.",        __func__, FD_BASE58_ENC_32_ALLOCA(hash_id) )); return 0; }
# endif
