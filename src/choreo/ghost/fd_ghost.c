...
void *
fd_ghost_new( void * shmem, ulong seed, ulong node_max, ulong vote_max ) {
...
fd_ghost_replay_vote( fd_ghost_t * ghost, fd_voter_t * voter, ulong slot ) {
  ...
  /* Return early if this new vote slot <= last tower vote. It is
  ...
  if( FD_UNLIKELY( slot <= voter->vote ) ) return;

  /* LMD-rule: subtract the voter's stake from previous vote. */

  if( FD_LIKELY( voter->vote != FD_SLOT_NULL && voter->vote >= root->slot ) ) {
    FD_LOG_DEBUG(( "[ghost] removing (%s, %lu, %lu)", FD_BASE58_ENC_32_ALLOCA( &voter->key ), voter->stake, voter->vote ));

    fd_ghost_node_t * node = fd_ghost_node_map_ele_query( node_map, &voter->vote, NULL, node_pool );
    #if FD_GHOST_USE_HANDHOLDING
    ...
    if( FD_UNLIKELY( cf ) ) FD_LOG_ERR(( "[%s] sub overflow. node->stake %lu latest_vote->stake %lu", __func__, node->replay_stake, voter->stake ));
  ...
  FD_LOG_DEBUG(( "[ghost] adding (%s, %lu, %lu)", FD_BASE58_ENC_32_ALLOCA( &voter->key ), voter->stake, slot ));
  ...
  voter->vote = slot; /* update the cached vote slot on voter */
}
