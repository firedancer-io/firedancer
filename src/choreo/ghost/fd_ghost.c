...
fd_ghost_mark_invalid( fd_ghost_t * ghost, ulong slot, ulong total_stake ) {
  ...
  while( FD_UNLIKELY( ele->twin != null ) ) {
    ...
    if( FD_LIKELY( !is_duplicate_confirmed( ghost, &twin->key, total_stake ) ) ) twin->valid = 0;
    ele = twin;
  }
}
...
fd_ghost_insert( fd_ghost_t * ghost, fd_hash_t const * parent_hash, ulong slot, fd_hash_t const * hash_id, ulong total_stake ) {
  VER_INC;
  FD_LOG_DEBUG(( "[%s] slot: %lu, %s. parent: %s.", __func__, slot, FD_BASE58_ENC_32_ALLOCA(hash_id), FD_BASE58_ENC_32_ALLOCA(parent_hash) ));
