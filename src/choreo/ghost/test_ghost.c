...
test_duplicate_simple( fd_wksp_t * wksp ){
  ...
  FD_TEST( memcmp( fd_ghost_hash_id( ghost, 1 ), &hash_1, sizeof(fd_hash_t) ) == 0 );
  FD_TEST( memcmp( fd_ghost_hash_id( ghost, 2 ), &hash_2, sizeof(fd_hash_t) ) == 0 );
  FD_TEST( memcmp( fd_ghost_hash_id( ghost, 3 ), &hash_3, sizeof(fd_hash_t) ) == 0 );
  FD_TEST( memcmp( fd_ghost_hash_id( ghost, 4 ), &hash_4, sizeof(fd_hash_t) ) == 0 );
...
run_test_state_duplicate_then_bank_frozen( fd_wksp_t * wksp ) {
  ...
  FD_TEST( fd_ghost_hash_id( ghost, 2 ) == NULL );
...
test_state_descendant_confirmed_ancestor_duplicate( fd_wksp_t * wksp ){
  ...
  for( ulong slot = 0; slot < 4; slot++ ) {
    fd_ghost_ele_t const * ele = fd_ghost_query( ghost, fd_ghost_hash_id( ghost, slot ) );
    FD_TEST( ele->valid );
