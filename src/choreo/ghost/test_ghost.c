...
test_ghost_simple( fd_ghost_t * ghost ) {
  ...
  fd_ghost_lmd_update(ghost, &sh2, &pk1, 1);

  fd_ghost_print( ghost );

  fd_slot_hash_t sh3 = { .slot = 3, .hash = pubkey_null };
  fd_ghost_lmd_update(ghost, &sh3, &pk1, 1);
...
main( int argc, char ** argv ) {
  ...
  void * mem        = fd_wksp_alloc_laddr(
      wksp, fd_ghost_align(), fd_ghost_footprint( node_max, lg_msg_max ), 1UL );
  FD_TEST( mem );
  fd_ghost_t * ghost = fd_ghost_join( fd_ghost_new( mem, node_max, lg_msg_max, 0UL ) );
  FD_TEST( ghost );
  FD_TEST( FD_SLOT_HASH_EQ( &ghost->root, &slot_hash_null ) );
  FD_TEST( ghost->node_pool );
  FD_TEST( ghost->node_map );
  FD_TEST( ghost->latest_msgs );
