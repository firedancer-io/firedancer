...
assert_rejected( signer_t const * signer,
  ...
  FD_TEST( fd_tower_file_de( buf, sz, &signer->pub, &out )<0 );
  FD_TEST( fd_memeq( &out, &expected, sizeof(out) ) );
}
...
main( int     argc,
  ...
  FD_TEST( fd_memeq( &out.bank_hash, &bank_hash, sizeof(fd_hash_t) ) );
  FD_TEST( fd_memeq( &out.block_id,  &block_id,  sizeof(fd_hash_t) ) );

  uchar valid_buf[ FD_TOWER_FILE_MAX ];
  fd_memcpy( valid_buf, file_buf, (ulong)sz );
  ...
  FD_TEST( fd_tower_file_de( file_buf, (ulong)sz, &other, &out )==FD_TOWER_FILE_ERR_SIG );
  ...
  /* Both block identifiers are signed.  The decoder returns the signed
     values, not whatever replay had at that slot. */
  ...
  fd_memcpy( file_buf, valid_buf, (ulong)sz );
  ...
  /* last_timestamp is decoded as stored, Agave checks nothing about it. */
  fd_memcpy( file_buf, valid_buf, (ulong)sz );
  FD_STORE( long, file_buf+(ulong)sz-8UL, 1234568L );
  resign( &signer, file_buf, (ulong)sz );
  FD_TEST( !fd_tower_file_de( file_buf, (ulong)sz, &signer.pub, &out ) );
  FD_TEST( out.timestamp==1234568L );
  ...
  /* The first vote expired by the newest slot, still a valid tower,
     Agave pops expired votes from the top only. */
  second_vote->slot = 107UL;
  third_vote->slot  = 111UL;
  long sz_expired = fd_tower_file_ser( votes, 90UL, &bank_hash, &block_id, 1234567L,
                                       &signer.pub, test_sign, &signer, file_buf, sizeof(file_buf) );
  FD_TEST( sz_expired>0L );
  FD_TEST( !fd_tower_file_de( file_buf, (ulong)sz_expired, &signer.pub, &out ) );
  FD_TEST( out.votes[ 2 ].slot==111UL );
  second_vote->slot = 101UL;
  third_vote->slot  = 105UL;
  ...
  fd_memcpy( agave2, file_buf, (ulong)sz2 );
  fd_sha512_t agave2_sha[ 1 ];
  fd_ed25519_sign( agave2+4UL, agave2+76UL, (ulong)sz2-76UL, signer.pub.uc, signer.prv, agave2_sha );
  FD_TEST( fd_memeq( fd_sha256_hash( agave2, (ulong)sz2, hash ), expected_no_root_hash, 32UL ) );
  FD_TEST( !fd_tower_file_de( file_buf, (ulong)sz2, &signer.pub, &out ) );
