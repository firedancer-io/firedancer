...
fd_tower_checkpt( fd_tower_t const *      tower,
  ...
  /* TODO check no accidental dangling pointers... */
  ...
  ulong buf_sz; err = fd_tower_serialize( &ser, pvtkey, pubkey, buf, buf_max, &buf_sz );
...
fd_tower_restore( fd_tower_t * tower,
  ...
  fd_tower_deserialize( buf, *buf_sz, pubkey, &de );
...
fd_tower_serialize( fd_tower_serde_t * ser,
  ...
  #define SER( T, name ) do {                                                                 \
      ...
      if( FD_LIKELY( ser->name ) ) {                                                          \
        ...
        ser->name = (T *)fd_type_pun( buf+off );                                              \
      }                                                                                       \
      off += sizeof(T);                                                                       \
  } while(0)
...
fd_tower_deserialize( uchar *            buf,
  ...
  for( ulong i = 0; i < 32; i++ ) {
    ...
    // FD_LOG_NOTICE(( "off %lu pubkey %s start_epoch %lu end_epoch %lu", off, FD_BASE58_ENC_32_ALLOCA( de->vote_state.prior_voters.buf[i].pubkey->uc ), *de->vote_state.prior_voters.buf[i].start_epoch, *de->vote_state.prior_voters.buf[i].end_epoch ));
  }
  ...
  uchar *     sig    = *de->signature;
  fd_sha512_t sha[1];
  int err = fd_ed25519_verify( msg, msg_sz, sig, pubkey, sha );
