...
fd_tower_verify( fd_tower_t const * tower ) {
  if( FD_UNLIKELY( fd_tower_votes_cnt( tower ) >= FD_TOWER_VOTE_MAX ) ) {
    FD_LOG_WARNING(( "[%s] invariant violation: cnt %lu >= FD_TOWER_VOTE_MAX %lu",
                     __func__, fd_tower_votes_cnt( tower ), (ulong)FD_TOWER_VOTE_MAX ));
    return -1;
  }
  ...
  for( fd_tower_votes_iter_t iter=fd_tower_votes_iter_init( tower );
    ...
    if( FD_UNLIKELY( prev && ( vote->slot < prev->slot || vote->conf < prev->conf ) ) ) {
      FD_LOG_WARNING(( "[%s] invariant violation: vote (slot:%lu conf:%lu) prev (slot:%lu conf:%lu)",
                       __func__, vote->slot, vote->conf, prev->slot, prev->conf ));
      return -1;
    }
    prev = vote;
  }
