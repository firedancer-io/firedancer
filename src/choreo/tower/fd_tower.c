...
fd_tower_lockout_check( fd_tower_t const * tower,
  ...
  int lockout_check = vote->slot < root->slot ||
                      fd_ghost_is_ancestor( ghost, fd_ghost_hash_id( ghost, vote->slot ), block_id );
...
fd_tower_switch_check( fd_tower_t const * tower,
  ...
  */
  ...
  #if FD_TOWER_USE_HANDHOLDING
  FD_TEST( !fd_ghost_is_ancestor( ghost, fd_ghost_hash_id( ghost, vote->slot ), switch_block_id ) );
  #endif
  fd_hash_t     const * vote_block_id = fd_ghost_hash_id( ghost, vote->slot );
