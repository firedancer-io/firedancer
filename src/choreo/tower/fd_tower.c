...
fd_tower_lockout_check( fd_tower_t const * tower,
  ...
  int lockout_check = vote->slot < root->slot ||
                      fd_ghost_is_ancestor( ghost, fd_ghost_block_id( ghost, vote->slot ), block_id );
...
fd_tower_switch_check( fd_tower_t const * tower,
                       ...
                       fd_ghost_t const * ghost,
                       ulong              switch_slot,
                       fd_hash_t const *  switch_block_id ) {
  #if FD_TOWER_USE_HANDHOLDING
  FD_TEST( !fd_tower_votes_empty( tower ) ); /* caller error */
  #endif
  ...
  */
  ...
  #if FD_TOWER_USE_HANDHOLDING
  FD_TEST( !fd_ghost_is_ancestor( ghost, fd_ghost_block_id( ghost, vote->slot ), switch_block_id ) );
  #endif
  fd_hash_t     const * vote_block_id = fd_ghost_block_id( ghost, vote->slot );
  fd_ghost_map_hash_t const * maph    = fd_ghost_map_hash_const( ghost );
  fd_ghost_ele_t      const * pool    = fd_ghost_pool_const( ghost );
  fd_ghost_ele_t      const * gca     = fd_ghost_gca( ghost, vote_block_id, switch_block_id );
  ulong                       gca_idx = fd_ghost_map_hash_idx_query_const( maph, &gca->key, ULONG_MAX, pool );
