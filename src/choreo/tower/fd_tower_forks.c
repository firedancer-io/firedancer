...
fd_forks_new( void * shmem, ulong slot_max, ulong voter_max ) {
  ...
  forks->lockout_slots_pool     = fd_lockout_slots_pool_join    ( fd_lockout_slots_pool_new    ( lockout_slots_pool, slot_max    ) );
...
void
fd_forks_add_lockouts( fd_forks_t * forks, ulong fork_slot, fd_hash_t const * vote_account_pubkey, fd_tower_accts_t * accts ) {
  ...
  for( fd_tower_iter_t iter = fd_tower_iter_init( scratch_tower );
    ...
    if( !fd_lockout_intervals_map_ele_query( forks->lockout_intervals_map, &key, NULL, forks->lockout_intervals_pool ) ) {
      ...
      fd_lockout_slots_map_ele_insert( forks->lockout_slots_map, slot, forks->lockout_slots_pool );
    }
    ...
    fd_lockout_intervals_map_ele_insert( forks->lockout_intervals_map, interval, forks->lockout_intervals_pool );
  }
}
