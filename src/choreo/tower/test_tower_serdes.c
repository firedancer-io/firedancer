...
/* Helper: serialize a valid CompactTowerSync into buf.  Returns the
   ...
   are generated with offset=1 and confirmation_count=i+1 (1..cnt,
   strictly increasing, all <= FD_TOWER_VOTE_MAX). */
...
make_valid( uchar * buf, ulong buf_max, ushort lockouts_cnt, int with_timestamp ) {
  ...
  for( ushort i = 0; i < lockouts_cnt; i++ ) {
    ...
    serde->lockouts[i].confirmation_count = (uchar)(i + 1);
  }
...
test_de_attacker( void ) {
  ...
  /* Sanity: max lockouts (31) is valid.  This also exercises
     confirmation_count == FD_TOWER_VOTE_MAX (31) for the last lockout,
     which is the boundary value (> 31 is rejected, <= 31 is accepted). */

  {
    ...
    FD_TEST( serde->lockouts[FD_TOWER_VOTE_MAX-1].confirmation_count==FD_TOWER_VOTE_MAX );
  }
  ...
  /* 7. Zero offset (offset must be > 0). */

  {
    ulong sz = make_valid( buf, sizeof(buf), 1, 0 );
    FD_TEST( sz );
    buf[9] = 0; /* overwrite first lockout offset to 0 */
    FD_TEST( -1==fd_compact_tower_sync_de( serde, buf, sz ) );
  }

  /* 8. confirmation_count > FD_TOWER_VOTE_MAX (32). */

  {
    ulong sz = make_valid( buf, sizeof(buf), 1, 0 );
    FD_TEST( sz );
    /* offset is at buf[9]=1 (valid), confirmation_count is at buf[10] */
    buf[10] = (uchar)(FD_TOWER_VOTE_MAX + 1); /* 32, which is > FD_TOWER_VOTE_MAX */
    FD_TEST( -1==fd_compact_tower_sync_de( serde, buf, sz ) );
  }

  /* 9. confirmation_count = 255 (maximum u8). */
  ...
  /* 10. Non-increasing confirmation counts: two lockouts both with
         confirmation_count=1. */

  {
    ulong sz = make_valid( buf, sizeof(buf), 2, 0 );
    FD_TEST( sz );
    /* lockout[0] at buf[9]: offset=1, conf=1
       lockout[1] at buf[11]: offset=1, conf=2
       overwrite lockout[1].conf to 1 (same as lockout[0]) */
    buf[12] = 1;
    FD_TEST( -1==fd_compact_tower_sync_de( serde, buf, sz ) );
  }

  /* 11. Decreasing confirmation counts. */

  {
    ...
    s->root         = 1UL;
    s->lockouts_cnt = 3;
    s->lockouts[0].offset = 1; s->lockouts[0].confirmation_count = 5;
    s->lockouts[1].offset = 1; s->lockouts[1].confirmation_count = 10;
    s->lockouts[2].offset = 1; s->lockouts[2].confirmation_count = 7; /* decreases */
  ...
  /* 12. timestamp_option = 2 (must be 0 or 1). */
  ...
  /* 14. Truncated: timestamp_option=1 but no timestamp bytes. */
  ...
  /* 15. Truncated block_id: missing the last byte of block_id. */
  ...
  /* 16. VarInt with non-minimal encoding (leading zero continuation
         byte).  A valid VarInt for value 0 is a single 0x00 byte.
         0x80 0x00 is a non-minimal encoding of 0 which should also be
         rejected because offset==0 is invalid. */
  ...
  /* 17. VarInt overflow: 10 continuation bytes (forces > 64 bits of
  ...
  /* 18. ShortU16 truncation: the first byte has continuation bit set
  ...
  /* 19. VarInt truncation: continuation bit set but buffer ends. */
  ...
  /* 20. Large valid VarInt offset (ULONG_MAX-1 fits in 10 VarInt bytes
  ...
  /* 21. Exactly 1 byte short everywhere: test progressive truncation
  ...
  /* 22. Progressive truncation of a valid 1-lockout message with
         timestamp. */
