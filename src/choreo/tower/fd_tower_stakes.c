...
fd_tower_stakes_new( void * shmem,
  ...
  tower_stakes->vtr_map          = fd_tower_stakes_vtr_map_join ( fd_tower_stakes_vtr_map_new ( voter_stake_map,  FD_VOTER_MAX * slot_max, 0 ) );
  tower_stakes->vtr_pool         = fd_tower_stakes_vtr_pool_join( fd_tower_stakes_vtr_pool_new( voter_stake_pool, FD_VOTER_MAX * slot_max )    );
  tower_stakes->blk_map          = fd_tower_stakes_blk_join     ( fd_tower_stakes_blk_new     ( tower_stakes_blk, lg_slot_cnt, 0UL )           );      /* FIXME seed? */
...
fd_tower_stakes_vtr_insert( fd_tower_stakes_t * tower_stakes,
  ...
  fd_tower_stakes_blk_t * blk = fd_tower_stakes_blk_query( tower_stakes->blk_map, slot, NULL );
  if( FD_UNLIKELY( !blk ) ) {
    blk = fd_tower_stakes_blk_insert( tower_stakes->blk_map, slot );
  }
...
fd_tower_stakes_blk_prune( fd_tower_stakes_t     * tower_stakes,
  ...
  fd_tower_stakes_blk_remove( tower_stakes->blk_map, blk );
}
