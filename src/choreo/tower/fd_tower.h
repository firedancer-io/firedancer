...
/* fd_tower_sync_serde defines a serialization / deserialization schema
   for a bincode-encoded TowerSync transaction.  The serde is structured
   for zero-copy access ie. x-raying individual fields. */
...
/* fd_tower_serde defines a serialization / deserialization schema for
...
fd_tower_sync_serde( fd_tower_t const * tower, ulong root, fd_hash_t * bank_hash, fd_hash_t * block_id, long ts, fd_tower_sync_serde_t * ser );
