...
#define STORE_BYTES( ptr, sz ) do {                        \
    ...
    fd_memcpy( buf+off, (ptr), (sz) );                        \
    off += (sz);                                           \
  } while(0)

#define STORE_ZERO( sz ) do {                              \
    if( FD_UNLIKELY( (sz)>buf_max-off ) ) return -1L;      \
    fd_memset( buf+off, 0, (sz) );                            \
    off += (sz);                                           \
  } while(0)
...
tower_sync_set_votes( fd_compact_tower_sync_serde_t * serde,
  ...
  if( FD_UNLIKELY( decoded_cnt!=votes_cnt || decoded_root!=root ||
                   !fd_memeq( decoded, votes, votes_cnt*sizeof(fd_tower_vote_t) ) ) ) return -1;
  return 0;
}
...
fd_tower_file_sign_msg( uchar const * body,
  ...
  fd_memcpy( out, FD_KEYGUARD_TOWER_FILE_PREFIX, FD_KEYGUARD_TOWER_FILE_PREFIX_SZ );
  fd_sha256_hash( body, body_sz, out+FD_KEYGUARD_TOWER_FILE_PREFIX_SZ );
}
...
fd_tower_file_de( uchar const *       buf,
  ...
  fd_sha512_t sha[ 1 ];
  fd_tower_file_sign_msg( buf+DATA_OFF, data_sz, msg );
  if( FD_UNLIKELY( FD_ED25519_SUCCESS!=fd_ed25519_verify( msg, sizeof(msg), sig, identity->uc, sha ) ) ) return -1;

  if( FD_UNLIKELY( 32UL>buf_sz-off ) ) return -1;
  if( FD_UNLIKELY( !fd_memeq( buf+off, identity->uc, 32UL ) ) ) return -1;
  off += 32UL;
  ...
  if( FD_UNLIKELY( canonical_sz!=sync_sz || !fd_memeq( sync_buf, buf+off, sync_sz ) ) ) return -1;
