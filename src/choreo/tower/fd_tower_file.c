...
fd_tower_file_sign_msg( uchar const * body,
                        ulong         body_sz,
                        uchar         out[ static 32 ] ) {
  fd_sha256_hash( body, body_sz, out );
  fd_sha256_hash( out, 32UL, out );
}
...
fd_tower_file_de( uchar const *       buf,
  ...
  if( FD_UNLIKELY( FD_ED25519_SUCCESS!=fd_ed25519_verify( msg, 32UL, sig, identity->uc, sha ) ) ) return -1;
  ...
  memset( out, 0, sizeof(fd_tower_file_t) );
  memcpy( out->votes, votes, votes_cnt*sizeof(fd_tower_vote_t) );
