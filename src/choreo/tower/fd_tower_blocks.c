...
fd_tower_blocks_new( void * shmem, ulong slot_max, ulong voter_max ) {
  ...
  void *       tower_forks        = FD_SCRATCH_ALLOC_APPEND( l, fd_tower_block_align(),            fd_tower_block_footprint   ( lg_slot_max )          );
  ...
  blocks->tower_forks            = fd_tower_block_join           ( fd_tower_block_new           ( tower_forks, lg_slot_max, 0UL ) ); /* FIXME seed */
...
int
is_ancestor( fd_tower_block_t * forks,
             ulong              slot,
             ulong              ancestor_slot ) {
  fd_tower_block_t * anc = fd_tower_block_query( forks, slot, NULL );
  while( FD_LIKELY( anc ) ) {
    if( FD_LIKELY( anc->parent_slot == ancestor_slot ) ) return 1;
    anc = anc->parent_slot == ULONG_MAX ? NULL : fd_tower_block_query( forks, anc->parent_slot, NULL );
  }
  return 0;
}
...
fd_tower_blocks_lowest_common_ancestor( fd_tower_blocks_t * forks,
                                 ulong        slot1,
                                 ulong        slot2 ) {
  fd_tower_block_t * fork1 = fd_tower_block_query( forks->tower_forks, slot1, NULL );
  fd_tower_block_t * fork2 = fd_tower_block_query( forks->tower_forks, slot2, NULL );
  ...
  while( FD_LIKELY( fork1 && fork2 ) ) {
    ...
    if( fork1->slot > fork2->slot                 ) fork1 = fd_tower_block_query( forks->tower_forks, fork1->parent_slot, NULL );
    else                                            fork2 = fd_tower_block_query( forks->tower_forks, fork2->parent_slot, NULL );
  }
...
fd_tower_blocks_canonical_block_id( fd_tower_blocks_t * forks,
                             ulong        slot ) {
  fd_tower_block_t * fork = fd_tower_block_query( forks->tower_forks, slot, NULL );
...
fd_tower_blocks_link( fd_tower_blocks_t * forks, ulong slot, ulong parent_slot ) {
  fd_tower_block_t * fork = fd_tower_block_insert( forks->tower_forks, slot );
...
fd_tower_blocks_insert( fd_tower_blocks_t *      forks,
  ...
  memset( fork, 0, sizeof(fd_tower_block_t) );
  fork->parent_slot = parent_slot;
  fork->slot        = slot;
  return fork;
}
...
fd_tower_blocks_query( fd_tower_blocks_t * forks, ulong slot ) {
  return fd_tower_block_query( forks->tower_forks, slot, NULL );
}
...
fd_tower_blocks_remove( fd_tower_blocks_t * forks, ulong slot ) {
  ...
  fd_tower_block_remove( forks->tower_forks, fork );
