...
static inline ulong
fd_lockout_interval_start( ulong key ) {
  return key >> 32;
}

static inline ulong
fd_lockout_interval_fork_slot( ulong key ) {
...
fd_forks_footprint( ulong slot_max, ulong voter_max ) {
  ...
  return FD_LAYOUT_FINI(
    ...
    FD_LAYOUT_INIT,
      ...
      fd_lockout_slots_pool_align(),     fd_lockout_slots_pool_footprint    ( slot_max ) ),
...
void
fd_forks_add_lockouts( fd_forks_t * forks,
                       ulong fork_slot,
                       fd_hash_t const * vote_account_pubkey,
                       fd_tower_accts_t * acct );

void
fd_forks_clear_lockouts( fd_forks_t * forks, ulong fork_slot );
