...
sign_tower( void *        ctx FD_PARAM_UNUSED,
  ...
  fd_ed25519_sign( sig, m, 32UL, public_key->uc, private_key, sha );
}
...
LLVMFuzzerTestOneInput( uchar const * data,
  ...
  fd_tower_file_t out;
  int err = fd_tower_file_de( buf, sz, public_key, &out );
  if( !err ) {
    FD_FUZZ_MUST_BE_COVERED;
    FD_TEST( out.votes_cnt && out.votes_cnt<=FD_TOWER_VOTE_MAX );
    for( ulong i=1UL; i<out.votes_cnt; i++ ) {
      FD_TEST( out.votes[ i-1UL ].slot<out.votes[ i ].slot );
      FD_TEST( out.votes[ i-1UL ].conf>out.votes[ i ].conf );
    }
  }
  return 0;
}
