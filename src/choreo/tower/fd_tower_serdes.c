...
de_short_u16( ushort * dst, uchar const * src, ulong src_sz ) {
  if( FD_UNLIKELY( src_sz<1UL ) ) return -1;
  if( FD_LIKELY( !(0x80U & src[0]) ) ) {
    *dst = (ushort)src[0];
    return 1;
  }
  if( FD_UNLIKELY( src_sz<2UL ) ) return -1;
  if( FD_LIKELY( !(0x80U & src[1]) ) ) {
    *dst = (ushort)((ulong)(src[0]&0x7FUL) + (((ulong)src[1])<<7));
    return 2;
  }
  if( FD_UNLIKELY( src_sz<3UL ) ) return -1;
  *dst = (ushort)((ulong)(src[0]&0x7FUL) + (((ulong)(src[1]&0x7FUL))<<7) + (((ulong)src[2])<<14));
  return 3;
}
...
de_var_int( ulong * dst, uchar const * src, ulong src_sz ) {
  *dst = 0;
  ...
  while( FD_LIKELY( bit < 64 ) ) {
    if( FD_UNLIKELY( off>=src_sz ) ) return -1;
    uchar byte = src[off];
    off       += 1;
    *dst      |= (byte & 0x7FUL) << bit;
...
fd_compact_tower_sync_de( fd_compact_tower_sync_serde_t * serde,
  ...
  int   sz  = 0;
  DE( ulong, root );
  rem = off<=buf_sz ? buf_sz-off : 0UL;
  sz  = de_short_u16( &serde->lockouts_cnt, buf+off, rem );
  if( FD_UNLIKELY( sz<0 ) ) return -1;
  off += (ulong)sz;
  if( FD_UNLIKELY( serde->lockouts_cnt > FD_TOWER_VOTE_MAX ) ) return -1;
  for( ulong i = 0; i < serde->lockouts_cnt; i++ ) {
    rem = off<=buf_sz ? buf_sz-off : 0UL;
    sz  = de_var_int( &serde->lockouts[i].offset, buf+off, rem );
    if( FD_UNLIKELY( sz<0 ) ) return -1;
    off += (ulong)sz;
