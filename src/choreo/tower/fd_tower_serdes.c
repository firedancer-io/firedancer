...
fd_txn_parse_simple_vote( fd_txn_t const *                txn,
  ...
  if( FD_UNLIKELY( kind == FD_VOTE_IX_KIND_TOWER_SYNC || kind == FD_VOTE_IX_KIND_TOWER_SYNC_SWITCH ) ) {
    ...
    /* Agave's vote program decodes this with limited_deserialize, which
       allows trailing bytes, so a vote with them still lands on chain
       and must count here too. */
    fd_compact_tower_sync_serde_t compact_tower_sync_serde[ 1 ];
    int err = fd_compact_tower_sync_de( compact_tower_sync_serde, instr_data+sizeof(uint), tower_sync_sz );
    if( FD_LIKELY( !err ) ) {
