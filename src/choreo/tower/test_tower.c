...
test_serde( void ) {
  ...
  uchar const pvtkey[32] = { 0xc9, 0x2a, 0x1a, 0x82, 0xec, 0x83, 0x65, 0x31, 0x0d, 0x44, 0xf9, 0xe9, 0xe7, 0x4a, 0xf8, 0x72, 0x50, 0xb5, 0x5a, 0xd4, 0x85, 0x0c, 0xf4, 0xd5, 0xef, 0x15, 0xa9, 0x7c, 0xf4, 0x60, 0xcd, 0x98 };
  uchar const pubkey[32] = { 0x32, 0x73, 0x61, 0x45, 0x02, 0x2d, 0x33, 0x72, 0x48, 0x01, 0x79, 0x11, 0x0d, 0x30, 0x71, 0x7e, 0xef, 0xf4, 0xf2, 0x84, 0xca, 0xe7, 0x6a, 0xbe, 0x4c, 0xaa, 0x77, 0x38, 0xda, 0xad, 0x06, 0x2b };

  fd_tower_serde_t serde = { 0 };
  fd_tower_deserialize( restore, sizeof(restore), pubkey, &serde );

  uchar checkpt[sizeof(restore)];
  ulong checkpt_sz;
  fd_tower_serialize( &serde, pvtkey, pubkey, checkpt, sizeof(checkpt), &checkpt_sz );

  FD_TEST( sizeof(restore) == checkpt_sz );
  FD_TEST( fd_uint_load_4( restore ) == fd_uint_load_4( checkpt ) );

  ulong off = sizeof(uint) + FD_ED25519_SIG_SZ; /* skip sig */
