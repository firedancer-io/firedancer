...
/* fd_hfork maintains four pools and four maps:
   ...
   vte_map  (capacity max): maps vte_key_t (vote_acc, block_id) -> vte_t
            for O(1) check of whether a voter has already voted for a
            given block_id.  If they have, the vote is ignored.

   vtr_map  (capacity vtr_max): maps vote_acc -> vtr_t, tracking each
     ...
     map[0] +--------------------+  map[0] +--------------------+
            | (vtr_t) {          |         | (blk_t) {          |
            |   .vote_acc = X,   |         |   .block_id  = A,  |
            |   ...              |         |   ...              |
            |   .vte_dlist = ... |         |   .bhm_dlist = ... |
            | }                  |         | }                  |
     map[1] +--------------------+  map[1] +--------------------+
            | (vtr_t) {          |         | (blk_t) {          |
            |   .vote_acc = Y,   |         |   .block_id = B    |
            |   ...              |         |   ...              |
            |   .vte_dlist = +   |         |   .bhm_dlist = +   |
            | }              |   |         | }              |   |
            +----------------|---+         +----------------|---+
                             ...
                             +------------------------+------------------------+------------------------+
                             | (vte_t) {              | (vte_t) {              | (vte_t) {              |
                             |   .key.vote_acc  = Y,  |   .key.vote_acc = Y,   |   .key.vote_acc = Y,   |
...
/* bhm_remove removes a bhm from bhm_map, its owning blk's bhm_dlist,
   ...
   entries, then the blk is also removed and released. */
...
compare( blk_t * blk,
  ...
  if( FD_UNLIKELY( 0==memcmp( &blk->our_bank_hash, &hash_null, sizeof(fd_hash_t) ) ) ) return 0;

  double pct = (double)bhm->stake * 100.0 / (double)total_stake;
  if( FD_UNLIKELY( pct < 52.0 ) ) return 0;

  if( FD_UNLIKELY( 0!=memcmp( &blk->our_bank_hash, &bhm->key.bank_hash, sizeof(fd_hash_t) ) ) ) return -1;
  return 1;
}
...
fd_hfork_new( void * shmem,
  ...
  hfork->blk_dlist   = blk_dlist_new( blk_dlist                                         );
...
fd_hfork_join( void * shhfork ) {
  ...
  hfork->blk_dlist = blk_dlist_join( hfork->blk_dlist );
...
blk_insert( fd_hfork_t      * hfork,
  ...
  if( FD_UNLIKELY( !blk_pool_free( hfork->blk_pool ) ) ) {
    if( FD_UNLIKELY( blk_dlist_is_empty( hfork->blk_dlist, hfork->blk_pool ) ) ) return NULL;
    blk_t * evicted = blk_dlist_ele_pop_head( hfork->blk_dlist, hfork->blk_pool );
    blk_map_ele_remove_fast( hfork->blk_map, evicted, hfork->blk_pool );
    blk_pool_ele_release( hfork->blk_pool, evicted );
  }
  blk_t * blk        = blk_pool_ele_acquire( hfork->blk_pool );
  blk->block_id      = *block_id;
  blk->our_bank_hash = hash_null;
  blk->bhm_cnt       = 0;
...
fd_hfork_count_vote( fd_hfork_t *        hfork,
  ...
  vte_key_t vte_key = { .vote_acc = *vote_acc, .block_id  = *block_id };
  ...
  /* Zero-stake votes don't contribute to hard fork detection. */
  ...
  else if( FD_UNLIKELY( !blk->bhm_cnt ) ) blk_dlist_ele_remove( hfork->blk_dlist, blk, hfork->blk_pool ); /* record_our_bank_hash before any count_vote */
  ...
  return compare( blk, bhm, total_stake );
}
...
fd_hfork_record_our_bank_hash( fd_hfork_t *      hfork,
  ...
  blk_t * blk = blk_map_ele_query( hfork->blk_map, block_id, NULL, hfork->blk_pool );
  if( FD_LIKELY( !blk ) ) {
    blk = blk_insert( hfork, block_id );
    if( FD_UNLIKELY( !blk ) ) return 0;
    blk_dlist_ele_push_tail( hfork->blk_dlist, blk, hfork->blk_pool );
  }
  blk->our_bank_hash = *fd_ptr_if( !!bank_hash, bank_hash, &hash_invalid );
  ...
  for( bhm_dlist_iter_t iter = bhm_dlist_iter_fwd_init( blk->bhm_dlist, hfork->bhm_pool );
    ...
    int     cmp = compare( blk, bhm, total_stake );
    if( cmp ) return cmp;
  }
  return 0;
}
...
fd_hfork_update_voters( fd_hfork_t *        hfork,
  ...
  for( ulong i=0UL; i<cnt; i++ ) {
    fd_pubkey_t const * vote_acc = &vote_accs[i];
    if( FD_LIKELY( vtr_map_ele_query( hfork->vtr_map, vote_acc, NULL, hfork->vtr_pool ) ) ) continue;
