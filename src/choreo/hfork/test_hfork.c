...
test_count_vote( void ) {
  ...
  FD_TEST( fd_hfork_record_our_bank_hash( hfork, &block_id, &bank_hash, 100 )==1 );
  ...
  /* Eviction where evicted bhm still has other voters (stake > 0
...
test_update_voters( void ) {
  ...
  /* a's vote was evicted: bhm stake decreased. */
  ...
  FD_TEST( blk );
  FD_TEST( !memcmp( &blk->our_bank_hash, &bank_hash, 32 ) );
...
test_record_our_bank_hash( void ) {
  ...
  FD_TEST( blk_dead );
  FD_TEST( (blk_dead->our_bank_hash.ul[0] & blk_dead->our_bank_hash.ul[1] &
            blk_dead->our_bank_hash.ul[2] & blk_dead->our_bank_hash.ul[3])==~0UL );
  ...
  /* Pool full with all blks having votes — dlist is empty,
...
test_lru_eviction( void ) {
  ...
  /* Create blks 0, 1, 2 via record_our_bank_hash.
     dlist order: 0 (head), 1, 2 (tail). */

  fd_hash_t bids[3];
  fd_hash_t bh = { .ul = { 200 } };
  for( ulong i = 0; i < 3; i++ ) {
    bids[i] = (fd_hash_t){ .ul = { 100+i } };
    fd_hfork_record_our_bank_hash( hfork, &bids[i], &bh, 100 );
  }

  /* Vote on blk 0 — removes it from dlist.
     dlist order: 1 (head), 2 (tail). */

  fd_pubkey_t voter = { .ul = { 1 } };
  register_voters( hfork, &voter, 1 );

  fd_hash_t bh_vote = { .ul = { 300 } };
  fd_hfork_count_vote( hfork, &voter, &bids[0], &bh_vote, 1000, 10, 100 );

  /* Fill remaining pool slots.  max = pow2_up(4*2) = 8, 3 used. */

  for( ulong i = 3; i < 8; i++ ) {
    fd_hash_t bid = { .ul = { 100+i } };
    fd_hfork_record_our_bank_hash( hfork, &bid, &bh, 100 );
  }
  FD_TEST( blk_pool_free( hfork->blk_pool )==0 );

  /* One more evicts blk 1 (head), NOT blk 0 (removed from dlist by vote). */

  fd_hash_t bid_new = { .ul = { 999 } };
  fd_hfork_record_our_bank_hash( hfork, &bid_new, &bh, 100 );

  FD_TEST( !blk_map_ele_query( hfork->blk_map, &bids[1], NULL, hfork->blk_pool ) );
  FD_TEST(  blk_map_ele_query( hfork->blk_map, &bids[0], NULL, hfork->blk_pool ) );
  FD_TEST(  blk_map_ele_query( hfork->blk_map, &bid_new, NULL, hfork->blk_pool ) );
