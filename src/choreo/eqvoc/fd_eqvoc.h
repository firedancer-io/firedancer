...
struct fd_eqvoc_proof {
  ...
  ulong                next;
  fd_eqvoc_proof_set_t set[4]; /* static declaration of an fd_set that
                                  occupies 4 words ie. 256 bits */

  /* Additional metadata */

  fd_pubkey_t producer; /* pubkey of the producer of the shreds */
  ulong       wallclock;
  ulong       chunk_cnt;
  ulong       chunk_len;
...
FD_FN_PURE static inline fd_shred_t *
...
FD_FN_PURE static inline fd_shred_t const *
...
fd_eqvoc_proof_shred2( fd_eqvoc_proof_t * proof ) {
  ...
  if( FD_UNLIKELY( !fd_eqvoc_proof_complete( proof ) ) ) FD_LOG_ERR(( "proof not complete" ));
  #endif
  ulong shred1_sz = *(ulong *)fd_type_pun_const( proof->shreds );
  return (fd_shred_t *)fd_type_pun_const( proof->shreds + shred1_sz + sizeof(ulong) );
}
...
fd_eqvoc_proof_shred2_const( fd_eqvoc_proof_t const * proof ) {
  ...
  return (fd_shred_t const *)fd_type_pun_const( proof->shreds + shred1_sz + sizeof(ulong) );
}
