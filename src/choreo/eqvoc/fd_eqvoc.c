...
fd_eqvoc_proof_t *
fd_eqvoc_proof_insert( fd_eqvoc_t * eqvoc, ulong slot, fd_pubkey_t const * from, fd_pubkey_t const * producer ) {
...
fd_eqvoc_proof_chunk_insert( fd_eqvoc_proof_t * proof, fd_gossip_duplicate_shred_t const * chunk ) {
  if( FD_UNLIKELY( fd_eqvoc_proof_set_is_null( proof->set ) ) ) { /* first chunk for this proof */
    proof->wallclock = chunk->wallclock;
    proof->chunk_cnt = chunk->num_chunks;
    proof->chunk_len = chunk->chunk_len;
  }

  if( FD_UNLIKELY( proof->wallclock != chunk->wallclock  ||
                   proof->chunk_cnt != chunk->num_chunks ||
                   ( chunk->chunk_index != chunk->num_chunks - 1 && chunk->chunk_len != proof->chunk_len ) ) ) {
    FD_LOG_WARNING(( "[%s] received conflicting chunk. slot: %lu from: %s. ignoring.", __func__, proof->key.slot, FD_BASE58_ENC_32_ALLOCA( proof->key.hash.uc ) ));
    return;
  }
  ...
  fd_memcpy( proof->shreds + chunk->chunk_len * chunk->chunk_index, chunk->chunk, chunk->chunk_len );
