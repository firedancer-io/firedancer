...
fd_eqvoc_chunk_insert( fd_eqvoc_t                        * eqvoc,
  ...
  FD_LOG_NOTICE(( "all chunks received for slot %lu from pubkey", chunk->slot ));
  ...
  ulong              shred2_sz = fd_ulong_load_8( proof->buf );
  fd_shred_t const * shred2    = (fd_shred_t const *)fd_type_pun_const( proof->buf + sizeof(ulong) + shred1_sz + sizeof(ulong) );

  if( FD_UNLIKELY( shred1_sz!=fd_shred_sz( shred1 ) ) ) return FD_EQVOC_ERR_SZ;
  if( FD_UNLIKELY( shred2_sz!=fd_shred_sz( shred2 ) ) ) return FD_EQVOC_ERR_SZ;
