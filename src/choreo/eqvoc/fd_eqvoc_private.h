...
typedef struct {
  ...
  ulong      prev;
  ulong      next;
  int        err; /* positive error code (FD_EQVOC_VERIFIED_{...}) if inserted */
} verified_t;

#define POOL_NAME verified_pool
#define POOL_T    verified_t
#include "../../util/tmpl/fd_pool.c"

#define MAP_NAME                           verified_map
#define MAP_ELE_T                          verified_t
#define MAP_KEY                            slot
#define MAP_OPTIMIZE_RANDOM_ACCESS_REMOVAL 1
#include "../../util/tmpl/fd_map_chain.c"

#define DEQUE_NAME verified_deque
#define DEQUE_T    ulong
#include "../../util/tmpl/fd_deque_dynamic.c"
...
struct fd_eqvoc {
  ...
  ulong  hist_max;
  ulong  from_max;
  ushort shred_version; /* eqvoc will ignore shreds or chunks with different shred versions */
  ...
  ulong *            shred_deque;
  verified_t *       verified_pool;
  verified_map_t *   verified_map;
  ulong *            verified_deque;
