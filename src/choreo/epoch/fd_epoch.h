...
/* fd_epoch_new formats an unused memory region for use as a epoch.  mem
...
/* fd_epoch_delete unformats a memory region used as a epoch.  Assumes
   ...
   error (e.g. epoch is obviously not a epoch ...  logs details).  The
   ownership of the memory region is transferred to the caller. */

void *
fd_epoch_delete( void * epoch );

/* fd_epoch_init initializes a epoch.  Assumes epoch is a valid local
   join and no other processes are joined.  root is the initial root
   that epoch will use.  This is the snapshot slot if booting from a
   snapshot, genesis slot otherwise.

   In general, this should be called by the same process that formatted
   epoch's memory, ie. the caller of fd_epoch_new. */

void fd_epoch_init( fd_epoch_t * epoch, fd_vote_accounts_t const * vote_accounts );

void fd_epoch_update( fd_epoch_t * epoch, fd_vote_accounts_t const * vote_accounts );
