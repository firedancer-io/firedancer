...
test_serde( void ) {
  ...
  ulong sz;
  FD_TEST( ag_aggsig_ser( agg, buf, sizeof(buf), &sz )==0 );
  FD_TEST( sz==AG_AGGSIG_SERIALIZED_SZ( nbits ) );

  ag_aggsig_t back[1];
  ulong consumed = ag_aggsig_de( back, buf, sz );
  FD_TEST( consumed==sz );
