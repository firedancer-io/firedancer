...
is_signer( ag_cert_t const * self,
           ulong             v ) {
  switch( self->kind ) {
  case AG_CERT_KIND_NOTAR:          return ag_bls_agg_is_signer( &self->inner.notar.agg_sig, v );
  case AG_CERT_KIND_FAST_FINAL:     return ag_bls_agg_is_signer( &self->inner.fast_final.agg_sig, v );
  case AG_CERT_KIND_FINAL:          return ag_bls_agg_is_signer( &self->inner.final.agg_sig, v );
  case AG_CERT_KIND_NOTAR_FALLBACK: return ag_bls_agg_is_signer( &self->inner.notar_fallback.agg_sig_notar, v ) || ag_bls_agg_is_signer( &self->inner.notar_fallback.agg_sig_notar_fallback, v );
  case AG_CERT_KIND_SKIP:           return ag_bls_agg_is_signer( &self->inner.skip.agg_sig_skip, v )            || ag_bls_agg_is_signer( &self->inner.skip.agg_sig_skip_fallback, v );
