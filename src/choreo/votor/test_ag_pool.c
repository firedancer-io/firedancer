...
test_boot_block_carries_skipped_window( void ) {
  ...
  ulong last       = next_start-1UL;
  for( ulong s=boot_slot+1UL; s<last; s++ ) add_skip_votes( pool, s, 0UL, 7UL );

  /* The skip cert closing the window is the one that grants readiness.
     Cast its votes without draining: the votor is told through the
     event, so checking the tracker alone would not catch the event
     going missing. */

  drain_events( pool );
  for( ulong v=0UL; v<7UL; v++ ) {
    ag_vote_t vote = ag_vote_construct_skip( sec_sign_fn, &g_sk[v], last, (ushort)v, TEST_SHRED_VERSION );
    FD_TEST( ag_pool_add_vote( pool, &vote, bad )==AG_POOL_SUCCESS );
  }
  ...
  while( ag_pool_poll_pool_event( pool, &event ) ) {
    if( event.kind!=AG_EVENT_POOL_PARENT_READY ) continue;
    FD_TEST( event.parent_ready.slot==next_start );
