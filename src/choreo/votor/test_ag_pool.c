...
add_notar_votes( ag_pool_t *           pool,
  ...
  for( ulong v=lo; v<hi; v++ ) {
    ag_vote_t vote = ag_vote_construct_notar( &g_sk[v], slot, hash, (ushort)v, TEST_SHRED_VERSION );
...
add_notar_fallback_votes( ag_pool_t *           pool,
                          ...
                          ulong                 hi ) {
  for( ulong v=lo; v<hi; v++ ) {
    ag_vote_t vote = ag_vote_construct_notar_fallback( &g_sk[v], slot, hash, (ushort)v, TEST_SHRED_VERSION );
    FD_TEST( ag_pool_add_vote( pool, &vote )==AG_POOL_SUCCESS );
    drain_events( pool );
  }
}
...
add_skip_votes( ag_pool_t * pool,
                ...
                ulong       hi ) {
  for( ulong v=lo; v<hi; v++ ) {
    ag_vote_t vote = ag_vote_construct_skip( &g_sk[v], slot, (ushort)v, TEST_SHRED_VERSION );
    FD_TEST( ag_pool_add_vote( pool, &vote )==AG_POOL_SUCCESS );
    drain_events( pool );
  }
}
...
add_final_votes( ag_pool_t * pool,
                 ...
                 ulong       hi ) {
  for( ulong v=lo; v<hi; v++ ) {
    ag_vote_t vote = ag_vote_construct_final( &g_sk[v], slot, (ushort)v, TEST_SHRED_VERSION );
    FD_TEST( ag_pool_add_vote( pool, &vote )==AG_POOL_SUCCESS );
    drain_events( pool );
  }
}
...
notar_cert( ag_pool_t *           pool,
  ...
  for( ulong v=0UL; v<signers; v++ ) nv[v] = ag_vote_construct_notar( &g_sk[v], slot, hash, (ushort)v, TEST_SHRED_VERSION ).notar;
...
fast_finalize( ag_pool_t *           pool,
               ...
               ag_block_hash_t const hash ) {
  ag_vote_notar_t nv[ NV ];
  for( ulong v=0UL; v<NV; v++ ) nv[v] = ag_vote_construct_notar( &g_sk[v], slot, hash, (ushort)v, TEST_SHRED_VERSION ).notar;
...
test_branch_certified_late_cert( void ) {
  ...
  for( ulong v=0UL; v<7UL; v++ ) nv[v] = ag_vote_construct_notar( &g_sk[v], slot1, hash1, (ushort)v, TEST_SHRED_VERSION ).notar;
...
test_duplicate_votes( void ) {
  ...
  ag_vote_t v1 = ag_vote_construct_notar( &g_sk[0], slot, gh, 0, TEST_SHRED_VERSION );
  FD_TEST( ag_pool_add_vote( pool, &v1 )==AG_POOL_SUCCESS );

  ag_vote_t v2 = ag_vote_construct_skip( &g_sk[1], slot, 1, TEST_SHRED_VERSION );
...
test_duplicate_certs( void ) {
  ...
  ag_vote_notar_t nv[ NV ];
  for( ulong v=0UL; v<NV; v++ ) nv[v] = ag_vote_construct_notar( &g_sk[v], first_slot, hash, (ushort)v, TEST_SHRED_VERSION ).notar;
  ...
  for( ulong v=0UL; v<NV; v++ ) sv[v] = ag_vote_construct_skip( &g_sk[v], second_slot, (ushort)v, TEST_SHRED_VERSION ).skip;
...
test_out_of_bounds_votes( void ) {
  ...
  for( ulong s=0UL; s<3UL*SLOTS_PER_WINDOW-1UL; s++ ) {
    for( ulong v=0UL; v<11UL; v++ ) {
      ag_vote_t vote = ag_vote_construct_final( &g_sk[v], s, (ushort)v, TEST_SHRED_VERSION );
  ...
  for( ulong v=0UL; v<11UL; v++ ) {
    ag_vote_t vote = ag_vote_construct_final( &g_sk[v], future, (ushort)v, TEST_SHRED_VERSION );
    FD_TEST( ag_pool_add_vote( pool, &vote )==AG_POOL_ERR_SLOT_OUT_OF_BOUNDS );
  }
...
test_out_of_bounds_certs( void ) {
  ...
  for( ulong s=1UL; s<=slot; s++ ) {
    ag_vote_notar_t nv[ NV ];
    for( ulong v=0UL; v<NV; v++ ) nv[v] = ag_vote_construct_notar( &g_sk[v], s, gh, (ushort)v, TEST_SHRED_VERSION ).notar;
    ag_cert_t c = cert_build_fast_final( nv, NV, g_epoch_info );
    FD_TEST( ag_pool_add_cert( pool, &c )==AG_POOL_SUCCESS );
    drain_events( pool );
  }
  ...
  for( ulong s=0UL; s<3UL*SLOTS_PER_WINDOW-1UL; s++ ) {
    ag_vote_skip_t sv[ NV ];
    for( ulong v=0UL; v<NV; v++ ) sv[v] = ag_vote_construct_skip( &g_sk[v], s, (ushort)v, TEST_SHRED_VERSION ).skip;
  ...
  ag_vote_skip_t sv[ NV ];
  for( ulong v=0UL; v<NV; v++ ) sv[v] = ag_vote_construct_skip( &g_sk[v], future, (ushort)v, TEST_SHRED_VERSION ).skip;
  ag_cert_t c = cert_build_skip( sv, NV, NULL, 0UL, g_epoch_info );
  FD_TEST( ag_pool_add_cert( pool, &c )==AG_POOL_ERR_SLOT_OUT_OF_BOUNDS );
...
test_parent_ready_upon_finalization( void ) {
  ...
  for( ulong v=0UL; v<11UL; v++ ) {
    ag_vote_t vote = ag_vote_construct_notar( &g_sk[v], block2.slot, block2.hash, (ushort)v, TEST_SHRED_VERSION );
    FD_TEST( ag_pool_add_vote( pool, &vote )==AG_POOL_SUCCESS );
  }
...
test_safe_to_notar_notar_cert_only( void ) {
  ...
  ag_vote_t skip = ag_vote_construct_skip( &g_sk[0], slot2, 0, TEST_SHRED_VERSION );
  FD_TEST( ag_pool_add_vote( pool, &skip )==AG_POOL_SUCCESS );
  for( ulong v=1UL; v<6UL; v++ ) {
    ag_vote_t vote = ag_vote_construct_notar( &g_sk[v], slot2, hash2, (ushort)v, TEST_SHRED_VERSION );
    FD_TEST( ag_pool_add_vote( pool, &vote )==AG_POOL_SUCCESS );
  }
...
test_handle_invalid_votes( void ) {
  ...
  ag_block_hash_t gh; genesis_hash( gh );
  ag_vote_t vote = ag_vote_construct_notar( &g_sk[0], 0UL, gh, 0, TEST_SHRED_VERSION );
  FD_TEST( ag_pool_add_vote( pool, &vote )==AG_POOL_SUCCESS );
...
test_hash_capacity( void ) {
  ...
  for( ulong i=0UL; i<NV; i++ ) {
    random_hash( hash[i] );
    ag_vote_t v = ag_vote_construct_notar( &g_sk[i], slot, hash[i], (ushort)i, TEST_SHRED_VERSION );
  ...
  for( ulong i=0UL; i<AG_NOTAR_FALLBACK_VOTE_MAX; i++ ) {
    random_hash( nf[i] );
    ag_vote_t v = ag_vote_construct_notar_fallback( &g_sk[0], slot, nf[i], (ushort)0, TEST_SHRED_VERSION );
    FD_TEST( ag_pool_add_vote( pool, &v )==AG_POOL_SUCCESS );
    drain_events( pool );
  }
  ...
  ag_vote_t v_past;
  v_past = ag_vote_construct_notar_fallback( &g_sk[0], slot, past, (ushort)0, TEST_SHRED_VERSION );
  FD_TEST( ag_pool_add_vote( pool, &v_past )==AG_POOL_ERR_SLASHABLE );
  FD_TEST( ag_pool_add_vote( pool, &v_past )==AG_POOL_ERR_SLASHABLE );

  ag_vote_t v_other;
  v_other = ag_vote_construct_notar_fallback( &g_sk[1], slot, past, (ushort)1, TEST_SHRED_VERSION );
...
test_unknown_signer_votes( void ) {
  ...
  ag_vote_t v1 = ag_vote_construct_notar( &g_sk[0], slot, gh, (ushort)NV, TEST_SHRED_VERSION );
  FD_TEST( ag_vote_rank( &v1 )>=epoch->validator_cnt );

  ag_vote_t v2 = ag_vote_construct_skip( &g_sk[0], slot, USHORT_MAX, TEST_SHRED_VERSION );
...
test_epoch_installed_late( void ) {
  ...
  ag_vote_t v = ag_vote_construct_notar( &g_sk[0], beyond, hash, (ushort)0, TEST_SHRED_VERSION );
...
test_retired_epoch_already_pruned( void ) {
  ...
  v_below = ag_vote_construct_notar( &g_sk[0], EPOCH_B_LO-1UL, h_below, (ushort)0, TEST_SHRED_VERSION );
