...
struct fd_vinyl_io_bd_rd {
  ...
  void *                dst;  /* " */
  ulong                 sz;   /* " */
  fd_vinyl_io_bd_rd_t * next; /* Next element in bd rd queue */
};

struct fd_vinyl_io_bd {
  ...
  fd_vinyl_io_bd_rd_t *    rd_head;      /* Pointer to queue head */
  fd_vinyl_io_bd_rd_t **   rd_tail_next; /* Pointer to queue &tail->next or &rd_head if empty. */
...
static void
fd_vinyl_io_bd_read( fd_vinyl_io_t *    io,
                     fd_vinyl_io_rd_t * _rd ) {
  fd_vinyl_io_bd_t *    bd = (fd_vinyl_io_bd_t *)   io;  /* Note: io must be non-NULL to have even been called */
  fd_vinyl_io_bd_rd_t * rd = (fd_vinyl_io_bd_rd_t *)_rd;

  rd->next          = NULL;
  *bd->rd_tail_next = rd;
  bd->rd_tail_next  = &rd->next;
...
static int
fd_vinyl_io_bd_poll( fd_vinyl_io_t *     io,
                     fd_vinyl_io_rd_t ** _rd,
                     int                 flags ) {
  fd_vinyl_io_bd_t * bd = (fd_vinyl_io_bd_t * )io; /* Note: io must be non-NULL to have even been called */
  (void)flags;

  fd_vinyl_io_bd_rd_t *  rd = bd->rd_head;
  ...
  fd_vinyl_io_bd_rd_t ** rd_tail_next = bd->rd_tail_next;
  fd_vinyl_io_bd_rd_t *  rd_next      = rd->next;

  bd->rd_head      = rd_next;
  bd->rd_tail_next = fd_ptr_if( !!rd_next, rd_tail_next, &bd->rd_head );
...
static ulong
fd_vinyl_io_bd_append( fd_vinyl_io_t * io,
                       void const *    _src,
                       ulong           sz ) {
  fd_vinyl_io_bd_t * bd  = (fd_vinyl_io_bd_t *)io; /* Note: io must be non-NULL to have even been called */
...
static int
fd_vinyl_io_bd_commit( fd_vinyl_io_t * io,
                       int             flags ) {
  fd_vinyl_io_bd_t * bd = (fd_vinyl_io_bd_t *)io; /* Note: io must be non-NULL to have even been called */
  (void)flags;

  bd->base->seq_present = bd->base->seq_future;
  bd->base->spad_used   = 0UL;

  return FD_VINYL_SUCCESS;
}

static ulong
fd_vinyl_io_bd_hint( fd_vinyl_io_t * io,
                     ulong           sz ) {
  fd_vinyl_io_bd_t * bd = (fd_vinyl_io_bd_t *)io; /* Note: io must be non-NULL to have even been called */
  ...
  return bd->base->seq_future;
}

static void *
fd_vinyl_io_bd_alloc( fd_vinyl_io_t * io,
                      ulong           sz,
                      int             flags ) {
  fd_vinyl_io_bd_t * bd = (fd_vinyl_io_bd_t *)io; /* Note: io must be non-NULL to have even been called */

  ulong spad_max  = bd->base->spad_max;
  ulong spad_used = bd->base->spad_used; if( FD_UNLIKELY( !sz ) ) return ((uchar *)(bd+1)) + spad_used;
  ...
  if( FD_UNLIKELY( sz > (spad_max - spad_used ) ) ) {
    if( FD_UNLIKELY( fd_vinyl_io_bd_commit( io, flags ) ) ) return NULL;
    spad_used = 0UL;
  }

  bd->base->spad_used = spad_used + sz;

  return ((uchar *)(bd+1)) + spad_used;
}

static ulong
fd_vinyl_io_bd_copy( fd_vinyl_io_t * io,
                     ulong           seq_src0,
                     ulong           sz ) {
  fd_vinyl_io_bd_t * bd = (fd_vinyl_io_bd_t *)io; /* Note: io must be non-NULL to have even been called */
  ...
  if( FD_UNLIKELY( sz>(spad_max-spad_used) ) ) {
    fd_vinyl_io_bd_commit( io, FD_VINYL_IO_FLAG_BLOCKING );
    spad_used = 0UL;
  }

  uchar * buf     = (uchar *)(bd+1) + spad_used;
...
static void
fd_vinyl_io_bd_forget( fd_vinyl_io_t * io,
                       ulong           seq ) {
  fd_vinyl_io_bd_t * bd = (fd_vinyl_io_bd_t *)io; /* Note: io must be non-NULL to have even been called */
  ...
  ulong seq_past    = bd->base->seq_past;
  ulong seq_present = bd->base->seq_present;
  ulong seq_future  = bd->base->seq_future;
  ...
  bd->base->seq_past = seq;
}

static void
fd_vinyl_io_bd_rewind( fd_vinyl_io_t * io,
                       ulong           seq ) {
  fd_vinyl_io_bd_t * bd = (fd_vinyl_io_bd_t *)io; /* Note: io must be non-NULL to have even been called */
  ...
  bd->base->seq_ancient = fd_ulong_if( fd_vinyl_seq_ge( seq, seq_ancient ), seq_ancient, seq );
  bd->base->seq_past    = fd_ulong_if( fd_vinyl_seq_ge( seq, seq_past    ), seq_past,    seq );
  bd->base->seq_present = seq;
  bd->base->seq_future  = seq;
}

static int
fd_vinyl_io_bd_sync( fd_vinyl_io_t * io,
                     int             flags ) {
  fd_vinyl_io_bd_t * bd = (fd_vinyl_io_bd_t *)io; /* Note: io must be non-NULL to have even been called */
  (void)flags;

  ulong seed        = bd->base->seed;
  ulong seq_past    = bd->base->seq_past;
  ulong seq_present = bd->base->seq_present;
...
static void *
fd_vinyl_io_bd_fini( fd_vinyl_io_t * io ) {
  fd_vinyl_io_bd_t * bd = (fd_vinyl_io_bd_t *)io; /* Note: io must be non-NULL to have even been called */

  ulong seq_present = bd->base->seq_present;
  ulong seq_future  = bd->base->seq_future;

  if( FD_UNLIKELY( bd->rd_head                                ) ) FD_LOG_WARNING(( "fini completing outstanding reads" ));
  if( FD_UNLIKELY( fd_vinyl_seq_ne( seq_present, seq_future ) ) ) FD_LOG_WARNING(( "fini discarding uncommited blocks" ));

  return io;
}

static fd_vinyl_io_impl_t fd_vinyl_io_bd_impl[1] = { {
  fd_vinyl_io_bd_read_imm,
  fd_vinyl_io_bd_read,
  fd_vinyl_io_bd_poll,
  fd_vinyl_io_bd_append,
  fd_vinyl_io_bd_commit,
  fd_vinyl_io_bd_hint,
  fd_vinyl_io_bd_alloc,
  fd_vinyl_io_bd_copy,
  fd_vinyl_io_bd_forget,
  fd_vinyl_io_bd_rewind,
  fd_vinyl_io_bd_sync,
  fd_vinyl_io_bd_fini
} };

FD_STATIC_ASSERT( alignof(fd_vinyl_io_bd_t)==FD_VINYL_BSTREAM_BLOCK_SZ, layout );

ulong
fd_vinyl_io_bd_align( void ) {
  return alignof(fd_vinyl_io_bd_t);
}

ulong
fd_vinyl_io_bd_footprint( ulong spad_max ) {
  if( FD_UNLIKELY( !((0UL<spad_max) & (spad_max<(1UL<<63)) & fd_ulong_is_aligned( spad_max, FD_VINYL_BSTREAM_BLOCK_SZ )) ) )
    return 0UL;
  return sizeof(fd_vinyl_io_bd_t) + spad_max;
}

fd_vinyl_io_t *
fd_vinyl_io_bd_init( void *       mem,
                     ulong        spad_max,
                     int          dev_fd,
                     int          reset,
                     void const * info,
                     ulong        info_sz,
                     ulong        io_seed ) {
  fd_vinyl_io_bd_t * bd = (fd_vinyl_io_bd_t *)mem;
  ...
  if( FD_UNLIKELY( !fd_ulong_is_aligned( (ulong)bd, fd_vinyl_io_bd_align() ) ) ) {
  ...
  memset( bd, 0, footprint );

  bd->base->type = FD_VINYL_IO_TYPE_BD;

  /* io_seed, seq_ancient, seq_past, seq_present, seq_future are init
     below */

  bd->base->spad_max  = spad_max;
  bd->base->spad_used = 0UL;
  bd->base->impl      = fd_vinyl_io_bd_impl;

  bd->dev_fd   = dev_fd;
  bd->dev_sync = 0UL;                            /* Use the beginning of the file for the sync block */
  bd->dev_base = FD_VINYL_BSTREAM_BLOCK_SZ;      /* Use the rest for the actual bstream store (at least 3.5 KiB) */
  bd->dev_sz   = dev_sz - FD_VINYL_BSTREAM_BLOCK_SZ;

  bd->rd_head      = NULL;
  bd->rd_tail_next = &bd->rd_head;
  ...
  if( reset ) {
    ...
    bd->base->seed        = io_seed;
    bd->base->seq_ancient = 0UL;
    bd->base->seq_past    = 0UL;
    bd->base->seq_present = 0UL;
    bd->base->seq_future  = 0UL;
  ...
  //block->sync.hash_blocks = ...; /* init by sync */
    ...
    int err = fd_vinyl_io_bd_sync( bd->base, FD_VINYL_IO_FLAG_BLOCKING ); /* logs details */
    if( FD_UNLIKELY( err ) ) {
    ...
    int bad_past_sz     = ((seq_present-seq_past) > bd->dev_sz);
    ...
    bd->base->seed        = io_seed;
    bd->base->seq_ancient = seq_past;
    bd->base->seq_past    = seq_past;
    bd->base->seq_present = seq_present;
    bd->base->seq_future  = seq_present;
