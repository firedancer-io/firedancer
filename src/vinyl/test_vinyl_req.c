...
req( int     type,
  ...
  case FD_VINYL_REQ_TYPE_ACQUIRE: {
    ...
    if( ref_exists ) {
      ...
      if( !fd_vinyl_req_flag_modify( flags ) ) {
        ...
        ulong ref_szc = fd_vinyl_data_szc( (ulong)ref_info->_val_sz );
      ...
      if( ref_info->_val_sz ) memcpy( backup_val, ref_val, (ulong)ref_info->_val_sz );

      if( fd_vinyl_req_flag_ignore( flags ) ) ref_info->_val_sz = 0U;

      ulong ref_szc = fd_vinyl_data_szc( fd_ulong_max( val_max, (ulong)ref_info->_val_sz ) );
      ref_val_max = fd_vinyl_data_szc_val_max( ref_szc );
  ...
  case FD_VINYL_REQ_TYPE_RELEASE: {
    ...
    if( ref_creating ) {
      ...
      if( (ulong)ref_info->_val_sz > ref_val_max ) FD_LOG_CRIT(( "val buffer overrun" ));
    ...
    if( !fd_vinyl_req_flag_modify( flags ) ) {
      ...
      if( backup_info->_val_sz ) memcpy( ref_val, backup_val, (ulong)backup_info->_val_sz );
      ref_info[0] = backup_info[0];

      ulong ref_szc = fd_vinyl_data_szc( (ulong)backup_info->_val_sz );
      ref_val_max = fd_vinyl_data_szc_val_max( ref_szc );
...
# define WAIT do {                                                   \
  ...
  for( ulong rem=iter_max; rem; rem-- ) {
      ...
      if( !ref_err ) {
        ...
        if( fd_vinyl_req_flag_modify( flags ) ) {
          ...
          if( do_mod ) {
            ...
            info->_val_sz = (uint)val_sz;                     ref_info->_val_sz = (uint)val_sz;
            acq_modified |= 1;
          }
        } else {
          FD_TEST( !memcmp( info, ref_info, sizeof(fd_vinyl_info_t) ) );
    ...
    case 37: { /* test */
      ...
      ulong             try_val_sz = fd_ulong_min( (ulong)try_info->_val_sz, FD_VINYL_VAL_MAX );
