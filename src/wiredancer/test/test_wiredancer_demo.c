...
#if FD_HAS_HOSTED && defined(__x86_64__) && FD_HAS_WIREDANCER
...
main( int     argc,
      char ** argv ) {
  fd_boot( &argc, &argv );
  FD_LOG_WARNING(( "skip: unit test requires FD_HAS_HOSTED, defined(__x86_64__) and FD_HAS_WIREDANCER capabilities" ));
  fd_halt();
  return 0;
}
