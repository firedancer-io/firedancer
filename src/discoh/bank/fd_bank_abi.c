...
typedef struct ABI_ALIGN(8UL) {
  ...
  union ABI_ALIGN(8UL) {
    ...
    /* when discr==9223372036854775808 */
...
#define MAP_PERFECT_1  ( KECCAK_SECP_PROG_ID ),
#define MAP_PERFECT_2  ( ED25519_SV_PROG_ID  ),
#define MAP_PERFECT_0  ( SECP256R1_PROG_ID   ),
...
fd_bank_abi_txn_init( fd_bank_abi_txn_t * out_txn,
  ...
  for( ulong i=0UL; i<txn->instr_cnt; i++ ) {
    ...
    /* Lookup prog_id in hash table.  If it's a miss, it'll return
       ...
       2. */
    sig_counters[ fd_uint_max( 3UL, precompile_table_hash_or_default( prog_id ) ) ] +=
  ...
  if( FD_LIKELY( txn->transaction_version==FD_TXN_VLEGACY ) ) {
    ...
    out_txn->message.discr = 9223372036854775808UL;
...
fd_bank_abi_get_lookup_addresses( fd_bank_abi_txn_t const * txn ) {
  return txn->message.discr==9223372036854775808UL ? NULL :
