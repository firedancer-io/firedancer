...
handle_microblock( fd_bank_ctx_t *     ctx,
  ...
  /* todo ... replace dummy values with correct ones from Agave */
  trailer->txn_load_start_pct = (ushort)(0    * USHORT_MAX);
  trailer->txn_load_end_pct   = (ushort)(0.25 * USHORT_MAX);
  trailer->txn_exec_start_pct = (ushort)(0.5  * USHORT_MAX);
  trailer->txn_exec_end_pct   = (ushort)(1    * USHORT_MAX);
