...
after_frag( fd_plugin_ctx_t *   ctx,
  ...
  switch( ctx->in_kind[ in_idx ] ) {
    case IN_KIND_REPLAY: {
      FD_TEST( sig==FD_PLUGIN_MSG_SLOT_ROOTED || sig==FD_PLUGIN_MSG_SLOT_OPTIMISTICALLY_CONFIRMED || sig==FD_PLUGIN_MSG_SLOT_COMPLETED || sig==FD_PLUGIN_MSG_SLOT_RESET || sig==FD_PLUGIN_MSG_START_PROGRESS || sig==FD_PLUGIN_MSG_GENESIS_HASH_KNOWN || sig==FD_PLUGIN_MSG_CONSENSUS_UPDATE );
      break;
    }
    case IN_KIND_GOSSIP: {
