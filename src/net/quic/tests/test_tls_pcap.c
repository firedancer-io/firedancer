...
void
test_secret_gen( uchar * new_secret, uchar const * secret, ulong secret_sz, char const * label, ulong output_sz ) {
...
main( int     argc,
  ...
  //enc_header[0] ^= mask[0] & 0x0f; // short would be "& 0x1fu"
  ...
  FD_TEST( 1==EVP_CipherInit_ex( cipher_ctx, FD_AES_128_GCM_ALG_HANDLE, NULL, NULL, NULL, 1 /* encryption */ ) );
  ...
  FD_LOG_NOTICE(( "plain_text_sz: %ld", (long)plain_text_sz ));
