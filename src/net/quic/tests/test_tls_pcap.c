...
void
test_secret_gen( uchar * new_secret, uchar const * secret, size_t secret_sz, char const * label, size_t output_sz ) {
...
int main(int argc, char **argv)
{
  ...
  uchar  initial_secret[32] = {};
  size_t initial_secret_sz  = 32;

  const EVP_MD *md = EVP_sha256(); // or 384 or 512

  fd_quic_hkdf_extract( initial_secret, sizeof( initial_secret ),
                        md,
  ...
  printf( "plain_text: " );
  for( size_t j = 0; j < plain_text_sz; ++j ) {
  ...
  if( EVP_CIPHER_CTX_reset( cipher_ctx ) != 1 ) {
    fprintf( stderr, "EVP_CIPHER_CTX_reset failed\n" );
    exit(1);
  }

  if( EVP_CipherInit_ex( cipher_ctx, FD_AES_128_GCM_ALG_HANDLE, NULL, NULL, NULL, 1 /* encryption */ ) != 1 ) {
    fprintf( stderr, "EVP_CipherInit_ex failed\n" );
    exit(1);
  }
  ...
  offset = 0;
  cipher_text_sz = 2048;
  plain_text_sz = sizeof( test_client_initial );
  printf( "plain_text_sz: %ld\n", (long)plain_text_sz );
  if( EVP_EncryptUpdate( cipher_ctx, cipher_text, &cipher_text_sz, test_client_initial, plain_text_sz ) != 1 ) {
    fprintf( stderr, "EVP_EncryptUpdate failed cipher\n" );
    exit(1);
