...
struct fd_microblock_trailer {
  ...
  /* If the duration of a microblock is the difference between the
     ...
     elapsed time for the first transaction's loading/executing step. At
     the moment this is useful because bank sends out at most one
     transaction per microblock.

     For example, if a microblock starts at t=10 and ends at t=20, and
     txn_exec_start_pct is 128, then this transaction started
     executing at roughly 10+(20-10)*(128/UCHAR_MAX)=15 */
  uchar txn_load_end_pct;
  uchar txn_exec_end_pct;

  /* The error code for the first transaction in this microblock, if it
