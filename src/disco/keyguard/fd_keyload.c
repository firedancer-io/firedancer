...
#define MAX_KEY_FILE_SZ     1023UL /* Unless it has extraneous whitespace, max is 64*4+1 */
  ...
  char * json_key_file = (char *)keypair+KEY_SZ;
  long bytes_read = read( key_fd, keypair+KEY_SZ, MAX_KEY_FILE_SZ );
  ...
  for( ulong i=1UL; i<63UL; i++ ) {
    if( FD_UNLIKELY( 1!=sscanf( tok[ i ], "%hhu", &keypair[ i ] ) ) )
      KEY_PARSE_ERR( "Parsing failed near the %luth value.", key_path, i );
  }
  if( FD_UNLIKELY( 1!=sscanf( tok[ 63 ], "%hhu ]", &keypair[ 63 ] ) ) )
  ...
  fd_memset_explicit( json_key_file, 0, MAX_KEY_FILE_SZ       );
  fd_memset_explicit( tok,           0, KEY_SZ*sizeof(char *) );
...
fd_keyload_load( char const * key_path,
  ...
  if( public_key_only ) fd_memset_explicit( key_page, 0, 32UL );
...
fd_keyload_unload( uchar const * key,
  ...
  fd_memset_explicit( key_page, 0, 4096UL );
