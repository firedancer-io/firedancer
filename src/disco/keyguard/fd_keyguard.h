...
#define FD_KEYGUARD_ROLE_TOWER        (10) /* Sign tower file messages */
#define FD_KEYGUARD_ROLE_CNT          (11) /* number of known roles */

/* The tower tile signs its saved tower file with plain Ed25519 over a
   16 byte domain prefix followed by the sha256 of the file body.  The
   prefix keeps that signature out of every other raw message domain,
   in particular the 32 byte shred roots the leader role signs. */
#define FD_KEYGUARD_TOWER_FILE_PREFIX    "FD_TOWER_FILE_V1"
