...
privileged_init_sensitive( fd_topo_t const *      topo,
  ...
  uchar * identity_key = fd_keyload_mprotect_wr( fd_keyload_load( FD_TOPO_STR( tile->sign.identity_key_path ), /* pubkey only: */ 0 ), /* public_key_only: */ 0 );
  ...
  for( ulong i=0UL; i<tile->sign.authorized_voter_paths_cnt; i++ ) {
    uchar const * authorized_voter_key = fd_keyload_load( FD_TOPO_STR( tile->sign.authorized_voter_paths[ i ] ), /* pubkey only: */ 0 );
