    ...
    case IN_KIND_REPAIR_NET: {
      ...
      if( FD_LIKELY( fd_ip4_udp_hdr_strip( ctx->buf, sz, &payload, &payload_sz, NULL, NULL, NULL ) ) ) {
        ...
        switch ( msg->kind ) {
          ...
          case FD_REPAIR_KIND_SHRED: fd_gui_handle_repair_slot( ctx->gui, msg->shred.slot, fd_clock_now( ctx->clock ) ); break;
          case FD_REPAIR_KIND_HIGHEST_SHRED: fd_gui_handle_repair_slot( ctx->gui, msg->highest_shred.slot, fd_clock_now( ctx->clock ) ); break;
