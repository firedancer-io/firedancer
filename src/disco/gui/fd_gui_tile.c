...
before_frag( fd_gui_ctx_t * ctx,
  ...
  if( FD_LIKELY( ctx->in_kind[ in_idx ]==IN_KIND_GOSSIP_OUT && (sig==FD_GOSSIP_UPDATE_TAG_CONTACT_INFO_REMOVE || sig==FD_GOSSIP_UPDATE_TAG_CONTACT_INFO || sig==FD_GOSSIP_UPDATE_TAG_WFS_DONE) ) ) return 1;
  return 0;
}
