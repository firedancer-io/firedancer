...
after_frag( fd_gui_ctx_t *      ctx,
  ...
  if( FD_LIKELY( ctx->in_kind[ in_idx ]==IN_KIND_PLUGIN ) ) fd_gui_plugin_message( ctx->gui, sig, ctx->buf );
    ...
    fd_gui_became_leader( ctx->gui, fd_frag_meta_ts_decomp( tspub, fd_tickcount() ), fd_disco_poh_sig_slot( sig ), became_leader->slot_start_ns, became_leader->slot_end_ns, FD_PACK_MAX_COST_PER_BLOCK, became_leader->max_microblocks_in_slot );
    ...
    if( FD_LIKELY( fd_disco_poh_sig_pkt_type( sig )==POH_PKT_TYPE_MICROBLOCK ) ) {
      ...
      fd_gui_microblock_execution_begin( ctx->gui, fd_frag_meta_ts_decomp( tspub, fd_tickcount() ), fd_disco_poh_sig_slot( sig ), (fd_txn_p_t *)ctx->buf, (sz-sizeof( fd_microblock_bank_trailer_t ))/sizeof( fd_txn_p_t ), trailer->pack_txn_idx, trailer->is_bundle );
    ...
    fd_gui_microblock_execution_end( ctx->gui,
                                     ...
                                     trailer->error_code,
                                     trailer->txn_load_start_pct,
                                     trailer->txn_load_end_pct,
                                     trailer->txn_exec_start_pct,
