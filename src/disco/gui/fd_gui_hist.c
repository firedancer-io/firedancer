...
/* fd_gui_hist_key_sz returns the KV key width for `dbi`: 16 bytes
   (slot, bank_seq) for slot-keyed DBs, 8 bytes for EPOCH and TIMELINE_DAY,
   0 for TS. */
static inline ulong
...
/* ---- space-pressure eviction -----------------------------------------
   ...
   phases:
     ...
     TIMELINE   -> deleting timeline days before the next epoch
...
fd_gui_hist_evict_one( fd_gui_t * gui ) {
  ...
  case FD_GUI_HIST_EVICT_TIMELINE: {
    if( hist->evict.have_ts ) {
      fd_gui_hist_timeline_day_key_t hi = { .day=(hist->evict.window_hi+1UL)/(24UL*60UL*60UL) };
      int drained = 1;
