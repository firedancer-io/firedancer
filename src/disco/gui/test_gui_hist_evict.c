...
test_timeline_db( fd_gui_t * gui ) {
  ...
  FD_TEST( descs[ FD_GUI_HIST_REPLAY_TXN_BATCH ].ts_off==offsetof(fd_gui_store_replay_txn_t,insert_time_ns) );
...
test_txn_insert_bounds( fd_gui_t * gui ) {
  ...
  cJSON * json = cJSON_ParseWithLength( (char const *)gui->http->oring+gui->http->stage_off%gui->http->oring_sz, len );
  FD_TEST( json );
  cJSON * value = cJSON_GetObjectItemCaseSensitive( json, "value" );
  cJSON * txns = cJSON_GetObjectItemCaseSensitive( value, "transactions" );
  cJSON * start_times = cJSON_GetObjectItemCaseSensitive( txns, "txn_mb_start_timestamps_nanos" );
  cJSON * end_times = cJSON_GetObjectItemCaseSensitive( txns, "txn_mb_end_timestamps_nanos" );
  FD_TEST( cJSON_GetArraySize( start_times )==2 && cJSON_GetArraySize( end_times )==2 );
  FD_TEST( !strcmp( cJSON_GetStringValue( cJSON_GetArrayItem( start_times, 0 ) ), "11000000000" ) );
  FD_TEST( !strcmp( cJSON_GetStringValue( cJSON_GetArrayItem( start_times, 1 ) ), "10000000000" ) );
  FD_TEST( !strcmp( cJSON_GetStringValue( cJSON_GetArrayItem( end_times, 0 ) ), "11000000100" ) );
  FD_TEST( !strcmp( cJSON_GetStringValue( cJSON_GetArrayItem( end_times, 1 ) ), "10000000100" ) );
  cJSON_Delete( json );
