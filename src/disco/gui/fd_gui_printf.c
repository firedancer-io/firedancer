...
#define HANDLE_SNAPSHOT_STATE(snapshot_type, snapshot_type_upper) \
      if( FD_LIKELY( gui->summary.boot_progress.phase>=FD_GUI_BOOT_PROGRESS_TYPE_LOADING_##snapshot_type_upper##_SNAPSHOT )) { \
        ...
        jsonp_long        ( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_read_throughput_compressed",       gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].read_throughput_compressed                           ); \
        ...
        jsonp_long        ( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_decompress_throughput_compressed", gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].decompress_throughput_compressed                     ); \
        ...
        jsonp_long        ( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_insert_throughput_decompressed",   gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].insert_throughput_decompressed                       ); \
        jsonp_long        ( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_insert_accounts_throughput",       gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].insert_accounts_throughput                           ); \
        ...
        jsonp_null( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_read_throughput_compressed"       ); \
        ...
        jsonp_null( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_decompress_throughput_compressed" ); \
        jsonp_null( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_insert_bytes_decompressed"        ); \
        jsonp_null( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_insert_throughput_decompressed"   ); \
...
fd_gui_peers_printf_gossip_stats( fd_gui_peers_ctx_t *  peers ) {
  ...
  jsonp_open_envelope( peers->http, "gossip", "network_stats" );
    jsonp_open_object( peers->http, "value" );
      ...
      double _window_seconds = (double)(fd_long_max( cur->sample_time,  ref->sample_time + 1L) - ref->sample_time) / 1000000000.0;
      ...
      jsonp_open_object( peers->http, "storage" );
        ...
        jsonp_open_array( peers->http, "eps_tx" );
          for( ulong i = 0UL; i<FD_METRICS_ENUM_CRDS_VALUE_CNT; i++ ) jsonp_double( peers->http, NULL, (double)(cur->storage_cnt_tx[ i ] - ref->storage_cnt_tx[ i ]) / _window_seconds );
        jsonp_close_array( peers->http );
        jsonp_open_array( peers->http, "bps_tx" );
          for( ulong i = 0UL; i<FD_METRICS_ENUM_CRDS_VALUE_CNT; i++ ) jsonp_double( peers->http, NULL, (double)(cur->storage_bytes_tx[ i ] - ref->storage_bytes_tx[ i ]) / _window_seconds );
        jsonp_close_array( peers->http );
      jsonp_close_object( peers->http );
      jsonp_open_object( peers->http, "messages" );
        jsonp_open_array( peers->http, "bps_rx" );
          for( ulong i = 0UL; i<FD_METRICS_ENUM_GOSSIP_MESSAGE_CNT; i++ ) jsonp_double( peers->http, NULL, (double)(cur->messages_bytes_rx[ i ] - ref->messages_bytes_rx[ i ]) / _window_seconds );
        jsonp_close_array( peers->http );
        jsonp_open_array( peers->http, "mps_rx" );
          for( ulong i = 0UL; i<FD_METRICS_ENUM_GOSSIP_MESSAGE_CNT; i++ ) jsonp_double( peers->http, NULL, (double)(cur->messages_count_rx[ i ] - ref->messages_count_rx[ i ]) / _window_seconds );
        jsonp_close_array( peers->http );
        jsonp_open_array( peers->http, "bps_tx" );
          for( ulong i = 0UL; i<FD_METRICS_ENUM_GOSSIP_MESSAGE_CNT; i++ ) jsonp_double( peers->http, NULL, (double)(cur->messages_bytes_tx[ i ] - ref->messages_bytes_tx[ i ]) / _window_seconds );
        jsonp_close_array( peers->http );
        jsonp_open_array( peers->http, "mps_tx" );
          for( ulong i = 0UL; i<FD_METRICS_ENUM_GOSSIP_MESSAGE_CNT; i++ ) jsonp_double( peers->http, NULL, (double)(cur->messages_count_tx[ i ] - ref->messages_count_tx[ i ]) / _window_seconds );
        jsonp_close_array( peers->http );
...
fd_gui_printf_shred_updates( fd_gui_t * gui ) {
  jsonp_open_envelope( gui->http, "slot", "shreds" );
    jsonp_open_object( gui->http, "value" );
        jsonp_open_array( gui->http, "slots" );
          for( ulong i=gui->shreds.staged_next_broadcast; i<gui->shreds.staged_cnt; i++ ) jsonp_ulong( gui->http, NULL, gui->shreds.staged[ i % FD_GUI_SHREDS_STAGING_SZ ].slot );
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "shred_idxs" );
          for( ulong i=gui->shreds.staged_next_broadcast; i<gui->shreds.staged_cnt; i++ ) {
        ...
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "events" );
          for( ulong i=gui->shreds.staged_next_broadcast; i<gui->shreds.staged_cnt; i++ ) {
        ...
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "events_ts" );
          for( ulong i=gui->shreds.staged_next_broadcast; i<gui->shreds.staged_cnt; i++ ) jsonp_long_as_str( gui->http, NULL, gui->shreds.staged[ i % FD_GUI_SHREDS_STAGING_SZ ].timestamp );
        jsonp_close_array( gui->http );
    jsonp_close_object( gui->http );
  jsonp_close_envelope( gui->http );
}
...
fd_gui_printf_slot_shred_updates( fd_gui_t * gui,
  ...
  jsonp_open_envelope( gui->http, "slot", "query_shreds" );
    ...
    jsonp_open_object( gui->http, "value" );
        jsonp_open_array( gui->http, "slots" );
          for( ulong i=_start_offset; i<(_end_offset+1UL); i++ ) jsonp_ulong( gui->http, NULL, _slot );
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "shred_idxs" );
          for( ulong i=_start_offset; i<(_end_offset+1UL); i++ ) {
        ...
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "events_ts" );
          for( ulong i=_start_offset; i<(_end_offset+1UL); i++ ) jsonp_long_as_str( gui->http, NULL, gui->shreds.history[ i % FD_GUI_SHREDS_HISTORY_SZ ].timestamp );
        jsonp_close_array( gui->http );
    jsonp_close_object( gui->http );
  jsonp_close_envelope( gui->http );
}