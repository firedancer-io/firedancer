...
fd_gui_printf_slot_max_known( fd_gui_t * gui ) {
  jsonp_open_envelope( gui, "summary", "slot_max_known" );
    ...
    if( FD_LIKELY( slot_max_known_hist_full  ) ) jsonp_ulong( gui, "value", gui->summary.slots_max_known[ 0 ].slot );
    else                                         jsonp_null ( gui, "value" );
  jsonp_close_envelope( gui );
}
...
fd_gui_printf_slot_caught_up( fd_gui_t * gui ) {
  jsonp_open_envelope( gui, "summary", "slot_caught_up" );
    if( FD_LIKELY( gui->summary.slot_caught_up  ) ) jsonp_ulong( gui, "value", gui->summary.slot_caught_up );
    else                                            jsonp_null ( gui, "value" );
  jsonp_close_envelope( gui );
}
...
fd_gui_printf_skipped_history_cluster( fd_gui_t * gui ) {
  jsonp_open_envelope( gui, "slot", "skipped_history_cluster" );
    jsonp_open_array( gui, "value" );
      for( ulong i=0UL; i<fd_ulong_min( gui->summary.slot_completed+1UL, FD_GUI_SLOTS_CNT ); i++ ) {
        ulong _slot = gui->summary.slot_completed-i;
        fd_gui_slot_t * slot = gui->slots[ _slot % FD_GUI_SLOTS_CNT ];
