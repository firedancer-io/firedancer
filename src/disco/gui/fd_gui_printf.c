      ...
      if( FD_LIKELY( !overwritten && processed_all_microblocks ) ) {
        jsonp_open_object( gui, "transactions" );
          ...
          jsonp_open_array( gui, "txn_stop_timestamps_nanos" );
            for( ulong i=0UL; i<txn_cnt; i++) jsonp_long_as_str( gui, NULL, slot->txs.reference_nanos + (long)gui->txs[ (slot->txs.start_offset + i)%FD_GUI_TXN_HISTORY_SZ ]->timestamp_delta_end_nanos );
          jsonp_close_array( gui );
          jsonp_open_array( gui, "txn_compute_units_estimated" );
          ...
          jsonp_open_array( gui, "txn_load_end_timstamps_nanos" );
            for( ulong i=0UL; i<txn_cnt; i++) {
              ...
              long timestamp_delta_load_end = (long)txn->timestamp_delta_start_nanos + (long)(((double)txn->txn_load_end_pct / (double)UCHAR_MAX) * (double)microblock_duration);
          ...
          jsonp_open_array( gui, "txn_exec_end_timstamps_nanos" );
            for( ulong i=0UL; i<txn_cnt; i++) {
              fd_gui_txn_t * txn = gui->txs[ (slot->txs.start_offset + i)%FD_GUI_TXN_HISTORY_SZ ];
              long microblock_duration = (long)txn->timestamp_delta_end_nanos - (long)txn->timestamp_delta_start_nanos;
              long timestamp_delta_load_end = (long)txn->timestamp_delta_start_nanos + (long)(((double)txn->txn_exec_end_pct / (double)UCHAR_MAX) * (double)microblock_duration);
              jsonp_long_as_str( gui, NULL, slot->txs.reference_nanos + timestamp_delta_load_end );
            }
          jsonp_close_array( gui );
