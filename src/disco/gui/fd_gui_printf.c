    ...
    jsonp_open_object( gui, "value" );
      ...
      if( FD_LIKELY( !overwritten && processed_all_microblocks ) ) {
        ...
        jsonp_open_object( gui, "transactions" );
          ...
          jsonp_close_array( gui );
          jsonp_open_array( gui, "txn_arrived_in_slot" );
            for( ulong i=0UL; i<txn_cnt; i++) jsonp_bool( gui, NULL, gui->txs[ (slot->txs.start_offset + i)%FD_GUI_TXN_HISTORY_SZ ]->flags & FD_GUI_TXN_FLAGS_SCHEDULED_WHILE_LEADER );
          jsonp_close_array( gui );
        jsonp_close_object( gui );
      } else {
