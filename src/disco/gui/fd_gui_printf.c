...
fd_gui_printf_client( fd_gui_t * gui ) {
  jsonp_open_envelope( gui->http, "summary", "client" );
    jsonp_open_array( gui->http, "value" );
      if( FD_LIKELY( gui->summary.is_full_client ) ) jsonp_string( gui->http, NULL, "firedancer" );
      else                                           jsonp_string( gui->http, NULL, "frankendancer" );
    jsonp_close_array( gui->http );
  jsonp_close_envelope( gui->http );
}
...
#define HANDLE_SNAPSHOT_STATE(snapshot_type, snapshot_type_upper) \
      if( FD_LIKELY( gui->summary.boot_progress.phase>=FD_GUI_BOOT_PROGRESS_TYPE_LOADING_##snapshot_type_upper##_SNAPSHOT )) { \
        ...
        if( FD_LIKELY( gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].peer_addr!=0U ) ) jsonp_string( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_peer", peer_addr ); \
        else jsonp_null( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_peer" ); /* local snapshots don't have a peer */ \
        jsonp_double      ( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_elapsed",                       (double)(gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].sample_time_nanos - gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].reset_time_nanos) / _ns_per_sec ); \
        ...
        jsonp_double      ( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_read_elapsed",                  (double)gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].sample_time_nanos / _ns_per_sec              ); \
        ...
        jsonp_double      ( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_read_remaining",                (double)gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].read_remaining_nanos / _ns_per_sec           ); \
        ...
        jsonp_double      ( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_decompress_remaining",          (double)gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].decompress_remaining_nanos / _ns_per_sec     ); \
        ...
        jsonp_double      ( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_insert_remaining",              (double)gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].insert_remaining_nanos / _ns_per_sec         ); \
        ...
        jsonp_null( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_peer"                          ); \
        jsonp_null( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_elapsed"                       ); \
        ...
        jsonp_null( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_read_elapsed"                  ); \
        ...
        jsonp_null( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_read_remaining"                ); \
        ...
        jsonp_null( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_decompress_remaining"          ); \
        ...
        jsonp_null( gui->http, "loading_" FD_STRINGIFY(snapshot_type) "_snapshot_insert_remaining"              ); \
...
void
fd_gui_printf_gossip_stats( fd_gui_t * gui ) {
  fd_gui_gossip_stats_t * cur = gui->summary.gossip_stats_current;
  fd_gui_gossip_stats_t * ref = gui->summary.gossip_stats_reference;

  jsonp_open_envelope( gui->http, "gossip", "network_stats" );
    jsonp_open_object( gui->http, "value" );

      jsonp_open_object( gui->http, "health" );
        ...
        if( FD_LIKELY( _ingress_msg_rx_total_delta ) ) jsonp_double( gui->http, "push_rx_pct", (double)_push_msg_rx_success_delta / (double)_ingress_msg_rx_total_delta );
        else                                           jsonp_null( gui->http, "push_rx_pct" );

        if( FD_LIKELY( _ingress_msg_rx_total_delta ) ) jsonp_double( gui->http, "pull_response_rx_pct", (double)_pull_response_msg_rx_success_delta / (double)_ingress_msg_rx_total_delta );
        else                                           jsonp_null( gui->http, "pull_response_rx_pct" );
        ...
        if( FD_LIKELY( _push_crds_rx_total_delta ) ) jsonp_double( gui->http, "push_rx_dup_pct", (double)_push_crds_rx_crds_duplicate_delta / (double)_push_crds_rx_total_delta );
        else                                         jsonp_null( gui->http, "push_rx_dup_pct" );
        ...
        if( FD_LIKELY( _pull_response_crds_rx_total_delta ) ) jsonp_double( gui->http, "pull_response_rx_dup_pct", (double)_pull_response_crds_rx_crds_duplicate_delta / (double)_pull_response_crds_rx_total_delta );
        else                                                  jsonp_null( gui->http, "pull_response_rx_dup_pct" );
        ...
        if( FD_LIKELY( _push_msg_rx_total_delta ) ) jsonp_double( gui->http, "push_rx_msg_bad_pct", (double)_push_msg_rx_failure_delta / (double)_push_msg_rx_total_delta );
        else                                        jsonp_null( gui->http, "push_rx_msg_bad_pct" );

        if( FD_LIKELY( _push_crds_rx_total_delta ) ) jsonp_double( gui->http, "push_rx_entry_bad_pct", (double)_push_crds_rx_failure_delta / (double)_push_crds_rx_total_delta );
        else                                         jsonp_null( gui->http, "push_rx_entry_bad_pct" );

        if( FD_LIKELY( _pull_response_msg_rx_total_delta ) ) jsonp_double( gui->http, "pull_response_rx_msg_bad_pct", (double)_pull_response_msg_rx_failure_delta / (double)_pull_response_msg_rx_total_delta );
        else                                                 jsonp_null( gui->http, "pull_response_rx_msg_bad_pct" );

        if( FD_LIKELY( _pull_response_crds_rx_total_delta ) ) jsonp_double( gui->http, "pull_response_rx_entry_bad_pct", (double)_pull_response_crds_rx_failure_delta / (double)_pull_response_crds_rx_total_delta );
        else                                                  jsonp_null( gui->http, "pull_response_rx_entry_bad_pct" );

        if( FD_LIKELY( 0 ) ) jsonp_double( gui->http, "pull_already_known_pct", 0.0 ); /* todo ... */
        else                 jsonp_null( gui->http, "pull_already_known_pct" );

        /* since these are gauges, we don't take a diff */
        jsonp_ulong( gui->http, "total_stake",              cur->network_health_total_stake              );
        jsonp_ulong( gui->http, "total_peers",              cur->network_health_total_peers              );
        jsonp_ulong( gui->http, "connected_stake",          cur->network_health_connected_stake          );
        jsonp_ulong( gui->http, "connected_staked_peers",   cur->network_health_connected_staked_peers   );
        jsonp_ulong( gui->http, "connected_unstaked_peers", cur->network_health_connected_unstaked_peers );
      jsonp_close_object( gui->http );

      double _window = (double)(fd_long_max( cur->sample_time,  ref->sample_time + 1L) - ref->sample_time) / 1000000000.0;
      jsonp_open_object( gui->http, "ingress" );
        jsonp_double( gui->http, "total_throughput", (double)(cur->network_ingress_total_bytes - ref->network_ingress_total_bytes) / _window );
        jsonp_open_array( gui->http, "peer_names" );
          /* todo ... */
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "peer_throughput" );
          /* todo ... */
        jsonp_close_array( gui->http );
      jsonp_close_object( gui->http );

      jsonp_open_object( gui->http, "egress" );
        jsonp_double( gui->http, "total_throughput", (double)(cur->network_egress_total_bytes - ref->network_egress_total_bytes) / _window );
        jsonp_open_array( gui->http, "peer_names" );
          /* todo ... */
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "peer_throughput" );
      ...
      jsonp_open_object( gui->http, "storage" );
        /* since these are gauges, we don't take a diff */
        jsonp_ulong( gui->http, "capacity", cur->storage_capacity );
        jsonp_ulong( gui->http, "expired_count", cur->storage_expired_cnt );
        jsonp_ulong( gui->http, "evicted_count", cur->storage_evicted_cnt );
        jsonp_open_array( gui->http, "count" );
          for( ulong i = 0UL; i<FD_GUI_GOSSIP_ENTRY_CNT; i++ ) jsonp_ulong( gui->http, NULL, cur->storage_active_cnt[ i ] );
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "eps_tx" );
          for( ulong i = 0UL; i<FD_GUI_GOSSIP_ENTRY_CNT; i++ ) jsonp_double( gui->http, NULL, (double)(cur->storage_cnt_tx[ i ] - ref->storage_cnt_tx[ i ]) / _window );
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "bps_tx" );
          for( ulong i = 0UL; i<FD_GUI_GOSSIP_ENTRY_CNT; i++ ) jsonp_double( gui->http, NULL, (double)(cur->storage_bytes_tx[ i ] - ref->storage_bytes_tx[ i ]) / _window );
        jsonp_close_array( gui->http );
      jsonp_close_object( gui->http );
      jsonp_open_object( gui->http, "messages" );
        jsonp_open_array( gui->http, "bps_rx" );
          for( ulong i = 0UL; i<FD_GUI_GOSSIP_MESSAGE_CNT; i++ ) jsonp_double( gui->http, NULL, (double)(cur->messages_bytes_rx[ i ] - ref->messages_bytes_rx[ i ]) / _window );
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "mps_rx" );
          for( ulong i = 0UL; i<FD_GUI_GOSSIP_MESSAGE_CNT; i++ ) jsonp_double( gui->http, NULL, (double)(cur->messages_count_rx[ i ] - ref->messages_count_rx[ i ]) / _window );
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "bps_tx" );
          for( ulong i = 0UL; i<FD_GUI_GOSSIP_MESSAGE_CNT; i++ ) jsonp_double( gui->http, NULL, (double)(cur->messages_bytes_tx[ i ] - ref->messages_bytes_tx[ i ]) / _window );
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "mps_tx" );
          for( ulong i = 0UL; i<FD_GUI_GOSSIP_MESSAGE_CNT; i++ ) jsonp_double( gui->http, NULL, (double)(cur->messages_count_tx[ i ] - ref->messages_count_tx[ i ]) / _window );
        jsonp_close_array( gui->http );

        /* since these are gauges, we don't take a diff */
        jsonp_open_array( gui->http, "bytes_rx_total" );
          for( ulong i = 0UL; i<FD_GUI_GOSSIP_MESSAGE_CNT; i++ ) jsonp_ulong( gui->http, NULL, cur->messages_bytes_rx[ i ] );
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "bytes_tx_total" );
          for( ulong i = 0UL; i<FD_GUI_GOSSIP_MESSAGE_CNT; i++ ) jsonp_ulong( gui->http, NULL, cur->messages_bytes_tx[ i ] );
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "count_rx_total" );
          for( ulong i = 0UL; i<FD_GUI_GOSSIP_MESSAGE_CNT; i++ ) jsonp_ulong( gui->http, NULL, cur->messages_count_rx[ i ] );
        jsonp_close_array( gui->http );
        jsonp_open_array( gui->http, "count_tx_total" );
          for( ulong i = 0UL; i<FD_GUI_GOSSIP_MESSAGE_CNT; i++ ) jsonp_ulong( gui->http, NULL, cur->messages_count_tx[ i ] );
        jsonp_close_array( gui->http );
      jsonp_close_object( gui->http );
    jsonp_close_object( gui->http );
  jsonp_close_envelope( gui->http );
    ...
    jsonp_open_object( peers->http, "value" );
      jsonp_open_array( peers->http, "changes" );
        ...
        for( fd_gui_peers_live_table_fwd_iter_t iter = fd_gui_peers_live_table_fwd_iter_init( peers->live_table, &peers->client_viewports[ ws_conn_id ].sort_key, peers->contact_info_table ), j = 0;
          ...
          /* Since the sampling interval is almost constant, we can just compare deltas */
          ulong cur_ingress_pull_bytes_diff = cur->cur.gossvf.bytes_rx[ FD_CRDS_OUTCOME_ROUTE_PULL_RESPONSE ]-ref->cur.gossvf.bytes_rx[ FD_CRDS_OUTCOME_ROUTE_PULL_RESPONSE ];
          if( FD_UNLIKELY( cur_ingress_pull_bytes_diff!=ref->diff.gossvf.bytes_rx[ FD_CRDS_OUTCOME_ROUTE_PULL_RESPONSE ] ) ) {
            jsonp_open_object( peers->http, NULL );
              jsonp_ulong ( peers->http, "row_index", peers->client_viewports[ ws_conn_id ].start_row + j );
              jsonp_string( peers->http, "column_name", "Ingress Pull" );
              jsonp_double( peers->http, "new_value", (double)cur_ingress_pull_bytes_diff / ((double)FD_GUI_PEERS_WS_VIEWPORT_UPDATE_INTERVAL_MILLIS / 1000.) );
            jsonp_close_object( peers->http );
          }

          ulong cur_ingress_push_bytes_diff = cur->cur.gossvf.bytes_rx[ FD_CRDS_OUTCOME_ROUTE_PUSH ]-ref->cur.gossvf.bytes_rx[ FD_CRDS_OUTCOME_ROUTE_PUSH ];
          if( FD_UNLIKELY( cur_ingress_push_bytes_diff!=ref->diff.gossvf.bytes_rx[ FD_CRDS_OUTCOME_ROUTE_PUSH ] ) ) {
            jsonp_open_object( peers->http, NULL );
              jsonp_ulong ( peers->http, "row_index", peers->client_viewports[ ws_conn_id ].start_row + j );
              jsonp_string( peers->http, "column_name", "Ingress Push" );
              jsonp_double( peers->http, "new_value", (double)cur_ingress_push_bytes_diff / ((double)FD_GUI_PEERS_WS_VIEWPORT_UPDATE_INTERVAL_MILLIS / 1000.) );
            jsonp_close_object( peers->http );
          }

          ulong cur_egress_pull_bytes_diff = cur->cur.gossip.bytes_tx[ FD_CRDS_OUTCOME_ROUTE_PULL_RESPONSE ]-ref->cur.gossip.bytes_tx[ FD_CRDS_OUTCOME_ROUTE_PULL_RESPONSE ];
          if( FD_UNLIKELY( cur_egress_pull_bytes_diff!=ref->diff.gossip.bytes_tx[ FD_CRDS_OUTCOME_ROUTE_PULL_RESPONSE ] ) ) {
            jsonp_open_object( peers->http, NULL );
              jsonp_ulong ( peers->http, "row_index", peers->client_viewports[ ws_conn_id ].start_row + j );
              jsonp_string( peers->http, "column_name", "Egress Pull" );
              jsonp_double( peers->http, "new_value", (double)cur_egress_pull_bytes_diff / ((double)FD_GUI_PEERS_WS_VIEWPORT_UPDATE_INTERVAL_MILLIS / 1000.) );
            jsonp_close_object( peers->http );
          }

          ulong cur_egress_push_bytes_diff = cur->cur.gossip.bytes_tx[ FD_CRDS_OUTCOME_ROUTE_PUSH ]-ref->cur.gossip.bytes_tx[ FD_CRDS_OUTCOME_ROUTE_PUSH ];
          if( FD_UNLIKELY( cur_egress_push_bytes_diff!=ref->diff.gossip.bytes_tx[ FD_CRDS_OUTCOME_ROUTE_PUSH ] ) ) {
            jsonp_open_object( peers->http, NULL );
              jsonp_ulong ( peers->http, "row_index", peers->client_viewports[ ws_conn_id ].start_row + j );
              jsonp_string( peers->http, "column_name", "Egress Push" );
              jsonp_double( peers->http, "new_value", (double)cur_egress_push_bytes_diff / ((double)FD_GUI_PEERS_WS_VIEWPORT_UPDATE_INTERVAL_MILLIS / 1000.) );
            jsonp_close_object( peers->http );
          }
    ...
    jsonp_open_object( peers->http, "value" );
      ...
      for( fd_gui_peers_live_table_fwd_iter_t iter = fd_gui_peers_live_table_fwd_iter_init( peers->live_table, &peers->client_viewports[ ws_conn_id ].sort_key, peers->contact_info_table ), j = 0;
        ...
        jsonp_open_array( peers->http, row_index_cstr );
          ...
          ulong cur_egress_pull_bytes_diff  = cur->cur.gossip.bytes_tx[ FD_CRDS_OUTCOME_ROUTE_PULL_RESPONSE ]-ref->cur.gossip.bytes_tx[ FD_CRDS_OUTCOME_ROUTE_PULL_RESPONSE ];
          ulong cur_egress_push_bytes_diff  = cur->cur.gossip.bytes_tx[ FD_CRDS_OUTCOME_ROUTE_PUSH ]         -ref->cur.gossip.bytes_tx[ FD_CRDS_OUTCOME_ROUTE_PUSH ];
          jsonp_double( peers->http, "Ingress Pull", (double)cur_ingress_pull_bytes_diff / ((double)FD_GUI_PEERS_WS_VIEWPORT_UPDATE_INTERVAL_MILLIS / 1000.) );
          jsonp_double( peers->http, "Ingress Push", (double)cur_ingress_push_bytes_diff / ((double)FD_GUI_PEERS_WS_VIEWPORT_UPDATE_INTERVAL_MILLIS / 1000.) );
          jsonp_double( peers->http, "Egress Pull", (double)cur_egress_pull_bytes_diff / ((double)FD_GUI_PEERS_WS_VIEWPORT_UPDATE_INTERVAL_MILLIS / 1000.) );
          jsonp_double( peers->http, "Egress Push", (double)cur_egress_push_bytes_diff / ((double)FD_GUI_PEERS_WS_VIEWPORT_UPDATE_INTERVAL_MILLIS / 1000.) );
...
fd_gui_printf_peers_view_resize( fd_gui_peers_ctx_t *  peers ) {
  jsonp_open_envelope( peers->http, "gossip", "peers_size_update" );
    jsonp_ulong( peers->http, "value", peers->contact_info_table_sz );
  jsonp_close_envelope( peers->http );
}
