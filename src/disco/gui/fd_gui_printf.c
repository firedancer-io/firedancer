      ...
      if( FD_LIKELY( !overwritten && processed_all_microblocks ) ) {
        ...
        jsonp_open_object( gui, "transactions" );
          ...
          jsonp_open_array( gui, "txn_compute_units_consumed" );
            for( ulong i=0UL; i<txn_cnt; i++) jsonp_ulong( gui, NULL, gui->txs[ (slot->txs.start_offset + i)%FD_GUI_TXN_HISTORY_SZ ]->compute_units_estimated - gui->txs[ (slot->txs.start_offset + i)%FD_GUI_TXN_HISTORY_SZ ]->compute_units_rebated );
          jsonp_close_array( gui );
