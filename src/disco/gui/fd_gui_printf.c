...
fd_gui_slot_is_late( fd_gui_epoch_t const * rec, ulong s ) {
  uchar exact = rec->latency_exact[ s-rec->start_slot ];
  return fd_int_if( exact==FD_GUI_VOTE_LATENCY_UNKNOWN, 0,
         fd_int_if( exact==FD_GUI_VOTE_LATENCY_NOT_VOTED, 1, exact>=2 ) );
}
...
fd_gui_printf_late_votes_history( fd_gui_t * gui ) {
  ...
  int   have       = rec && gui->summary.slot_rooted!=ULONG_MAX && gui->summary.vote_state!=FD_GUI_VOTE_STATE_NON_VOTING;
        ...
        jsonp_open_array( gui->http, "latency_exact" );
          if( FD_LIKELY( have ) ) {
            ...
            while( s<=end_slot ) {
              ...
              uchar run_exact = rec->latency_exact[ s-rec->start_slot ];
              while( s<=end_slot && fd_gui_slot_is_late( rec, s ) && rec->latency_exact[ s-rec->start_slot ]==run_exact ) s++;
              if( FD_LIKELY( run_exact<=FD_GUI_VOTE_LATENCY_MAX ) ) jsonp_ulong( gui->http, NULL, run_exact );
              else                                                  jsonp_null( gui->http, NULL );
            }
          }
        jsonp_close_array( gui->http );
  ...
  jsonp_open_envelope( gui->http, "slot", "update" );
    jsonp_open_object( gui->http, "value" );
      ...
      jsonp_open_object( gui->http, "publish" );
        ...
        if( FD_UNLIKELY( slot->vote_latency_exact!=UCHAR_MAX ) ) jsonp_ulong( gui->http, "vote_latency_exact", slot->vote_latency_exact );
