...
mytable_private_row_cmp(myrow_t const * a, myrow_t const * b) {
  ...
  for( ulong col = 0UL; col < LIVE_TABLE_COLUMN_CNT; col++ ) {
    ...
    if( FD_LIKELY( mytable_private_sort_keys[ mytable_private_active_sort_key_idx ].dir[ sort_col ]==0 ) ) continue;
    ...
    if( FD_LIKELY( mytable_private_sort_keys[ mytable_private_active_sort_key_idx ].dir[ sort_col ]==-1 ) ) {
      return -res;
    } else if( FD_LIKELY( mytable_private_sort_keys[ mytable_private_active_sort_key_idx ].dir[ sort_col ]==1 ) ) {
      return res;
    } else {
