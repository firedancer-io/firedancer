...
fd_gui_shred_event_iter_begin( fd_gui_t *                  gui,
  ...
  if( FD_LIKELY( gui->db && gui->hist ) ) {
    ...
    if( FD_LIKELY( !fd_gui_hist_range_begin( gui, &iter->_hist_iter, FD_GUI_HIST_SHRED_EVENTS, lo_ns, hi_ns, NULL, NULL ) ) ) {
      iter->_hist_active = 1;
    }
  }
...
fd_gui_shred_batch_flush( fd_gui_t *                     gui,
  ...
  fd_gui_hist_ts_append( gui, FD_GUI_HIST_SHRED_EVENTS, &builder->batch );
...
fd_gui_shred_event_slot_complete( fd_gui_t * gui,
  ...
  if( FD_UNLIKELY( !fd_gui_shred_batch_try_add( builder, slot, timestamp, USHORT_MAX,
