...
#define EMA_FILTER(filt, sample) (filt) = (FD_GUI_EMA_FILTER_ALPHA * (sample)) + (( 1. - FD_GUI_EMA_FILTER_ALPHA ) * (filt))
      ...
      gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].time_remaining_nanos = (long)((double)(_total_size_estimate - _insert_bytes) / gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].insert_throughput_ema);
...
fd_gui_extract_snapshot_slot( fd_restore_snapshot_update_t const * update,
    ...
    if (update->type == FD_PLUGIN_MSG_SNAPSHOT_TYPE_INCREMENTAL) {
        ulong slot1, slot2;
        if ( FD_LIKELY( sscanf( filename, "incremental-snapshot-%lu-%lu-", &slot1, &slot2 ) ) ) {
          *snapshot_slot = slot2;
          return 0;
        }
    } else if (update->type == FD_PLUGIN_MSG_SNAPSHOT_TYPE_FULL) {
        ulong slot;
        if ( FD_LIKELY( sscanf( filename, "snapshot-%lu-", &slot ) ) ) {
          *snapshot_slot = slot;
          return 0;
        }
    }
...
fd_gui_handle_snapshot_update( fd_gui_t *    gui,
  ...
  ulong slot;
  if( FD_UNLIKELY( fd_gui_extract_snapshot_slot( msg, &slot ) ) ) {
    FD_LOG_ERR(( "failed to parse snapshot path %s", msg->read_path ));
  }
