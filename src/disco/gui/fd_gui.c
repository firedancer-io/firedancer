...
fd_gui_new( void *                shmem,
  ...
  gui->shreds.staged_cnt = 0UL;
  gui->shreds.history_cnt = 0UL;
  ...
  switch ( gui->summary.boot_progress.phase ) {
    ...
    case FD_GUI_BOOT_PROGRESS_TYPE_LOADING_INCREMENTAL_SNAPSHOT: {
      ...
      if( FD_UNLIKELY( gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].reset_cnt!=_retry_cnt ) ) {
        ...
        gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].read_throughput_compressed       = 0.;
        gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].decompress_throughput_compressed = 0.;
        gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].insert_throughput_decompressed   = 0.;
        gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].insert_accounts_throughput       = 0.;
      }
      ...
      /* metadata */
      double _secs_elapsed_since_last_sample                                        = (double)fd_long_max(now - gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].sample_time_nanos, 1L) / 1000000000L;
      ...
      /* read stage */
      long _read_bytes_delta = fd_long_max( (long)_read_bytes - (long)gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].read_bytes_compressed, 0L );
      gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].read_throughput_compressed = (long)((double)_read_bytes_delta / _secs_elapsed_since_last_sample);
      gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].read_bytes_compressed      = _read_bytes;

      /* decompress stage */
      long _decompress_compressed_bytes_delta = fd_long_max( (long)_decompress_compressed_bytes - (long)gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].decompress_bytes_compressed, 0L );
      gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].decompress_throughput_compressed = (long)((double)_decompress_compressed_bytes_delta / _secs_elapsed_since_last_sample);
      ...
      /* insert stage */
      long _insert_bytes_delta = fd_long_max( (long)_insert_bytes - (long)gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].insert_bytes_decompressed, 0L);
      gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].insert_throughput_decompressed = (long)((double)_insert_bytes_delta / _secs_elapsed_since_last_sample);
      ...
      ulong _insert_accounts_since_delta = (ulong)fd_long_max( (long)_insert_accounts - (long)gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].insert_accounts_current, 0L);
      gui->summary.boot_progress.loading_snapshot[ snapshot_idx ].insert_accounts_throughput = (long)((double)_insert_accounts_since_delta / _secs_elapsed_since_last_sample );
      ...
      if( FD_UNLIKELY( snapshot_phase == 12UL && slot_max_known_hist_full && gui->summary.slot_completed ) ) {
    ...
    case FD_GUI_BOOT_PROGRESS_TYPE_CATCHING_UP: {
      ...
      gui->summary.boot_progress.catching_up_latest_repair_slot  = gui->summary.slot_repaired;
...
fd_gui_poll( fd_gui_t * gui, long now ) {
  ...
  if( FD_LIKELY( now>gui->next_sample_10millis ) ) {
    ...
    fd_http_server_ws_broadcast( gui->http );
    gui->shreds.staged_next_broadcast = gui->shreds.staged_cnt;
...
fd_gui_request_slot_shreds( fd_gui_t *    gui,
  ...
  ulong slot = slot_param->valueulong;

  ulong _start_offset = gui->slots[ slot % FD_GUI_SLOTS_CNT ]->shreds.start_offset;
  ulong _end_offset   = gui->slots[ slot % FD_GUI_SLOTS_CNT ]->shreds.end_offset;
  ulong _slot         = gui->slots[ slot % FD_GUI_SLOTS_CNT ]->slot;
  if( FD_UNLIKELY( _slot==ULONG_MAX || _start_offset==UINT_MAX || _end_offset==UINT_MAX || gui->shreds.history_cnt > _end_offset + FD_GUI_SHREDS_HISTORY_SZ ) ) {
...
static inline void
fd_gui_try_insert_slot_max_known( fd_gui_t * gui, ulong slot, long now ) {
  for( ulong i=0UL; i<FD_GUI_SHRED_SLOT_HISTORY_SZ; i++ ) {
    /* evict any slots older than 4.8 seconds */
    if( FD_UNLIKELY( gui->summary.slots_max_known[ i ].slot!=ULONG_MAX && now-gui->summary.slots_max_known[ i ].timestamp_arrival_nanos>4800000000L ) ) {
    ...
    if( FD_UNLIKELY( gui->summary.slots_max_known[ i ].slot==slot ) ) {
      gui->summary.slots_max_known[ i ].timestamp_arrival_nanos = now;
      return;
    }
  }

  /* Insert the new slot number, evicting a smaller slot if necessary */
  gui->summary.slots_max_known[ FD_GUI_SHRED_SLOT_HISTORY_SZ ].timestamp_arrival_nanos = now;
...
fd_gui_handle_shred( fd_gui_t * gui,
                     ...
                     int        is_turbine,
                     long       tsorig ) {
  if( FD_LIKELY( is_turbine ) ) fd_gui_try_insert_slot_max_known( gui, slot, tsorig );

  fd_gui_slot_staged_shred_event_t * recv_event = &gui->shreds.staged[ gui->shreds.staged_cnt++ ];
...
fd_gui_handle_repair_slot( fd_gui_t * gui, ulong slot, long now ) {
  (void)now;
  gui->summary.slot_repaired = fd_ulong_max( gui->summary.slot_repaired, slot );
}
...
fd_gui_handle_reset_slot( fd_gui_t * gui,
  ...
  if( FD_LIKELY( _slot!=gui->summary.slot_completed ) ) {
    ...
    if( FD_UNLIKELY( gui->summary.slots_max_known[ 0 ].slot!=ULONG_MAX && gui->summary.slot_completed > gui->summary.slots_max_known[ 0 ].slot ) ) {
      fd_gui_try_insert_slot_max_known( gui, gui->summary.slot_completed, now );
    }
  }
...
fd_gui_handle_completed_slot( fd_gui_t * gui,
                              ...
                              ulong      shred_count,
                              long       replay_time ) {
  fd_gui_slot_t * slot = gui->slots[ completed_slot % FD_GUI_SLOTS_CNT ];
  if( FD_UNLIKELY( slot->slot!=completed_slot ) ) fd_gui_clear_slot( gui, completed_slot, parent_slot );

  slot->completed_time    = replay_time;
  ...
  if( FD_LIKELY( slot->level<FD_GUI_SLOT_LEVEL_COMPLETED ) ) {
    ...
    if( FD_LIKELY( completed_slot<gui->summary.slot_optimistically_confirmed ) ) {
  ...
  slot->total_txn_cnt          = (uint)total_txn_count;
  slot->vote_txn_cnt           = (uint)(total_txn_count - nonvote_txn_count);
  slot->failed_txn_cnt         = (uint)failed_txn_count;
  slot->nonvote_failed_txn_cnt = (uint)nonvote_failed_txn_count;
  slot->transaction_fee        = transaction_fee;
  slot->priority_fee           = priority_fee;
  slot->tips                   = tips;
  slot->shred_cnt              = (uint)shred_count;

  /* Add a "slot complete" event for all of the shreds in this slot */
  if( FD_UNLIKELY( slot->shred_cnt > FD_GUI_MAX_SHREDS_PER_BLOCK ) ) FD_LOG_ERR(( "unexpected shred_cnt=%lu", shred_count ));
  fd_gui_slot_staged_shred_event_t * slot_complete_event = &gui->shreds.staged[ gui->shreds.staged_cnt++ ];
  slot_complete_event->event     = FD_GUI_SLOT_SHRED_SHRED_SLOT_COMPLETE;
  slot_complete_event->timestamp = replay_time;
  slot_complete_event->shred_idx = USHORT_MAX;
  slot_complete_event->fec_idx   = USHORT_MAX;
  slot_complete_event->slot      = completed_slot;
  ...
  if( FD_LIKELY( slot->leader_state==FD_GUI_SLOT_LEADER_UNSTARTED ) ) {
    ...
    slot->compute_units   = (uint)compute_units;
  }

  if( FD_UNLIKELY( gui->epoch.has_epoch[ 0 ] && completed_slot==gui->epoch.epochs[ 0 ].end_slot ) ) {
    gui->epoch.epochs[ 0 ].end_time = slot->completed_time;
  } else if( FD_UNLIKELY( gui->epoch.has_epoch[ 1 ] && completed_slot==gui->epoch.epochs[ 1 ].end_slot ) ) {
  ...
  if( FD_LIKELY( slot->mine ) ) {
    for( ulong i=0UL; i<2UL; i++ ) {
      if( FD_LIKELY( completed_slot>=gui->epoch.epochs[ i ].start_slot && completed_slot<=gui->epoch.epochs[ i ].end_slot ) ) {
        fd_gui_printf_skip_rate( gui, i );
        fd_http_server_ws_broadcast( gui->http );
        break;
      }
...
fd_gui_plugin_message( fd_gui_t *    gui,
  ...
  switch( plugin_msg ) {
    ...
    case FD_PLUGIN_MSG_SLOT_COMPLETED: {
      fd_gui_handle_completed_slot( gui, msg[ 0 ], msg[ 1 ], msg[ 2 ], msg[ 3 ], msg[ 4 ], msg[ 5 ], msg[ 6 ], msg[ 7 ], msg[ 8 ], msg[ 9 ], msg[ 10 ], 0UL, now );
      break;
    }
...
fd_gui_init_slot_txns( fd_gui_t * gui,
  ...
  if ( FD_UNLIKELY( LONG_MAX==slot->txs.reference_nanos ) ) {
    slot->txs.reference_nanos = now;
  }
}
...
fd_gui_microblock_execution_begin( fd_gui_t *   gui,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    txn_entry->timestamp_delta_start_nanos = (int)(now - slot->txs.reference_nanos);
...
fd_gui_microblock_execution_end( fd_gui_t *   gui,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    txn_entry->timestamp_delta_end_nanos = (int)(now - slot->txs.reference_nanos);
