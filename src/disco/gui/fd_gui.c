...
fd_gui_tile_timers_snap( fd_gui_t * gui ) {
  ...
  for( ulong i=0UL; i<gui->topo->tile_cnt; i++ ) {
    ...
    cur[ i ].heartbeat = tile_metrics[ MIDX( GAUGE, TILE, HEARTBEAT_NANOS ) ];
...
fd_gui_network_stats_snap( fd_gui_t *               gui,
  ...
  if( FD_LIKELY( bundle_tile_idx!=ULONG_MAX ) ) {
    ...
    cur->in.tpu += bundle_metrics[ MIDX( COUNTER, BUNDLE, PROTO_RX_BYTES ) ];
  }
...
fd_gui_txn_waterfall_snap( fd_gui_t *               gui,
  ...
  if( pack_tile_idx!=ULONG_MAX ) {
    ...
    cur->out.pack_invalid_bundle =
        pack_metrics[ MIDX( COUNTER, PACK, TXN_PARTIAL_BUNDLE_DROPPED ) ]
    ...
    cur->out.pack_already_executed = pack_metrics[ MIDX( COUNTER, PACK, TXN_ALREADY_EXECUTED_DROPPED ) ];
  ...
  for( ulong i=0UL; i<gui->summary.resolh_tile_cnt; i++ ) {
    ...
    cur->out.resolv_no_ledger += resolv_metrics[ MIDX( COUNTER, RESOLH, NO_BANK_DROPPED ) ];
  ...
  for( ulong i=0UL; i<gui->summary.resolv_tile_cnt; i++ ) {
    ...
    cur->out.resolv_no_ledger += resolv_metrics[ MIDX( COUNTER, RESOLV, NO_BANK_DROPPED ) ];
  ...
  if( FD_UNLIKELY( gui->summary.is_full_client ) ) {
    ...
    for( ulong i=0UL; i<gui->summary.verify_tile_cnt; i++ ) {
      fd_topo_tile_t const * verify = &topo->tiles[ fd_topo_find_tile( topo, "verify", i ) ];
      volatile ulong const * verify_metrics = fd_metrics_tile( verify->metrics );
      cur->in.gossip += verify_metrics[ MIDX( COUNTER, VERIFY, VOTE_RX_GOSSIP ) ];
    }
  } else if( dedup_tile_idx!=ULONG_MAX ) {
    fd_topo_tile_t const * dedup = &topo->tiles[ dedup_tile_idx ];
    volatile ulong const * dedup_metrics = fd_metrics_tile( dedup->metrics );
    cur->in.gossip = dedup_metrics[ MIDX( COUNTER, DEDUP, VOTE_RX_GOSSIP ) ];
  }
...
fd_gui_tile_stats_snap( fd_gui_t *                     gui,
  ...
  for( ulong i=0UL; i<gui->summary.quic_tile_cnt; i++ ) {
    ...
    stats->quic_conn_cnt += quic_metrics[ MIDX( GAUGE, QUIC, CONN_ALLOCATED ) ];
  }
...
fd_gui_run_boot_progress( fd_gui_t * gui, long now ) {
  ...
  switch ( gui->summary.boot_progress.phase ) {
    ...
    case FD_GUI_BOOT_PROGRESS_TYPE_LOADING_INCREMENTAL_SNAPSHOT: {
      ...
      ulong _retry_cnt = fd_ulong_if( snapshot_idx==FD_GUI_BOOT_PROGRESS_FULL_SNAPSHOT_IDX, snapct_metrics[ MIDX( GAUGE, SNAPCT, FULL_DOWNLOAD_RETRY ) ], snapct_metrics[ MIDX( GAUGE, SNAPCT, INCREMENTAL_DOWNLOAD_RETRY ) ]);
