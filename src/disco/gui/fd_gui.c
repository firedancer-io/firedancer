...
fd_gui_clear_slot( fd_gui_t * gui,
  ...
  slot->cus.max_microblocks    = USHORT_MAX;
  ...
  for( ulong i=0UL; i<65UL; i++ ) slot->cus.start_offset[ i ] = UINT_MAX;
  for( ulong i=0UL; i<65UL; i++ ) slot->cus.end_offset[ i ]   = UINT_MAX;
...
fd_gui_became_leader( fd_gui_t * gui,
  ...
  if( FD_LIKELY( slot->cus.max_microblocks==USHORT_MAX ) ) slot->cus.max_microblocks   = (ushort)max_microblocks;
}
...
fd_gui_unbecame_leader( fd_gui_t * gui,
  ...
  slot->cus.max_microblocks = (ushort)microblocks_in_slot;
}
...
fd_gui_execution_begin( fd_gui_t *   gui,
  ...
  if( FD_UNLIKELY( slot->cus.start_offset[ 64UL ]==UINT_MAX ) ) slot->cus.start_offset[ 64UL ] = gui->cus.offset;
...
fd_gui_execution_end( fd_gui_t *   gui,
  ...
  if( FD_UNLIKELY( slot->cus.start_offset[ bank_idx ]==UINT_MAX ) ) slot->cus.start_offset[ bank_idx ] = gui->cus.offset;
  if( FD_UNLIKELY( slot->cus.reference_ticks==LONG_MAX ) ) slot->cus.reference_ticks = tickcount;
