...
fd_gui_microblock_execution_begin( fd_gui_t *   gui,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    txn_entry->compute_units_estimated     = cost_estimate           & ((1u<<21)-1);
    txn_entry->compute_units_requested     = requested_execution_cus & ((1u<<21)-1);
    ...
    txn_entry->flags                      |= fd_uint_if(fd_txn_is_simple_vote_transaction( txn, txn_payload->payload ), FD_GUI_TXN_FLAGS_IS_SIMPLE_VOTE, 0u) & ((1u<<4)-1);
    txn_entry->flags                      |= fd_uint_if(is_bundle, FD_GUI_TXN_FLAGS_FROM_BUNDLE, 0u) & ((1u<<4)-1);
...
fd_gui_microblock_execution_end( fd_gui_t *   gui,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    fd_gui_txn_t * txn_entry = gui->txs[ (pack_txn_idx + i)%FD_GUI_TXN_HISTORY_SZ ];
    txn_entry->bank_idx                  = bank_idx                   & ((1u<<6)-1);
    txn_entry->compute_units_rebated     = txn_p->bank_cu.rebated_cus & ((1u<<21)-1);
    txn_entry->error_code                = error_code                 & ((1u<<6)-1);
