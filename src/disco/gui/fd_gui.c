...
fd_gui_handle_tower_update( fd_gui_t *                   gui,
  ...
  for( ulong i=0UL; i<tower->tower_cnt; i++ ) {
    if( FD_UNLIKELY( tower->tower[ i ].slot<gui->summary.slot_rooted ) ) continue;
  ...
  if( FD_LIKELY( tower->tower_cnt && epoch_idx!=ULONG_MAX ) ) {
    ...
    if( FD_LIKELY( tower->tower_cnt==FD_TOWER_VOTE_MAX ) ) {
      ...
      /* initialize history */
      if( FD_UNLIKELY( uninitialized || gui->summary.vote_latency_history_cnt==0UL ) ) {
        gui->summary.vote_latency_history[ 0 ].slot = epoch_start;
      ...
      if( FD_LIKELY( gui->summary.vote_latency_history[ gui->summary.vote_latency_history_cnt ].slot==vote_slot ) ) {
        if( FD_LIKELY( gui->summary.vote_latency_history[ gui->summary.vote_latency_history_cnt-1UL ].latency==vote_latency ) ) {
          /* we vote for the next slot with the same latency as our
             previous vote */
          gui->summary.vote_latency_history[ gui->summary.vote_latency_history_cnt ].slot++;
        } else {
