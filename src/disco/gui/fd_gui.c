...
fd_gui_microblock_execution_begin( fd_gui_t *   gui,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    txn_entry->flags                      |= (uchar)fd_uint_if(fd_txn_is_simple_vote_transaction( txn, txn_payload->payload ), FD_GUI_TXN_FLAGS_IS_SIMPLE_VOTE, 0u);
    txn_entry->flags                      |= (uchar)fd_uint_if(is_bundle, FD_GUI_TXN_FLAGS_FROM_BUNDLE, 0u);
  }
...
fd_gui_microblock_execution_end( fd_gui_t *   gui,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    txn_entry->compute_units_rebated     = txn_p->bank_cu.rebated_cus & 0x1FFFFFu;
    txn_entry->error_code                = error_code                 & 0x3Fu;
