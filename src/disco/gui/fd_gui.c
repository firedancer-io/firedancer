...
static void
fd_gui_on_recieve_microblock_event(fd_gui_t * gui,
                             long       tickcount,
                             ulong      _slot) {
  fd_gui_slot_t * slot = gui->slots[ _slot % FD_GUI_SLOTS_CNT ];
  if( FD_UNLIKELY( slot->slot!=_slot ) ) fd_gui_clear_slot( gui, _slot, ULONG_MAX );

  /* initialize reference timestamp, or set it to the minimum recieved
     value for this slot */
...
fd_gui_became_leader( fd_gui_t * gui,
  ...
  fd_gui_on_recieve_microblock_event( gui, tickcount, _slot );
  fd_gui_slot_t * slot = gui->slots[ _slot % FD_GUI_SLOTS_CNT ];
...
fd_gui_microblock_execution_begin( fd_gui_t *   gui,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    ulong cost_estimate = fd_pack_compute_cost( txn, txn_payload->payload, &txn_payload->flags, &requested_execution_cus, &priority_rewards, &precompile_sigs, &requested_loaded_accounts_data_cost );

    fd_gui_txn_t * txn_entry = gui->txs[ (pack_txn_idx + i)%FD_GUI_TXN_HISTORY_SZ ];
    txn_entry->compute_units_estimated     = (uint)(cost_estimate);
    txn_entry->compute_units_requested     = (uint)(requested_execution_cus);
    ...
    txn_entry->flags                      |= FD_GUI_TXN_FLAGS_STARTED;
    txn_entry->flags                      |= (uchar)fd_txn_is_simple_vote_transaction( txn, txn_payload->payload ) * FD_GUI_TXN_FLAGS_IS_SIMPLE_VOTE;
    txn_entry->flags                      |= (uchar)is_bundle * FD_GUI_TXN_FLAGS_FROM_BUNDLE;
  }
...
fd_gui_microblock_execution_end( fd_gui_t *   gui,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    fd_gui_txn_t * txn_entry = gui->txs[ (pack_txn_idx + i)%FD_GUI_TXN_HISTORY_SZ ];
    txn_entry->bank_idx                  = (uchar)bank_idx;
    txn_entry->compute_units_rebated     = txn_p->bank_cu.rebated_cus;
    txn_entry->timestamp_delta_end_nanos = (int)((double)(tickcount - slot->txs.reference_ticks) / fd_tempo_tick_per_ns( NULL ));
    txn_entry->error_code                = error_code;
    txn_entry->txn_load_start_pct        = txn_load_start_pct;
    txn_entry->txn_load_end_pct          = txn_load_end_pct;
    txn_entry->txn_exec_start_pct        = txn_exec_start_pct;
    txn_entry->txn_exec_end_pct          = txn_exec_end_pct;
    txn_entry->flags                    |= FD_GUI_TXN_FLAGS_ENDED;
  }
