...
fd_gui_request_timeline( fd_gui_t *   gui,
  ...
  if( !strcmp( key, "query_slots" ) ) {
    ...
    if( FD_UNLIKELY( fd_jtok_fini( j ) ) ) return FD_HTTP_SERVER_CONNECTION_CLOSE_BAD_REQUEST;
    if( !strcmp( key, "query_shreds" ) ) {
      ...
      if( FD_UNLIKELY( count>FD_GUI_TIMELINE_QUERY_MAX_BUCKETS ) ) return FD_HTTP_SERVER_CONNECTION_CLOSE_BAD_REQUEST;
...
fd_gui_timeline_day_get( fd_gui_t * gui,
                         ulong      day_idx ) {
  if( FD_UNLIKELY( !gui->db || !gui->hist || day_idx>=(ulong)LONG_MAX/(ulong)FD_GUI_TIMELINE_DAY_NS ) ) return NULL;
  ...
  for( ulong i=0UL; i<cnt; i++ ) {
    if( gui->timeline_day_cache.entries[ i ].day_idx!=day_idx ) continue;
  ...
  if( fd_gui_store_ts_live_timestamp_bounds( gui->db, FD_GUI_HIST_TIMELINE_DAY, &first, &last ) ) {
    ...
    if( FD_UNLIKELY( fd_gui_hist_range_begin( gui, &it, FD_GUI_HIST_TIMELINE_DAY, first, last, NULL, NULL ) ) ) return NULL;
    while( fd_gui_hist_range_next( &it ) ) {
      ...
      gui->timeline_day_cache.entries[ cnt%FD_GUI_TIMELINE_DAY_CACHE_MAX ].day_idx = d;
      gui->timeline_day_cache.entries[ cnt%FD_GUI_TIMELINE_DAY_CACHE_MAX ].day     = day;
      cnt++;
  ...
  gui->timeline_day_cache.complete = cnt<=FD_GUI_TIMELINE_DAY_CACHE_MAX;
  gui->timeline_day_cache.count    = fd_ulong_min( cnt, FD_GUI_TIMELINE_DAY_CACHE_MAX );
  gui->timeline_day_cache.last_idx = 0UL;
  for( ulong i=0UL; i<gui->timeline_day_cache.count; i++ ) {
    if( gui->timeline_day_cache.entries[ i ].day_idx!=day_idx ) continue;
    gui->timeline_day_cache.last_idx = i;
    return gui->timeline_day_cache.entries[ i ].day;
  }
  if( cnt>FD_GUI_TIMELINE_DAY_CACHE_MAX ) {
...
fd_gui_timeline_day_create( fd_gui_t * gui,
                            ulong      day_idx,
                            long       now ) {
  if( FD_UNLIKELY( !gui->db || !gui->hist || now<0L || day_idx>(ulong)now/(ulong)FD_GUI_TIMELINE_DAY_NS+1UL ||
  ...
  /* Day records are ~36 MB, so build the record in place in the store
     rather than copying it in.  All-ones marks every bucket unknown. */
  long end_time_ns = (long)((day_idx+1UL)*(ulong)FD_GUI_TIMELINE_DAY_NS);
...
static void
fd_gui_timeline_field_add( fd_gui_timeline_day_t * day,
                           int                     g,
                           int                     field,
                           ulong                   idx,
                           ulong                   value ) {
...
static void
fd_gui_timeline_observed( fd_gui_timeline_day_t * day,
                          int                     g,
                          ulong                   idx,
                          ulong                   slot ) {
...
fd_gui_timeline_handle_fec( fd_gui_t * gui,
  ...
  for( int g=0; g<(int)FD_GUI_TIMELINE_STORED_GRANULARITY_CNT; g++ ) {
    ...
    fd_gui_timeline_field_add( day, g, FD_GUI_TIMELINE_FIELD_PUBLISHED, idx, published ? FD_GUI_FEC_PUBLISHED_SHRED_CNT : 0UL );
  }
}
...
fd_gui_timeline_mine_add( fd_gui_t * gui,
  ...
  ulong day_ns = (ulong)timestamp_ns % (ulong)FD_GUI_TIMELINE_DAY_NS;
  for( int g=0; g<(int)FD_GUI_TIMELINE_STORED_GRANULARITY_CNT; g++ ) {
    fd_gui_timeline_field_add( day, g, FD_GUI_TIMELINE_FIELD_MINE,
  ...
  while( cs!=anchor.slot || cb!=anchor.bank_seq ) {
    ...
    for( ulong k=1UL; k<=count; k++ ) {
      ...
      if( skipped ) {
        ...
        FD_TEST( day );
        for( int g=0; g<(int)FD_GUI_TIMELINE_STORED_GRANULARITY_CNT; g++ ) {
          fd_gui_timeline_field_add( day, g, FD_GUI_TIMELINE_FIELD_SKIPPED,
        ...
        for( fd_gui_hist_kv_iter_begin( gui, &it, FD_GUI_HIST_SLOT, s ); it.rec; fd_gui_hist_kv_iter_next( &it ) ) {
          ...
          if( fork->mine && fork->completed_time>=0L && fork->completed_time!=LONG_MAX ) {
            ...
            if( !fork->timeline_mine_skipped_accounted ) {
              ...
              if( md ) {
                for( int g=0; g<(int)FD_GUI_TIMELINE_STORED_GRANULARITY_CNT; g++ ) {
                  fd_gui_timeline_field_add( md, g, FD_GUI_TIMELINE_FIELD_MINE_SKIPPED,
...
fd_gui_record_replay_txn( fd_gui_t *                        gui,
  ...
  fd_gui_timeline_handle_txn( gui, rec->slot, rec->commit_end_ns, rec->compute_units_consumed,
                              rec->block_compute_unit_limit, rec->transaction_fee, rec->priority_fee,
                              rec->tips, rec->is_simple_vote, !rec->error_code, rec->insert_time_ns );
