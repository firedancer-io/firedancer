...
struct fd_gui_slot {
  ...
  struct {
    long leader_start_time;
    long leader_end_time;

    long reference_ticks;
    long reference_nanos;

    ushort max_microblocks;
    ushort begin_microblocks;
    ushort end_microblocks;

    uint   max_compute_units;
    uchar  has_offset[ 65UL ];
    uint   start_offset[ 65UL ];
    uint   end_offset[ 65UL ];
  } cus;
...
fd_gui_cu_history_compress( int   execution_type,
  ...
  ulong comp = 0UL;
  comp |= (ulong)((execution_type&1UL)<<0);
  comp |= (ulong)((active_bank_change&1UL)<<1);
