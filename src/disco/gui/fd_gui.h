...
/* Ideally, we want to store an entire epoch's worth of transactions.
   If we assume any given validator will not have more than 5% stake,
   and average transactions per slot is around 10k, then an epoch will
   have about 432000*0.05*10000 transactions (~2^28).

   Unfortunately, the transaction struct is ~32 bytes.  If we allocated
   all 2^28 bytes then the memory usage here would be ~8.5GB.  In order
   to keep memory usage to a more reasonable level, we'll use a fourth
   of that size. */
#define FD_GUI_TXN_HISTORY_SZ (1UL<<26UL)

#define FD_GUI_TXN_FLAGS_STARTED        ((uchar)1)
#define FD_GUI_TXN_FLAGS_ENDED          ((uchar)2)
#define FD_GUI_TXN_FLAGS_IS_SIMPLE_VOTE ((uchar)4)
#define FD_GUI_TXN_FLAGS_FROM_BUNDLE    ((uchar)8)
...
struct fd_gui {
  ...
  struct {
    ...
    uint compute_units_estimated;
    uint compute_units_rebated;
    uint timestamp_delta_start_nanos;
    uint timestamp_delta_end_nanos;
    uchar flags; /* assigned with the FD_GUI_TXN_FLAGS_* macros */
    uchar bank_idx; /* in [0, 64) */
    uchar error_code; /*  */
    /* todo ... add tip, execution duration */
  } txs[ FD_GUI_TXN_HISTORY_SZ ];
