...
struct LIVE_TABLE_() {
  ...
  ulong activity_timers_nanos[ LIVE_TABLE_MAX_SORT_KEY_CNT ]; /* ULONG_MAX if treap is inactive, nanos UNIX timestamp of last iter.  First entry is special gets initalized to a default sort key and is not removable */
  ulong max_rows;
};
...
LIVE_TABLE_(active_sort_key_cnt)( LIVE_TABLE_(t) * join ) {
  ...
  for( ulong i = 0; i<LIVE_TABLE_MAX_SORT_KEY_CNT; i++ ) {
    FD_TEST( join->activity_timers_nanos[ LIVE_TABLE_DEFAULT_TREAP_IDX ]!=ULONG_MAX );
    if( FD_LIKELY( join->activity_timers_nanos[ i ]!=ULONG_MAX ) ) count++;
  }
  return count;
}
...
LIVE_TABLE_(private_sort_key_delete)( LIVE_TABLE_(t) * join, ulong sort_key_idx ) {
  ...
  join->activity_timers_nanos[ sort_key_idx ] = ULONG_MAX;
}
...
LIVE_TABLE_(private_sort_key_create)( LIVE_TABLE_(t) * join, ulong sort_key_idx, LIVE_TABLE_(sort_key_t) const * sort_key, LIVE_TABLE_ROW_T * pool ) {
  ...
  join->activity_timers_nanos[ sort_key_idx ] = (ulong)fd_log_wallclock();
...
LIVE_TABLE_(private_query_sort_key)( LIVE_TABLE_(t) * join, LIVE_TABLE_(sort_key_t) const * sort_key ) {
  for( ulong i = 0; i<LIVE_TABLE_MAX_SORT_KEY_CNT; i++ ) {
    if( FD_UNLIKELY( join->activity_timers_nanos[ i ]==ULONG_MAX ) ) continue;
    int equal = 1;
    ulong j = 0;
    ulong k = 0;
    ...
    join->activity_timers_nanos[ i ] = (ulong)fd_log_wallclock(); /* todo ... use new clock api */
...
LIVE_TABLE_(private_query_or_add_sort_key)( LIVE_TABLE_(t) * join, LIVE_TABLE_(sort_key_t) const * sort_key, LIVE_TABLE_ROW_T * pool ) {
  ...
  for( ulong i=0UL; i<LIVE_TABLE_MAX_SORT_KEY_CNT; i++ ) {
    ...
    if( FD_LIKELY( join->activity_timers_nanos[ i ]==ULONG_MAX ) ) {
  ...
  ulong oldest_timer_val = join->activity_timers_nanos[ 0UL ];
  ulong oldest_timer_idx = 0UL;
  for( ulong i=0UL; i<LIVE_TABLE_MAX_SORT_KEY_CNT; i++ ) {
    if( FD_UNLIKELY( i==LIVE_TABLE_DEFAULT_TREAP_IDX ) ) continue;
    if( FD_UNLIKELY( oldest_timer_val < join->activity_timers_nanos[ i ] ) ) {
...
LIVE_TABLE_(new)( void * shmem, ulong max_rows ) {
  ...
  for( ulong i = 0; i<LIVE_TABLE_MAX_SORT_KEY_CNT; i++ ) _table->activity_timers_nanos[ i ] = ULONG_MAX;
  ...
  _table->activity_timers_nanos[ LIVE_TABLE_DEFAULT_TREAP_IDX ] = (ulong)fd_log_wallclock();
...
LIVE_TABLE_(leave)( LIVE_TABLE_(t) * join ) {
  ...
  for( ulong i = 0; i<LIVE_TABLE_MAX_SORT_KEY_CNT; i++ ) {
    if( FD_LIKELY( join->activity_timers_nanos[ i ] == ULONG_MAX ) ) continue;
    LIVE_TABLE_(private_active_sort_key_idx) = i;
...
LIVE_TABLE_(delete)( void * shtable ) {
  ...
  for( ulong i = 0; i<LIVE_TABLE_MAX_SORT_KEY_CNT; i++ ) {
    if( FD_LIKELY( table->activity_timers_nanos[ i ] == ULONG_MAX ) ) continue;
    LIVE_TABLE_(private_active_sort_key_idx) = i;
...
LIVE_TABLE_(idx_insert)( LIVE_TABLE_(t) * join, ulong pool_idx, LIVE_TABLE_ROW_T * pool ) {
  ...
  for( ulong i = 0; i<LIVE_TABLE_MAX_SORT_KEY_CNT; i++ ) {
    ...
    join->activity_timers_nanos[ i ] = (ulong)fd_log_wallclock();
  }
