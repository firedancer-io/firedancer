...
LIVE_TABLE_(private_row_cmp)(LIVE_TABLE_ROW_T const * a, LIVE_TABLE_ROW_T const * b) {
  ...
  for( ulong col = 0UL; col < LIVE_TABLE_COLUMN_CNT; col++ ) {
    ...
    if( FD_LIKELY( LIVE_TABLE_(private_sort_keys)[ LIVE_TABLE_(private_active_sort_key_idx) ].dir[ sort_col ]==0 ) ) continue;
    ...
    if( FD_LIKELY( LIVE_TABLE_(private_sort_keys)[ LIVE_TABLE_(private_active_sort_key_idx) ].dir[ sort_col ]==-1 ) ) {
      return -res;
    } else if( FD_LIKELY( LIVE_TABLE_(private_sort_keys)[ LIVE_TABLE_(private_active_sort_key_idx) ].dir[ sort_col ]==1 ) ) {
      return res;
    } else {
