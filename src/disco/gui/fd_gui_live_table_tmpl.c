...
Typical usage:
    ...
    struct myrow {
      ...
      } treaps; // technically LIVE_TABLE_TREAP but defaults to "treaps"
    };
...
#define LIVE_TABLE_COL_ENTRY(col_id, field, lt_func) \
    { .col_name = col_id, .off = __builtin_offsetof(LIVE_TABLE_ROW_T, field), .lt = lt_func }
...
/* global state is ugly. We only have one type of treap and they all
   ...
   LIVE_TABLE_(lt) */
static LIVE_TABLE_(sort_key_t) LIVE_TABLE_(private_sort_keys)[ LIVE_TABLE_MAX_SORT_KEY_CNT+1UL ] = { 0UL };
...
LIVE_TABLE_(private_query_or_add_sort_key)( LIVE_TABLE_(t) * join, LIVE_TABLE_(sort_key_t) const * sort_key, LIVE_TABLE_ROW_T * pool ) {
  ...
  for( ulong i=0UL; i<LIVE_TABLE_MAX_SORT_KEY_CNT; i++ ) {
    ...
    if( FD_UNLIKELY( fd_log_wallclock() > join->activity_timers_nanos[ i ] + LIVE_TABLE_GC_INTERVAL_NANOS || LIVE_TABLE_GC_INTERVAL_NANOS==1 ) ) {
  ...
  long oldest_timer_val = join->activity_timers_nanos[ 0UL ];
...
static inline int LIVE_TABLE_(lt) ( LIVE_TABLE_(sort_key_t) const * sort_key, LIVE_TABLE_ROW_T const * e0, LIVE_TABLE_ROW_T const * e1 ) {
  fd_memcpy( &LIVE_TABLE_(private_sort_keys)[ LIVE_TABLE_MAX_SORT_KEY_CNT ], sort_key, sizeof(LIVE_TABLE_(sort_key_t)) );
