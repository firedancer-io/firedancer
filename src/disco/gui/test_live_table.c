...
#define LIVE_TABLE_GC_INTERVAL_NANOS (1)
#define LIVE_TABLE_NAME test_live_table_gc
#define LIVE_TABLE_COLUMN_CNT (3UL)
#define LIVE_TABLE_MAX_SORT_KEY_CNT TEST_LIVE_TABLE_MAX_SORT_KEY_CNT
#define LIVE_TABLE_COLUMNS LIVE_TABLE_COL_ARRAY( LIVE_TABLE_COL_ENTRY("Pubkey", key, live_table_col_pubkey_lt), LIVE_TABLE_COL_ENTRY("IP Address", ipv4, live_table_col_ipv4_lt), LIVE_TABLE_COL_ENTRY("Some Metric", counter, live_table_col_counter_lt) )
#define LIVE_TABLE_ROW_T test_live_table_row_t
...
static inline void
test_live_table_gc_key( test_live_table_gc_t * table, test_live_table_gc_sort_key_t const * key,  test_live_table_row_t * pool, ulong const * expected, ulong const expected_sz ) {
  for( test_live_table_gc_fwd_iter_t iter = test_live_table_gc_fwd_iter_init( table, key, pool ), i = 0; !test_live_table_gc_fwd_iter_done( iter ); iter = test_live_table_gc_fwd_iter_next( iter, pool ), i++ ) {
    FD_TEST( i<expected_sz );
    if( test_live_table_gc_fwd_iter_idx( iter )!=expected[ i ] ) FD_LOG_ERR(( "expected=%lu actual=%lu", expected[ i ], test_live_table_gc_fwd_iter_idx( iter ) ));
  }
}
...
main( int argc, char ** argv ) {
  ...
  /* rerun tests but garbage collect every time */
  test_live_table_gc_t * table_gc = test_live_table_gc_join( test_live_table_gc_new( scratch, TEST_LIVE_TABLE_ROW_CNT ) );

  test_live_table_gc_sort_key_t keys_gc[] = {
    { .col = { 0, 1, 2 }, .dir =  {  0,  1,  0 } },
    { .col = { 0, 1, 2 }, .dir =  {  0, -1,  0 } },
    { .col = { 0, 1, 2 }, .dir =  { -1, -1,  0 } },
  };
  test_live_table_gc_seed( pool, sizeof(pool)/sizeof(pool[0]), 42UL );
  test_live_table_gc_idx_insert( table_gc, 0UL, pool );
  test_live_table_gc_idx_insert( table_gc, 1UL, pool );
  test_live_table_gc_idx_insert( table_gc, 2UL, pool );
  test_live_table_gc_idx_insert( table_gc, 3UL, pool );

  test_live_table_gc_key( table_gc, &keys_gc[ 0 ], pool, (ulong[]){ 1, 2, 0, 3 }, 4UL );
  test_live_table_gc_key( table_gc, &keys_gc[ 1 ], pool, (ulong[]){ 3, 0, 2, 1 }, 4UL );
  test_live_table_gc_key( table_gc, &keys_gc[ 2 ], pool, (ulong[]){ 3, 2, 1, 0 }, 4UL );
