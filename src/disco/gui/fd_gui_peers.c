...
fd_gui_peers_new( void *             shmem,
    ...
    fd_gui_peers_live_table_verify( ctx->live_table, ctx->contact_info_table ); /* todo ... remove */
...
fd_gui_peers_gossip_stats_snap( fd_gui_peers_ctx_t *          peers,
  ...
  //   gossip_stats->network_ingress_peer_bytes_per_ns[ j ] = cur->gossvf_rx_sum->rate;
  // }
...
fd_gui_peers_poll( fd_gui_peers_ctx_t * peers ) {
  ...
  if( FD_UNLIKELY( fd_gui_peers_ws_conn_rr_advance( peers, now_nanos ) ) ) {
  ...
  if( FD_UNLIKELY( now_nanos >= peers->next_metric_rate_update_nanos ) ) {
    for( fd_gui_peers_node_pubkey_map_iter_t iter = fd_gui_peers_node_pubkey_map_iter_init( peers->node_pubkey_map, peers->contact_info_table );
      ...
      for( ulong i=0UL; i<FD_METRICS_ENUM_GOSSIP_MESSAGE_CNT; i++ ) {
        ...
        long new_rate = (long)(((double)((long)metric->cur - (long)metric->ref) / window) * 1000000000.0);
      ...
      for( ulong i=0UL; i<FD_METRICS_ENUM_GOSSIP_MESSAGE_CNT; i++ ) {
        fd_gui_peers_metric_rate_t * metric = &peer->gossvf_rx[ i ];
        metric->rate = (long)(((double)((long)metric->cur - (long)metric->ref) / window) * 1000000000.0);
...
fd_gui_peers_ws_open( fd_gui_peers_ctx_t * peers, ulong ws_conn_id ) {
  ...
  peers->client_viewports[ ws_conn_id ].row_cnt = 40;
