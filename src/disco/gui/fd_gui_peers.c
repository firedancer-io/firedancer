...
#define ROUNDED(d) ((d) >= 0) ? (long)((d) + 0.5) : (long)((d) - 0.5)
    long cur_egress_push_bps           = ROUNDED( cur->gossip_tx[ FD_CRDS_OUTCOME_ROUTE_PUSH ].rate          * 1000000000. );
    long cur_ingress_push_bps          = ROUNDED( cur->gossvf_rx[ FD_CRDS_OUTCOME_ROUTE_PUSH ].rate          * 1000000000. );
    long cur_egress_pull_response_bps  = ROUNDED( cur->gossip_tx[ FD_CRDS_OUTCOME_ROUTE_PULL_RESPONSE ].rate * 1000000000. );
    long cur_ingress_pull_response_bps = ROUNDED( cur->gossvf_rx[ FD_CRDS_OUTCOME_ROUTE_PULL_RESPONSE ].rate * 1000000000. );
#undef ROUNDED
