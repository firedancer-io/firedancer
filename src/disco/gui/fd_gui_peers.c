...
fd_gui_peers_poll( fd_gui_peers_ctx_t * peers, long now ) {
  ...
  if( FD_UNLIKELY( now >= peers->next_metric_rate_update_nanos ) ) {
    for( fd_gui_peers_node_pubkey_map_iter_t iter = fd_gui_peers_node_pubkey_map_iter_init( peers->node_pubkey_map, peers->contact_info_table );
      ...
      peer->gossvf_rx_sum.rate = (long)(((double)((long)peer->gossvf_rx_sum.cur - (long)peer->gossvf_rx_sum.ref) / window) * 1000000000.0);
      peer->gossvf_rx_sum.ref  = peer->gossvf_rx_sum.cur;

      peer->gossip_tx_sum.rate = (long)(((double)((long)peer->gossip_tx_sum.cur - (long)peer->gossip_tx_sum.ref) / window) * 1000000000.0);
  ...
  if( FD_LIKELY( now >= peers->next_gossip_stats_update_nanos ) ) {
    *peers->gossip_stats_reference = *peers->gossip_stats_current;
    fd_gui_peers_gossip_stats_snap( peers, peers->gossip_stats_current, now );
