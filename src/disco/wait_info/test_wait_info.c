...
/* Writer idles outside its critical section, else seq is odd nearly
   always and the reader can never land a clean read. */

#define HAMMER_WRITER_IDLE_PAUSES (64UL)
...
writer_thread( void * arg ) {
  ...
  while( !hammer_done ) {
    ...
    fd_wait_info_write_end( box );
    for( ulong i=0UL; i<HAMMER_WRITER_IDLE_PAUSES; i++ ) FD_SPIN_PAUSE();
  }
  return NULL;
}
