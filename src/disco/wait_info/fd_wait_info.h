...
/* fd_wait_info provides a shared topology object that holds the data
   ...
   The seqlock uses FD_VOLATILE + FD_COMPILER_MFENCE rather than C11
   atomics.  This is sufficient on x86-64 (TSO): stores are globally
   visible in program order, so compiler fences alone provide the
   acquire/release semantics the seqlock requires.  Single producer
   means no CAS is needed on the write side. */
...
struct fd_wait_info {
  ...
  /* Leader gap */
  ulong reset_slot;
  int   next_leader_known;          /* 0 -> leader schedule unavailable, next_leader_slot means nothing */
  ...
  /* Delinquent stake.  Tower derives it from vote account
     last_vote_slot; Alpenglow from the block footer reward certs.
     0 until the first real figure is computed. */
