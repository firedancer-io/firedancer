...
fd_restart_recv_enough_stake( fd_restart_t * restart ) {
  ulong percentile0 = restart->total_stake_received[0]*100/restart->total_stake[0];
  ulong percentile1 = restart->total_stake_received[1]*100/restart->total_stake[1];
...
fd_restart_recv_last_voted_fork_slots( fd_restart_t * restart,
  ...
  if( FD_UNLIKELY(( voted_epoch!=restart->root_epoch && voted_epoch!=restart->root_epoch+1 )) ) {
    FD_LOG_WARNING(( "Ignore last_voted_fork_slots message for epoch%lu (root_epoch=%lu, MAX_EPOCHS=%lu) from validator %s",
                     voted_epoch, restart->root_epoch, MAX_EPOCHS,
  ...
  if( FD_UNLIKELY( fd_restart_recv_enough_stake( restart ) ) ) {
    ulong stake_threshold[ MAX_EPOCHS ] = { restart->total_stake_received[0]
