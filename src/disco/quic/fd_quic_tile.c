...
metrics_write( fd_quic_ctx_t * ctx ) {
  ...
  FD_MGAUGE_SET( QUIC, CONN_ALLOCATED,  ctx->quic->metrics.conn_alloc_cnt );
  ...
  FD_MCNT_ENUM_COPY( QUIC, PKT_RETRANSMITTED,   ctx->quic->metrics.pkt_retransmissions_cnt );
  FD_MCNT_ENUM_COPY( QUIC, INITIAL_TOKEN_LENGTH, ctx->quic->metrics.initial_token_len_cnt );
