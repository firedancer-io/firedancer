...
static const uint sock_filter_policy_quic_instr_cnt = 28;

static void populate_sock_filter_policy_quic( ulong out_cnt, struct sock_filter out[ static 28 ], uint logfile_fd, uint keylog_fd ) {
  FD_TEST( out_cnt >= 28 );
  struct sock_filter filter[28] = {
    ...
    /* check futex */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_futex, /* check_futex */ 15, 0 ),
