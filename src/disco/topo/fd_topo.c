...
#define SECTION( ... ) do {                                                            \
  ...
  for( ulong i=0UL; i<topo->link_cnt; i++ ) {
    ...
    PRINT( "  %3lu  %s%10s%s  %-13s  %4lu  %4lu  %8lu  %9lu  %5lu\n",
           i, fd_topo_mem_sz_style( sz, c_bold, c_dim ), size, c_normal,
           link->name, link->kind_id, topo->objs[ link->dcache_obj_id ].wksp_id, link->depth, link->mtu, link->burst );
  }
...
/* json_escaped copies src into out, escaping quotes, backslashes and
   control characters.  Truncates safely if out_sz is too small. */

static void
json_escaped( char const * src, char * out, ulong out_sz ) {
  ulong j = 0UL;
  for( ulong i=0UL; src[ i ] && j+7UL<out_sz; i++ ) {
    uchar c = (uchar)src[ i ];
    if( FD_UNLIKELY( c=='"' || c=='\\' ) ) { out[ j++ ] = '\\'; out[ j++ ] = (char)c; }
    else if( FD_UNLIKELY( c<0x20 ) ) {
      FD_TEST( fd_cstr_printf_check( out+j, out_sz-j, NULL, "\\u%04x", (uint)c ) );
      j += 6UL;
    }
    else out[ j++ ] = (char)c;
  }
...
#define PRINT( ... ) do {                                                             \
  ...
  for( ulong i=0UL; i<topo->wksp_cnt; i++ ) {
    fd_topo_wksp_t * wksp = &topo->workspaces[ i ];
    PRINT( "    { \"id\": %lu, \"name\": \"%s\", \"page_cnt\": %lu, \"page_sz\": \"%s\", \"numa_idx\": %lu, \"total_bytes\": %lu, \"footprint\": %lu, \"loose\": %lu }%s\n",
           i, wksp->name, wksp->page_cnt, fd_shmem_page_sz_to_cstr( wksp->page_sz ), wksp->numa_idx,
  ...
  for( ulong i=0UL; i<topo->obj_cnt; i++ ) {
    fd_topo_obj_t * obj = &topo->objs[ i ];
    PRINT( "    { \"id\": %lu, \"name\": \"%s\", \"wksp\": \"%s\", \"wksp_id\": %lu, \"footprint\": %lu, \"offset\": %lu, \"props\": {",
           i, obj->name, topo->workspaces[ obj->wksp_id ].name, obj->wksp_id, obj->footprint, obj->offset );
    int first = 1;
    for( fd_pod_iter_t iter=fd_pod_iter_init( fd_pod_queryf_subpod( topo->props, "obj.%lu", obj->id ) );
         !fd_pod_iter_done( iter );
         iter=fd_pod_iter_next( iter ) ) {
      fd_pod_info_t info = fd_pod_iter_info( iter );
      if( !strncmp( info.key, "seed", info.key_sz ) ) continue;
      char key[ 64 ]; char escaped[ 128 ];
      FD_TEST( fd_cstr_printf_check( key, sizeof(key), NULL, "%.*s", (int)info.key_sz, info.key ) );
      json_escaped( key, escaped, sizeof(escaped) );
      switch( info.val_type ) {
      case FD_POD_VAL_TYPE_CSTR: {
        char val[ 256 ]; char val_escaped[ 512 ];
        FD_TEST( fd_cstr_printf_check( val, sizeof(val), NULL, "%.*s", (int)info.val_sz, (char const *)info.val ) );
        json_escaped( val, val_escaped, sizeof(val_escaped) );
        PRINT( "%s \"%s\": \"%s\"", first ? "" : ",", escaped, val_escaped );
        first = 0;
        break;
      }
      case FD_POD_VAL_TYPE_ULONG: {
        ulong val; fd_ulong_svw_dec( info.val, &val );
        PRINT( "%s \"%s\": %lu", first ? "" : ",", escaped, val );
        first = 0;
        break;
      }
      }
  ...
  for( ulong i=0UL; i<topo->link_cnt; i++ ) {
    fd_topo_link_t * link = &topo->links[ i ];
    PRINT( "    { \"id\": %lu, \"name\": \"%s\", \"kind_id\": %lu, \"wksp_id\": %lu, \"depth\": %lu, \"mtu\": %lu, \"burst\": %lu, \"dcache_bytes\": %lu }%s\n",
           i, link->name, link->kind_id, topo->objs[ link->dcache_obj_id ].wksp_id, link->depth, link->mtu, link->burst,
  ...
  for( ulong i=0UL; i<topo->tile_cnt; i++ ) {
    ...
    PRINT( "    { \"id\": %lu, \"name\": \"%s\", \"kind_id\": %lu, \"wksp_id\": %lu, ",
           i, tile->name, tile->kind_id, topo->objs[ tile->tile_obj_id ].wksp_id );
