...
FD_FN_CONST static inline ulong
fd_disco_replay_repair_sig( ulong slot, uint shred_idx, uint fec_set_idx, int req_type ) {
  ulong slot_ul        = fd_ulong_min( (ulong)slot,        (ulong)UINT_MAX              );
  ulong shred_idx_ul   = fd_ulong_min( (ulong)shred_idx,   (ulong)FD_SHRED_MAX_PER_SLOT );
  ulong fec_set_idx_ul = fd_ulong_min( (ulong)fec_set_idx, (ulong)FD_SHRED_MAX_PER_SLOT );
  ulong req_type_ul    = (ulong)req_type;
  return slot_ul << 32 | shred_idx_ul << 17 | fec_set_idx_ul << 2 | req_type_ul;
}
...
FD_FN_CONST static inline uint  fd_disco_replay_repair_sig_shred_idx  ( ulong sig ) { return  (uint)fd_ulong_extract    ( sig, 17, 31 ); }
FD_FN_CONST static inline uint  fd_disco_replay_repair_sig_fec_set_idx( ulong sig ) { return  (uint)fd_ulong_extract    ( sig, 2, 16  ); }
