...
#ifndef HEADER_fd_src_disco_net_sock_generated_fd_sock_tile_arm64_seccomp_h
#define HEADER_fd_src_disco_net_sock_generated_fd_sock_tile_arm64_seccomp_h
...
#endif
static const unsigned int sock_filter_policy_fd_sock_tile_arm64_instr_cnt = 33;

static void populate_sock_filter_policy_fd_sock_tile_arm64( ulong out_cnt, struct sock_filter * out, uint logfile_fd, uint tx_fd, uint rx_fd0, uint rx_fd1 ) {
  FD_TEST( out_cnt >= 33 );
  struct sock_filter filter[33] = {
    /* Check: Jump to RET_KILL_PROCESS if the script's arch != the runtime arch */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, ( offsetof( struct seccomp_data, arch ) ) ),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, ARCH_NR, 0, /* RET_KILL_PROCESS */ 29 ),
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_sendmmsg, /* check_sendmmsg */ 13, 0 ),
