...
#define TEST_LINKS_OUT_CNT 1
#define TEST_LINKS_CNT     2
...
static void
xdp_find_in_idx( test_link_t  * test_link,
                 fd_net_ctx_t * ctx ) {
...
static ulong
quic_publish( test_ctx_t  * test_ctx,
              test_link_t * quic_net_link ) {
...
static ulong
quic_make_sig( test_ctx_t  * test_ctx,
               test_link_t * quic_net_link FD_PARAM_UNUSED ) {
...
/* before_frag check for quic_net input link: mainly verify the routing logic
   works as expected */
static int
quic_bf_check( test_ctx_t   * test_ctx,
               fd_net_ctx_t * ctx ) {
...
static int
xsk_af_check( test_ctx_t   * test_ctx,
              fd_net_ctx_t * ctx  ) {
  tile_test_locals_t * locals = test_ctx->locals;
...
static void
select_tx_path_in_link( test_link_t  ** test_links,
                        test_ctx_t   *  test_ctx,
