...
add_neighbor( fd_neigh4_hmap_t * join,
  ...
  fd_neigh4_entry_t * e = fd_neigh4_hmap_insert( join, ip4_addr );
  if( FD_UNLIKELY( !e ) ) e = fd_neigh4_hmap_query( join, ip4_addr, NULL );
  FD_TEST( e );
  *e = (fd_neigh4_entry_t) {
    .ip4_addr = ip4_addr,
    .state = FD_NEIGH4_STATE_ACTIVE,
    .mac_addr = { mac0, mac1, mac2, mac3, mac4, mac5 },
    .probe_suppress_until = e->probe_suppress_until
  };
}
