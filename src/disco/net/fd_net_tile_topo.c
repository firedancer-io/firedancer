...
setup_ibeth_tile( fd_topo_t *             topo,
                  ...
                  ulong                   route_max,
                  ulong                   route_peer_max ) {
  fd_topo_tile_t * tile = fd_topob_tile( topo, "ibeth", "ibeth", "metric_in", tile_to_cpu[ topo->tile_cnt ], 0, 0, 0 );
  fd_topob_link( topo, "net_netlnk", "net_netlnk", 128UL, 0UL, 0UL );
  fd_topob_tile_in(  topo, "netlnk", 0UL,         "metric_in", "net_netlnk", tile_kind_id, FD_TOPOB_UNRELIABLE, FD_TOPOB_POLLED );
  fd_topob_tile_out( topo, "ibeth", tile_kind_id,               "net_netlnk", tile_kind_id );
  fd_topob_tile_in(  topo, "ibeth", tile_kind_id, "metric_in", "iproute_out", 0UL, FD_TOPOB_RELIABLE, FD_TOPOB_POLLED );
  fd_netlink_topo_join( topo, netlink_tile, tile );
  ...
  FD_STATIC_ASSERT( sizeof(tile->ibeth.if_name)==IF_NAMESIZE, str_bounds );
  FD_STATIC_ASSERT( sizeof(tile->ibeth.rdma_device)==sizeof(net_cfg->ibverbs.rdma_device), str_bounds );
  fd_cstr_ncpy( tile->ibeth.if_name, net_cfg->interface, IF_NAMESIZE );
  tile->ibeth.net.bind_address = net_cfg->bind_address_parsed;
  fd_cstr_ncpy( tile->ibeth.rdma_device, net_cfg->ibverbs.rdma_device, sizeof(tile->ibeth.rdma_device) );
  tile->ibeth.rdma_port     = net_cfg->ibverbs.rdma_port;
  tile->ibeth.rx_queue_size = net_cfg->ibverbs.rx_queue_size;
  tile->ibeth.tx_queue_size = net_cfg->ibverbs.tx_queue_size;
  tile->ibeth.batch_size    = net_cfg->ibverbs.batch_size;
  tile->ibeth.flow_rule_max = net_cfg->ibverbs.flow_rule_max;

  tile->ibeth.umem_dcache_obj_id = umem_obj->id;
  tile->ibeth.netdev_tbl_obj_id  = netlink_tile->netlink.netdev_tbl_obj_id;
  tile->ibeth.route_max          = route_max;
  tile->ibeth.route_peer_max     = route_peer_max;
  tile->ibeth.route_peer_seed    = 1UL + tile_kind_id;
...
fd_topos_net_tiles( fd_topo_t *             topo,
  ...
  if( 0==strcmp( net_cfg->provider, "xdp" ) ) {
    ...
    if( FD_UNLIKELY( net_tile_cnt!=1UL ) ) {
      FD_LOG_ERR(( "net.provider=\"ibverbs\" requires layout.net_tile_count=1" ));
    }
    setup_ibeth_tile( topo, 0UL, netlink_tile, tile_to_cpu, net_cfg, netlnk_max_routes, netlnk_max_peer_routes );
...
static void
add_umem_rx_link( fd_topo_t *  topo,
...
fd_topos_net_rx_link( fd_topo_t *  topo,
  ...
  if( 0==strcmp( provider, "xdp" ) ) {
    add_umem_rx_link( topo, link_name, net_kind_id, depth, 0UL );
    ...
    if( FD_UNLIKELY( tile_id==ULONG_MAX ) ) FD_LOG_ERR(( "tile ibeth:%lu not found", net_kind_id ));
    add_umem_rx_link( topo, link_name, net_kind_id, depth, topo->tiles[ tile_id ].ibeth.batch_size );
