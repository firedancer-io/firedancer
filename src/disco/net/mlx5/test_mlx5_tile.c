...
test_rx_cqe_normal( void ) {
  ...
  uint rq_chunk[ depth ];
  for( uint i=0U; i<depth; i++ ) rq_chunk[ i ] = 1000U+i;
  fd_mlx5_qp_t qp[1] = {{
    .rx_cq=cq, .rq_chunk=rq_chunk, .rx_depth=depth,
    .rq_prod=5U, .rq_cons=3U
  }};

  test_cqe_push( cq, 3U, FD_MLX5_CQE_RX_OK,  3U, 64U );
  test_cqe_push( cq, 4U, FD_MLX5_CQE_RX_ERR, 4U,  0U );
  ...
  FD_TEST( ok_cqe->op_own==(uchar)(FD_MLX5_CQE_RX_OK<<4) );
  FD_TEST( fd_uint_bswap( err_cqe->byte_cnt )==0U );
  FD_TEST( fd_ushort_bswap( err_cqe->wqe_counter )==4U );
  FD_TEST( err_cqe->op_own==(uchar)((FD_MLX5_CQE_RX_ERR<<4) | 1U) );

  fd_mlx5_tile_rx_comp_t comp[2];
  FD_TEST( fd_mlx5_hw_poll_rx_cq( qp, comp, 2U )==2 );
  FD_TEST( comp[0].chunk==rq_chunk[3] && comp[0].byte_len==64U && comp[0].opcode==FD_MLX5_CQE_RX_OK  );
  FD_TEST( comp[1].chunk==rq_chunk[0] && comp[1].byte_len== 0U && comp[1].opcode==FD_MLX5_CQE_RX_ERR );
  FD_TEST( cq->cons_idx==5U && qp->rq_cons==5U );
  FD_TEST( fd_uint_bswap( control->consumer_idx )==5U );

  qp->rq_prod += 2U;
  test_cqe_push( cq, 5U, FD_MLX5_CQE_RX_OK, qp->rq_cons+1U, 64U );
...
#define CHECK( chunk ) do {                                            \
  ...
  for( ulong i=0UL; i<pending_cnt; i++ ) {
    CHECK( tile->rx_pending[ tile->batch_size-i-1UL ] );
  }

  for( uint idx=mock->rq_nic_cons; idx<tile->hw.qp.rq_prod; idx++ ) {
    CHECK( tile->hw.qp.rq_chunk[ idx & (tile->hw.qp.rx_depth-1U) ] );
  }
  ...
  for( uint idx=tile->hw.rx_cq.cons_idx; idx<mock->rx_cq_prod; idx++ ) {
    ...
    CHECK( tile->hw.qp.rq_chunk[ wqe_counter & (tile->hw.qp.rx_depth-1U) ] );
  }
...
rx_comp_one( fd_mlx5_tile_mock_t * mock,
  ...
  uint chunk = tile->hw.qp.rq_chunk[ wqe_counter & (tile->hw.qp.rx_depth-1U) ];
...
main( int     argc,
  ...
  FD_TEST( batch_sz && batch_sz<=FD_MLX5_BATCH_MAX );
  ...
  tile->hw.uar.reg = (volatile uchar *)&mock->sq_doorbell;
  tile->hw.rx_cq = (fd_mlx5_cq_t) {
    .entries=rx_cq, .control=rx_cq_control, .depth=(uint)rxq_depth };
  tile->hw.tx_cq = (fd_mlx5_cq_t) {
    .entries=tx_cq, .control=tx_cq_control, .depth=(uint)txq_depth };
  ...
  ulong const rx_cqe_err_batch0 = tile->metrics.rx_cqe_err_cnt;
  for( ulong i=1UL; i<batch_size; i++ ) {
    rx_comp_one( mock, tile, FD_MLX5_CQE_RX_ERR, 0UL );
  }
  ...
  FD_TEST( tile->rx_pending_rem==batch_size );
  FD_TEST( test_rx_wq_cnt( mock, tile )==rx_fill_cnt );
  FD_TEST( tile->metrics.rx_cqe_err_cnt==rx_cqe_err_batch0+batch_size );
  ...
  rx_comp_one( mock, tile, FD_MLX5_CQE_RX_OK, 0UL );
  after_credit( tile, stem, &poll_in, &charge_busy );
  FD_TEST( fd_seq_ne( fd_frag_meta_seq_query( rx_link->mcache+rx_seq ), rx_seq ) );
  ...
  FD_TEST( fd_seq_ne( fd_frag_meta_seq_query( rx_link->mcache+rx_seq ), rx_seq ) );
  ulong   rx_chunk  = rx_comp_one( mock, tile, FD_MLX5_CQE_RX_OK, sizeof(rx_pkt_templ) );
  ...
  FD_TEST( fd_seq_ne( fd_frag_meta_seq_query( rx_link->mcache+rx_seq ), rx_seq ) );
  rx_chunk  = rx_comp_one( mock, tile, FD_MLX5_CQE_RX_OK, sizeof(rx_pkt_templ) );
  ...
  FD_TEST( fd_seq_ne( fd_frag_meta_seq_query( rx_link->mcache+rx_seq ), rx_seq ) );
  rx_packet = fd_chunk_to_laddr( tile->umem_base, rx_comp_one( mock, tile, FD_MLX5_CQE_RX_OK, 64UL ) );
  ...
  ulong const rx_cqe_err_batch1 = tile->metrics.rx_cqe_err_cnt;
  ...
  FD_TEST( !test_rx_cq_cnt( mock, tile ) );
  verify_balances( tile, stem, mock, frame_track );
  FD_TEST( tile->metrics.rx_cqe_err_cnt==rx_cqe_err_batch1+batch_size+1UL );
  ...
  ulong const tx_route_fail_before = tile->router.metrics.tx_route_fail_cnt;
  ...
  FD_TEST( tile->metrics.tx_gre_unsupported_cnt==1UL );

  /* TX packet targeting unknown neighbor */
  FD_TEST( 1==before_frag( tile, 0UL, tx_seq,
           fd_disco_netmux_sig( 0U, 0, neigh1_ip4_addr, DST_PROTO_OUTGOING, 0UL ) ) );
  verify_balances( tile, stem, mock, frame_track );
  FD_TEST( tile->router.metrics.tx_route_fail_cnt==tx_route_fail_before+3UL );
  ...
  ulong const tx_cqe_before   = tile->metrics.tx_cqe_err_cnt;
  tx_comp_batch( mock, tile, FD_MLX5_CQE_TX_OK );
  verify_balances( tile, stem, mock, frame_track );
  charge_busy = 0;
  before_credit( tile, stem, &charge_busy );
