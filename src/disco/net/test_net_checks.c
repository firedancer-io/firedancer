...
static void
test_fd_ip4_hdr_check_len( void ) {
  ...
  err = fd_ip4_hdr_check_len( test_ip4, pkt_sz, FD_IP4_HDR_PROTO_MASK_UDP );
  FD_TEST( err==FD_NET_SUCCESS );
  err = fd_ip4_hdr_check_len( test_ip4, pkt_sz, FD_IP4_HDR_PROTO_MASK_BOTH );
  FD_TEST( err==FD_NET_SUCCESS );
  err = fd_ip4_hdr_check_len( test_ip4, pkt_sz, FD_IP4_HDR_PROTO_MASK_GRE );
  ...
  err = fd_ip4_hdr_check_len( test_ip4, test_sz, FD_IP4_HDR_PROTO_MASK_BOTH );
  FD_TEST( err==FD_NET_ERR_INVAL_IP4_HDR );

  FD_LOG_NOTICE(( "fd_ip4_hdr_check_len: pass" ));
}

static void
test_fd_udp_hdr_check_len( void ) {
  ...
  err = fd_udp_hdr_check_len( test_udp, pkt_sz );
  FD_TEST( err==FD_NET_SUCCESS );

  /* udp_sz < sizeof(fd_udp_hdr_t) fails */
  fd_memcpy( test_udp, valid_udp, pkt_sz );
  err = fd_udp_hdr_check_len( (fd_udp_hdr_t *)pkt, sizeof(fd_udp_hdr_t)-1 );
  ...
  FD_LOG_NOTICE(( "fd_udp_hdr_check_len: pass" ));
}

static void
test_fd_eth_ip4_hdrs_check_len( void ) {
  uchar pkt[128];
  ulong pkt_sz;
  int err;
  ...
  err = fd_eth_ip4_hdrs_check_len( test_eth, pkt_sz, &out_ip4, FD_IP4_HDR_PROTO_MASK_UDP );
  FD_TEST( err==FD_NET_SUCCESS && out_ip4==exp_ip4 ); out_ip4 = NULL;
  err = fd_eth_ip4_hdrs_check_len( test_eth, pkt_sz, &out_ip4, FD_IP4_HDR_PROTO_MASK_BOTH );
  FD_TEST( err==FD_NET_SUCCESS && out_ip4==exp_ip4 ); out_ip4 = NULL;
  err = fd_eth_ip4_hdrs_check_len( test_eth, pkt_sz, &out_ip4, FD_IP4_HDR_PROTO_MASK_GRE );
  ...
  fd_memcpy( test_eth, valid_headers, valid_headers_sz );
  pkt_sz = valid_headers_sz;
  err = fd_eth_ip4_hdrs_check_len( test_eth, pkt_sz, NULL, FD_IP4_HDR_PROTO_MASK_UDP );
  FD_TEST( err==FD_NET_SUCCESS && out_ip4==NULL );

  FD_LOG_NOTICE(( "fd_eth_ip4_hdrs_check_len: pass" ));
}

static void
test_fd_ip4_udp_hdrs_check_len( void ) {
  uchar pkt[128];
  int err;
  ...
  fd_memcpy( test_eth, valid_headers, valid_headers_sz );
  err = fd_ip4_udp_hdrs_check_len( test_eth, valid_headers_sz, &out_ip4, &out_udp );
  FD_TEST( err==FD_NET_SUCCESS && out_ip4==test_ip4 && out_udp==test_udp );
  out_ip4 = NULL; out_udp = NULL;

  /* NULL opt_ip4 pointer passes */
  fd_memcpy( test_eth, valid_headers, valid_headers_sz );
  err = fd_ip4_udp_hdrs_check_len( test_eth, valid_headers_sz, NULL, &out_udp );
  FD_TEST( err==FD_NET_SUCCESS && out_udp==test_udp ); out_udp = NULL;

  /* NULL opt_udp pointer passes */
  fd_memcpy( test_eth, valid_headers, valid_headers_sz );
  err = fd_ip4_udp_hdrs_check_len( test_eth, valid_headers_sz, &out_ip4, NULL );
  ...
  err = fd_ip4_udp_hdrs_check_len( test_eth, test_pkt_sz, &out_ip4, &out_udp );
  ...
  fd_memcpy( test_eth, valid_gre_headers, valid_gre_headers_sz );
  err = fd_ip4_udp_hdrs_check_len( test_eth, valid_gre_headers_sz, &out_ip4, &out_udp );
  ...
  FD_LOG_NOTICE(( "fd_ip4_udp_hdrs_check_len: pass" ));
}

static void
test_fd_gre_hdr_check_len( void ) {
  fd_gre_hdr_t const * gre_hdr = fd_type_pun_const( valid_gre_headers );
  ulong const too_small = sizeof( fd_gre_hdr_t ) - 1;
  FD_TEST( fd_gre_hdr_check_len( gre_hdr, too_small ) == FD_NET_ERR_INVAL_GRE_HDR );
  FD_TEST( fd_gre_hdr_check_len( gre_hdr, sizeof( fd_gre_hdr_t ) ) == FD_NET_SUCCESS );
}
...
main( int     argc,
  ...
  test_fd_ip4_hdr_check_len();
  test_fd_udp_hdr_check_len();
  test_fd_eth_ip4_hdrs_check_len();
  test_fd_ip4_udp_hdrs_check_len();
  test_fd_gre_hdr_check_len();
