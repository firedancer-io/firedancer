...
#define FD_IBETH_RX_BATCH_MAX (64UL)
#define FD_IBETH_TX_BATCH_MAX (64UL)
...
struct fd_ibeth_tx_cqe {
  ulong              wr_id;
  enum ibv_wc_status status;
};
typedef struct fd_ibeth_tx_cqe fd_ibeth_tx_cqe_t;
...
struct fd_ibeth_tile {
  ...
  uchar          in_kind[ FD_IBETH_TXQ_MAX ];
  ...
  fd_ibeth_recv_wr_t rx_pending[ FD_IBETH_RX_BATCH_MAX ];
  fd_ibeth_send_wr_t tx_pending[ FD_IBETH_TX_BATCH_MAX ];
...
static ulong
scratch_align( void ) {
  return fd_ulong_max( fd_ulong_max( alignof(fd_ibeth_tile_t), tx_free_align() ),
...
fd_ibeth_dev_contains_if( struct ibv_device * dev,
  ...
  if( FD_UNLIKELY( !dir ) ) {
    FD_LOG_WARNING(( "opendir(%s) failed (%i-%s), skipping ibverbs device %s",
...
fd_ibeth_dev_open( fd_topo_tile_t const * tile ) {
  ...
  if( FD_UNLIKELY( !dev && fd_bonding_is_master( tile->ibeth.if_name ) ) ) {
    fd_bonding_slave_iter_t iter_[1];
    fd_bonding_slave_iter_t * iter = fd_bonding_slave_iter_init( iter_, tile->ibeth.if_name );
    for( ; !fd_bonding_slave_iter_done( iter ); fd_bonding_slave_iter_next( iter ) ) {
      char const * slave = fd_bonding_slave_iter_ele( iter );
      for( int i=0; i<device_cnt; i++ ) {
        struct ibv_device * dev_candidate = dev_list[ i ];
        if( FD_UNLIKELY( !dev_candidate ) ) break;
        if( !fd_ibeth_dev_contains_if( dev_candidate, slave ) ) continue;
        if( FD_UNLIKELY( dev && dev!=dev_candidate ) ) {
          FD_LOG_ERR(( "bond interface `%s` spans multiple ibverbs devices (`%s` and `%s`)",
                       tile->ibeth.if_name, dev->name, dev_candidate->name ));
        }
        dev = dev_candidate;
      }
    }
...
fd_ibeth_rx_recycle( fd_ibeth_tile_t * ctx,
  ...
  if( !ctx->rx_pending_rem || flush ) {
    ...
    ctx->rx_pending_rem = FD_IBETH_RX_BATCH_MAX;
  }
}
...
rxq_assign_all( fd_ibeth_tile_t *       ctx,
  ...
  rxq_assign( ctx, topo, tile, DST_PROTO_REPAIR,   "net_shred",  tile->ibeth.net.repair_client_listen_port      );
...
privileged_init( fd_topo_t const *      topo,
  ...
  struct ibv_cq_init_attr_ex rx_cq_attr = {
    .cqe       = tile->ibeth.rx_queue_size,
    .wc_flags  = IBV_WC_EX_WITH_BYTE_LEN,
    .comp_mask = IBV_CQ_INIT_ATTR_MASK_FLAGS,
    .flags     = IBV_CREATE_CQ_ATTR_SINGLE_THREADED
  };
  ctx->rx_cq = ibv_create_cq_ex( ibv_context, &rx_cq_attr );
  if( FD_UNLIKELY( !ctx->rx_cq ) ) {
    FD_LOG_ERR(( "ibv_create_cq_ex(rx,cqe=%u) failed", tile->ibeth.rx_queue_size ));
  }

  struct ibv_cq_init_attr_ex tx_cq_attr = {
    .cqe       = tile->ibeth.tx_queue_size,
    .comp_mask = IBV_CQ_INIT_ATTR_MASK_FLAGS,
    .flags     = IBV_CREATE_CQ_ATTR_SINGLE_THREADED
  };
  ctx->tx_cq = ibv_create_cq_ex( ibv_context, &tx_cq_attr );
  if( FD_UNLIKELY( !ctx->tx_cq ) ) {
    FD_LOG_ERR(( "ibv_create_cq_ex(tx,cqe=%u) failed", tile->ibeth.tx_queue_size ));
  }

  /* Create queue pair */
  struct ibv_qp_init_attr qp_init_attr = {
    .qp_context = NULL,
    .recv_cq = ibv_cq_ex_to_cq( ctx->rx_cq ),
    .send_cq = ibv_cq_ex_to_cq( ctx->tx_cq ),
    .cap = {
...
unprivileged_init( fd_topo_t const *      topo,
  ...
  for( uint i=0U; i<FD_IBETH_RX_BATCH_MAX; i++ ) {
    fd_ibeth_recv_wr_t * verb = &ctx->rx_pending[ i ];
    memset( verb, 0, sizeof(fd_ibeth_recv_wr_t) );
    verb->wr->next    = (i<(FD_IBETH_RX_BATCH_MAX-1)) ? ctx->rx_pending[i+1].wr : NULL;
  ...
  for( ulong i=0UL; i<rx_fill_cnt; i++ ) {
    fd_ibeth_rx_recycle( ctx, next_chunk, i+1UL==rx_fill_cnt );
  ...
  /* Init TX */
  if( FD_UNLIKELY( tile->in_cnt>FD_IBETH_TXQ_MAX ) ) {
    FD_LOG_ERR(( "ibeth tile in link count %lu exceeds max of %lu", tile->in_cnt, FD_IBETH_TXQ_MAX ));
  }
...
fd_ibeth_rx_pkt( fd_ibeth_tile_t *   ctx,
  ...
  if( FD_UNLIKELY( rule_idx==ULONG_MAX ) ) return wr_id;
  ulong const out_idx = ctx->dst_out_idx[ rule_idx ];
  if( FD_UNLIKELY( out_idx>=ctx->rx_out_cnt ) ) return wr_id;
  ...
  ulong const proto  = ctx->dst_protos[ rule_idx ];
...
static inline ulong
fd_ibeth_tx_wr_id( uint chunk,
                   uint sz ) {
  return (ulong)chunk | ((ulong)sz<<32);
}

static inline uint
fd_ibeth_tx_wr_chunk( ulong wr_id ) {
  return (uint)wr_id;
}

static inline uint
fd_ibeth_tx_wr_sz( ulong wr_id ) {
  return (uint)(wr_id>>32);
}
...
fd_ibeth_tx_flush( fd_ibeth_tile_t * ctx ) {
  ...
  int send_err = ibv_post_send( ctx->qp, ctx->tx_pending[ 0 ].wr, &bad_wr );
  if( FD_UNLIKELY( send_err ) ) {
    struct ibv_send_wr * wr = bad_wr ? bad_wr : ctx->tx_pending[ 0 ].wr;
    for( ; wr; wr=wr->next ) fd_ibeth_tx_recycle( ctx, fd_ibeth_tx_wr_chunk( wr->wr_id ) );
  }
...
fd_ibeth_poll_rx( fd_ibeth_tile_t *   ctx,
                  fd_stem_context_t * stem ) {
  struct ibv_cq_ex *      cq      = ctx->rx_cq;
  struct ibv_poll_cq_attr cq_attr = {0};
  int poll_err = ibv_start_poll( cq, &cq_attr );
  if( FD_UNLIKELY( poll_err ) ) {
    if( FD_UNLIKELY( poll_err!=ENOENT ) ) FD_LOG_ERR(( "ibv_cq_ex RX poll failed (%i)", poll_err ));
    return 0;
  }

  fd_ibeth_rx_cqe_t batch[ FD_IBETH_RX_BATCH_MAX ];
  uint batch_cnt = 0U;
  for(;;) {
    batch[ batch_cnt++ ] = (fd_ibeth_rx_cqe_t) {
      .wr_id    = cq->wr_id,
      .byte_len = ibv_wc_read_byte_len( cq ),
      .status   = cq->status
    };
    if( FD_UNLIKELY( batch_cnt==FD_IBETH_RX_BATCH_MAX ) ) {
      poll_err = 0;
      break;
    }
  ...
  if( FD_UNLIKELY( poll_err && poll_err!=ENOENT ) ) {
    FD_LOG_ERR(( "ibv_cq_ex RX poll failed (%i)", poll_err ));
  }

  for( uint i=0U; i<batch_cnt; i++ ) {
    ulong const chunk = batch[ i ].wr_id;
    if( FD_LIKELY( batch[ i ].status==IBV_WC_SUCCESS &&
  ...
  for( uint i=0U; i<batch_cnt; i++ ) {
    ...
    ulong freed_chunk = fd_ibeth_rx_pkt( ctx, stem, cqe->wr_id, cqe->byte_len, cqe->status );
    if( FD_UNLIKELY( !freed_chunk ) ) FD_LOG_CRIT(( "invalid chunk in mcache" ));
    fd_ibeth_rx_recycle( ctx, freed_chunk, i+1U==batch_cnt );
  }
  return 1;
}
...
fd_ibeth_poll_tx( fd_ibeth_tile_t * ctx ) {
  struct ibv_cq_ex *      cq      = ctx->tx_cq;
  struct ibv_poll_cq_attr cq_attr = {0};
  int poll_err = ibv_start_poll( cq, &cq_attr );
  if( FD_UNLIKELY( poll_err ) ) {
    if( FD_UNLIKELY( poll_err!=ENOENT ) ) FD_LOG_ERR(( "ibv_cq_ex TX poll failed (%i)", poll_err ));
    return 0;
  }

  fd_ibeth_tx_cqe_t batch[ FD_IBETH_TX_BATCH_MAX ];
  uint batch_cnt = 0U;
  for(;;) {
    batch[ batch_cnt++ ] = (fd_ibeth_tx_cqe_t) {
      .wr_id  = cq->wr_id,
      .status = cq->status
    };
    if( FD_UNLIKELY( batch_cnt==FD_IBETH_TX_BATCH_MAX ) ) {
      poll_err = 0;
      break;
    }
    poll_err = ibv_next_poll( cq );
  ...
  if( FD_UNLIKELY( poll_err && poll_err!=ENOENT ) ) {
    FD_LOG_ERR(( "ibv_cq_ex TX poll failed (%i)", poll_err ));
  }

  for( uint i=0U; i<batch_cnt; i++ ) {
    fd_ibeth_tx_cqe_t const * cqe = batch+i;
    if( FD_LIKELY( cqe->status==IBV_WC_SUCCESS ) ) {
      ctx->metrics.tx_pkt_cnt++;
      ctx->metrics.tx_bytes_total += fd_ibeth_tx_wr_sz( cqe->wr_id );
    }
    fd_ibeth_tx_recycle( ctx, fd_ibeth_tx_wr_chunk( cqe->wr_id ) );
  }
  return 1;
}
...
before_frag( fd_ibeth_tile_t * ctx,
  ...
  uint is_gre_inf   = 0U;
  if( FD_UNLIKELY( !fd_net_tx_route( &ctx->r, dst_ip, &is_gre_inf ) ) ) return 1;
  if( FD_UNLIKELY( is_gre_inf ) ) return 1;
...
after_frag( fd_ibeth_tile_t *   ctx,
  ...
  fd_ibeth_send_wr_t * verb = &ctx->tx_pending[ batch_idx ];
  verb->sge[0] = (struct ibv_sge) {
    .addr   = (ulong)frame,
    .length = (uint)sz,
    .lkey   = ctx->mr_lkey
  };
  verb->wr[0] = (struct ibv_send_wr) {
    .wr_id      = fd_ibeth_tx_wr_id( (uint)chunk, (uint)sz ),
    .sg_list    = verb->sge,
    .num_sge    = 1,
    .opcode     = IBV_WR_SEND,
    .send_flags = IBV_SEND_SIGNALED
  };
  if( batch_idx ) ctx->tx_pending[ batch_idx-1U ].wr->next = verb->wr;

  /* Consume frame */
  tx_free_pop_head( ctx->tx_free );
  ctx->tx_pending_cnt  = batch_idx+1U;
  ctx->tx_pending_idle = 0U;
  if( FD_UNLIKELY( ctx->tx_pending_cnt==FD_IBETH_TX_BATCH_MAX ) ) fd_ibeth_tx_flush( ctx );
}
...
#define STEM_BURST                  FD_IBETH_RX_BATCH_MAX
