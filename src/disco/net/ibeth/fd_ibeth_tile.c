...
unprivileged_init( fd_topo_t *      topo,
  ...
  /* Write the sentinel value to all out links (in case mcache is not
...
after_credit( fd_ibeth_tile_t *   ctx,
  ...
  *charge_busy = 1;
  uint cqe_avail = (uint)ctx->cq->cqe;
  do {
    ...
    if( FD_LIKELY( opcode==IBV_WC_RECV ) ) {
      ...
      if( FD_LIKELY( freed_chunk ) ) {  /* assumes 0 is sentinel */
        fd_ibeth_rx_recycle( ctx, wr_id, 0 );
      }
