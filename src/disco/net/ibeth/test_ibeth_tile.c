...
#define CHECK( chunk ) do {                                            \
  ...
  /* RX pending batch */
  FD_TEST( tile->rx_pending_rem <= FD_IBETH_RX_BATCH_MAX );
  ulong const pending_cnt = FD_IBETH_RX_BATCH_MAX - tile->rx_pending_rem;
  for( ulong i=0UL; i<pending_cnt; i++ ) {
    struct ibv_recv_wr const * wr = tile->rx_pending[ FD_IBETH_RX_BATCH_MAX-i-1 ].wr;
    CHECK( wr->wr_id );
  ...
  struct ibv_wc * wc_q = mock->rx_cq->wc_q;
...
#define CHECK( chunk ) do {                                            \
  ...
  /* TX pending batch */
  FD_TEST( tile->tx_pending_cnt<=FD_IBETH_TX_BATCH_MAX );
  for( uint i=0U; i<tile->tx_pending_cnt; i++ ) {
    struct ibv_send_wr const * wr = tile->tx_pending[ i ].wr;
    uint const chunk = fd_ibeth_tx_wr_chunk( wr->wr_id );
    CHECK( chunk );
    void const * frame = fd_chunk_to_laddr_const( tile->umem_base, chunk );
    FD_TEST( wr->num_sge==1 );
    FD_TEST( wr->sg_list[0].addr==(ulong)frame );
    FD_TEST( wr->sg_list[0].length==fd_ibeth_tx_wr_sz( wr->wr_id ) );
  }
  ...
  for( fd_ibv_send_wr_q_iter_t iter = fd_ibv_send_wr_q_iter_init( tx_q );
    ...
    FD_TEST( wr );
    CHECK( fd_ibeth_tx_wr_chunk( wr->wr_id ) );
  }

  /* Completion queue entries */
  struct ibv_wc * wc_q = mock->tx_cq->wc_q;
  for( fd_ibv_wc_q_iter_t iter = fd_ibv_wc_q_iter_init( wc_q );
       !fd_ibv_wc_q_iter_done( wc_q, iter );
       iter = fd_ibv_wc_q_iter_next( wc_q, iter ) ) {
    struct ibv_wc const * wc = fd_ibv_wc_q_iter_ele_const( wc_q, iter );
    ...
    if( wc->opcode == IBV_WC_SEND ) {
      CHECK( fd_ibeth_tx_wr_chunk( wc->wr_id ) );
    }
  }
...
rx_complete_one( fd_ibverbs_mock_qp_t * mock,
  ...
  FD_TEST( fd_ibv_wc_q_avail   ( mock->rx_cq->wc_q ) );
  struct ibv_recv_wr const * wr = fd_ibv_recv_wr_q_pop_head_nocopy( mock->rx_q );
  struct ibv_wc *            wc = fd_ibv_wc_q_push_tail_nocopy    ( mock->rx_cq->wc_q );
  wc->wr_id    = wr->wr_id;
  wc->opcode   = IBV_WC_RECV;
  wc->status   = status;
  wc->byte_len = (uint)sz;
  ulong wr_id = wr->wr_id;
  fd_ibv_sge_p_ele_release( mock->sge_pool, (fd_ibv_mock_sge_t *)wr->sg_list );
  return wr_id;
}
...
tx_complete_one( fd_ibverbs_mock_qp_t * mock,
                 enum ibv_wc_status     status,
                 ulong                  wr_id ) {
  FD_TEST( fd_ibv_wc_q_avail( mock->tx_cq->wc_q ) );
  struct ibv_wc * wc = fd_ibv_wc_q_push_tail_nocopy( mock->tx_cq->wc_q );
  wc->wr_id    = wr_id;
...
main( int     argc,
  ...
  ulong const  cq_depth   = fd_ulong_max( rxq_depth, txq_depth );
  ...
  fd_topo_link_t * rx_link = fd_topob_link( topo, "net_shred", "wksp", link_depth, 0UL, FD_IBETH_RX_BATCH_MAX );
  ...
  tile->rx_cq = fd_ibverbs_mock_qp_get_rx_cq_ex( mock );
  tile->tx_cq = fd_ibverbs_mock_qp_get_tx_cq_ex( mock );
  tile->qp    = fd_ibverbs_mock_qp_get_qp      ( mock );
  tile->mr_lkey = MR_LKEY;
  FD_TEST( tile->rx_cq!=tile->tx_cq );
  FD_TEST( tile->qp->recv_cq!=tile->qp->send_cq );
  ...
  FD_TEST( fd_ibv_wc_q_cnt     ( mock->rx_cq->wc_q )==0UL );
  FD_TEST( fd_ibv_wc_q_cnt     ( mock->tx_cq->wc_q )==0UL );
  FD_TEST( tile->rx_pending_rem==FD_IBETH_RX_BATCH_MAX   );
  FD_TEST( stem_seq[0]==0UL );

  /* A partial RX batch is reposted immediately. */
  for( ulong i=1UL; i<FD_IBETH_RX_BATCH_MAX; i++ ) {
    rx_complete_one( mock, IBV_WC_GENERAL_ERR, 0UL );
  }
  FD_TEST( fd_ibv_recv_wr_q_cnt( mock->rx_q )==rxq_depth-FD_IBETH_RX_BATCH_MAX+1 );
  FD_TEST( fd_ibv_wc_q_cnt( mock->rx_cq->wc_q )==FD_IBETH_RX_BATCH_MAX-1 );
  verify_balances( tile, stem, mock, frame_track );
  int poll_in     = 1;
  int charge_busy = 0;
  ...
  for( ulong i=0UL; i<FD_IBETH_RX_BATCH_MAX+1UL; i++ ) rx_complete_one( mock, IBV_WC_GENERAL_ERR, 0UL );
  after_credit( tile, stem, &poll_in, &charge_busy );
  FD_TEST( fd_ibv_wc_q_cnt( mock->rx_cq->wc_q )==1UL );
  verify_balances( tile, stem, mock, frame_track );
  after_credit( tile, stem, &poll_in, &charge_busy );
  FD_TEST( fd_ibv_wc_q_cnt( mock->rx_cq->wc_q )==0UL );
  verify_balances( tile, stem, mock, frame_track );
  ...
  uint const tx_wr_chunk = fd_ibeth_tx_wr_chunk( tx_wr.wr_id );
  FD_TEST( tx_wr_chunk >= tile->tx_chunk0 && tx_wr_chunk < tile->tx_chunk1 );
  FD_TEST( fd_ibeth_tx_wr_sz( tx_wr.wr_id )==sizeof(tx_pkt_templ) );
  ...
  tx_complete_one( mock, IBV_WC_SUCCESS, tx_wr.wr_id );
  verify_balances( tile, stem, mock, frame_track );
  charge_busy = 0;
  before_credit( tile, stem, &charge_busy );
  ...
  for( ulong i=0UL; i<FD_IBETH_TX_BATCH_MAX+1UL; i++ ) {
    fd_memcpy( tx_packet, &tx_pkt_templ, sizeof(tx_pkt_templ) );
    during_frag( tile, 0UL, tx_seq, tx_sig, tx_chunk, sizeof(tx_pkt_templ), 1UL );
    after_frag( tile, 0UL, tx_seq, tx_sig, sizeof(tx_pkt_templ), 0UL, 0UL, stem );
  }
  FD_TEST( tile->tx_pending_cnt==1U );
  FD_TEST( fd_ibv_send_wr_q_cnt( mock->tx_q )==FD_IBETH_TX_BATCH_MAX );
  FD_TEST( fd_ibeth_tx_flush( tile ) );
  FD_TEST( tile->tx_pending_cnt==0U );
  FD_TEST( fd_ibv_send_wr_q_cnt( mock->tx_q )==FD_IBETH_TX_BATCH_MAX+1UL );
  verify_balances( tile, stem, mock, frame_track );

  for( ulong i=0UL; i<FD_IBETH_TX_BATCH_MAX+1UL; i++ ) {
    tx_wr = fd_ibv_send_wr_q_pop_head( mock->tx_q );
    fd_ibv_sge_p_ele_release( mock->sge_pool, (fd_ibv_mock_sge_t *)tx_wr.sg_list );
    tx_complete_one( mock, IBV_WC_SUCCESS, tx_wr.wr_id );
  }
  charge_busy = 0;
  before_credit( tile, stem, &charge_busy );
  FD_TEST( charge_busy );
  FD_TEST( fd_ibv_wc_q_cnt( mock->tx_cq->wc_q )==1UL );
  FD_TEST( tile->metrics.tx_pkt_cnt==1UL+FD_IBETH_TX_BATCH_MAX );
  before_credit( tile, stem, &charge_busy );
  FD_TEST( fd_ibv_wc_q_cnt( mock->tx_cq->wc_q )==0UL );
  FD_TEST( tile->metrics.tx_pkt_cnt==2UL+FD_IBETH_TX_BATCH_MAX );
  FD_TEST( tile->metrics.tx_bytes_total==(2UL+FD_IBETH_TX_BATCH_MAX)*sizeof(tx_pkt_templ) );
  verify_balances( tile, stem, mock, frame_track );
