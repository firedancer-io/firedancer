/* test_ibeth_tile.c runs parts of the ibeth tile against mock direct
   mlx5 queues. */
...
static void
test_mlx5_owner_fini( void ) {
  fd_ibeth_tile_t tile[1];

  fd_memset( tile, 0, sizeof(tile) );
  FD_TEST( !fd_ibeth_mlx5_fini( tile ) );

  fd_memset( tile, 0, sizeof(tile) );
  tile->hw.context.cmd_fd = -1;
  tile->hw.context.async_fd = -1;
  tile->hw.initialized = FD_IBETH_MLX5_CONTEXT;
  FD_TEST( !fd_ibeth_mlx5_fini( tile ) );
  FD_TEST( !tile->hw.initialized );

#define TEST_FAILS_AT(resource,mask) do {                               \
    fd_memset( tile, 0, sizeof(tile) );                                 \
    tile->hw.context.cmd_fd = -1;                                       \
    tile->hw.resource.ctx = &tile->hw.context;                          \
    tile->hw.initialized = (mask);                                      \
    FD_TEST( fd_ibeth_mlx5_fini( tile )==-1 );                          \
    FD_TEST( tile->hw.initialized==(mask) );                            \
  } while(0)

  TEST_FAILS_AT( uar,   FD_IBETH_MLX5_CONTEXT | FD_IBETH_MLX5_UAR );
  TEST_FAILS_AT( rx_cq, FD_IBETH_MLX5_CONTEXT | FD_IBETH_MLX5_UAR | FD_IBETH_MLX5_RX_CQ );
  TEST_FAILS_AT( tx_cq, FD_IBETH_MLX5_CONTEXT | FD_IBETH_MLX5_UAR | FD_IBETH_MLX5_RX_CQ |
                       FD_IBETH_MLX5_TX_CQ );
  TEST_FAILS_AT( pd,    FD_IBETH_MLX5_CONTEXT | FD_IBETH_MLX5_UAR | FD_IBETH_MLX5_RX_CQ |
                       FD_IBETH_MLX5_TX_CQ | FD_IBETH_MLX5_PD );

  fd_memset( tile, 0, sizeof(tile) );
  tile->hw.context.cmd_fd = -1;
  tile->hw.pd.ctx = &tile->hw.context;
  tile->hw.mr.pd = &tile->hw.pd;
  tile->hw.initialized = FD_IBETH_MLX5_CONTEXT | FD_IBETH_MLX5_UAR | FD_IBETH_MLX5_RX_CQ |
                         FD_IBETH_MLX5_TX_CQ | FD_IBETH_MLX5_PD | FD_IBETH_MLX5_MR;
  FD_TEST( fd_ibeth_mlx5_fini( tile )==-1 );
  FD_TEST( tile->hw.initialized==(FD_IBETH_MLX5_CONTEXT | FD_IBETH_MLX5_UAR | FD_IBETH_MLX5_RX_CQ |
                                  FD_IBETH_MLX5_TX_CQ | FD_IBETH_MLX5_PD | FD_IBETH_MLX5_MR) );

  TEST_FAILS_AT( qp, FD_IBETH_MLX5_CONTEXT | FD_IBETH_MLX5_UAR | FD_IBETH_MLX5_RX_CQ |
                     FD_IBETH_MLX5_TX_CQ | FD_IBETH_MLX5_PD | FD_IBETH_MLX5_MR | FD_IBETH_MLX5_QP );

  fd_memset( tile, 0, sizeof(tile) );
  tile->hw.context.cmd_fd = -1;
  tile->hw.qp.ctx = &tile->hw.context;
  tile->hw.flows[0].qp = &tile->hw.qp;
  tile->hw.initialized = FD_IBETH_MLX5_CONTEXT | FD_IBETH_MLX5_UAR | FD_IBETH_MLX5_RX_CQ |
                         FD_IBETH_MLX5_TX_CQ | FD_IBETH_MLX5_PD | FD_IBETH_MLX5_MR | FD_IBETH_MLX5_QP;
  tile->hw.flow_cnt = 1U;
  FD_TEST( fd_ibeth_mlx5_fini( tile )==-1 );
  FD_TEST( tile->hw.flow_cnt==1U );
  FD_TEST( tile->hw.initialized==(FD_IBETH_MLX5_CONTEXT | FD_IBETH_MLX5_UAR | FD_IBETH_MLX5_RX_CQ |
                                  FD_IBETH_MLX5_TX_CQ | FD_IBETH_MLX5_PD | FD_IBETH_MLX5_MR | FD_IBETH_MLX5_QP) );
...
test_rx_routes( void ) {
  ...
  FD_TEST( fd_topob_new( topo, "test_ibeth_rx_routes" ) );
  fd_topob_wksp( topo, "net_umem" );

  fd_topo_tile_t * topo_tile = fd_topob_tile( topo, "ibeth", "net_umem", "net_umem", 0UL, 0, 0, 0 );
  fd_topo_obj_t * umem_obj = fd_topob_obj( topo, "dcache", "net_umem" );
  fd_pod_insert_cstr( topo->props, "net.provider", "ibverbs" );
  fd_pod_insertf_ulong( topo->props, umem_obj->id, "net.%lu.umem", 0UL );
  topo_tile->ibeth.batch_size = 8U;
  topo_tile->ibeth.flow_rule_max = 8U;
  topo_tile->ibeth.net.shred_listen_port        = 8000U;
  topo_tile->ibeth.net.gossip_listen_port       = 8001U;
  topo_tile->ibeth.net.repair_client_listen_port = 8002U;
  topo_tile->ibeth.net.repair_serve_listen_port = 8003U;
  topo_tile->ibeth.net.txsend_src_port          = 8004U;
...
typedef struct fd_ibeth_test_mlx5 fd_ibeth_test_mlx5_t;

static inline uint
test_rx_wq_cnt( fd_ibeth_test_mlx5_t const * mock,
                fd_ibeth_tile_t const *      tile ) {
  return tile->hw.qp.rq_prod-mock->rq_nic_cons;
}

static inline uint
test_tx_wq_cnt( fd_ibeth_test_mlx5_t const * mock,
                fd_ibeth_tile_t const *      tile ) {
  return tile->hw.qp.sq_prod-mock->sq_nic_cons;
}

static inline uint
test_rx_cq_cnt( fd_ibeth_test_mlx5_t const * mock,
                fd_ibeth_tile_t const *      tile ) {
  return mock->rx_cq_prod-tile->hw.rx_cq.cons_idx;
}

static inline uint
test_tx_cq_cnt( fd_ibeth_test_mlx5_t const * mock,
                fd_ibeth_tile_t const *      tile ) {
...
verify_rx_balance( fd_ibeth_tile_t const *      tile,
                   ...
                   fd_ibeth_test_mlx5_t const * mock,
...
#define CHECK( chunk ) do {                                            \
  ...
  for( uint idx=tile->hw.rx_cq.cons_idx; idx<mock->rx_cq_prod; idx++ ) {
    fd_mlx5_cqe_t const * cqe = tile->hw.rx_cq.entries+(idx & (tile->hw.rx_cq.depth-1U));
    uint wqe_counter = fd_mlx5_cqe_wqe_counter( cqe );
...
#define CHECK( chunk ) do {                                            \
  ...
  for( uint idx=tile->hw.qp.sq_cons; idx<tile->hw.qp.sq_prod; idx++ ) {
    CHECK( fd_ibeth_tx_meta( tile->hw.qp.tx_user_data[ idx & (tile->hw.qp.tx_depth-1U) ] ).chunk );
  }
...
static ulong
rx_complete_one( fd_ibeth_test_mlx5_t * mock,
                 fd_ibeth_tile_t *      tile,
                 uint                   opcode,
                 ulong                  sz ) {
...
static ulong
tx_complete_batch( fd_ibeth_test_mlx5_t * mock,
                   fd_ibeth_tile_t *      tile,
                   uint                   opcode,
                   uint                   wqe_cnt ) {
  FD_TEST( wqe_cnt );
  ...
  fd_ibeth_send_wr_t meta = fd_ibeth_tx_meta( tile->hw.qp.tx_user_data[ wqe_counter & (tile->hw.qp.tx_depth-1U) ] );
  test_cqe_push( &tile->hw.tx_cq, mock->tx_cq_prod++, opcode, wqe_counter, 0U );
  return meta.chunk;
}

static ulong
tx_complete_one( fd_ibeth_test_mlx5_t * mock,
                 fd_ibeth_tile_t *      tile,
                 uint                   opcode ) {
...
main( int     argc,
  ...
  fd_ibeth_test_mlx5_t mock[1] = {{0}};

  /* Mock a topology */
  static fd_topo_t topo[1];
  FD_TEST( fd_topob_new( topo, "test_ibeth_tile" ) );
  fd_topo_wksp_t * topo_wksp = fd_topob_wksp( topo, "wksp" );
  topo_wksp->wksp = wksp;
  fd_topo_tile_t * topo_tile = fd_topob_tile( topo, "ibeth", "wksp", "wksp", cpu_idx, 0, 0, 0 );
  topo_tile->ibeth.rx_queue_size = (uint)rxq_depth;
  topo_tile->ibeth.tx_queue_size = (uint)txq_depth;
  topo_tile->ibeth.batch_size    = batch_size;
  topo_tile->ibeth.flow_rule_max = flow_rule_max;
  topo_tile->ibeth.route_max      = route_max;
  topo_tile->ibeth.route_peer_max = route_peer_max;
  topo_tile->ibeth.route_peer_seed = route_seed;
  ...
  tile->flow_rule_max = flow_rule_max;
  ...
  FD_TEST( FD_SCRATCH_ALLOC_APPEND( scratch, alignof(fd_ibeth_tile_t), sizeof(fd_ibeth_tile_t) )==tile );
  ulong queue_footprint = fd_mlx5_queue_footprint( (uint)rxq_depth, (uint)txq_depth );
  tile->queue_mem = FD_SCRATCH_ALLOC_APPEND( scratch, FD_MLX5_PAGE_SZ, queue_footprint );
  tile->rx_user_data = FD_SCRATCH_ALLOC_APPEND( scratch, alignof(ulong), rxq_depth*sizeof(ulong) );
  tile->tx_user_data = FD_SCRATCH_ALLOC_APPEND( scratch, alignof(ulong), txq_depth*sizeof(ulong) );
  tile->hw.context.bf_reg_size      = sizeof(mock->bf);
  tile->hw.context.eth_min_inline_sz = 0U;
  FD_TEST( fd_mlx5_queue_layout_init( &tile->queue_layout, &(fd_mlx5_context_t) {
                                      .max_recv_wr=(uint)rxq_depth, .max_send_wqebb=(uint)txq_depth,
                                      .max_cqe=(uint)fd_ulong_max( rxq_depth, txq_depth ) },
                                      (uint)rxq_depth, (uint)txq_depth ) );
  FD_TEST( fd_mlx5_queue_mem_init( tile->queue_mem, &tile->queue_layout ) );
  tile->hw.uar.ctx = &tile->hw.context;
  tile->hw.uar.reg = mock->bf;
  tile->hw.rx_cq = (fd_mlx5_cq_t) {
    .ctx=&tile->hw.context, .uar=&tile->hw.uar,
    .entries=(fd_mlx5_cqe_t *)((uchar *)tile->queue_mem+tile->queue_layout.rx_cq_off),
    .dbrec=(uint *)((uchar *)tile->queue_mem+tile->queue_layout.rx_cq_db_off), .depth=(uint)rxq_depth };
  tile->hw.tx_cq = (fd_mlx5_cq_t) {
    .ctx=&tile->hw.context, .uar=&tile->hw.uar,
    .entries=(fd_mlx5_cqe_t *)((uchar *)tile->queue_mem+tile->queue_layout.tx_cq_off),
    .dbrec=(uint *)((uchar *)tile->queue_mem+tile->queue_layout.tx_cq_db_off), .depth=(uint)txq_depth };
  tile->hw.qp = (fd_mlx5_qp_t) {
    .ctx=&tile->hw.context, .uar=&tile->hw.uar, .rx_cq=&tile->hw.rx_cq, .tx_cq=&tile->hw.tx_cq,
    .rq=(fd_mlx5_rx_wqe_t *)((uchar *)tile->queue_mem+tile->queue_layout.rq_off),
    .sq=(fd_mlx5_tx_wqe_t *)((uchar *)tile->queue_mem+tile->queue_layout.sq_off),
    .rx_user_data=tile->rx_user_data, .tx_user_data=tile->tx_user_data,
    .dbrec=(uint *)((uchar *)tile->queue_mem+tile->queue_layout.qp_db_off),
    .rx_depth=(uint)rxq_depth, .tx_depth=(uint)txq_depth, .qpn=1U, .state=FD_MLX5_QPS_RTS };
  ...
  FD_TEST( fd_neigh4_hmap_new( neigh4_hmap_mem, neigh4_ele_max, neigh4_seed ) );
  ...
  topo_tile->ibeth.netdev_tbl_obj_id = topo_netdev_tbl->id;
  ...
  fd_ibeth_send_wr_t tx_meta = fd_ibeth_tx_meta( tile->hw.qp.tx_user_data[ tx_wqe_idx ] );
  fd_mlx5_tx_wqe_t const * tx_wqe = tile->hw.qp.sq+tx_wqe_idx;
  uint const tx_wr_chunk = tx_meta.chunk;
  FD_TEST( tx_wr_chunk >= tile->tx_chunk0 && tx_wr_chunk < tile->tx_chunk1 );
  ...
  FD_TEST( fd_uint_bswap( FD_LOAD( uint, tx_wqe->bytes+36 ) )==tile->mr_lkey );
  ...
  FD_TEST( fd_ibeth_tx_flush( tile ) );
  FD_TEST( tile->tx_pending_cnt==0U );
