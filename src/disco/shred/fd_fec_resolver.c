...
fd_fec_resolver_add_shred( fd_fec_resolver_t         * resolver,
  ...
  if( !is_data_shred ) { /* Roughly 50/50 branch */
    ...
    /* if it has data complete, it must be the last data shred in the FEC set
       https://github.com/anza-xyz/agave/blob/v4.3.0-beta.0/ledger/src/shred/filter.rs#L346-L354 */
  ...
  if( FD_UNLIKELY( !ctx ) ) { /* This is the first shred in the FEC set */
    ...
    if( FD_UNLIKELY( resolver->free_list_cnt<=partial_depth ) ) {
      ...
      FD_MCNT_INC( SHRED, FEC_SPILLED, 1UL );
    }
    ...
    ctx->set->turbine_shred_rcvd = 0UL;
    ctx->set->repair_shred_rcvd  = 0UL;
