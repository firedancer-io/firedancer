...
ci_dest_add_one_unstaked( fd_stake_ci_t *            info,
                          ...
                          fd_per_epoch_info_t *      ei ) {
  if( fd_shred_dest_cnt_all( ei->sdest )+ 1UL>=MAX_SHRED_DESTS ) {
    FD_LOG_ERR(( "too many validators in shred table to add a new validator" ));
  }
...
ci_dest_update_impl( fd_stake_ci_t *       info,
                     ...
                     ushort                port,
                     fd_per_epoch_info_t * ei ) {
  fd_shred_dest_idx_t idx = fd_shred_dest_pubkey_to_idx( ei->sdest, (fd_pubkey_t *)pubkey );
...
ci_dest_remove_impl( fd_stake_ci_t *       info,
                     ...
                     fd_per_epoch_info_t * ei ) {
  fd_shred_dest_idx_t idx = fd_shred_dest_pubkey_to_idx( ei->sdest, (fd_pubkey_t *)pubkey );
  if( idx==FD_SHRED_DEST_NO_DEST ) return;
  ...
  /* construct shred_dest_temp, with all entries except idx */
  ulong cur_cnt = fd_shred_dest_cnt_all( ei->sdest );
