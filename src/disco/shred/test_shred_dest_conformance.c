...
fd_stake_ci_t _stake_ci[ 1 ];
fd_stake_ci_t _stake_ci_broadcast[ 1 ];
...
/* Expected leader schedule from C code (vote-keyed, chacha8) */
static const struct {
  const char * pubkey_base58;
  ulong slot;
} EXPECTED_LEADERS[9] = {
  { "jwV7SyvqCSrVcKibYvurCCWr7DUmT7yRYPmY9QwvrGo",   0 },
  { "GcdayuLaLyrdmUu324nahyv33G5poQdLUEZ1nEytDeP",    4 },
  { "UKrXU5bFrTzrqqpZXs8GVDbp4xPweiM65ADXNAy3ddR",   12 },
  { "YEGAxog9gxiGXxo538aAQxq55XAebpFfwU72ZUxmSHm",   20 },
  { "skJQSS6csSHJzZfcZToe3gyN8M2BMKnbH1YYY2wNTbV",   24 },
  { "4uQeVj5tqViQh7yWWGStvkEG1Zmhx6uasJtWCJziofM",   36 },
  { "oqtkwi1j2wZuJSh74CMk7wk77nFUQDt1Qhf3Liweew9",   44 },
  { "HyepegvEV1ZwAMnuunwLoBT81GzgD8Uud6pJwK537cpT",   52 },
  { "GcdayuLaLyrdmUu324nahyv33G5poQdLUEZ1nEytDeP",    56 },
};
...
/* Expected broadcast node (slot 52, index 31, Data shred)
   Slot 52 is where first node (HyepegvEV1ZwAMnuunwLoBT81GzgD8Uud6pJwK537cpT) is the leader.
   Root/broadcast node (first recipient from leader) is c8fpTXm3XTRgE5maYQ24Li4L65wMYvAFomzXknxVEx7 */
#define EXPECTED_BROADCAST_SLOT    (52UL)
...
static const expected_first_layer_t EXPECTED_FIRST_LAYER[10] = {
  ...
  { "YEGAxog9gxiGXxo538aAQxq55XAebpFfwU72ZUxmSHm", 0 },
  { "4uQeVj5tqViQh7yWWGStvkEG1Zmhx6uasJtWCJziofM", 0 },
};

/* Expected second layer of turbine tree.
   Each first-layer node may have their own children. */
static const expected_second_layer_t EXPECTED_SECOND_LAYER[8] = {
  { 0, "GcdayuLaLyrdmUu324nahyv33G5poQdLUEZ1nEytDeP" },
  { 1, "LX3EUdRUBUa3TbsYXLEUdj9J3prXkWXvLYSWyYyc2Jj" },
  { 2, "wei3wABWhvzigge84jFXySCd8untJRhB9KS3jLw6GFq" },
  { 3, "21Z7hRtGQYRi8NocdZzhRuBRt9UZbFXbm1dKYvevp4vB" },
  { 4, "29MvzRLSCDR8wm3ZeaXbDkftQAc719jQvkF6ZKGvFgEs" },
  { 5, "25TXLvcMJNvRY4vb95G9Kpvf9A3LJCdWLswD47xvXsaX" },
  { 6, "2HAkHQnbytQZm9HWfb4V1cALvBjeR3wE6UrsZhtuhHZZ" },
  { 7, "2DGLdv4X63urMTAYA5o37gR7fBAsi6qKWcYz4WauyUuD" },
};

static void
test_shred_dest_conformance( void ) {
  FD_LOG_NOTICE(( "=== Testing Shred Dest Conformance ===" ));
  ...
  fd_pubkey_t const * test_slot_leader = fd_epoch_leaders_get( lsched, EXPECTED_BROADCAST_SLOT );
  FD_TEST( test_slot_leader );

  /* Get shred dest for the test slot */
  fd_shred_dest_t * sdest = fd_stake_ci_get_sdest_for_slot( stake_ci, EXPECTED_BROADCAST_SLOT );
  FD_TEST( sdest );
  ...
  uchar * decode_result = fd_base58_decode_32( EXPECTED_BROADCAST_PUBKEY, expected_broadcast_pubkey );
  FD_TEST( decode_result != NULL );
  ...
  if( !memcmp( broadcast_peer->pubkey.uc, expected_broadcast_pubkey, 32UL ) ) {
    ...
    FD_LOG_NOTICE(( "  PASS: slot=%lu, shred_idx=%u -> broadcast=%s",
      EXPECTED_BROADCAST_SLOT, EXPECTED_BROADCAST_IDX, result_pubkey_base58 ));
  } else {
    broadcast_failed++;
    FD_LOG_WARNING(( "  FAIL: slot=%lu, shred_idx=%u, expected broadcast=%s, got broadcast=%s",
      EXPECTED_BROADCAST_SLOT, EXPECTED_BROADCAST_IDX,
      EXPECTED_BROADCAST_PUBKEY, result_pubkey_base58 ));
  }
  ...
  for( ulong i=0UL; i<20UL; i++ ) {
    ...
    dest_info_broadcast[i].ip4 = (uint)(i+1);
    dest_info_broadcast[i].port = (ushort)(8000 + i);
  }
  fd_stake_ci_dest_add_fini( stake_ci_broadcast, 20UL );

  fd_shred_dest_t * sdest_broadcast = fd_stake_ci_get_sdest_for_slot( stake_ci_broadcast, EXPECTED_BROADCAST_SLOT );
  ...
  fd_shred_dest_idx_t * children_ptr = fd_shred_dest_compute_children(
    sdest_broadcast, shred_ptr, 1UL, children_result, 1UL, fanout, fanout, &max_dest_cnt );
  ...
  for( ulong parent_idx=0UL; parent_idx<10UL; parent_idx++ ) {
    ...
    for( ulong i=0UL; i<20UL; i++ ) {
      ...
      parent_dest_info[i].ip4 = (uint)(i+1);
      parent_dest_info[i].port = (ushort)(8000 + i);
    }
    fd_stake_ci_dest_add_fini( parent_stake_ci, 20UL );

    fd_shred_dest_t * parent_sdest = fd_stake_ci_get_sdest_for_slot( parent_stake_ci, EXPECTED_BROADCAST_SLOT );
    ...
    fd_shred_dest_idx_t * parent_children_ptr = fd_shred_dest_compute_children(
      parent_sdest, shred_ptr, 1UL, parent_children_result, 1UL, fanout, fanout, &parent_max_dest_cnt );
    ...
    for( ulong child_idx=0UL; child_idx<expected_num_children; child_idx++ ) {
      ...
      uchar expected_child_pubkey[32];
      uchar * decode_result2 = fd_base58_decode_32( EXPECTED_SECOND_LAYER[second_layer_child_idx].child_pubkey_base58, expected_child_pubkey );
      FD_TEST( decode_result2 != NULL );
  ...
  if( leader_failed > 0UL || broadcast_failed > 0UL || children_failed > 0UL || second_layer_failed > 0UL ) {
    FD_LOG_WARNING(( "Some tests do not match expected values!" ));
  } else {
    FD_LOG_NOTICE(( "SUCCESS: All tests passed!" ));
  }
    ...
    mod conformance_tests {
        ...
        pub fn make_deterministic_test_cluster(
            ...
            let stakes: HashMap<Pubkey, u64> = nodes
                ...
                .enumerate()
                .map(|(i, node)| {
                    if i < (num_nodes * 7 / 10) {  // 70% staked
                        (*node.pubkey(), 100*(i / 2 + 1) as u64)
                    } else {
                        (*node.pubkey(), 0)
                    }
                })
        ...
        #[test]
        fn test_deterministic_cluster_conformance() {
            let num_nodes = 20;
            let use_chacha8 = true;
            ...
            // Create a test shred
            let shred = Shredder::new(0, 1, 0, 0) // placeholder slot
                    .unwrap()
            ...
            covered.insert(slot_leader);
            covered.insert(root_pubkey);
            for child_pk in &root_children_vec {
                covered.insert(*child_pk);
            }
