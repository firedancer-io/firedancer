...
after_frag( fd_shred_ctx_t *    ctx,
  ...
  if( FD_LIKELY( ctx->in_kind[ in_idx ]==IN_KIND_NET ) ) {
...
#if FD_HAS_NO_AGAVE
    /* There is currently no path to query the feature activation from
        here (TODO) */
    fanout = 200UL;
#else
    ...
    if( FD_LIKELY(shred->slot >= ctx->features_activation->slots[FD_SHRED_FEATURES_ACTIVATION_IDX_DISABLE_TURBINE_FANOUT_EXPERIMENTS]) ) {
      fanout = 200UL;
    } else {
      if( FD_LIKELY(shred->slot >= ctx->features_activation->slots[FD_SHRED_FEATURES_ACTIVATION_IDX_ENABLE_TURBINE_EXTENDED_FANOUT_EXPERIMENTS]) ) {
