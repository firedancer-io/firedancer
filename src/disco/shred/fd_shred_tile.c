...
after_frag( fd_shred_ctx_t *    ctx,
  ...
  if( FD_LIKELY( ctx->in_kind[ in_idx ]==IN_KIND_NET ) ) {
    ...
    uint nonce = UINT_MAX;
    if( FD_UNLIKELY( (fd_disco_netmux_sig_proto( sig )==DST_PROTO_REPAIR) & (shred_buffer_sz>=fd_shred_sz( shred )+sizeof(uint)) ) ) {
    ...
    if( FD_UNLIKELY( ctx->shred_out_idx!=ULONG_MAX &&  /* Only send to repair in full Firedancer */
      ...
      ulong sig_ = fd_disco_shred_out_shred_sig( 0, spilled_fec.slot, spilled_fec.fec_set_idx, 0, 0 /* FIXME: TODO */ );
  ...
  for( ulong fset_k=0; fset_k<ctx->send_fec_set_cnt; fset_k++ ) {
    ...
    if( FD_LIKELY( ctx->shred_out_idx!=ULONG_MAX ) ) { /* firedancer-only */
      ...
      ulong   sig   = fd_disco_shred_out_fec_sig( last->slot, last->fec_set_idx, (uint)FD_FEC_SHRED_CNT, last->data.flags & FD_SHRED_DATA_FLAG_SLOT_COMPLETE, last->data.flags & FD_SHRED_DATA_FLAG_DATA_COMPLETE );
      ...
      ulong tspub = fd_frag_meta_ts_comp( fd_tickcount() );
      ulong sz = fd_ulong_min( sizeof(fd_fec_set_t), USHORT_MAX ); /* The size is actually larger than USHORT_MAX */
...
unprivileged_init( fd_topo_t *      topo,
  ...
  if( FD_LIKELY( ctx->shred_out_idx!=ULONG_MAX ) ) { /* firedancer-only */
    ...
    ulong const * nonce_initialized = (ulong const *)(sizeof(fd_rnonce_ss_t)+(uchar const *)shared_rnonce);
