...
after_frag( fd_shred_ctx_t *    ctx,
  ...
  if( FD_UNLIKELY( ctx->in_kind[ in_idx ]==IN_KIND_REPAIR ) ) {
    ...
    int rv = fd_fec_resolver_shred_query( ctx->resolver, shred_sig, last_idx, out_last_shred );
    if( FD_UNLIKELY( rv != FD_FEC_RESOLVER_SHRED_OKAY ) ) {
      /* We will hit this case if FEC is no longer in done map or in the curr_map*/
      FD_MCNT_INC( SHRED, FORCE_COMPLETE_FAILURE, 1UL );
      return;
  ...
  if( FD_UNLIKELY( data_shred_cnt == SHRED_CNT_NOT_SET || parity_shred_cnt == SHRED_CNT_NOT_SET ) ) {
    /* At this point we are in a FD_FEC_RESOLVER_SHRED_COMPLETES, but
       shred_cnts have not been set, implying we have yet to recieve any
       coding shreds. Thus, we must have arrived here via a force
       completes situation. */
    FD_TEST( ctx->in_kind[ in_idx ]==IN_KIND_REPAIR );
