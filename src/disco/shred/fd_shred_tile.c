...
typedef struct {
  ...
  uchar store_buffer[ FD_REEDSOL_DATA_SHREDS_MAX * 955UL ]; /* at 67 shreds, at most 955 bytes per shred (see fd_shredder.h) */
  uchar store_buffer2[ FD_REEDSOL_DATA_SHREDS_MAX * 955UL ]; /* at 67 shreds, at most 955 bytes per shred (see fd_shredder.h) */
...
after_frag( fd_shred_ctx_t *    ctx,
  ...
  if( FD_LIKELY( ctx->in_kind[ in_idx ]==IN_KIND_NET ) ) {
    ...
    int rv = fd_fec_resolver_add_shred( ctx->resolver, shred, shred_buffer_sz, slot_leader->uc, out_fec_set, out_shred, NULL );
  ...
  for( ulong fset_k=0; fset_k<ctx->send_fec_set_cnt; fset_k++ ) {
    ...
    fd_hash_t merkle_root; fd_hash_t chained_merkle_root;
    ...
    fd_shred_merkle_root( last, bmtree_mem, &bmtree_root );
    memcpy( merkle_root.uc, bmtree_root.hash, FD_SHRED_MERKLE_ROOT_SZ );
    memcpy( chained_merkle_root.uc, (uchar *)last + fd_shred_chain_off( last->variant ), FD_SHRED_MERKLE_ROOT_SZ );
    ...
    if( FD_LIKELY( ctx->store ) ) { /* firedancer-only */
      ...
      for( ulong i=0UL; i<set->data_shred_cnt; i++ ) {
        ...
        memcpy( ctx->store_buffer + off, fd_shred_data_payload( data_shred ), payload_sz );
        off += payload_sz;
      }

      /* See top-level documentation in fd_store.h under CONCURRENCY to
         understand why it is safe to take out a Store read lock instead
         of a write lock in Shred tile. */

      fd_rwlock_write( &ctx->store->lock ); /* TODO demote to read lock after hash function change */
      fd_store_insert( ctx->store, &merkle_root, ctx->store_buffer, off );
    ...
    if( FD_LIKELY( ctx->repair_out_idx!=ULONG_MAX ) ) { /* firedancer-only */
      ...
      memcpy( chunk+FD_SHRED_DATA_HEADER_SZ, merkle_root.hash, FD_SHRED_MERKLE_ROOT_SZ );
...
#define NONNULL( x ) (__extension__({                                        \
  ...
  FD_TEST( fd_store_pool_free( fd_store_pool( ctx->store ) ) );
