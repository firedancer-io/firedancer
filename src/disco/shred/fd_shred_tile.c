  ...
  if( FD_UNLIKELY( ctx->in_kind[ in_idx ]==IN_KIND_REPAIR ) ) {
    ...
    if( FD_UNLIKELY( fd_fec_resolver_done_contains( ctx->resolver, shred_sig ) ) ) {
      /* This is a FEC completion message from the repair tile.  We need
         ...
         been completed. */
      FD_LOG_INFO(( "FEC completion message from repair tile, but shred tile %lu has just completed it.", ctx->round_robin_id ));
      return;
    ...
    if( rv != FD_FEC_RESOLVER_SHRED_COMPLETES )
      FD_LOG_ERR(( "Shred tile %lu cannot force complete the slot %lu fec_set_idx %u", ctx->round_robin_id, out_last_shred->slot, out_last_shred->fec_set_idx ));
