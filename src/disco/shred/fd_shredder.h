...
/* fd_shredder_next_fec_set extracts the next FEC set from the in
   ...
   signature field of each shred is scratch (its tail holds the Merkle
   leaf prefix) until the caller overwrites it.  Stores the
   generated FEC set in result, which is clobbered.  Populates all
