...
/* Sign requests, for leader FEC sets and for resigned FEC sets from
   the network, wait for the sign tile in a queue of up to
...
fd_shred_tile_fec_set_cnt( ulong fec_exposure,
  ...
  return 2UL*fec_exposure + fec_resolver_depth + 2UL*FD_SHRED_SIGN_PEND_MAX + 2UL;
}
