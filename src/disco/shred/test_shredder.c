...
test_multiple_shredders( void ) {
  ...
  ulong data_sz = 1024UL*1024UL;
  uchar data    [ 1024UL*1024UL ];
  for( ulong i=0; i<data_sz; i++ ) data[ i ] = fd_rng_uchar( r );

  /* Randomly choose a shredder to process a batch until the data buffer is exhausted. */
  while( idx<data_sz ) {
    ulong sz        = ( fd_rng_ulong_roll( r, 4UL )+1UL )*25000UL;
    ulong batch_sz  = fd_ulong_if( idx+sz>data_sz, data_sz-idx, sz );
    ulong shredder  = fd_rng_ulong_roll( r, SHREDDERS );
    for( ulong i=0; i<SHREDDERS; i++ ) {
      if( FD_UNLIKELY( i==shredder ) ) {
        FD_TEST( fd_shredder_init_batch( shredders[ i ], data+idx, batch_sz, slot, meta ) );
  ...
  FD_TEST( fd_shredder_init_batch( shredders[ 0 ], data, idx, slot, meta ) );
  ...
  for( ulong i=1; i<SHREDDERS; i++ ) {
    ...
    FD_TEST( fd_shredder_init_batch( shredders[ i ], data, idx, slot, meta ) );
