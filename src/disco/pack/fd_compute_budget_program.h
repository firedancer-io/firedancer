...
/* In general, compute budget instructions can occur at most once in a
   ...
   Have I seen a ... */
#define FD_COMPUTE_BUDGET_PROGRAM_FLAG_SET_CU             (0x01) /* ... SetComputeUnitLimit ... */
#define FD_COMPUTE_BUDGET_PROGRAM_FLAG_SET_FEE            (0x02) /* ... SetComputeUnitPrice ... */
#define FD_COMPUTE_BUDGET_PROGRAM_FLAG_SET_HEAP           (0x04) /* ... RequestHeapFrame ... */
#define FD_COMPUTE_BUDGET_PROGRAM_FLAG_SET_TOTAL_FEE      (0x08) /* ... RequestUnitsDeprecated ... */
                                                                        /* ... so far? */

/* Any requests for larger heap frames must be a multiple of 1k or the
   transaction is malformed. */
#define FD_COMPUTE_BUDGET_PROGRAM_HEAP_FRAME_GRANULARITY  (1024UL)
...
fd_compute_budget_program_parse( uchar const * instr_data,
  ...
  switch( *instr_data ) {
    ...
    case 1:
      ...
      if( (state->heap_size%FD_COMPUTE_BUDGET_PROGRAM_HEAP_FRAME_GRANULARITY) ) return 0;
...
/* fd_compute_budget_program_finalize: digests the state that resulted from
   ...
   a previously initalized fd_compute_budget_program_state_t.  out_rewards and
   out_compute must be non-null.  The total priority rewards for the
   transaction (i.e. not counting the per-signature fee) is stored in
   out_rewards.  The maximum number of compute units this transaction can
   consume is stored in out_compute.  If the transaction execution has not
   completed by this limit, it is terminated and considered failed. */
static inline void 
fd_compute_budget_program_finalize( fd_compute_budget_program_state_t * state,
                                    ulong                               instr_cnt, /* The number of instructions in the txn */
                                    ulong *                             out_rewards,
                                    uint *                              out_compute ) {
  ulong cu_limit = 0UL;
...
#undef FD_COMPUTE_BUDGET_PROGRAM_HEAP_FRAME_GRANULARITY
#undef FD_COMPUTE_BUDGET_MICRO_LAMPORTS_PER_LAMPORT 
