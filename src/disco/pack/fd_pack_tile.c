  ...
  if( FD_LIKELY( ctx->crank->enabled ) ) {
    ...
    if( FD_UNLIKELY( top_meta ) ) {
      ...
      if( FD_LIKELY( txn_sz==0UL ) ) { /* Everything in good shape! */
        ...
        ulong deleted;
        int retval = fd_pack_insert_bundle_fini( ctx->pack, bundle, 1UL, ctx->_became_leader->block_height-1UL, 1, NULL, &deleted );
        FD_MCNT_INC( PACK, TXN_DELETED, deleted );
  ...
  case IN_KIND_RESOLV: {
    ...
    if( FD_UNLIKELY( (ctx->leader_slot==ULONG_MAX) & (sig>ctx->root_block_height) ) ) {
      /* Using the resolv tile's knowledge of the current last root
         ...
         clogged with expired but high-fee-paying transactions.  That
         can only happen if we are getting transactions. */
