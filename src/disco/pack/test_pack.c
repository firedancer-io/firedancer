...
schedule_validate_microblock( fd_pack_t * pack,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    for( ulong i=0UL; i<txn->instr_cnt; i++ ) {
      ...
      builtin_instr_cost    +=  (ulong)is_builtin * 3000UL;
      non_builtin_cnt += !is_builtin;
    }
    fd_compute_budget_program_finalize( &cbp, non_builtin_cnt, builtin_instr_cost, &rewards, &compute, &requested_loaded_accounts_data_cost );
