...
test_bundle_nonce( void ) {
  ...
  /* Replace nonce transaction with bundle */
  bundle = fd_pack_insert_bundle_init( pack, _bundle, 3UL );
  ...
  fd_ed25519_sig_t sig; memcpy( &sig, fd_txn_get_signatures( TXN( bundle[1]->txnp ), bundle[1]->txnp->payload ), sizeof(fd_ed25519_sig_t) );
  result = fd_pack_insert_bundle_fini( pack, bundle, 3UL, 1000UL, 0, NULL );
  ...
  make_nonce_transaction1( bundle[2]->txnp, 2UL, 2.0, 4, 0, 'a' );
  result = fd_pack_insert_bundle_fini( pack, bundle, 3UL, 1000UL, 0, NULL );
  FD_TEST( result==FD_PACK_INSERT_ACCEPT_NONCE_NONVOTE_REPLACE );
