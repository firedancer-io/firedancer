...
test_expiration( void ) {
  ...
  schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 0.0f, 4UL, 0UL, 0UL, &outcome );
  ...
  schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 0.0f, 1UL, 0UL, 0UL, &outcome );
  schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 0.0f, 0UL, 0UL, 1UL, &outcome );
...
#define INNER_ROUNDS (FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND/(1020UL * 1024UL))
...
void performance_end_block( void ) {
  ...
  fd_pack_limits_t limits[ 1 ] = { {
    .max_cost_per_block          = 13UL*FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND,
      .max_vote_cost_per_block   = 0UL,
      .max_write_cost_per_acct   = FD_PACK_MAX_WRITE_COST_PER_ACCT_UPPER_BOUND,
      .max_data_bytes_per_block  = ULONG_MAX/2UL,
...
test_limits( void ) {
  ...
  for( ulong max=1UL; max<=15UL; max++ ) {
    ...
    schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 1.0f, max, 0UL, 0UL, &outcome );
  ...
  if( 1 ) {
    ...
    for( ulong j=0UL; j<FD_PACK_MAX_VOTE_COST_PER_BLOCK_UPPER_BOUND/(1024UL*FD_PACK_SIMPLE_VOTE_COST); j++ ) {
      for( ulong i=0UL; i<1024UL; i++ ) { make_vote_transaction( i ); insert( i, pack ); }
      schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 1.0f, 1024UL, 0UL, 0UL, &outcome );
    }

    for( ulong i=0UL; i<1024UL; i++ ) { make_vote_transaction( i ); insert( i, pack ); }
    ulong consumed_cost = (1024UL*FD_PACK_SIMPLE_VOTE_COST)*(FD_PACK_MAX_VOTE_COST_PER_BLOCK_UPPER_BOUND/(1024UL*FD_PACK_SIMPLE_VOTE_COST));
    ulong expected_votes = (FD_PACK_MAX_VOTE_COST_PER_BLOCK_UPPER_BOUND-consumed_cost)/FD_PACK_SIMPLE_VOTE_COST;

    schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 1.0f,        expected_votes, 0UL, 0UL, &outcome );
    FD_TEST( fd_pack_avail_txn_cnt( pack )==1024UL-expected_votes );

    fd_pack_end_block( pack );
    schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 1.0f, 1024UL-expected_votes, 0UL, 0UL, &outcome );
  }
  ...
  if( 1 ) {
    ...
    for( j=0UL; j<FD_PACK_MAX_WRITE_COST_PER_ACCT_UPPER_BOUND/total_cus; j++ ) {
      make_transaction( j, 10000UL, 500U, 11.0, "A", "B", NULL, &total_cus );  /* transaction cost estimate = total_cus*/
      FD_TEST( 20334UL==total_cus );
      insert( j, pack );
      schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 0.0f, 1UL, 0UL, 0UL, &outcome );
    }
    /* Consumed total_cus*FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND/(4*total_cus)=11,966,340
       cost units, so this next one can't fit (due to
       FD_PACK_MAX_WRITE_COST_PER_ACCT_UPPER_BOUND) */

    make_transaction( j, 10000UL, 500U, 11.0, "A", "B", NULL, NULL );
    insert( j++, pack );
    schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 0.0f, 0UL, 0UL, 0UL, &outcome );
    FD_TEST( fd_pack_avail_txn_cnt( pack )==1UL );

    outcome.results->bank_cu.rebated_cus = (uint)((total_cus + (total_cus*FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND/(4*total_cus))) - FD_PACK_MAX_WRITE_COST_PER_ACCT_UPPER_BOUND);
  ...
  if( 1 ) {
    ...
    const ulong almost_full_iter = (FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND/( 8*total_cus ));
    ulong i=0UL;
    for( ulong j=0UL; j<almost_full_iter; j++ ) {
      ...
      schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 0.0f, 8UL, 0UL, 0UL, &outcome );
      i = i%512UL;
    }
    ...
    outcome.results[ 0 ].bank_cu.rebated_cus = (uint)(total_cus - (FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND - almost_full_iter*8UL*total_cus - 7UL*total_cus));
    fd_pack_rebate_sum_add_txn( rebater, outcome.results, rebate_alt, 1UL );
    fd_pack_rebate_sum_report( rebater, report->rebate );
    fd_pack_rebate_cus( pack, report->rebate );
    schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 0.0f, 1UL, 0UL, 0UL, &outcome );
  ...
  if( 1 ) {
    ...
    for( ulong j=0UL; j<almost_full_iter; j++ ) {
      ...
      schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 0.0f, 5UL, 0UL, 0UL, &outcome );
      i = i%512UL;
    }
    ...
    schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 0.0f, 3UL, 0UL, 0UL, &outcome );
    FD_TEST( fd_pack_avail_txn_cnt( pack )==2UL );

    fd_pack_end_block( pack );
    schedule_validate_microblock( pack, FD_PACK_MAX_COST_PER_BLOCK_UPPER_BOUND, 0.0f, 2UL, 0UL, 0UL, &outcome );
  }
}
