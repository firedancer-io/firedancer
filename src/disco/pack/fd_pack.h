...
/* fd_pack_set_block_limits: Updates the limits provided fd_pack_new to
   ...
   before).  future microblocks will also exclude those that cause the
   total block cost to exceed max_cost_per_block. Similarly those that
   cause the total vote-only cost to exceed max_vote_cost_per_block.
   Also, those that cause the total per-account, per block write cost
   to exceed max_write_cost_per_acct.  Limits are inclusive, as per
   usual (i.e. a block may have exactly max_microblocks_per_block
...
void fd_pack_set_block_limits( fd_pack_t * pack,
