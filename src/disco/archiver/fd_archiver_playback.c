  ...
  if( FD_LIKELY( in_idx==1 && ctx->use_rocksdb ) ) {
    ...
    if( msg->type==FD_REPLAY_SLOT_TYPE ) {
      ...
      if( !memcmp( bank_hash, &versioned->inner.current.frozen_hash, sizeof(fd_hash_t) ) ) {
        FD_LOG_WARNING(( "Bank hash matches rocksdb: slot=%lu, hash=%s", slot, FD_BASE58_ENC_32_ALLOCA( bank_hash->hash ) ));
      } else {
        FD_LOG_ERR(( "Playback detects bank hash mismatch for slot%lu\nreplay gives %s\nrocksdb gives %s",
                     slot,
      ...
      if( FD_UNLIKELY( slot==ctx->rocksdb_end_slot ) ) {
        FD_LOG_ERR(( "Finish playing back all shreds in rocksdb till slot=%lu", slot ));
      }
    }
  }
}
