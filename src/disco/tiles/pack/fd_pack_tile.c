...
fd_pack_tile_align( void ) {
  return FD_PACK_TILE_ALIGN;
}

FD_FN_PURE ulong
fd_pack_tile_footprint( fd_pack_tile_args_t const * args ) {
  ulong l = FD_LAYOUT_INIT;
  l = FD_LAYOUT_APPEND( l, alignof( fd_pack_tile_t ), sizeof( fd_pack_tile_t ) );
  l = FD_LAYOUT_APPEND( l, fd_pack_align(), fd_pack_footprint( args->max_pending_transactions,
                                                               args->bank_tile_count,
                                                               MAX_TXN_PER_MICROBLOCK ) );
  return FD_LAYOUT_FINI( l, fd_pack_tile_align() );
}
...
metrics_write( void * _ctx ) {
  fd_pack_tile_t * ctx = (fd_pack_tile_t *)_ctx;
...
during_frag( void * _ctx,
  ...
  if( FD_UNLIKELY( in_idx==ctx->stake_info_in_idx ) ) {
...
after_frag( void *             _ctx,
  ...
  } else if( FD_UNLIKELY( in_idx==ctx->stake_info_in_idx ) ) {
...
fd_pack_tile_join_privileged( void *                      shpack,
  ...
  FD_SCRATCH_ALLOC_INIT( l, shpack );
  fd_pack_tile_t * ctx = FD_SCRATCH_ALLOC_APPEND( l, alignof( fd_pack_tile_t ), sizeof( fd_pack_tile_t ) );
  ...
  /* This seems like overkill for just the public key, but it's not
     ...
     private key. */
  void const * identity_pubkey = fd_keyload_load( args->identity_key_path, 1 /* public_key_only */ );
...
fd_pack_tile_join( void *                      shpack,
  ...
  if( FD_UNLIKELY( !topo->bank_tile_cnt ) ) FD_LOG_ERR(( "pack tile connects to no banking tiles" ));
  if( FD_UNLIKELY( topo->bank_tile_cnt>FD_PACK_TILE_PACK_MAX_OUT ) ) FD_LOG_ERR(( "pack tile connects to too many banking tiles" ));
  if( FD_UNLIKELY( topo->bank_tile_cnt!=args->bank_tile_count ) ) FD_LOG_ERR(( "pack tile connects to %lu banking tiles, but args->bank_tile_count is %lu", topo->bank_tile_cnt, args->bank_tile_count ));

  ulong pack_footprint = fd_pack_footprint( args->max_pending_transactions, topo->bank_tile_cnt, MAX_TXN_PER_MICROBLOCK );

  FD_SCRATCH_ALLOC_INIT( l, shpack );
  fd_pack_tile_t * ctx = FD_SCRATCH_ALLOC_APPEND( l, alignof( fd_pack_tile_t ), sizeof( fd_pack_tile_t ) );
  fd_rng_t *  rng = fd_rng_join( fd_rng_new( ctx->rng, 0U, 0UL ) );
  ...
  ctx->pack = fd_pack_join( fd_pack_new( FD_SCRATCH_ALLOC_APPEND( l, fd_pack_align(), pack_footprint ),
                                         args->max_pending_transactions, topo->bank_tile_cnt, MAX_TXN_PER_MICROBLOCK, FD_PACK_MAX_MICROBLOCKS_PER_BLOCK, rng ) );
  if( FD_UNLIKELY( !ctx->pack ) ) FD_LOG_ERR(( "fd_pack_new failed" ));

  ctx->cur_spot = NULL;
  ctx->leader_slot = ULONG_MAX;

  ctx->out_cnt  = topo->bank_tile_cnt;
  for( ulong i=0UL; i<topo->bank_tile_cnt; i++ ) {
    ctx->out_current[ i ] = topo->out_busy[ i ];
  ...
  ctx->stake_info_in_idx = topo->stake_info_in_idx;
  ...
  FD_LOG_INFO(( "packing blocks of at most %lu transactions to %lu bank tiles", MAX_TXN_PER_MICROBLOCK, topo->bank_tile_cnt ));

  ulong scratch_top = FD_SCRATCH_ALLOC_FINI( l, 1UL );
  if( FD_UNLIKELY( scratch_top > (ulong)shpack + fd_pack_tile_footprint( args ) ) )
    FD_LOG_ERR(( "scratch overflow %lu %lu %lu", scratch_top - (ulong)shpack - fd_pack_tile_footprint( args ), scratch_top, (ulong)shpack + fd_pack_tile_footprint( args ) ));
