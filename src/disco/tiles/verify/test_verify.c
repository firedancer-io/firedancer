...
static void
setup_verify_ctx( fd_verify_tile_t * ctx, void ** mem ) {
  /* tcache - note: using aligned_alloc for tests */
  ulong depth     = FD_VERIFY_TILE_TCACHE_DEPTH;
  ulong map_cnt   = FD_VERIFY_TILE_TCACHE_MAP_CNT;
  ...
  *mem = aligned_alloc( align, footprint ); FD_TEST( *mem );
  fd_tcache_t * tcache = fd_tcache_join( fd_tcache_new( *mem, FD_VERIFY_TILE_TCACHE_DEPTH, FD_VERIFY_TILE_TCACHE_MAP_CNT ) );
...
static void
free_verify_ctx( fd_verify_tile_t * ctx, void * mem ) {
  free(mem);
...
test_verify_success( void ) {
  ...
  res = fd_verify_tile_txn_verify( ctx, payload, (ushort)payload_sz, txn, &opt_sig );
  ...
  int res1 = fd_verify_tile_txn_verify( ctx, payload, (ushort)payload_sz, txn, &opt_sig );
