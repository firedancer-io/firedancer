...
fd_store_query( fd_store_t       * store,
  ...
  store->metrics.query_cnt++;

  for( uint part_idx = 0; part_idx < store->part_cnt; part_idx++ ) {
    fd_store_key_t   key = { *merkle_root, part_idx };
    fd_store_fec_t * fec = fd_store_map_ele_query( map_laddr( store ), &key, NULL, pool_laddr( store ) );
    if( FD_LIKELY( fec ) ) return fec;
  }

  store->metrics.query_missing_cnt++;
...
fd_store_insert( fd_store_t * store,
  ...
  for( ulong part_idx = 0; part_idx < store->part_cnt; part_idx++ ) {
    ...
    if( FD_UNLIKELY( fec ) ) {
      ...
      store->metrics.insert_duplicate_cnt++;
  ...
  case FD_POOL_ERR_EMPTY:   FD_LOG_WARNING(( "store full %s",    fd_store_pool_strerror( err ) )); store->metrics.insert_full_cnt++; store->metrics.insert_full_mr = merkle_root->ul[0]; return NULL;
...
fd_store_remove( fd_store_t      * store,
  ...
  store->metrics.remove_cnt++;
  store->metrics.remove_mr = merkle_root->ul[0];

  for( uint part_idx = 0; part_idx < store->part_cnt; part_idx++ ) {
    fd_store_key_t   key = { *merkle_root, part_idx };
    fd_store_fec_t * fec = fd_store_map_ele_remove( map_laddr( store ), &key, NULL, pool_laddr( store ) );
    if( FD_LIKELY( fec ) ) return fec;
  }

  store->metrics.remove_missing_cnt++;
