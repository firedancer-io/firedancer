...
fd_store_new( void * shmem, ulong fec_max, ulong seed ) {
  ...
  store->seed = seed;
  store->root = fd_store_pool_idx_null( fd_store_pool( store ) );
  FD_COMPILER_MFENCE();
...
fd_store_clear( fd_store_t * store ) {
  ...
  fd_store_fec_t * pool = fd_store_pool( store );
  ulong            null = fd_store_pool_idx_null( pool );
  if( FD_UNLIKELY( !fd_store_pool_used( pool ) ) ) return store;

  fd_store_fec_t * head = fd_store_root( store );
  fd_store_fec_t * tail = head;
  for( fd_store_map_iter_t iter = fd_store_map_iter_init( map, pool );
       !fd_store_map_iter_done( iter, map, pool );
       iter = fd_store_map_iter_next( iter, map, pool ) ) {
    ulong idx = fd_store_map_iter_idx( iter, map, pool );
  ...
  for( ulong idx = fd_store_pool_idx( pool, head );
    ...
    fd_store_map_idx_remove  ( map, &fec->key, null, pool );
    fd_store_pool_idx_release( pool, idx );
  }
  store->root = null;
# if FD_STORE_USE_HANDHOLDING
  FD_TEST( !fd_store_pool_used( pool ) );
# endif
  return store;
}
...
print( fd_store_t const * store, fd_store_fec_t const * fec, int space, const char * prefix ) {
  if( fec == NULL ) return;
  fd_store_fec_t const * pool = fd_store_pool_const( store );
  if( space > 0 ) printf( "\n" );
  for( int i = 0; i < space; i++ ) printf( " " );
  printf( "%s%s", prefix, FD_BASE58_ENC_32_ALLOCA( &fec->key ) );

  fd_store_fec_t const * child = fd_store_pool_ele_const( pool, fec->child );
  char new_prefix[1024]; /* FIXME size this correctly */
  while( child ) {
    if( fd_store_pool_ele_const( pool, child->sibling ) ) {
      sprintf( new_prefix, "├── " ); /* branch indicating more siblings follow */
      print( store, child, space + 4, new_prefix );
    } else {
      sprintf( new_prefix, "└── " ); /* end branch */
      print( store, child, space + 4, new_prefix );
    }
    child = fd_store_pool_ele_const( pool, child->sibling );
  }
}
