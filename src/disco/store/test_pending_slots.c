...
void
random_shuffle( ulong* array, uint len, fd_rng_t * rng ) {
    for (ulong i = 0; i < len; i++) {
      ulong idx_to_swap = fd_rng_uint_roll( rng, len );
      FD_TEST( idx_to_swap < len );
...
main( int argc, char ** argv ) {
  fd_boot( &argc, &argv );
  ulong seed    = fd_env_strip_cmdline_ulong( &argc, &argv, "--seed", "",  16UL );
  ...
  fd_pending_slots_t * pending_slots = fd_pending_slots_join( fd_pending_slots_new( (void *)buf, seed ) );
  ...
  random_shuffle( slots, FD_BLOCK_MAX, rng );
  ...
  for (int i = 0; i < FD_BLOCK_MAX / 2 + 1; i++) {
    fd_pending_slots_add( pending_slots, FD_BLOCK_MAX * 2 + (ulong)i, 0 );
  }
