...
def _write_off_enum(f: TextIO, metrics: List[Metric], prefix: str, start_expr: str = None, end_name: str = None):
    """Write an anonymous enum defining all _OFF values for a list of metrics."""
    if not metrics and not end_name:
        return
    f.write('enum {\n')
    ...
    for metric in metrics:
        ...
        off_name = f'FD_METRICS_{metric.type.name.upper()}_{prefix.upper()}_{full_name}_OFF'

        if first and start_expr:
            f.write(f'  {off_name} = {start_expr},\n')
        else:
            f.write(f'  {off_name},\n')
        first = False
        ...
        if isinstance(metric, (CounterEnumMetric, GaugeEnumMetric)):
            for idx, value in enumerate(metric.enum.values):
                val_off_name = f'FD_METRICS_{metric.type.name.upper()}_{prefix.upper()}_{full_name}_{camel2snake(value.name)}_OFF'
                if idx == 0:
                    f.write(f'  {val_off_name} = {off_name},\n')
                else:
                    f.write(f'  {val_off_name},\n')
...
def _write_common(metrics: Metrics):
    with open(Path(__file__).parent / '../generated' / 'fd_metrics_all.h', 'w') as f:
        ...
        f.write('/* LINK IN offset enum */\n\n')
        _write_off_enum(f, metrics.link_in, "link")

        f.write('/* TILE offset enum */\n\n')
        _write_off_enum(f, metrics.common, "tile")
        ...
        f.write('/* TILE metric properties */\n\n')
    ...
    with open(Path(__file__).parent / '../generated' / 'fd_metrics_all.c', 'w') as f:
        ...
        for tile in Tile:
            if tile in metrics.tiles:
                ...
                total = sum([int(m.count()) for m in tile_metrics])
                if total > 0:
...
def _write_tile(tile: Tile, metrics: List[Metric]):
    with open(Path(__file__).parent / '../generated' / f'fd_metrics_{tile.name.lower()}.h', 'w') as f:
        ...
        if metrics:
            _write_off_enum(f, metrics, tile.name.lower(), start_expr='FD_METRICS_TILE_OFF')
...
def _write_enums(enums: List[MetricEnum], common_offset: int):
    with open(Path(__file__).parent / '../generated' / 'fd_metrics_enums.h', 'w') as f:
        ...
        f.write('/* THIS FILE IS GENERATED BY gen_metrics.py. DO NOT HAND EDIT. */\n\n')
        f.write(f'#define FD_METRICS_TILE_OFF ({common_offset}UL)\n\n')
        for enum in enums:
