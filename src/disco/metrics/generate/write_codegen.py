...
def write_event_snap_codegen(metrics: Metrics):
    ...
    with open(Path(__file__).parent / '../generated' / 'fd_metric_event_snap.h', 'w') as f:
        ...
        f.write('#include "../../events/generated/fd_event.h"\n\n')
        f.write('void\n')
        f.write('fd_metric_event_snap( fd_topo_t *          topo,\n')
        f.write('                      fd_event_metrics_t * metrics );\n\n')
    ...
    with open(Path(__file__).parent / '../generated' / 'fd_metric_event_snap.c', 'w') as f:
        ...
        f.write(f'                      fd_event_metrics_t * event ) {{\n')
        f.write(f'  uchar * buffer = (uchar *)event;\n\n')

        f.write(f'  for( ulong i=0UL; i<event->tile_len; i++ ) {{\n')
        f.write(f'    fd_event_metrics_tile_t * event_tile = ((fd_event_metrics_tile_t *)(buffer+event->tile_off))+i;\n')
        ...
        f.write(f'      fd_event_metrics_link_t * link = ((fd_event_metrics_link_t *)(buffer+event->link_off))+link_idx;\n')
        ...
        for tile in Tile:
            ...
            f.write(f'    fd_event_metrics_{tile.name.lower()}_t * {tile.name.lower()} = ((fd_event_metrics_{tile.name.lower()}_t *)(buffer+event->{tile.name.lower()}_off))+i;\n\n')
