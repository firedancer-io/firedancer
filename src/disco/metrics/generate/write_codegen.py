...
def _write_tile(tile: Tile, metrics: List[Metric], common_offset: int):
    with open(Path(__file__).parent / '../generated' / f'fd_metrics_{tile.name.lower()}.h', 'w') as f:
        ...
        if metrics:
            _write_off_enum(f, metrics, tile.name.lower(), start_expr=str(common_offset))
...
def write_codegen(metrics: Metrics):
    ...
    for (tile, tile_metrics) in metrics.tiles.items():
        _write_tile(tile, tile_metrics, common_offset)
