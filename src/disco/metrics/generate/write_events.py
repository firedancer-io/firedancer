...
def write_events(metrics: Metrics):
    with open(Path(__file__).parent / '../../events/schema/metrics.json', 'w') as f:
        f.write('{\n')
        f.write('    "name": "metrics",\n')
        f.write('    "id": 2,\n')
    ...
    print(f"Wrote {metrics.count()} metrics to src/disco/events/schema/metrics.json")
