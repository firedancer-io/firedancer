...
fd_metric_event_snap( fd_topo_t *          topo,
                      fd_event_metrics_t * event ) {
  uchar * buffer = (uchar *)event;

  for( ulong i=0UL; i<event->tile_len; i++ ) {
    fd_event_metrics_tile_t * event_tile = ((fd_event_metrics_tile_t *)(buffer+event->tile_off))+i;
  ...
  for( ulong i=0UL; i<topo->tile_cnt; i++ ) {
    ...
    for( ulong j=0UL; j<tile->in_cnt; j++ ) {
      ...
      fd_event_metrics_link_t * link = ((fd_event_metrics_link_t *)(buffer+event->link_off))+link_idx;
  ...
  for( ulong i=0UL; i<event->net_len; i++ ) {
    fd_event_metrics_net_t * net = ((fd_event_metrics_net_t *)(buffer+event->net_off))+i;
  ...
  for( ulong i=0UL; i<event->quic_len; i++ ) {
    fd_event_metrics_quic_t * quic = ((fd_event_metrics_quic_t *)(buffer+event->quic_off))+i;
  ...
  for( ulong i=0UL; i<event->verify_len; i++ ) {
    fd_event_metrics_verify_t * verify = ((fd_event_metrics_verify_t *)(buffer+event->verify_off))+i;
  ...
  for( ulong i=0UL; i<event->dedup_len; i++ ) {
    fd_event_metrics_dedup_t * dedup = ((fd_event_metrics_dedup_t *)(buffer+event->dedup_off))+i;
  ...
  for( ulong i=0UL; i<event->resolv_len; i++ ) {
    fd_event_metrics_resolv_t * resolv = ((fd_event_metrics_resolv_t *)(buffer+event->resolv_off))+i;
  ...
  for( ulong i=0UL; i<event->pack_len; i++ ) {
    fd_event_metrics_pack_t * pack = ((fd_event_metrics_pack_t *)(buffer+event->pack_off))+i;
  ...
  for( ulong i=0UL; i<event->bank_len; i++ ) {
    fd_event_metrics_bank_t * bank = ((fd_event_metrics_bank_t *)(buffer+event->bank_off))+i;
   ...
   (void)bank; (void)metrics;
    ...
    bank->transaction_load.success = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_SUCCESS ) ];
    bank->transaction_load.account_in_use = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_ACCOUNT_IN_USE ) ];
    bank->transaction_load.account_loaded_twice = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_ACCOUNT_LOADED_TWICE ) ];
    bank->transaction_load.account_not_found = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_ACCOUNT_NOT_FOUND ) ];
    bank->transaction_load.program_account_not_found = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_PROGRAM_ACCOUNT_NOT_FOUND ) ];
    bank->transaction_load.insufficient_funds_for_fee = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_INSUFFICIENT_FUNDS_FOR_FEE ) ];
    bank->transaction_load.invalid_account_for_fee = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_INVALID_ACCOUNT_FOR_FEE ) ];
    bank->transaction_load.already_processed = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_ALREADY_PROCESSED ) ];
    bank->transaction_load.blockhash_not_found = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_BLOCKHASH_NOT_FOUND ) ];
    bank->transaction_load.instruction_error = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_INSTRUCTION_ERROR ) ];
    bank->transaction_load.call_chain_too_deep = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_CALL_CHAIN_TOO_DEEP ) ];
    bank->transaction_load.missing_signature_for_fee = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_MISSING_SIGNATURE_FOR_FEE ) ];
    bank->transaction_load.invalid_account_index = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_INVALID_ACCOUNT_INDEX ) ];
    bank->transaction_load.signature_failure = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_SIGNATURE_FAILURE ) ];
    bank->transaction_load.invalid_program_for_execution = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_INVALID_PROGRAM_FOR_EXECUTION ) ];
    bank->transaction_load.sanitize_failure = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_SANITIZE_FAILURE ) ];
    bank->transaction_load.cluster_maintenance = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_CLUSTER_MAINTENANCE ) ];
    bank->transaction_load.account_borrow_outstanding = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_ACCOUNT_BORROW_OUTSTANDING ) ];
    bank->transaction_load.would_exceed_max_block_cost_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_WOULD_EXCEED_MAX_BLOCK_COST_LIMIT ) ];
    bank->transaction_load.unsupported_version = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_UNSUPPORTED_VERSION ) ];
    bank->transaction_load.invalid_writable_account = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_INVALID_WRITABLE_ACCOUNT ) ];
    bank->transaction_load.would_exceed_max_account_cost_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_WOULD_EXCEED_MAX_ACCOUNT_COST_LIMIT ) ];
    bank->transaction_load.would_exceed_account_data_block_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_WOULD_EXCEED_ACCOUNT_DATA_BLOCK_LIMIT ) ];
    bank->transaction_load.too_many_account_locks = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_TOO_MANY_ACCOUNT_LOCKS ) ];
    bank->transaction_load.address_lookup_table_not_found = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_ADDRESS_LOOKUP_TABLE_NOT_FOUND ) ];
    bank->transaction_load.invalid_address_lookup_table_owner = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_INVALID_ADDRESS_LOOKUP_TABLE_OWNER ) ];
    bank->transaction_load.invalid_address_lookup_table_data = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_INVALID_ADDRESS_LOOKUP_TABLE_DATA ) ];
    bank->transaction_load.invalid_address_lookup_table_index = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_INVALID_ADDRESS_LOOKUP_TABLE_INDEX ) ];
    bank->transaction_load.invalid_rent_paying_account = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_INVALID_RENT_PAYING_ACCOUNT ) ];
    bank->transaction_load.would_exceed_max_vote_cost_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_WOULD_EXCEED_MAX_VOTE_COST_LIMIT ) ];
    bank->transaction_load.would_exceed_account_data_total_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_WOULD_EXCEED_ACCOUNT_DATA_TOTAL_LIMIT ) ];
    bank->transaction_load.duplicate_instruction = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_DUPLICATE_INSTRUCTION ) ];
    bank->transaction_load.insufficient_funds_for_rent = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_INSUFFICIENT_FUNDS_FOR_RENT ) ];
    bank->transaction_load.max_loaded_accounts_data_size_exceeded = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_MAX_LOADED_ACCOUNTS_DATA_SIZE_EXCEEDED ) ];
    bank->transaction_load.invalid_loaded_accounts_data_size_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_INVALID_LOADED_ACCOUNTS_DATA_SIZE_LIMIT ) ];
    bank->transaction_load.resanitization_needed = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_RESANITIZATION_NEEDED ) ];
    bank->transaction_load.program_execution_temporarily_restricted = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_PROGRAM_EXECUTION_TEMPORARILY_RESTRICTED ) ];
    bank->transaction_load.unbalanced_transaction = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_UNBALANCED_TRANSACTION ) ];
    bank->transaction_load.program_cache_hit_max_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_LOAD_PROGRAM_CACHE_HIT_MAX_LIMIT ) ];
    bank->transaction_executing.success = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_SUCCESS ) ];
    bank->transaction_executing.account_in_use = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_ACCOUNT_IN_USE ) ];
    bank->transaction_executing.account_loaded_twice = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_ACCOUNT_LOADED_TWICE ) ];
    bank->transaction_executing.account_not_found = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_ACCOUNT_NOT_FOUND ) ];
    bank->transaction_executing.program_account_not_found = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_PROGRAM_ACCOUNT_NOT_FOUND ) ];
    bank->transaction_executing.insufficient_funds_for_fee = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_INSUFFICIENT_FUNDS_FOR_FEE ) ];
    bank->transaction_executing.invalid_account_for_fee = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_INVALID_ACCOUNT_FOR_FEE ) ];
    bank->transaction_executing.already_processed = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_ALREADY_PROCESSED ) ];
    bank->transaction_executing.blockhash_not_found = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_BLOCKHASH_NOT_FOUND ) ];
    bank->transaction_executing.instruction_error = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_INSTRUCTION_ERROR ) ];
    bank->transaction_executing.call_chain_too_deep = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_CALL_CHAIN_TOO_DEEP ) ];
    bank->transaction_executing.missing_signature_for_fee = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_MISSING_SIGNATURE_FOR_FEE ) ];
    bank->transaction_executing.invalid_account_index = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_INVALID_ACCOUNT_INDEX ) ];
    bank->transaction_executing.signature_failure = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_SIGNATURE_FAILURE ) ];
    bank->transaction_executing.invalid_program_for_execution = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_INVALID_PROGRAM_FOR_EXECUTION ) ];
    bank->transaction_executing.sanitize_failure = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_SANITIZE_FAILURE ) ];
    bank->transaction_executing.cluster_maintenance = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_CLUSTER_MAINTENANCE ) ];
    bank->transaction_executing.account_borrow_outstanding = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_ACCOUNT_BORROW_OUTSTANDING ) ];
    bank->transaction_executing.would_exceed_max_block_cost_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_WOULD_EXCEED_MAX_BLOCK_COST_LIMIT ) ];
    bank->transaction_executing.unsupported_version = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_UNSUPPORTED_VERSION ) ];
    bank->transaction_executing.invalid_writable_account = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_INVALID_WRITABLE_ACCOUNT ) ];
    bank->transaction_executing.would_exceed_max_account_cost_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_WOULD_EXCEED_MAX_ACCOUNT_COST_LIMIT ) ];
    bank->transaction_executing.would_exceed_account_data_block_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_WOULD_EXCEED_ACCOUNT_DATA_BLOCK_LIMIT ) ];
    bank->transaction_executing.too_many_account_locks = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_TOO_MANY_ACCOUNT_LOCKS ) ];
    bank->transaction_executing.address_lookup_table_not_found = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_ADDRESS_LOOKUP_TABLE_NOT_FOUND ) ];
    bank->transaction_executing.invalid_address_lookup_table_owner = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_INVALID_ADDRESS_LOOKUP_TABLE_OWNER ) ];
    bank->transaction_executing.invalid_address_lookup_table_data = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_INVALID_ADDRESS_LOOKUP_TABLE_DATA ) ];
    bank->transaction_executing.invalid_address_lookup_table_index = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_INVALID_ADDRESS_LOOKUP_TABLE_INDEX ) ];
    bank->transaction_executing.invalid_rent_paying_account = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_INVALID_RENT_PAYING_ACCOUNT ) ];
    bank->transaction_executing.would_exceed_max_vote_cost_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_WOULD_EXCEED_MAX_VOTE_COST_LIMIT ) ];
    bank->transaction_executing.would_exceed_account_data_total_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_WOULD_EXCEED_ACCOUNT_DATA_TOTAL_LIMIT ) ];
    bank->transaction_executing.duplicate_instruction = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_DUPLICATE_INSTRUCTION ) ];
    bank->transaction_executing.insufficient_funds_for_rent = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_INSUFFICIENT_FUNDS_FOR_RENT ) ];
    bank->transaction_executing.max_loaded_accounts_data_size_exceeded = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_MAX_LOADED_ACCOUNTS_DATA_SIZE_EXCEEDED ) ];
    bank->transaction_executing.invalid_loaded_accounts_data_size_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_INVALID_LOADED_ACCOUNTS_DATA_SIZE_LIMIT ) ];
    bank->transaction_executing.resanitization_needed = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_RESANITIZATION_NEEDED ) ];
    bank->transaction_executing.program_execution_temporarily_restricted = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_PROGRAM_EXECUTION_TEMPORARILY_RESTRICTED ) ];
    bank->transaction_executing.unbalanced_transaction = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_UNBALANCED_TRANSACTION ) ];
    bank->transaction_executing.program_cache_hit_max_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTING_PROGRAM_CACHE_HIT_MAX_LIMIT ) ];
    bank->transaction_executed.success = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_SUCCESS ) ];
    bank->transaction_executed.account_in_use = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_ACCOUNT_IN_USE ) ];
    bank->transaction_executed.account_loaded_twice = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_ACCOUNT_LOADED_TWICE ) ];
    bank->transaction_executed.account_not_found = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_ACCOUNT_NOT_FOUND ) ];
    bank->transaction_executed.program_account_not_found = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_PROGRAM_ACCOUNT_NOT_FOUND ) ];
    bank->transaction_executed.insufficient_funds_for_fee = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_INSUFFICIENT_FUNDS_FOR_FEE ) ];
    bank->transaction_executed.invalid_account_for_fee = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_INVALID_ACCOUNT_FOR_FEE ) ];
    bank->transaction_executed.already_processed = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_ALREADY_PROCESSED ) ];
    bank->transaction_executed.blockhash_not_found = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_BLOCKHASH_NOT_FOUND ) ];
    bank->transaction_executed.instruction_error = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_INSTRUCTION_ERROR ) ];
    bank->transaction_executed.call_chain_too_deep = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_CALL_CHAIN_TOO_DEEP ) ];
    bank->transaction_executed.missing_signature_for_fee = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_MISSING_SIGNATURE_FOR_FEE ) ];
    bank->transaction_executed.invalid_account_index = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_INVALID_ACCOUNT_INDEX ) ];
    bank->transaction_executed.signature_failure = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_SIGNATURE_FAILURE ) ];
    bank->transaction_executed.invalid_program_for_execution = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_INVALID_PROGRAM_FOR_EXECUTION ) ];
    bank->transaction_executed.sanitize_failure = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_SANITIZE_FAILURE ) ];
    bank->transaction_executed.cluster_maintenance = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_CLUSTER_MAINTENANCE ) ];
    bank->transaction_executed.account_borrow_outstanding = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_ACCOUNT_BORROW_OUTSTANDING ) ];
    bank->transaction_executed.would_exceed_max_block_cost_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_WOULD_EXCEED_MAX_BLOCK_COST_LIMIT ) ];
    bank->transaction_executed.unsupported_version = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_UNSUPPORTED_VERSION ) ];
    bank->transaction_executed.invalid_writable_account = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_INVALID_WRITABLE_ACCOUNT ) ];
    bank->transaction_executed.would_exceed_max_account_cost_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_WOULD_EXCEED_MAX_ACCOUNT_COST_LIMIT ) ];
    bank->transaction_executed.would_exceed_account_data_block_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_WOULD_EXCEED_ACCOUNT_DATA_BLOCK_LIMIT ) ];
    bank->transaction_executed.too_many_account_locks = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_TOO_MANY_ACCOUNT_LOCKS ) ];
    bank->transaction_executed.address_lookup_table_not_found = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_ADDRESS_LOOKUP_TABLE_NOT_FOUND ) ];
    bank->transaction_executed.invalid_address_lookup_table_owner = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_INVALID_ADDRESS_LOOKUP_TABLE_OWNER ) ];
    bank->transaction_executed.invalid_address_lookup_table_data = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_INVALID_ADDRESS_LOOKUP_TABLE_DATA ) ];
    bank->transaction_executed.invalid_address_lookup_table_index = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_INVALID_ADDRESS_LOOKUP_TABLE_INDEX ) ];
    bank->transaction_executed.invalid_rent_paying_account = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_INVALID_RENT_PAYING_ACCOUNT ) ];
    bank->transaction_executed.would_exceed_max_vote_cost_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_WOULD_EXCEED_MAX_VOTE_COST_LIMIT ) ];
    bank->transaction_executed.would_exceed_account_data_total_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_WOULD_EXCEED_ACCOUNT_DATA_TOTAL_LIMIT ) ];
    bank->transaction_executed.duplicate_instruction = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_DUPLICATE_INSTRUCTION ) ];
    bank->transaction_executed.insufficient_funds_for_rent = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_INSUFFICIENT_FUNDS_FOR_RENT ) ];
    bank->transaction_executed.max_loaded_accounts_data_size_exceeded = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_MAX_LOADED_ACCOUNTS_DATA_SIZE_EXCEEDED ) ];
    bank->transaction_executed.invalid_loaded_accounts_data_size_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_INVALID_LOADED_ACCOUNTS_DATA_SIZE_LIMIT ) ];
    bank->transaction_executed.resanitization_needed = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_RESANITIZATION_NEEDED ) ];
    bank->transaction_executed.program_execution_temporarily_restricted = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_PROGRAM_EXECUTION_TEMPORARILY_RESTRICTED ) ];
    bank->transaction_executed.unbalanced_transaction = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_UNBALANCED_TRANSACTION ) ];
    bank->transaction_executed.program_cache_hit_max_limit = metrics[ MIDX( COUNTER, BANK, TRANSACTION_EXECUTED_PROGRAM_CACHE_HIT_MAX_LIMIT ) ];
    bank->cost_model_undercount = metrics[ MIDX( COUNTER, BANK, COST_MODEL_UNDERCOUNT ) ];
  }

  for( ulong i=0UL; i<event->shred_len; i++ ) {
    fd_event_metrics_shred_t * shred = ((fd_event_metrics_shred_t *)(buffer+event->shred_off))+i;
  ...
  for( ulong i=0UL; i<event->store_len; i++ ) {
    fd_event_metrics_store_t * store = ((fd_event_metrics_store_t *)(buffer+event->store_off))+i;
