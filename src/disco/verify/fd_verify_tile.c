  ...
  if( FD_LIKELY( is_bundle_packet || ctx->in_kind[ in_idx ]==IN_KIND_QUIC ) ) {
    ...
    return ctx->round_robin_idx!=0UL;
      ...
      return fd_gossip_update_message_sig_tag( sig )!=FD_GOSSIP_UPDATE_TAG_VOTE ||
             (seq % ctx->round_robin_cnt) != ctx->round_robin_idx;
  }
...
during_frag( fd_verify_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( ctx->in_kind[ in_idx ]==IN_KIND_QUIC ) ) {
    ...
    fd_gossip_update_msg_t const * msg = (fd_gossip_update_msg_t const *)fd_chunk_to_laddr_const( ctx->in[in_idx].mem, chunk );
    fd_txn_m_t * dst = (fd_txn_m_t *)fd_chunk_to_laddr( ctx->out_mem, ctx->out_chunk );
...
unprivileged_init( fd_topo_t *      topo,
  ...
  for( ulong i=0UL; i<tile->in_cnt; i++ ) {
    ...
    else if( FD_UNLIKELY( !strcmp( link->name, "gossip_out" ) ) ) ctx->in_kind[ i ] = IN_KIND_GOSSIP;
