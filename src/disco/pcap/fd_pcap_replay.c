#include "fd_pcap_replay.h"

#include "../../util/net/fd_pcap.h"
#include <stdio.h>
#include <errno.h>

FD_STATIC_ASSERT( FD_FCTL_ALIGN<=FD_PCAP_REPLAY_TILE_SCRATCH_ALIGN, packing );
...
fd_pcap_replay_tile_scratch_align( void ) {
  return FD_PCAP_REPLAY_TILE_SCRATCH_ALIGN;
}

ulong
fd_pcap_replay_tile_scratch_footprint( ulong out_cnt ) {
  if( FD_UNLIKELY( out_cnt>FD_PCAP_REPLAY_TILE_OUT_MAX ) ) return 0UL;
  ulong l = FD_LAYOUT_INIT;
  l = FD_LAYOUT_APPEND( l, fd_fctl_align(), fd_fctl_footprint( out_cnt ) ); /* fctl */
  return FD_LAYOUT_FINI( l, fd_pcap_replay_tile_scratch_align() );
}

int
fd_pcap_replay_tile( fd_cnc_t *       cnc,
  ...
  /* cnc state */
  ulong * cnc_diag;               /* ==fd_cnc_app_laddr( cnc ), local address of the pcap_replay tile cnc diagnostic region */
  ...
  ulong   cnc_diag_pcap_done;     /* is the pcap file stream pcap_replay done */
  ...
  ulong * sync;   /* ==fd_mcache_seq_laddr( mcache ), local addr where pcap_replay mcache sync info is published */
  ulong   seq;    /* seq pcap_replay frag sequence number to publish */
  ...
  do {
    ...
    FD_LOG_INFO(( "Booting pcap_replay (out-cnt %lu)", out_cnt ));
    if( FD_UNLIKELY( out_cnt>FD_PCAP_REPLAY_TILE_OUT_MAX ) ) { FD_LOG_WARNING(( "out_cnt too large" )); return 1; }
    ...
    if( FD_UNLIKELY( !fd_ulong_is_aligned( (ulong)scratch, fd_pcap_replay_tile_scratch_align() ) ) ) {
    ...
    FD_COMPILER_MFENCE();
    cnc_diag[ FD_PCAP_REPLAY_CNC_DIAG_PCAP_DONE ] = 0UL; /* Clear before entering running state */
    FD_COMPILER_MFENCE();
    ...
    chunk  = FD_VOLATILE_CONST( cnc_diag[ FD_PCAP_REPLAY_CNC_DIAG_CHUNK_IDX ] );
  ...
  FD_LOG_INFO(( "Running pcap_replay (orig %lu)", orig ));
  ...
  for(;;) {
    ...
    if( FD_UNLIKELY( (now-then)>=0L ) ) {
      ...
      cnc_diag[ FD_PCAP_REPLAY_CNC_DIAG_CHUNK_IDX     ]  = chunk;
      cnc_diag[ FD_PCAP_REPLAY_CNC_DIAG_PCAP_DONE     ]  = cnc_diag_pcap_done;
      cnc_diag[ FD_PCAP_REPLAY_CNC_DIAG_PCAP_PUB_CNT  ] += cnc_diag_pcap_pub_cnt;
      cnc_diag[ FD_PCAP_REPLAY_CNC_DIAG_PCAP_PUB_SZ   ] += cnc_diag_pcap_pub_sz;
      cnc_diag[ FD_PCAP_REPLAY_CNC_DIAG_PCAP_FILT_CNT ] += cnc_diag_pcap_filt_cnt;
      cnc_diag[ FD_PCAP_REPLAY_CNC_DIAG_PCAP_FILT_SZ  ] += cnc_diag_pcap_filt_sz;
      FD_COMPILER_MFENCE();
      cnc_diag_backp_cnt     = 0UL;
      cnc_diag_pcap_pub_cnt  = 0UL;
      cnc_diag_pcap_pub_sz   = 0UL;
      ...
      if( FD_UNLIKELY( s!=FD_CNC_SIGNAL_RUN ) ) {
        ...
        if( FD_UNLIKELY( s!=FD_PCAP_REPLAY_CNC_SIGNAL_ACK ) ) {
  ...
  do {
    ...
    FD_LOG_INFO(( "Halting pcap_replay" ));
    ...
    FD_LOG_INFO(( "Halted pcap_replay" ));
