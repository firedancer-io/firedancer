...
after_credit( fd_bundle_ctx_t *   ctx,
  ...
  *charge_busy = 1;
  ...
  /* Increment, skip 0 on overflow */
  ctx->bundle_id = fd_ulong_max( ctx->bundle_id+1UL, 1UL );
...
replace_domain_with_ip( char const * url,
  ...
  if( FD_LIKELY( !domain_end ) ) domain_end = url + strlen( url );

  ulong domain_len = (ulong)(domain_end-domain_start);
  if( FD_UNLIKELY( domain_len>=256UL ) ) FD_LOG_ERR(( "[tiles.bundle.url] `%s` is too long (%lu>255)", url, domain_len ));
...
populate_allowed_seccomp( fd_topo_t const *      topo,
  ...
  populate_sock_filter_policy_fd_bundle_tile( out_cnt, out, (uint)fd_log_private_logfile_fd() );
