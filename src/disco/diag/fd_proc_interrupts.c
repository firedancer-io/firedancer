...
read_until( fd_io_buffered_istream_t * is,
  ...
  while( fd_io_buffered_istream_peek_sz( is )<min_sz ) {
    int err = fd_io_buffered_istream_fetch( is );
    if( err==-1 ) break;
    if( err!=0  ) return err;
  }
...
/* read_cpu_map reads the first line of /proc/interrupts mapping columns
   to CPUs.  Usually 0,1,2,3,... but has gaps for offline CPUs.  Also
   used for /proc/softirqs.  Formatted like this:
   "   CPU0  CPU1  CPU2  CPU3\n" */
