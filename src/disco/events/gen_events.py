...
from .check_event_schemas import ClickHouseType, Field, Event, check_schema_compatability

def generate_header(events: Dict[str, Event]):
    os.makedirs(Path(__file__).parent / 'generated', exist_ok=True)

    with open(Path(__file__).parent / 'generated' / 'fd_event.h', 'w') as f:
    ...
    with open(Path(__file__).parent / 'generated' / 'fd_event_metrics.h', 'w') as f:
        ...
        f.write('  l = FD_LAYOUT_APPEND( l, alignof( fd_event_metrics_t ), sizeof( fd_event_metrics_t ) );\n')
        f.write('  l = FD_LAYOUT_APPEND( l, alignof( fd_event_metrics_tile_t ), topo->tile_cnt*sizeof( fd_event_metrics_tile_t ) );\n')
        f.write('  l = FD_LAYOUT_APPEND( l, alignof( fd_event_metrics_link_t ), fd_topo_polled_in_cnt( topo )*sizeof( fd_event_metrics_link_t ) );\n')
        for field in events['metrics'].fields.values():
            if field.deprecated or field.server_only:
                continue
            ...
            f.write(f'  l = FD_LAYOUT_APPEND( l, alignof( fd_event_metrics_{field.name}_t ), fd_topo_tile_name_cnt( topo, "{field.name}" )*sizeof( fd_event_metrics_{field.name}_t ) );\n')
        ...
        f.write('  fd_event_metrics_t * metrics = (fd_event_metrics_t *)(buffer+off);\n')
        f.write('  off += sizeof( fd_event_metrics_t );\n\n')
        ...
        for field in events['metrics'].fields.values():
            ...
            f.write(f'  off = fd_ulong_align_up( off, alignof( fd_event_metrics_{field.name}_t ) );\n')
            ...
            f.write(f'  off += {cnt}*sizeof( fd_event_metrics_{field.name}_t );\n\n')

        f.write('  ulong link_idx = 0UL;\n')
        f.write('  for( ulong i=0UL; i<topo->tile_cnt; i++ ) {\n')
        f.write('    fd_event_metrics_tile_t * tile = (fd_event_metrics_tile_t *)(buffer+((fd_event_metrics_t*)buffer)->tile_off)+i;\n')
        ...
        f.write('      fd_event_metrics_link_t * link = (fd_event_metrics_link_t *)(buffer+((fd_event_metrics_t*)buffer)->link_off)+link_idx;\n')
...
def generate_impl(events: Dict[str, Event]):
    ...
    with open(Path(__file__).parent / 'generated' / 'fd_event.c', 'w') as f:
        f.write('#include "fd_event.h"\n\n')
...
from .check_event_schemas import check_schema_compatability
def main():
    ...
    print(f'Wrote {len(events)} events to src/disco/events/generated')
