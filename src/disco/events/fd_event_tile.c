...
#include "generated/fd_event_metric_render.h"
...
struct fd_event_tile {
  ...
  ulong metrics_sample_id;
  long next_metrics_sample_deadline_ticks;
...
void
sample_metrics( fd_event_tile_t * ctx,
                fd_topo_t const * topo ) {
  ...
  for( ulong i=0UL; i<topo->tile_cnt; i++ ) {
    ...
    if( FD_UNLIKELY( ctx->tile_shutdown_rendered[ i ] ) ) continue;

    uchar * buffer = fd_circq_push_back( ctx->circq, 1UL, 8192UL );
    FD_TEST( buffer );
    FD_TEST( ctx->circq->cursor_push_seq );

    ulong nonce = ctx->circq->cursor_push_seq - 1UL;
    ulong event_id = fd_event_client_id_reserve( ctx->client );
    uint  sample_reason = 1U; /* PERIODIC */
    ulong sample_slot = 0UL;
    long rendered_sz = fd_event_metric_render( topo, tile, ctx->metrics_sample_id, sample_reason, sample_slot, nonce, event_id, timestamp_nanos, buffer, 8192UL );
    FD_TEST( rendered_sz>=0L );
    fd_circq_resize_back( ctx->circq, (ulong)rendered_sz );

    FD_COMPILER_MFENCE(); // Make sure metrics written before reading shutdown volatile

    ulong tile_status = fd_metrics_tile( tile->metrics )[ FD_METRICS_GAUGE_TILE_STATUS_OFF ];
    if( FD_UNLIKELY( tile_status==2UL ) ) ctx->tile_shutdown_rendered[ i ] = 1;
  }

  ctx->metrics_sample_id++;
}
...
before_credit( fd_event_tile_t *   ctx,
  ...
  if( FD_UNLIKELY( tickcount>=ctx->next_metrics_sample_deadline_ticks ) ) {
    sample_metrics( ctx, ctx->topo );
    ctx->next_metrics_sample_deadline_ticks = tickcount + (long)(fd_tempo_tick_per_ns( NULL)*5L*(long)1e9L); /* every 5 seconds */
  }
...
after_frag( fd_event_tile_t *   ctx,
  ...
  switch( ctx->in_kind[ in_idx ] ) {
    case IN_KIND_SHRED: {
      ...
      uchar * buffer = fd_circq_push_back( ctx->circq, 1UL, 4096UL ); /* TODO: HOW TO DETERMINE A WELL BOUNDED SIZE AUTOMATICALLY? */
      FD_TEST( buffer ); /* shred should be guaranteed to fit, since sz<=FD_NET_MTU */

      uint ip_addr = fd_disco_netmux_sig_ip( sig );
      uint source_port = 0U; /* TODO: Pass this along ... */
      int protocol;
      ...
      fd_pb_submsg_open( encoder, 3U ); /* Shred */
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->metrics_sample_id = 0UL;
  ctx->next_metrics_sample_deadline_ticks = 0L;
  ctx->topo = topo;
