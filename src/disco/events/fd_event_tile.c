...
metrics_write( fd_event_tile_t * ctx ) {
  FD_MGAUGE_SET( EVENT, EVENT_QUEUE_DEPTH, ctx->circq->cnt );
  FD_MCNT_SET( EVENT, EVENT_QUEUE_DROPPED, ctx->circq->metrics.drop_cnt );
  ...
  FD_MCNT_SET( EVENT, EVENT_SENT,          metrics->events_sent );
  FD_MCNT_SET( EVENT, EVENT_ACKED,         metrics->events_acked );
