...
#define FD_EVENT_METRICS_REASON_PERIODIC         (1)
#define FD_EVENT_METRICS_REASON_LEADER_START     (2)
#define FD_EVENT_METRICS_REASON_LEADER_END_START (3)
#define FD_EVENT_METRICS_REASON_LEADER_END       (4)

static inline char const *
fd_event_metrics_reason_str( uchar value ) {
  switch( value ) {
    case FD_EVENT_METRICS_REASON_PERIODIC: return "periodic";
    case FD_EVENT_METRICS_REASON_LEADER_START: return "leader_start";
    case FD_EVENT_METRICS_REASON_LEADER_END_START: return "leader_end_start";
    case FD_EVENT_METRICS_REASON_LEADER_END: return "leader_end";
    default: return "unknown";
  }
}
...
typedef struct fd_event_metrics_tile fd_event_metrics_tile_t;

struct fd_event_metrics_link {
  /* Tile type. */
  char kind[21];
...
typedef struct fd_event_metrics_link fd_event_metrics_link_t;

struct fd_event_metrics_net {
...
typedef struct fd_event_metrics_net fd_event_metrics_net_t;

struct fd_event_metrics_quic {
...
typedef struct fd_event_metrics_quic fd_event_metrics_quic_t;

struct fd_event_metrics_verify {
...
typedef struct fd_event_metrics_verify fd_event_metrics_verify_t;

struct fd_event_metrics_dedup {
...
typedef struct fd_event_metrics_dedup fd_event_metrics_dedup_t;

struct fd_event_metrics_resolv {
...
typedef struct fd_event_metrics_resolv fd_event_metrics_resolv_t;

struct fd_event_metrics_pack {
...
typedef struct fd_event_metrics_pack fd_event_metrics_pack_t;

struct fd_event_metrics_bank {
  ...
  /* Result of loading a transaction. */
  struct {
    ulong success; /* Success */ 
  ...
  /* Result of executing a transaction. Could be a failure or success. */
  struct {
    ulong success; /* Success */ 
    ulong account_in_use; /* An account is already being processed in another transaction in a way that does not support parallelism. */ 
    ulong account_loaded_twice; /* A `Pubkey` appears twice in the transaction's `account_keys`.  Instructions can reference `Pubkey`s more than once but the message must contain a list with no duplicate keys. */ 
  ...
  /* When a transaction executes (makes it onto the chain), result of
     executing a transaction. The transaction can still fail. */
  struct {
    ulong success; /* Success */ 
    ulong account_in_use; /* An account is already being processed in another transaction in a way that does not support parallelism. */ 
    ulong account_loaded_twice; /* A `Pubkey` appears twice in the transaction's `account_keys`.  Instructions can reference `Pubkey`s more than once but the message must contain a list with no duplicate keys. */ 
...
typedef struct fd_event_metrics_bank fd_event_metrics_bank_t;

struct fd_event_metrics_shred {
...
typedef struct fd_event_metrics_shred fd_event_metrics_shred_t;

struct fd_event_metrics_store {
...
typedef struct fd_event_metrics_store fd_event_metrics_store_t;

/* Metric data periodically sampled by the application. */
struct fd_event_metrics {
  /* Reason the metrics snapshot was sampled. Must be one of
     FD_EVENT_METRICS_REASON_* */
  uchar reason;
...
typedef struct fd_event_metrics fd_event_metrics_t;
