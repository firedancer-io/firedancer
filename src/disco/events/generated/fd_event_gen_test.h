...
fd_event_runtime_block_fill_max( fd_event_runtime_block_t * msg ) {
  ...
  msg->clock_sysvar.slot = ULONG_MAX;
  msg->clock_sysvar.epoch_start_timestamp = LONG_MAX;
  msg->clock_sysvar.epoch = ULONG_MAX;
  msg->clock_sysvar.leader_schedule_epoch = ULONG_MAX;
  msg->clock_sysvar.unix_timestamp = LONG_MAX;
  msg->recent_blockhashes_cnt = 151UL;
  for( ulong k=0UL; k<151UL; k++ ) {
    fd_memset( msg->recent_blockhashes[ k ], 0xFF, 32UL );
  }
  msg->recent_blockhashes_sysvar_cnt = 150UL;
  for( ulong k=0UL; k<150UL; k++ ) {
    fd_memset( msg->recent_blockhashes_sysvar[ k ].blockhash, 0xFF, 32UL );
    msg->recent_blockhashes_sysvar[ k ].lamports_per_signature = ULONG_MAX;
  }
...
fd_event_runtime_epoch_fill_max( fd_event_runtime_epoch_t * msg ) {
  ...
  msg->stake_history_sysvar.epoch = ULONG_MAX;
  msg->stake_history_sysvar.effective = ULONG_MAX;
  msg->stake_history_sysvar.activating = ULONG_MAX;
  msg->stake_history_sysvar.deactivating = ULONG_MAX;
  msg->epoch_schedule_sysvar.slots_per_epoch = ULONG_MAX;
  msg->epoch_schedule_sysvar.leader_schedule_slot_offset = ULONG_MAX;
  msg->epoch_schedule_sysvar.warmup = 1;
  msg->epoch_schedule_sysvar.first_normal_epoch = ULONG_MAX;
  msg->epoch_schedule_sysvar.first_normal_slot = ULONG_MAX;
  msg->feature_activations_cnt = 16UL;
  for( ulong k=0UL; k<16UL; k++ ) {
    fd_memset( msg->feature_activations[ k ], 0xFF, 32UL );
  }
  msg->epoch_rewards_sysvar.total_rewards = ULONG_MAX;
  msg->epoch_rewards_sysvar.distributed = ULONG_MAX;
  msg->epoch_rewards_sysvar.distribution_starting_block_height = ULONG_MAX;
  msg->epoch_rewards_sysvar.num_partitions = ULONG_MAX;
  msg->epoch_rewards_sysvar.total_points = (uint128)-1;
  fd_memset( msg->epoch_rewards_sysvar.parent_blockhash, 0xFF, 32UL );
  msg->vat_burn_per_vote_account = ULONG_MAX;
}
...
static inline void
fd_event_alpenglow_vote_fill_max( fd_event_alpenglow_vote_t * msg ) {
  fd_memset( msg, 0, sizeof(*msg) );
  msg->slot = ULONG_MAX;
  fd_memset( msg->block_id, 0xFF, 32UL );
  msg->voter_rank = USHORT_MAX;
  fd_memset( msg->voter_identity, 0xFF, 32UL );
  msg->kind = INT_MAX;
  ...
  for( ulong k=0UL; k<2000UL; k++ ) {
    ...
    fd_memset( msg->broadcast_to[ k ].ip, 0xFF, 16UL );
  ...
  msg->done_time = ULONG_MAX;
}

static void
fd_event_alpenglow_vote_fill_max_v( void * msg ) { fd_event_alpenglow_vote_fill_max( (fd_event_alpenglow_vote_t *)msg ); }

static inline void
fd_event_alpenglow_cert_fill_max( fd_event_alpenglow_cert_t * msg ) {
  fd_memset( msg, 0, sizeof(*msg) );
  msg->slot = ULONG_MAX;
  fd_memset( msg->block_id, 0xFF, 32UL );
  msg->kind = INT_MAX;
...
static const fd_event_gen_test_case_t fd_event_gen_test_cases[] = {
  ...
  { 13UL, FD_EVENT_RUNTIME_BLOCK_BUF_MAX, sizeof(fd_event_runtime_block_t), "runtime_block", fd_event_runtime_block_fill_max_v },
  { 14UL, FD_EVENT_RUNTIME_REWARD_BUF_MAX, sizeof(fd_event_runtime_reward_t), "runtime_reward", fd_event_runtime_reward_fill_max_v },
  { 15UL, FD_EVENT_RUNTIME_STAKE_DELEGATION_BUF_MAX, sizeof(fd_event_runtime_stake_delegation_t), "runtime_stake_delegation", fd_event_runtime_stake_delegation_fill_max_v },
  { 16UL, FD_EVENT_RUNTIME_ROOTED_BUF_MAX, sizeof(fd_event_runtime_rooted_t), "runtime_rooted", fd_event_runtime_rooted_fill_max_v },
  { 17UL, FD_EVENT_RUNTIME_EPOCH_BUF_MAX, sizeof(fd_event_runtime_epoch_t), "runtime_epoch", fd_event_runtime_epoch_fill_max_v },
