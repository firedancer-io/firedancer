...
struct fd_event_block_completed {
  ...
  int                                   votor_repaired;                            /* Whether the block reached replay after a votor event named that block and we repaired that version, rather than through the version received over turbine. Always false before alpenglow, and false for blocks this validator produced as leader. A slot can report both, on separate rows. */
  uint                                  repair_request_shred_block_id_count;       /* Alpenglow ShredForBlockId requests, analogous to legacy window requests. Asks a peer for a specific missing shred, addressed by the block id it belongs to. Always 0 before alpenglow and for leader blocks. */
  uint                                  repair_request_parent_count;               /* Alpenglow ParentAndFecCount requests, analogous to legacy orphan requests. Asks a peer for the block's parent and its FEC set count. Always 0 before alpenglow and for leader blocks. */
...
/* Worst-case encoded size of a block_completed event (envelope + Event
   submsg + inner submsg + all fields, padded for encoder slack). */
#define FD_EVENT_BLOCK_COMPLETED_BUF_MAX (11275623UL)
...
struct fd_event_runtime_block {
  ...
  uchar                                              block_id[ 32UL ];                    /* Block id of the slot. Under Tower this is the merkle root of the last FEC set (the last fec_merkle_roots entry). Under Alpenglow it is the block's double merkle root (DMR), which is not any FEC set's merkle root */
...
struct fd_event_runtime_rooted {
  ...
  ulong stake_delegations_removes; /* Number of root stake-delegations removed by this root advance: tombstone delta entries (counted per account per bank, tombstones for accounts absent from the root map still count) plus any epoch-boundary inactive-stake prunes. Matches the count of runtime_stake_delegation rows with kind remove whose slot is in (prev_root_slot, slot] except when one account is removed more than once in a block. */
...
struct fd_event_runtime_epoch {
  ulong                                          bank_seq;                            /* Monotonic sequence number identifying this block within the current run; the join key to runtime_block. Restarts at 1 each time a snapshot is loaded, so pair it with the stream's boot id. 0 means unavailable. */
  ulong                                          slot;                                /* First replayed slot of the epoch-boundary bank (the bank that crosses into this epoch); with skipped slots this is not necessarily the epoch's nominal first slot */
...
#define FD_EVENT_ALPENGLOW_VOTE_REASON_BLOCK_DEAD             (3) /* Replay ruled the block dead, so we vote skip on the entire rest of the window containing this slot. */
#define FD_EVENT_ALPENGLOW_VOTE_REASON_PARENT_READY           (4) /* We hold the certs that make a block a valid parent for the leader window starting at this slot (a notar, notar-fallback or fast-final cert for it, or it is finalized, and skip certs for every slot in between), so we vote notar for the window's first block we held pending. */
#define FD_EVENT_ALPENGLOW_VOTE_REASON_BLOCK_NOTARIZED        (5) /* We hold a notar cert for the block we voted notar for and cast no skip or fallback vote in the slot, so we vote final for it. */
#define FD_EVENT_ALPENGLOW_VOTE_REASON_TIMEOUT                (6) /* The slot's timeout fired with no vote cast in it, so we vote skip on the entire rest of the window containing this slot. */
#define FD_EVENT_ALPENGLOW_VOTE_REASON_TIMEOUT_CRASHED_LEADER (7) /* The window's crashed-leader timeout fired with no shred and no vote for its first slot, so we vote skip on every slot of the window. */
#define FD_EVENT_ALPENGLOW_VOTE_REASON_SAFE_TO_NOTAR          (8) /* No block but this one can be fast-finalized in the slot: notar votes for it reach 40% of stake, or 20% with skip votes bringing the total to 60%, and its parent is notar-fallback certified (Definition 16). Having voted skip or notar for another block in the slot, we vote notar-fallback for it and skip every unvoted slot of the window. */
#define FD_EVENT_ALPENGLOW_VOTE_REASON_SAFE_TO_SKIP           (9) /* No block can be fast-finalized in the slot: skip votes plus notar votes for every block but the most-voted one reach 40% of stake (Definition 16). Having voted notar in the slot, we vote skip-fallback for it and skip every unvoted slot of the window. */
