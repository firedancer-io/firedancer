...
static long
format_metrics( fd_event_metrics_t const * event,
                     ulong                           event_len,
                     char *                          buffer,
                     ulong                           buffer_len ) {
  ...
  success = fd_cstr_printf_check( buffer+off, buffer_len-off, &printed, "\"reason\":\"%s\",", fd_event_metrics_reason_str( event->reason ) );
  if( FD_UNLIKELY( !success ) ) return FD_EVENT_FORMAT_OVERFLOW;
  off += printed;
  ...
  if( FD_UNLIKELY( event->tile_off+event->tile_len*sizeof(fd_event_metrics_tile_t)>event_len ) ) return FD_EVENT_FORMAT_INVALID;
  for( ulong i=0UL; i<event->tile_len; i++ ) {
    fd_event_metrics_tile_t const * tile = ((fd_event_metrics_tile_t const *)(((char*)event)+event->tile_off))+i;
  ...
  if( FD_UNLIKELY( event->link_off+event->link_len*sizeof(fd_event_metrics_link_t)>event_len ) ) return FD_EVENT_FORMAT_INVALID;
  for( ulong i=0UL; i<event->link_len; i++ ) {
    fd_event_metrics_link_t const * link = ((fd_event_metrics_link_t const *)(((char*)event)+event->link_off))+i;
  ...
  if( FD_UNLIKELY( event->net_off+event->net_len*sizeof(fd_event_metrics_net_t)>event_len ) ) return FD_EVENT_FORMAT_INVALID;
  for( ulong i=0UL; i<event->net_len; i++ ) {
    fd_event_metrics_net_t const * net = ((fd_event_metrics_net_t const *)(((char*)event)+event->net_off))+i;
  ...
  if( FD_UNLIKELY( event->quic_off+event->quic_len*sizeof(fd_event_metrics_quic_t)>event_len ) ) return FD_EVENT_FORMAT_INVALID;
  for( ulong i=0UL; i<event->quic_len; i++ ) {
    fd_event_metrics_quic_t const * quic = ((fd_event_metrics_quic_t const *)(((char*)event)+event->quic_off))+i;
  ...
  if( FD_UNLIKELY( event->verify_off+event->verify_len*sizeof(fd_event_metrics_verify_t)>event_len ) ) return FD_EVENT_FORMAT_INVALID;
  for( ulong i=0UL; i<event->verify_len; i++ ) {
    fd_event_metrics_verify_t const * verify = ((fd_event_metrics_verify_t const *)(((char*)event)+event->verify_off))+i;
  ...
  if( FD_UNLIKELY( event->dedup_off+event->dedup_len*sizeof(fd_event_metrics_dedup_t)>event_len ) ) return FD_EVENT_FORMAT_INVALID;
  for( ulong i=0UL; i<event->dedup_len; i++ ) {
    fd_event_metrics_dedup_t const * dedup = ((fd_event_metrics_dedup_t const *)(((char*)event)+event->dedup_off))+i;
  ...
  if( FD_UNLIKELY( event->resolv_off+event->resolv_len*sizeof(fd_event_metrics_resolv_t)>event_len ) ) return FD_EVENT_FORMAT_INVALID;
  for( ulong i=0UL; i<event->resolv_len; i++ ) {
    fd_event_metrics_resolv_t const * resolv = ((fd_event_metrics_resolv_t const *)(((char*)event)+event->resolv_off))+i;
  ...
  if( FD_UNLIKELY( event->pack_off+event->pack_len*sizeof(fd_event_metrics_pack_t)>event_len ) ) return FD_EVENT_FORMAT_INVALID;
  for( ulong i=0UL; i<event->pack_len; i++ ) {
    fd_event_metrics_pack_t const * pack = ((fd_event_metrics_pack_t const *)(((char*)event)+event->pack_off))+i;
  ...
  if( FD_UNLIKELY( event->bank_off+event->bank_len*sizeof(fd_event_metrics_bank_t)>event_len ) ) return FD_EVENT_FORMAT_INVALID;
  for( ulong i=0UL; i<event->bank_len; i++ ) {
    fd_event_metrics_bank_t const * bank = ((fd_event_metrics_bank_t const *)(((char*)event)+event->bank_off))+i;
    ...
    success = fd_cstr_printf_check( buffer+off, buffer_len-off, &printed,
      ...
      bank->transaction_load.success,
      bank->transaction_load.account_in_use,
      bank->transaction_load.account_loaded_twice,
      bank->transaction_load.account_not_found,
      bank->transaction_load.program_account_not_found,
      bank->transaction_load.insufficient_funds_for_fee,
      bank->transaction_load.invalid_account_for_fee,
      bank->transaction_load.already_processed,
      bank->transaction_load.blockhash_not_found,
      bank->transaction_load.instruction_error,
      bank->transaction_load.call_chain_too_deep,
      bank->transaction_load.missing_signature_for_fee,
      bank->transaction_load.invalid_account_index,
      bank->transaction_load.signature_failure,
      bank->transaction_load.invalid_program_for_execution,
      bank->transaction_load.sanitize_failure,
      bank->transaction_load.cluster_maintenance,
      bank->transaction_load.account_borrow_outstanding,
      bank->transaction_load.would_exceed_max_block_cost_limit,
      bank->transaction_load.unsupported_version,
      bank->transaction_load.invalid_writable_account,
      bank->transaction_load.would_exceed_max_account_cost_limit,
      bank->transaction_load.would_exceed_account_data_block_limit,
      bank->transaction_load.too_many_account_locks,
      bank->transaction_load.address_lookup_table_not_found,
      bank->transaction_load.invalid_address_lookup_table_owner,
      bank->transaction_load.invalid_address_lookup_table_data,
      bank->transaction_load.invalid_address_lookup_table_index,
      bank->transaction_load.invalid_rent_paying_account,
      bank->transaction_load.would_exceed_max_vote_cost_limit,
      bank->transaction_load.would_exceed_account_data_total_limit,
      bank->transaction_load.duplicate_instruction,
      bank->transaction_load.insufficient_funds_for_rent,
      bank->transaction_load.max_loaded_accounts_data_size_exceeded,
      bank->transaction_load.invalid_loaded_accounts_data_size_limit,
      bank->transaction_load.resanitization_needed,
      bank->transaction_load.program_execution_temporarily_restricted,
      bank->transaction_load.unbalanced_transaction,
      bank->transaction_load.program_cache_hit_max_limit,
      bank->transaction_executing.success,
      bank->transaction_executing.account_in_use,
      bank->transaction_executing.account_loaded_twice,
      bank->transaction_executing.account_not_found,
      bank->transaction_executing.program_account_not_found,
      bank->transaction_executing.insufficient_funds_for_fee,
      bank->transaction_executing.invalid_account_for_fee,
      bank->transaction_executing.already_processed,
      bank->transaction_executing.blockhash_not_found,
      bank->transaction_executing.instruction_error,
      bank->transaction_executing.call_chain_too_deep,
      bank->transaction_executing.missing_signature_for_fee,
      bank->transaction_executing.invalid_account_index,
      bank->transaction_executing.signature_failure,
      bank->transaction_executing.invalid_program_for_execution,
      bank->transaction_executing.sanitize_failure,
      bank->transaction_executing.cluster_maintenance,
      bank->transaction_executing.account_borrow_outstanding,
      bank->transaction_executing.would_exceed_max_block_cost_limit,
      bank->transaction_executing.unsupported_version,
      bank->transaction_executing.invalid_writable_account,
      bank->transaction_executing.would_exceed_max_account_cost_limit,
      bank->transaction_executing.would_exceed_account_data_block_limit,
      bank->transaction_executing.too_many_account_locks,
      bank->transaction_executing.address_lookup_table_not_found,
      bank->transaction_executing.invalid_address_lookup_table_owner,
      bank->transaction_executing.invalid_address_lookup_table_data,
      bank->transaction_executing.invalid_address_lookup_table_index,
      bank->transaction_executing.invalid_rent_paying_account,
      bank->transaction_executing.would_exceed_max_vote_cost_limit,
      bank->transaction_executing.would_exceed_account_data_total_limit,
      bank->transaction_executing.duplicate_instruction,
      bank->transaction_executing.insufficient_funds_for_rent,
      bank->transaction_executing.max_loaded_accounts_data_size_exceeded,
      bank->transaction_executing.invalid_loaded_accounts_data_size_limit,
      bank->transaction_executing.resanitization_needed,
      bank->transaction_executing.program_execution_temporarily_restricted,
      bank->transaction_executing.unbalanced_transaction,
      bank->transaction_executing.program_cache_hit_max_limit,
      bank->transaction_executed.success,
      bank->transaction_executed.account_in_use,
      bank->transaction_executed.account_loaded_twice,
      bank->transaction_executed.account_not_found,
      bank->transaction_executed.program_account_not_found,
      bank->transaction_executed.insufficient_funds_for_fee,
      bank->transaction_executed.invalid_account_for_fee,
      bank->transaction_executed.already_processed,
      bank->transaction_executed.blockhash_not_found,
      bank->transaction_executed.instruction_error,
      bank->transaction_executed.call_chain_too_deep,
      bank->transaction_executed.missing_signature_for_fee,
      bank->transaction_executed.invalid_account_index,
      bank->transaction_executed.signature_failure,
      bank->transaction_executed.invalid_program_for_execution,
      bank->transaction_executed.sanitize_failure,
      bank->transaction_executed.cluster_maintenance,
      bank->transaction_executed.account_borrow_outstanding,
      bank->transaction_executed.would_exceed_max_block_cost_limit,
      bank->transaction_executed.unsupported_version,
      bank->transaction_executed.invalid_writable_account,
      bank->transaction_executed.would_exceed_max_account_cost_limit,
      bank->transaction_executed.would_exceed_account_data_block_limit,
      bank->transaction_executed.too_many_account_locks,
      bank->transaction_executed.address_lookup_table_not_found,
      bank->transaction_executed.invalid_address_lookup_table_owner,
      bank->transaction_executed.invalid_address_lookup_table_data,
      bank->transaction_executed.invalid_address_lookup_table_index,
      bank->transaction_executed.invalid_rent_paying_account,
      bank->transaction_executed.would_exceed_max_vote_cost_limit,
      bank->transaction_executed.would_exceed_account_data_total_limit,
      bank->transaction_executed.duplicate_instruction,
      bank->transaction_executed.insufficient_funds_for_rent,
      bank->transaction_executed.max_loaded_accounts_data_size_exceeded,
      bank->transaction_executed.invalid_loaded_accounts_data_size_limit,
      bank->transaction_executed.resanitization_needed,
      bank->transaction_executed.program_execution_temporarily_restricted,
      bank->transaction_executed.unbalanced_transaction,
      bank->transaction_executed.program_cache_hit_max_limit,
  ...
  if( FD_UNLIKELY( event->shred_off+event->shred_len*sizeof(fd_event_metrics_shred_t)>event_len ) ) return FD_EVENT_FORMAT_INVALID;
  for( ulong i=0UL; i<event->shred_len; i++ ) {
    fd_event_metrics_shred_t const * shred = ((fd_event_metrics_shred_t const *)(((char*)event)+event->shred_off))+i;
  ...
  if( FD_UNLIKELY( event->store_off+event->store_len*sizeof(fd_event_metrics_store_t)>event_len ) ) return FD_EVENT_FORMAT_INVALID;
  for( ulong i=0UL; i<event->store_len; i++ ) {
    fd_event_metrics_store_t const * store = ((fd_event_metrics_store_t const *)(((char*)event)+event->store_off))+i;
...
fd_event_format( fd_event_common_t const * common,
  ...
  switch( event_type ) {
    case FD_EVENT_GENERAL_BOOT:
      ...
      break;
    case FD_EVENT_METRICS:
      printed2 = format_metrics( &event->metrics, event_len, buffer+off, buffer_len-off );
      break;
    default:
