...
test_bundle_env_create( test_bundle_env_t * env,
  ...
  *env->stem = (fd_stem_context_t) {
  ...
  state->grpc_client_mem = fd_wksp_alloc_laddr( wksp, fd_grpc_client_align(), fd_grpc_client_footprint( state->grpc_buf_max, state->grpc_buf_max ), 1UL );
  FD_TEST( state->grpc_client_mem );
  state->grpc_client     = fd_grpc_client_new( state->grpc_client_mem, &fd_bundle_client_grpc_callbacks, state->grpc_metrics, state, state->grpc_buf_max, state->grpc_buf_max, 1UL );
