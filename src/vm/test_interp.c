...
test_program_success( char *                test_case_name,
  ...
  if (expected_result != ctx.register_file[0]) {
    ...
    FD_LOG_WARNING(( "x: %lu 0x%x", ctx.program_counter, FD_SBPF_OP_JLE_IMM ));
  }
...
main( int     argc,
  ...
  TEST_PROGRAM_SUCCESS("lddw", 0x1122334455667788, 3,
  ...
  TEST_PROGRAM_SUCCESS("stdw-heap", 0x11223344, 5,
    FD_SBPF_INSTR(FD_SBPF_OP_LDDW,       FD_SBPF_R1,  0,      0, 0x0),
    FD_SBPF_INSTR(FD_SBPF_OP_ADDL_IMM,  0,      0,      0, 0x3),
  ...
  TEST_PROGRAM_SUCCESS("stxdw-heap", 0x1122334455667788, 7,
    FD_SBPF_INSTR(FD_SBPF_OP_LDDW,       FD_SBPF_R1,  0,      0, 0x0),
    FD_SBPF_INSTR(FD_SBPF_OP_ADDL_IMM,  0,      0,      0, 0x3),
