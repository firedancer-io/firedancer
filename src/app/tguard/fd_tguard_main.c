...
static fd_cnc_t * fd_tguard_main_cnc = NULL;

static void
fd_tguard_sigaction( int         sig,
                    siginfo_t * info,
                    void *      context ) {
  (void)info;
  (void)context;
  ...
  if( FD_UNLIKELY( fd_cnc_open( fd_tguard_main_cnc ) ) ) { /* logs details */
  ...
  fd_cnc_signal( fd_tguard_main_cnc, FD_CNC_SIGNAL_HALT );
  fd_cnc_close ( fd_tguard_main_cnc );
}

static void
fd_tguard_signal_trap( int sig ) {
  struct sigaction act[1];
  act->sa_sigaction = fd_tguard_sigaction;
...
main( int     argc,
  ...
  do {
    ...
    FD_LOG_NOTICE(( "joining %s.tqos.cnc", cfg_path ));
    ...
    FD_LOG_NOTICE(( "joining %s.tmon.cnc", cfg_path ));
  ...
  for( ulong tile_idx=1UL; tile_idx<tile_cnt; tile_idx++ ) {
    ...
    fd_tile_task_t task; /* TODO, replace with array and filled along each tile_name[] */
    switch( tile_idx ) {
    case 0UL: task = main;                              break;
    case 1UL: task = fd_tguard_tqos_task;               break;
    case 2UL: task = fd_tguard_tmon_task;               break;
    default: FD_LOG_ERR(( "invalid tile_idx %lu", tile_idx ));
    }
    ...
    if( FD_UNLIKELY( fd_cnc_wait( tile_cnc[ tile_idx ], FD_CNC_SIGNAL_BOOT, (long)5e9, NULL )!=FD_CNC_SIGNAL_RUN ) )
      FD_LOG_ERR(( "tile %lu failed to boot in a timely fashion", tile_idx ));
  ...
  FD_VOLATILE( cnc_diag[ FD_TGUARD_CNC_DIAG_HA_FILT_CNT ] ) = 0UL;
  FD_VOLATILE( cnc_diag[ FD_TGUARD_CNC_DIAG_HA_FILT_SZ  ] ) = 0UL;
  FD_VOLATILE( cnc_diag[ FD_TGUARD_CNC_DIAG_SV_FILT_CNT ] ) = 0UL;
  FD_VOLATILE( cnc_diag[ FD_TGUARD_CNC_DIAG_SV_FILT_SZ  ] ) = 0UL;

  /* Configure normal kill and ctrl-c to do a clean shutdown */

  FD_VOLATILE( fd_tguard_main_cnc ) = cnc;
  fd_tguard_signal_trap( SIGTERM );
  fd_tguard_signal_trap( SIGINT  );
  ...
  for( ulong tile_idx=tile_cnt-1UL; tile_idx; tile_idx-- ) {
    FD_LOG_NOTICE(( "halting tile %s", tile_name[ tile_idx ] ));
