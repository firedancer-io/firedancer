...
runtime_replay( fd_ledger_args_t * ledger_args ) {
    ...
    if( ledger_args->on_demand_block_ingest ) {
      if( fd_blockstore_block_query( blockstore, slot ) == NULL && slot_meta.slot == slot ) {
        int err = fd_rocksdb_import_block_blockstore( &rocks_db, &slot_meta, blockstore,
                                                      ledger_args->copy_txn_status, slot == ( ledger_args->trash_hash ) ? 
                                                                                              trash_hash_buf : NULL );
        if( FD_UNLIKELY( err ) ) {
  ...
  if( tpool_scr_mem ) {
    fd_valloc_free( ledger_args->slot_ctx->valloc, tpool_scr_mem );
  }
...
/***************************** Helpers ****************************************/
fd_valloc_t
allocator_setup( fd_wksp_t * wksp, char const * allocator ) {
...
init_scratch( fd_wksp_t * wksp ) {
  ...
  ulong  sdepth = 2048UL;     /* 2048 1MiB frames */
...
replay( fd_ledger_args_t * args ) {
  ...
  */
  ...
  if( !rec_cnt ) { /* If funk is empty set up epoch and slot ctxs */
    /* Load in snapshot(s) */
    if( args->snapshot ) {
...
prune( fd_ledger_args_t * args ) {
  ...
  /* Handle case where pruning from a specific start slot */
  if( args->start_slot != 0 ) {
  ...
  /* Setup slot_ctx and epoch ctx for the unpruned workspace */
  fd_valloc_t valloc = allocator_setup( unpruned_wksp, args->allocator );
  ...
  fd_exec_slot_ctx_t * slot_ctx_unpruned = fd_exec_slot_ctx_join( fd_exec_slot_ctx_new( slot_ctx_mem_unpruned, valloc ) );
  slot_ctx_unpruned->epoch_ctx = epoch_ctx_unpruned;
  slot_ctx_unpruned->valloc = valloc;
  ...
  args->epoch_ctx = epoch_ctx_unpruned;
  args->slot_ctx = slot_ctx_unpruned;
  ...
  /* Setup epoch and slot ctx for the pruned wksp */
  ...
  if( args->snapshot ) {
    fd_snapshot_load( args->snapshot, args->slot_ctx, args->verify_acc_hash,
                      args->check_acc_hash, FD_SNAPSHOT_TYPE_FULL );
    FD_LOG_NOTICE(( "imported %lu records from snapshot",
                    fd_funk_rec_cnt( fd_funk_rec_map( unpruned_funk, fd_funk_wksp( unpruned_funk ) ) ) ));
  }

  if( args->incremental ) {
    fd_snapshot_load( args->incremental, args->slot_ctx, args->verify_acc_hash,
  ...
  if( !args->on_demand_block_ingest ) {
    ...
    if( args->end_slot >= slot_ctx_unpruned->slot_bank.slot + args->slot_history_max ) {
      args->end_slot = slot_ctx->slot_bank.slot + args->slot_history_max - 1UL;
    }

    ingest_rocksdb( args->alloc, args->rocksdb_list[ 0UL ], args->start_slot, 
                    args->end_slot, unpruned_blockstore, 0, args->trash_hash );
...
initial_setup( int argc, char ** argv, fd_ledger_args_t * args ) {
  if( FD_UNLIKELY( argc==1 ) ) {
    FD_LOG_ERR(( "no arguments provided" ));
  }
...
int main( int argc, char ** argv ) {
  ...
  if( args.cmd == NULL ) {
    ...
    /* Replay ledgers offline taking in either a wksp checkpoint or a snapshot */
    return replay( &args );
  } else if( strcmp( args.cmd, "ingest" ) == 0 ) {
    /* Ingest is used to turn a rocksdb/snapshot into a wksp checkpoint */
    ingest( &args );
  } else if( strcmp( args.cmd, "minify" ) == 0 ) {
    /* Minify is used to make rocksdb smaller wrt a specified block range */
    minify( &args );
  } else if( strcmp( args.cmd, "prune" ) == 0 ) {
    /* Prune is used to prune down funk for offline ledgers */
    prune( &args );
