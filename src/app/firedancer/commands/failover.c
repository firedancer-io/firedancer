...
static char const * const READINESS_NAMES[] = {
  ...
  "active is unhealthy", "standby is unhealthy", "standby is behind on replication"
};
  ...
  switch( result ) {
    case FD_ADMINCTL_RESULT_SUCCESS: {
      ...
      char const * health = resp.readiness_reason<READINESS_NAME_CNT
        ...
        : "unknown";
      FD_LOG_STDOUT(( "%-22s %s\n", "pair health:", health ));
      FD_LOG_STDOUT(( "%-22s %s\n", "handoff ready:", "decided by the Phase 1 controller" ));
...
action_t fd_action_failover = {
  ...
  .detail         = "`failover status` prints the pair channel's role, link state, peer view,\n"
                    "replication lag, measured round trip time, frame counters, and TLS admission\n"
                    "and handshake counters.\n"
                    "\n"
