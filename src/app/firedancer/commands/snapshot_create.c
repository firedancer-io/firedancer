...
wait_snapshot_create( fd_topo_tile_t const *            snapmk_tile,
  ...
  for(;;) {
    ...
    if( FD_LIKELY( metrics_rebased ) ) {
      snapshot_create_metric_sample_t sample = sample_snapshot_create_metrics( snapmk_metrics, snapzp_metrics, snapzp_tile_cnt );
    ...
    if( FD_UNLIKELY( now>=next_log ) ) {
      if( FD_LIKELY( metrics_rebased ) ) {
        log_snapshot_create_progress( snapmk_metrics, snaprd_metrics, snapzp_metrics, snapzp_tile_cnt, &rate_estimator, now );
      }
