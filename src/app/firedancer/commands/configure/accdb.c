...
init( config_t const * config ) {
  ...
  int accounts_fd = open( config->paths.accounts, O_RDWR|O_CREAT|O_CLOEXEC, S_IRUSR|S_IWUSR );
  if( FD_UNLIKELY( accounts_fd<0 ) ) {
  ...
  if( FD_UNLIKELY( fchown( accounts_fd, config->uid, config->gid )<0 ) ) {
