...
render_status( wait_check_t const * health,
  ...
  /* Hide cursor, disable line wrap, reposition if not first frame. */

  PRINT( "\033[?25l\033[?7l" );
...
wait_for_safe_window( fd_wait_info_box_t const * wait_info_box,
  ...
  /* config must match the running validator's config. */
  ...
  /* With no full interval the replay tile never schedules a full, so
     requiring one would block forever. */
  int full_configured    = config->firedancer.snapshots.full_snapshot_interval_blocks>0UL;

  /* Record the current incremental slot so we require a fresh one. */

  ulong snap_incr_baseline = 0UL;
  if( FD_LIKELY( incremental_configured ) ) {
    fd_wait_info_t init_info;
    while( FD_LIKELY( g_running ) && !fd_wait_info_try_read( &init_info, wait_info_box ) ) {
      struct pollfd pfd = { .fd = pidfd, .events = POLLIN };
      struct timespec zero_ts = { .tv_sec = 0, .tv_nsec = 0 };
      int pret = (int)ppoll( &pfd, 1, &zero_ts, NULL );
      if( FD_UNLIKELY( pret>0 ) )
        FD_LOG_ERR(( "validator (pid %d) exited while waiting for snapshot data", target_pid ));
      if( FD_UNLIKELY( pret<0 && errno!=EINTR ) )
        FD_LOG_ERR(( "ppoll(pidfd) failed (%i-%s)", errno, fd_io_strerror( errno ) ));
      fd_log_sleep( 1000L * 1000L ); /* 1 ms; do not spin on the seqlock */
    }
    if( FD_UNLIKELY( !g_running ) ) {
      restore_cursor();
      FD_LOG_WARNING(( "interrupted before finding a safe window" ));
      _exit( 1 );
    }
    snap_incr_baseline = init_info.snap_finished_incr;
  }
  ...
  while( FD_LIKELY( g_running ) ) {
    ...
    if( FD_UNLIKELY( !fd_wait_info_try_read( &info, wait_info_box ) ) ) {
      FD_SPIN_PAUSE();
      fd_log_sleep( 1L+(long)fd_rng_ulong_roll( rng, 1000000UL ) ); /* 1ns-1ms jitter to break synchronization */
    ...
    /* 2. Leader-slot idle gap check (intervening slots between
       ...
       reset_slot may lag the tip by a few slots, overstating the gap,
       so the real leader slot can be slightly nearer than reported. */
    ...
    /* When --min-idle-seconds is used, convert the time threshold to
    ...
    if( FD_UNLIKELY( effective_min_idle_slots && (reset_slot==0UL || epoch_end_slot==0UL) ) ) {
      ...
      leader.result = CHECK_FAIL;
      fd_cstr_printf_check( leader.desc, sizeof(leader.desc), NULL, "waiting for data" );
    } else if( FD_UNLIKELY( effective_min_idle_slots && epoch_end_slot<reset_slot ) ) {
    ...
    } else if( FD_UNLIKELY( !info.next_leader_known ) ) {
      /* Cannot see the schedule; do not pass the gap check on silence. */
      leader.result = CHECK_FAIL;
      fd_cstr_printf_check( leader.desc, sizeof(leader.desc), NULL, "waiting for leader schedule" );
...
post_leader_check:;
    ...
    if( FD_UNLIKELY( skip_snapshot || !snapmk_present ) ) {
      ...
      } else if( FD_UNLIKELY( full_configured && !finished_full ) ) {
        snap.result = CHECK_FAIL;
        fd_cstr_printf_check( snap.desc, sizeof(snap.desc), NULL, "waiting for first full snapshot" );
      } else if( FD_UNLIKELY( incremental_configured && (finished_incr<=finished_full || finished_incr<=snap_incr_baseline) ) ) {
        snap.result = CHECK_FAIL;
    ...
    if( FD_UNLIKELY( max_delinquent_stake_pct>=100UL ) ) {
      delinquent.result = CHECK_SKIP;
      fd_cstr_printf_check( delinquent.desc, sizeof(delinquent.desc), NULL, "skipped" );
    } else if( FD_UNLIKELY( !info.delinquent_known ) ) {
      /* Block rather than skip; covers the Alpenglow warmup window. */
      ...
      /* d > p*(T/100) implies d*100 > p*T.  floor(T/100) makes the
         threshold at most 99 lamports stricter (safe direction). */

      ulong t100 = total_lamports / 100UL;

      if( FD_UNLIKELY( !t100 ) ) {
        delinquent.result = CHECK_FAIL;
        fd_cstr_printf_check( delinquent.desc, sizeof(delinquent.desc), NULL, "waiting for stake data" );
      } else {
        /* The shown percentage is rounded up, hence <=, which keeps it
           within max exactly when the check passes.  Comparison is exact. */
        int   over = delinquent_lamports > max_delinquent_stake_pct * t100;
        ulong pct  = (delinquent_lamports + t100 - 1UL) / t100;
        pct = fd_ulong_min( pct, 100UL );
        delinquent.result = over ? CHECK_FAIL : CHECK_PASS;
        fd_cstr_printf_check( delinquent.desc, sizeof(delinquent.desc), NULL, "%s%lu%% delinquent (max %lu%%)",
                              pct ? "\xe2\x89\xa4" : "", pct, max_delinquent_stake_pct );
      }
    }
    ...
    if( FD_UNLIKELY( all_pass ) ) {
      ...
      _exit( 0 ); /* bypass atexit handlers, avoiding sync() (not in seccomp policy) */
    }
  ...
  restore_cursor();
  FD_LOG_WARNING(( "interrupted before finding a safe window" ));
  _exit( 1 ); /* bypass atexit handlers, avoiding sync() (not in seccomp policy) */
}
...
wait_cmd_fn( args_t *   args,
  ...
  if( FD_LIKELY( pidfd!=STDIN_FILENO ) ) close( STDIN_FILENO );
  ...
  /* Consensus mode decides where delinquent stake comes from. */
  char const * consensus = fd_topo_find_tile( &config->topo, "votor", 0UL )!=ULONG_MAX ? "alpenglow" : "tower";

  if( FD_UNLIKELY( min_idle_time_ns ) ) {
    ulong min_idle_time_s = min_idle_time_ns / 1000000000UL;
    FD_LOG_NOTICE(( "waiting for a safe window "
                    "(consensus=%s, min-idle-seconds=%lus, max-delinquent-stake=%lu%%, skip-health=%s, skip-snapshot=%s)",
                    consensus,
                    min_idle_time_s,
    ...
    FD_LOG_NOTICE(( "waiting for a safe window "
                    "(consensus=%s, min-idle-slots=%lu, max-delinquent-stake=%lu%%, skip-health=%s, skip-snapshot=%s)",
                    consensus,
                    min_idle_slots,
                    max_delinquent_stake_pct,
                    skip_health ? "yes" : "no",
...
wait_cmd_args( int *    pargc,
  ...
  ulong min_idle_time_s    = fd_env_strip_cmdline_ulong( pargc, pargv, "--min-idle-seconds",  NULL, ULONG_MAX );

  if( FD_UNLIKELY( min_idle_slots_raw!=ULONG_MAX && min_idle_time_s!=ULONG_MAX ) ) {
    FD_LOG_ERR(( "--min-idle-slots and --min-idle-seconds are mutually exclusive" ));
  }

  if( FD_UNLIKELY( min_idle_time_s!=ULONG_MAX ) ) {
    /* Convert seconds to nanoseconds. */
    if( FD_UNLIKELY( min_idle_time_s > ULONG_MAX/1000000000UL ) )
      FD_LOG_ERR(( "--min-idle-seconds value too large" ));
    if( FD_UNLIKELY( !min_idle_time_s ) )
      FD_LOG_NOTICE(( "--min-idle-seconds 0 disables the leader gap and epoch boundary checks (still blocks during active block production)" ));
  ...
  if( FD_UNLIKELY( max_delinquent>=100UL ) )
    FD_LOG_NOTICE(( "--max-delinquent-stake %lu disables the delinquent stake check", max_delinquent ));
  ...
  for( int i = 0; i < *pargc; i++ ) {
    if( !strcmp( (*pargv)[ i ], "--name" ) ) { has_name_flag = 1; break; }
  }
  char const * name = fd_env_strip_cmdline_cstr( pargc, pargv, "--name", NULL, NULL );
  if( FD_UNLIKELY( !name && has_name_flag ) )
    FD_LOG_ERR(( "--name requires a value" ));
  if( FD_UNLIKELY( name ) ) {
...
wait_args_help( fd_action_help_t * help ) {
  fd_action_help_arg( help, "--min-idle-slots",     "<slots>",     "Minimum number of idle slots required before the next leader\n"
                                                                   "slot.  Default: 1500.  0 disables the gap check" );
  fd_action_help_arg( help, "--min-idle-seconds",   "<seconds>",   "Minimum idle time in seconds before the next leader slot.\n"
...
action_t fd_action_wait = {
  ...
  .detail         = "Attaches to a running validator and polls every second until\n"
                    ...
                    "  - No snapshot is currently being written. When periodic\n"
                    "    snapshot production is enabled, a full snapshot exists on\n"
                    "    disk and, if incremental snapshots are configured, a fresh\n"
                    "    incremental has been produced since the command started\n"
