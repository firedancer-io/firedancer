...
wait_for_safe_window( config_t * config,
  ...
  /* Stale metrics: if the replay tile's seqno was >0 but stops
     changing for stale_threshold_ns, the tile is dead. */

  ulong last_seen_seqno   = 0UL;
  long  seqno_stale_since = 0L;
  long const stale_threshold_ns = 10L * 1000L * 1000L * 1000L;  /* 10 s */
  ...
  while( FD_LIKELY( g_running ) ) {
    ...
    /* 2. Leader-slot idle gap check (next_leader_slot - reset_slot).
       reset_slot may lag the tip by a few slots, slightly
       overestimating the gap.

       Seqlock: the replay tile toggles MetricsSeqno odd/even around
       gauge writes.  On x86-64 TSO, volatile loads suffice. */

    wait_check_t leader  = { .result = CHECK_FAIL };
    wait_check_t metrics = { .result = CHECK_FAIL };
    ...
    ulong slots_per_epoch     = replay_metrics[ FD_METRICS_GAUGE_REPLAY_SLOTS_PER_EPOCH_OFF ];
    ...
    /* Detect stale metrics (replay tile stopped updating). */

    if( FD_LIKELY( seqno_before==last_seen_seqno ) ) {
      if( FD_UNLIKELY( seqno_before>0UL && !seqno_stale_since ) ) {
        seqno_stale_since = fd_log_wallclock();
      }
    } else {
      last_seen_seqno   = seqno_before;
      seqno_stale_since = 0L;
    }

    if( FD_UNLIKELY( seqno_stale_since && (fd_log_wallclock()-seqno_stale_since>stale_threshold_ns) ) ) {
      restore_cursor();
      FD_LOG_ERR(( "validator metrics stopped updating for >10s (pid %d); validator is likely shutting down or dead", target_pid ));
    }
    ...
    if( FD_UNLIKELY( reset_slot==0UL || epoch_end_slot==0UL ) ) {
      ...
      fd_cstr_printf_check( metrics.desc, sizeof(metrics.desc), NULL, "ok" );
      leader.result  = CHECK_SKIP;
      /* Saturate: next_leader_slot can lag behind reset_slot
         briefly after a leader slot ends (stale metric). */
      idle_gap = fd_ulong_sat_sub( next_leader_slot, reset_slot );
    }
...
post_leader_check:;
    /* 3. Snapshot check: no snapshot currently being written, and if
       ...
       Firedancer does not require a snapshot produced after entering
       the idle window: freshness does not survive restart latency, so
       the real concerns are corruption (no in-flight write) and
       ensuring an incremental exists to avoid replaying from the full
       snapshot base. */

    wait_check_t snap = { .result = CHECK_FAIL };

    int snapshots_disabled = !config->firedancer.snapshots.full_snapshot_interval_blocks &&
                             !config->firedancer.snapshots.incremental_snapshot_interval_blocks;
    if( FD_UNLIKELY( skip_snapshot || !snapmk_tile || snapshots_disabled ) ) {
      snap.result = CHECK_SKIP;
      fd_cstr_printf_check( snap.desc, sizeof(snap.desc), NULL, "skipped" );
    } else {
      /* Not seqlock-protected: each gauge is a monotonic slot number
         whose 8-byte aligned write is atomic on x86-64 TSO. */
      ...
      if( FD_UNLIKELY( started_full>finished_full || started_incr>finished_incr ) ) {
        ...
        /* snapmk created a new full but no incremental based on it
           yet.  Restarting now would force the validator to replay
           from the full snapshot base, which is much slower.  When
           finished_full==0 the boot snapshot + existing incrementals
           provide a valid restart point. */
        snap.result = CHECK_FAIL;
    ...
    } else if( FD_UNLIKELY( metrics.result==CHECK_FAIL ) ) {
      delinquent.result = CHECK_SKIP;
      fd_cstr_printf_check( delinquent.desc, sizeof(delinquent.desc), NULL, "waiting for metrics" );
    } else {
      /* delinquent_lamports and total_lamports are read inside the
         seqlock window above. */
...
wait_cmd_perm( args_t *         args FD_PARAM_UNUSED,
  ...
  if( FD_UNLIKELY( wksp_id==ULONG_MAX ) ) FD_LOG_ERR(( "no metric_in workspace found in topology" ));
  fd_topo_wksp_t const * metric_wksp = &config->topo.workspaces[ wksp_id ];
  ulong mlock_limit = metric_wksp->page_cnt * metric_wksp->page_sz;
...
wait_cmd_fn( args_t *   args,
  ...
  int is_alpenglow    = config->firedancer.development.alpenglow;
  int skip_leader     = is_alpenglow;
  int skip_delinquent = is_alpenglow;
  if( FD_UNLIKELY( is_alpenglow ) ) {
    FD_LOG_WARNING(( "skipping leader gap and delinquent stake checks (not supported under Alpenglow)" ));
  }
  ...
  wait_for_safe_window( config, min_idle_slots, min_idle_time_ns, max_delinquent_stake_pct,
