...
fd_topo_initialize( config_t * config ) {
  ...
  if( alpenglow_enabled ) {
    ...
    /**/               fd_topob_link( topo, "votor_net",     "net_votor",     config->net.ingress_buffer_size,          FD_NET_MTU,                                    FD_VOTOR_NET_BURST );
  ...
  if( !alpenglow_enabled && config->firedancer.failover.enabled ) {
    /**/               fd_topob_tile_in (   topo, "admin",   0UL,          "metric_in", "tower_out",     0UL,          FD_TOPOB_UNRELIABLE, FD_TOPOB_POLLED );
  }
...
#define PARSE_BUNDLE_PUBKEY( _tile, f ) \
    ...
    fd_cstr_ncpy( tile->tower.failover_staked_identity_path, config->firedancer.failover.staked_identity_path, sizeof(tile->tower.failover_staked_identity_path) );
    fd_cstr_ncpy( tile->tower.failover_first_use, config->failover_first_use, sizeof(tile->tower.failover_first_use) );
