...
fd_topo_initialize( config_t * config ) {
  ...
  if( leader_enabled ) {
    ...
    FOR(execle_tile_cnt)   fd_topob_link( topo, "execle_poh",    "execle_poh",    32UL,                                     FD_EXECLE_POH_MTU,             1UL );
    ...
    /**/                   fd_topob_link( topo, "pack_execle",   "pack_execle",   256UL,                                    FD_PACK_EXECLE_MTU,            1UL );
    if( FD_LIKELY( config->tiles.pack.use_consumed_cus ) ) {
      FOR(execle_tile_cnt) fd_topob_link( topo, "execle_pack",   "execle_pack",   32UL,                                     FD_PACK_REBATE_MAX_SZ,         1UL );
    }
