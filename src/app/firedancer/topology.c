...
setup_topo_progcache( fd_topo_t *  topo,
  ...
  ulong part_max  = fd_progcache_wksp_part_max( wksp_size );
  ulong shared_sz = fd_progcache_shared_sz( wksp_size );
  ulong progcache_footprint = shared_sz ? fd_progcache_shmem_footprint( txn_max, shared_sz ) : 0UL;
  if( FD_UNLIKELY( !part_max || !progcache_footprint ) )
    FD_LOG_ERR(( "progcache wksp of %lu B is too small", wksp_size ));
