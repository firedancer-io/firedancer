...
#define MAX_RESOLVED_ADDRS 8UL
    ...
    for( ulong i=0UL; i<tile->snapct.sources.servers_cnt; i++ ) {
      ...
      if( FD_UNLIKELY( 0==num_resolved ) ) {
        ...
        if( FD_UNLIKELY( resolved_peers_cnt+(ulong)num_resolved>=FD_TOPO_SNAPSHOTS_SERVERS_MAX_RESOLVED ) ) {
          FD_LOG_WARNING(( "ignoring peer `%s` in [[snapshots.sources.http.peers]] because reached maximum supported peers %lu", config->firedancer.snapshots.sources.servers[ i ], FD_TOPO_SNAPSHOTS_SERVERS_MAX_RESOLVED ));
          continue;
        };
