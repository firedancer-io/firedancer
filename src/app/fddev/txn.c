...
send_quic_transactions( fd_quic_t *         quic,
  ...
  if( FD_UNLIKELY( conn->state != FD_QUIC_CONN_STATE_ACTIVE ) )
    FD_LOG_ERR(( "unable to connect to QUIC endpoint on port %hu, is it running? state is %d", dst_port, conn->state ));
...
txn_cmd_fn( args_t *         args,
  ...
  uint dst_ip;
  if( FD_UNLIKELY( !args->txn.dst_ip && config->development.netns.enabled ) ) {
    if( FD_UNLIKELY( !fd_cstr_to_ip4_addr( config->development.netns.interface0_addr, &dst_ip ) ) ) FD_LOG_ERR(( "invalid `interface0_addr` for netns in config file" ));
  } else if( FD_LIKELY( args->txn.dst_ip ) ) {
    if( FD_UNLIKELY( !fd_cstr_to_ip4_addr( args->txn.dst_ip, &dst_ip  ) ) ) FD_LOG_ERR(( "invalid --dst-ip" ));
  } else {
    FD_LOG_ERR(( "--dst-ip must be provided when sending transactions" ));
  }

  ushort dst_port = config->tiles.quic.listen_port;
