...
run_firedancer_threaded( config_t * config ) {
  pthread_t threads[ FD_TOPO_TILE_MAX+1UL ];
  ulong thread_cnt = fd_topo_run_single_process( config->pod, 2, config->uid, config->gid, threads );

  solana_labs_main( config );

  for( ulong i=0; i<thread_cnt; i++ ) {
...
dev_cmd_fn( args_t *         args,
  ...
  if( FD_UNLIKELY( strcmp( "", args->dev.debug_tile ) ) ) {
    ...
    strncpy( config->development.debug_tile, args->dev.debug_tile, sizeof( config->development.debug_tile ) );
    config->development.debug_tidx = 0UL;
  }
