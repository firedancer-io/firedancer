...
int main( int     argc,
  ...
  ulong        bank_cnt = fd_env_strip_cmdline_ulong( &argc, &argv, "--banks", NULL, 4UL );
  FILE * pcap_file = fopen( path, "r" );
  FD_TEST( pcap_file );

  fd_pcap_iter_t * pack_iter = fd_pcap_iter_new( pcap_file );
  FD_TEST( pack_iter );

  fd_pcap_iter_t * bank_iters[ 64UL ];
  for( ulong i=0UL; i<bank_cnt; i++ ) {
    bank_iters[ i ] = fd_pcap_iter_new( fopen( path, "r" ) );
    FD_TEST( bank_iters[ i ] );
  }
  ...
  FD_LOG_NOTICE(( "%lu txn: arrived at: %lu, result: %s, CUs estimated: %lu, CUs consumed: %lu, rewards: %lu" ));
  FD_LOG_NOTICE(( "%lu bundle success: arrived at: %lu, CUs estimated: %lu, CUs consumed: %lu, rewards: %lu+%lu" ));
  FD_LOG_NOTICE(( "%lu bundle failed: arrived at: %lu, result: %s, triggered by idx: %lu, which arrived at %lu, CUs estimated: %lu, CUs consumed: %lu, rewards: %lu+%lu" ));
