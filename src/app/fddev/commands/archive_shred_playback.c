/* The shred_playback command spawns a smaller topology for 2 purposes:
   ...
   Some tiles are not shown, such as the thread pool for replay (rtpool),
   the metric tile, and the bhole tile which is used to take the place of
   tiles that do not exist above but exist in the full topology such as
   the poh and sender tiles.
 */
...
ulong fdctl_obj_align( fd_topo_t const * topo, fd_topo_obj_t const * obj );
ulong fdctl_obj_footprint( fd_topo_t const * topo, fd_topo_obj_t const * obj );
ulong fdctl_obj_loose( fd_topo_t const * topo, fd_topo_obj_t const * obj );
...
playback_topo( config_t * config ) {
  ...
  strncpy( playback_tile->archiver.archive_path, config->tiles.archiver.archive_path, PATH_MAX );
  if( FD_UNLIKELY( 0==strlen( playback_tile->archiver.archive_path ) ) ) {
    FD_LOG_ERR(( "Archive file not found for playback" ));
  } else {
    FD_LOG_WARNING(( "Found archive file from config: %s", playback_tile->archiver.archive_path ));
  }
  ...
  replay_tile->replay.tpool_thread_count = config->tiles.replay.tpool_thread_count;
  if( FD_UNLIKELY( replay_tile->replay.tpool_thread_count == 0 || replay_tile->replay.tpool_thread_count>=FD_TILE_MAX-1 ) ) {
    FD_LOG_ERR(( "bad tpool_thread_count %lu", replay_tile->replay.tpool_thread_count ));
  }
  ...
  /**********************************************************************/
  /* Add the rtpool tiles to topo                                       */
  /**********************************************************************/
  fd_topob_wksp( topo, "rtpool" );
  #define FOR(cnt) for( ulong i=0UL; i<cnt; i++ )
  ulong replay_tpool_thread_count = config->tiles.replay.tpool_thread_count;
  FOR(replay_tpool_thread_count-1) fd_topob_tile( topo, "rtpool", "rtpool", "metric_in", static_end_idx+i,    0,        0 );
  ...
  FOR(exec_tile_cnt)               fd_topob_tile( topo, "exec",   "exec",   "metric_in", static_end_idx+replay_tpool_thread_count+i, 0,        0 );
...
void
playback_cmd_fn( args_t *   args FD_PARAM_UNUSED,
                 config_t * config ) {
  playback_topo( config );
...
void
playback_cmd_perm( args_t *         args FD_PARAM_UNUSED,
                   fd_cap_chk_t *   chk FD_PARAM_UNUSED,
                   config_t const * config FD_PARAM_UNUSED ) {}

void
playback_cmd_args( int *    pargc FD_PARAM_UNUSED,
