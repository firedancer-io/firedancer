...
gossip_topo( config_t * config ) {
  ...
  fd_topos_net_tiles( topo, 1UL, config->tiles.netlink.max_routes, config->tiles.netlink.max_neighbors, config->development.net.provider, config->tiles.net.interface, config->tiles.net.flush_timeout_micros,
  ...
  net_tile->net.gossip_listen_port = config->tiles.gossip.gossip_listen_port;
  ...
  gossip_tile->gossip.gossip_listen_port     = config->tiles.gossip.gossip_listen_port;
  ...
  gossip_tile->gossip.entrypoints_cnt        = config->tiles.gossip.peer_ports_cnt;
  ...
  for( ulong i=0UL; i<config->tiles.gossip.entrypoints_cnt; i++ ) {
    if( FD_UNLIKELY( !fd_cstr_to_ip4_addr( config->tiles.gossip.entrypoints[i], &gossip_tile->gossip.entrypoints[i] ) ) ) {
      FD_LOG_ERR(( "configuration specifies invalid gossip peer IP address `%s`", config->tiles.gossip.entrypoints[i] ));
    }
    gossip_tile->gossip.peer_ports[i] = (ushort)config->tiles.gossip.peer_ports[i];
  }
