...
/* Renders every member of the config; a config layout change breaks
   ...
   lists below. */
FD_STATIC_ASSERT( sizeof(fd_config_t)==22976480UL, update_fd_config_to_json_for_the_layout_change );
...
fd_config_to_json( fd_config_t const * config,
  ...
  jw_obj_open( &w, "failover" );
    ...
    jw_path ( &w, "pair_secret_path",         f->failover.pair_secret_path );
