...
fd_config_auto( fd_config_t * config ) {
  ...
  fd_cstr_printf( config->auto_config_log, sizeof(config->auto_config_log), NULL,
      "network auto configure level=%s, provider=%s xdp_mode=%s poll_mode=%s zero_copy=%d native_bond=%d (driver=%s kernel=%lu.%lu virtual_if=%d bonded_if=%d lacp_if=%d slaves=%u, net_tile_cnt=%u)",
