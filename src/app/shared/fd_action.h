...
struct fd_action {
  ...
  uint is_help : 1;
  uint is_immediate : 1;
  uint require_config : 1;    /* halt if the user tries to use the default config */
  uint is_local_cluster : 1;  /* If a command is one which runs a local cluster, certain information in
                                 the configuration file will be changed. */
  uint is_diagnostic : 1;     /* 1 implies action should be allowed for prod debugging */
  uint no_log_file : 1;       /* Don't create a log file (development only) */
