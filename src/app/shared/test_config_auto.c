...
reset_auto( uint net_tile_cnt ) {
  ...
  FD_TEST( fd_config_str_set( config, &config->net.xdp.xdp_mode, "auto", 4UL ) );
  FD_TEST( fd_config_str_set( config, &config->net.xdp.poll_mode, "auto", 4UL ) );
...
main( int     argc,
  ...
  FD_TEST( 0==strcmp( FD_TOPO_STR( config->net.provider ), "mlx5" ) );
  ...
  reset_auto( 1U );
  fd_auto_net( config, &info1 );
  FD_TEST( 0==strcmp( FD_TOPO_STR( config->net.xdp.xdp_mode ),  "drv"      ) );
  FD_TEST( 0==strcmp( FD_TOPO_STR( config->net.xdp.poll_mode ), "prefbusy" ) );
  FD_TEST( 0==strcmp( FD_TOPO_STR( config->net.xdp.rss_queue_mode ), "auto" ) );
  ...
  FD_TEST( 0==strcmp( FD_TOPO_STR( config->net.xdp.xdp_mode ),  "skb"     ) );
  FD_TEST( 0==strcmp( FD_TOPO_STR( config->net.xdp.poll_mode ), "softirq" ) );
  FD_TEST( 0==strcmp( FD_TOPO_STR( config->net.xdp.rss_queue_mode ), "auto" ) );
  ...
  FD_TEST( config->net.xdp.listen_gre==1 );
  FD_TEST( 0==strcmp( FD_TOPO_STR( config->net.xdp.rss_queue_mode ), "simple" ) );

  /* Explicit values are not changed by auto configuration. */

  reset_auto( 1U );
  config->net.xdp.listen_gre = 0;
  FD_TEST( fd_config_str_set( config, &config->net.xdp.rss_queue_mode, "dedicated", 9UL ) );
  fd_auto_net( config, &info_gre_mlx5 );
  FD_TEST( config->net.xdp.listen_gre==0 );
  FD_TEST( 0==strcmp( FD_TOPO_STR( config->net.xdp.rss_queue_mode ), "dedicated" ) );
  ...
  reset_auto( 1U );
  FD_TEST( fd_config_str_set( config, &config->net.provider, "socket", 6UL ) );
  fd_auto_net( config, &info1 );
  FD_TEST( 0==strcmp( FD_TOPO_STR( config->net.xdp.xdp_mode ),  "skb"     ) );
  FD_TEST( 0==strcmp( FD_TOPO_STR( config->net.xdp.poll_mode ), "softirq" ) );
  FD_TEST( 0==strcmp( FD_TOPO_STR( config->net.xdp.rss_queue_mode ), "simple" ) );
