...
fd_config_extract_pod( uchar *       pod,
  ...
  CFG_POP      ( bool,   tiles.replay.silent_runtime_logs                 );
  ...
  if( FD_UNLIKELY( config->is_firedancer ) ) {
    ...
    CFG_POP      ( bool,    tiles.feccap.enabled                          );
    CFG_POP      ( cstr,    tiles.feccap.folder_path                      );

    CFG_POP      ( bool,    tiles.simulate.enabled                       );
    CFG_POP      ( cstr,    tiles.simulate.file_path                    );
  }
