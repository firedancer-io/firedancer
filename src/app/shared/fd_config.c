...
fd_config_validatef( fd_configf_t const * config ) {
  ...
  if( FD_UNLIKELY( config->failover.enabled ) ) {
    ...
    CFG_HAS_NON_EMPTY( failover.peer_junk_pubkey );
    uchar peer_junk_pubkey[ 32 ];
    if( FD_UNLIKELY( !fd_base58_decode_32( config->failover.peer_junk_pubkey, peer_junk_pubkey ) ) )
      FD_LOG_ERR(( "failover.peer_junk_pubkey must be a base58 Ed25519 public key" ));
