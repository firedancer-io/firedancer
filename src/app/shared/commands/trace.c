...
trace_fn( args_t *   args,
  ...
  for( ulong i=0UL; i<(topo->link_cnt); i++ ) {
    ...
    ulong link_name_strlen = strlen( link->name );
    if( link_name_strlen<=6 || 0!=memcmp( link->name+link_name_strlen-6, "_trace", 7UL ) ) continue;

    fd_topo_wksp_t * link_wksp = &topo->workspaces[ topo->objs[ link->mcache_obj_id ].wksp_id ];
    fd_topo_join_workspace( topo, link_wksp, FD_SHMEM_JOIN_MODE_READ_ONLY, FD_TOPO_CORE_DUMP_LEVEL_REGULAR );
    fd_topo_workspace_fill( topo, link_wksp );
    if( FD_UNLIKELY( !link->mcache || !link->dcache ) ) continue;
    ...
    FD_LOG_NOTICE(( "monitoring %s:%lu", link->name, link->kind_id ));
    fxt_in_t * in = &collect->in[ collect->in_cnt++ ];
    in->mcache = link->mcache;
    in->base   = link->dcache;
    in->seq    = 0UL;
