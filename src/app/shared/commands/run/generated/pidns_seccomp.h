...
static const uint sock_filter_policy_pidns_instr_cnt = 34;

static void populate_sock_filter_policy_pidns( ulong out_cnt, struct sock_filter out[ static 34 ], uint logfile_fd ) {
  FD_TEST( out_cnt >= 34 );
  struct sock_filter filter[34] = {
    ...
    /* check poll */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_poll, /* check_poll */ 14, 0 ),
    /* check wait4 */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_wait4, /* check_wait4 */ 17, 0 ),
    /* allow exit_group */
...
//  check_poll:
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, FD_SECCOMP_ARG_LO((int)-1), /* poll_ALLOW */ 1, /* poll_KILL */ 0 ),
//  poll_KILL:
    BPF_STMT( BPF_RET | BPF_K, SECCOMP_RET_KILL_PROCESS ),
//  poll_ALLOW:
    BPF_STMT( BPF_RET | BPF_K, SECCOMP_RET_ALLOW ),
...
#endif /* HEADER_fd_src_app_shared_commands_run_generated_pidns_seccomp_h */
