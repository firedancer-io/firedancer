...
tile_main( void * _args ) {
  ...
  if( strcmp( args->tile->name, "back" ) == 0 ) {
    fd_topo_run_tile( &args->config->topo, args->tile, 0, 0, args->config->development.core_dump, args->config->uid, args->config->gid, args->pipefd, wait, debug, &run_tile );
  } else {
