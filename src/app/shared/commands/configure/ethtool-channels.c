...
init_device( char const *        device,
  ...
  if( FD_UNLIKELY( ret != 0 ) ) {
    if( FD_LIKELY( ret == EBUSY ) )
      FD_LOG_ERR(( "error configuring network device, failed to set number of channels. "
                   ...
                   "blacklisting the irdma kernel module.", device ));
    else
      FD_LOG_ERR(( "error configuring network device, failed to set number of channels" ));
  }
  ...
  if( dedicated_mode ) {
    ...
    if( FD_UNLIKELY( 0!=fd_ethtool_ioctl_rxfh_set_suffix( &ioc, 1 ) ) )
      FD_LOG_ERR(( "error configuring network device, failed to isolate queue zero. Try `net.xdp.rss_queue_mode=\"simple\"`" ));

    if( FD_UNLIKELY( 0!=fd_ethtool_ioctl_feature_set( &ioc, FD_ETHTOOL_FEATURE_NTUPLE, 1 ) ) )
      FD_LOG_ERR(( "error configuring network device, failed to enable ntuple feature. Try `net.xdp.rss_queue_mode=\"simple\"`" ));
    ...
    if( FD_UNLIKELY( error ) )
      FD_LOG_ERR(( "error configuring network device, failed to install ntuple rules. Try `net.xdp.rss_queue_mode=\"simple\"`" ));
  }
...
fini_device( char const * device ) {
  ...
  uint rxfh_table_orig[ FD_ETHTOOL_MAX_RXFH_TABLE_SIZE ] = { 0 };
  uint rxfh_table_orig_size;
  error |= (0!=fd_ethtool_ioctl_rxfh_get_table( &ioc, rxfh_table_orig, &rxfh_table_orig_size ));
  int ntuple_rules_empty_orig;
  error |= (0!=fd_ethtool_ioctl_ntuple_validate_udp_dport( &ioc, NULL, 0, 0, &ntuple_rules_empty_orig ));
  if( FD_UNLIKELY( error ) )
    FD_LOG_ERR(( "error configuring network device, unable to determine initial state" ));
  ...
  if( FD_UNLIKELY( error ) )
    FD_LOG_ERR(( "error configuring network device, unable to set to default state" ));

  fd_ethtool_ioctl_channels_t channels_new;
  error |= (0!=fd_ethtool_ioctl_channels_get_num( &ioc, &channels_new ));
  uint rxfh_table_new[ FD_ETHTOOL_MAX_RXFH_TABLE_SIZE ] = { 0 };
  uint rxfh_table_new_size;
  error |= (0!=fd_ethtool_ioctl_rxfh_get_table( &ioc, rxfh_table_new, &rxfh_table_new_size ));
  int ntuple_rules_empty_new;
  error |= (0!=fd_ethtool_ioctl_ntuple_validate_udp_dport( &ioc, NULL, 0, 0, &ntuple_rules_empty_new ));
  if( FD_UNLIKELY( error ) )
    FD_LOG_ERR(( "error configuring network device, unable to determine final state" ));

  fd_ethtool_ioctl_fini( &ioc );

  int modified = (0!=memcmp( &channels_orig, &channels_new, sizeof(fd_ethtool_ioctl_channels_t) )) ||
                 (rxfh_table_orig_size != rxfh_table_new_size) ||
                 (0!=memcmp( rxfh_table_orig, rxfh_table_new, rxfh_table_orig_size * sizeof(uint) )) ||
