/* This stage disables various ethtool features on the main and
   loopback interfaces that are known to be incompatible with AF_XDP.
     ...
     packet loss for GRE packets on the TX path.  */
...
init_device( char const * device ) {
  ...
  /* turn off tx-gre-segmentation and tx-gre-csum-segmentation, which
     have been known to cause issues with packet loss due to interactions
     between AF_XDP and GRE. */
