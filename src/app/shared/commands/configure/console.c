/* The console stage quiets periodic kernel console rendering work
   ...
   work that runs on the CPU that queued it:
     ...
     - Any printk at or below the console loglevel renders to the
       ...
       tile's own syscall (e.g. a ratelimited netdev TX warning)
       therefore schedules display work onto that tile's core.  On a
       serial console it is worse: the UART write runs synchronously
       in the printk caller's context.
   ...
   console; WARNING-class ratelimited spam no longer reaches it.  All
   messages still land in the kernel ring buffer (dmesg/journal)
   regardless of the console loglevel.  The console itself is never
   unbound: it must remain able to show a panic to a crash cart or
   BMC KVM.
...
/* read_ulong_field reads the idx-th whitespace-separated ulong from
   path.  Fails fast on IO or parse errors; returns ULONG_MAX only if
...
check( config_t const * config,
  ...
  if( FD_LIKELY( has_fbcon() ) ) {
    ...
    if( FD_UNLIKELY( blink!=0UL && blink!=ULONG_MAX ) )
      NOT_CONFIGURED( "framebuffer console cursor blink is enabled; its timer preempts whichever CPU it was armed on" );
  }

  ulong level = read_ulong_field( PRINTK_PATH, 0UL );
  if( FD_UNLIKELY( level!=ULONG_MAX && level>CONSOLE_LOGLEVEL_QUIET ) )
    NOT_CONFIGURED( "console loglevel is %lu; kernel warnings render console work on the printing CPU (want <= %lu)", level, CONSOLE_LOGLEVEL_QUIET );
