...
fd_topo_kind_str_to_topo_config_fn( char const * topo_kind_str ) {
  ...
  if( strncmp( topo_kind_str, "firedancer",    FD_TOPO_KIND_CSTR_LEN_MAX )==0 ) { return &fd_topo_firedancer; }
  if( strncmp( topo_kind_str, "wd-f1",    FD_TOPO_KIND_CSTR_LEN_MAX )==0 ) { return &fd_topo_wd_f1; }
  else {
