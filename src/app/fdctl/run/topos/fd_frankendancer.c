...
fd_topo_initialize( config_t * config ) {
  ...
  for( ulong i=0UL; i<topo->tile_cnt; i++ ) {
    ...
    if( FD_UNLIKELY( !strcmp( tile->name, "net" ) ) ) {
      ...
      /* multihome support */
      ulong multi_cnt = tile->net.multihome.cnt = config->tiles.net.multihome.src_ip_addr_cnt;
      for( ulong j = 0; j < multi_cnt; ++j ) {
        tile->net.multihome.src_ip_addr[j] = config->tiles.net.multihome.src_ip4_addr[j];
      }
