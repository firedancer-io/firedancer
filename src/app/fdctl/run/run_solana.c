...
solana_labs_main( void * args ) {
  ...
  if( FD_UNLIKELY( config->development.debug_tile ) ) {
    if( FD_UNLIKELY( !strcmp( config->development.debug_tile, "solana" ) ) ) {
      FD_LOG_WARNING(( "waiting for debugger to attach to tile solana-labs pid:%d", fd_sandbox_getpid() ));
  ...
  pthread_t ignore[ FD_TOPO_TILE_MAX ];
  fd_topo_run_single_process( config->pod, 1, config->uid, config->gid, ignore );
