...
after_credit( fd_nettx_ctx_t *    ctx,
  ...
  /* If we didn't get a frag between this callback and the last
     after_credit call then the tile is idle, and we can flush
     any pending frames. */

  int handled_frag = ctx->handled_frag;
  ctx->handled_frag = 0;
  if( FD_LIKELY( (handled_frag || !ctx->frame_cnt) && ctx->frame_cnt<1024UL ) ) return;
...
privileged_init( fd_topo_t *      topo,
  ...
  ctx->send_fd = socket( AF_INET, SOCK_RAW, IPPROTO_UDP );
