/* Eqvoc tile signs and sends transactions to the current leader. Currently
...
static void
send_packet( fd_eqvoc_tile_ctx_t * ctx,
             uint                   dst_ip_addr,
             ushort                 dst_port,
             uchar const *          payload,
             ulong                  payload_sz,
...
static int
get_current_leader_tpu_vote_contact( fd_eqvoc_tile_ctx_t *      ctx,
...
static inline void
handle_new_cluster_gossip_info( fd_eqvoc_tile_ctx_t * ctx,
                                 uchar const *         buf,
                                 ulong                 buf_sz ) {
...
static inline void
finalize_new_cluster_gossip_info( fd_eqvoc_tile_ctx_t * ctx ) {
...
during_frag( void * _ctx,
  ...
  if( FD_UNLIKELY( in_idx==ctx->gossip_in_idx ) ) {
    if( FD_UNLIKELY( chunk<ctx->gossip_in_chunk0 || chunk>ctx->gossip_in_wmark ) ) {
      FD_LOG_ERR(( "chunk %lu %lu corrupt, not in range [%lu,%lu]", chunk, sz, ctx->gossip_in_chunk0, ctx->gossip_in_wmark ));
    }

    uchar const * dcache_entry = fd_chunk_to_laddr_const( ctx->gossip_in_mem, chunk );
    handle_new_cluster_gossip_info( ctx, dcache_entry, sz );
  }
...
after_frag( void *             _ctx,
  ...
  if( FD_UNLIKELY( in_idx==ctx->gossip_in_idx ) ) {
    finalize_new_cluster_gossip_info( ctx );
    return;
  }
...
privileged_init( fd_topo_t *      topo  FD_PARAM_UNUSED,
  ...
  ctx->identity_key[ 0 ] = *(fd_pubkey_t const *)fd_type_pun_const( fd_keyload_load( tile->eqvoc.identity_key_path, /* pubkey only: */ 1 ) );
}
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->net_id = (ushort)0;
  fd_memcpy( ctx->src_mac_addr, tile->eqvoc.src_mac_addr, 6 );

  ctx->tpu_serve_addr.addr = tile->eqvoc.ip_addr;
  ctx->tpu_serve_addr.port = fd_ushort_bswap( tile->eqvoc.tpu_listen_port );
  ...
  ctx->gossip_in_idx = fd_topo_find_tile_in_link( topo, tile, "gossip_voter", 0 );
