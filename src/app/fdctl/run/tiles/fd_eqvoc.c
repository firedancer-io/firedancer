...
after_frag( fd_eqvoc_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( in_idx == ctx->contact_in_idx ) ) {
    ...
    fd_eqvoc_proof_t const * proof = fd_eqvoc_proof_query( ctx->eqvoc, ctx->duplicate_shred.slot, ctx->duplicate_shred.from );

    fd_slot_hash_t key = { .slot = ctx->duplicate_shred.slot };
    memcpy( &key.hash, &ctx->duplicate_shred.from, 32UL );

    // fd_pubkey_t const * leader = fd_epoch_leaders_get( ctx->eqvoc->leaders, ctx->duplicate_shred.slot );

    if( FD_UNLIKELY( fd_eqvoc_proof_complete( proof ) ) ) {
      FD_LOG_NOTICE(( "[eqvoc proof] slot: %lu from: %s", key.slot, FD_BASE58_ENC_32_ALLOCA( &key.hash ) ));
      FD_LOG_HEXDUMP_NOTICE(("shreds", proof->shreds, FD_SHRED_MAX_SZ * 2 ));
      FD_LOG_NOTICE(( "fd_eqvoc_test: %d?", fd_eqvoc_proof_verify( proof ) ));
      __asm__("int $3");
    }
