...
after_frag( fd_eqvoc_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( in_idx == ctx->contact_in_idx ) ) {
    ...
    fd_eqvoc_proof_t * proof = fd_eqvoc_proof_insert( ctx->eqvoc, &ctx->duplicate_shred );

    fd_slot_hash_t key = { .slot = ctx->duplicate_shred.slot };
    memcpy( &key.hash, &ctx->duplicate_shred.from, 32UL );

    if ( FD_UNLIKELY( proof->bit_vec == 7 ) ) {
      FD_LOG_NOTICE(( "got all chunks for slot: %lu from: %s", key.slot, FD_BASE58_ENC_32_ALLOCA( &key.hash ) ));
      FD_LOG_NOTICE(( "fd_eqvoc_test: %d?", fd_eqvoc_test( ctx->eqvoc, &proof->shred1, &proof->shred2 ) ));
    }
...
unprivileged_init( fd_topo_t *      topo,
  ...
  void * eqvoc_mem          = FD_SCRATCH_ALLOC_APPEND( l, fd_eqvoc_align(), fd_eqvoc_footprint( FD_EQVOC_MAX ) );
  ...
  ctx->eqvoc    = fd_eqvoc_join( fd_eqvoc_new( eqvoc_mem, FD_EQVOC_MAX, 0 ) );
