...
#ifndef HEADER_fd_src_app_fdctl_run_tiles_generated_nettx_seccomp_h
#define HEADER_fd_src_app_fdctl_run_tiles_generated_nettx_seccomp_h
...
#endif
static const unsigned int sock_filter_policy_nettx_instr_cnt = 19;

static void populate_sock_filter_policy_nettx( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int send_fd) {
  FD_TEST( out_cnt >= 19 );
...
//  check_sendmmsg:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, send_fd, /* lbl_2 */ 0, /* RET_KILL_PROCESS */ 2 ),
//  lbl_2:
