...
#ifndef HEADER_fd_src_app_fdctl_run_tiles_generated_netrx_seccomp_h
#define HEADER_fd_src_app_fdctl_run_tiles_generated_netrx_seccomp_h
...
#endif
static const unsigned int sock_filter_policy_netrx_instr_cnt = 28;

static void populate_sock_filter_policy_netrx( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int xsk_fd, unsigned int lo_xsk_fd) {
  FD_TEST( out_cnt >= 28 );
  struct sock_filter filter[28] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_recvmsg, /* check_recvmsg */ 8, 0 ),
    /* allow getsockopt based on expression */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_getsockopt, /* check_getsockopt */ 13, 0 ),
...
//  lbl_3:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, lo_xsk_fd, /* lbl_2 */ 0, /* RET_KILL_PROCESS */ 8 ),
//  lbl_2:
...
//  check_getsockopt:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, xsk_fd, /* lbl_4 */ 0, /* RET_KILL_PROCESS */ 4 ),
//  lbl_4:
    /* load syscall argument 1 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[1])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SOL_XDP, /* lbl_5 */ 0, /* RET_KILL_PROCESS */ 2 ),
//  lbl_5:
    /* load syscall argument 2 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[2])),
