...
#endif
static const unsigned int sock_filter_policy_snaps_instr_cnt = 50;

static void populate_sock_filter_policy_snaps( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int tmp_fd, unsigned int tmp_inc_fd, unsigned int full_snapshot_fd, unsigned int incremental_snapshot_fd) {
  FD_TEST( out_cnt >= 50 );
  struct sock_filter filter[50] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_fchmod, /* check_fchmod */ 18, 0 ),
    /* allow ftruncate based on expression */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_ftruncate, /* check_ftruncate */ 19, 0 ),
