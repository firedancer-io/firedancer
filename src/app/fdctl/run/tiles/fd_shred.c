...
after_frag( void *             _ctx,
  ...
  if( FD_LIKELY( in_idx==NET_IN_IDX ) ) {
    ...
    /* For breakpoint demo only, compute the number of transactions.  It
        ...
        * This FEC set is exactly one microblock batch, which means that
          ...
          data.
        * All transactions other than the occasional vote are 212 bytes.
          This means that we can count the transactions by dividing the
          transaction data size by 212.  Votes are so infrequent, that
          the average transaction size is something like 212.0001.
          Plus, as long as we only have 1 vote per microblock batch,
          since the vote size is 319<2*212, the floor division will
    ...
    if( 1 ) {
      ...
      ctx->shredded_txn_cnt = ( (data_shred_cnt-1UL) * fd_shred_payload_sz( shred0 ) + fd_shred_payload_sz( shredN )
                                - 8UL - 48UL*FD_LOAD( ulong, fd_shred_data_payload( shred0 ) ) )/212UL;
    }
  } else {
