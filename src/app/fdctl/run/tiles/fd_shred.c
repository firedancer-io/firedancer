...
after_frag( fd_shred_ctx_t *    ctx,
  ...
  if( FD_LIKELY( ctx->in_kind[ in_idx ]==IN_KIND_NET ) ) {
    ...
    if( (rv==FD_FEC_RESOLVER_SHRED_OKAY) | (rv==FD_FEC_RESOLVER_SHRED_COMPLETES) ) {
      ...
      ulong   sz  = fd_shred_sz( shred );
      fd_memcpy( buf, shred, sz );
      ulong tspub       = fd_frag_meta_ts_comp( fd_tickcount() );
      uchar shred_flags = fd_shred_is_data( fd_shred_type( shred->variant ) ) ? shred->data.flags : 0;
      ulong replay_sig  = fd_disco_replay_sig( shred->slot, shred->data.parent_off, shred->fec_set_idx, shred_flags, ctx->in_kind[ in_idx ]==IN_KIND_NET );
