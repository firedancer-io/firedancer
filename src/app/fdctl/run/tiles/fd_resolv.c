...
after_credit( fd_resolv_ctx_t *   ctx,
  ...
  *opt_poll_in = 0;
  ...
  if( FD_LIKELY( publish_txn( ctx, stem, pool_ele( ctx->pool, ctx->flush_pool_idx ) ) ) ) {
    ctx->metrics.stash[ FD_METRICS_COUNTER_RESOLV_STASH_OPERATION_PUBLISHED_OFF-FD_METRICS_COUNTER_RESOLV_STASH_OPERATION_OFF ]++;
  } else {
    ctx->metrics.stash[ FD_METRICS_COUNTER_RESOLV_STASH_OPERATION_REMOVED_OFF-FD_METRICS_COUNTER_RESOLV_STASH_OPERATION_OFF ]++;
  }
...
after_frag( fd_resolv_ctx_t *   ctx,
  ...
  if( FD_UNLIKELY( !is_bundle_member && !is_durable_nonce && !blockhash ) ) {
    ...
    if( FD_UNLIKELY( !pool_free( ctx->pool ) ) ) {
      ...
      ctx->metrics.stash[ FD_METRICS_COUNTER_RESOLV_STASH_OPERATION_OVERRUN_OFF-FD_METRICS_COUNTER_RESOLV_STASH_OPERATION_OFF ]++;
    } else {
    ...
    ctx->metrics.stash[ FD_METRICS_COUNTER_RESOLV_STASH_OPERATION_INSERTED_OFF-FD_METRICS_COUNTER_RESOLV_STASH_OPERATION_OFF ]++;
