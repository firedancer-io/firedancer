...
static const ulong CUS_PER_MICROBLOCK = 1500000UL;

static const float VOTE_FRACTION = 0.75;
...
after_credit( void *             _ctx,
  ...
  for( ulong i=0UL; i<ctx->out_cnt; i++ ) {
    if( FD_LIKELY( ctx->out_ready_at[i]<now ) ) { /* optimize for the case we send a microblock */
      fd_pack_microblock_complete( ctx->pack, i );

      uchar * microblock_dst = fd_chunk_to_laddr( ctx->out_mem, ctx->out_chunk );
      microblock_dst += sizeof(ulong) + sizeof(fd_hash_t);

      long schedule_duration = -fd_tickcount();
      ulong schedule_cnt = fd_pack_schedule_next_microblock( ctx->pack, CUS_PER_MICROBLOCK, VOTE_FRACTION, i, fd_type_pun( microblock_dst ) );
      ...
      if( FD_LIKELY( schedule_cnt ) ) {
        ...
        ulong msg_sz = schedule_cnt;

        ulong sig = fd_disco_replay_sig( ctx->leader_slot, REPLAY_FLAG_PACKED_MICROBLOCK );
...
fd_tile_config_t fd_tile_pack_int = {
