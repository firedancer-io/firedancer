...
scratch_footprint( fd_topo_tile_t const * tile FD_PARAM_UNUSED ) {
  ...
  l = FD_LAYOUT_APPEND( l, fd_exec_epoch_ctx_align(), fd_exec_epoch_ctx_footprint( FD_RESTART_MAX_PEERS ) );
...
unprivileged_init( fd_topo_t      * topo FD_PARAM_UNUSED,
  ...
  void * epoch_ctx_mem        = FD_SCRATCH_ALLOC_APPEND( l, fd_exec_epoch_ctx_align(), fd_exec_epoch_ctx_footprint( FD_RESTART_MAX_PEERS ) );
  ...
  FD_LOG_NOTICE(("restart_mem=%p", restart_mem));
  ...
  ctx->epoch_ctx = fd_exec_epoch_ctx_join( fd_exec_epoch_ctx_new( epoch_ctx_mem, FD_RESTART_MAX_PEERS ) );
...
after_frag( fd_restart_tile_ctx_t * ctx,
  ...
  /* TODO: handle store tile input and generate snapshot */
/*
     if( FD_UNLIKELY( curr_slot==ctx->restart->heaviest_fork_slot ) ) {
  ...
  if( FD_UNLIKELY( !ctx->is_funk_active ) ) {
    ...
    fd_epoch_bank_t epoch_bank;
    {
      ...
      if( magic==FD_RUNTIME_ENC_BINCODE ) {
        ...
        epoch_bank = *(fd_epoch_bank_t *)mem;
      } else {
        FD_LOG_ERR(( "failed to read banks record: invalid magic number" ));
      }
    }
    ...
    /* Get the current hard fork information from slot bank */
