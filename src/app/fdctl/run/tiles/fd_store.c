...
privileged_init( fd_topo_t *      topo,
  ...
  ctx->identity_key[ 0 ] = *(fd_pubkey_t *)load_key_into_protected_memory( tile->store.identity_key_path, /* pubkey only: */ 1 );
}
...
fd_store_tile_slot_prepare( fd_store_tile_ctx_t * ctx,
  ...
  if( repair_req_cnt != 0 ) {
    ...
    // FD_LOG_WARNING(("QQ: %lu %lu %lu", repair_req_cnt, repair_req_sz, ctx->repair_req_out_seq));
...
unprivileged_init( fd_topo_t *      topo,
  ...
  if( FD_UNLIKELY( tile->out_cnt != 2 ||
                   ...
                   topo->links[ tile->out_link_id[ REPLAY_OUT_IDX ] ].kind != FD_TOPO_LINK_KIND_STORE_TO_REPLAY ) )
  ...
  for( ulong i = 0; i < topo->wksp_cnt; i++ ) {
    ...
    if( topo_wksp->kind == FD_TOPO_WKSP_KIND_BLOCKSTORE ) {
  ...
  ctx->replay_in_mem    = topo->workspaces[ replay_in_link->wksp_id ].wksp;
  ...
  ctx->replay_out_mem    = topo->workspaces[ replay_out->wksp_id ].wksp;
