...
struct fd_gossip_tile_ctx {
  ...
  uchar restart_last_vote_msg[ LAST_VOTED_FORK_LINK_BYTES_MAX ];
};
...
gossip_deliver_fun( fd_crds_data_t * data, void * arg ) {
  ...
  if( fd_crds_data_is_vote( data ) ) {
    ...
    if ( FD_LIKELY( data->inner.restart_last_voted_fork_slots.offsets.discriminant==fd_restart_slots_offsets_enum_raw_offsets ) ) {
      ...
      uchar bitmap_src [ LAST_VOTED_FORK_RAW_BITMAP_BYTES_MAX ];
      fd_restart_convert_runlength_to_raw_bitmap( &data->inner.restart_last_voted_fork_slots, bitmap_src, &bitmap_len );
      if( FD_UNLIKELY( bitmap_len>LAST_VOTED_FORK_RAW_BITMAP_BYTES_MAX ) ) {
        FD_LOG_WARNING(( "Ignore an invalid gossip message with bitmap length greater than %lu", LAST_VOTED_FORK_RAW_BITMAP_BYTES_MAX ));
        return;
      }
      memcpy( bitmap_dst, bitmap_src, bitmap_len );
    }
...
during_frag( fd_gossip_tile_ctx_t * ctx,
  ...
  if( in_idx==REPLAY_IN_IDX ) {
    if( FD_UNLIKELY( chunk<ctx->replay_in_chunk0 || chunk>ctx->replay_in_wmark || sz>LAST_VOTED_FORK_LINK_BYTES_MAX+sizeof(uint) ) ) {
...
after_credit( fd_gossip_tile_ctx_t * ctx,
  ...
  *charge_busy = 1;
  ...
  if( FD_UNLIKELY(( ctx->restart_last_push_time+GOSSIP_MSG_PUBLISH_PERIOD_NS<now )) ) {
    ...
    if( FD_UNLIKELY( ctx->restart_last_vote_msg_sz>0 ) ) {
      ...
      /* We need to ensure that the encoded last_voted_fork_slots message does not exceed PACKET_DATA_SIZE bytes */
      ulong bitmap_len = restart_last_vote_msg.inner.restart_last_voted_fork_slots.offsets.inner.raw_offsets.offsets.bits.bits_len;
      if( FD_UNLIKELY( bitmap_len>LAST_VOTED_FORK_PACKET_BITMAP_BYTES_MAX ) ) {
        fd_restart_convert_raw_bitmap_to_runlength( &restart_last_vote_msg.inner.restart_last_voted_fork_slots );
      }
