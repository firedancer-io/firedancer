  ...
  *charge_busy = 1;
  ...
  if( FD_UNLIKELY(( ctx->restart_last_push_time+GOSSIP_MSG_PUBLISH_PERIOD_NS<now )) ) {
    ...
    if( FD_UNLIKELY( ctx->restart_last_vote_msg_sz>0 ) ) {
      ...
      /* Convert the bitmap to RunLengthEncoding before sending out */
      fd_restart_run_length_encoding_inner_t runlength_encoding[ LAST_VOTED_FORK_PACKET_BITMAP_BYTES_MAX/sizeof(ushort) ];
