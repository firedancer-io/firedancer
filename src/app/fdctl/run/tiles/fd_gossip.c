...
gossip_deliver_fun( fd_crds_data_t * data, void * arg ) {
  ...
  if( fd_crds_data_is_restart_last_voted_fork_slots( data ) ) {
    ...
    FD_LOG_ERR(( "Received a restart_heaviest_fork message: slot=%lu, hash=%s",
                  data->inner.restart_heaviest_fork.last_slot,
                  FD_BASE58_ENC_32_ALLOCA( &data->inner.restart_heaviest_fork.last_slot_hash ) ));
