...
metrics_write( fd_quic_ctx_t * ctx ) {
  ...
  FD_MGAUGE_SET( QUIC_TILE, TXN_REASMS_ACTIVE,       (ulong)fd_long_max( ctx->reasm->busy_cnt, 0L ) );
...
quic_conn_final( fd_quic_conn_t * conn,
  ...
  ctx->metrics.reasm_abandoned += (ulong)fd_long_max( conn->srx->rx_streams_active, 0L );
}
