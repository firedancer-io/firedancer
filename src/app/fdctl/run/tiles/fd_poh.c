...
void
fd_ext_event_publish_replay_stage( ulong sig,
                                   uchar * data,
                                   ulong   data_len ) {
  poh_link_publish( &event_replay, sig, data, data_len );
}
...
#define NONNULL( x ) (__extension__({                                        \
  ...
  poh_link_init( &event_replay, topo, tile, 4UL );
