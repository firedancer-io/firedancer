...
publish_votes_to_plugin( fd_replay_tile_ctx_t * ctx,
  ...
  *(ulong *)dst = i;
  ...
  ulong tspub = (ulong)fd_frag_meta_ts_comp( fd_tickcount() );
  fd_stem_publish( stem, VOTES_PLUG_OUT_IDX, FD_PLUGIN_MSG_VOTE_ACCOUNT_UPDATE, ctx->votes_plugin_out_chunk, data_sz, 0UL, 0UL, tspub );
  ctx->votes_plugin_out_chunk = fd_dcache_compact_next( ctx->votes_plugin_out_chunk, data_sz, ctx->votes_plugin_out_chunk0, ctx->votes_plugin_out_wmark );
}
