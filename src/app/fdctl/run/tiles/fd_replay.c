...
loose_footprint( fd_topo_tile_t const * tile FD_PARAM_UNUSED ) {
  return 36UL * FD_SHMEM_GIGANTIC_PAGE_SZ;
}
...
after_frag( fd_replay_tile_ctx_t * ctx,
  ...
  long execute_time_ns = -fd_log_wallclock();
    ...
    if( flags & REPLAY_FLAG_FINISHED_BLOCK ) {
      ...
      fd_fseq_update( ctx->smr, ctx->blockstore->smr );
...
init_after_snapshot( fd_replay_tile_ctx_t * ctx ) {
  ...
  fd_epoch_init( ctx->epoch, &snapshot_fork->slot_ctx.epoch_ctx->epoch_bank.stakes.vote_accounts );
...
during_housekeeping( void * _ctx ) {
  ...
  if( FD_LIKELY( ctx->funk && ctx->blockstore ) ) funk_and_txncache_publish( ctx, smr );
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->ghost     = fd_ghost_join( fd_ghost_new( ghost_mem, 42UL, FD_BLOCK_MAX, FD_VOTER_MAX ) );
