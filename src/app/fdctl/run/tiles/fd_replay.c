...
after_frag( fd_replay_tile_ctx_t * ctx,
            ...
            ulong                  tsorig,
            fd_stem_context_t *    stem ) {
  FD_LOG_INFO(("[%s] after_frag", __func__ ));
  (void)sig;
  (void)sz;
  ...
  if( FD_UNLIKELY( in_idx == STORE_IN_IDX ) ) {
    ...
    uint block_replayed_idx = 0;
    uint block_data_complete_idx = 0;

    fd_block_map_query_t quer[1] = { 0 };
    int err = FD_MAP_ERR_AGAIN;
    while( err == FD_MAP_ERR_AGAIN ) {
      int err = fd_block_map_query_try( ctx->blockstore->block_map, &ctx->curr_slot, NULL, quer, 0 );
      fd_block_meta_t * block_map_entry = fd_block_map_query_ele( quer );
      if( FD_UNLIKELY( err == FD_MAP_ERR_KEY ) ) FD_LOG_ERR(( "block map missing curr_slot" )); /* should be impossible, as from store */

      fd_memcpy( data_complete_idxs, block_map_entry->data_complete_idxs, sizeof(data_complete_idxs) );
      block_replayed_idx = block_map_entry->replayed_idx;
      block_data_complete_idx = block_map_entry->data_complete_idx;

      err = fd_block_map_query_test( quer );
    }

    /* TODO: doing simple thing now */

    if( FD_LIKELY( block_data_complete_idx != FD_SHRED_IDX_NULL ) ) {
      uint i = block_replayed_idx + 1; uint j = block_data_complete_idx;
      for ( uint idx = i; idx <= j; idx++ ) {
        if( FD_UNLIKELY( fd_block_set_test( data_complete_idxs, idx ) ) ) {
          ...
          replay_mbatch( ctx, block_replayed_idx + 1U );
          block_replayed_idx = idx;
        }
      }
    }

    err = fd_block_map_prepare( ctx->blockstore->block_map, &ctx->curr_slot, NULL, quer, FD_MAP_FLAG_BLOCKING );
    fd_block_meta_t * block_map_entry = fd_block_map_query_ele( quer );
    if( FD_UNLIKELY( err == FD_MAP_ERR_KEY ) ) {
      FD_LOG_ERR(( "map corruption." )); /* should be impossible, as from store */
      return;
    }
    block_map_entry->replayed_idx = block_replayed_idx;
