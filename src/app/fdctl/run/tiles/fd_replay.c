...
during_frag( void *           _ctx,
  ...
  if( FD_UNLIKELY( chunk < ctx->store_in_chunk0 || chunk > ctx->store_in_wmark ||
                   sz > MAX_TXNS_PER_REPLAY ) ) {
    FD_LOG_WARNING( ( "chunk %lu %lu corrupt, not in range [%lu,%lu]",
                      chunk,
                      sz,
                      ctx->store_in_chunk0,
                      ctx->store_in_wmark ) );
  ...
  if( ctx->flags & REPLAY_FLAG_PACKED_MICROBLOCK ) {
    ...
    for( fd_fork_frontier_iter_t iter =
         ...
         iter = fd_fork_frontier_iter_next(
             iter, ctx->replay->forks->frontier, ctx->replay->forks->pool ) ) {
      fd_exec_slot_ctx_t * ele =
          &fd_fork_frontier_iter_ele( iter, ctx->replay->forks->frontier, ctx->replay->forks->pool )
               ->slot_ctx;
  ...
  if( FD_LIKELY( block_ ) ) {
    if( fd_uchar_extract_bit( block_->flags, FD_BLOCK_FLAG_PROCESSED ) ) { *opt_filter = 1; }
  }
...
after_frag( void *                 _ctx,
            ...
            FD_PARAM_UNUSED int *  opt_filter,
...
#define NOTIFY_END                                                                                 \
  fd_mcache_publish( ctx->notif_out_mcache,                                                        \
                     ctx->notif_out_depth,                                                         \
                     ctx->notif_out_seq,                                                           \
                     0UL,                                                                          \
                     ctx->notif_out_chunk,                                                         \
                     sizeof( fd_replay_notif_msg_t ),                                              \
                     0UL,                                                                          \
                     tsorig,                                                                       \
                     tsorig );                                                                     \
  ctx->notif_out_seq   = fd_seq_inc( ctx->notif_out_seq, 1UL );                                    \
  ctx->notif_out_chunk = fd_dcache_compact_next( ctx->notif_out_chunk,                             \
                                                 sizeof( fd_replay_notif_msg_t ),                  \
                                                 ctx->notif_out_chunk0,                            \
                                                 ctx->notif_out_wmark );                           \
  msg                  = NULL

      ulong                   tsorig = fd_frag_meta_ts_comp( fd_tickcount() );
      fd_replay_notif_msg_t * msg    = NULL;
      for( fd_funk_rec_t const * rec =
               fd_funk_txn_first_rec( ctx->replay->funk, fork->slot_ctx.funk_txn );
           rec != NULL;
        ...
        fd_memcpy( msg->acct_saved.acct_id[msg->acct_saved.acct_id_cnt++].uc,
                   rec->pair.key->uc,
                   sizeof( fd_pubkey_t ) );
        if( msg->acct_saved.acct_id_cnt == FD_REPLAY_NOTIF_ACCT_MAX ) { NOTIFY_END; }
      }
      if( msg ) { NOTIFY_END; }

      {
        ...
        memcpy(
            &msg->slot_exec.bank_hash, &fork->slot_ctx.slot_bank.banks_hash, sizeof( fd_hash_t ) );
        NOTIFY_END;
      }
...
#undef NOTIFY_END
      ...
      fd_fork_t * child = fd_fork_frontier_ele_remove(
          ctx->replay->forks->frontier, &fork->slot, NULL, ctx->replay->forks->pool );
      ...
      if( FD_UNLIKELY( fd_fork_frontier_ele_query(
        ...
        FD_LOG_ERR(
            ( "invariant violation: child slot %lu was already in the frontier", ctx->curr_slot ) );
      }
...
tpool_boot( fd_topo_t * topo, ulong total_thread_count ) {
  ...
  if( main_thread_seen ) { thread_count++; }

  if( thread_count != total_thread_count )
    FD_LOG_ERR(
        ( "thread count mismatch thread_count=%lu total_thread_count=%lu main_thread_seen=%lu",
          thread_count,
          total_thread_count,
          main_thread_seen ) );
...
after_credit( void * _ctx, fd_mux_context_t * mux_ctx ) {
  ...
  if( FD_UNLIKELY( ctx->slot_ctx->blockstore == NULL ) ) {
    ...
    if( ctx->slot_ctx->blockstore != NULL ) {
      FD_SCRATCH_SCOPE_BEGIN {
        ...
        if( is_snapshot ) { read_snapshot( ctx, ctx->snapshot, ctx->incremental ); }
        fd_bank_hash_cmp_t * bank_hash_cmp = ctx->epoch_ctx->bank_hash_cmp;
  ...
  if( now - ctx->last_stake_weights_push_time > (long)5e9 ) {
    ...
    if( epoch_bank->next_epoch_stakes.vote_accounts_root != NULL ) {
      ...
      fd_stake_weight_t * stake_weights = (fd_stake_weight_t *)&stake_weights_msg[4];
      ulong               stake_weight_idx =
          fd_stake_weights_by_node( &epoch_bank->next_epoch_stakes, stake_weights );

      stake_weights_msg[0] = fd_slot_to_leader_schedule_epoch(
          &epoch_bank->epoch_schedule, ctx->slot_ctx->slot_bank.slot ); /* epoch */
      stake_weights_msg[1] = stake_weight_idx;                          /* staked_cnt */
      stake_weights_msg[2] =
          fd_epoch_slot0( &epoch_bank->epoch_schedule, stake_weights_msg[0] ); /* start_slot */
      stake_weights_msg[3] = epoch_bank->epoch_schedule.slots_per_epoch;       /* slot_cnt */
      ...
      ulong stake_weights_sz =
          4 * sizeof( ulong ) + ( stake_weight_idx * sizeof( fd_stake_weight_t ) );
      ulong stake_weights_sig = 4UL;
      fd_mcache_publish( ctx->stake_weights_out_mcache,
                         ctx->stake_weights_out_depth,
                         ctx->stake_weights_out_seq,
                         stake_weights_sig,
                         ctx->stake_weights_out_chunk,
                         stake_weights_sz,
                         0UL,
...
privileged_init( fd_topo_t * topo      FD_PARAM_UNUSED,
  ...
  fd_replay_tile_ctx_t * ctx =
      FD_SCRATCH_ALLOC_APPEND( l, alignof( fd_replay_tile_ctx_t ), sizeof( fd_replay_tile_ctx_t ) );
...
unprivileged_init( fd_topo_t * topo, fd_topo_tile_t * tile, void * scratch ) {
  ...
  void * smem = FD_SCRATCH_ALLOC_APPEND(
      l, fd_scratch_smem_align(), fd_scratch_smem_footprint( SCRATCH_MAX ) );
  void * fmem = FD_SCRATCH_ALLOC_APPEND(
      l, fd_scratch_fmem_align(), fd_scratch_fmem_footprint( SCRATCH_DEPTH ) );
  /* NOTE: incremental snapshot load resets this and care should be taken if
    adding any setup here. */
  ctx->epoch_ctx_mem = FD_SCRATCH_ALLOC_APPEND(
      l, fd_exec_epoch_ctx_align(), fd_exec_epoch_ctx_footprint( VOTE_ACC_MAX ) );
  void * replay_mem = FD_SCRATCH_ALLOC_APPEND( l, fd_replay_align(), fd_replay_footprint() );
  void * forks_mem =
      FD_SCRATCH_ALLOC_APPEND( l, fd_forks_align(), fd_forks_footprint( FORKS_MAX ) );
  void * latest_votes_mem =
      FD_SCRATCH_ALLOC_APPEND( l, fd_latest_vote_deque_align(), fd_latest_vote_deque_footprint() );
  void * capture_ctx_mem =
  ...
  void * ghost_mem = FD_SCRATCH_ALLOC_APPEND(
      ...
      fd_ghost_footprint( 1 << FD_BFT_LG_SLOT_MAX, 1 << FD_BFT_LG_NODE_PUBKEY_MAX ) );
  void * root_votes_mem =
      FD_SCRATCH_ALLOC_APPEND( l, fd_root_vote_map_align(), fd_root_vote_map_footprint() );
  void * root_stakes_mem =
      FD_SCRATCH_ALLOC_APPEND( l, fd_root_stake_map_align(), fd_root_stake_map_footprint() );
  ...
  if( ctx->blockstore_wksp == NULL ) { FD_LOG_ERR( ( "no blocktore workspace" ) ); }
  ...
  if( strncmp( ctx->snapshot, "wksp:", 5 ) == 0 ) {
    ...
    if( fd_wksp_tag_query( ctx->funk_wksp, &tag, 1, &info, 1 ) > 0 ) {
      ...
      if( funk == NULL ) { FD_LOG_ERR( ( "failed to join a funky in %s", ctx->snapshot ) ); }
    } else {
    ...
    shmem =
        fd_wksp_alloc_laddr( ctx->funk_wksp, fd_funk_align(), fd_funk_footprint(), FD_FUNK_MAGIC );
    if( shmem == NULL ) FD_LOG_ERR( ( "failed to allocate a funky" ) );
    funk = fd_funk_join( fd_funk_new(
        shmem, FD_FUNK_MAGIC, ctx->funk_seed, tile->replay.txn_max, tile->replay.index_max ) );
    if( funk == NULL ) {
  ...
  ctx->epoch_ctx =
      fd_exec_epoch_ctx_join( fd_exec_epoch_ctx_new( ctx->epoch_ctx_mem, VOTE_ACC_MAX ) );
  ...
  if( FD_LIKELY( ctx->max_workers > 1 ) ) tpool_boot( topo, ctx->max_workers );
  ...
  if( FD_LIKELY( ctx->max_workers > 1 ) ) {
    ...
    uchar * tpool_worker_mem      = fd_wksp_alloc_laddr( ctx->wksp,
                                                    FD_SCRATCH_ALIGN_DEFAULT,
                                                    tpool_worker_stack_sz * ctx->max_workers,
                                                    421UL ); /* 256MB per thread */
    for( ulong i = 1; i < ctx->max_workers; i++ ) {
      if( fd_tpool_worker_push( ctx->tpool,
                                i,
                                tpool_worker_mem + tpool_worker_stack_sz * ( i - 1U ),
                                tpool_worker_stack_sz ) == NULL ) {
  ...
  if( ctx->tpool == NULL ) { FD_LOG_ERR( ( "failed to create thread pool" ) ); }

  /* add it to the frontier */
  fd_fork_frontier_ele_insert(
      ctx->replay->forks->frontier, replay_slot, ctx->replay->forks->pool );
  ...
  ctx->store_in_wmark =
      fd_dcache_compact_wmark( ctx->store_in_mem, store_in_link->dcache, store_in_link->mtu );
  ...
  ctx->poh_out_wmark =
      fd_dcache_compact_wmark( ctx->poh_out_mem, poh_out_link->dcache, poh_out_link->mtu );
  ...
  ctx->notif_out_wmark =
      fd_dcache_compact_wmark( ctx->notif_out_mem, notif_out->dcache, notif_out->mtu );
  ...
  ctx->stake_weights_out_mem =
      topo->workspaces[topo->objs[stake_weights_out->dcache_obj_id].wksp_id].wksp;
  ctx->stake_weights_out_chunk0 =
      fd_dcache_compact_chunk0( ctx->stake_weights_out_mem, stake_weights_out->dcache );
  ctx->stake_weights_out_wmark = fd_dcache_compact_wmark(
      ctx->stake_weights_out_mem, stake_weights_out->dcache, stake_weights_out->mtu );
