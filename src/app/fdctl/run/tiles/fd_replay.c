...
during_frag( fd_replay_tile_ctx_t * ctx,
  ...
  if( in_idx == STORE_IN_IDX ) {
    ...
    if( FD_UNLIKELY( chunk<ctx->shred_in_chunk0 || chunk>ctx->shred_in_wmark || sz>FD_SHRED_MAX_SZ ) ) {
      FD_LOG_ERR(( "chunk %lu %lu corrupt, not in range [%lu,%lu]", chunk, sz, ctx->shred_in_chunk0, ctx->shred_in_wmark ));
    }
    ulong  slot       = fd_disco_replay_sig_slot( sig );
    ushort parent_off = fd_disco_replay_sig_parent_off( sig );
    uint   fec_idx    = fd_disco_replay_sig_fec_idx( sig );
    uchar  flags      = fd_disco_replay_sig_flags( sig );

    FD_LOG_DEBUG(( "Notified of shred sent from the shred tile: slot: %lu, shred_idx: %u, flags: %u, parent_slot: %lu", slot, fec_idx, flags, slot - parent_off ));
    ctx->skip_frag = 1;
    return;
