...
#include "../../../../disco/events/fd_circbuf.h"

#include "../../../../disco/events/generated/fd_event.h"
#include "../../../../disco/events/generated/fd_event_metrics.h"
...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  l = FD_LAYOUT_APPEND( l, alignof( fd_circbuf_t ), fd_circbuf_footprint( 1UL<<30UL ) );
...
snap_metrics( fd_event_ctx_t * ctx ) {
  uchar * buffer = fd_circbuf_push_back( ctx->circbuf, alignof( fd_event_snap_t ), ctx->metrics_snap_footprint );
  if( FD_UNLIKELY( !buffer ) ) return;

  fd_event_snap_t * event = (fd_event_snap_t *)buffer;
  event->timestamp      = fd_log_wallclock();
  event->event_type     = FD_EVENT_METRICS;
  event->event.metrics.slot   = 0UL;
  event->event.metrics.reason = FD_EVENT_METRICS_REASON_PERIODIC;

  fd_event_metrics_layout( ctx->topo, (uchar*)&event->event.metrics );
  fd_metric_event_snap( ctx->topo, &event->event.metrics );
}
...
check_message( fd_event_ctx_t * ctx,
  ...
  uchar const * event = fd_circbuf_pop_front( ctx->circbuf );
...
unprivileged_init( fd_topo_t *      topo,
  ...
  FD_SCRATCH_ALLOC_INIT( l, scratch );
  fd_event_ctx_t * ctx = FD_SCRATCH_ALLOC_APPEND( l, alignof( fd_event_ctx_t ), sizeof( fd_event_ctx_t ) );
  void * circbuf = FD_SCRATCH_ALLOC_APPEND( l, alignof( fd_circbuf_t ), fd_circbuf_footprint( 1UL<<30UL ) );

  ctx->circbuf = fd_circbuf_new( circbuf, 1UL<<30UL );
  ctx->topo = topo;
  ...
  ctx->metrics_snap_footprint = fd_ulong_align_up( sizeof( fd_event_snap_t ), alignof( fd_event_metrics_t ) ) + fd_event_metrics_footprint( topo );
