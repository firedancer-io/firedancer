...
fd_store_tile_slot_prepare( fd_store_tile_ctx_t * ctx,
  ...
  if( store_slot_prepare_mode == FD_STORE_SLOT_PREPARE_CONTINUE ) {
    ...
    FD_SCRATCH_SCOPE_BEGIN {
      ...
      FD_LOG_NOTICE(( "block prepared - slot: %lu", slot ));
  ...
  if( repair_req_cnt != 0 ) {
    ...
    ulong repair_req_sz = repair_req_cnt;
...
after_credit( void *             _ctx,
  ...
  for( ulong i = fd_pending_slots_iter_init( ctx->store->pending_slots );
    ...
    fd_store_tile_slot_prepare( ctx, mux, store_slot_prepare_mode, slot );
  }
}
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->blockstore_wksp = NULL;
  for( ulong i = 0; i < topo->wksp_cnt; i++ ) {
    fd_topo_wksp_t * topo_wksp = &topo->workspaces[i];
    if( strcmp( topo_wksp->name, "bstore" ) == 0 ) {
      ctx->blockstore_wksp = topo_wksp->wksp;
      break;
    }
  }
