...
#include "generated/netrx_seccomp.h"
...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  l = FD_LAYOUT_APPEND( l, fd_xsk_align(),     fd_xsk_footprint( FD_NET_MTU, tile->netrx.xdp_rx_queue_size, tile->netrx.xdp_rx_queue_size, tile->netrx.xdp_tx_queue_size, tile->netrx.xdp_tx_queue_size ) );
  l = FD_LAYOUT_APPEND( l, fd_xsk_aio_align(), fd_xsk_aio_footprint( tile->netrx.xdp_tx_queue_size, tile->netrx.xdp_aio_depth ) );
  if( FD_UNLIKELY( strcmp( tile->netrx.interface, "lo" ) && tile->kind_id == 0 ) ) {
    l = FD_LAYOUT_APPEND( l, fd_xsk_align(),     fd_xsk_footprint( FD_NET_MTU, tile->netrx.xdp_rx_queue_size, tile->netrx.xdp_rx_queue_size, tile->netrx.xdp_tx_queue_size, tile->netrx.xdp_tx_queue_size ) );
    l = FD_LAYOUT_APPEND( l, fd_xsk_aio_align(), fd_xsk_aio_footprint( tile->netrx.xdp_tx_queue_size, tile->netrx.xdp_aio_depth ) );
  }
...
metrics_write( fd_net_ctx_t * ctx ) {
  ...
  FD_MCNT_SET( NET_RX_TILE, RECEIVED_PACKETS, rx_cnt );
  FD_MCNT_SET( NET_RX_TILE, RECEIVED_BYTES,   rx_sz  );
}
...
before_credit( fd_net_ctx_t *      ctx,
  ...
  for( ulong i=0UL; i<ctx->xsk_aio_cnt; i++ ) {
...
poll_xdp_statistics( fd_net_ctx_t * ctx ) {
  ...
  if( FD_LIKELY( optlen==sizeof(struct xdp_statistics_v1) ) ) {
    FD_MCNT_SET( NET_RX_TILE, XDP_RX_DROPPED_OTHER, stats.rx_dropped );
    FD_MCNT_SET( NET_RX_TILE, XDP_RX_DROPPED_RING_FULL, stats.rx_ring_full );
...
init_link_session( fd_xdp_session_t *      session,
  ...
  ushort udp_port_candidates[ FDCTL_NET_BIND_MAX ] = { 
    [ FDCTL_NET_BIND_TPU_USER_UDP  ] = (ushort)tile->netrx.legacy_transaction_listen_port,
    [ FDCTL_NET_BIND_TPU_USER_QUIC ] = (ushort)tile->netrx.quic_transaction_listen_port,
    [ FDCTL_NET_BIND_SHRED         ] = (ushort)tile->netrx.shred_listen_port,
    [ FDCTL_NET_BIND_GOSSIP        ] = (ushort)tile->netrx.gossip_listen_port,
    [ FDCTL_NET_BIND_REPAIR_IN     ] = (ushort)tile->netrx.repair_intake_listen_port,
    [ FDCTL_NET_BIND_REPAIR_SERVE  ] = (ushort)tile->netrx.repair_serve_listen_port,
  };
  ...
  uint xdp_mode = 0;
  if(      FD_LIKELY( !strcmp( tile->netrx.xdp_mode, "skb" ) ) ) xdp_mode = XDP_FLAGS_SKB_MODE;
  else if( FD_LIKELY( !strcmp( tile->netrx.xdp_mode, "drv" ) ) ) xdp_mode = XDP_FLAGS_DRV_MODE;
  else if( FD_LIKELY( !strcmp( tile->netrx.xdp_mode, "hw"  ) ) ) xdp_mode = XDP_FLAGS_HW_MODE;
  else FD_LOG_ERR(( "unknown XDP mode `%.4s`", tile->netrx.xdp_mode ));
  ...
  if( 0!=strcmp( tile->netrx.interface, "lo" ) ) {
...
privileged_init( fd_topo_t *      topo,
  ...
  uint if_idx = if_nametoindex( tile->netrx.interface );
  if( FD_UNLIKELY( !if_idx ) ) FD_LOG_ERR(( "if_nametoindex(%s) failed", tile->netrx.interface ));

  uint lo_idx = if_nametoindex( "lo" );
  if( FD_UNLIKELY( !lo_idx ) ) FD_LOG_ERR(( "if_nametoindex(lo) failed" ));

  ulong            p_net0_pid_id = fd_pod_query_ulong( topo->props, "netrx0_pid", ULONG_MAX ); FD_TEST( p_net0_pid_id!=ULONG_MAX );
  ...
  fd_xsk_t * xsk =
      fd_xsk_join(
      fd_xsk_new( FD_SCRATCH_ALLOC_APPEND( l, fd_xsk_align(), fd_xsk_footprint( FD_NET_MTU, tile->netrx.xdp_rx_queue_size, tile->netrx.xdp_rx_queue_size, tile->netrx.xdp_tx_queue_size, tile->netrx.xdp_tx_queue_size ) ),
                  FD_NET_MTU,
                  tile->netrx.xdp_rx_queue_size,
                  tile->netrx.xdp_rx_queue_size,
                  tile->netrx.xdp_tx_queue_size,
                  tile->netrx.xdp_tx_queue_size ) );
  if( FD_UNLIKELY( !xsk ) ) FD_LOG_ERR(( "fd_xsk_new failed" ));
  
  uint flags = tile->netrx.zero_copy ? XDP_ZEROCOPY : XDP_COPY;
  ...
  init_ctx->xsk_aio = fd_xsk_aio_new( FD_SCRATCH_ALLOC_APPEND( l, fd_xsk_aio_align(), fd_xsk_aio_footprint( tile->netrx.xdp_tx_queue_size, tile->netrx.xdp_aio_depth ) ),
                                      tile->netrx.xdp_tx_queue_size,
                                      tile->netrx.xdp_aio_depth );
  ...
  if( FD_UNLIKELY( 0!=strcmp( tile->netrx.interface, "lo" ) && !tile->kind_id ) ) {
    ...
    init_ctx->lo_xsk_aio = fd_xsk_aio_new( FD_SCRATCH_ALLOC_APPEND( l, fd_xsk_aio_align(), fd_xsk_aio_footprint( tile->netrx.xdp_tx_queue_size, tile->netrx.xdp_aio_depth ) ),
                                           tile->netrx.xdp_tx_queue_size,
                                           tile->netrx.xdp_aio_depth );
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->src_ip_addr = tile->netrx.src_ip_addr;
  memcpy( ctx->src_mac_addr, tile->netrx.src_mac_addr, 6UL );

  ctx->shred_listen_port = tile->netrx.shred_listen_port;
  ctx->quic_transaction_listen_port = tile->netrx.quic_transaction_listen_port;
  ctx->legacy_transaction_listen_port = tile->netrx.legacy_transaction_listen_port;
  ctx->gossip_listen_port = tile->netrx.gossip_listen_port;
  ctx->repair_intake_listen_port = tile->netrx.repair_intake_listen_port;
  ctx->repair_serve_listen_port = tile->netrx.repair_serve_listen_port;
  
  for( ulong i = 0; i < tile->out_cnt; i++ ) {
    fd_topo_link_t * out_link = &topo->links[ tile->out_link_id[ i  ] ];
    if( strcmp( out_link->name, "netrx_quic" ) == 0 ) {
    ...
    } else if( strcmp( out_link->name, "netrx_shred" ) == 0 ) {
    ...
    } else if( strcmp( out_link->name, "netrx_gossip" ) == 0 ) {
    ...
    } else if( strcmp( out_link->name, "netrx_repair" ) == 0 ) {
...
populate_allowed_seccomp( fd_topo_t const *      topo,
  ...
  populate_sock_filter_policy_netrx( out_cnt, out, (uint)fd_log_private_logfile_fd(), (uint)ctx->init.xsk->xsk_fd, (uint)allow_fd2 );
  return sock_filter_policy_netrx_instr_cnt;
}
...
fd_topo_run_tile_t fd_tile_netrx = {
  .name                     = "netrx",
