...
/* fd_gossip_dedup_in_ctx_t is a context object for each in (producer) mcache
...
typedef struct {
  ...
  ulong       chunk0;
  ulong       wmark;
} fd_gossip_dedup_in_ctx_t;

/* fd_gossip_dedup_ctx_t is the context object provided to callbacks from the
...
scratch_align( void ) {
  return alignof( fd_gossip_dedup_ctx_t );
}
...
unprivileged_init( fd_topo_t *      topo,
  ...
  fd_gossip_dedup_ctx_t * ctx = FD_SCRATCH_ALLOC_APPEND( l, alignof( fd_gossip_dedup_ctx_t ), sizeof( fd_gossip_dedup_ctx_t ) );
...
fd_tile_config_t fd_tile_gossip_dedup = {
