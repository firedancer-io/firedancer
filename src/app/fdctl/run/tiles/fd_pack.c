...
insert_from_extra( fd_pack_ctx_t * ctx ) {
  ...
  long receive_time = (long)((((ulong)insert->pack_cu.non_execution_cus)<<32) | ((ulong)insert->pack_cu.requested_execution_cus));
...
during_frag( fd_pack_ctx_t * ctx,
  ...
  case IN_KIND_DEDUP: {
    ...
    if( FD_LIKELY( ctx->leader_slot!=ULONG_MAX || fd_pack_avail_txn_cnt( ctx->pack )<ctx->max_pending_transactions ) ) {
      ...
      ctx->cur_spot->pack_cu.requested_execution_cus = (uint)((ulong)now & UINT_MAX);
      ctx->cur_spot->pack_cu.non_execution_cus       = (uint)((ulong)now>>32       );
