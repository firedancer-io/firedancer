...
main_pid_namespace( void * args ) {
  ...
  if( FD_UNLIKELY( affinity_tile_cnt<tile_cnt ) ) FD_LOG_ERR(( "The topology you are using has %lu active tiles, but the CPU affinity specified in the config tile as [layout.affinity] only provides for %lu cores. "
  ...
  if( FD_UNLIKELY( affinity_tile_cnt>tile_cnt ) ) FD_LOG_WARNING(( "The topology you are using has %lu active tiles, but the CPU affinity specified in the config tile as [layout.affinity] provides for %lu cores. "
