...
run_cmd_perm( args_t *         args,
  ...
  ulong mlock_limit = fd_topo_mlock_max_tile( &config->topo ) + FD_SHMEM_HUGE_PAGE_SZ * stack_pages;
...
tile_main( void * _args ) {
  ...
  const fd_frag_meta_t ** in_mcache = fd_alloca( alignof(fd_frag_meta_t *), sizeof(fd_frag_meta_t *)*args->tile->in_cnt );
  ulong ** in_fseq = fd_alloca( alignof(ulong *), sizeof(ulong *)*args->tile->in_cnt );
  if( FD_UNLIKELY( !in_mcache || !in_fseq ) ) FD_LOG_ERR(( "fd_alloca failed" ));
  ...
  ulong ** out_fseq = fd_alloca( alignof(ulong *), sizeof(ulong *)*TOPO_MAX_LINKS );
  for( ulong i=0; i<args->config->topo.tile_cnt; i++ ) {
    ...
    for( ulong j=0; j<tile->in_cnt; j++ ) {
      if( FD_UNLIKELY( tile->in_link_id[ j ] == tile->out_link_id_primary && tile->in_link_reliable[ j ] ) ) {
        ...
        out_cnt_reliable++;
        FD_TEST( out_cnt_reliable<TOPO_MAX_LINKS );
      }
    }
  }
