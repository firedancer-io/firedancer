...
#endif
static const unsigned int sock_filter_policy_pidns_instr_cnt = 27;
...
static void populate_sock_filter_policy_pidns( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd) {
  ...
  struct sock_filter filter[27] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_ppoll, /* check_ppoll */ 9, 0 ),
    /* allow wait4 based on expression */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_wait4, /* check_wait4 */ 12, 0 ),
    /* simply allow exit_group */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_exit_group, /* RET_ALLOW */ 18, 0 ),
...
//  check_ppoll:
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, 0, /* lbl_2 */ 0, /* RET_KILL_PROCESS */ 8 ),
//  lbl_2:
...
//  check_wait4:
    /* load syscall argument 2 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[2])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, (int)__WALL | (int)WNOHANG, /* lbl_3 */ 2, /* lbl_4 */ 0 ),
//  lbl_4:
    /* load syscall argument 2 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[2])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, (int)__WALL, /* lbl_3 */ 0, /* RET_KILL_PROCESS */ 2 ),
//  lbl_3:
    /* load syscall argument 3 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[3])),
