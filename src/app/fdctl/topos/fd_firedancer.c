...
fd_topo_initialize( config_t * config ) {
  ...
  fd_topob_wksp( topo, "replay_repai" );
  ...
  FOR(shred_tile_cnt)  fd_topob_link( topo, "shred_replay", "shred_replay", pending_fec_shreds_depth,                 FD_SHRED_REPLAY_MTU,       config->tiles.shred.max_pending_shred_sets );
  ...
  /**/                 fd_topob_link( topo, "replay_repai", "replay_repai", pending_fec_shreds_depth,                 FD_SHRED_MAX_SZ,               FD_REEDSOL_DATA_SHREDS_MAX );
  /**/                 fd_topob_link( topo, "repair_shred", "repair_shred", pending_fec_shreds_depth,                 FD_SHRED_MAX_SZ,               FD_REEDSOL_DATA_SHREDS_MAX );
  ...
  fd_topos_net_tiles( topo, config->layout.net_tile_count, config->tiles.netlink.max_routes, config->tiles.netlink.max_neighbors, config->development.net.provider, config->tiles.net.interface, config->tiles.net.flush_timeout_micros,
    config->tiles.net.xdp_rx_queue_size, config->tiles.net.xdp_tx_queue_size, config->tiles.net.xdp_zero_copy, config->tiles.net.xdp_mode, tile_to_cpu );
  ...
  FOR(shred_tile_cnt)  fd_topob_tile_in(  topo, "shred",  i,             "metric_in", "repair_shred",  0UL,          FD_TOPOB_RELIABLE,     FD_TOPOB_POLLED );
  ...
  /**/                 fd_topob_tile_out( topo, "replay",  0UL,                       "replay_repai",  0UL                                                  );
  ...
  /**/                 fd_topob_tile_in(  topo, "repair",  0UL,          "metric_in", "replay_repai",  0UL,          FD_TOPOB_UNRELIABLE,     FD_TOPOB_POLLED );

  /**/                 fd_topob_tile_out( topo, "repair",   0UL,                       "repair_shred",  0UL                                                  );
