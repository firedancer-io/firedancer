...
pub(crate) struct LayoutConfig {
    ...
    pub(crate) verify_tile_count: u32,
}

#[derive(Deserialize)]
pub(crate) struct ShmemConfig {
    pub(crate) path: String,
    pub(crate) kernel_gigantic_pages: u32,
    pub(crate) kernel_huge_pages: u32,
    pub(crate) workspace_page_size: String,
    pub(crate) workspace_page_count: u32,
}
...
impl Config {
    pub(crate) fn dump_to_bash(&self) {
        ...
        std::fs::write(&format!("{}/config.cfg", self.scratch_directory), format!("#!/bin/bash \n\
            # AUTOGENERATED \n\
            BUILD={build} \n\
            WKSP={name}.wksp \n\
            AFFINITY={affinity} \n\
