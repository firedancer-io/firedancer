...
#define TOPO_MAX_WKSPS (256UL)
#define TOPO_MAX_LINKS (256UL)
...
typedef struct fd_topo_t {
  ...
  fd_topo_wksp_t workspaces[ TOPO_MAX_WKSPS ];
  fd_topo_link_t links[ TOPO_MAX_LINKS ];
  fd_topo_tile_t tiles[ TOPO_MAX_WKSPS ];
} fd_topo_t;
...
/* Create all the workspaces needed by the topology on the system.
   This does not "join" the workspaces (map their memory into the
   process), but only creates the .wksp files on disk and formats
   them correctly as workspaces. */
void
fd_topo_create_workspaces( char *      app_name,
                           fd_topo_t * topo );

/* Join all IPC objects needed by the topology of this particular
   tile.  This will load all mcaches, dcaches, and fseqs into the
   topo object.  This must be called after all of the tile workspaces
   have been joined, either by calling fd_topo_join_workspaces, or
...
/* Determine the precise amount of memory, in bytes, that will be locked
   into memory (via. mlock()) when running this tile.  This is the sum
   of the size of all the workspaces that the tile uses.  The tile may
   lock other memory on its own, if it calls mlock outside of the
   toploogy system, so the actual amount locked may be larger than this,
   although it will never be less. */
ulong
fd_topo_mlock_max_tile( fd_topo_t * topo );

/* Same as fd_topo_mlock_max_tile, but for the entire topology rather
   than just one tile.  This is not just the sum over all the tiles,
   but rather the sum over all the workspaces used by the tiles. */
ulong
fd_topo_mlock_all( fd_topo_t * topo );
...
/* Dumps a diagnostic message describing the topology to the log. */
void
fd_topo_dump( fd_topo_t * topo );
