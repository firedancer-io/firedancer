...
fd_topo_create_workspaces( char *      app_name,
  ...
  for( ulong i=0; i<topo->wksp_cnt; i++ ) {
    ...
    /* Footprint has been predetermined so that this alloc() call will precisely succeed
...
#define INSERT_POD( path, laddr ) do {                                                              \
  ...
  if( FD_LIKELY( mode == FD_TOPO_FILL_MODE_FOOTPRINT ) ) {
    ...
    /* Give any leftover space in the workspace to the data region, since we might as well */
...
fd_topo_fill_tile( fd_topo_t *      topo,
  ...
  ulong wksp_seen[ TOPO_MAX_WKSPS ];
...
fd_topo_mlock_max_tile1( fd_topo_t * topo,
  ...
  #define ADD( wksp ) do {                          \
      int found = 0;                                \
      for( ulong j=0; j<wksp_seen_cnt; j++ ) {      \
...
fd_topo_validate( fd_topo_t * topo ) {
  ...
  for( ulong i=0; i<topo->link_cnt; i++ ) {
    for( ulong j=0; j<topo->link_cnt; j++ ) {
      if( FD_UNLIKELY( i==j ) ) continue;
  ...
  for( ulong i=0; i<FD_TOPO_TILE_KIND_MAX; i++ ) {
    for( ulong j=0; j<FD_TOPO_TILE_KIND_MAX; j++ ) {
      if( FD_UNLIKELY( i==j ) ) continue;
  ...
  for( ulong i=0; i<topo->tile_cnt; i++ ) {
    for( ulong j=0; j<topo->tiles[ i ].in_cnt; j++ ) {
      for( ulong k=0; k<topo->tiles[ i ].in_cnt; k++ ) {
  ...
  for( ulong i=0; i<topo->tile_cnt; i++ ) {
    for( ulong j=0; j<topo->tiles[ i ].out_cnt; j++ ) {
      for( ulong k=0; k<topo->tiles[ i ].out_cnt; k++ ) {
        if( FD_UNLIKELY( j==k ) ) continue;
  ...
  for( ulong i=0; i<FD_TOPO_WKSP_KIND_MAX; i++ ) {
  ...
  for( ulong i=0; i<FD_TOPO_WKSP_KIND_MAX; i++ ) {
    for( ulong j=0; j<FD_TOPO_WKSP_KIND_MAX; j++ ) {
      if( FD_UNLIKELY( i==j ) ) continue;
  ...
  for( ulong i=0; i<topo->wksp_cnt; i++ ) {
    for( ulong j=0; j<topo->wksp_cnt; j++ ) {
      if( FD_UNLIKELY( i==j ) ) continue;
  ...
  for( ulong i=0; i<topo->link_cnt; i++ ) {
    ...
    for( ulong j=0; j<topo->tile_cnt; j++ ) {
      for( ulong k=0; k<topo->tiles[ j ].out_cnt; k++ ) {
...
#define PRINT( ... ) do {                                                           \
  ...
  ulong total_bytes = fd_topo_mlock_all( topo ) + stack_pages + private_key_pages;
