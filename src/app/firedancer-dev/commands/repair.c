...
//  FOR(resolv_tile_cnt) fd_topob_tile_in(  topo, "resolv",  i,            "metric_in", "replay_resol", 0UL,          FD_TOPOB_RELIABLE,   FD_TOPOB_POLLED );
  ...
  if( 1 ) {
    ...
    fd_topob_tile_uses( topo, shrdcp_tile, replay_manifest_dcache, FD_SHMEM_JOIN_MODE_READ_ONLY );
    fd_topob_tile_in( topo, "shrdcp", 0UL, "metric_in", "snap_out", 0UL, FD_TOPOB_RELIABLE, FD_TOPOB_POLLED );
  }
