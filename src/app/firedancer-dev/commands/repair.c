...
repair_cmd_fn_catchup( args_t *   args,
  ...
  for(;;) {
    ...
    if( FD_UNLIKELY( now - last_print > 1e9L ) ) {
      print_tile_metrics( shred_metrics, repair_metrics, repair_net_links, net_shred_links, net_tile_cnt, &last_sent, last_print, now );
...
repair_cmd_fn_metrics( args_t *   args,
  ...
  for(;;) {
    ...
    if( FD_UNLIKELY( now - last_print_ts > 1e9L ) ) {
      print_tile_metrics( shred_metrics, repair_metrics, repair_net_links, net_shred_links, net_tile_cnt, &last_sent, last_print_ts, now );
      last_print_ts = now;
    }
  }
}
...
repair_cmd_fn_inflight( args_t *   args,
  ...
  ulong inflight_dlist_off = (ulong)inflights->dlist  - (ulong)repair_ctx->inflights;
