...
backtest_topo( config_t * config,
  ...
  if( FD_UNLIKELY( snapmk_enabled ) ) {
    ...
    backup_obj = fd_topob_obj( topo, "backup", "snapmk" );
    FD_TEST( fd_pod_insertf_ulong( topo->props, config->firedancer.accounts.max_accounts, "obj.%lu.max_accounts", backup_obj->id ) );
    FD_TEST( fd_pod_insert_ulong( topo->props, "backup", backup_obj->id ) );
  }
  ...
  if( FD_UNLIKELY( snapmk_enabled ) ) {
    ...
    fd_topob_tile_uses( topo, snapmk_tile, backup_obj,     FD_SHMEM_JOIN_MODE_READ_WRITE );
...
fixup_config( config_t *     config,
  ...
  backtest_topo( config, args->backtest.snapshot_slot );
}
