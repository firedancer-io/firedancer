...
during_frag( dump_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( fd_frag_meta_ctl_err( ctl ) ) ) return;
...
after_frag( dump_ctx_t *        ctx,
  ...
  /* This will happen if a frag is ignored due to the err bit.  The
     after_frag callback is still delivered in this case. */
...
dump_cmd_fn( args_t      * args,
  ...
  if( args->dump.once ) {
    ...
    ulong const fseq_footprint = fd_ulong_align_up( fd_fseq_footprint(), fd_fseq_align() );
    uchar * fseq_mem = fd_alloca( FD_FSEQ_ALIGN, ctx.link_cnt * fseq_footprint );
    ulong * fseqs[ FD_TOPO_MAX_LINKS ];
    for( ulong i=0UL; i<ctx.link_cnt; i++ ) {
      fseqs[ i ] = fd_fseq_join( fd_fseq_new( fseq_mem + (i * fseq_footprint), 0UL ) );
    }

    fd_rng_t * rng = fd_rng_join( fd_rng_new( fd_alloca( FD_RNG_ALIGN, fd_rng_footprint() ), (uint)fd_tickcount(), 0UL ) );
