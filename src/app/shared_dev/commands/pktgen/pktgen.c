...
static char const *
get_net_tile_name( char const * provider ) {
...
pktgen_topo( config_t * config ) {
  ...
  char const * net_tile_name = get_net_tile_name( config->net.provider );
...
get_net_stats( ulong volatile const * net_metrics[ FD_TOPO_MAX_TILES ],
  ...
  for( ulong kind_id=0UL; kind_id<net_tile_cnt; kind_id++ ) {
    ...
    if( 0==strcmp( provider, "xdp" ) ) {
      ...
      rx_ok_now     = metrics[ MIDX( COUNTER, MLX5, RX_PKT_CNT     ) ];
      rx_byte_now   = metrics[ MIDX( COUNTER, MLX5, RX_BYTES_TOTAL ) ];
      rx_drop_now   = metrics[ MIDX( COUNTER, MLX5, RX_CQE_ERR_CNT       ) ];
      /* */ rx_drop_now += metrics[ MIDX( COUNTER, MLX5, RX_OUT_OF_BUFFER_CNT ) ];
      /* */ rx_drop_now += metrics[ MIDX( COUNTER, MLX5, RX_MALFORMED_CNT     ) ];
      /* */ rx_drop_now += metrics[ MIDX( COUNTER, MLX5, RX_ROUTE_FAIL_CNT    ) ];
      tx_ok_now     = metrics[ MIDX( COUNTER, MLX5, TX_PKT_CNT     ) ];
      tx_byte_now   = metrics[ MIDX( COUNTER, MLX5, TX_BYTES_TOTAL ) ];
    }
...
pktgen_cmd_fn( args_t *   args FD_PARAM_UNUSED,
  ...
  for( ulong kind_id=0UL; kind_id<net_tile_cnt; kind_id++ ) {
    net_tiles[ kind_id ] = &topo->tiles[ fd_topo_find_tile( topo, get_net_tile_name( config->net.provider ), kind_id ) ];
