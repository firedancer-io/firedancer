...
pktgen_topo( config_t * config ) {
  ...
  fd_topos_net_tiles( topo, config->layout.net_tile_count, &config->net, config->tiles.netlink.max_routes, config->tiles.netlink.max_peer_routes, config->tiles.netlink.lock_cnt_peer_routes, config->tiles.netlink.max_neighbors, tile_to_cpu );
