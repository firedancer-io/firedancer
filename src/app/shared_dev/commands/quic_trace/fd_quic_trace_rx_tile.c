...
#       define EMPTY ((uchar[16]){0})
        ...
        FD_LOG_DEBUG(( "conn out of bounds in fd_quic_trace_initial for conn id %lu with key idx %u", conn_id, key_idx ));
      }
    } else {
    ...
    FD_LOG_DEBUG(( "Bogus destination connection id length %u for conn id %lu with key idx %u", (uint)initial->dst_conn_id_len, fd_ulong_load_8( initial->dst_conn_id ), key_idx ));
    return FD_QUIC_PARSE_FAIL;
  }

  if( !keys || memcmp( keys->pkt_key, EMPTY, sizeof( keys->pkt_key ) ) == 0 ) {
    /* if tracing client, we should always have keys here, so shouldn't enter this block */
    if( ctx->trace_send ) return FD_QUIC_PARSE_FAIL;
