...
flame_cmd_fn( args_t *   args,
  ...
  for( ulong i=0UL; i<tile_cnt; i++ ) {
    ...
    if( FD_UNLIKELY( -1==kill( (int)tid, 0 ) ) ) {
      ...
      if( FD_UNLIKELY( errno==ESRCH ) ) FD_LOG_ERR(( "tile %s%s:%lu%s is not running", fd_log_style_bold(), config->topo.tiles[ i ].name, config->topo.tiles[ i ].kind_id, fd_log_style_normal() ));
