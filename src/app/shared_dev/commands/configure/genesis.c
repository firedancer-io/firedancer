...
/* alpenglow_enable_features enables the alpenglow feature gate and the
   ...
   Two optional gates are deliberately left off.
   alpenglow_fast_leader_handover is not needed for a first cut of the
   leader path, and without it block creation just waits for
   ParentReady.  discard_unexpected_data_complete_shreds only tightens
   shred filtering and is independent of block production. */
