...
fd_frank_shredder_task( int     argc,
  ...
  float corruption_rate = (float)fd_pod_query_ulong( shredder_pod, "corrupt_pct", 0UL )/100.0f;
  ...
  FD_LOG_NOTICE(( "Transmitting on interface %s queue %d. Artificially corrupting %.2f%% of packets", fd_xsk_ifname( xsk ), fd_xsk_ifqueue( xsk ), 100.0*(double)corruption_rate ));
