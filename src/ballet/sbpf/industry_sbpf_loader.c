...
set_error( unsigned long * out_result_sz,
  ...
  assert( result_buf_sz >= sizeof(int) );

  int * res_out = (int *) out_result_buf;
  *res_out = -1;
  *out_result_sz = sizeof(int);
}

int
industry_test_one( ulong * out_result_sz,
                   uchar * out_result_buf,
                   ulong   result_buf_sz,
                   uchar * data,
                   ulong  data_sz ) {
  int retval = 0;
  ...
  /* - i32: 0 if unpack succeded, -1 if failed */
  uint * res_out = (uint *) &out_result_buf[cursor];
  cursor += sizeof(uint);
  *res_out = 0;

  /* - u64: len of rodata */
  ulong * len_rodata = (ulong *) &out_result_buf[cursor];
  cursor += sizeof(ulong);
  *len_rodata = prog->rodata_sz;
  ...
  /* - u64: entry_pc */
  ulong * entry_pc = (ulong *) &out_result_buf[cursor];
  cursor += sizeof(ulong);
  *entry_pc = prog->entry_pc;

  /* - i64: text_off */
  long * text_off = (long *) &out_result_buf[cursor];
  cursor += sizeof(long);
  *text_off = (uchar *)prog->text - (uchar *)prog->rodata;

  /* - u64: text_sz */
  ulong * text_sz = (ulong *) &out_result_buf[cursor];
  cursor += sizeof(ulong);
