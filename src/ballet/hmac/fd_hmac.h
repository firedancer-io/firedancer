...
/* fd_hkdf_extract computes HKDF-Extract.
   ...
   prk must point to a buffer of hash_sz bytes (32 for SHA-256, etc).
   Returns prk.
...
/* fd_hkdf_expand_label computes HKDF-Expand-Label.
   ...
   out_sz must be within [1, hash_sz]
   label_sz must be less than or equal to 64
   Returns out.
   ...
   Only single-expand (counter=1) is supported, as it is sufficinet for
