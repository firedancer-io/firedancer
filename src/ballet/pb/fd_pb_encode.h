...
fd_pb_lp_open( fd_pb_encoder_t * encoder,
  ...
  encoder->cur              = fd_pb_append_tag( encoder->cur, tag );
...
fd_pb_push_fixed32( fd_pb_encoder_t * encoder,
  ...
  uint tag = fd_pb_tag( FD_PB_WIRE_TYPE_I32, field_id );
  encoder->cur = fd_pb_append_tag    ( encoder->cur, tag   );
...
fd_pb_push_fixed64( fd_pb_encoder_t * encoder,
                    ...
                    ulong             value ) {
  if( FD_UNLIKELY( fd_pb_encoder_space( encoder ) <
                   (fd_pb_varint32_sz_max + sizeof(ulong)) ) ) {
    return NULL;
  }
  uint tag = fd_pb_tag( FD_PB_WIRE_TYPE_I64, field_id );
  encoder->cur = fd_pb_append_tag    ( encoder->cur, tag   );
...
#define FD_PB_PUSH_PACKED( type, ctype )                                  \
  FD_FN_UNUSED static fd_pb_encoder_t *                                   \
...
#define FD_PB_PUSH_PACKED_STATIC( type, ctype )                           \
  FD_FN_UNUSED static fd_pb_encoder_t *                                   \
  fd_pb_push_packed_##type( fd_pb_encoder_t * encoder,                    \
                            uint              field_id,                   \
                            ctype const *     values,                     \
    ...
    uint  tag     = fd_pb_tag( FD_PB_WIRE_TYPE_LEN, field_id );           \
    ulong data_sz = cnt*fd_pb_##type##_sz_max;                            \
    encoder->cur  = fd_pb_append_uint32 ( encoder->cur, tag      );       \
    encoder->cur  = fd_pb_append_uint64 ( encoder->cur, data_sz );        \
    /* write data */                                                      \
    if( FD_UNLIKELY( fd_pb_encoder_space( encoder ) < data_sz ) ) {       \
      return NULL;                                                        \
    }                                                                     \
    fd_memcpy( encoder->cur, values, data_sz );                           \
    encoder->cur += data_sz;                                              \
    return encoder;                                                       \
  }

FD_PB_PUSH_PACKED_STATIC( fixed32, uint  )
FD_PB_PUSH_PACKED_STATIC( fixed64, ulong )
