...
const char
    SIGNATURE_SUFFIX[FD_TXN_SIGNATURE_SZ - sizeof(ulong) - sizeof(uint)] =
...
/* Makes enough of a transaction to schedule that reads one account for
   ...
   in lamports. */
static ulong make_transaction(ulong i, uint compute, double priority,
                              char const *writes, char const *reads) {
  ...
  *(p++) = (uchar)1;
  ...
  ulong programs_to_include =
      2UL; /* 1 for compute budget, 1 for "work" program */
  t->readonly_unsigned_cnt = (uchar)(strlen(reads) + programs_to_include);
  t->acct_addr_cnt =
      (ushort)(1UL + strlen(reads) + programs_to_include + strlen(writes));
...
static void schedule_validate_microblock(fd_pack_t *pack, ulong total_cus,
                                         float vote_fraction, ulong min_txns,
                                         ulong min_rewards, ulong bank_tile,
                                         pack_outcome_t *outcome) {

  ulong pre_txn_cnt = fd_pack_avail_txn_cnt(pack);
  fd_pack_microblock_complete(pack, bank_tile);
  ulong txn_cnt = fd_pack_schedule_next_microblock(
      pack, total_cus, vote_fraction, bank_tile, outcome->results);
  ulong post_txn_cnt = fd_pack_avail_txn_cnt(pack);

#if DETAILED_STATUS_MESSAGES
  FD_LOG_NOTICE(("Scheduling microblock. %lu avail -> %lu avail. %lu scheduled",
                 pre_txn_cnt, post_txn_cnt, txn_cnt));
#endif
  ...
  for (ulong i = 0UL; i < txn_cnt; i++) {
    ...
    if (FD_LIKELY(txn->instr_cnt > 1UL)) {
      fd_txn_instr_t ix = txn->instr[0]; /* For these transactions, the compute
                                            budget instr is always the 1st */
      FD_TEST(fd_compute_budget_program_parse(txnp->payload + ix.data_off,
                                              ix.data_sz, &cbp));
      fd_compute_budget_program_finalize(&cbp, txn->instr_cnt, &rewards,
                                         &compute);
    } /* else it's a vote */
    ...
    for (ulong j = fd_txn_acct_iter_init(
             txn, FD_TXN_ACCT_CAT_WRITABLE_NONSIGNER_IMM, ctrl);
         j < fd_txn_acct_iter_end(); j = fd_txn_acct_iter_next(j, ctrl)) {
      uchar b0 = acct[j].b[0];
      uchar b1 = acct[j].b[1];
    ...
    for (ulong j = fd_txn_acct_iter_init(
             txn, FD_TXN_ACCT_CAT_READONLY_NONSIGNER_IMM, ctrl);
         j < fd_txn_acct_iter_end(); j = fd_txn_acct_iter_next(j, ctrl)) {
      uchar b0 = acct[j].b[0];
      uchar b1 = acct[j].b[1];
      if ((0x30UL <= b0) & (b0 < 0x70UL) & (b0 == b1))
  ...
  for (ulong i = 0UL; i < fd_pack_bank_tile_cnt(pack); i++) {
    ...
    FD_TEST(
        aset_is_null(aset_intersect(write_accts, outcome->r_accts_in_use[i])));
    FD_TEST(
        aset_is_null(aset_intersect(write_accts, outcome->w_accts_in_use[i])));
    FD_TEST(
        aset_is_null(aset_intersect(read_accts, outcome->w_accts_in_use[i])));
  }
...
int LLVMFuzzerTestOneInput(uchar const *data, ulong data_sz) {
  ...
  if (footprint > PACK_SCRATCH_SZ)
    FD_LOG_ERR(("Test required %lu bytes, but scratch was only %lu", footprint,
                PACK_SCRATCH_SZ));
#if DETAILED_STATUS_MESSAGES
  else
    FD_LOG_NOTICE(("Test required %lu bytes of %lu available bytes", footprint,
                   PACK_SCRATCH_SZ));
#endif

  void *_mem = fd_pack_new(pack_scratch, pack_depth, gap,
                           max_txn_per_microblock, MAX_TEST_TXNS, rng);
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 8; j++) {
        ...
        if (bit) {
          ...
          rewards += make_transaction(insert_idx, (uint)ptr[0], (double)ptr[1],
                                      acc1, acc2);
          ptr += 5;
          s -= 5;
          ...
          fd_ed25519_sig_t const *sig = fd_txn_get_signatures(
              (fd_txn_t *)txn_scratch[insert_idx], payload_scratch);
          fd_pack_delete_transaction(pack, sig);
        }
      }
      schedule_validate_microblock(pack, 30000UL, 0.0f, 1, rewards, 0UL,
                                   &outcome);
    }
  }
