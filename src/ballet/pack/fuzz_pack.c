...
#define CHAR_SET_START 0x30
#define CHAR_SET_END   0x70

void generate_random_string(fd_rng_t *rng, char *buffer, size_t length, char *prev_string) {
    int char_exists;
    char c;
    int used_chars[64] = {0};
    ...
    for (size_t i = 0; i < length; ++i) {
        do {
            uint random_offset = fd_rng_uint( rng ) % (64U);
            c = (char)(CHAR_SET_START + random_offset);
            char_exists = 0;
            for (size_t j = 0; j < strlen( prev_string ); ++j) {
                if (c == prev_string[j]) {
                    char_exists = 1;
                    break;
                }
            }
            if (!char_exists && used_chars[(int)(c - CHAR_SET_START)]) {
                char_exists = 1;
            }
        } while (char_exists);
        used_chars[(int)(c - CHAR_SET_START)] = 1;
        buffer[i] = c;
    }
    buffer[length] = '\0';
}
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 8; j++) {
        ...
        if (bit) {
          ...
          generate_random_string( rng, &account1[0], 10, "" );
          generate_random_string( rng, &account2[0], 10, account1 );
          ...
          if (firstThreeBytes[i] % 8 == 0) {
          // fd_pack_expire_before(pack, 100UL);
          } else {
      ...
      schedule_validate_microblock( pack, 30000UL, 0.0f, 1, rewards % 100, 0UL, &outcome );
    }
  }
