  ...
  for( treap_rev_iter_t _cur=treap_rev_iter_init( pack->pending_bundles, pack->pool ); !treap_rev_iter_done( _cur ); _cur=_next ) {
    ...
    if( FD_UNLIKELY( last_in_bundle ) ) {
      ...
      for( ulong j=0UL; j<i; j++ ) {
        delete_transaction( pack, fd_txn_get_signatures( TXN(bundle[j]->txn), bundle[j]->txn->payload ), 0, 0 );
        FD_TEST( trp_pool_ele_acquire( pack->pool )==bundle[j] );
      }

      rewriter( bundle, i, expires_at, ctx );

      insert_bundle_impl( pack, cur_bundle, i, bundle, *expires_at );
...
fd_pack_try_schedule_bundle( fd_pack_t * pack,
  ...
  /* At this point, we have our candidate bundle, so we'll schedule it
     if we can.  If we can't, we won't schedule anything.  It would be
     cleaner if we could check all the transactions, and then only try
     to schedule the whole bundle if they all pass, but the account
     write lock cost check is tricky because multiple transactions in
     the bundle can write to the same account.  Instead, we'll
     optimistically schedule the bundle and roll back any changes */
  ...
  if( FD_LIKELY( retval>0 ) ) {
    /* TODO: Schedule from _txn0 up to but excluding _cur */
