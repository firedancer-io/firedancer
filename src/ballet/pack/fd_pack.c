...
fd_pack_estimate_rewards_and_compute( fd_txn_p_t        * txnp,
  ...
  */
  ...
  out->txn->pack_cu.non_execution_cus       = (uint)cost - (uint)execution_cus;
...
#   endif
    ...
    if(
      ...
      FD_STATIC_ASSERT( offsetof(fd_txn_p_t, pack_cu.requested_execution_cus)+sizeof(((fd_txn_p_t*)NULL)->pack_cu.requested_execution_cus)<=1280UL, nt_memcpy );
      FD_STATIC_ASSERT( offsetof(fd_txn_p_t, pack_cu.non_execution_cus      )+sizeof(((fd_txn_p_t*)NULL)->pack_cu.non_execution_cus      )<=1280UL, nt_memcpy );
