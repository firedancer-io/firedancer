...
request( fd_http_server_request_t request ) {
  ...
  if( FD_LIKELY( request.method==FD_HTTP_SERVER_METHOD_GET ) ) {
    if( FD_LIKELY( request.headers.upgrade_websocket ) ) {
      ...
      fd_hcache_printf( state->hcache, state->server, "<!doctype html> <html lang=\"en\"> <head> <meta charset=\"utf-8\"> <title>Nothing</title> </head> <body> <h1>Hello, world!</h1> </body> </html>\r\n" );
    ...
    fd_hcache_printf( state->hcache, state->server, "{\"jsonrpc\": \"2.0\", \"result\": {\"absoluteSlot\": 166598, \"blockHeight\": 166500, \"epoch\": 27, \"slotIndex\": 2790, \"slotsInEpoch\": 8192, \"transactionCount\": 22661093}, \"id\": 1}\r\n" );
    ulong body_len     = body_len;
    uchar const * body = fd_hcache_snap_response( state->hcache, &body_len );
    FD_TEST( body );
...
main( int     argc,
  ...
  state.hcache = fd_hcache_join( fd_hcache_new( aligned_alloc( fd_hcache_align(), fd_hcache_footprint( 1<<16 ) ), 1<<16 ) );
