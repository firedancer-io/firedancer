...
/* fd_der.h provides a parser for DER (Distinguished Encoding Rules)
   ...
   success=0 & failure=nonzero.  On any parse error, the macro evalutes
   to "return -1".
...
#define FD_DER_TAG_UTF8_STRING      ((uchar)0x07)
...
fd_der_int_to_fixed( uchar const * p,
  ...
  if( FD_UNLIKELY( len == 0 || len > out_sz + 1 ) ) return -1;

  if( len == out_sz + 1 ) {
    if( FD_UNLIKELY( p[0] != 0x00 ) ) return -1;
    p++;
    len--;
  }
...
#define FD_DER_CURSOR_FROM_BUF( c, buf, sz )  \
  fd_der_cursor_t c = { .p = (buf), .end = (buf) + (sz) }
...
/* Read a primitive TLV. Verifies the tag and sets ptr/len to the
   content bytes. Advances the cursor past the TLV. */
...
/* Read a TLV including its tag+length prefix.  Sets raw_ptr to the
   start of the tag byte and raw_len to tag+length+content.
   Also sets content_{ptr,len} to just the content. */
