...
/* fd_x509_verify.h provides certificate chain verification.  Walks the
   ...
   Ensures:
   - Each cert's issuer matches the next cert's subject
   - Each cert's signature is verified against the issuer's public key
   - Intermediate certificates must have basicConstraints cA=TRUE
   - The root cert's issuer must be in the CA trust store
