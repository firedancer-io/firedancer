...
main( int     argc,
     ...
     filtering, capacity, and its bundle-size guard. */
  {
    ...
    FD_TEST( fd_x509_ca_store_load( &store, path )==1L );
    FD_TEST( store.cnt==1UL );
    FD_TEST( store.entries[0].key_type==FD_X509_KEY_ECDSA_P384 );
    FD_TEST( store.entries[0].pubkey_len==97UL );
     ...
     by an untrusted root mixed in. */
  {
    ...
    /* Noise before the intermediate */
    {
      ...
      FD_TEST( fd_x509_verify_chain( chain_der, chain_der_sz, 3UL, &store, NULL, 0UL, TEST_NOW )
               ==FD_X509_VERIFY_OK );
    }
