...
main( int     argc,
     ...
     stopping at the first match rejects a perfectly good chain. */
  {
    ...
    FD_TEST( fd_x509_verify_chain( chain_der, chain_der_sz, 1UL, &ca_store, NULL, 0UL )
             ==FD_X509_VERIFY_OK );
     ...
     to an older root we do not hold. */
  {
    ...
    FD_TEST( fd_x509_verify_chain( chain_der, chain_der_sz, 2UL, &ca_store, NULL, 0UL )
             ==FD_X509_VERIFY_OK );
     ...
     by an untrusted root mixed in. */
  {
    ...
    /* Same subject as inter and issued by the untrusted root, but with a
       different key, so it cannot have signed the leaf. */
    uchar pub_other[ 32 ]; memset( pub_other, 0x66, sizeof(pub_other) );
    uchar cross[ 1024 ];
    ulong cross_len = mk_cert_signed( cross, older_name, older_name_len,
                                      inter_name, inter_name_len,
                                      pub_other, prv_old, exts, exts_len );
    ...
    /* Noise before the intermediate */
    {
      ...
      FD_TEST( fd_x509_verify_chain( chain_der, chain_der_sz, 3UL, &ca_store, NULL, 0UL )
               ==FD_X509_VERIFY_OK );
    }

    /* The wrong-key cross-sign of the same subject comes first; the walk
    ...
    /* With only the wrong-key cross-sign on offer, its failure is what
  ...
  {
    memset( &ca_store, 0, sizeof(ca_store) );
    int err = fd_x509_verify_chain( NULL, NULL, 0UL, &ca_store, "example.com", 11UL );
  ...
  /* Test 15: Chain with garbage cert returns FD_X509_VERIFY_ERR_PARSE */
  {
    ...
    memset( &ca_store, 0, sizeof(ca_store) );
    int err = fd_x509_verify_chain( chain_der, chain_der_sz, 1UL, &ca_store, "example.com", 11UL );
