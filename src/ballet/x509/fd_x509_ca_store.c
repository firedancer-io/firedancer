...
fd_x509_ca_store_load( fd_x509_ca_store_t * store,
  ...
  while( p < end ) {
    ...
    if( info.key_type != FD_X509_KEY_ED25519 &&
        info.key_type != FD_X509_KEY_ECDSA_P256 &&
        info.key_type != FD_X509_KEY_ECDSA_P384 )            continue;
    ...
    if( info.name_constraints_permitted_len+
        info.name_constraints_excluded_len >
        FD_X509_CA_NAME_CONSTRAINTS_MAX )                    continue;
