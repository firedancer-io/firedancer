...
int fd_private_test_inclusion( fd_bmtree_commit_t * state );

uchar memory[ 128*1024 ];
...
test_inclusion( ulong leaf_cnt ) {
  ...
  for( ulong i=0UL; i<leaf_cnt; i++ ) {
    ...
    FD_TEST(  fd_bmtree_validate_inclusion_proof( leaf, i, root, inc_proof, depth-1UL, 20UL ) );
  ...
  fd_private_test_inclusion( tree );
}
