...
fd_bmtree_commit_init( void * mem,     /* Assumed unused with required alignment and footprint */
  ...
  fd_memset( state->inclusion_proofs, 0xDD, sizeof(fd_bmtree_node_t)*state->inclusion_proof_sz ); //FIXME: Remove
  return state;
}
...
fd_bmtree_validate_inclusion_proof( fd_bmtree_node_t const * leaf,
  ...
  return 0==memcmp( tmp, root, hash_sz );
}


/* FIXME: Delete this function when I'm confident in the proofs */
int fd_private_test_inclusion( fd_bmtree_commit_t * state ) {
  /* Indices 0, 2, 4, ..., 2*(leaf_cnt-1) correspond to leaf nodes, so
     there's nothing to check there. */
  for( ulong i=1UL; i<(1UL<<fd_bmtree_depth( state->leaf_cnt ))-1UL; i+=2UL ) {
    ulong layer = (ulong)fd_ulong_find_lsb( ~i );
    ulong left  = i-(1UL<<(layer-1UL));
    ulong right = i+(1UL<<(layer-1UL));

    ulong max_inc_idx_for_layer = fd_ulong_insert_lsb( (state->leaf_cnt - 1UL)<<1, 1+(int)layer, (1UL<<layer)-1UL );
    if( i>max_inc_idx_for_layer ) continue;

    if( i > 2UL*(state->leaf_cnt-1UL) ) right = left;
    fd_bmtree_node_t tmp[1];
    fd_bmtree_private_merge( tmp, &state->inclusion_proofs[ left ], &state->inclusion_proofs[ right ], state->hash_sz );
    FD_TEST( 0==memcmp( tmp, &state->inclusion_proofs[ i ], state->hash_sz ) );
  }
  return 0;
}
