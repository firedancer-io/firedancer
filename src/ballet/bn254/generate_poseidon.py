...
def render():
    chunks = [
        "/* Width-specific Poseidon parameters, for 2 <= FD_POSEIDON_WIDTH <= 13.",
        "",
        "   Generated by generate_poseidon.py.",
        "",
        "   All elements are in Montgomery form to avoid unnecessary conversions.",
        "   Only the branch selected by FD_POSEIDON_WIDTH is parsed by the C compiler.",
        "   fd_poseidon_mds is a width x width matrix flattened as a vector. */",
        "",
    ...
    for width, partial_rounds in PARTIAL_ROUNDS.items():
        ...
        chunks.append(
            f"#if FD_POSEIDON_WIDTH=={width}UL"
            if width == min(PARTIAL_ROUNDS)
            else f"#elif FD_POSEIDON_WIDTH=={width}UL"
        )
        chunks.append("")
        chunks.append(f"#define FD_POSEIDON_PARTIAL_ROUNDS ({partial_rounds}UL)")
        chunks.append("")
        chunks.append(c_array("fd_poseidon_mds", flatten(mds)))
        chunks.append(c_array("fd_poseidon_ark_start", flatten(start)))
        chunks.append(c_array("fd_poseidon_ark_partial", partial))
        chunks.append(c_array("fd_poseidon_ark_end", flatten(end)))
        chunks.append(c_array("fd_poseidon_pre_sparse_mds", flatten(pre_sparse)))
        chunks.append(c_array("fd_poseidon_sparse_mds_row", sparse_row))
        chunks.append(c_array("fd_poseidon_sparse_mds_col", sparse_col))

    chunks.extend(
        [
            "#else",
            '#error "unsupported FD_POSEIDON_WIDTH"',
            "#endif",
        ]
    )
    return "\n".join(chunks).rstrip() + "\n"
