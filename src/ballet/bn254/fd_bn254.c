...
fd_bn254_g2_add_syscall( uchar       out[128],
                         ...
                         ulong       in_sz,
                         int         big_endian ) {
  /* Expected 256-byte input (2 points). Pad input with 0s (only big endian). */
  if( FD_UNLIKELY( in_sz > 256UL ) ) {
    return -1;
  }
  if( FD_UNLIKELY( !big_endian && in_sz != 256UL ) ) {
    return -1;
  }
  ...
  if( FD_UNLIKELY( !fd_bn254_g2_frombytes_check_subgroup( b, &buf[128], big_endian ) ) ) {
    return -1;
  }
...
fd_bn254_g2_scalar_mul_syscall( uchar       out[128],
                                ...
                                ulong       in_sz,
                                int         big_endian ) {
  /* Expected 160-byte input (1 point + 1 scalar). Pad input with 0s (only big endian). */
  if( FD_UNLIKELY( in_sz > 160UL ) ) {
    return -1;
  }
  if( FD_UNLIKELY( !big_endian && in_sz != 160UL ) ) {
    return -1;
  }
  uchar FD_ALIGNED buf[160] = { 0 };
  fd_memcpy( buf, in, fd_ulong_min( in_sz, 160UL ) );
