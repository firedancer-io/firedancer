...
fd_bn254_Fq2_sol_to_native(uchar const * sol, libff::alt_bn128_Fq2 * X) {
  fd_bn254_Fq_sol_to_native(sol, &X->c1);
  fd_bn254_Fq_sol_to_native(sol + FD_BN254_FIELD_FOOTPRINT, &X->c0);
}

static void
fd_bn254_Fq2_native_to_sol(libff::alt_bn128_Fq2 const * X, uchar * sol) {
  fd_bn254_Fq_native_to_sol(&X->c1, sol);
  fd_bn254_Fq_native_to_sol(&X->c0, sol + FD_BN254_FIELD_FOOTPRINT);
}
...
fd_bn254_g2_check( fd_bn254_point_g2_t const * p ) {
  ...
  fd_bn254_Fq2_sol_to_native(p->v, &X);
  libff::alt_bn128_Fq2 Y;
  fd_bn254_Fq2_sol_to_native(p->v + 2U*FD_BN254_FIELD_FOOTPRINT, &Y);
  libff::alt_bn128_G2 g(X, Y, libff::alt_bn128_Fq2::one());
  return g.is_well_formed();
}
