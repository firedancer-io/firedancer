...
fd_bn254_g1_decompress( fd_bn254_point_g1_compressed const * in, fd_bn254_point_g1 * out ) {
  ...
  X2.squared();
  libff::alt_bn128_Fq X3_plub_b = X*X2 + libff::alt_bn128_coeff_b;
  libff::alt_bn128_Fq Y(X3_plub_b.sqrt());
...
fd_bn254_Fq2_sol_to_native(uchar const * sol, libff::alt_bn128_Fq2 * X) {
  FD_STATIC_ASSERT( sizeof(X->c0.mont_repr.data) + sizeof(X->c1.mont_repr.data) == 2U*FD_BN254_FIELD_FOOTPRINT, fd_ballet );
  /* Convert big-endian to little-endian while copying */
  uchar * t = (uchar *)X->c0.mont_repr.data;
  for (ulong i = 0; i < FD_BN254_FIELD_FOOTPRINT; ++i)
    t[FD_BN254_FIELD_FOOTPRINT-1U-i] = sol[i];
  t = (uchar *)X->c1.mont_repr.data;
  for (ulong i = 0; i < FD_BN254_FIELD_FOOTPRINT; ++i)
    t[FD_BN254_FIELD_FOOTPRINT-1U-i] = sol[i + FD_BN254_FIELD_FOOTPRINT];
}
...
fd_bn254_Fq2_native_to_sol(libff::alt_bn128_Fq2 const * X, uchar * sol) {
  FD_STATIC_ASSERT( sizeof(X->c0.mont_repr.data) + sizeof(X->c1.mont_repr.data) == 2U*FD_BN254_FIELD_FOOTPRINT, fd_ballet );
  /* Convert little-endian to big-endian while copying */
  const uchar * t = (const uchar *)X->c0.mont_repr.data;
  for (ulong i = 0; i < FD_BN254_FIELD_FOOTPRINT; ++i)
    sol[i] = t[FD_BN254_FIELD_FOOTPRINT-1U-i];
  t = (const uchar *)X->c1.mont_repr.data;
  for (ulong i = 0; i < FD_BN254_FIELD_FOOTPRINT; ++i)
    sol[i + FD_BN254_FIELD_FOOTPRINT] = t[FD_BN254_FIELD_FOOTPRINT-1U-i];
}
...
fd_bn254_g2_decompress( fd_bn254_point_g2_compressed const * in, fd_bn254_point_g2 * out ) {
  ...
  X2.squared();
  libff::alt_bn128_Fq2 X3_plub_b = X*X2 + libff::alt_bn128_twist_coeff_b;
  libff::alt_bn128_Fq2 Y(X3_plub_b.sqrt());
