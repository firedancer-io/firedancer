...
/* Returns 1 if the Jacobian points a and b represent the same curve point.
   Both points have their coordinates in the Montgomery domain. */
static inline int
fd_secp384r1_point_eq( fd_secp384r1_point_t const * a,
                       ulong                const   b[ 18 ] ) {
  int a_is_zero = fd_uint384_is_zero( a->z );
  int b_is_zero = !(b[12] | b[13] | b[14] | b[15] | b[16] | b[17]);

  if( FD_UNLIKELY( a_is_zero ) ) return b_is_zero;
  if( FD_UNLIKELY( b_is_zero ) ) return 0;

  fd_secp384r1_fp_t z1z1[1], z2z2[1], lhs[1], rhs[1];
  bignum_montsqr_p384( z1z1->limbs, a->z->limbs );
  bignum_montsqr_p384( z2z2->limbs, b+12 );

  /* Compare X1*Z2^2 and X2*Z1^2. */
  bignum_montmul_p384( lhs->limbs, a->x->limbs, z2z2->limbs );
  bignum_montmul_p384( rhs->limbs, b,           z1z1->limbs );
  if( FD_LIKELY( !fd_uint384_eq( lhs, rhs ) ) ) return 0;

  /* Compare Y1*Z2^3 and Y2*Z1^3. */
  bignum_montmul_p384( z1z1->limbs, z1z1->limbs, a->z->limbs );
  bignum_montmul_p384( z2z2->limbs, z2z2->limbs, b+12 );
  bignum_montmul_p384( lhs->limbs, a->y->limbs, z2z2->limbs );
  bignum_montmul_p384( rhs->limbs, b+6,         z1z1->limbs );
...
fd_secp384r1_double_scalar_mul_base( fd_secp384r1_point_t        * r,
  ...
  /* u1*G + u2*A */
  if( FD_UNLIKELY( fd_secp384r1_point_eq( r, u1G ) ) ) {
    p384_montjdouble( (ulong *)r, (ulong *)r );
  } else {
