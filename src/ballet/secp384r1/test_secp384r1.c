...
main( int     argc,
  ...
  /* Test 1: invalid signature (all zeros) must be rejected */
  {
    ...
    FD_TEST( fd_secp384r1_verify_allow_high_s( (uchar const *)"x", 1, zero_sig, zero_pub, sha ) == FD_SECP384R1_FAILURE );
    FD_LOG_INFO(( "OK: invalid signatures rejected" ));
  }

  /* Test 2: valid signature must verify (no low-S check) */
  {
    int result = fd_secp384r1_verify_allow_high_s( test_msg, TEST_MSG_SZ, test_sig, test_pubkey, sha );
    ...
    FD_LOG_INFO(( "OK: valid ECDSA-P384 signature verified (allow_high_s)" ));
  }
  ...
  /* Test 3: corrupted signature must fail */
  {
    ...
    bad_sig[0] ^= 0x01;
    FD_TEST( fd_secp384r1_verify_allow_high_s( test_msg, TEST_MSG_SZ, bad_sig, test_pubkey, sha ) == FD_SECP384R1_FAILURE );
  ...
  /* Test 4: wrong message must fail */
  {
    ...
    FD_TEST( fd_secp384r1_verify_allow_high_s( wrong_msg, sizeof(wrong_msg)-1, test_sig, test_pubkey, sha ) == FD_SECP384R1_FAILURE );
  ...
  /* Test 5: wrong pubkey must fail */
  {
    ...
    bad_pub[1] ^= 0x01;
    FD_TEST( fd_secp384r1_verify_allow_high_s( test_msg, TEST_MSG_SZ, test_sig, bad_pub, sha ) == FD_SECP384R1_FAILURE );
