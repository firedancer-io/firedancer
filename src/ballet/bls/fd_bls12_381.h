...
/* fd_bls12_381_g2_compress_syscall compresses the G2 point `a` into
   `r`, the inverse of fd_bls12_381_g2_decompress_syscall.
   Input and output are expected to be big endian if big_endian==1,
   or little endian if big_endian==0.
   The function returns 0 on success, -1 if `a` is not a G2 point. */
int
fd_bls12_381_g2_compress_syscall( uchar       r[ 48*2 ], /* Compressed G2 point */
