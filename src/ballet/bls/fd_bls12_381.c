...
int
fd_bls12_381_g2_compress_syscall( uchar       _r[ 48*2 ],
                                  uchar const _a[ 96*2 ],
                                  int         big_endian ) {
  /* blst expects input in big endian. if little endian, bswap. */
  ulong be[ 96*2/sizeof(ulong) ];
  ...
  if( FD_UNLIKELY( !blst_p2_affine_in_g2( a ) ) ) {
    return -1;
  }
  blst_p2_affine_compress( _r, a );

  /* blst output is big endian.  If we want little endian, reverse the
     whole 96 byte compressed point as one number, which is exactly what
     fd_bls12_381_g2_decompress_syscall undoes on the way in. */
  if( !big_endian ) {
    ulong e[ 48*2/sizeof(ulong) ];
    memcpy( e, _r, 48*2 );
    fd_ulong_n_bswap( e, 6*2 );
    memcpy( _r, e, 48*2 );
  }
  return 0;
}
