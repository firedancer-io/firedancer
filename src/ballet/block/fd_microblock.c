...
fd_microblock_mixin( fd_microblock_t const * block,
  ...
  fd_bmtree_commit_t * tree = fd_bmtree_commit_init( commit, 32UL, FD_BMTREE_SHORT_PREFIX_SZ, 0UL );
  ...
  for( ulong i=0; i<txn_cnt; i++ ) {
    ...
    for ( ulong j = 0; j < txn->signature_cnt; j++ ) {
      ...
      fd_bmtree_hash_leaf( &leaf, &sigs[j], sizeof(fd_ed25519_sig_t), FD_BMTREE_SHORT_PREFIX_SZ );
