...
/* FIPS 197, Appendix A.2 and A.3. */

static uchar const __attribute__((aligned(16UL)))
fixture_key_expansion_192[ 208 ] = {
...
static void
test_aes_gcm_key_expansion( ulong         key_sz,
                            uchar const * expected_enc,
                            ulong         expected_enc_sz ) {
  uchar key[ 32 ];
  for( ulong i=0UL; i<sizeof(key); i++ ) key[i] = (uchar)i;

  uchar const iv[ 12 ] = {0};
  fd_aes_gcm_t gcm[1] __attribute__((aligned(FD_AES_GCM_ALIGN)));
  fd_aes_gcm_init( gcm, key, key_sz, iv );

  ulong nr = (key_sz>>2) + 6UL;
  FD_TEST( expected_enc_sz==(nr+1UL)*16UL );
  FD_TEST( gcm->key.key_sz==(uint)key_sz );
  FD_TEST( 0==memcmp( gcm->key.key_enc, expected_enc, expected_enc_sz ) );

  fd_aes_key_ref_t expected_dec[1];
  fd_aes_set_decrypt_key( key, key_sz<<3, expected_dec );
  FD_TEST( 0==memcmp( gcm->key.key_dec, expected_dec->rd_key, expected_enc_sz ) );

  FD_LOG_INFO(( "OK: AES-%lu GCM key expansion (AES-NI)", key_sz<<3 ));
}
...
static void
test_aes_192_gcm( void ) {
  /* NIST GCMVS, 96-bit IV, one-block plaintext, no AAD. */
  uchar const key       [ 24 ] = {0};
  uchar const iv        [ 12 ] = {0};
  uchar const plaintext [ 16 ] = {0};
  uchar const ciphertext[ 16 ] = { 0x98,0xe7,0x24,0x7c,0x07,0xf0,0xfe,0x41,
                                   0x1c,0x26,0x7e,0x43,0x84,0xb0,0xf6,0x00 };
  uchar const tag       [ 16 ] = { 0x2f,0xf5,0x8d,0x80,0x03,0x39,0x27,0xab,
                                   0x8e,0xf4,0xd4,0x58,0x75,0x14,0xf0,0xfb };

  uchar ct[ 16 ], t[ 16 ], pt[ 16 ];
  fd_aes_gcm_t gcm[1];

  fd_aes_gcm_init( gcm, key, sizeof(key), iv );
  fd_aes_gcm_encrypt( gcm, ct, plaintext, sizeof(plaintext), NULL, 0UL, t );
  FD_TEST( 0==memcmp( ct, ciphertext, sizeof(ciphertext) ) );
  FD_TEST( 0==memcmp( t,  tag,        sizeof(tag)        ) );

  fd_aes_gcm_init( gcm, key, sizeof(key), iv );
  FD_TEST( fd_aes_gcm_decrypt( gcm, ciphertext, pt, sizeof(ciphertext), NULL, 0UL, tag ) );
  FD_TEST( 0==memcmp( pt, plaintext, sizeof(plaintext) ) );

  FD_LOG_INFO(( "OK: AES-192-GCM encrypt/decrypt" ));
}
...
# if FD_HAS_AESNI
  test_aes_gcm_key_expansion( 24UL, fixture_key_expansion_192, sizeof(fixture_key_expansion_192) );
  test_aes_gcm_key_expansion( 32UL, fixture_key_expansion_256, sizeof(fixture_key_expansion_256) );
# endif
