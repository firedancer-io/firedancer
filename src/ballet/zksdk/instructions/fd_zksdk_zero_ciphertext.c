...
fd_zksdk_verify_proof_zero_ciphertext(
  ...
  */
  ...
  /* Validate all inputs. Agave does decompression and validation in various places,
     but any failure simply results in an invalid proof. */
  uchar scalars[ 5 * 32 ];
  ...
  /* https://github.com/solana-program/zk-elgamal-proof/blob/zk-sdk%40v5.0.0/zk-sdk/src/sigma_proofs/zero_ciphertext.rs#L104-L108
     ...
     The representation of the identity point in Ristretto is unique so we can
     check the compressed buffer before decompressing it. */
