...
/* fd_shredder_count_{data_shreds, parity_shreds, fec_sets}: returns the
   ...
   multiple of the default FEC set total data size of 31200, then we
   make sz_byte/31200 FEC sets, where each FEC set has 32 data shreds,
   and each data shred contains 975 bytes of payload.  Otherwise, we
   make 31200 B FEC sets until we have less than 62400 bytes left, and
   ...
   payload_bytes_per_shred = 1095 - 20*ceiling( log2( num_shreds ) )
...
#define NORMAL_FEC_SET_PAYLOAD_SZ (31200UL)
FD_FN_CONST static inline ulong
fd_shredder_count_fec_sets(      ulong sz_bytes ) {
  /* if sz_bytes < 2*31200, we make 1 FEC set.  If sz_bytes is a
     multiple of 31200, we make exactly sz_bytes/31200 sets.  Otherwise,
     we make floor(sz_bytes/31200)-1 normal set + one odd-sized set.
...
fd_shredder_count_data_shreds(   ulong sz_bytes ) {
  ...
  if(      FD_UNLIKELY( remaining_bytes <=  8955UL ) ) shreds += fd_ulong_max( 1UL, (remaining_bytes + 994UL)/995UL );
  else if( FD_LIKELY(   remaining_bytes <= 31200UL ) ) shreds +=                    (remaining_bytes + 974UL)/975UL;
  else if( FD_LIKELY(   remaining_bytes <= 61120UL ) ) shreds +=                    (remaining_bytes + 954UL)/955UL;
  else                                                 shreds +=                    (remaining_bytes + 934UL)/935UL;
  return shreds;
}
FD_FN_CONST static inline ulong
fd_shredder_count_parity_shreds( ulong sz_bytes ) {
  ...
  ulong remaining_bytes = sz_bytes - normal_sets * NORMAL_FEC_SET_PAYLOAD_SZ;
  ulong shreds = normal_sets * 32UL;
  if(      FD_UNLIKELY( remaining_bytes <=  8955UL ) ) shreds += fd_shredder_data_to_parity_cnt[ fd_ulong_max( 1UL, (remaining_bytes + 994UL)/995UL ) ];
  else if( FD_LIKELY(   remaining_bytes <= 31200UL ) ) shreds += fd_shredder_data_to_parity_cnt[                    (remaining_bytes + 974UL)/975UL   ];
  else if( FD_LIKELY(   remaining_bytes <= 61120UL ) ) shreds +=                                                    (remaining_bytes + 954UL)/955UL;
  else                                                 shreds +=                                                    (remaining_bytes + 934UL)/935UL;
  return shreds;
}
