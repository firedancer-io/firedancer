...
fd_shredder_next_fec_set( fd_shredder_t * shredder,
  ...
  ulong tree_depth              = fd_bmtree_depth( data_shred_cnt+parity_shred_cnt );
  ...
  for( ulong i=0UL; i<data_shred_cnt; i++ ) {
    ...
    shred->signature[ FD_ED25519_SIG_SZ-1UL ] = (uchar)0;
  ...
  for( ulong i=0UL; i<data_shred_cnt; i++ )
    fd_sha256_batch_add( sha256, data_shreds[i]+sizeof(fd_ed25519_sig_t)-1UL,   data_merkle_sz+1UL,   leaves[i].hash );
  for( ulong j=0UL; j<parity_shred_cnt; j++ )
    fd_sha256_batch_add( sha256, parity_shreds[j]+sizeof(fd_ed25519_sig_t)-1UL, parity_merkle_sz+1UL, leaves[j+data_shred_cnt].hash );
  fd_sha256_batch_fini( shredder->sha256 );


  /* Generate Merkle Proofs */
  fd_bmtree_commit_t * bmtree = fd_bmtree_commit_init( shredder->_bmtree_footprint, FD_SHRED_MERKLE_NODE_SZ, tree_depth );
  fd_bmtree_commit_append( bmtree, leaves, data_shred_cnt+parity_shred_cnt );
  uchar * root = fd_bmtree_commit_fini( bmtree );

  /* Sign Merkle Root */
  fd_ed25519_sign( root_signature, root, FD_SHRED_MERKLE_NODE_SZ, signing_public_key, signing_private_key, shredder->sha512 );
  ...
  for( ulong i=0UL; i<data_shred_cnt; i++ ) {
    ...
    fd_memcpy( merkle, root, FD_SHRED_MERKLE_NODE_SZ );
    fd_bmtree_get_inclusion_proof( bmtree, merkle+FD_SHRED_MERKLE_NODE_SZ, i );
  }
  ...
  for( ulong j=0UL; j<parity_shred_cnt; j++ ) {
    ...
    fd_memcpy( merkle, root, FD_SHRED_MERKLE_NODE_SZ );
    fd_bmtree_get_inclusion_proof( bmtree, merkle+FD_SHRED_MERKLE_NODE_SZ, data_shred_cnt+j );
  }
