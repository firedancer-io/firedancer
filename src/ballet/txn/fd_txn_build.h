...
/* fd_txn_build.h is an API to procedurally build Solana transactions
   from instructions (program invocations). */
...
struct fd_txn_builder {
  ...
  /* ALUT */
  uchar alut_i; /* current ALUT account, UCHAR_MAX if none open */
  ...
  uchar           acct_rev[ FD_TXN_ACCT_ADDR_MAX ];
  uchar           acct_cnt;
...
FD_FN_UNUSED static fd_txn_builder_t *
...
fd_txn_build_p( fd_txn_builder_t * builder,
                fd_txn_p_t *       out ) {
  uint payload_sz = fd_txn_build( builder, out->payload, TXN( out ), NULL );
  out->payload_sz = (ushort)payload_sz;
  return payload_sz;
}
