...
void txn1_correctness( void ) {
  ...
  ulong out_sz = fd_txn_parse( transaction1, transaction1_sz, out_buf, &counters, NULL );
  FD_TEST( out_sz );
...
void txn2_correctness( void ) {
  ...
  fd_txn_t * parsed = (fd_txn_t *)out_buf;
  ulong out_sz = fd_txn_parse( transaction2, transaction2_sz, out_buf, &counters, NULL );
  FD_TEST( out_sz );
  FD_TEST( counters.success_cnt==1UL );
  FD_TEST( counters.failure_cnt==0UL );
...
void test_mutate( uchar const * orig_payload,
                  ulong         len ) {
  /* Copy payload to mutable buffer. */
  uchar payload[ 1232UL ]; /* TODO hardcoded constant */
  FD_TEST( len<=sizeof(payload) );
