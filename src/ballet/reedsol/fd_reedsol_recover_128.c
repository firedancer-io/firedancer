...
fd_reedsol_private_recover_var_128( ulong           shred_sz,
  ...
  fd_reedsol_private_gen_pi_128( _erased, pi );
  ...
  #define STORE_COMPARE( n, var ) do{                                                         \
    ...
    switch( fd_ulong_min( shred_cnt, 128UL ) ) {
    ...
    if( shreds_remaining>0UL ) {
      ...
      shreds_remaining -= fd_ulong_min( shreds_remaining, 128UL );
    }
