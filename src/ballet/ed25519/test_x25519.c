...
static const fd_x25519_test_vector_t test_x25519_vector[] = {
  ...
  /* Bug avx512 not reducing input */
  {
    .self = {
      88, 132, 168, 29, 121, 189, 192, 139, 226, 38, 152, 5, 74, 213, 131, 19, 174, 89, 135, 6, 193, 73, 207, 253, 199, 70, 73, 115, 62, 39, 207, 84
    },
    .peer = {
      111, 4, 137, 49, 214, 46, 178, 60, 232, 138, 250, 70, 99, 4, 0, 0, 150, 204, 19, 222, 15, 156, 206, 193, 213, 146, 25, 84, 6, 73, 26, 114
    },
    .secret = {
      185, 124, 240, 172, 204, 173, 3, 84, 178, 39, 94, 143, 209, 238, 2, 124, 147, 105, 159, 53, 78, 11, 46, 204, 22, 112, 49, 173, 181, 107, 231, 83
    }
  }
};
...
test_wycheproofs( void ) {
  ...
  for( fd_x25519_verify_wycheproof_t const * proof = x25519_verify_wycheproofs;
    ...
    int actual = ( fd_x25519_exchange( shared, proof->prv, proof->pub )
                     != NULL );
    FD_CHECK_ERR( actual == proof->ok, fd_cstr_printf( cstr, 128UL, NULL, "fd_x25519_exchange_wycheproof id=%u", proof->tc_id ) );
    if (proof->ok) {
      FD_CHECK_ERR( fd_memeq( shared, proof->shared, 32 ), fd_cstr_printf( cstr, 128UL, NULL, "fd_x25519_exchange_wycheproof id=%u (shared)", proof->tc_id ) );
    }
  }
