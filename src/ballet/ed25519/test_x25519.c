...
test_wycheproofs( void ) {
  ...
  for( fd_x25519_verify_wycheproof_t const * proof = x25519_verify_wycheproofs;
    ...
    int actual = ( fd_x25519_exchange( shared, proof->prv, proof->pub )
                     != NULL );
    FD_TEST_CUSTOM( actual == proof->ok, "fd_x25519_exchange_wycheproof id=%u", proof->tc_id );
    if (proof->ok) {
      FD_TEST_CUSTOM( fd_memeq( shared, proof->shared, 32 ), "fd_x25519_exchange_wycheproof id=%u (shared)", proof->tc_id );
    }
  }
