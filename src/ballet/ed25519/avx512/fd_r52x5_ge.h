...
#define QP_SQR( D, S )    do { FD_R52X5_QUAD_SQR_FAST( D, S );    FD_R52X5_QUAD_REDUCE( D, D ); } while(0)
#define QP_MUL( D, A, B ) do { FD_R52X5_QUAD_MUL_FAST( D, A, B ); FD_R52X5_QUAD_REDUCE( D, D ); } while(0)

#define FD_R52X5_POW22523_QUAD( D, A ) do {            \
    FD_R52X5_QUAD_DECL( _t0 );                         \
    FD_R52X5_QUAD_DECL( _t1 );                         \
    FD_R52X5_QUAD_DECL( _t2 );                         \
    QP_SQR( _t0, A );                                  \
    QP_SQR( _t1, _t0 );                                \
    for( int _i=1; _i<  2; _i++ ) QP_SQR( _t1, _t1 );  \
    QP_MUL( _t1, A,  _t1 );                            \
    QP_MUL( _t0, _t0, _t1 );                           \
    QP_SQR( _t0, _t0 );                                \
    QP_MUL( _t0, _t1, _t0 );                           \
    QP_SQR( _t1, _t0 );                                \
    for( int _i=1; _i<  5; _i++ ) QP_SQR( _t1, _t1 );  \
    QP_MUL( _t0, _t1, _t0 );                           \
    QP_SQR( _t1, _t0 );                                \
    for( int _i=1; _i< 10; _i++ ) QP_SQR( _t1, _t1 );  \
    QP_MUL( _t1, _t1, _t0 );                           \
    QP_SQR( _t2, _t1 );                                \
    for( int _i=1; _i< 20; _i++ ) QP_SQR( _t2, _t2 );  \
    QP_MUL( _t1, _t2, _t1 );                           \
    QP_SQR( _t1, _t1 );                                \
    for( int _i=1; _i< 10; _i++ ) QP_SQR( _t1, _t1 );  \
    QP_MUL( _t0, _t1, _t0 );                           \
    QP_SQR( _t1, _t0 );                                \
    for( int _i=1; _i< 50; _i++ ) QP_SQR( _t1, _t1 );  \
    QP_MUL( _t1, _t1, _t0 );                           \
    QP_SQR( _t2, _t1 );                                \
    for( int _i=1; _i<100; _i++ ) QP_SQR( _t2, _t2 );  \
    QP_MUL( _t1, _t2, _t1 );                           \
    QP_SQR( _t1, _t1 );                                \
    for( int _i=1; _i< 50; _i++ ) QP_SQR( _t1, _t1 );  \
    QP_MUL( _t0, _t1, _t0 );                           \
    QP_SQR( _t0, _t0 );                                \
    for( int _i=1; _i<  2; _i++ ) QP_SQR( _t0, _t0 );  \
    QP_MUL( D, _t0, A );                               \
  } while(0)
...
FD_FN_UNUSED static int
fd_r52x5_ge_decode2( wl_t * _Pa0, wl_t * _Pa1, wl_t * _Pa2, wl_t * _Pa3, wl_t * _Pa4,
                     uchar const bufa[ 32 ],
                     wl_t * _Pb0, wl_t * _Pb1, wl_t * _Pb2, wl_t * _Pb3, wl_t * _Pb4,
                     uchar const bufb[ 32 ] ) {
  fd_f25519_t  ya[1],  ua[1],  va[1], ysqa[1], v2a[1], v3a[1], v4a[1], uv3a[1], uv7a[1];
  fd_f25519_t  yb[1],  ub[1],  vb[1], ysqb[1], v2b[1], v3b[1], v4b[1], uv3b[1], uv7b[1];
  fd_f25519_t  xa[1], x2a[1], vx2a[1], t1a[1], t2a[1], t3a[1], xya[1], neg_xa[1];
  fd_f25519_t  xb[1], x2b[1], vx2b[1], t1b[1], t2b[1], t3b[1], xyb[1], neg_xb[1];

  int x_0a = bufa[31] >> 7;
  int x_0b = bufb[31] >> 7;

  uchar buf2a[32]; fd_memcpy( buf2a, bufa, 32UL ); buf2a[31] &= 0x7f;
  uchar buf2b[32]; fd_memcpy( buf2b, bufb, 32UL ); buf2b[31] &= 0x7f;
  fd_f25519_frombytes( ya, buf2a );
  fd_f25519_frombytes( yb, buf2b );

  /* Recover x from x^2 = (y^2 - 1) / (d y^2 + 1). */
  fd_f25519_sqr2( ysqa, ya,  ysqb, yb );
  fd_f25519_sub ( ua, ysqa, fd_f25519_one );
  fd_f25519_sub ( ub, ysqb, fd_f25519_one );
  fd_f25519_mul2( va, fd_f25519_d, ysqa,  vb, fd_f25519_d, ysqb );
  fd_f25519_add ( va, va, fd_f25519_one );
  fd_f25519_add ( vb, vb, fd_f25519_one );

  /* Compute the candidate root x = u v^3 (u v^7)^((p-5)/8). */
  fd_f25519_sqr2( v2a, va,   v2b, vb  );
  fd_f25519_sqr2( v4a, v2a,  v4b, v2b );
  fd_f25519_mul2( v3a, va, v2a,  v3b, vb, v2b );
  fd_f25519_mul2( uv3a, ua, v3a,  uv3b, ub, v3b );
  fd_f25519_mul2( uv7a, uv3a, v4a,  uv7b, uv3b, v4b );

  /* Pack uv7a and uv7b into lanes 0,1 of a QUAD and raise both to
     (2^252-3). Lanes 2,3 are ignored. */
  {
    long ea[8] FD_F25519_ALIGN; wwl_st( ea, uv7a->el );
    long eb[8] FD_F25519_ALIGN; wwl_st( eb, uv7b->el );
    FD_R52X5_QUAD_DECL( _Quv );
    _Quv0 = wl( ea[0], eb[0], ea[0], eb[0] );
    _Quv1 = wl( ea[1], eb[1], ea[1], eb[1] );
    _Quv2 = wl( ea[2], eb[2], ea[2], eb[2] );
    _Quv3 = wl( ea[3], eb[3], ea[3], eb[3] );
    _Quv4 = wl( ea[4], eb[4], ea[4], eb[4] );
    FD_R52X5_QUAD_DECL( _Qx );
    FD_R52X5_POW22523_QUAD( _Qx, _Quv );
    xa->el = wwl( wl_extract( _Qx0, 0 ), wl_extract( _Qx1, 0 ), wl_extract( _Qx2, 0 ),
                  wl_extract( _Qx3, 0 ), wl_extract( _Qx4, 0 ), 0L, 0L, 0L );
    xb->el = wwl( wl_extract( _Qx0, 1 ), wl_extract( _Qx1, 1 ), wl_extract( _Qx2, 1 ),
                  wl_extract( _Qx3, 1 ), wl_extract( _Qx4, 1 ), 0L, 0L, 0L );
  }
  fd_f25519_mul2( xa, xa, uv3a,  xb, xb, uv3b );

  /* Check v x^2 = u.  If v x^2 = -u, multiply by sqrt(-1). */
  fd_f25519_sqr2( x2a, xa,  x2b, xb );
  fd_f25519_mul2( vx2a, va, x2a,  vx2b, vb, x2b );
  fd_f25519_sub( t1a, vx2a, ua ); fd_f25519_add( t2a, vx2a, ua );
  fd_f25519_sub( t1b, vx2b, ub ); fd_f25519_add( t2b, vx2b, ub );
  int t1nza = fd_f25519_is_nonzero( t1a ); int t2nza = fd_f25519_is_nonzero( t2a );
  int t1nzb = fd_f25519_is_nonzero( t1b ); int t2nzb = fd_f25519_is_nonzero( t2b );
  int erra = ( t1nza & t2nza );
  int errb = ( t1nzb & t2nzb );
  if( FD_UNLIKELY( erra ) ) {
    FD_R52X5_GE_ZERO( *_Pb );
    return -1;
  }
  if( FD_UNLIKELY( errb ) ) {
    FD_R52X5_GE_ZERO( *_Pb );
  ...
  fd_f25519_if( t3a, t1nza, fd_f25519_sqrtm1, fd_f25519_one );
  fd_f25519_if( t3b, t1nzb, fd_f25519_sqrtm1, fd_f25519_one );
  fd_f25519_mul2( xa, xa, t3a,  xb, xb, t3b );

  /* Select the root with the requested sign bit. */
  int x_mod_2a = fd_f25519_sgn( xa );
  int x_mod_2b = fd_f25519_sgn( xb );
  fd_f25519_neg( neg_xa, xa );
  fd_f25519_neg( neg_xb, xb );
  fd_f25519_if( xa, x_0a != x_mod_2a, neg_xa, xa );
  fd_f25519_if( xb, x_0b != x_mod_2b, neg_xb, xb );

  /* Return the decoded points (x,y). */
  fd_f25519_mul2( xya, xa, ya,  xyb, xb, yb );

  {
    FD_R52X5_QUAD_PACK( *_Pa, xa->el, ya->el, fd_f25519_one->el, xya->el );
    FD_R52X5_QUAD_PACK( *_Pb, xb->el, yb->el, fd_f25519_one->el, xyb->el );
  }
  return 0;
}

/* FD_R52X5_GE_DECODE2(Pa,sa,Pb,sb) decodes two compressed points. */
#define FD_R52X5_GE_DECODE2( Pa, sa, Pb, sb )                  \
    fd_r52x5_ge_decode2( &Pa##0, &Pa##1, &Pa##2, &Pa##3, &Pa##4, (sa), \
                         &Pb##0, &Pb##1, &Pb##2, &Pb##3, &Pb##4, (sb) )
