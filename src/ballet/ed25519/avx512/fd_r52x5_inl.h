...
/* D = 32*p - S.
   A lazy negation, the 32*p bias keeps all limbs.
   We use 32*p instead of 16*p here as r52x5_quad_mul_fast can reach
   ~2^55, which would underflow 16*p.  32*p (ceiling 2^56-32) leaves enough
   headroom above the worst case. */
#define FD_R52X5_QUAD_NEGATE_LAZY( D, S ) do { \
    wl_t _lo = wl_bcast( 0xFFFFFFFFFFFDA0 );   \
    wl_t _hi = wl_bcast( 0xFFFFFFFFFFFFE0 );   \
    D##0 = wl_sub( _lo, S##0 );                \
    D##1 = wl_sub( _hi, S##1 );                \
    D##2 = wl_sub( _hi, S##2 );                \
    D##3 = wl_sub( _hi, S##3 );                \
...
/* Weak carry propagation modulo p.
