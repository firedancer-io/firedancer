...
main( int     argc,
  ...
  FD_LOG_NOTICE(( "Testing MSM cached-add witness" ));
  do {
    static char const * const witness_hex[4] = {
      "28d756c7782a0af9aade336470d3b57f9ec06c4cea9e7972fda6714c57f9d7b4",
      "cf969e9f0bb78a152f2832c21677c8dcba6e45ab8b93998161a8642e1c0677b9",
      "830f317e82904ccea97d99d53c4ef5effbad52cd88a8e5d48fb1a12e72bb5bbd",
      "fcf22d262b0f5f58433f895b17742027154bce171a73036fb0367de9df925ccd"
    };
    ulong const witness_pos[4] = { 0UL, 1UL, 32UL, 33UL };

#   define MSM_CNT 34UL
    uchar              scalars [MSM_CNT][32];
    uchar              encoding[MSM_CNT][32];
    fd_ed25519_point_t points  [MSM_CNT];

    fd_memset( scalars,  0, sizeof(scalars)  );
    fd_memset( encoding, 0, sizeof(encoding) );
    for( ulong i=0UL; i<MSM_CNT; i++ ) encoding[i][0] = 1U;

    for( ulong w=0UL; w<4UL; w++ ) {
      ulong pos = witness_pos[w];
      char const * h = witness_hex[w];
      for( ulong b=0UL; b<32UL; b++ ) {
        int hi = test_r52x5_hex_nibble( h[2UL*b]     );
        int lo = test_r52x5_hex_nibble( h[2UL*b+1UL] );
        FD_TEST( hi>=0 && lo>=0 );
        encoding[pos][b] = (uchar)( (hi<<4) | lo );
      }
      scalars[pos][0] = 1U;
    }

    for( ulong i=0UL; i<MSM_CNT; i++ ) {
      FD_TEST( fd_ed25519_scalar_validate( scalars[i] ) );
      FD_TEST( fd_ed25519_point_frombytes( &points[i], encoding[i] ) );
    }

    /* Direct */
    fd_ed25519_point_t direct[1];
    fd_ed25519_multi_scalar_mul( direct, (uchar const *)scalars, points, MSM_CNT );

    /* 32 + 2 */
    fd_ed25519_point_t batched[1], tmp[1];
    fd_ed25519_point_set_zero( batched );
    for( ulong i=0UL; i<MSM_CNT; i+=FD_BALLET_CURVE25519_MSM_BATCH_SZ ) {
      ulong batch_cnt = fd_ulong_min( MSM_CNT-i, FD_BALLET_CURVE25519_MSM_BATCH_SZ );
      fd_ed25519_multi_scalar_mul( tmp, (uchar const *)&scalars[i], &points[i], batch_cnt );
      fd_ed25519_point_add( batched, batched, tmp );
    }

    uchar direct_bytes[32], batched_bytes[32];
    fd_ed25519_point_tobytes( direct_bytes,  direct  );
    fd_ed25519_point_tobytes( batched_bytes, batched );

    FD_TEST( !memcmp( batched_bytes, direct_bytes, 32UL ) );
#   undef MSM_CNT
  } while(0);
