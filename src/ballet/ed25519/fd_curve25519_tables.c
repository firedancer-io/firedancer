...
#if FD_HAS_AVX512
#define PRINT_POINT_CONST_ROW( row ) do {                                           \
    fprintf( file, "    { " );                                                      \
    for( int i=0; i<4; i++ ) fprintf( file, "0x%llx, ", (unsigned long long)row[i] ); \
    fprintf( file, "},\n" );                                                        \
  } while(0)
  PRINT_POINT_CONST_ROW( value->P0 );
  PRINT_POINT_CONST_ROW( value->P1 );
  PRINT_POINT_CONST_ROW( value->P2 );
  PRINT_POINT_CONST_ROW( value->P3 );
  PRINT_POINT_CONST_ROW( value->P4 );
#undef PRINT_POINT_CONST_ROW
#else
      fprintf( file, "    {{{ ");
      for( int i=0; i<5; i++ ) {
...
rangeproofs_tables_file( FILE * file ) {
  ...
  char const * header[] = {
    "/* Do NOT modify. This file is auto generated by fd_curve25519_tables. */",
    "",
    "#ifndef HEADER_fd_src_ballet_zksdk_rangeproofs_fd_rangeproofs_h",
...
heea_tables_file( FILE * file ) {
  ...
  fd_ed25519_point_t table[8];
  fd_ed25519_point_t b128[1];
  fd_ed25519_point_t b128_2x[1];
  fd_ed25519_point_dbln( b128, fd_ed25519_base_point, 128 );
  fd_ed25519_point_dbln( b128_2x, b128, 1 );
  fd_ed25519_point_set( &table[0], b128 );
  for( int i=1; i<8; i++ ) {
    fd_ed25519_point_add( &table[i], b128_2x, &table[i-1] );
  }
  for( int i=0; i<8; i++ ) {
    fd_curve25519_into_precomputed( &table[i] );
  }

  fprintf( file, "/* Ed25519 base point multiplied by 2^128, w-NAF table. */\n");
  points_array( file, "fd_ed25519_base_point_128_wnaf_table", table, 8 );
}
...
main( int     argc,
  ...
  {
    snprintf( path, 256, "%s/fd_curve25519_heea_table_%s.c", dir, path_suffix );
#if STDOUT
    snprintf( path, 256, "/dev/stdout" );
#endif
    FILE * file = fopen( path, "wb" );
