...
#if FD_HAS_AVX512
  ulong limbs[8];
  wwl_stu( limbs, value->el );
  for( int i=0; i<5; i++ ) {
    fprintf( file, "0x%016lx, ", limbs[i]);
  }
  fprintf( file, "0x0, 0x0, 0x0, ");
#else
  for( int i=0; i<5; i++ ) {
...
main( int     argc,
  ...
  char const * default_dir = "./src/ballet/";
  ...
  {
    snprintf( path, 256, "%s/ed25519/table/fd_f25519_table_%s.c", dir, path_suffix );
#if STDOUT
  ...
  {
    snprintf( path, 256, "%s/ed25519/table/fd_curve25519_table_%s.c", dir, path_suffix );
#if STDOUT
    snprintf( path, 256, "/dev/stdout" );
#endif
    FILE * file = fopen( path, "wb" );
  ...
  {
    snprintf( path, 256, "%s/zksdk/rangeproofs/fd_rangeproofs_table_%s.c", dir, path_suffix );
#if STDOUT
    snprintf( path, 256, "/dev/stdout" );
#endif
    FILE * file = fopen( path, "wb" );
