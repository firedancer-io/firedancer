...
void FD_FN_NO_ASAN
fd_curve25519_scalar_wnaf( short       _t[ static 256 ], /* 256-entry */
                           uchar const _vs[ static 32 ], /* 32-byte, assumes valid scalar */
...
/* Arithmetic modulo the group order n = 8 * ( 2^252 +  27742317777372353535851937790883648493 )
   Source: https://hyperelliptic.org/nacl/nacl-20110221.tar.bz2 */

typedef struct {
  uint v[32];
} sc25519;

// static const uint m[32] = {0xED, 0xD3, 0xF5, 0x5C, 0x1A, 0x63, 0x12, 0x58, 0xD6, 0x9C, 0xF7, 0xA2, 0xDE, 0xF9, 0xDE, 0x14,
//                                     0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10};

// static const uint mu[33] = {0x1B, 0x13, 0x2C, 0x0A, 0xA3, 0xE5, 0x9C, 0xED, 0xA7, 0x29, 0x63, 0x08, 0x5D, 0x21, 0x06, 0x21,
//                                      0xEB, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x0F};

static const uint m[32] = { 0x68, 0x9f, 0xae, 0xe7, 0xd2, 0x18, 0x93, 0xc0, 0xb2, 0xe6, 0xbc, 0x17, 0xf5, 0xce, 0xf7, 0xa6, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80 };

static const uint mu[33] = { 0x63, 0x82, 0x45, 0x61, 0xb4, 0x9c, 0xb3, 0xfd, 0x34, 0x65, 0x0c, 0xa1, 0x2b, 0xc4, 0x20, 0x64, 0xfd, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x01 };
...
static void barrett_reduce(sc25519 *r, const uint x[64])
{
  /* See HAC, Alg. 14.42 */
  int i,j;
  uint q2[66] = {0};
...
{
  int i;
  for(i=0;i<32;i++) r[i] = (uchar)x->v[i];
}
...
fd_curve25519_scalar_mul_mod8l( uchar *       s,
                                uchar const * a,
                                uchar const * b ) {
  sc25519 rr[1], aa[1], bb[1];
  sc25519_from32bytes( aa, a );
  sc25519_from32bytes( bb, b );

  sc25519_mul( rr, aa, bb );

  sc25519_to32bytes( s, rr );
  return s;
}
