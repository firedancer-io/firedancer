...
fd_blake3_batch_hash( fd_blake3_op_t const * ops,
                      ulong                  op_cnt ) {
  uchar const * batch_data   [ FD_BLAKE3_BATCH_MAX ] __attribute__((aligned(64)));
  uint          batch_data_sz[ FD_BLAKE3_BATCH_MAX ];
  uchar *       batch_hash   [ FD_BLAKE3_BATCH_MAX ] __attribute__((aligned(64)));
...
fd_blake3_append_blocks( fd_blake3_pos_t * s,
  ...
  for( ulong i=0UL; i<buf_cnt; i++ ) {
    fd_blake3_op_t op[1];
    do {
      if( !fd_blake3_prepare_fast( s, tbl, op, FD_BLAKE3_BATCH_MAX, 4 ) )
        return;
#if FD_HAS_AVX512
...
fd_blake3_private_batch( fd_blake3_batch_t * batch,
  ...
  while( (uint)fd_uint_popcnt( mask ) >= min_live ) {
    ...
    for( ulong state_idx=0UL; state_idx<FD_BLAKE3_BATCH_MAX; state_idx++ ) {
      ...
      op_cnt++;
      if( op_cnt==FD_BLAKE3_BATCH_MAX ) goto go;
    }
...
fd_blake3_batch_add( fd_blake3_batch_t * batch,
  ...
  fd_blake3_op_t op[1];
  for(;;) {
    if( !fd_blake3_prepare_fast( &batch->pos[ idx ], &batch->buf[ idx ], op, FD_BLAKE3_BATCH_MAX, 4 ) )
      break;
#if FD_HAS_AVX512
    fd_blake3_avx512_compress16_fast( op->msg, op->out, op->counter, op->flags );
#elif FD_HAS_AVX
