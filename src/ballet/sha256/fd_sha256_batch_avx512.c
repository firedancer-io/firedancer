...
fd_sha256_hash_32_repeated_batch_avx512( uchar const * hash_in,
  ...
  memcpy( scratch, hash_in, 32UL*batch_cnt );
...
# define EXPAND_ROUNDS(i)                                                                            \
  ...
  /* Transpose 16 lanes x 8 words into 8 vectors of 16 lanes each. */

  wwu_t s0; wwu_t s1; wwu_t s2; wwu_t s3; wwu_t s4; wwu_t s5; wwu_t s6; wwu_t s7;
  ...
  memcpy( hash_out, scratch, 32UL*batch_cnt );
