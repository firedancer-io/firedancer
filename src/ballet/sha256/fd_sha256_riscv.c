...
/* This implementation follows OpenSSL's
   sha256-riscv64-zvkb-zvknha_or_zvknhb.pl (Apache-2.0 or BSD-2-Clause).
   The SHA-2 vector instructions operate on four rounds at a time.  They
   expect the state as {f,e,b,a},{h,g,d,c}, while message vectors remain in
...
fd_sha256_core_riscv( uint *        state,
  ...
  do {
    vuint32m1_t w0 = __riscv_vle32_v_u32m1( (uint const *)(block     ), FD_SHA256_RISCV_VL );
    vuint32m1_t w1 = __riscv_vle32_v_u32m1( (uint const *)(block+16UL), FD_SHA256_RISCV_VL );
    vuint32m1_t w2 = __riscv_vle32_v_u32m1( (uint const *)(block+32UL), FD_SHA256_RISCV_VL );
    vuint32m1_t w3 = __riscv_vle32_v_u32m1( (uint const *)(block+48UL), FD_SHA256_RISCV_VL );
...
fd_sha256_hash_32_repeated_riscv( uchar const * data,
  ...
  vuint32m1_t const initial0 = __riscv_vle32_v_u32m1( fd_sha256_riscv_initial0,
                                                      FD_SHA256_RISCV_VL );
  vuint32m1_t const initial1 = __riscv_vle32_v_u32m1( fd_sha256_riscv_initial1,
                                                      FD_SHA256_RISCV_VL );
  vuint32m1_t const padding0 = __riscv_vle32_v_u32m1( fd_sha256_riscv_padding0,
                                                      FD_SHA256_RISCV_VL );
  vuint32m1_t const padding1 = __riscv_vle32_v_u32m1( fd_sha256_riscv_padding1,
                                                      FD_SHA256_RISCV_VL );
  vuint32m1_t const reverse_idx = __riscv_vle32_v_u32m1( fd_sha256_riscv_reverse_idx,
                                                        FD_SHA256_RISCV_VL );
  vuint32m1_t const lane_ids = __riscv_vid_v_u32m1( FD_SHA256_RISCV_VL );
  vbool32_t const lane0 = __riscv_vmseq_vx_u32m1_b32( lane_ids, 0U, FD_SHA256_RISCV_VL );
  ...
  w0 = __riscv_vrev8_v_u32m1( w0, FD_SHA256_RISCV_VL );
  w1 = __riscv_vrev8_v_u32m1( w1, FD_SHA256_RISCV_VL );
  __riscv_vse32_v_u32m1( (uint *)hash,      w0, FD_SHA256_RISCV_VL );
  __riscv_vse32_v_u32m1( (uint *)(hash+16), w1, FD_SHA256_RISCV_VL );
}
