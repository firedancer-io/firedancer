...
fd_sha256_hash_32_repeated_batch_arm( uchar const * hash_in,
  ...
  if( FD_UNLIKELY( batch_cnt<2UL ) ) {
    for( ulong i=0UL; i<batch_cnt; i++ ) fd_sha256_hash_32_repeated_arm( hash_in+32UL*i, hash+32UL*i, cnt );
    return;
  }

  uint32x4_t msg[ 4 ];
  repbatch_lane_load( msg, 0UL, hash_in       );
  repbatch_lane_load( msg, 1UL, hash_in+32UL  );
  fd_sha256_repeat_lanes_x2( msg, cnt );
  repbatch_lane_store( hash,      msg, 0UL );
  repbatch_lane_store( hash+32UL, msg, 1UL );
}
