...
/* A fd_funkier_txn_xid_t identifies a funk transaction currently in
   preparation.  Compact binary identifiers are encouraged but a cstr
   can be used so long as it has
   strlen(cstr)<FD_FUNKIER_TXN_XID_FOOTPRINT and characters c[i] for i
   in [strlen(cstr),FD_FUNKIER_TXN_KEY_FOOTPRINT) zero.  (Also, if
