...
fd_funkier_open_file( const char * filename,
  ...
  if( mode == FD_FUNKIER_READONLY || mode == FD_FUNKIER_READ_WRITE ) {
    ...
    if( !fd_shmem_join_query_by_name("funk", &info) ) {
      ...
      fd_funkier_t * funk = fd_funkier_join( funk_shmem );
  ...
  switch (mode) {
  case FD_FUNKIER_READONLY:
    if( filename == NULL  || filename[0] == '\0' ) {
      FD_LOG_WARNING(( "mode FD_FUNKIER_READONLY can not be used with an anonymous workspace, funk file required" ));
      return NULL;
    }
    ...
    do_new = 0;
    break;
  case FD_FUNKIER_READ_WRITE:
    if( filename == NULL  || filename[0] == '\0' ) {
      FD_LOG_WARNING(( "mode FD_FUNKIER_READ_WRITE can not be used with an anonymous workspace, funk file required" ));
      return NULL;
    }
    open_flags = O_RDWR;
    can_create = 0;
    ...
    do_new = 0;
    break;
  case FD_FUNKIER_CREATE:
    open_flags = O_CREAT|O_RDWR;
    can_create = 1;
    can_resize = 0;
    do_new = 0;
    break;
  case FD_FUNKIER_OVERWRITE:
    open_flags = O_CREAT|O_RDWR;
    can_create = 1;
    can_resize = 1;
    do_new = 1;
    break;
  case FD_FUNKIER_CREATE_EXCL:
  ...
  if( do_new ) {
    ...
    void * funk_shmem = fd_wksp_alloc_laddr( wksp, fd_funkier_align(), fd_funkier_footprint( txn_max, rec_max ), wksp_tag );
    ...
    fd_funkier_t * funk = fd_funkier_join( fd_funkier_new( funk_shmem, wksp_tag, seed, txn_max, rec_max ) );
    if( FD_UNLIKELY( funk == NULL ) ) {
      FD_LOG_WARNING(( "failed to allocate a funky" ));
      munmap( shmem, total_sz );
      close( fd );
