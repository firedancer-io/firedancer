...
fd_funkier_txn_prepare( fd_funkier_t *               funk,
  ...
  if( FD_UNLIKELY( parent && !fd_funkier_txn_valid( funk, parent ) ) ) {
    if( FD_UNLIKELY( verbose ) ) FD_LOG_WARNING(( "bad txn" ));
    return NULL;
  }
  if( FD_UNLIKELY( fd_funkier_txn_xid_eq_root( xid ) ) ) {
    if( FD_UNLIKELY( verbose ) ) FD_LOG_WARNING(( "xid is the root" ));
    return NULL;
  }
  if( FD_UNLIKELY( fd_funkier_txn_xid_eq( xid, funk->last_publish ) ) ) {
  ...
  if( FD_UNLIKELY( fd_funkier_txn_map_query_try( &txn_map, xid, NULL, query ) != FD_MAP_ERR_KEY ) ) {
  ...
  if( FD_LIKELY( !parent ) ) { /* opt for incr pub */
    ...
    parent_idx = FD_FUNKIER_TXN_IDX_NULL;
  ...
  fd_funkier_txn_t * txn = fd_funkier_txn_pool_acquire( &txn_pool, NULL, 1, NULL );
  if( txn == NULL ) {
    if( FD_UNLIKELY( verbose ) ) FD_LOG_WARNING(( "transaction pool is exhuasted" ));
    return NULL;
  }
  fd_funkier_txn_xid_copy( &txn->xid, xid );
  ulong txn_idx = (ulong)(txn - txn_pool.ele);

  /* Join the family */

  ulong sibling_prev_idx = fd_funkier_txn_idx( *_child_tail_cidx );

  int first_born = fd_funkier_txn_idx_is_null( sibling_prev_idx );

  txn->parent_cidx       = fd_funkier_txn_cidx( parent_idx           );
  txn->child_head_cidx   = fd_funkier_txn_cidx( FD_FUNKIER_TXN_IDX_NULL );
  txn->child_tail_cidx   = fd_funkier_txn_cidx( FD_FUNKIER_TXN_IDX_NULL );
  txn->sibling_prev_cidx = fd_funkier_txn_cidx( sibling_prev_idx     );
  txn->sibling_next_cidx = fd_funkier_txn_cidx( FD_FUNKIER_TXN_IDX_NULL );
  txn->stack_cidx        = fd_funkier_txn_cidx( FD_FUNKIER_TXN_IDX_NULL );
  txn->tag               = 0UL;

  txn->rec_head_idx = FD_FUNKIER_REC_IDX_NULL;
  txn->rec_tail_idx = FD_FUNKIER_REC_IDX_NULL;

  /* TODO: consider branchless impl */
  if( FD_LIKELY( first_born ) ) *_child_head_cidx                                  = fd_funkier_txn_cidx( txn_idx ); /* opt for non-compete */
  else                          txn_pool.ele[ sibling_prev_idx ].sibling_next_cidx = fd_funkier_txn_cidx( txn_idx );

  *_child_tail_cidx = fd_funkier_txn_cidx( txn_idx );

  fd_funkier_txn_map_insert( &txn_map, txn, FD_MAP_FLAG_BLOCKING );
...
fd_funkier_txn_is_full( fd_funkier_t * funk ) {
  fd_wksp_t * wksp = fd_funkier_wksp( funk );
  fd_funkier_txn_pool_t txn_pool = fd_funkier_txn_pool( funk, wksp );
  return fd_funkier_txn_pool_is_empty( &txn_pool );
}

/* fd_funkier_txn_cancel_childless cancels a transaction that is known
...
static void
fd_funkier_txn_cancel_childless( fd_funkier_t * funk,
                                 ulong          txn_idx ) {
  ...
  fd_funkier_txn_t * txn = &txn_pool.ele[ txn_idx ];
  ...
  while( !fd_funkier_rec_idx_is_null( rec_idx ) ) {
    ...
    if( FD_UNLIKELY( fd_funkier_txn_idx( rec_pool.ele[ rec_idx ].txn_cidx )!=txn_idx ) )
      FD_LOG_CRIT(( "memory corruption detected (cycle or bad idx)" ));

    fd_funkier_rec_t * rec = &rec_pool.ele[ rec_idx ];
    ...
    for(;;) {
      ...
      if( rec != fd_funkier_rec_map_query_ele( rec_query ) ) break;
  ...
  ulong sibling_prev_idx = fd_funkier_txn_idx( txn->sibling_prev_cidx );
  ulong sibling_next_idx = fd_funkier_txn_idx( txn->sibling_next_cidx );

  /* TODO: Consider branchless impl */

  if( FD_LIKELY( fd_funkier_txn_idx_is_null( sibling_prev_idx ) ) ) { /* opt for non-compete */
    ulong parent_idx = fd_funkier_txn_idx( txn_pool.ele[ txn_idx ].parent_cidx );
    if( FD_LIKELY( fd_funkier_txn_idx_is_null( parent_idx ) ) ) { /* No older sib and is a funk child, opt for incr pub */
      funk->child_head_cidx = fd_funkier_txn_cidx( sibling_next_idx );
    } else { /* No older sib and has parent */
      txn_pool.ele[ parent_idx ].child_head_cidx = fd_funkier_txn_cidx( sibling_next_idx );
    }
  } else { /* Has older sib */
    txn_pool.ele[ sibling_prev_idx ].sibling_next_cidx = fd_funkier_txn_cidx( sibling_next_idx );
  }

  if( FD_LIKELY( fd_funkier_txn_idx_is_null( sibling_next_idx ) ) ) { /* opt for non-compete */
    ulong parent_idx = fd_funkier_txn_idx( txn_pool.ele[ txn_idx ].parent_cidx );
    if( FD_LIKELY( fd_funkier_txn_idx_is_null( parent_idx ) ) ) { /* No younger sib and is a funk child, opt for incr pub */
      funk->child_tail_cidx = fd_funkier_txn_cidx( sibling_prev_idx );
    } else { /* No younger sib and has parent */
      txn_pool.ele[ parent_idx ].child_tail_cidx = fd_funkier_txn_cidx( sibling_prev_idx );
    }
  } else { /* Has younger sib */
    txn_pool.ele[ sibling_next_idx ].sibling_prev_cidx = fd_funkier_txn_cidx( sibling_prev_idx );
  }

  fd_funkier_txn_map_query_t query[1];
  if( fd_funkier_txn_map_remove( &txn_map, fd_funkier_txn_xid( txn ), NULL, query, FD_MAP_FLAG_BLOCKING ) == FD_MAP_SUCCESS ) {
    fd_funkier_txn_pool_release( &txn_pool, txn, 1 );
  }
}

/* fd_funkier_txn_cancel_family cancels a transaction and all its
...
static ulong
fd_funkier_txn_cancel_family( fd_funkier_t *  funk,
                              ulong           tag,
                              ulong           txn_idx ) {
  ...
  ulong parent_stack_idx = FD_FUNKIER_TXN_IDX_NULL;
  ...
  for(;;) {
    ...
    ulong youngest_idx = fd_funkier_txn_idx( txn->child_tail_cidx );
    if( FD_LIKELY( fd_funkier_txn_idx_is_null( youngest_idx ) ) ) { /* txn is is childless, opt for incr pub */

      fd_funkier_txn_cancel_childless( funk, txn_idx ); /* If this can fail, return cancel_cnt here on fail */
      cancel_cnt++;

      txn_idx = parent_stack_idx;                                  /* Pop the parent stack */
      if( FD_LIKELY( fd_funkier_txn_idx_is_null( txn_idx ) ) ) break; /* If stack is empty, we are done, opt for incr pub */
      parent_stack_idx = fd_funkier_txn_idx( txn_pool.ele[ txn_idx ].stack_cidx );
      continue;
    }
    ...
    txn->stack_cidx = fd_funkier_txn_cidx( parent_stack_idx );
...
fd_funkier_txn_cancel( fd_funkier_t *     funk,
  ...
  if( FD_UNLIKELY( !fd_funkier_txn_valid( funk, txn ) ) ) {
    if( FD_UNLIKELY( verbose ) ) FD_LOG_WARNING(( "bad txn" ));
    return 0UL;
  }
#else
  ...
  return fd_funkier_txn_cancel_family( funk, funk->cycle_tag++, txn_idx );
}

/* fd_funkier_txn_oldest_sibling returns the index of the oldest sibling
   in txn_idx's family.  Callers have already validated our input
   arguments.  The caller should validate the return index. */

static inline ulong
fd_funkier_txn_oldest_sibling( fd_funkier_t *  funk,
                               ulong           txn_idx ) {
  ...
  if( FD_LIKELY( fd_funkier_txn_idx_is_null( parent_idx ) ) ) return fd_funkier_txn_idx( funk->child_head_cidx ); /* opt for incr pub */

  return fd_funkier_txn_idx( txn_pool.ele[ parent_idx ].child_head_cidx );
}

/* fd_funkier_txn_cancel_sibling_list cancels siblings from sibling_idx down
   ...
   FD_FUNKIER_TXN_IDX_NULL for skip_idx will cancel all siblings from
   sibling_idx to the youngest inclusive. */

static ulong
fd_funkier_txn_cancel_sibling_list( fd_funkier_t * funk,
                                    ulong          tag,
                                    ulong          sibling_idx,
                                    ulong          skip_idx ) {
  ...
  ulong cancel_stack_idx = FD_FUNKIER_TXN_IDX_NULL;
  ...
  for(;;) {
    ...
    fd_funkier_txn_t * sibling = &txn_pool.ele[ sibling_idx ];
    sibling->tag = tag;

    if( FD_UNLIKELY( sibling_idx!=skip_idx ) ) { /* Not skip_idx so push onto the cancel stack, opt for non-compete */
      sibling->stack_cidx = fd_funkier_txn_cidx( cancel_stack_idx );
      cancel_stack_idx = sibling_idx;
    }

    ulong younger_idx = fd_funkier_txn_idx( sibling->sibling_next_cidx );
    if( FD_LIKELY( fd_funkier_txn_idx_is_null( younger_idx ) ) ) break; /* opt for non-compete */
  ...
  while( !fd_funkier_txn_idx_is_null( cancel_stack_idx ) ) { /* TODO: peel first iter to make more predictable? */
    ulong sibling_idx = cancel_stack_idx;
    cancel_stack_idx  = fd_funkier_txn_idx( txn_pool.ele[ sibling_idx ].stack_cidx );
    cancel_cnt += fd_funkier_txn_cancel_family( funk, tag, sibling_idx );
  }
...
fd_funkier_txn_cancel_siblings( fd_funkier_t *     funk,
  ...
  ulong oldest_idx = fd_funkier_txn_oldest_sibling( funk, txn_idx );

  return fd_funkier_txn_cancel_sibling_list( funk, funk->cycle_tag++, oldest_idx, txn_idx );
}
...
fd_funkier_txn_cancel_children( fd_funkier_t *     funk,
  ...
  if( FD_LIKELY( txn == NULL ) ) {
    oldest_idx = fd_funkier_txn_idx( funk->child_head_cidx ); /* opt for non-compete */
  } else {
    oldest_idx = fd_funkier_txn_idx( txn->child_head_cidx );
  }

  if( fd_funkier_txn_idx_is_null( oldest_idx ) ) {
    return 0UL;
  }

  return fd_funkier_txn_cancel_sibling_list( funk, funk->cycle_tag++, oldest_idx, FD_FUNKIER_TXN_IDX_NULL );
}
...
fd_funkier_txn_cancel_all( fd_funkier_t *     funk,
                           int             verbose ) {
  return fd_funkier_txn_cancel_children( funk, NULL, verbose );
}

/* fd_funkier_txn_update applies the record updates in transaction txn_idx
...
static void
fd_funkier_txn_update( fd_funkier_t *            funk,
                       ...
                       fd_funkier_txn_xid_t const * dst_xid,        /* dst xid */
  ...
  while( !fd_funkier_rec_idx_is_null( rec_idx ) ) {
    ...
    fd_funkier_xid_key_pair_init( pair, dst_xid, rec->pair.key );
    for(;;) {
      fd_funkier_rec_map_query_t rec_query[1];
      ...
      fd_funkier_rec_t * rec2 = fd_funkier_rec_map_query_ele( rec_query );
      ulong prev_idx = rec2->prev_idx;
      ulong next_idx = rec2->next_idx;
      if( fd_funkier_rec_idx_is_null( prev_idx ) ) {
        *_dst_rec_head_idx = next_idx;
      } else {
        rec_pool.ele[ prev_idx ].next_idx = next_idx;
      }
      if( fd_funkier_rec_idx_is_null( next_idx ) ) {
        *_dst_rec_tail_idx = prev_idx;
      } else {
        rec_pool.ele[ next_idx ].prev_idx = prev_idx;
      }
      /* Clean up value */
      fd_funkier_val_flush( rec2, alloc, wksp );
      rec2->txn_cidx = fd_funkier_txn_cidx( FD_FUNKIER_TXN_IDX_NULL );
      fd_funkier_rec_pool_release( &rec_pool, rec2, 1 );
      break;
    }
    ...
    /* Add the new record to the transaction. We can update the xid in
       ...
       chain. fd_funkier_rec_query_global relies on this subtle
       property. */

    rec->pair.xid[0] = *dst_xid;
    rec->txn_cidx = fd_funkier_txn_cidx( dst_txn_idx );

    if( fd_funkier_rec_idx_is_null( *_dst_rec_head_idx ) ) {
  ...
  txn_pool.ele[ txn_idx ].rec_head_idx = FD_FUNKIER_REC_IDX_NULL;
  txn_pool.ele[ txn_idx ].rec_tail_idx = FD_FUNKIER_REC_IDX_NULL;
}

/* fd_funkier_txn_publish_funk_child publishes a transaction that is known
   to be a child of funk.  Callers have already validated our input
   arguments.  Returns FD_FUNKIER_SUCCESS on success and an FD_FUNKIER_ERR_*
...
static int
fd_funkier_txn_publish_funk_child( fd_funkier_t *  funk,
                                   ulong           tag,
                                   ulong           txn_idx ) {

  /* Apply the updates in txn to the last published transactions */

  fd_funkier_txn_update( funk, &funk->rec_head_idx, &funk->rec_tail_idx, FD_FUNKIER_TXN_IDX_NULL, fd_funkier_root( funk ), txn_idx );

  /* Cancel all competing transaction histories */

  ulong oldest_idx = fd_funkier_txn_oldest_sibling( funk, txn_idx );
  fd_funkier_txn_cancel_sibling_list( funk, tag, oldest_idx, txn_idx );
  ...
  ulong child_head_idx = fd_funkier_txn_idx( txn_pool.ele[ txn_idx ].child_head_cidx );
  ulong child_tail_idx = fd_funkier_txn_idx( txn_pool.ele[ txn_idx ].child_tail_cidx );

  ulong child_idx = child_head_idx;
  while( FD_UNLIKELY( !fd_funkier_txn_idx_is_null( child_idx ) ) ) { /* opt for incr pub */
    fd_funkier_txn_t * txn = &txn_pool.ele[ child_idx ];
    txn->tag = tag;
    txn->parent_cidx = fd_funkier_txn_cidx( FD_FUNKIER_TXN_IDX_NULL );
    child_idx = fd_funkier_txn_idx( txn->sibling_next_cidx );
  }

  funk->child_head_cidx = fd_funkier_txn_cidx( child_head_idx );
  funk->child_tail_cidx = fd_funkier_txn_cidx( child_tail_idx );

  fd_funkier_txn_xid_copy( funk->last_publish, fd_funkier_txn_xid( &txn_pool.ele[ txn_idx ] ) );

  /* Remove the mapping */

  fd_funkier_txn_map_query_t query[1];
  if( fd_funkier_txn_map_remove( &txn_map, funk->last_publish, NULL, query, FD_MAP_FLAG_BLOCKING ) == FD_MAP_SUCCESS ) {
    fd_funkier_txn_pool_release( &txn_pool, &txn_pool.ele[ txn_idx ], 1 );
  }
...
fd_funkier_txn_publish( fd_funkier_t *     funk,
  ...
  ulong publish_stack_idx = FD_FUNKIER_TXN_IDX_NULL;

  for(;;) {
    fd_funkier_txn_t * txn2 = &txn_pool.ele[ txn_idx ];
    txn2->tag = tag;
    ...
    ulong parent_idx = fd_funkier_txn_idx( txn2->parent_cidx );
    if( FD_LIKELY( fd_funkier_txn_idx_is_null( parent_idx ) ) ) break; /* opt for incr pub */

    /* txn_idx has a parent.  Validate and tag it.  Push txn to the
       publish stack and recurse into the parent. */

    txn2->stack_cidx = fd_funkier_txn_cidx( publish_stack_idx );
  ...
  for(;;) {
    ...
    if( FD_UNLIKELY( fd_funkier_txn_publish_funk_child( funk, funk->cycle_tag++, txn_idx ) ) ) break;
    publish_cnt++;

    txn_idx = publish_stack_idx;
    if( FD_LIKELY( fd_funkier_txn_idx_is_null( txn_idx ) ) ) break; /* opt for incr pub */
    publish_stack_idx = fd_funkier_txn_idx( txn_pool.ele[ txn_idx ].stack_cidx );
  }
...
fd_funkier_txn_publish_into_parent( fd_funkier_t *     funk,
  ...
  ulong oldest_idx = fd_funkier_txn_oldest_sibling( funk, txn_idx );
  fd_funkier_txn_cancel_sibling_list( funk, funk->cycle_tag++, oldest_idx, txn_idx );

  ulong parent_idx = fd_funkier_txn_idx( txn->parent_cidx );
  if( fd_funkier_txn_idx_is_null( parent_idx ) ) {
    /* Publish to root */
    fd_funkier_txn_update( funk, &funk->rec_head_idx, &funk->rec_tail_idx, FD_FUNKIER_TXN_IDX_NULL, fd_funkier_root( funk ), txn_idx );
    ...
    fd_funkier_txn_t * parent_txn = &txn_pool.ele[ parent_idx ];
    fd_funkier_txn_update( funk, &parent_txn->rec_head_idx, &parent_txn->rec_tail_idx, parent_idx, &parent_txn->xid, txn_idx );
    /* Inherit the children */
  ...
  ulong child_idx = fd_funkier_txn_idx( txn->child_head_cidx );
  while( FD_UNLIKELY( !fd_funkier_txn_idx_is_null( child_idx ) ) ) {
    txn_pool.ele[ child_idx ].parent_cidx = fd_funkier_txn_cidx( parent_idx );
    child_idx = fd_funkier_txn_idx( txn_pool.ele[ child_idx ].sibling_next_cidx );
  }
...
fd_funkier_txn_first_rec( fd_funkier_t *           funk,
  ...
  if( fd_funkier_rec_idx_is_null( rec_idx ) ) return NULL;
  fd_wksp_t * wksp = fd_funkier_wksp( funk );
  fd_funkier_rec_pool_t rec_pool = fd_funkier_rec_pool( funk, wksp );
...
fd_funkier_txn_xid_t
fd_funkier_generate_xid(void) {
...
static void
fd_funkier_txn_all_iter_skip_nulls( fd_funkier_txn_all_iter_t * iter ) {
  if( iter->chain_idx == iter->chain_cnt ) return;
  while( fd_funkier_txn_map_iter_done( iter->txn_map_iter ) ) {
    if( ++(iter->chain_idx) == iter->chain_cnt ) break;
    iter->txn_map_iter = fd_funkier_txn_map_iter( &iter->txn_map, iter->chain_idx );
  }
}

void
fd_funkier_txn_all_iter_new( fd_funkier_t * funk, fd_funkier_txn_all_iter_t * iter ) {
  fd_wksp_t * wksp = fd_funkier_wksp( funk );
  iter->txn_map = fd_funkier_txn_map( funk, wksp );
  iter->chain_cnt = fd_funkier_txn_map_chain_cnt( &iter->txn_map );
  iter->chain_idx = 0;
  iter->txn_map_iter = fd_funkier_txn_map_iter( &iter->txn_map, 0 );
  fd_funkier_txn_all_iter_skip_nulls( iter );
}

int
fd_funkier_txn_all_iter_done( fd_funkier_txn_all_iter_t * iter ) {
  return ( iter->chain_idx == iter->chain_cnt );
}

void
fd_funkier_txn_all_iter_next( fd_funkier_txn_all_iter_t * iter ) {
  iter->txn_map_iter = fd_funkier_txn_map_iter_next( iter->txn_map_iter );
  fd_funkier_txn_all_iter_skip_nulls( iter );
}

fd_funkier_txn_t const *
fd_funkier_txn_all_iter_ele_const( fd_funkier_txn_all_iter_t * iter ) {
  return fd_funkier_txn_map_iter_ele_const( iter->txn_map_iter );
}

fd_funkier_txn_t *
fd_funkier_txn_all_iter_ele( fd_funkier_txn_all_iter_t * iter ) {
  return fd_funkier_txn_map_iter_ele( iter->txn_map_iter );
}
...
fd_funkier_txn_verify( fd_funkier_t * funk ) {
  ...
  ulong funk_child_head_idx = fd_funkier_txn_idx( funk->child_head_cidx ); /* Previously verified */
  ulong funk_child_tail_idx = fd_funkier_txn_idx( funk->child_tail_cidx ); /* Previously verified */

  fd_funkier_txn_xid_t const * last_publish = funk->last_publish; /* Previously verified */

# define TEST(c) do {                                                                           \
    if( FD_UNLIKELY( !(c) ) ) { FD_LOG_WARNING(( "FAIL: %s", #c )); return FD_FUNKIER_ERR_INVAL; } \
  } while(0)

# define IS_VALID( idx ) ( (idx==FD_FUNKIER_TXN_IDX_NULL) || \
                           ((idx<txn_max) && (!fd_funkier_txn_xid_eq( fd_funkier_txn_xid( &txn_pool.ele[idx] ), last_publish ))) )

  TEST( !fd_funkier_txn_map_verify( &txn_map ) );
  TEST( !fd_funkier_txn_pool_verify( &txn_pool ) );
  ...
  do {
    ...
    ulong stack_idx = FD_FUNKIER_TXN_IDX_NULL;
    ulong child_idx = funk_child_head_idx;
    while( !fd_funkier_txn_idx_is_null( child_idx ) ) {
      ...
      TEST( fd_funkier_txn_idx_is_null( fd_funkier_txn_idx( txn_pool.ele[ child_idx ].parent_cidx ) ) );
    ...
    while( !fd_funkier_txn_idx_is_null( stack_idx ) ) {

      /* Pop the next transaction to traverse */

      ulong txn_idx = stack_idx;
      stack_idx = fd_funkier_txn_idx( txn_pool.ele[ txn_idx ].stack_cidx );

      /* Push all children of txn to the stack */

      ulong child_idx = fd_funkier_txn_idx( txn_pool.ele[ txn_idx ].child_head_cidx );
      while( !fd_funkier_txn_idx_is_null( child_idx ) ) {
        ...
        TEST( IS_VALID( child_idx ) );
        TEST( !txn_pool.ele[ child_idx ].tag );
        TEST( fd_funkier_txn_idx( txn_pool.ele[ child_idx ].parent_cidx )==txn_idx );
        txn_pool.ele[ child_idx ].tag        = 1UL;
        txn_pool.ele[ child_idx ].stack_cidx = fd_funkier_txn_cidx( stack_idx );
        stack_idx                   = child_idx;
  ...
  do {
    ...
    while( !fd_funkier_txn_idx_is_null( stack_idx ) ) {
      ...
      ulong child_idx = fd_funkier_txn_idx( txn_pool.ele[ txn_idx ].child_tail_cidx );
      while( !fd_funkier_txn_idx_is_null( child_idx ) ) {
...
fd_funkier_txn_valid( fd_funkier_t * funk, fd_funkier_txn_t const * txn ) {
  ...
  fd_funkier_txn_map_t txn_map = fd_funkier_txn_map( funk, wksp );
  fd_funkier_txn_map_query_t query[1];
  if( FD_UNLIKELY( fd_funkier_txn_map_query_try( &txn_map, &txn->xid, NULL, query ) ) ) return 0;
