...
static const uint sock_filter_policy_fd_solcap_tile_instr_cnt = 52;

static void populate_sock_filter_policy_fd_solcap_tile( ulong out_cnt, struct sock_filter out[ static 52 ], uint logfile_fd, uint solcap_fd_0, uint solcap_fd_1 ) {
  FD_TEST( out_cnt >= 52 );
  struct sock_filter filter[52] = {
    ...
    /* check ftruncate */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_ftruncate, /* check_ftruncate */ 22, 0 ),
    /* check fsync */
