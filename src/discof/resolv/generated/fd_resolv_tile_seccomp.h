...
#endif
static const unsigned int sock_filter_policy_fd_resolv_tile_instr_cnt = 20;

static void populate_sock_filter_policy_fd_resolv_tile( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int accdb_ro_fd ) {
  FD_TEST( out_cnt >= 20 );
  struct sock_filter filter[20] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_preadv2, /* check_preadv2 */ 9, 0 ),
...
//  check_clock_nanosleep:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, CLOCK_REALTIME, /* RET_ALLOW */ 3, /* RET_KILL_PROCESS */ 2 ),
//  check_preadv2:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, accdb_ro_fd, /* RET_ALLOW */ 1, /* RET_KILL_PROCESS */ 0 ),
//  RET_KILL_PROCESS:
