...
static const uint sock_filter_policy_fd_resolv_tile_instr_cnt = 40;

static void populate_sock_filter_policy_fd_resolv_tile( ulong out_cnt, struct sock_filter out[ static 40 ], uint logfile_fd, uint accdb_ro_fd, uint accdb_ring_fd ) {
  FD_TEST( out_cnt >= 40 );
  struct sock_filter filter[40] = {
    ...
    /* check futex */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_futex, /* check_futex */ 22, 0 ),
    /* allow restart_syscall */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_restart_syscall, /* RET_ALLOW */ 2, 0 ),
    /* check io_uring_enter */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_io_uring_enter, /* check_io_uring_enter */ 26, 0 ),
//  RET_KILL_PROCESS:
    /* default deny */
