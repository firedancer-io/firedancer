...
peek_alut( fd_resolv_ctx_t *  ctx,
           ...
           ulong              alut_idx ) {
  fd_funk_txn_xid_t const xid = { .ul = { ctx->bank->data->fields.slot, ctx->bank->data->fields.slot } };
...
peek_aluts( fd_resolv_ctx_t * ctx,
  ...
  ulong const               slot         = ctx->bank->data->fields.slot;
  fd_sysvar_cache_t const * sysvar_cache = &ctx->bank->data->fields.sysvar_cache;
