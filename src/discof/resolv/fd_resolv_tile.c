...
metrics_write( fd_resolv_ctx_t * ctx ) {
  ...
  FD_MCNT_SET(       RESOLF, RACE_DROP,                       ctx->metrics.race_drop );
}
...
#define ERR_PEEK_RACE -8192

/* peek_aluts speculatively reads address lookup tables from database
   cache.  Gracefully recovers from data races and missing accounts. */
...
peek_aluts( fd_resolv_ctx_t * ctx,
  ...
  for( ulong i=0UL; i<alut_cnt; i++ ) {
    fd_txn_acct_addr_lut_t const * addr_lut = &fd_txn_get_address_tables_const( txn )[i];
  ...
  if( FD_UNLIKELY( err ) ) {
    if( err==ERR_PEEK_RACE ) ctx->metrics.race_drop++;
    else                     ctx->metrics.lut[ ctr_idx ]++;
    return 0;
  }
