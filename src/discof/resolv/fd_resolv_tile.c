...
#define FD_RESOLV_IN_KIND_BANK          (1) /* UNIMPLEMENTED IN FULL CLIENT */

#define FD_RESOLVE_SPAD_FOOTPRINT ( 1UL<<24UL ) /* UNTESTED */
...
typedef struct {
  ...
  ulong *  root_slot; /* see replay_tile_ctx->published_wmark */
...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  l = FD_LAYOUT_APPEND( l, fd_spad_align(),            fd_spad_footprint  ( FD_RESOLVE_SPAD_FOOTPRINT ) );
...
publish_txn( fd_resolv_ctx_t *          ctx,
  ...
  fd_bank_shim_ctx_t bank_shim_ctx = {
    .funk = ctx->funk,
    .spad = ctx->resolv_spad
  };

  if( FD_UNLIKELY( txnt->addr_table_adtl_cnt ) ) {
    int result = fd_bank_abi_resolve_address_lookup_tables( &bank_shim_ctx,
                                                            0,
                                                            fd_fseq_query( ctx->root_slot ),
                                                            txnt,
...
after_frag( fd_resolv_ctx_t *   ctx,
  ...
  //       ctx->root_bank = frag->bank; /* analagous to funk txn pointed at current root? */
...
unprivileged_init( fd_topo_t *      topo,
  ...
  /* INIT SPAD */
  ctx->resolv_spad = fd_spad_join( fd_spad_new( FD_SCRATCH_ALLOC_APPEND( l, fd_spad_align(), fd_spad_footprint( FD_RESOLVE_SPAD_FOOTPRINT ) ), FD_RESOLVE_SPAD_FOOTPRINT ) );
  ...
  if( root_slot_obj_id==ULONG_MAX )
    FD_LOG_ERR(( "root_slot not found in props" ));

  ctx->root_slot = fd_fseq_join( fd_topo_obj_laddr( topo,  root_slot_obj_id ) );
  if( FD_UNLIKELY( !ctx->root_slot ) )
    FD_LOG_ERR(( "resolv tile has no root_slot fseq" ));
  if( FD_UNLIKELY( ULONG_MAX!=fd_fseq_query( ctx->root_slot ) ) )
    FD_LOG_ERR(( "bad fseq value on resolve init" ));
