...
#ifndef HEADER_fd_src_discof_execle_generated_fd_execle_tile_seccomp_h
#define HEADER_fd_src_discof_execle_generated_fd_execle_tile_seccomp_h
...
static const uint sock_filter_policy_fd_execle_tile_instr_cnt = 62;

static void populate_sock_filter_policy_fd_execle_tile( ulong out_cnt, struct sock_filter out[ static 62 ], uint logfile_fd, uint accounts_fd, uint stake_spill_fd, uint accdb_ring_fd ) {
  FD_TEST( out_cnt >= 62 );
  struct sock_filter filter[62] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_clock_nanosleep, /* check_clock_nanosleep */ 20, 0 ),
    /* check pwritev2 */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_pwritev2, /* check_pwritev2 */ 23, 0 ),
    /* check preadv2 */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_preadv2, /* check_preadv2 */ 26, 0 ),
    /* check fallocate */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_fallocate, /* check_fallocate */ 29, 0 ),
    /* check pread64 */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_pread64, /* check_pread64 */ 34, 0 ),
    /* check pwrite64 */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_pwrite64, /* check_pwrite64 */ 37, 0 ),
    /* check futex */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_futex, /* check_futex */ 40, 0 ),
    /* allow restart_syscall */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_restart_syscall, /* RET_ALLOW */ 2, 0 ),
    /* check io_uring_enter */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_io_uring_enter, /* check_io_uring_enter */ 44, 0 ),
//  RET_KILL_PROCESS:
    /* default deny */
...
//  check_io_uring_enter:
    /* arg 0 low 32 bits */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, FD_SECCOMP_ARG_LO_OFFSET(0)),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, ((uint)(accdb_ring_fd)), /* io_uring_enter_ALLOW */ 1, /* io_uring_enter_KILL */ 0 ),
//  io_uring_enter_KILL:
    BPF_STMT( BPF_RET | BPF_K, SECCOMP_RET_KILL_PROCESS ),
...
#endif /* HEADER_fd_src_discof_execle_generated_fd_execle_tile_seccomp_h */
