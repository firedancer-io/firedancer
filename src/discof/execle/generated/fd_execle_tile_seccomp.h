...
#endif
static const unsigned int sock_filter_policy_fd_execle_tile_instr_cnt = 28;

static void populate_sock_filter_policy_fd_execle_tile( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int accounts_fd ) {
  FD_TEST( out_cnt >= 28 );
  struct sock_filter filter[28] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_preadv2, /* check_preadv2 */ 12, 0 ),
    /* allow fallocate based on expression */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_fallocate, /* check_fallocate */ 13, 0 ),
...
//  check_clock_nanosleep:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, CLOCK_REALTIME, /* RET_ALLOW */ 9, /* RET_KILL_PROCESS */ 8 ),
//  check_pwritev2:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
...
//  check_fallocate:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, accounts_fd, /* lbl_2 */ 0, /* RET_KILL_PROCESS */ 2 ),
//  lbl_2:
