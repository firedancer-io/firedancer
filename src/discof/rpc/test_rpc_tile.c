...
ingest_test_epoch( fd_multi_epoch_leaders_t * mleaders,
  ...
  emsg->epoch           = epoch;
  emsg->start_slot      = epoch*32UL;
  emsg->slot_cnt        = 32UL;
  emsg->staked_vote_cnt = 1UL;
  emsg->staked_id_cnt   = 1UL;
  FD_TEST( fd_epoch_schedule_derive( &emsg->epoch_schedule, 32UL, 0UL, 0 ) );

  fd_vote_stake_weight_t * weight = fd_epoch_info_msg_stake_weights( emsg );
  memset( weight->vote_key.uc, leader_byte ^ (uchar)0xffU, sizeof(fd_pubkey_t) );
  memset( weight->id_key.uc,   leader_byte,                 sizeof(fd_pubkey_t) );
  weight->stake = 1000000UL;

  fd_rpc_mleaders_ingest( mleaders, emsg );
}

static void
check_test_epoch( fd_multi_epoch_leaders_t const * mleaders,
                  ulong                            epoch,
                  int                              present,
                  uchar                            leader_byte ) {
  ulong slot0 = epoch*32UL;
  fd_epoch_leaders_t const * lsched = fd_rpc_mleaders_get_lsched_for_slot( mleaders, slot0 );
  if( !present ) {
    FD_TEST( !lsched );
    FD_TEST( !fd_rpc_mleaders_get_leader_for_slot( mleaders, slot0 ) );
    return;
  }

  FD_TEST( lsched );
  FD_TEST( lsched->epoch==epoch );
  FD_TEST( lsched->slot0==slot0 );
  FD_TEST( lsched->slot_cnt==32UL );
  for( ulong slot=slot0; slot<slot0+32UL; slot++ ) {
    fd_pubkey_t const * leader = fd_rpc_mleaders_get_leader_for_slot( mleaders, slot );
    FD_TEST( leader );
    for( ulong i=0UL; i<sizeof(fd_pubkey_t); i++ ) FD_TEST( leader->uc[ i ]==leader_byte );
  }
}

static void
test_rpc_mleaders( fd_rpc_tile_t * ctx ) {
  fd_multi_epoch_leaders_t * mleaders = ctx->mleaders;
  fd_rpc_mleaders_init( mleaders );

  for( ulong epoch=0UL; epoch<FD_RPC_MLEADERS_EPOCH_CNT; epoch++ ) ingest_test_epoch( mleaders, epoch, (uchar)(epoch+1UL) );
  for( ulong epoch=0UL; epoch<FD_RPC_MLEADERS_EPOCH_CNT; epoch++ ) check_test_epoch( mleaders, epoch, 1, (uchar)(epoch+1UL) );

  ingest_test_epoch( mleaders, 10UL, 11U );
  check_test_epoch( mleaders, 0UL, 0, 0U );
  for( ulong epoch=1UL; epoch<=10UL; epoch++ ) check_test_epoch( mleaders, epoch, 1, (uchar)(epoch+1UL) );

  ingest_test_epoch( mleaders, 10UL, 0x66U );
  for( ulong epoch=1UL; epoch<=10UL; epoch++ )
    check_test_epoch( mleaders, epoch, 1, epoch==10UL ? 0x66U : (uchar)(epoch+1UL) );

  ingest_test_epoch( mleaders, 11UL, 12U );
  check_test_epoch( mleaders, 1UL, 0, 0U );
  for( ulong epoch=2UL; epoch<=11UL; epoch++ )
    check_test_epoch( mleaders, epoch, 1, epoch==10UL ? 0x66U : (uchar)(epoch+1UL) );

  fd_pubkey_t epoch2_leader;
  memset( epoch2_leader.uc, 3, sizeof(fd_pubkey_t) );
  FD_BASE58_ENCODE_32_BYTES( epoch2_leader.uc, epoch2_leader_b58 );
  ...
  FD_TEST( fd_cstr_printf_check( res_buf, sizeof(res_buf), NULL,
      "{\"jsonrpc\":\"2.0\",\"result\":{\"%s\":[%s]},\"id\":1}", epoch2_leader_b58, slots_buf ) );
  expect_rpc_response( ctx,
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"getLeaderSchedule\",\"params\":[64]}", res_buf );
  expect_rpc_response( ctx,
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"getLeaderSchedule\",\"params\":[0]}",
      "{\"jsonrpc\":\"2.0\",\"result\":null,\"id\":1}" );

  FD_TEST( fd_cstr_printf_check( req_buf, sizeof(req_buf), NULL,
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"getSlotLeaders\",\"params\":[64,4]}" ) );
  FD_TEST( fd_cstr_printf_check( res_buf, sizeof(res_buf), NULL,
      "{\"jsonrpc\":\"2.0\",\"result\":[\"%s\",\"%s\",\"%s\",\"%s\"],\"id\":1}",
      epoch2_leader_b58, epoch2_leader_b58, epoch2_leader_b58, epoch2_leader_b58 ) );
...
#undef PUBLISH_SHRED
     ...
     accounts. */
  {
    ...
    fd_epoch_leaders_t const * dup_lsched = fd_rpc_mleaders_get_lsched_for_slot( ctx->mleaders, 0UL );
