#ifndef HEADER_fd_src_discof_rpc_fd_rpc_h
#define HEADER_fd_src_discof_rpc_fd_rpc_h
...
struct fd_rpc_tile {
  ...
  /* From bzip2 docs:
        To guarantee that the compressed data will fit in its buffer,Expand commentComment on line R260Resolved
...
static inline ulong
fd_rpc_file_as_tarball( fd_rpc_tile_t * ctx FD_PARAM_UNUSED,
                        char const *    filename_cstr,
                        uchar const *   data,
                        ulong           data_sz,
                        uchar *         scratch,
                        ulong           scratch_sz,
                        uchar *         out FD_PARAM_UNUSED,
...
#define FD_RPC_STAGE(__ctx) (__extension__({ \
...
#define FD_RPC_RESPONSE(__ctx, ...) (__extension__({ \
  fd_http_server_printf( __ctx->http, __VA_ARGS__ ); \
  fd_http_server_response_t __res = FD_RPC_STAGE( __ctx ); \
  __res; }))
...
fd_rpc_validate_params( fd_rpc_tile_t *             ctx,
  ...
  */
  ...
  if( FD_UNLIKELY( !params ) ) param_cnt = 0UL;
    ...
    *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32600,\"message\":\"Invalid request\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
    return 0;
  }
  else if( FD_UNLIKELY( cJSON_IsObject( params ) && max_cnt==0UL ) ) {
    *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid parameters: No parameters were expected\",\"data\":\"\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
    return 0;
  }
  else if( FD_UNLIKELY( cJSON_IsObject( params ) && max_cnt>0UL ) ) {
    *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"`params` should be an array\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
    return 0;
  }
  ...
  if( FD_UNLIKELY( param_cnt<min_cnt ) ) {
    *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"`params` should have at least %lu argument(s)\"},\"id\":%s}\n", min_cnt, cJSON_PrintUnformatted( id ) );
    return 0;
  }
  if( param_cnt>max_cnt ) {
    *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid parameters: Expected from %lu to %lu parameters.\",\"data\":\"\\\"Got: %lu\\\"\"},\"id\":%s}\n", min_cnt, max_cnt, param_cnt, cJSON_PrintUnformatted( id ) );
    return 0;
  }
...
fd_rpc_validate_config( fd_rpc_tile_t *             ctx,
  ...
  if( FD_UNLIKELY( config && (cJSON_IsNumber( config ) || cJSON_IsBool( config )) ) ) {
    *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s `%s`, expected %s.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( config ), cJSON_PrintUnformatted( config ), config_rust_type, cJSON_PrintUnformatted( id ) );
    return 0;
  }
  ...
  if( FD_UNLIKELY( cJSON_IsArray( config ) ) ) {
    *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Firedancer Error: Positional config params not supported\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
    return 0;
  }
  if( FD_UNLIKELY( config && !(cJSON_IsNull( config ) || cJSON_IsObject( config )) ) ) {
    *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s, expected %s.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( config ), config_rust_type, cJSON_PrintUnformatted( id ) );
    return 0;
  }
  ...
  *bank_idx = _bank_idx;
  ...
  if( FD_LIKELY( has_encoding ) ) {
    ...
    if( FD_UNLIKELY( cJSON_IsNumber( encoding ) || cJSON_IsBool( encoding ) ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s `%s`, expected string or map.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( encoding ), cJSON_PrintUnformatted( encoding ), cJSON_PrintUnformatted( id ) );
      return 0;
    }
    if( FD_UNLIKELY( cJSON_IsObject( encoding ) && !(encoding->child && encoding->child->next==NULL) ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid value: map, expected map with a single key.\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
      return 0;
    }
    if( FD_UNLIKELY( encoding && !cJSON_IsString( encoding ) && !cJSON_IsNull( encoding ) && !cJSON_IsObject( encoding ) ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s, expected string or map.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( encoding ), cJSON_PrintUnformatted( id ) );
      return 0;
    }
    ...
    if( FD_UNLIKELY( cJSON_IsObject( encoding ) ) ) {
      if( cJSON_HasObjectItem( encoding, "binary" ) ) encoding_cstr = "binary";
        ...
        *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: unknown variant `%s`, expected one of `binary`, `base58`, `base64`, `jsonParsed`, `base64+zstd`.\"},\"id\":%s}\n", encoding->child->string, cJSON_PrintUnformatted( id ) );
        return 0;
      }
    } else {
    ...
    if( FD_UNLIKELY( cJSON_IsObject( encoding ) && (cJSON_IsNumber( encoding->child ) || cJSON_IsBool( encoding->child )) ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s `%s`, expected unit.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( encoding->child ), cJSON_PrintUnformatted( encoding->child ), cJSON_PrintUnformatted( id ) );
      return 0;
    }
    ...
    if( FD_UNLIKELY( cJSON_IsObject( encoding ) && !cJSON_IsNull( encoding->child ) ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s, expected unit.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( encoding->child ), cJSON_PrintUnformatted( id ) );
      return 0;
    }

    if( 0==strcmp( encoding_cstr, "jsonParsed" ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32065,\"message\":\"Firedancer Error: jsonParsed is unsupported\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
      return 0;
  ...
  if( FD_LIKELY( has_data_slice ) ) {
    ...
    if( FD_UNLIKELY( cJSON_IsNumber( dataSlice ) || cJSON_IsBool( dataSlice ) ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s `%s`, expected struct UiDataSliceConfig.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( dataSlice ), cJSON_PrintUnformatted( dataSlice ), cJSON_PrintUnformatted( id ) );
      return 0;
    }
    ...
    if( FD_UNLIKELY( dataSlice && !cJSON_IsObject( dataSlice ) && !cJSON_IsNull( dataSlice ) && !cJSON_IsArray( dataSlice ) ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s, expected struct UiDataSliceConfig.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( dataSlice ), cJSON_PrintUnformatted( id ) );
      return 0;
    }
    ...
    if( FD_UNLIKELY( cJSON_IsBool( _offset ) || (cJSON_IsNumber( _offset ) && !fd_rpc_cjson_is_integer( _offset )) ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s `%s`, expected usize.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( _offset ), cJSON_PrintUnformatted( _offset ), cJSON_PrintUnformatted( id ) );
      return 0;
    }
    ...
    if( FD_UNLIKELY( cJSON_IsNumber( _offset ) && _offset->valueint<0 ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid value: %s `%s`, expected usize.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( _offset ), cJSON_PrintUnformatted( _offset ), cJSON_PrintUnformatted( id ) );
      return 0;
    }
    ...
    if( FD_UNLIKELY( _offset && !fd_rpc_cjson_is_integer( _offset ) ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s, expected usize.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( _offset ), cJSON_PrintUnformatted( id ) );
      return 0;
    }
    ...
    if( FD_UNLIKELY( cJSON_IsObject( dataSlice ) && !has_offset ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: missing field `offset`.\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
      return 0;
    }
    if( FD_UNLIKELY( cJSON_IsObject( dataSlice ) && !has_length ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: missing field `length`.\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
      return 0;
    }

    if( FD_UNLIKELY( cJSON_IsArray( dataSlice ) && cJSON_GetArraySize( dataSlice )!=2 ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid length %lu, expected struct UiDataSliceConfig with 2 elements.\"},\"id\":%s}\n", (ulong)cJSON_GetArraySize( dataSlice ), cJSON_PrintUnformatted( id ) );
      return 0;
    }
  ...
  if( FD_LIKELY( has_min_context_slot ) ) {
    ...
    if( FD_UNLIKELY( cJSON_IsBool( _minContextSlot ) || (cJSON_IsNumber( _minContextSlot ) && !fd_rpc_cjson_is_integer( _minContextSlot )) ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s `%s`, expected u64.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( _minContextSlot ), cJSON_PrintUnformatted( _minContextSlot ), cJSON_PrintUnformatted( id ) );
      return 0;
    }

    if( FD_UNLIKELY( cJSON_IsNumber( _minContextSlot ) && _minContextSlot->valueint<0 ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid value: %s `%s`, expected u64.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( _minContextSlot ), cJSON_PrintUnformatted( _minContextSlot ), cJSON_PrintUnformatted( id ) );
      return 0;
    }
    ...
    if( FD_UNLIKELY( _minContextSlot && !cJSON_IsNull( _minContextSlot ) && !fd_rpc_cjson_is_integer( _minContextSlot ) ) ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s, expected u64.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( _minContextSlot ), cJSON_PrintUnformatted( id ) );
      return 0;
    }
    ...
    if( _bank_idx!=ULONG_MAX && ctx->banks[ _bank_idx ].slot<minContextSlot ) {
      *res = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":%d,\"message\":\"Minimum context slot has not been reached\",\"data\":{\"contextSlot\":%lu}},\"id\":%s}\n", FD_RPC_ERROR_MIN_CONTEXT_SLOT_NOT_REACHED, ctx->banks[ _bank_idx ].slot, cJSON_PrintUnformatted( id ) );
      return 0;
    }
  }
...
fd_rpc_validate_address( fd_rpc_tile_t *             ctx,
  ...
  if( FD_UNLIKELY( cJSON_IsNumber( address_in ) || cJSON_IsBool( address_in ) ) ) {
    *response = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s `%s`, expected a string.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( address_in ), cJSON_PrintUnformatted( address_in ), cJSON_PrintUnformatted( id ) );
    return 0;
  }
  if( FD_UNLIKELY( !cJSON_IsString( address_in ) ) ) {
    *response = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s, expected a string.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( address_in ), cJSON_PrintUnformatted( id ) );
    return 0;
  }
  int invalid_char = fd_rpc_cstr_contains_non_base58( address_in->valuestring );
  if( FD_UNLIKELY( invalid_char ) ) {
    *response = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid param: Invalid\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
    return 0;
  }
  int valid = !!fd_base58_decode_32( address_in->valuestring, address_out->uc );
  if( FD_UNLIKELY( !valid ) ) {
    *response = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid param: WrongSize\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
    return 0;
  }
...
getAccountInfo( fd_rpc_tile_t * ctx,
  ...
  if( FD_UNLIKELY( !fd_accdb_open_ro( ctx->accdb, ro, &xid, address.uc ) ) ) {
    return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"result\":{\"context\":{\"slot\":%lu},\"value\":null},\"id\":%s}\n", info->slot, cJSON_PrintUnformatted( id ) );
  }
  ...
  if( FD_UNLIKELY( (is_binary || is_base58) && snip_sz>128UL ) ) {
    ...
    return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Encoded binary (base 58) data should be less than {MAX_BASE58_BYTES} bytes, please use Base64 encoding.\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
  }
...
# else
  if( is_zstd ) {
    fd_accdb_close_ro( ctx->accdb, ro );
    return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32065,\"message\":\"Firedancer Error: zstandard is disabled\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
  }
# endif
...
getBalance( fd_rpc_tile_t * ctx,
  ...
  return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"result\":{\"context\":{\"apiVersion\":\"%s\",\"slot\":%lu},\"value\":%lu},\"id\":%s}\n", FD_RPC_AGAVE_API_VERSION, ctx->banks[ bank_idx ].slot, balance, cJSON_PrintUnformatted( id ) );
}
...
getHealth( fd_rpc_tile_t * ctx,
  ...
  switch( health_status ) {
    case FD_RPC_HEALTH_STATUS_UNKNOWN: return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":%d,\"message\":\"Node is unhealthy\",\"data\":{\"slotsBehind\":null}},\"id\":%s}\n", FD_RPC_ERROR_NODE_UNHEALTHY, cJSON_PrintUnformatted( id ) );
    case FD_RPC_HEALTH_STATUS_BEHIND:  return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":%d,\"message\":\"Node is unhealthy\",\"data\":{\"slotsBehind\":%lu}},\"id\":%s}\n", FD_RPC_ERROR_NODE_UNHEALTHY, fd_ulong_sat_sub( ctx->cluster_confirmed_slot, ctx->banks[ ctx->confirmed_idx ].slot ), cJSON_PrintUnformatted( id ) );
    case FD_RPC_HEALTH_STATUS_OK:      return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"result\":\"ok\",\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
...
getIdentity( fd_rpc_tile_t * ctx,
  ...
  return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"result\":{\"identity\":\"%s\"},\"id\":%s}\n", identity_pubkey_b58, cJSON_PrintUnformatted( id ) );
}
...
getInflationGovernor( fd_rpc_tile_t * ctx,
  ...
  return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"result\":{\"foundation\":%g%s,\"foundationTerm\":%g%s,\"initial\":%g%s,\"taper\":%g%s,\"terminal\":%g%s},\"id\":%s}\n",
...
getLatestBlockhash( fd_rpc_tile_t * ctx,
  ...
  return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"result\":{\"context\":{\"slot\":%lu,\"apiVersion\":\"%s\"},\"value\":{\"blockhash\":\"%s\",\"lastValidBlockHeight\":%lu}},\"id\":%s}\n", bank->slot, FD_RPC_AGAVE_API_VERSION, block_hash_b58, bank->block_height + 150UL - age, cJSON_PrintUnformatted( id ) );
}
...
getMinimumBalanceForRentExemption( fd_rpc_tile_t * ctx,
  ...
  if( FD_UNLIKELY( cJSON_IsBool( acct_sz ) || (cJSON_IsNumber( acct_sz ) && !fd_rpc_cjson_is_integer( acct_sz )) ) ) {
    return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s `%s`, expected usize.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( acct_sz ), cJSON_PrintUnformatted( acct_sz ), cJSON_PrintUnformatted( id ) );
  }
  if( FD_UNLIKELY( cJSON_IsNumber( acct_sz ) && acct_sz->valueint<0 ) ) {
    return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid value: %s `%s`, expected usize.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( acct_sz ), cJSON_PrintUnformatted( acct_sz ), cJSON_PrintUnformatted( id ) );
  }
  ...
  if( FD_UNLIKELY( acct_sz && !fd_rpc_cjson_is_integer( acct_sz ) ) ) {
    return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32602,\"message\":\"Invalid params: invalid type: %s, expected usize.\"},\"id\":%s}\n", fd_rpc_cjson_type_to_cstr( acct_sz ), cJSON_PrintUnformatted( id ) );
  }
...
getVersion( fd_rpc_tile_t * ctx,
  ...
  return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"result\":{\"solana-core\":\"%s\",\"feature-set\":%u},\"id\":%s}\n", ctx->version_string, FD_FEATURE_SET_ID, cJSON_PrintUnformatted( id ) );
}
...
static inline fd_http_server_response_t
rpc_http_request( fd_http_server_request_t const * request ) {
  ...
  if( FD_UNLIKELY( request->method==FD_HTTP_SERVER_METHOD_GET && !strcmp( request->path, "/health" ) ) ) {
    ...
    switch( health_status ) {
      case FD_RPC_HEALTH_STATUS_UNKNOWN: return FD_RPC_RESPONSE( ctx, "unknown" );
      case FD_RPC_HEALTH_STATUS_BEHIND:  return FD_RPC_RESPONSE( ctx, "behind" );
      case FD_RPC_HEALTH_STATUS_OK:      return FD_RPC_RESPONSE( ctx, "ok" );
      default: FD_LOG_ERR(( "unknown health status" ));
    }
  }
  ...
  if( FD_UNLIKELY( !json || !cJSON_IsObject( json ) ) ) {
    cJSON_Delete( json );
    return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32700,\"message\":\"Parse error\"},\"id\":null}\n" );
  }
  ...
  while( item ) {
    if( FD_UNLIKELY( strcmp( item->string, "jsonrpc" ) && strcmp( item->string, "id" ) && strcmp( item->string, "method" ) && strcmp( item->string, "params" ) ) ) {
      cJSON_Delete( json );
      return FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32600,\"message\":\"Invalid request\"},\"id\":%s}\n", id ? cJSON_PrintUnformatted( id ) : "null" );
    }
    item = item->next;
  }
  ...
  if( FD_UNLIKELY( !cJSON_HasObjectItem( json, "jsonrpc" ) && cJSON_HasObjectItem( json, "method" ) ) ) {
    fd_http_server_response_t response = FD_RPC_RESPONSE( ctx, "{\"error\":{\"code\":-32600,\"message\":\"Unsupported JSON-RPC protocol version\"},\"id\":%s}\n", id ? cJSON_PrintUnformatted( id ) : "null" );
    cJSON_Delete( json );
    return response;
  }
  ...
  if( FD_UNLIKELY( !cJSON_IsString( jsonrpc ) || strcmp( jsonrpc->valuestring, "2.0" ) ) ) {
    fd_http_server_response_t response = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32600,\"message\":\"Invalid request\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );
    cJSON_Delete( json );
    return response;
  }
  ...
  else response = FD_RPC_RESPONSE( ctx, "{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32601,\"message\":\"Method not found\"},\"id\":%s}\n", cJSON_PrintUnformatted( id ) );

  cJSON_Delete( json );
  return response;
}

#endif /* HEADER_fd_src_discof_rpc_fd_rpc_h */
