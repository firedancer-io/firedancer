...
/* The default STEM_LAZY is based on cr_max, which is the minimum depth
   ...
   RPC has one tiny output link used to release banks, with a significanly
   slower line rate than assumed in the forumla for the default
   STEM_LAZY value.

   Instead, lazy just needs to be frequent enough to relinquish credits
   to upstream producers faster than they are exausted. 384us is a
