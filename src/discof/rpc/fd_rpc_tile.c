...
loose_footprint( fd_topo_tile_t const * tile FD_PARAM_UNUSED ) {
  return 96UL * (1UL<<20UL); /* heap for the cJSON request parser (requests capped at 8 KiB) and the one-shot bz2 genesis compressor */
}
...
populate_allowed_seccomp( fd_topo_t const *      topo,
  ...
  populate_sock_filter_policy_fd_rpc_tile( out_cnt, out, (uint)fd_log_private_logfile_fd(), (uint)fd_http_server_fd( ctx->http ), (uint)FD_ACCDB_FD_RO, (uint)FD_ACCDB_IDX_FD_RO );
...
populate_allowed_fds( fd_topo_t const *      topo,
  ...
  out_fds[ out_cnt++ ] = FD_ACCDB_IDX_FD_RO; /* accounts index readonly fd */
