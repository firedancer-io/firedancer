...
static uchar file_bin[ FD_GENESIS_MAX_MESSAGE_SIZE ];
static uchar tar_scratch[ FD_RPC_TAR_SZ ];

#define CEIL_DIV(a, b) (((a)+(b)-1)/(b))
static uchar tar_bz_out[ FD_RPC_TAR_SZ+CEIL_DIV( FD_RPC_TAR_SZ, 100UL )+600UL ];
#undef CEIL_DIV

static fd_rpc_tile_t ctx[ 1 ];
...
#define FETCH_TYPE(__type) (FD_LOAD(__type, FETCH_REF(sizeof(__type))))
  ...
  ulong file_bin_sz = FETCH_TYPE( uint ) % (FD_GENESIS_MAX_MESSAGE_SIZE + 1UL);

  fd_hash( FETCH_TYPE( ulong ), file_bin, file_bin_sz );
