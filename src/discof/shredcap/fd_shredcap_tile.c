...
struct fd_capture_tile_ctx {
  ...
  /* vote_stakes buffers for manifest epoch_stakes.  E-1, E, and E+1 use
     the slim struct since shredcap only needs vote/identity/stake
     (no epoch_credits). */
  fd_snapshot_epoch_stakes_slim_t * vote_stakes_slim[ 3 ];
  ulong                             vote_stakes_slim_len[ 3 ];
};
...
shredcap_on_vote_stakes( void *                                     ctx_,
  ...
  if( FD_UNLIKELY( epoch_idx>=3UL ) ) return;
...
publish_stake_weights_manifest( fd_capture_tile_ctx_t * ctx,
  ...
  ulong stake_weights_sz = generate_epoch_info_msg_manifest( epoch, schedule, ctx->vote_stakes_slim[1], ctx->vote_stakes_slim_len[1], stake_weights_msg );
  ...
  stake_weights_sz = generate_epoch_info_msg_manifest( epoch + 1, schedule, ctx->vote_stakes_slim[2], ctx->vote_stakes_slim_len[2], stake_weights_msg );
...
after_credit( fd_capture_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( !ctx->manifest_load_done ) ) {
    if( FD_LIKELY( !!strcmp( ctx->manifest_path, "") ) ) {
      ...
      ctx->vote_stakes_slim_len[2] = 0UL;
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->vote_stakes_slim[2]     = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_snapshot_epoch_stakes_slim_t), sizeof(fd_snapshot_epoch_stakes_slim_t)*FD_SHREDCAP_MAX_VOTE_STAKES_BUFFERED );
  ctx->vote_stakes_slim_len[0] = 0UL;
  ctx->vote_stakes_slim_len[1] = 0UL;
  ctx->vote_stakes_slim_len[2] = 0UL;
