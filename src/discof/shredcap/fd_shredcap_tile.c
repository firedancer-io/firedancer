...
#define MANIFEST_MAX_BANKS                 (2UL) /* the minimum is 2 */
...
manifest_bank_footprint( void ) {
  return fd_banks_footprint( MANIFEST_MAX_BANKS );
}
...
after_credit( fd_capture_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( !ctx->manifest_load_done ) ) {
    if( FD_LIKELY( !!strcmp( ctx->manifest_path, "") ) ) {
      ...
      /* ctx->shared_spad is used to load the raw manifest file. */
      fd_spad_reset( ctx->shared_spad );
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->manifest_exec_slot_ctx->banks = fd_banks_join( fd_banks_new( ctx->manifest_bank_mem, MANIFEST_MAX_BANKS ) );
