...
static void
update_fec_count_in_file( fd_feccap_tile_ctx_t * ctx, ulong count ) {
  /* Flush buffer before seeking */
  if( FD_UNLIKELY( fd_io_buffered_ostream_flush( &ctx->feccap_ostream ) != 0 ) ) {
    FD_LOG_WARNING(( "FECCAP: Failed to flush before updating count" ));
  }

  /* Seek to count position and write directly */
  long seek_pos = lseek( ctx->feccap_fd, 8, SEEK_SET );
  if( FD_UNLIKELY( seek_pos != 8L ) ) {
    FD_LOG_WARNING(( "FECCAP: Failed to seek to count position" ));
    return;
  }

  ulong wsz;
  int err = fd_io_write( ctx->feccap_fd, &count, sizeof(ulong), sizeof(ulong), &wsz );
  if( FD_UNLIKELY( err != 0 || wsz != sizeof(ulong) ) ) {
    FD_LOG_WARNING(( "FECCAP: Failed to write count: err=%d wsz=%lu", err, wsz ));
  }
}
...
feccap_cleanup( void ) {
  if( FD_LIKELY( g_feccap_ctx && g_feccap_ctx->feccap_fd >= 0 ) ) {
    ...
    update_fec_count_in_file( g_feccap_ctx, g_feccap_ctx->fecs_count );
...
setup_feccap_file( fd_feccap_tile_ctx_t * ctx ) {
  ...
  ctx->feccap_fd = open( feccap_file_path, O_RDWR | O_CREAT | O_TRUNC, 0666 );
...
process_fec_complete( fd_feccap_tile_ctx_t * ctx, fd_hash_t const * merkle_root ) {
  ...
  FD_STORE_SHARED_LOCK( ctx->store, shacq_start, shacq_end, shrel_end ) {
    ...
    if( FD_LIKELY( fd_io_buffered_ostream_write( &ctx->feccap_ostream, &hdr, sizeof(fd_feccap_chunk_hdr_t) ) == 0 &&
      ...
      ctx->fecs_count++;
      update_fec_count_in_file( ctx, ctx->fecs_count );
    } else {
...
process_shred_received( fd_feccap_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( fd_io_buffered_ostream_write( &ctx->feccap_ostream, &hdr, sizeof(fd_feccap_chunk_hdr_t) ) != 0 ||
