...
privileged_init( fd_topo_t *     topo FD_PARAM_UNUSED,
  ...
  if ( FD_UNLIKELY( ftruncate(ctx->volume_0_fd, FD_GROOVE_VOLUME_FOOTPRINT) == -1 ) ) {
    FD_LOG_ERR(("Failed to truncate cold_store_volume_0_fd at path %s: %s (errno = %d)",
      volume_0_path, strerror(errno), errno));
  }
}
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->volume_0_shmem = mmap( NULL, FD_GROOVE_VOLUME_FOOTPRINT, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_HUGETLB, ctx->volume_0_fd, 0 );
  ...
  if( FD_UNLIKELY( fd_groove_data_volume_add(
    ...
    NULL,
    0UL ) ) ) {
    FD_LOG_ERR(( "Failed to add volume to Groove" ));
  }
  ctx->volume_count = 1UL;
...
fd_groove_map_new_volume( fd_groove_tile_ctx_t * ctx ) {
  ...
  uchar * new_volume = mmap( NULL, FD_GROOVE_VOLUME_FOOTPRINT, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_HUGETLB, volume_fd, 0 );
  if( FD_UNLIKELY( new_volume == MAP_FAILED ) ) {
    FD_LOG_ERR(("Failed to memory map new volume at path %s: %s (errno = %d)", volume_path, strerror(errno), errno));
  }
