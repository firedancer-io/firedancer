...
fd_groove_map_new_volume( fd_groove_tile_ctx_t * ctx ) {
  ...
  uchar * new_volume = mmap(
    (uchar *)ctx->volume_0_mmap_addr + ctx->volume_count * FD_GROOVE_VOLUME_FOOTPRINT,
...
fd_snapshot_acc_finish_read_groove_cb( void *              _ctx,
  ...
  if( FD_LIKELY( is_evictable_funk_record( pubkey, fd_type_pun_const( meta->info.owner ) ) ) ) {
    return;
  }
  ...
  fd_funk_txn_t * funk_txn = fd_funk_txn_query( fd_funk_root( funk ), ctx->tile_ctx->txn_map );
  fd_funk_txn_end_read( funk );

  fd_account_meta_t const * rec_meta = fd_funk_get_acc_meta_readonly(
      funk, funk_txn, pubkey, NULL, NULL, NULL );
  ...
  int write_result = fd_txn_account_init_from_funk_mutable( acct, pubkey, funk, funk_txn, /* do_create */ 1, meta->dlen );
  ...
  fd_funk_rec_lru_add( funk, acct->private_state.rec );

  fd_txn_account_mutable_fini( acct, funk, funk_txn );
}
...
evict_least_recently_used_account( fd_groove_tile_ctx_t * ctx ) {
  ...
  for ( ; ; ) {
    ...
    /* Check that the account is evictable */
