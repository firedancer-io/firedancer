...
quic_tls_cv_sign( void *      signer_ctx,
  ...
  long dt = -fd_clock_epoch_y( ctx->epoch, fd_tickcount() );
  fd_keyguard_client_sign( ctx->keyguard_client, signature, payload, 130UL, FD_KEYGUARD_SIGN_TYPE_ED25519 );
  dt += ( ctx->now = fd_clock_epoch_y( ctx->epoch, fd_tickcount() ) );
...
during_housekeeping( fd_send_tile_ctx_t * ctx ) {
  ...
  if( FD_UNLIKELY( ctx->recal_next <= ctx->now ) ) {
    ctx->recal_next = fd_clock_recal( ctx->clock, ctx->now, ctx->now );
    fd_clock_epoch_refresh( ctx->epoch, fd_type_pun( ctx->clock_mem ) );
  }
...
ensure_conn_for_slot( fd_send_tile_ctx_t * ctx,
  ...
  for( ulong i=0UL; i<FD_SEND_PORT_QUIC_CNT; i++ ) {
    ...
    if( !entry->conn[i] ) {
      ...
      fd_quic_service_sampled( ctx->quic, ctx->epoch );
    } else {
...
leader_send( fd_send_tile_ctx_t  *  ctx,
  ...
  for( ulong i=0UL; i<FD_SEND_PORT_CNT; i++ ) {
    ...
    if( port_idx_is_quic( i ) ) {
      ...
      fd_quic_service_sampled( ctx->quic, ctx->epoch ); /* trigger send ASAP */
    } else {
...
before_credit( fd_send_tile_ctx_t * ctx,
  ...
  ctx->now = fd_clock_epoch_y( ctx->epoch, fd_tickcount() );
  /* Publishes to mcache via callbacks */
  *charge_busy = fd_quic_service_sampled( ctx->quic, ctx->epoch );
}
...
after_frag( fd_send_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( kind==IN_KIND_NET ) ) {
    ...
    fd_quic_process_packet_sampled( quic, ip_pkt, ip_sz, ctx->epoch );
  }
...
static double
quic_init_w_fn(void) {
  return fd_tempo_tick_per_ns(NULL);
}
...
unprivileged_init( fd_topo_t *      topo,
  ...
  fd_clock_t * clock = fd_clock_default_init( ctx->clock_ljoin, ctx->clock_mem, ctx->epoch, quic_init_w_fn );
