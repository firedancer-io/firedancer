...
static void populate_sock_filter_policy_fd_execrp_tile( ulong out_cnt, struct sock_filter out[ static 57 ], uint logfile_fd, uint accounts_fd, uint stake_spill_fd ) {
  FD_TEST( out_cnt >= 57 );
  struct sock_filter filter[57] = {
    ...
    /* check pwritev2 */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_pwritev2, /* check_pwritev2 */ 22, 0 ),
    /* check preadv2 */
