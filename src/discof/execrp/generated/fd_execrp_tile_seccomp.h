...
#endif
static const unsigned int sock_filter_policy_fd_execrp_tile_instr_cnt = 23;
...
static void populate_sock_filter_policy_fd_execrp_tile( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int accounts_fd ) {
  ...
  struct sock_filter filter[23] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_clock_nanosleep, /* check_clock_nanosleep */ 9, 0 ),
    /* allow pwritev2 based on expression */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_pwritev2, /* check_pwritev2 */ 10, 0 ),
...
//  check_clock_nanosleep:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, CLOCK_REALTIME, /* RET_ALLOW */ 5, /* RET_KILL_PROCESS */ 4 ),
//  check_pwritev2:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
...
//  check_pread64:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, accounts_fd, /* RET_ALLOW */ 1, /* RET_KILL_PROCESS */ 0 ),
//  RET_KILL_PROCESS:
