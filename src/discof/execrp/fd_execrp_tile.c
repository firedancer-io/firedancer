...
unprivileged_init( fd_topo_t const *      topo,
  ...
  FD_TEST( ctx->accdb );
  if( FD_UNLIKELY( tile->execrp.accdb_delta_obj_id!=ULONG_MAX ) ) {
    fd_accdb_delta_t * delta = fd_accdb_delta_join( fd_topo_obj_laddr( topo, tile->execrp.accdb_delta_obj_id ) );
    FD_TEST( delta );
