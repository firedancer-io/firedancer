...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  l = FD_LAYOUT_APPEND(   l, FD_RUNTIME_EBPF_HOST_ALIGN,   FD_BPF_SER_FRAME1_WINDOW_FOOTPRINT                          );
...
metrics_write( fd_execrp_tile_t * ctx ) {
  ...
  FD_MCNT_SET  ( EXECRP, SER_ARENA_PROMOTE,        runtime->bpf_loader_serialization.promote_cnt      );
  FD_MGAUGE_SET( EXECRP, SER_TOP_FRAME_HIGH_WATER, runtime->bpf_loader_serialization.frame1_watermark );
  FD_MGAUGE_SET( EXECRP, SER_WINDOW_HIGH_WATER,    runtime->bpf_loader_serialization.window_watermark );
...
unprivileged_init( fd_topo_t const *      topo,
  ...
  void * _bpfser_frame1     = FD_SCRATCH_ALLOC_APPEND( l, FD_RUNTIME_EBPF_HOST_ALIGN,   FD_BPF_SER_FRAME1_WINDOW_FOOTPRINT                          );
  ...
  FD_TEST( fd_progcache_join( ctx->progcache, fd_topo_obj_laddr( topo, tile->execrp.progcache_obj_id ), ctx->runtime->bpf_loader_program.rodata, FD_PROGCACHE_SCRATCH_FOOTPRINT ) );
...
populate_allowed_seccomp( fd_topo_t const *      topo FD_PARAM_UNUSED,
  ...
  populate_sock_filter_policy_fd_execrp_tile( out_cnt, out, (uint)fd_log_private_logfile_fd(), (uint)FD_ACCDB_FD_RW, FD_STAKE_DELEGATIONS_FD, (uint)FD_TXNCACHE_FD, (uint)FD_ACCDB_IDX_FD_RW );
