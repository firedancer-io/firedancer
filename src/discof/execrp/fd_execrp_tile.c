    ...
    switch( sig>>32 ) {
      case FD_EXECRP_TT_TXN_EXEC: {
        ...
        if( FD_LIKELY( ctx->txn_out.err.is_committable ) ) {
          ...
          fd_runtime_cancel_txn( ctx->runtime, ctx->bank, &ctx->txn_in, &ctx->txn_out );
        }
...
populate_allowed_seccomp( fd_topo_t const *      topo FD_PARAM_UNUSED,
  ...
  populate_sock_filter_policy_fd_execrp_tile( out_cnt, out, (uint)fd_log_private_logfile_fd(), (uint)FD_ACCDB_FD_RW, FD_STAKE_DELEGATIONS_FD, FD_COST_TRACKER_FD, FD_COLLECTOR_OVERRIDES_FD );
