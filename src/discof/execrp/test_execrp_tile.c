...
FD_UNIT_TEST( execrp_seccomp ) {
  ...
  /* logfile fd is optional; accounts db, stake spill, txncache tier,
     then accounts index are last */
  FD_TEST( out_fds[ nfds-4UL ]==FD_ACCDB_FD_RW );
  FD_TEST( out_fds[ nfds-3UL ]==FD_STAKE_DELEGATIONS_FD );
  FD_TEST( out_fds[ nfds-2UL ]==FD_TXNCACHE_FD );
  FD_TEST( out_fds[ nfds-1UL ]==FD_ACCDB_IDX_FD_RW );
