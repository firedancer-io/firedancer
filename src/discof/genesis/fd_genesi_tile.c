...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  l = FD_LAYOUT_APPEND( l, fd_genesis_client_align(),   fd_genesis_client_footprint( tile->genesi.entrypoints_cnt, tile->genesi.max_message_size ) );
  ...
  if( FD_LIKELY( ctx->local_genesis ) ) {
    ...
    if( FD_UNLIKELY( msg_sz>fd_genesi_tile_mtu( ctx->max_message_size ) ) ) {
      FD_LOG_ERR(( "The genesis file `%s` is too large for this Firedancer configuration (msg_sz=%lu max=%lu).\n"
                   "Cannot start Firedancer. Please use a different genesis config or increase `development.genesis.max_file_size_mib`.",
  ...
  for(;;) {
    if( FD_UNLIKELY( ctx->genesis_blob_sz==ctx->max_message_size ) ) {
      ...
      if( FD_UNLIKELY( result ) ) {
        FD_LOG_ERR(( "The genesis file at `%s` exceeds `development.genesis.max_file_size_mib` (%lu MiB).",
...
privileged_init( fd_topo_t const *      topo,
  ...
  fd_genesis_client_t * _client = FD_SCRATCH_ALLOC_APPEND( l, fd_genesis_client_align(),   fd_genesis_client_footprint( tile->genesi.entrypoints_cnt, tile->genesi.max_message_size ) );
    ...
    if( FD_LIKELY( errno==ENOENT  ) ) {
      ...
      if( FD_UNLIKELY( !tile->genesi.entrypoints_cnt ) ) {
        ...
        if( FD_UNLIKELY( !tile->genesi.allow_download ) ) {
          ...
          ctx->local_genesis = 0;
          ctx->client = fd_genesis_client_join( fd_genesis_client_new( _client, tile->genesi.entrypoints_cnt, tile->genesi.max_message_size ) );
          FD_TEST( ctx->client );
...
unprivileged_init( fd_topo_t const *      topo,
  ...
  FD_SCRATCH_ALLOC_INIT( l, scratch );
  fd_genesi_tile_t * ctx = FD_SCRATCH_ALLOC_APPEND( l, alignof( fd_genesi_tile_t ), sizeof( fd_genesi_tile_t )                        );
                           FD_SCRATCH_ALLOC_APPEND( l, fd_genesis_client_align(),   fd_genesis_client_footprint( tile->genesi.entrypoints_cnt, tile->genesi.max_message_size ) );
