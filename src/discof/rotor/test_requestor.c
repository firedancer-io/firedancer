...
shred( fd_chainer_t * chainer, ulong slot, uint idx, int slot_complete, fd_hash_t const * mr, ulong parent_slot, fd_hash_t const * parent_bid ) {
  fd_chainer_shred_insert( chainer, slot, idx, slot_complete, mr, parent_slot, parent_bid );
...
hash_insert( fd_chainer_t * chainer, ulong slot, fd_hash_t * block_id, uint fec_set_idx, fd_hash_t * mr ) {
  fd_chainer_verified_hash_insert( chainer, slot, block_id, fec_set_idx, mr );
  drain_chainer( chainer );
}
...
fec_complete( fd_chainer_t * chainer, ulong slot, uint fec_set_idx, int slot_complete, fd_hash_t const * mr ) {
  fd_hash_t m = *mr;
  int rejected;
  fd_chainer_fec_complete( chainer, slot, fec_set_idx, slot_complete, slot_complete, 0, &m, &rejected );
  FD_TEST( !rejected );
  drain_chainer( chainer );
}
...
static int
advance( fd_requestor_t * r, fd_chainer_t const * chainer, fd_rotor_request_t * req ) {
...
expect_req( fd_rotor_request_t const * req, uint kind, uint idx, fd_hash_t const * block_id, fd_hash_t const * fec_root ) {
  ...
  FD_TEST( fd_hash_eq( &req->fec_root, fec_root ? fec_root : &ZERO ) );
}

/* Turbine block: metadata rungs first (HighestShred while the tip is
   unknown), then a fill walk that requests exactly the missing shreds
   past the buffered prefix, then DONE once nothing is missing. */
...
test_turbine( fd_wksp_t * wksp ) {
  ...
  /* slot 11: first FEC set complete, tip unknown -> HighestShred */
  ...
  FD_TEST( cnt==1UL ); expect_req( &reqs[ 0 ], FD_REPAIR_KIND_HIGHEST_SHRED, 0U, NULL, NULL );
  ...
  fd_chainer_block_t const * s11 = fd_chainer_block_query( chainer, 11UL, &ZERO );
  ...
  FD_TEST( !fd_chainer_block_query( chainer, 11UL, &ZERO ) );
...
test_ancestry( fd_wksp_t * wksp ) {
  ...
  /* slot 20 names parent 15, which we do not have */
  ...
  /* slot 22 names absent parent 15 and knows its tip (99): the walk
     fills the first FD_REQUESTOR_ORPHAN_FILL_MAX holes, then Orphan */
  fd_hash_t r22a = mkhash( 22UL ), r22b = mkhash( 23UL ), r22c = mkhash( 24UL ); /* one root per FEC set touched: 0, 96, 32 */
  shred( chainer, 22UL, 0U,  0, &r22a, 15UL, &bid15 );
  shred( chainer, 22UL, 99U, 1, &r22b, AG_UNKNOWN_SLOT, NULL );
  fd_chainer_block_t const * s22 = fd_chainer_block_query( chainer, 22UL, &ZERO );
  FD_TEST( s22 && s22->complete_idx==99U );
  ...
  /* holes 1..32 land: the next walk asks for 33..64, then Orphan again */
  for( uint i=1U; i<=FD_REQUESTOR_ORPHAN_FILL_MAX; i++ ) shred( chainer, 22UL, i, 0, i<FD_FEC_SHRED_CNT ? &r22a : &r22c, AG_UNKNOWN_SLOT, NULL );
  FD_TEST( run_walk( r, chainer, 22UL, &ZERO, reqs, &cnt )==FD_REQUESTOR_ADVANCE_REQUESTED_PARENT );
  FD_TEST( cnt==FD_REQUESTOR_ORPHAN_FILL_MAX+1UL );
  expect_req( &reqs[ 0 ], FD_REPAIR_KIND_SHRED, FD_REQUESTOR_ORPHAN_FILL_MAX+1U, NULL, NULL );
  expect_req( &reqs[ FD_REQUESTOR_ORPHAN_FILL_MAX ], FD_REPAIR_KIND_ORPHAN, 0U, NULL, NULL );

  /* the parent shows up under the block_id the child names: the fill
     is no longer bounded and no Orphan is asked */
  ...
  FD_TEST( cnt==99UL-FD_REQUESTOR_ORPHAN_FILL_MAX-1UL ); /* 33..98 */
  expect_req( &reqs[ 0 ],     FD_REPAIR_KIND_SHRED, FD_REQUESTOR_ORPHAN_FILL_MAX+1U, NULL, NULL );
  expect_req( &reqs[ cnt-1 ], FD_REPAIR_KIND_SHRED, 98U, NULL, NULL );
...
test_gone( fd_wksp_t * wksp ) {
  ...
  fd_chainer_block_t * s14 = fd_chainer_block_query( chainer, 14UL, &ZERO );
...
test_moving( fd_wksp_t * wksp ) {
  ...
  fd_requestor_block_start( r, 12UL, &bid12 );
  FD_TEST( advance( r, chainer, req )==FD_REQUESTOR_ADVANCE_REQUEST && req->kind==AG_REPAIR_KIND_SHRED_FOR_BLOCK_ID && req->idx==0U && fd_hash_eq( &req->fec_root, &root0 ) );
  ...
  FD_TEST( fd_chainer_block_query( chainer, 12UL, &bid12 )->buffered_idx==31U );
  FD_TEST( advance( r, chainer, req )==FD_REQUESTOR_ADVANCE_REQUEST && req->kind==AG_REPAIR_KIND_FEC_ROOT && req->idx==32U );
  FD_TEST( advance( r, chainer, req )==FD_REQUESTOR_ADVANCE_REQUESTED );
  FD_TEST( advance( r, chainer, req )==FD_REQUESTOR_ADVANCE_IDLE );
