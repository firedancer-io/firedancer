...
pump( ctx_t * ctx ) {
  for( ulong i=0UL; i<200000UL; i++ ) {
    ...
    if( pub_cnt==before &&
        ...
        out_queue_empty( ctx->deliver_queue ) ) return;
  }
...
setup_ctx( ctx_t * ctx, fd_wksp_t * wksp ) {
  ...
  ulong deliver_max = TEST_BLOCK_MAX * ( FD_SHRED_BLK_MAX/FD_FEC_SHRED_CNT );
  ...
  void * deliver_mem   = fd_wksp_alloc_laddr( wksp, out_queue_align(),       out_queue_footprint( deliver_max ),                      1UL );
  void * store_mem     = fd_wksp_alloc_laddr( wksp, fd_store_align(),        fd_store_footprint( 1024UL, 64UL, 0UL, 0UL, 0UL ),       1UL );
  FD_TEST( chainer_mem && schedulor_mem && requestor_mem && stats_mem && policy_mem && rtt_mem && signs_map_mem && toss_mem && repair_mem && deliver_mem && store_mem );
  ...
  ctx->deliver_queue = out_queue_join      ( out_queue_new      ( deliver_mem,   deliver_max                                           ) );
  ctx->store         = fd_store_join       ( fd_store_new       ( store_mem,     1024UL, 64UL, 0UL, 0UL, 0UL, FD_SHRED_BLK_MAX, 42UL   ) );
  FD_TEST( ctx->chainer && ctx->schedulor && ctx->requestor && ctx->stats && ctx->policy && ctx->rtt && ctx->signs_map && ctx->toss_queue && ctx->protocol && ctx->deliver_queue && ctx->store );
  ...
  ctx->ag_nonce          = 0U;
  ctx->repair_out_cr_min = ULONG_MAX;
...
/* A turbine block arrives shred by shred.  The shred-0 marker names the
   parent, catchup seeding fires on the first turbine shred, each FEC
...
test_turbine_block( fd_wksp_t * wksp ) {
  ...
  FD_TEST( !v0->complete_ts );                              /* not whole yet */
  ...
  FD_TEST( v0->complete_ts );                               /* stamped on completion */
  FD_TEST( fd_hash_eq( &v0->block_id, &blk->block_id ) );   /* matches the independent computation */
  FD_TEST( fd_chainer_block_query( ctx->chainer, blk->slot, &blk->block_id )==v0 );
...
/* A votor block id for a slot turbine is still streaming creates a
   second block.  Both are repaired independently: the turbine block
   completes, finalizes and delivers under its own id, while the votor
   block is driven by ParentAndFecSetCount, FecSetRoot and
   ShredForBlockId and delivers under the id votor named. */

static void
test_votor_block_coexists( fd_wksp_t * wksp ) {
  static ctx_t ctx[1];
  setup_ctx( ctx, wksp );
  ...
  FD_TEST( !fd_chainer_verify( ctx->chainer ) );
  fd_chainer_block_t * v1 = fd_chainer_block_query( ctx->chainer, vot->slot, &vot->block_id );
  ...
  pump( ctx );
  FD_TEST( fd_chainer_block_complete( v1 ) && v1->complete_ts );
  FD_TEST( ctx->metrics->shred_match_block_id==FD_FEC_SHRED_CNT );

  /* the turbine block finishes on its own, unaffected */
  deliver_turbine_fec_set( ctx, turb, 1U );
  pump( ctx );
  FD_TEST( fd_chainer_block_complete( v0 ) && v0->complete_ts );
  FD_TEST( fd_hash_eq( &v0->block_id, &turb->block_id ) );
  FD_TEST( !fd_hash_eq( &v0->block_id, &vot->block_id ) );

  /* both delivered their own slot-complete FEC */
  ...
  FD_TEST( turb_cnt==1UL && vot_cnt==1UL );

  FD_TEST( !fd_chainer_verify( ctx->chainer ) );
  FD_LOG_NOTICE(( "pass: votor and turbine blocks of one slot coexist" ));
}
...
test_meta_verify( fd_wksp_t * wksp ) {
  ...
  FD_TEST( meta );
  fd_chainer_block_t * v = fd_chainer_block_query( ctx->chainer, blk->slot, &blk->block_id );
...
test_block_id_only( fd_wksp_t * wksp ) {
  ...
  deliver_turbine_fec_set( ctx, turb, 0U );
  pump( ctx );
  FD_TEST( req_find( 0UL, FD_REPAIR_KIND_SHRED, turb->slot, 0U, NULL ) ); /* the catchup seed */

  /* the block's own check asks for nothing: every rung it could reach
     is a legacy kind */
  ulong from = req_cnt;
...
test_publish_prunes( fd_wksp_t * wksp ) {
  ...
  FD_TEST( fd_schedulor_block_query( ctx->schedulor, b2->slot, &(fd_hash_t){0} ) );
  FD_TEST( ctx->metrics->replay_by_sig[ REPLAY_SIG_ROOT_ADVANCED ]==1UL );
...
test_deliver_from_root( fd_wksp_t * wksp ) {
  ...
  FD_TEST( ctx->metrics->replay_by_sig[ REPLAY_SIG_MISSING_FEC ]==1UL );
...
test_metrics_exported( fd_wksp_t * wksp ) {
  ...
  FD_TEST( m[ MIDX( COUNTER, ROTOR, REPLAY_ROOT_ADVANCED) ]==ctx->metrics->replay_by_sig[ REPLAY_SIG_ROOT_ADVANCED ] );
...
main( int argc, char ** argv ) {
  ...
  fd_wksp_reset( wksp, 1U );
  test_votor_block_coexists( wksp );
