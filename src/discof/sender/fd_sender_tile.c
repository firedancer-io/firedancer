/* Sender tile signs and sends transactions to the current leader. Currently
   only supports transactions which require one signature. Designed with voting
   as primary use case. */
#define _GNU_SOURCE
...
struct fd_sender_tile_ctx {
  ...
  #define IN_LINK_CNT 4UL
  fd_sender_link_in_t in_links[ IN_LINK_CNT ];

  fd_sender_link_out_t gossip_dedup_out[1];
  ...
  struct {
    ...
    /* Transaction metrics          */
    ulong txns_signed;              /* Number of transactions signed                 */
    ulong txns_sent_to_leader;      /* Successfully sent to leader                   */
    ulong txns_sprayed;             /* Successfully sent to gossip and dedup         */
    ulong txns_rcvd;                /* Number of transactions received from replay   */

    /* Stake/Contact metrics        */
    ulong contact_updates;          /* Number of contact info updates                */
    ulong contact_update_indiv_cnt; /* Number of individual contact updates finished */
    ulong stake_update_init;        /* Number of stake updates processed             */
    ulong stake_update_fini;        /* Number of stake updates finished              */
...
during_frag( fd_sender_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( in_idx>=IN_LINK_CNT ) ) {
    FD_LOG_WARNING(( "in_idx %lu >= IN_LINK_CNT %lu", in_idx, IN_LINK_CNT ));
    return;
  }
...
after_frag( fd_sender_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( kind==IN_KIND_GOSSIP ) ) {
    ...
    ctx->metrics.contact_update_indiv_cnt += ctx->new_dest_cnt;
    return;
  }

  if( FD_UNLIKELY( kind==IN_KIND_STAKE ) ) {
    fd_stake_ci_stake_msg_fini( ctx->stake_ci );
    ctx->metrics.stake_update_fini++;
    return;
  }
  ...
  if( FD_UNLIKELY( kind==IN_KIND_REPLAY ) ) {
    ...
    fd_sender_link_out_t * gossip_dedup_out = ctx->gossip_dedup_out;
    uchar * msg_to_gossip = fd_chunk_to_laddr( gossip_dedup_out->mem, gossip_dedup_out->chunk );
    fd_memcpy( msg_to_gossip, txn->payload, txn->payload_sz );
    fd_stem_publish( stem, gossip_dedup_out->idx, 1UL, gossip_dedup_out->chunk, txn->payload_sz, 0UL, 0, 0 );
    gossip_dedup_out->chunk = fd_dcache_compact_next( gossip_dedup_out->chunk, txn->payload_sz, gossip_dedup_out->chunk0,
        gossip_dedup_out->wmark );
    ctx->metrics.txns_sprayed++;
  }
}
...
unprivileged_init( fd_topo_t *      topo,
  ...
  FD_TEST( tile->in_cnt==IN_LINK_CNT );
  ...
  setup_output_link( ctx->gossip_dedup_out, topo, tile, "sender_spray" );
  setup_output_link( ctx->net_out,          topo, tile, "sender_net" );

  /* Set up keyguard(s) */

  ulong                sign_in_idx         = fd_topo_find_tile_in_link( topo, tile, "sign_send", 0 );
  fd_topo_link_t     * sign_in             = &topo->links[ tile->in_link_id[ sign_in_idx ] ];
  fd_sender_link_in_t * sign_in_desc        = &ctx->in_links[ sign_in_idx ];
  /* *** */            sign_in_desc->kind  = IN_KIND_SIGN;

  ctx->sign_out_idx = fd_topo_find_tile_out_link( topo, tile, "sender_sign", 0 );
...
metrics_write( fd_sender_tile_ctx_t * ctx ) {
  /* Transaction metrics */
  FD_MCNT_SET( SENDER, TXNS_SIGNED,           ctx->metrics.txns_signed         );
  FD_MCNT_SET( SENDER, TXNS_SENT_TO_LEADER,   ctx->metrics.txns_sent_to_leader );
  FD_MCNT_SET( SENDER, TXNS_SPRAYED,          ctx->metrics.txns_sprayed        );
  FD_MCNT_SET( SENDER, TXNS_RCVD,             ctx->metrics.txns_rcvd           );

  /* Stake/Contact metrics */
  FD_MCNT_SET( SENDER, CONTACT_UPDATES,          ctx->metrics.contact_updates );
  FD_MCNT_SET( SENDER, CONTACT_UPDATE_INDIV_CNT, ctx->metrics.contact_update_indiv_cnt );
  FD_MCNT_SET( SENDER, STAKE_UPDATE_INIT,        ctx->metrics.stake_update_init );
  FD_MCNT_SET( SENDER, STAKE_UPDATE_FINI,        ctx->metrics.stake_update_fini );
