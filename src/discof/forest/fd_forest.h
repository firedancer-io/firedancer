...
struct __attribute__((aligned(128UL))) fd_forest_ele {
  ...
  uint data_cnt;     /* number of data shreds in the FEC set */
  //uint complete_idx; /* shred_idx with SLOT_COMPLETE_FLAG ie. last shred idx in the slot */

  uchar completed;   /* 1 if we have already force completed the FEC set, or shred tile has completed the FEC set */
  fd_fec_shred_idxs_t idxs[fd_fec_shred_idxs_word_cnt]; /* data shred idxs */
};
