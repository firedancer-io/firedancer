...
/* fd_forest_evict is called when the forest has no more free elements,
  ...
  orphan process is much faster than the turbine process, so for now we
  make the choice to optimistically keep orphans, and rely on the repair
  orphan process to quickly connect ancestry, faster than the future
  slots can come and evict it.

  WHAT IF WE HAVE NO ORPHANS.

  Now if we don't have orphans, we need to evict the newest unconfirmed
  leaf. I.e. start by trimming from the tip of the tree, but ideally on
