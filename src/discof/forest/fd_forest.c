...
fd_forest_blk_t *
fd_forest_data_shred_insert( fd_forest_t * forest, ulong slot, ulong parent_slot FD_PARAM_UNUSED, uint shred_idx, uint fec_set_idx, int slot_complete, shred_src_t src ) {
  VER_INC;
  fd_forest_blk_t * ele = query( forest, slot );
# if FD_FOREST_USE_HANDHOLDING
...
# endif
  ...
  for( uint idx = fec_set_idx; idx <= last_shred_idx; idx++ ) {
    ele = fd_forest_data_shred_insert( forest, slot, parent_slot, idx, fec_set_idx, slot_complete & (idx == last_shred_idx), RECOVERED );
  }
  return ele;
}
...
ancestry_print( fd_forest_t const * forest, fd_forest_blk_t const * ele, int space, const char * prefix ) {
  fd_forest_blk_t const * pool = fd_forest_pool_const( forest );
