...
test_store_simple( fd_wksp_t * wksp ) {
  ...
  fd_hash_t mr5 = { { 5 } };
  fd_hash_t mr6 = { { 6 } };
  fd_store_init  ( store, &mr0,       0UL                );
  fd_store_insert( store, &mr1, &mr0, 1UL, 1, 0, NULL, 0 );
  fd_store_insert( store, &mr2, &mr1, 2UL, 1, 0, NULL, 0 );
  fd_store_insert( store, &mr4, &mr2, 4UL, 2, 0, NULL, 0 );
  fd_store_insert( store, &mr3, &mr1, 3UL, 2, 0, NULL, 0 );
  fd_store_insert( store, &mr5, &mr3, 5UL, 2, 0, NULL, 0 );
  fd_store_insert( store, &mr6, &mr5, 6UL, 1, 0, NULL, 0 );

  FD_TEST( fd_store_query_const( store, &mr0 )->slot == 0 );
  FD_TEST( fd_store_query_const( store, &mr1 )->slot == 1 );
  FD_TEST( fd_store_query_const( store, &mr2 )->slot == 2 );
  FD_TEST( fd_store_query_const( store, &mr3 )->slot == 3 );
  FD_TEST( fd_store_query_const( store, &mr4 )->slot == 4 );
  FD_TEST( fd_store_query_const( store, &mr5 )->slot == 5 );
  FD_TEST( fd_store_query_const( store, &mr6 )->slot == 6 );
