...
struct fd_store_tile_ctx {
  ...
  ulong            archive_publish;
  fd_frag_meta_t * archive_out_mcache;
  ulong *          archive_out_sync;
  ulong            archive_out_depth;
  ulong            archive_out_seq;
...
during_frag( fd_store_tile_ctx_t * ctx,
  ...
  ctx->archive_publish=0;
  if( FD_UNLIKELY( ctx->archive_out_mem ) ) {
    uchar * dst = fd_chunk_to_laddr( ctx->archive_out_mem, ctx->archive_out_chunk );
    if( in_idx==REPAIR_IN_IDX ) {
      ctx->archive_publish=1;
      fd_memcpy( dst, fd_chunk_to_laddr_const( ctx->repair_in_mem, chunk ), sz );
    } else if( in_idx>=NON_SHRED_LINKS ) {
      ctx->archive_publish=1;
      fd_store_in_ctx_t * shred_in = &ctx->shred_in[ in_idx-NON_SHRED_LINKS ];
      fd_memcpy( dst, fd_chunk_to_laddr_const( shred_in->mem, chunk ), sz );
    }
  }
...
after_frag( fd_store_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( ctx->archive_publish ) ) {
    ulong new_sig = ( in_idx==REPAIR_IN_IDX )? FD_ARCHIVER_SIG_MARK_REPAIR(sig) :
                                               FD_ARCHIVER_SIG_MARK_SHRED(sig);
    fd_mcache_publish( ctx->archive_out_mcache, ctx->archive_out_depth, ctx->archive_out_seq, new_sig, ctx->archive_out_chunk,
                       sz, 0UL, tsorig, tspub );
    ctx->archive_out_seq   = fd_seq_inc( ctx->archive_out_seq, 1UL );
...
unprivileged_init( fd_topo_t *      topo,
  ...
  if( FD_UNLIKELY( tile->out_cnt==4 ) ) {
    FD_LOG_NOTICE(( "storei tile has archiver output link" ));
    fd_topo_link_t * archive_out = &topo->links[ tile->out_link_id[ ARCHIVE_OUT_IDX ] ];
    ctx->archive_out_mcache = archive_out->mcache;
    ctx->archive_out_sync   = fd_mcache_seq_laddr( ctx->archive_out_mcache );
    ctx->archive_out_depth  = fd_mcache_depth( ctx->archive_out_mcache );
    ctx->archive_out_seq    = fd_mcache_seq_query( ctx->archive_out_sync );
