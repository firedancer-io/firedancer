    ...
    FD_SCRATCH_SCOPE_BEGIN {
      ...
      for( uint idx = 0; idx <= buffered_idx; idx++ ) {
        if( FD_UNLIKELY( fd_block_set_test( data_complete_idxs, idx ) ) ) {
          ...
          replay_sig = fd_disco_repair_replay_sig( slot, idx, complete_idx == idx, (ushort)( slot - parent_slot ));
          fd_stem_publish( stem, 0UL, replay_sig, ctx->replay_out_chunk, 0, 0UL, tsorig, tspub );
