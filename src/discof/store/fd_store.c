...
# if FD_STORE_USE_HANDHOLDING
  ...
  if( FD_UNLIKELY( fd_store_root( store ) && slot < fd_store_root( store )->slot ) ) {
...
# if FD_STORE_USE_HANDHOLDING
  ...
  if( FD_UNLIKELY( newr->slot <= oldr->slot ) ) { FD_LOG_WARNING(( "slot %lu <= root %lu.",    newr->slot, oldr->slot                 )); return NULL; }
# endif
  ...
  fd_store_fec_t * tail = head; /* tail of prune queue */
  ...
  while( FD_LIKELY( head ) ) {
    ...
    while( FD_LIKELY( child ) ) {                                             /* iterate over children */
      if( FD_LIKELY( child != newr ) ) {                                      /* stop at new root */
        ...
        tail       = fd_store_pool_ele( pool, tail->next );                   /* push onto prune queue (so descendants can be pruned) */
        tail->next = null;                                                    /* clear map next */
      }
      child = fd_store_pool_ele( pool, child->sibling ); /* right-sibling */
    }
    fd_store_fec_t * next = fd_store_pool_ele( pool, head->next ); /* pop prune queue head */
    fd_store_pool_ele_release( pool, head );                       /* free prune queue head */
