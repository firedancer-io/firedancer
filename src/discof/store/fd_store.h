...
/* fd_store_init initializes a store.  Assumes store is a valid local
   join and no one else is joined.  root is the initial root store will
   ...
   store's memory, ie. the caller of fd_store_new. */
...
/* fd_store_insert inserts a new FEC set keyed by merkle.  Returns the
   ...
   Assumes the store has been initialized and has space for another
   element, and that the parent element (keyed by chained_merkle_root)
   has already been inserted.  Does additional checks when handholding
   is enabled and fails insertion (returning NULL) if checks fail.

   Assumes caller has already acquired a read lock via fd_rwlock_write.
