...
/* fd_store is a high-performance in-memory storage engine for shreds as
   ...
   where the merkle root uniquely identifies a given FEC set.
   ...
   For example, there is a block for slot 100 and another block for slot
   100'.  The actual conflict occurs at the shred level (eg. a leader
   produces two conflicting shreds, for example two conflicting payloads
   for slot 100 idx 1), but the result will be two different FEC sets
   (recall every shred in a FEC set has the same signature that would
   change if any of the shreds' payloads change).
   ...
   It is possible for writers (Shred tile(s)) to write and readers
   (Replay tile) to read from Store without the lock.  This can enable
   parallel writes and reduce lock contention between readers and
   writers.
   ...
   same Shred tile processing both keys.  Then, two writers will never
   attempt to write to the same fd_map_chain slot at the same time.

   For reducing lock contention, the application should structure Store
   reads to always occur after append-only writes.  In the architecture,
   Replay tile (reader) can read without a lock and writer (Shred tile)
   write without a lock because 1. Replay will only read from the store
   after Repair has notified Replay it is time to read, and 2. the Shred
   tile is append-only, so it will never modify or remove what it has
   written.  Store is backed by a fd_map_chain, which is not thread-safe
   generally, but in the case of a slot collision where Replay tile is
   reading an element and Shred tile writes a new element to the same
   slot, that new element is always guaranteed to be appended to the end
   of the hash chain within that slot (which modifies the second-to-last
   element in the hash chain's next field but does not touch application
   data).  So this can enable lock-free concurrent reads and writes.
   Note Replay tile (caller) should always use fd_store_query_const to
   ensure the underlying fd_map_chain is not modified during query.
   ...
   per slot, so it is a relatively infrequent compared to Store queries
   and inserts. */
...
struct __attribute__((aligned(128UL))) fd_store_fec {
  ...
  fd_hash_t cmr; /* chained merkle root (merkle root of the parent FEC set ie. parent key) */
  ...
  ulong parent_off;  /* offset to parent slot, [0,USHORT_MAX-1] */
  ulong fec_set_idx; /* fec_set_idx of the FEC set, [0,32768-1] */
