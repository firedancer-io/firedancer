...
file_test_sign( void *        keypair,
  ...
  fd_ed25519_sign( sig, m, 32UL, key+32UL, key, sha );
}
...
test_tower_file_load( void ) {
  ...
  FD_TEST( !memcmp( &out.bank_hash, &hash, sizeof(hash) ) );
...
test_fixture_replay( fd_wksp_t * wksp,
  ...
  for( ulong slot = start_slot; slot < start_slot + num_slots; slot++ ) {
    ...
    if( recover ) {
      ...
      if( slot<start_slot+3UL ) {
        FD_TEST( ctx->recovery_pending && fd_tower_vote_empty( ctx->tower->votes ) );
      } else {
        FD_TEST( !ctx->recovery_pending && fd_tower_vote_cnt( ctx->tower->votes )==3UL );
        for( ulong i=0UL; i<3UL; i++ )
          FD_TEST( !memcmp( fd_tower_vote_peek_index_const( ctx->tower->votes, i ), &ctx->recovery.saved.votes[ i ], sizeof(fd_tower_vote_t) ) );
      }
    }
  }
