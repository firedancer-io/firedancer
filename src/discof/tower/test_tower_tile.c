...
mock_topo_with_accdb( fd_wksp_t *      wksp,
  ...
  ulong shmem_fp = fd_accdb_shmem_footprint( max_accounts, 0UL, tile->tower.max_live_slots, max_writes_per_slot, partition_cnt, cache_fp, cache_min_reserved, joiner_cnt, 0UL );
  void * shmem_mem = fd_wksp_alloc_laddr( wksp, fd_accdb_shmem_align(), shmem_fp, 1UL );
  FD_TEST( shmem_mem );
  FD_TEST( fd_accdb_shmem_new( shmem_mem, max_accounts, 0UL, tile->tower.max_live_slots, max_writes_per_slot, partition_cnt, partition_sz, cache_fp, cache_min_reserved, 0, 42UL, joiner_cnt, 0UL ) );
