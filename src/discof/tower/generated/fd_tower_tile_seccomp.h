...
#endif
static const unsigned int sock_filter_policy_fd_tower_tile_instr_cnt = 26;

static void populate_sock_filter_policy_fd_tower_tile( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int checkpt_fd, unsigned int restore_fd, unsigned int debug_fd ) {
  FD_TEST( out_cnt >= 26 );
  struct sock_filter filter[26] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_fstat, /* check_fstat */ 12, 0 ),
...
//  lbl_2:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, checkpt_fd, /* RET_ALLOW */ 11, /* lbl_3 */ 0 ),
//  lbl_3:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, debug_fd, /* RET_ALLOW */ 9, /* RET_KILL_PROCESS */ 8 ),
//  check_read:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
