...
#endif
static const unsigned int sock_filter_policy_fd_tower_tile_instr_cnt = 19;

static void populate_sock_filter_policy_fd_tower_tile( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int checkpt_fd ) {
  FD_TEST( out_cnt >= 19 );
  struct sock_filter filter[19] = {
    ...
    /* simply allow rename */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_rename, /* RET_ALLOW */ 12, 0 ),
...
//  lbl_2:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, checkpt_fd, /* RET_ALLOW */ 5, /* RET_KILL_PROCESS */ 4 ),
//  check_fsync:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
