...
typedef struct {
  ...
  uchar vote_txn_buf[ 4096 ]; /* FIXME maximum footprint of deser vote instruction */
  ...
  fd_replay_tower_t          replay_towers[ FD_VOTER_MAX ];
...
replay_slot_completed( ctx_t *                      ctx,
  ...
  for( ulong i=0UL; i<ctx->replay_towers_cnt; i++ ) {
    ...
    ctx->vote_keys[ i ] = ctx->replay_towers[ i ].pubkey;
    ...
    if( FD_UNLIKELY( 0==memcmp( &ctx->vote_keys[ i ], ctx->vote_account, sizeof(fd_pubkey_t) ) ) ) {
      ...
      if( FD_UNLIKELY( fd_tower_votes_empty( ctx->tower ) ) ) { /* FIXME this is wrong and can only be done on "caught up" */
        ...
        ctx->tower_root = fd_ulong_load_8_fast( fd_voter_root_laddr( (fd_voter_state_t const *)fd_type_pun_const( acc ) ) );
      }
    }
  }
...
returnable_frag( ctx_t *   ctx,
  ...
  case IN_KIND_GOSSIP: {
    ...
    if( FD_UNLIKELY( sig==FD_GOSSIP_UPDATE_TAG_VOTE ) ) {
      ...
      if( FD_UNLIKELY( sz==0 ) ) { FD_LOG_WARNING(( "gossip vote txn parse failed" )); return 0; }
      fd_txn_t * txn = (fd_txn_t *)ctx->vote_txn_buf;
      if( FD_UNLIKELY( !fd_txn_is_simple_vote_transaction( txn, vote->txn ) ) ) { FD_LOG_WARNING(( "gossip vote is not vote txn" )); return 0; }  /* TODO Agave doesn't have the same validation of <=2 signatures */
      ...
      if( FD_UNLIKELY( err != FD_ED25519_SUCCESS ) ) { FD_LOG_WARNING(( "gossip vote failed verify" )); return 0; }
      ...
      if( FD_UNLIKELY( err==-1 ) ) { FD_LOG_WARNING(( "gossip vote txn failed bincode deser" )); return 0; }
      ...
      // fd_pubkey_t const * vote_acc = &pubkeys[0]; /* the first pubkey */
      // fd_notar_vote( ctx->notar,  )
    }
    return 0;
  }
  case IN_KIND_REPLAY: {
    ...
    if( FD_LIKELY( sig==REPLAY_SIG_SLOT_COMPLETED ) ) {
      ...
      if( FD_UNLIKELY( ctx->replay_towers_cnt>=FD_VOTER_MAX ) ) FD_LOG_ERR(( "tower received more vote states than expected" ));
