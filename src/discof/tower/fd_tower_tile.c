...
#define FILE_MAX (8192UL) /* the maximum size of a bincode serialized tower. FIXME tighter bound */
...
static char *
filepath( char const * ledger_path,
...
privileged_init( fd_topo_t *      topo,
  ...
  *ctx->vote_acc     = *(fd_pubkey_t const *)fd_type_pun_const( vote_key );
  ...
  filepath( tile->tower.ledger_path, strlen( tile->tower.ledger_path ), base58_pubkey, base58_pubkey_len, "bin.new", sizeof("bin.new") - 1, ctx->checkpt_path );
  filepath( tile->tower.ledger_path, strlen( tile->tower.ledger_path ), base58_pubkey, base58_pubkey_len, "bin",     sizeof("bin") - 1,     ctx->restore_path );

  ctx->checkpt_fd = open( ctx->checkpt_path, O_WRONLY | O_CREAT | O_TRUNC, 0600 );
  if( FD_UNLIKELY( ctx->checkpt_fd<0 ) ) FD_LOG_ERR(( "failed to open %s: %s", ctx->checkpt_path, strerror( errno ) ));

  ctx->restore_fd = open( ctx->restore_path, O_RDONLY );
  if( FD_UNLIKELY( ctx->restore_fd<0 ) ) FD_LOG_ERR(( "failed to open %s: %s", ctx->restore_path, strerror( errno ) ));

  int err = fd_io_sz( ctx->restore_fd, &ctx->file_sz );
  if( FD_UNLIKELY( err ) ) FD_LOG_ERR(( "%s", fd_io_strerror( err ) ));

  ulong sz; err = fd_io_read( ctx->restore_fd, ctx->ser, ctx->file_sz, sizeof(ctx->ser), &sz );
  FD_TEST( ctx->file_sz==sz ); /* sanity check */
  if( FD_UNLIKELY( err<0 ) ) FD_LOG_ERR(( "unexpected EOF"            ));
  if( FD_UNLIKELY( err>0 ) ) FD_LOG_ERR(( "%s", fd_io_strerror( err ) ));

  uchar arr[2319];
  memcpy( arr, ctx->ser, sizeof(arr) );
  FD_LOG_HEXDUMP_NOTICE(( "arr", arr, ctx->file_sz ));
...
unprivileged_init( fd_topo_t *      topo,
  ...
  fd_tower_restore( ctx->tower, &ctx->root, ctx->identity_key->uc, ctx->ser, ctx->file_sz, ctx->de, sizeof(ctx->de) );
  FD_LOG_ERR(( "done" ));
...
populate_allowed_seccomp( fd_topo_t const *      topo,
  ...
  populate_sock_filter_policy_fd_tower_tile( out_cnt, out, (uint)fd_log_private_logfile_fd(), (uint)ctx->checkpt_fd );
