...
after_frag( ctx_t *             ctx,
  ...
  fd_txn_p_t * vote_txn = (fd_txn_p_t *)fd_chunk_to_laddr( ctx->sender_out_mem, ctx->sender_out_chunk );
  ...
  fd_stem_publish( stem, ctx->sender_out_idx, 0UL /* unused sig */, ctx->sender_out_chunk, sizeof(fd_txn_p_t), 0UL, tsorig, fd_frag_meta_ts_comp( fd_tickcount() ) );
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->sender_out_idx = fd_topo_find_tile_out_link( topo, tile, "tower_sender", 0 );
  FD_TEST( ctx->sender_out_idx!=ULONG_MAX );
  fd_topo_link_t * sender_out = &topo->links[ tile->out_link_id[ ctx->sender_out_idx ] ];
  ...
  FD_TEST( fd_dcache_compact_is_safe( ctx->sender_out_mem, sender_out->dcache, sender_out->mtu, sender_out->depth ) );
}
