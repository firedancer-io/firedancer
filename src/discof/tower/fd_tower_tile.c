...
ulong QUERY_VOTERS( fd_tower_tile_t *, ulong, fd_funk_txn_xid_t, fd_top_votes_t const *, epoch_vtr_t *, epoch_vtr_map_t * );
...
deser_auth_vtr( fd_tower_tile_t * ctx,
  ...
  FD_CRIT( fd_vote_state_versioned_deserialize( vsv, data, data_sz ), "unable to decode vote state versioned" );
...
update_voters( fd_tower_tile_t *            ctx,
  ...
  if( FD_LIKELY( ctx->banks ) ) {
    ...
    ctx->vtr_cnt = 0;
    ctx->root_epoch_total_stake = QUERY_VOTERS( ctx, epoch,     bank_xid, fd_bank_top_votes_t_2_query( bank ), ctx->root_epoch_vtr_pool, ctx->root_epoch_vtr_map );
    ctx->next_epoch_total_stake = QUERY_VOTERS( ctx, epoch+1UL, bank_xid, fd_bank_top_votes_t_1_query( bank ), ctx->next_epoch_vtr_pool, ctx->next_epoch_vtr_map );
  }
