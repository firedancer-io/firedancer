...
tower_sign_cb( void *        _ctx,
  ...
  /* The sign tile hashes again, so the signed message is sha256(sha256(msg)). */
  uchar digest[ 32 ];
  fd_sha256_hash( msg, msg_sz, digest );
  fd_keyguard_client_sign( ctx->keyguard_client, sig, digest, 32UL, FD_KEYGUARD_SIGN_TYPE_SHA256_ED25519 );
}
...
tower_recovery_init( fd_tower_tile_t *            ctx,
  ...
  fd_acc_t hashes_acc  = fd_accdb_read_one( ctx->accdb, bank->accdb_fork_id, fd_sysvar_slot_hashes_id.uc );
  fd_slot_history_view_t history;
  fd_slot_hashes_t hashes;
  fd_slot_history_view_t * h = NULL;
  fd_slot_hashes_t * sh = NULL;
  if( history_acc.lamports && !memcmp( history_acc.owner, fd_sysvar_owner_id.uc, 32UL ) )
    h = fd_sysvar_slot_history_view( &history, history_acc.data, history_acc.data_len );
  if( hashes_acc.lamports && !memcmp( hashes_acc.owner, fd_sysvar_owner_id.uc, 32UL ) )
    sh = fd_sysvar_slot_hashes_view( &hashes, hashes_acc.data, hashes_acc.data_len );
  int result = fd_tower_recover_init( &ctx->recovery, &ctx->recovery.saved, slot_completed->slot,
                                      &slot_completed->bank_hash, &slot_completed->block_id, h, sh );
  fd_accdb_unread_one( ctx->accdb, &hashes_acc );
  fd_accdb_unread_one( ctx->accdb, &history_acc );
  if( FD_UNLIKELY( result<0 ) )
    FD_LOG_ERR(( "tower recovery cannot anchor the saved checkpoint to snapshot slot %lu (error %d).", slot_completed->slot, result ));
  ...
  *found_our_vote_acct = 0;
  ...
  if( FD_LIKELY( reconcile_ro.lamports ) ) {
    ...
    *our_vote_acct_bal = reconcile_ro.lamports;
    ...
    int skip_reconcile = ( !ctx->init && ctx->wfs ) || ctx->recovery_pending || ctx->recovery_initialized;
    if( FD_UNLIKELY( ctx->recovery_pending || ctx->recovery_initialized || ctx->first_use_pending ) ) {
      ...
      if( FD_UNLIKELY( ctx->recovery_initialized && fd_tower_recover_check_onchain( &ctx->recovery, ctx->scratch_tower, root ) ) )
        FD_LOG_ERR(( "tower file is stale, the vote account has a later vote or root." ));
    }
...
replay_slot_completed( fd_tower_tile_t *            ctx,
  ...
  if( FD_UNLIKELY( ctx->recovery_pending && ++ctx->recovery_replay_cnt>=ctx->tower->blk_max ) )
    FD_LOG_ERR(( "tower recovery exhausted the replay window before resolving the saved tip. Preserve the tower and restart with a compatible snapshot or the missing fork data" ));
  ...
  if( FD_UNLIKELY( ctx->recovery_pending ) ) {
    FD_TEST( ctx->recovery_initialized );
    ulong recovered_root;
    fd_hash_t recovered_root_id;
    int result = fd_tower_recover_try( &ctx->recovery, ctx->tower, ctx->ghost, &recovered_root, &recovered_root_id );
    if( FD_UNLIKELY( result<0 ) )
      FD_LOG_ERR(( "tower recovery failed to reconcile saved votes with replay (error %d).", result ));
    if( result==FD_TOWER_RECOVER_READY ) {
      ctx->recovery_pending = 0;
      ctx->metrics.last_vote_slot = ctx->recovery.saved.votes[ ctx->recovery.saved.votes_cnt-1UL ].slot;
      if( recovered_root>ctx->tower->root ) {
        out.root_slot     = recovered_root;
        out.root_block_id = recovered_root_id;
      }
      FD_LOG_NOTICE(( "restored signed tower through slot %lu, retained %lu unrooted votes", ctx->metrics.last_vote_slot, fd_tower_vote_cnt( ctx->tower->votes ) ));
    }
    fd_ghost_blk_t * reset_root = out.root_slot==ULONG_MAX ? fd_ghost_root( ctx->ghost ) : fd_ghost_query( ctx->ghost, &out.root_block_id );
    FD_TEST( reset_root );
    fd_ghost_blk_t const * best = fd_ghost_best( ctx->ghost, reset_root );
  ...
  if( FD_UNLIKELY( out.root_slot!=ULONG_MAX ) ) {
    ...
    if( FD_UNLIKELY( oldr_tower_blk->epoch!=newr_tower_blk->epoch && oldr_tower_blk->epoch+1!=newr_tower_blk->epoch ) )
...
privileged_init( fd_topo_t const *      topo,
  ...
  if( tile->tower.failover_enabled ) {
    ...
    memcpy( &checkpoint_identity, staked, 32UL );
  ...
  if( FD_UNLIKELY( ctx->tower_file_enabled ) ) {
    ...
    if( ctx->recovery_pending ) FD_LOG_NOTICE(( "loaded signed tower through slot %lu, voting will wait for replay reconciliation", ctx->recovery.saved.votes[ ctx->recovery.saved.votes_cnt-1UL ].slot ));
