...
metrics_write( ctx_t * ctx ) {
  ...
  FD_MCNT_SET( TOWER, PROOF_ERR_SHRED_SZ,        ctx->metrics.proof_err_shred_sz        );
...
record_eqvoc_metric( ctx_t * ctx,
  ...
  case FD_EQVOC_PROOF_MERKLE:  ctx->metrics.proof_verified_merkle++; break;
  case FD_EQVOC_PROOF_META:    ctx->metrics.proof_verified_merkle++; break;
  case FD_EQVOC_PROOF_LAST:    ctx->metrics.proof_verified_merkle++; break;
  case FD_EQVOC_PROOF_OVERLAP: ctx->metrics.proof_verified_merkle++; break;
  case FD_EQVOC_PROOF_CHAINED: ctx->metrics.proof_verified_merkle++; break;

  case FD_EQVOC_ERR_SZ:        ctx->metrics.proof_err_shred_sz++;        break;
  ...
  case FD_EQVOC_ERR_SIGNATURE: ctx->metrics.proof_err_shred_signature++; break;
  ...
  case IN_KIND_GOSSIP: {
    if( FD_LIKELY( sig==FD_GOSSIP_UPDATE_TAG_DUPLICATE_SHRED ) ) {
      ...
      if     ( FD_UNLIKELY( duplicate_shred->num_chunks  != FD_EQVOC_CHUNK_CNT ) ) ctx->metrics.proof_err_chunk_cnt++;
        ...
        fd_tower_slot_duplicate_t * msg = fd_chunk_to_laddr( ctx->out_mem, ctx->out_chunk );
        int err = fd_eqvoc_chunk_insert( ctx->eqvoc, from, duplicate_shred, msg->chunks );
        if( err > FD_EQVOC_SUCCESS )
          fd_stem_publish( stem, 0UL, FD_TOWER_SIG_SLOT_DUPLICATE, chunk, sz, 0UL, tsorig, fd_frag_meta_ts_comp( fd_tickcount() ) );
      }
    }
    return 0;
  }
