...
update_ghost( ctx_t * ctx, fd_funk_txn_t * txn ) {
  ...
  for( ulong i = 0; i < fd_epoch_voters_slot_cnt( epoch_voters ); i++ ) {
    ...
    if( FD_LIKELY( vote != FD_SLOT_NULL && vote >= fd_ghost_root( ghost )->slot ) ) {
      ...
      fd_ghost_ele_t const * ele = fd_ghost_query_const( ghost, fd_ghost_hash_id( ghost, vote ) );
    ...
    if( FD_LIKELY( root != FD_SLOT_NULL && root >= fd_ghost_root( ghost )->slot ) ) {
      fd_ghost_ele_t const * ele = fd_ghost_query( ghost, fd_ghost_hash_id( ghost, root ) );
...
after_frag( ctx_t *             ctx,
  ...
  fd_ghost_ele_t const * ghost_ele  = fd_ghost_insert( ctx->ghost, &ctx->parent_hash, slot, &ctx->slot_hash );
  FD_TEST( ghost_ele );
  ...
  if( FD_LIKELY( root != FD_SLOT_NULL ) ) {
    fd_hash_t const * root_bid = fd_ghost_hash_id( ctx->ghost, root );
