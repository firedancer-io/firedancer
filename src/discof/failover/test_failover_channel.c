...
hello( uchar const * key, uchar role ) {
  ...
  fd_memset( h.staked_pubkey, 0xAA, 32UL );
...
main( int argc, char ** argv ) {
  ...
  /* The wrong peer identity cannot change the listener or peer state. */
  ...
  now += 2000000001L;
  poll_channel( a );
  FD_LOG_NOTICE(( "pass: wrong peer key" ));
  ...
  /* An established connection is not displaced, even by the expected peer. */
  ulong starts = a->metrics.connection_attempt_cnt;
  ...
  /* Eight addresses hold every handshake slot.  The configured peer is
     admitted anyway, the oldest holder is closed for it, and the pair
     completes while the rest hold on. */
  ...
  ulong evicted = a->metrics.evicted_cnt;
  pump( a, b );
  FD_TEST( paired( a ) && paired( b ) && !fd_failover_channel_pending( a ) );
