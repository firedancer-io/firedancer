...
#define FD_FAILOVER_STATUS_BUSY    (16U) /* transitional role state, Phase 1 sets it */
...
/* fd_failover_session_step returns the next session state upon event
   for a listener or dialer endpoint.  The channel drives every state
   change through it.  A listener stays LISTENING while candidates
   handshake, pairs on HELLO_OK and returns to LISTENING on any loss.
   A dialer leaves BACKOFF on RETRY, reaches HELLO once TCP connects,
   pairs on HELLO_OK and falls back to BACKOFF on any loss.  An event
   that does not apply to the state, or an out of range input, leaves
   the state unchanged. */
ulong
