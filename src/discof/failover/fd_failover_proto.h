...
FD_STATIC_ASSERT( sizeof(fd_failover_hello_t)==143UL, wire_layout );
...
/* fd_failover_session_step returns the next session state upon event
   ...
   the state leaves the state unchanged. Rejection ends the connection,
   but not the listener. Out of range inputs leave the state unchanged. */
ulong
