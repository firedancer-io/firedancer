...
test_bounds( void ) {
  ...
  fd_memcpy( big, frame, sz );
  fd_failover_frame_hdr_t hdr;
  fd_memcpy( &hdr, big, FD_FAILOVER_FRAME_HDR_SZ );
  hdr.len = (uint)( FD_FAILOVER_FRAME_MAX+1UL );
  fd_memcpy( big, &hdr, FD_FAILOVER_FRAME_HDR_SZ );
...
test_unknown_type( void ) {
  ...
  fd_failover_frame_hdr_t hdr;
  fd_memcpy( &hdr, frame, sizeof(hdr) );
  hdr.type = (ushort)FD_FAILOVER_MSG_RESERVED;
  fd_memcpy( frame, &hdr, sizeof(hdr) );
