...
void
fd_failover_channel_expect_peer( fd_failover_channel_t * ch, uint address ) {
...
fd_failover_channel_set_identity( fd_failover_channel_t * ch, uchar const * keypair,
  ...
  if( !fd_memeq( keypair+32, hello->junk_pubkey, 32UL ) ||
      fd_memeq( hello->junk_pubkey, hello->staked_pubkey, 32UL ) ||
      fd_memeq( peer_pubkey, hello->staked_pubkey, 32UL ) ) return -1;
  reset( ch );
  ch->state = fd_failover_session_init( ch->dial_peer );
...
static void drop( fd_failover_channel_t * ch, ulong idx, long now, int event );
...
admit( fd_failover_channel_t * ch, uint address, long now ) {
  int peer = ch->peer_addr && address==ch->peer_addr;
...
/* A full table does not keep the configured peer out.  The oldest
   candidate from another address makes room, so holding every slot from
   other addresses only ever costs the holder a slot. */
static ulong
evict_for_peer( fd_failover_channel_t * ch, uint address, long now ) {
  if( !ch->peer_addr || address!=ch->peer_addr ) return FD_FAILOVER_CHANNEL_CANDIDATE_MAX;
  ulong victim = FD_FAILOVER_CHANNEL_CANDIDATE_MAX;
  for( ulong i=0; i<FD_FAILOVER_CHANNEL_CANDIDATE_MAX; i++ ) {
    struct candidate * c = &ch->candidates[i];
    if( c->fd==-1 || (int)i==ch->active || c->address==ch->peer_addr ) continue;
