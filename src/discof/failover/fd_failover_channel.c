...
fd_failover_channel_init_listener( fd_failover_channel_t * ch, uint address, ushort port ) {
  reset( ch );
  ch->dial_peer = 0;
  ch->state = FD_FAILOVER_SESSION_LISTENING;
...
fd_failover_channel_init_dialer( fd_failover_channel_t * ch, uint address, ushort port ) {
  ...
  ch->listen_fd = -1;
  ch->dial_peer = 1;
  ch->state = FD_FAILOVER_SESSION_BACKOFF;
...
fd_failover_channel_set_identity( fd_failover_channel_t * ch, uchar const * keypair,
  ...
  reset( ch );
  ch->state = ch->dial_peer ? FD_FAILOVER_SESSION_BACKOFF : FD_FAILOVER_SESSION_LISTENING;
...
static void
drop( fd_failover_channel_t * ch, ulong idx, long now ) {
  ...
  if( ch->dial_peer ) {
    ch->retry_at = fd_long_sat_add( now, paired ? 0L : ch->backoff );
    ch->backoff = paired ? ch->backoff_min : fd_long_min( fd_long_sat_add( ch->backoff, ch->backoff ), ch->backoff_max );
    ch->state = FD_FAILOVER_SESSION_BACKOFF;
  } else if( paired ) ch->state = FD_FAILOVER_SESSION_LISTENING;
}
...
accept_candidates( fd_failover_channel_t * ch, long now, int * busy ) {
  for( ulong n=0; n<8UL; n++ ) {
    ...
    *busy = 1;
    ...
    start( ch, idx, fd, addr.sin_addr.s_addr, now, PHASE_TLS );
  }
}
...
flush( fd_failover_channel_t * ch, ulong idx, long now, int * busy ) {
  ...
  if( n<0L ) { drop( ch, idx, now ); return -1; }
  if( !n ) return 1;
  *busy = 1;
  c->tx_sent += (ulong)n;
...
decode( fd_failover_channel_t * ch, ulong idx, long now, int * busy,
  ...
  if( err ) { ch->metrics.wire_fatal_cnt++; drop( ch, idx, now ); return -1; }
...
read_frame( fd_failover_channel_t * ch, ulong idx, long now, int * busy,
  ...
  if( c->rx_used>=max ) { ch->metrics.wire_fatal_cnt++; drop( ch, idx, now ); return -1; }
...
service_candidate( fd_failover_channel_t * ch, ulong idx, long now, int * busy, uchar * payload ) {
  ...
  if( c->phase==PHASE_CONNECT ) {
    ...
    if( connect( c->fd, fd_type_pun( &addr ), sizeof(addr) ) && errno!=EISCONN ) {
      ...
      drop( ch, idx, now ); return;
    }
    c->phase = PHASE_TLS;
    ch->state = FD_FAILOVER_SESSION_HELLO;
  }
  if( c->phase==PHASE_TLS ) {
    int rc = fd_failover_tls_handshake( &c->tls );
    if( rc<0 ) { ch->metrics.tls_fail_cnt++; drop( ch, idx, now ); return; }
    if( !rc ) return;
    *busy = 1;
    c->phase = PHASE_HELLO;
  ...
  if( c->phase==PHASE_HELLO ) {
    ...
    if( type!=FD_FAILOVER_MSG_HELLO || sz!=sizeof(c->hello) ) {
      ch->metrics.wire_fatal_cnt++; drop( ch, idx, now ); return;
    }
    ...
    if( memcmp( c->hello.junk_pubkey, ch->tls_ctx.peer_pubkey, 32UL ) ||
      ...
      ch->metrics.hello_reject_cnt++; drop( ch, idx, now ); return;
    }
    c->phase = PHASE_READY;
  }
  if( c->phase==PHASE_READY && !c->tx_used ) {
    ...
    ch->state = FD_FAILOVER_SESSION_PAIRED;
...
fd_failover_channel_poll( fd_failover_channel_t * ch, long now, int * busy,
  ...
  if( ch->active>=0 ) {
    ...
    if( !flushed ) {
      ...
      if( rc>0 ) {
        if( *type==FD_FAILOVER_MSG_HELLO ) { ch->metrics.wire_fatal_cnt++; drop( ch, idx, now ); }
        else { ch->last_rx = now; delivered = 1; }
      } else if( !rc && now>=ch->last_rx && now-ch->last_rx>ch->silence_timeout ) drop( ch, idx, now );
    }
    if( flushed>0 && now>=ch->last_rx && now-ch->last_rx>ch->silence_timeout ) drop( ch, idx, now );
  }
  ...
  for( ulong i=0; i<FD_FAILOVER_CHANNEL_CANDIDATE_MAX; i++ ) {
    ...
    if( c->fd!=-1 && (int)i!=ch->active && now>=c->deadline ) {
      ch->metrics.handshake_timeout_cnt++; drop( ch, i, now ); *busy = 1;
    }
  }
  if( !ch->dial_peer ) accept_candidates( ch, now, busy );
    ...
    *busy = 1;
    if( fd==-1 ) { drop( ch, 0UL, now ); return delivered; }
    ch->state = FD_FAILOVER_SESSION_DIALING;
...
fd_failover_channel_send( fd_failover_channel_t * ch, long now, ushort type, uchar const * payload, ulong sz ) {
  ...
  if( !c->tx_used ) { drop( ch, idx, now ); return -1; }
  int busy = 0;
