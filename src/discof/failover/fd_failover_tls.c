...
fd_failover_tls_ctx_init( fd_failover_tls_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( !fd_memeq( public_key, keypair+32UL, 32UL ) || fd_memeq( public_key, peer_pubkey, 32UL ) ) ) return -1;

  fd_memcpy( ctx->private_key, keypair,      32UL );
  fd_memcpy( ctx->public_key,  keypair+32UL, 32UL );
  fd_memcpy( ctx->peer_pubkey, peer_pubkey,  32UL );
  ...
  /* The template holds no key share, every connection draws its own in
     fd_failover_tls_new, so nothing that outlives a connection can
     rebuild its key. */
  fd_memcpy( tls->cert_public_key, ctx->public_key, 32UL );
...
fd_failover_tls_new( fd_failover_tls_t *     tls,
  ...
  /* Fresh X25519 key share per connection, straight from the kernel.  It
     ...
     RNG only feeds the public handshake randoms and is reseeded here so
     it keeps nothing from the previous connection either. */
  uchar seed[ 32 ];
  if( FD_UNLIKELY( !fd_rng_secure( seed, 32UL ) ) ) return -1;
  fd_chacha_rng_init( ctx->rng, seed, FD_CHACHA_RNG_ALGO_CHACHA20 );
  fd_memzero_explicit( seed, sizeof(seed) );
  if( FD_UNLIKELY( !fd_rng_secure( tls->conn.tls.kex_private_key, 32UL ) ) ) return -1;
  ...
  if( FD_LIKELY( dial_peer ) ) {
    ...
    fd_memcpy( tls->conn.hs.cli.server_pubkey, ctx->peer_pubkey, 32UL );
  }
...
fd_failover_tls_handshake( fd_failover_tls_t * tls ) {
  ...
  /* Handshake done, the peer signed with some key and agreed on our ALPN.
     That key has to be the peer's junk key. */
  ...
  if( tls->dial_peer ) {
    ...
    if( FD_UNLIKELY( !cli->server_pubkey_pin || cli->server_pubkey_len!=32UL || !cli->alpn_negotiated ||
                     !fd_memeq( cli->server_pubkey, ctx->peer_pubkey, 32UL ) ) ) return -1;
  } else {
    /* fd_tls already verified CertificateVerify against client_pubkey
       (zero if the client sent no cert), so only the key check remains. */
    fd_tls_estate_srv_t const * srv = &tls->conn.hs.srv;
    if( FD_UNLIKELY( !fd_memeq( srv->client_pubkey, ctx->peer_pubkey, 32UL ) ) ) return -1;
  }
  tls->verified = 1;
  return 1;
}
