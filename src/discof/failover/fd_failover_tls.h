...
struct fd_failover_tls {
  ...
  int   fd;
  int   dial_peer;
  int   verified;     /* handshake done, peer key matches the pin */
...
/* fd_failover_tls_ctx_init installs a 64 byte Ed25519 junk keypair
   (seed then pubkey) and the peer's pinned junk pubkey.  Call before
   sandboxing: it allocates the protected key page and reads entropy
   for the RNG and the X25519 key.  Returns 0 on success, -1 if the
   keypair is inconsistent, matches the peer pin, or entropy fails. */
...
/* fd_failover_tls_handshake returns 1 once the handshake is done and
   the peer key matches the pin, 0 while pending, -1 on failure.
