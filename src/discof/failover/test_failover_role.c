...
sample( void ) {
  ...
  role.term    = 42UL;
  role.role    = (uchar)FD_FAILOVER_ROLE_FILE_ACTIVE;
  fd_memset( role.staked_pubkey, 0xAB, sizeof(role.staked_pubkey) );
  role.paused        = 1U;
  role.baton_slot    = 1000UL;
  role.engaged_floor = 999UL;
  return role;
}
...
test_codec( void ) {
  ...
  FD_TEST( buf[ 12UL ]==FD_FAILOVER_ROLE_FILE_ACTIVE );
  ...
  buf[ 12UL ] = FD_FAILOVER_ROLE_FILE_CNT;
  refresh_digest();
  FD_TEST( fd_failover_role_de( buf, sz, &out )==EPROTO );
  buf[ 12UL ] = role.role;
  buf[ 45UL ] = 2U;
