  ...
  switch( ctx->in[ in_idx ].kind ) {
    ...
    case IN_KIND_SNAPIN_MANIF: {
      ...
      if( FD_UNLIKELY( fd_ssmsg_sig_message( sig )==FD_SSMSG_DONE ) ) {
        ...
        int wfs_mode = fd_wfs_mode( ctx->wfs_slot, wfs_hash_is_zero, ctx->wfs_shred_version, ctx->wfs_boot_slot );
...
unprivileged_init( fd_topo_t const *      topo,
  ...
  int wfs_hash_is_zero   = !memcmp( ctx->wfs_bank_hash.uc, ((fd_pubkey_t){ 0 }).uc, sizeof(fd_pubkey_t) );
  ctx->wfs_state = fd_int_if( fd_wfs_configured( ctx->wfs_slot, wfs_hash_is_zero, ctx->wfs_shred_version ),
