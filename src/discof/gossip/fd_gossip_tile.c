  ...
  if( ( now - ctx->last_shred_dest_push_time )>CONTACT_INFO_PUBLISH_TIME_NS &&
    ...
    for( fd_contact_info_table_iter_t iter = fd_contact_info_table_iter_init( ctx->contact_info_table );
...

      {
        ...
        sender_peers[sender_peers_cnt].ip4_addr = ci->addrs[ ci->sockets[ sender_socket_idx].index ].inner.ip4;
        sender_peers[sender_peers_cnt].udp_port = ci->ports[ sender_socket_idx ]; /* NOT converted to net order */
        memcpy( sender_peers[sender_peers_cnt].pubkey, &ci->ci_crd.from, sizeof(fd_pubkey_t) );
