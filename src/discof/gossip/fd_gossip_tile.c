...
unprivileged_init( fd_topo_t const *      topo,
  ...
  *ctx->gossvf_out = out1( topo, tile, "gossip_gossvf" );
  ...
  ctx->my_contact_info->sockets[ FD_GOSSIP_CONTACT_INFO_SOCKET_ALPENGLOW ]         = (fd_gossip_socket_t){ .is_ipv6 = 0, .ip4 = tile->gossip.ports.alpen    ? tile->gossip.ip_addr : 0, .port = fd_ushort_bswap( tile->gossip.ports.alpen )    };

  FD_LOG_WARNING(( "alpenglow port: %hu", tile->gossip.ports.alpen ));
