...
during_frag( fd_gossvf_tile_ctx_t * ctx,
  ...
  switch( ctx->in[ in_idx ].kind ) {
    ...
    case IN_KIND_EPOCH: {
      ...
      ulong msg_sz = fd_epoch_info_msg_sz( msg->staked_cnt, msg->id_staked_cnt );
...
handle_epoch( fd_gossvf_tile_ctx_t *      ctx,
  ...
  /* Use pre-computed identity-deduped stakes if available (Firedancer
     path).  These are computed from FULL vote weights and include all
     vote accounts' stake per identity.  Falls back to deduping from
     compressed weights (which may lose non-leader vote accounts). */
  if( FD_LIKELY( msg->id_staked_cnt > 0UL ) ) {
    fd_stake_weight_t const * id_stakes = fd_epoch_info_msg_id_stakes_const( msg );
    for( ulong i=0UL; i<msg->id_staked_cnt; i++ ) {
