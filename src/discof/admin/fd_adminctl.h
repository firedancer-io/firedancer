...
struct fd_adminctl_failover_status_req_v2 {
  ulong version; /* ==FD_ADMINCTL_FAILOVER_STATUS_PAYLOAD_VERSION */
};
typedef struct fd_adminctl_failover_status_req_v2 fd_adminctl_failover_status_req_t;

struct fd_adminctl_failover_status_resp_v2 {
  ulong version; /* ==FD_ADMINCTL_FAILOVER_STATUS_PAYLOAD_VERSION */
  uchar enabled;
  uchar role;
  ...
  uchar pair_healthy;        /* 1 when readiness_reason is PAIR_HEALTHY */
...
typedef struct fd_adminctl_failover_status_resp_v2 fd_adminctl_failover_status_resp_t;
#define FD_ADMINCTL_FAILOVER_STATUS_PAYLOAD_VERSION (2UL)

/* Phase 0 reports pair health only.  The handoff readiness verdict
   itself, including whether the standby is caught up, arrives with the
   Phase 1 controller. */
#define FD_FAILOVER_READINESS_PAIR_HEALTHY      (0U)
...
FD_STATIC_ASSERT( sizeof(fd_adminctl_failover_status_req_t )==8UL, failover_status_req_v2_layout );
FD_STATIC_ASSERT( sizeof(fd_adminctl_failover_status_resp_t)==232UL, failover_status_resp_v2_layout );
