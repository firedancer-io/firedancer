...
fd_reasm_footprint( ulong fec_max ) {
  ...
  // TODO: PLUS ONE? because max capacity of this map is slot_cnt - 1????
  return FD_LAYOUT_FINI(
    ...
    FD_LAYOUT_INIT,
      ...
      evicted_align(),     evicted_footprint ( max_fec_per_slot ) ),
    fd_reasm_align() );
}
...
fd_reasm_new( void * shmem,
  ...
  void * evicted  = FD_SCRATCH_ALLOC_APPEND( l, evicted_align(),     evicted_footprint ( max_fec_per_slot ) );
  ...
  reasm->evicted    = evicted_new ( evicted, max_fec_per_slot );
...
fd_reasm_join( void * shreasm ) {
  ...
  reasm->evicted  = evicted_join ( reasm->evicted  );
...
fd_reasm_peek( fd_reasm_t * reasm ) {
  for( out_iter_t iter = out_iter_init( reasm->out       );
    ...
    fd_reasm_fec_t * fec = pool_ele( reasm_pool( reasm ), *out_iter_ele( reasm->out, iter ) );
    if( FD_LIKELY( !fec->popped && ( !fec->eqvoc || fec->confirmed ) ) ) return fec;
  }
  return NULL;
}
...
fd_reasm_pop( fd_reasm_t * reasm ) {
  while( FD_LIKELY( !out_empty( reasm->out ) ) ) {
    fd_reasm_fec_t * fec = pool_ele( reasm_pool( reasm ), out_pop_head( reasm->out ) );
    if( FD_LIKELY( !fec->popped && ( !fec->eqvoc || fec->confirmed ) ) ) {
      fec->popped = 1;
      return fec;
    }
  }
...
clear_slot_metadata( fd_reasm_t     * reasm,
  ...
  if( FD_LIKELY( opt_store ) ) fd_store_remove( opt_store, &fec->key );
  return fec;
}
...
clear_chain( fd_reasm_t     * reasm,
  ...
  /* we search up the tree until the bank idx changes. This is usually
     ...
     could also be the middle of the slot.
     ...
     0 ── 32 ──  64 ──  96          (confirmed)
      ...
      Now we know we should evict until the max(parent bank idx changes, or parent has > 1 child) */

  while( FD_LIKELY( clear_slot ) ) {
    fd_reasm_fec_t * parent = fd_reasm_parent( reasm, head );
    if( FD_LIKELY( !parent || parent->bank_idx != head->bank_idx || head->sibling != ULONG_MAX ) ) break;
    head = parent;
  }

  FD_BASE58_ENCODE_32_BYTES( head->key.key, head_key_b58 );
  FD_LOG_NOTICE(( "evicting chain from head slot %lu, fec idx %u, fec: %s, bank_idx %lu", head->slot, head->fec_set_idx, head_key_b58, head->bank_idx ));
   ...
   /* Clean up the parent pointing to this head, and remove block from the maps
    ...
    if( FD_LIKELY( 0==memcmp( &child->key, &head->key, sizeof(fd_hash_t) ) ) ) {
      ...
      while( FD_LIKELY( sibling ) ) {
        if( FD_LIKELY( 0==memcmp( &sibling->key, &head->key, sizeof(fd_hash_t) ) ) ) {
  ...
  /* Remove elements from out queue if they are free */
...
# endif
  ...
  if( FD_UNLIKELY( !pool_free( pool ) ) ){
    int rv = fd_reasm_evict( reasm, opt_store, merkle_root, chained_merkle_root );
...
# endif
  ...
  while( FD_LIKELY( !bfs_empty( bfs ) ) ) {
    ...
    clear_slot_metadata( reasm, head, opt_store );
