...
fd_reasm_pop( fd_reasm_t * reasm ) {
  fd_reasm_fec_t * pool = reasm_pool( reasm );
  while( FD_LIKELY( !out_is_empty( reasm->out, pool ) ) ) {
    fd_reasm_fec_t * fec = out_ele_pop_head( reasm->out, pool );
    fec->in_out = 0;
...
reasm_link( fd_reasm_t     * reasm,
  ...
  if( FD_LIKELY( parent->child == pool_idx_null( pool ) ) ) {
    parent->child = (uint)pool_idx( pool, child ); /* set as left-child. */
  } else {
    ...
    sibling->sibling = (uint)pool_idx( pool, child ); /* set as right-sibling. */
  }
}
...
subtrees_remove( fd_reasm_t     * reasm,
  ...
  while( FD_LIKELY( !bfs_empty( bfs ) ) ) {
    ...
    if( FD_LIKELY( opt_store ) ) {
      fd_store_fec_t * sfec = fd_store_remove( opt_map_join, &ele->key );
      if( FD_LIKELY( sfec ) ) fd_store_fec_release( opt_store, sfec );
    }
...
fd_reasm_remove( fd_reasm_t     * reasm,
  ...
  if( FD_LIKELY( orphaned_ele_query( orphaned, &head->key, NULL, pool ) ||
    ...
    FD_TEST( head->child == null ); /* must be a leaf node */
  } else {
  ...
  FD_LOG_INFO(( "evicting reasm slot %lu, fec idx %u, down to %u bank_idx %u", head->slot, head->fec_set_idx, tail->fec_set_idx, head->bank_idx ));
  ...
  if( FD_LIKELY( parent ) ) {
   /* Clean up the parent pointing to this head, and remove block from the maps
    ...
    if( FD_LIKELY  ( removed_orphan = orphaned_ele_remove( orphaned, &head->key, NULL, pool ) ) ) {
      ...
      if( FD_LIKELY( opt_store ) ) {
        fd_store_fec_t * sfec = fd_store_remove( opt_map_join, &head->key );
        if( FD_LIKELY( sfec ) ) fd_store_fec_release( opt_store, sfec );
      }
      return head;
    }
    ...
    while( FD_LIKELY( curr ) ) {
      ...
      if( FD_LIKELY( opt_store ) ) {
        fd_store_fec_t * sfec = fd_store_remove( opt_map_join, &removed->key );
        if( FD_LIKELY( sfec ) ) fd_store_fec_release( opt_store, sfec );
      }
    }
...
evict( fd_reasm_t      * reasm,
  ...
  for( subtreel_iter_t iter = subtreel_iter_fwd_init(       subtreel, pool );
    ...
    if( ele->child != null || memcmp( &ele->key, parent_root, sizeof(fd_hash_t) ) == 0 ) continue;
  ...
  /* for any subtree:
    ...
    if( subtrees_ele_query( subtrees, parent_root, NULL, pool )  ||
      ...
      while( FD_LIKELY( root->parent != null ) ) {
...
fd_reasm_init( fd_reasm_t *      reasm,
  ...
  fd_reasm_fec_t * pool = reasm_pool( reasm );
  uint             idx_null = (uint)pool_idx_null( pool );
  ...
  fec->bank_dead       = 0;
  fec->dead_reported   = 0;
  fec->bank_idx        = UINT_MAX;
  fec->parent_bank_idx = UINT_MAX;
  fec->bank_seq        = ULONG_MAX;
  fec->fec_completed_ts_nanos = 0UL;
  fec->out.next        = idx_null;
  fec->out.prev        = idx_null;
  fec->in_out          = 0;
  fec->xid_next        = UINT_MAX;
  fec->subtreel.next   = idx_null;
  fec->subtreel.prev   = idx_null;
...
# endif
  ...
  *evicted = NULL;
  ...
  fec->data_complete   = (uchar)!!data_complete;
  fec->slot_complete   = (uchar)!!slot_complete;
  fec->is_leader       = (uchar)!!is_leader;
  fec->eqvoc           = 0;
  fec->confirmed       = 0;
  fec->popped          = 0;
  fec->bank_dead       = 0;
