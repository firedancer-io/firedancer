...
# endif
  ...
  if( FD_UNLIKELY( !bid           ) ) bid = bid_insert( reasm->bid, slot );
  if( FD_UNLIKELY( slot_complete  ) ) bid->idx = pool_idx( pool, fec );
  ...
  if( FD_UNLIKELY( parent && parent->eqvoc ) ) eqvoc( reasm, fec );
