...
test_setup( int * out_fd,
  ...
  *out_fd = fd;
  ...
  FD_TEST( mem );
  fd_accdb_t * accdb = fd_accdb_join( fd_accdb_new( mem, max_accounts, max_live_slots, max_account_writes_per_slot, partition_cnt, partition_sz, 42UL ), fd );
  FD_TEST( accdb );
  return accdb;
}
...
test_basic( void ) {
  ...
  FD_TEST( !fd_accdb_read( accdb, slot1, pubkey0, NULL, NULL, NULL, owner ) );
  FD_TEST( !fd_accdb_read( accdb, slot1, pubkey1, NULL, NULL, NULL, owner ) );
  fd_accdb_write( accdb, slot1, pubkey1, 1UL, NULL, 0UL, owner2 );
  FD_TEST( !fd_accdb_read( accdb, slot1, pubkey0, NULL, NULL, NULL, owner ) );
  FD_TEST( fd_accdb_read( accdb, slot1, pubkey1, &lamports, &d, &data_len, owner ) );
...
test_fork_basic( void ) {
  ...
  FD_TEST( !fd_accdb_read( accdb, f1, pubkey0, NULL, NULL, NULL, owner ) );
  FD_TEST( !fd_accdb_read( accdb, f2, pubkey0, NULL, NULL, NULL, owner ) );
  FD_TEST( !fd_accdb_read( accdb, f3, pubkey0, NULL, NULL, NULL, owner ) );

  fd_accdb_write( accdb, f1, pubkey1, 1UL, NULL, 0UL, owner2 );
  FD_TEST(  fd_accdb_read( accdb, f1, pubkey1, &lamports, &d, &data_len, owner ) );
  FD_TEST( !fd_accdb_read( accdb, f2, pubkey1, &lamports, &d, &data_len, owner ) );
  FD_TEST( !fd_accdb_read( accdb, f3, pubkey1, &lamports, &d, &data_len, owner ) );

  fd_accdb_write( accdb, f2, pubkey1, 1UL, NULL, 0UL, owner2 );
  FD_TEST(  fd_accdb_read( accdb, f1, pubkey1, &lamports, &d, &data_len, owner ) );
  FD_TEST(  fd_accdb_read( accdb, f2, pubkey1, &lamports, &d, &data_len, owner ) );
  FD_TEST( !fd_accdb_read( accdb, f3, pubkey1, &lamports, &d, &data_len, owner ) );
  ...
  FD_TEST(  fd_accdb_read( accdb, f4, pubkey1, &lamports, &d, &data_len, owner ) );
  FD_TEST( !fd_accdb_read( accdb, f5, pubkey1, &lamports, &d, &data_len, owner ) );
...
test_root_forks( void ) {
  ...
  fd_accdb_write( accdb, f2, pubkey1, 1UL, NULL, 0UL, owner2 );
  fd_accdb_write( accdb, f1, pubkey1, 2UL, NULL, 0UL, owner2 );
  fd_accdb_fork_id_t f3 = fd_accdb_attach_child( accdb, f1 );
  fd_accdb_write( accdb, f3, pubkey1, 3UL, NULL, 0UL, owner2 );
...
test_compact( void ) {
  ...
  for( ulong i=0UL; i<writes_fit_in_partition; i++ ) {
    fd_accdb_write( accdb, slot1, pubkey1, 1UL, big_data, acct_sz, owner2 );
  }
...
test_overwrite_same_fork( void ) {
  ...
  fd_accdb_write( accdb, slot1, pubkey1, 100UL, data_a, 4UL, owner2 );
  FD_TEST( fd_accdb_read( accdb, slot1, pubkey1, &lamports, d, &data_len, owner ) );
  FD_TEST( lamports==100UL );
  FD_TEST( data_len==4UL );
  FD_TEST( !memcmp( d, data_a, 4UL ) );

  fd_accdb_write( accdb, slot1, pubkey1, 200UL, data_b, 4UL, owner3 );
  FD_TEST( fd_accdb_read( accdb, slot1, pubkey1, &lamports, d, &data_len, owner ) );
  ...
  fd_accdb_write( accdb, slot1, pubkey1, 300UL, data_c, 2UL, owner2 );
  FD_TEST( fd_accdb_read( accdb, slot1, pubkey1, &lamports, d, &data_len, owner ) );
...
test_multiple_accounts( void ) {
  ...
  fd_accdb_write( accdb, f1, pk_a, 10UL, NULL, 0UL, owner2 );
  fd_accdb_write( accdb, f1, pk_b, 20UL, NULL, 0UL, owner2 );
  fd_accdb_write( accdb, f1, pk_c, 30UL, NULL, 0UL, owner3 );

  FD_TEST( fd_accdb_read( accdb, f1, pk_a, &lamports, &d, &data_len, owner ) );
  FD_TEST( lamports==10UL );
  FD_TEST( fd_accdb_read( accdb, f1, pk_b, &lamports, &d, &data_len, owner ) );
  FD_TEST( lamports==20UL );
  FD_TEST( fd_accdb_read( accdb, f1, pk_c, &lamports, &d, &data_len, owner ) );
...
test_sequential_rooting( void ) {
  ...
  fd_accdb_write( accdb, root, pubkey1, 1UL, NULL, 0UL, owner2 );
  fd_accdb_write( accdb, a, pubkey1, 2UL, NULL, 0UL, owner2 );

  fd_accdb_advance_root( accdb, a );

  fd_accdb_fork_id_t b = fd_accdb_attach_child( accdb, a );
  fd_accdb_write( accdb, b, pubkey1, 3UL, NULL, 0UL, owner2 );
  FD_TEST( fd_accdb_read( accdb, b, pubkey1, &lamports, &d, &data_len, owner ) );
  FD_TEST( lamports==3UL );

  fd_accdb_advance_root( accdb, b );

  fd_accdb_fork_id_t c = fd_accdb_attach_child( accdb, b );
  FD_TEST( fd_accdb_read( accdb, c, pubkey1, &lamports, &d, &data_len, owner ) );
  FD_TEST( lamports==3UL );

  fd_accdb_write( accdb, c, pubkey1, 4UL, NULL, 0UL, owner3 );
  FD_TEST( fd_accdb_read( accdb, c, pubkey1, &lamports, &d, &data_len, owner ) );
...
test_purge( void ) {
  ...
  fd_accdb_write( accdb, keep, pk_a, 100UL, NULL, 0UL, owner2 );
  fd_accdb_write( accdb, drop, pk_b,  50UL, NULL, 0UL, owner2 );

  FD_TEST(  fd_accdb_read( accdb, keep, pk_a, &lamports, &d, &data_len, owner ) );
  FD_TEST(  fd_accdb_read( accdb, drop, pk_b, &lamports, &d, &data_len, owner ) );
...
test_child_inherits_parent( void ) {
  ...
  fd_accdb_write( accdb, parent, pubkey1, 10UL, NULL, 0UL, owner2 );

  fd_accdb_fork_id_t child = fd_accdb_attach_child( accdb, parent );

  /* Child can see parent's write */
  FD_TEST( fd_accdb_read( accdb, child, pubkey1, &lamports, &d, &data_len, owner ) );
  FD_TEST( lamports==10UL );

  /* Overwrite on child */
  fd_accdb_write( accdb, child, pubkey1, 99UL, NULL, 0UL, owner3 );
  FD_TEST( fd_accdb_read( accdb, child, pubkey1, &lamports, &d, &data_len, owner ) );
  FD_TEST( lamports==99UL );
  FD_TEST( !memcmp( owner, owner3, 32UL ) );

  /* Parent still sees original */
  FD_TEST( fd_accdb_read( accdb, parent, pubkey1, &lamports, &d, &data_len, owner ) );
  FD_TEST( lamports==10UL );
  FD_TEST( !memcmp( owner, owner2, 32UL ) );
...
# define DEPTH (10UL)
  ...
  fd_accdb_write( accdb, chain[ 0 ], pubkey1, 1UL, NULL, 0UL, owner2 );
  for( ulong i=1UL; i<DEPTH; i++ ) {
    chain[ i ] = fd_accdb_attach_child( accdb, chain[ i-1UL ] );
    fd_accdb_write( accdb, chain[ i ], pubkey1, i+1UL, NULL, 0UL, owner2 );
  }

  /* Each fork should see its own write. */
  for( ulong i=0UL; i<DEPTH; i++ ) {
    FD_TEST( fd_accdb_read( accdb, chain[ i ], pubkey1, &lamports, &d, &data_len, owner ) );
  ...
  FD_TEST( fd_accdb_read( accdb, chain[ 4 ], pubkey1, &lamports, &d, &data_len, owner ) );
...
# define SIBLINGS (16UL)
  ...
  for( ulong i=0UL; i<SIBLINGS; i++ ) {
    ...
    fd_accdb_write( accdb, sibs[ i ], pubkey1, (i+1UL)*100UL, NULL, 0UL, owner2 );
  }

  /* Each sibling should read back exactly its own lamports. */
  for( ulong i=0UL; i<SIBLINGS; i++ ) {
    FD_TEST( fd_accdb_read( accdb, sibs[ i ], pubkey1, &lamports, &d, &data_len, owner ) );
...
test_purge_deep_subtree( void ) {
  ...
  fd_accdb_write( accdb, drop,            pk_a, 1UL, NULL, 0UL, owner2 );
  fd_accdb_write( accdb, drop_child,      pk_b, 2UL, NULL, 0UL, owner2 );
  fd_accdb_write( accdb, drop_grandchild, pk_c, 3UL, NULL, 0UL, owner2 );
  fd_accdb_write( accdb, keep,            pk_a, 9UL, NULL, 0UL, owner3 );
...
test_root_tombstones_old_version( void ) {
  ...
  fd_accdb_write( accdb, root, pubkey1, 10UL, NULL, 0UL, owner2 );
  FD_TEST( fd_accdb_metrics( accdb )->accounts_total==1UL );

  fd_accdb_write( accdb, a, pubkey1, 20UL, NULL, 0UL, owner3 );
  ...
  FD_TEST( fd_accdb_read( accdb, a, pubkey1, &lamports, &d, &data_len, owner ) );
  FD_TEST( lamports==20UL );
  FD_TEST( !memcmp( owner, owner3, 32UL ) );
...
# define N_ACCTS (200UL)
  ...
  for( ulong i=0UL; i<N_ACCTS; i++ ) {
    ...
    fd_accdb_write( accdb, f, pks[ i ], i+1UL, NULL, 0UL, owner2 );
  }
  ...
  for( ulong i=0UL; i<N_ACCTS; i++ ) {
    FD_TEST( fd_accdb_read( accdb, f, pks[ i ], &lamports, &d, &data_len, owner ) );
    FD_TEST( lamports==i+1UL );
  }

  /* Overwrite the first 50 and verify again. */
  for( ulong i=0UL; i<50UL; i++ ) {
    fd_accdb_write( accdb, f, pks[ i ], (i+1UL)*1000UL, NULL, 0UL, owner3 );
  }
