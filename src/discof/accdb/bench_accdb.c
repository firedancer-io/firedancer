...
bench_setup( int * out_fd,
  ...
  *out_fd = fd;
  ...
  FD_TEST( mem );
  fd_accdb_lsm_t * accdb = fd_accdb_lsm_join(
    fd_accdb_lsm_new( mem, max_accounts, max_live_slots,
...
bench_write( ulong   account_cnt,
  ...
  fd_accdb_lsm_t * accdb = bench_setup( &fd,
                                    account_cnt + 1024UL,
                                    64UL,
  ...
  fd_accdb_fork_id_t root  = fd_accdb_lsm_attach_child( accdb, (fd_accdb_fork_id_t){ .val = USHORT_MAX } );
  fd_accdb_fork_id_t fork  = fd_accdb_lsm_attach_child( accdb, root );
  ...
  for( ulong i=0UL; i<account_cnt; i++ ) {
    ...
    fd_accdb_lsm_write( accdb, fork, pubkey, i+1UL,
...
bench_read( ulong   account_cnt,
  ...
  for( ulong i=0UL; i<account_cnt; i++ ) {
    ...
    if( fd_accdb_lsm_read( accdb, fork, pubkey, &lamports,
...
bench_replay( ulong   slot_cnt,
  ...
  for( ulong s=0UL; s<slot_cnt; s++ ) {
    fd_accdb_fork_id_t cur = fd_accdb_lsm_attach_child( accdb, prev );
    ...
    for( ulong r=0UL; r<reads_per_slot; r++ ) {
      ...
      make_pubkey( pubkey, idx );
      if( fd_accdb_lsm_read( accdb, cur, pubkey, &lamports,
                         rdata, &data_len, owner ) ) {
        total_read_hits++;
      }
      total_reads++;
    ...
    for( ulong w=0UL; w<writes_per_slot; w++ ) {
      ...
      ulong sz = sample_account_size( rng );
      if( sz>BENCH_MAX_DATA_SZ ) sz = BENCH_MAX_DATA_SZ;
      fd_accdb_lsm_write( accdb, cur, pubkey, (s*writes_per_slot + w)+1UL,
                      sz ? data_buf : NULL, sz, dummy_owner );
    ...
    /* --- root previous slot --- */
    if( FD_LIKELY( s>0UL ) ) {
      fd_accdb_lsm_advance_root( accdb, prev );
    }
  ...
  fd_accdb_lsm_metrics_t const * m = fd_accdb_lsm_metrics( accdb );
...
bench_mixed( ulong   base_cnt,
  ...
  for( ulong i=0UL; i<op_cnt; i++ ) {
    ...
    make_pubkey( pubkey, idx );
    if( coin < read_pct ) {
      fd_accdb_lsm_read( accdb, fork, pubkey, &lamports,
                     rdata, &data_len, owner );
      reads++;
    } else {
      ulong sz = sample_account_size( rng );
