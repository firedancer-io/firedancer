...
#define MAP_ELE_T                  fd_accdb_cache_ele_t
#define MAP_KEY_T                  fd_accdb_cache_key_t
#define MAP_KEY                    acc_idx
#define MAP_KEY_FROM_ELE(ctx,ele)  (&((fd_accdb_cache_map_ctx_t const *)(ctx))->acc_pool[ (ele)->acc_idx ].key)
#define MAP_KEY_EQ(k0,k1)          fd_accdb_cache_key_eq( (k0), (k1) )
#define MAP_KEY_HASH(key,seed)     fd_accdb_cache_key_hash( (key), (seed) )
#define MAP_KEY_EQ_IS_SLOW         1
#define MAP_ELE_IS_FREE(ctx,ele)   ((ele)->acc_idx==UINT_MAX)
#define MAP_ELE_FREE(ctx,ele)      do { (ele)->acc_idx = UINT_MAX; } while(0)
#define MAP_ELE_MOVE(ctx,dst,src)  do { fd_accdb_cache_ele_t * _src = (src); *(dst) = *_src; _src->acc_idx = UINT_MAX; } while(0)
...
fd_accdb_new( void *             ljoin,
  ...
  ulong cache_map_lock_cnt = cache_map_lock_cnt_est( cache_map_ele_max );
  ...
  void * _cache_shmap      = FD_SCRATCH_ALLOC_APPEND( l, cache_map_align(),              cache_map_footprint( cache_map_ele_max, cache_map_lock_cnt, cache_map_probe ) );
  ...
  FD_TEST( cache_map_join( accdb->cache_map, _cache_shmap, _cache_shele ) );
  fd_accdb_cache_map_ctx_t * cache_ctx = (fd_accdb_cache_map_ctx_t *)cache_map_ctx( accdb->cache_map );
  cache_ctx->acc_pool = accdb->acc_pool;
...
cache_query( fd_accdb_t *  accdb,
             uchar const * pubkey,
             ulong         generation ) {
  fd_accdb_cache_key_t qkey;
  fd_memcpy( qkey.pubkey, pubkey, 32UL );
  qkey.generation = generation;

  fd_accdb_cache_ele_t sentinel[1];
  sentinel->acc_idx = UINT_MAX;
  cache_map_query_t query[1];

  for(;;) {
    int err = cache_map_query_try( accdb->cache_map, &qkey, sentinel, query, 0 );
    if( FD_UNLIKELY( err==FD_MAP_ERR_KEY ) ) return NULL;
    if( FD_UNLIKELY( err==FD_MAP_ERR_AGAIN ) ) { FD_SPIN_PAUSE(); continue; }

    fd_accdb_cache_ele_t const * ele = cache_map_query_ele_const( query );
    if( FD_UNLIKELY( ele->acc_idx==UINT_MAX ) ) return NULL;

    ulong cache_idx = ele->cache_idx;
    if( FD_UNLIKELY( cache_map_query_test( query ) ) ) { FD_SPIN_PAUSE(); continue; }

    return cache_line( accdb, FD_ACCDB_CACHE_PACK_CLASS( cache_idx ), FD_ACCDB_CACHE_PACK_IDX( cache_idx ) );
  }
}
...
cache_remove( fd_accdb_t *            accdb,
  ...
  cache_map_query_t query[1] = {0};
  int err = cache_map_remove( accdb->cache_map, &line->key, query, FD_MAP_FLAG_BLOCKING );
  FD_TEST( !err );
...
cache_insert( fd_accdb_t *            accdb,
  ...
  int err = cache_map_prepare( accdb->cache_map, &key_acc->key, sentinel, query, FD_MAP_FLAG_BLOCKING );
  FD_TEST( !err );

  fd_accdb_cache_ele_t * ele = cache_map_query_ele( query );
  FD_TEST( ele->acc_idx==UINT_MAX ); /* must be a fresh slot */
  ele->acc_idx   = acc_idx;
  ele->cache_idx = FD_ACCDB_CACHE_PACK( size_class, cache_line_idx( accdb, size_class, line ) );

  cache_map_publish( query );

  line->cache_idx = (uint)( ele - (fd_accdb_cache_ele_t *)cache_map_shele( accdb->cache_map ) );
}

/* cache_free_push pushes a fully-freed cache line onto the per-class
   CAS free list (Treiber stack).  The caller must have already removed
   the line from the cache_map (key.generation==ULONG_MAX) and set
   persisted=1 before pushing. */
...
acc_unlink( fd_accdb_t * accdb,
  ...
  /* If the freed acc is still referenced by a cache line, remove it so
     the eviction path does not try to write back stale data from a
     recycled pool slot.  Lock-free: CAS the refcnt 0 -> EVICT_SENTINEL
     to claim it exclusively, then remove from cache map and push to
     the CAS free list.  If the line is pinned (refcnt>0), skip — the
     pinner's release will handle it. */
  fd_accdb_cache_line_t * stale = cache_query( accdb, acc->key.pubkey, acc->key.generation );
  if( FD_UNLIKELY( stale ) ) {
    ...
    if( FD_LIKELY( !old_rc ) ) {
      /* Claimed.  Validate key (ABA — slot could have been recycled
         between cache_query and the CAS). */
  ...
  if( txn!=UINT_MAX ) {
    ...
    while( txn!=UINT_MAX ) {
      ...
      while( acc!=UINT_MAX ) {
        ...
        if( FD_LIKELY( (cur_acc->key.generation<=parent_fork->shmem->generation || descends_set_test( fork->descends, cur_acc->fork_id ) ) && !memcmp( new_acc->key.pubkey, cur_acc->key.pubkey, 32UL ) ) ) {
          uint next = cur_next;
...
acquire_cache_line( fd_accdb_t * accdb,
  ...
  /* Priority 1: CAS free list — already removed from cache_map,
     persisted==1, generation==ULONG_MAX.  Cheapest path. */
  ...
  for(;;) {
    ...
    if( FD_UNLIKELY( line->key.generation==ULONG_MAX && line->acc_idx==UINT_MAX ) ) continue;
    ...
    if( FD_LIKELY( line->key.generation!=ULONG_MAX ) ) cache_remove( accdb, line );
...
fd_accdb_acquire( fd_accdb_t *          accdb,
  ...
  for( ulong i=0UL; i<pubkeys_cnt; i++ ) {
    ...
    while( acc!=UINT_MAX ) {
      ...
      if( FD_UNLIKELY( (candidate_acc->key.generation>root_generation && candidate_acc->fork_id!=fork_id.val && !descends_set_test( fork->descends, candidate_acc->fork_id )) ) || memcmp( pubkeys[ i ], candidate_acc->key.pubkey, 32UL ) ) {
        acc = next_acc;
        continue;
      }
  ...
  for( ulong i=0UL; i<pubkeys_cnt; i++ ) {
    ...
    if( FD_LIKELY( accs[ i ] ) ) {
      fd_accdb_cache_line_t * hit = cache_query( accdb, pubkeys[ i ], accs[ i ]->key.generation );
      if( FD_LIKELY( hit ) ) original_cache_line[ i ] = cache_try_pin( hit, pubkeys[ i ], accs[ i ]->key.generation );
    }
    ...
    if( FD_UNLIKELY( writable[ i ] ) ) {
      for( ulong j=0UL; j<FD_ACCDB_CACHE_CLASS_CNT; j++ ) {
        ...
        if( FD_LIKELY( evicted->offset!=ULONG_MAX ) ) {
          fd_accdb_shmem_bytes_freed( accdb->shmem, evicted->offset, entry_sz );
  ...
  for( ulong i=0UL; i<pubkeys_cnt; i++ ) {
    ...
    while( FD_UNLIKELY( FD_VOLATILE_CONST( accs[ i ]->offset )==ULONG_MAX ) ) FD_SPIN_PAUSE();
...
fd_accdb_release( fd_accdb_t *       accdb,
  ...
  for( ulong i=0UL; i<entries_cnt; i++ ) {
    ...
    /* For overwrite commits, defer the refcnt decrement on
       original_cache_line until after cache_remove completes.  If
       we dropped refcnt to 0 here, a concurrent CLOCK sweep could
       CAS(refcnt, 0, EVICT_SENTINEL) and steal the line before we
       get to cache_remove, causing an assertion failure or data
       corruption.  Non-overwrite and non-commit paths unpin
       immediately because they never call cache_remove on the
       original line. */
    ...
    /* For overwrites, invalidate the on-disk offset BEFORE removing
       ...
       the cache will see offset==ULONG_MAX and spin-wait, rather
    ...
    ulong old_offset = ULONG_MAX;
    if( FD_LIKELY( entries[ i ]._overwrite ) ) {
      fd_accdb_acc_t * ow_acc = &accdb->acc_pool[ original_acc_idx ];
      old_offset = FD_ATOMIC_XCHG( &ow_acc->offset, ULONG_MAX );
      if( FD_LIKELY( old_offset!=ULONG_MAX ) ) {
        fd_accdb_shmem_bytes_freed( accdb->shmem, old_offset, (ulong)ow_acc->size+sizeof(fd_accdb_disk_meta_t) );
    ...
    if( FD_UNLIKELY( new_size_class==7UL ) ) {
      ...
      if( FD_LIKELY( entries[ i ]._overwrite ) ) {
        ...
        /* Now safe to unpin and free — line is out of the cache map. */
        FD_ATOMIC_FETCH_AND_SUB( &original_cache_line->refcnt, 1U );
      ...
      if( FD_LIKELY( original_size_class==new_size_class ) ) {
        if( FD_LIKELY( entries[ i ]._overwrite ) ) {
          ...
          /* Keep refcnt>=1 through the reuse window so CLOCK cannot
             steal the line between cache_remove and cache_insert. The
             pin is released in the destination cleanup loop after
             cache_insert has republished the line. */
          original_cache_line->acc_idx = UINT_MAX;
    ...
    /* Handle every destination cache line: committed ones keep
       refcnt>=1 until cache_insert publishes them (the deferred
       unpin happens after cache_insert below), uncommitted ones are
    ...
    if( FD_LIKELY( entries[ i ]._overwrite ) ) {
      ...
      /* The offset was already atomically swapped to ULONG_MAX and
         bytes freed above (before cache_remove), so just update the
         metadata and re-insert into the cache. */
      ...
      /* Now that cache_insert has published the line, unpin it so
  ...
  for( ulong c=0UL; c<FD_ACCDB_CACHE_CLASS_CNT; c++ ) {
    ...
    for( ulong tick=0UL; tick<budget && depth+evicted<target; tick++ ) {
      ...
      if( FD_UNLIKELY( !line->persisted && acc_idx!=UINT_MAX ) ) {
        ...
        /* Atomically swap the old offset to ULONG_MAX so that a
        ...
        ulong old_offset = FD_ATOMIC_XCHG( &acc->offset, ULONG_MAX );
        if( FD_LIKELY( old_offset!=ULONG_MAX ) ) {
