...
acc_unlink( fd_accdb_t * accdb,
  ...
  if( FD_LIKELY( fd_accdb_acc_offset(acc)!=FD_ACCDB_OFF_INVAL ) ) {
    fd_accdb_shmem_bytes_freed( accdb->shmem, fd_accdb_acc_offset(acc), (ulong)acc->size+sizeof(fd_accdb_disk_meta_t) );
...
fd_accdb_acquire( fd_accdb_t *          accdb,
  ...
  for( ulong i=0UL; i<pubkeys_cnt; i++ ) {
    ...
    read_sizes[ read_ops_cnt ]   = 32UL + accs[ i ]->size;
    read_ops[ read_ops_cnt++ ]   = (struct iovec){ .iov_base = original_cache_line[ i ]->owner, .iov_len = 32UL + accs[ i ]->size };
  }
