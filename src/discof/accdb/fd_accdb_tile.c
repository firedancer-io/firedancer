...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  l = FD_LAYOUT_APPEND( l, fd_accdb_lsm_align(),           fd_accdb_lsm_footprint( tile->accdb.max_unrooted_slots ) );
  return FD_LAYOUT_FINI( l, scratch_align() );
}

static inline void
metrics_write( fd_accdb_tile_ctx_t * ctx ) {
  fd_accdb_shmem_metrics_t const * metrics = fd_accdb_lsm_metrics( ctx->accdb );
...
before_credit( fd_accdb_tile_ctx_t * ctx,
               ...
               int *                 charge_busy ) {
  if( FD_UNLIKELY( ctx->idle ) ) fd_accdb_lsm_compact( ctx->accdb, charge_busy );
  ctx->idle = 1;
}
...
during_frag( fd_accdb_tile_ctx_t * ctx,
  ...
  switch( sig ) {
    case FD_ACCDB_OP_READ: {
      ...
      fd_accdb_lsm_read( ctx->accdb, req->fork_id, req->pubkey, &resp->lamports, (uchar *)(resp+1UL), &resp->data_len, resp->owner );
      break;
    }
    case FD_ACCDB_OP_WRITE: {
      ...
      fd_accdb_lsm_write( ctx->accdb, req->fork_id, req->pubkey, req->lamports, (uchar *)(req+1UL), req->data_len, req->owner );
      break;
    }
    default:
...
unprivileged_init( fd_topo_t *      topo,
  ...
  void * lsm_mem = FD_SCRATCH_ALLOC_APPEND( l, fd_accdb_lsm_align(),
                                                fd_accdb_lsm_footprint( tile->accdb.max_unrooted_slots ) );
  FD_TEST( fd_accdb_lsm_new( lsm_mem, shmem ) );
  ctx->accdb = fd_accdb_lsm_join( lsm_mem, ctx->fd );
  FD_TEST( ctx->accdb );
