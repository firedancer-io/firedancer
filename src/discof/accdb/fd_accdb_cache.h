...
/* fd_accdb_cache.h provides a static algorithm for determining, given a
   ...
   x4 geometric progression:
     ...
     Class 0: 0-128 B      (slot: 168 B)
     Class 1: 129-512 B    (slot: 552 B)
     Class 2: 513-2 KiB    (slot: 2,088 B)
     Class 3: 2K-8 KiB     (slot: 8,232 B)
     Class 4: 8K-32 KiB    (slot: 32,808 B)
     Class 5: 32K-128 KiB  (slot: 131,112 B)
     Class 6: 128K-1 MiB   (slot: 1,048,616 B)
     Class 7: 1M-10 MiB    (slot: 10,485,800 B)

   Each slot has 40 bytes of fixed metadata overhead on top of the max
   data capacity for its class.  Slot sizes are 8-byte aligned.
...
#define FD_ACCDB_CACHE_META_SZ  (40UL)
...
/* fd_accdb_cache_class_cnt computes the number of slots to allocate for
   ...
   The cache and staging pools are unified: class 7 slots serve
   double duty as both cache entries for large accounts and as
   10 MiB staging buffers for writable accounts.  On commit, data
   is copied from the staging slot into a right-sized cache slot
   and the class 7 slot is released.

   cache_footprint is the total memory budget in bytes.

   class_cnt is populated with the slot count for each class on
   return.  The sum of class_cnt[c]*slot_sz[c] will not exceed
   cache_footprint.

   Every class gets at least 1300 entries, guaranteeing a worst
   case 5-transaction bundle (130 accounts * 5 transactions) can
   execute fully in memory regardless of account size mix.
   Returns 0 if the budget is too small for these minimums, or 1
   on success.

   The algorithm:
   1) Reserves 650 of each class off the top.
   2) Reserves additional per-class minimums (at most 1% of
      remaining budget per class, clamped to [1, 1024] slots).
   3) Iteratively allocates remaining budget proportional to
      access density weights derived from mainnet replay data.
   4) Caps classes at estimated population maximums and
      redistributes surplus to uncapped classes. */
