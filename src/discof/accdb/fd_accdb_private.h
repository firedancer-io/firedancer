...
/* cache_map: concurrent linear-probed map from (pubkey, generation) to
   a cache_idx ulong encoding the cache class (high 3 bits) and pool
   index (low 61 bits).  Elements store an acc_pool index and look up
   the actual key indirectly via MAP_KEY_FROM_ELE. */

struct fd_accdb_cache_ele {
  uint  acc_idx;
  ulong cache_idx;
};

typedef struct fd_accdb_cache_ele fd_accdb_cache_ele_t;
...
struct fd_accdb_cache_map_ctx {
  fd_accdb_acc_t const * acc_pool;
};

typedef struct fd_accdb_cache_map_ctx fd_accdb_cache_map_ctx_t;

FD_FN_PURE static inline int
fd_accdb_cache_key_eq( fd_accdb_cache_key_t const * k0,
                       fd_accdb_cache_key_t const * k1 ) {
  return k0->generation==k1->generation && !memcmp( k0->pubkey, k1->pubkey, 32UL );
}

FD_FN_PURE static inline ulong
fd_accdb_cache_key_hash( fd_accdb_cache_key_t const * key,
                         ulong                        seed ) {
  return key->generation ^ fd_funk_rec_key_hash1( key->pubkey, seed );
}

#define MAP_NAME                    cache_map
#define MAP_ELE_T                   fd_accdb_cache_ele_t
#define MAP_KEY_T                   fd_accdb_cache_key_t
#define MAP_KEY                     acc_idx /* unused but required by template */
