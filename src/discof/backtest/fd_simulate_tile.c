  ...
  if( FD_LIKELY( ctx->in_kind[ in_idx ] == IN_KIND_REPLAY ) ) {
    if( FD_LIKELY( sig == REPLAY_SIG_SLOT_COMPLETED) ) {
      if (ctx->replay_initialized == 0) {
        ...
        FD_LOG_NOTICE(( "SIMULATE: Received genesis completion signal, starting FEC processing" ));
      }
      ...
      FD_LOG_NOTICE(( "SIMULATE: Replayed slot %lu", ctx->replayed_slot ));
...
after_credit( ctx_t *             ctx,
  ...
  if ( FD_UNLIKELY( ctx->replayed_slot + MAX_SIMULATE_AHEAD_OF_PUBLISHED_SLOT <= ctx->published_slot ) ) {
    // FD_LOG_NOTICE(( "SIMULATE: Replayed slot %lu is too far ahead of published slot %lu, stopping FEC processing", ctx->replayed_slot, ctx->published_slot ));
    *charge_busy = 1;
    return;
  }
...
unprivileged_init( fd_topo_t *      topo,
  ...
  FD_LOG_NOTICE(( "fread result: %lu", res ));
