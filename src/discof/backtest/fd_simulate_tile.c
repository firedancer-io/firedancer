...
typedef struct {
  ...
  fd_fork_counter_ele_t * fork_counter_elements;
...
/*
 ...
 messages. These slot completion messages are used to determine two main
 things:
    - When the simulation has complete, (reached end of )

*/

void
log_success_error( ctx_t * ctx ) {
  printf("\033[0;32mSIMULATE: Success, replayed %lu slots and %lu forks\033[0m\n", ctx->max_slot - ctx->min_slot + 1, ctx->fork_counter_cnt );
  exit(0);
}
...
during_frag( ctx_t * ctx,
  ...
  /* Handle messages from replay tile */
  if( FD_LIKELY( ctx->in_kind[ in_idx ] == IN_KIND_REPLAY ) ) {
    if( FD_LIKELY( sig == REPLAY_SIG_SLOT_COMPLETED) ) {
      if (ctx->replay_initialized == 0) {
        ctx->replay_initialized = 1;
        FD_LOG_INFO(( "SIMULATE: Received start of replay signal, starting FEC processing" ));
      }
      ...
      if (ctx->published_slot == 0) {
        ctx->published_slot = slot_info->slot;
      }
      FD_LOG_INFO(( "SIMULATE: Replayed slot %lu, published slot %lu, parent slot %lu", ctx->replayed_slot, ctx->published_slot, slot_info->parent_slot ));
      // take slot_info->parent_slot and increment the count in the fork counter
      ulong parent_slot = slot_info->parent_slot;
      fd_fork_counter_ele_t * ele = fd_fork_counter_map_ele_query( ctx->fork_counter_map, &parent_slot, NULL, ctx->fork_counter_elements );
      if (ele) {
        ele->count++;
        ctx->fork_counter_cnt++;
      } else {
        // Acquire a new element from the pool
        ele = fd_fork_counter_pool_ele_acquire( ctx->fork_counter_pool );
        if ( FD_LIKELY( ele ) ) {
          ele->parent_slot = parent_slot;
          ele->count = 1;
          fd_fork_counter_map_ele_insert( ctx->fork_counter_map, ele, ctx->fork_counter_elements );
        }
      }
    }
  }
...
after_credit( ctx_t *             ctx,
  ...
  //   FD_LOG_INFO(( "SIMULATE: too far ahead %lu published slot %lu, stopping FEC processing", ctx->replayed_slot, ctx->published_slot ));
  //   *charge_busy = 1;
  //   return;
  // }

  /* Read the FEC message */
  fd_feccap_fec_msg_t fec_msg;
  if( FD_UNLIKELY( fread( &fec_msg, sizeof(fd_feccap_fec_msg_t), 1, ctx->input_file ) != 1 ) ) {
    FD_LOG_ERR(( "Failed to read FEC message at index %lu", ctx->current_fec_idx ));
    *charge_busy = 1;
    return;
  }
    ...
    FD_LAYOUT_APPEND(
      FD_LAYOUT_APPEND(
        FD_LAYOUT_APPEND(
          ...
          fd_fork_counter_pool_align(), fd_fork_counter_pool_footprint( fork_counter_max ) ),
        fd_fork_counter_map_align(), fd_fork_counter_map_footprint( chain_cnt ) ),
      alignof(fd_fork_counter_ele_t), sizeof(fd_fork_counter_ele_t) * fork_counter_max ),
    scratch_align()
  );
}
...
unprivileged_init( fd_topo_t *      topo,
  ...
  fd_fork_counter_ele_t * fork_counter_elements = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_fork_counter_ele_t), sizeof(fd_fork_counter_ele_t) * fork_counter_max );

  ulong scratch_top = FD_SCRATCH_ALLOC_FINI( l, scratch_align() );
  FD_TEST( scratch_top == (ulong)scratch + scratch_footprint( tile ) );

  /* Initialize fork counter pool and map */
  ...
  /* Store reference to elements array for use with map operations */
  ctx->fork_counter_elements = fork_counter_elements;


  ctx->initialized = 0;

  /* Setup file path from configuration and open persistent file */
  fd_memcpy( ctx->file_path, tile->simulate.file_path, PATH_MAX );

  /* Open persistent file for reading using exact file path */
  ...
  ulong res = fread( magic_header, sizeof(magic_header), 1, ctx->input_file );
  FD_LOG_INFO(( "fread result: %lu", res ));
