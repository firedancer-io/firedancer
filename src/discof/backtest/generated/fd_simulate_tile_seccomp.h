...
#ifndef HEADER_fd_src_discof_backtest_generated_fd_simulate_tile_seccomp_h
#define HEADER_fd_src_discof_backtest_generated_fd_simulate_tile_seccomp_h
...
#endif
static const unsigned int sock_filter_policy_fd_simulate_tile_instr_cnt = 23;

static void populate_sock_filter_policy_fd_simulate_tile( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int simulate_fd ) {
  FD_TEST( out_cnt >= 23 );
  struct sock_filter filter[23] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_read, /* check_read */ 8, 0 ),
    /* allow fstat based on expression */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_fstat, /* check_fstat */ 9, 0 ),
...
//  check_read:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, simulate_fd, /* RET_ALLOW */ 7, /* RET_KILL_PROCESS */ 6 ),
//  check_fstat:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, simulate_fd, /* RET_ALLOW */ 5, /* RET_KILL_PROCESS */ 4 ),
//  check_lseek:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, simulate_fd, /* RET_ALLOW */ 3, /* RET_KILL_PROCESS */ 2 ),
//  check_fsync:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
