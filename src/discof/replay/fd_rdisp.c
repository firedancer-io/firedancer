...
/* To simplify the code, we make fd_rdisp_ptxn_t such that it can be cast
   as a fd_rdisp_txn_t * and mostly function.  This greatly simplifies
   the code but is technically UB.  The alias rules say that a write to
   a field of a pointer of type T1* and a write to a field of a pointer
   of type T2* should be independent.  To combat this, we need to be
   careful in each function to either use the ptxn_t * pointer or to
   immediately cast to txn_t * and only access it via that pointer. */
...
#define FOLLOW_EDGE(disp, x, w) (__extension__({                                                                            \
        ...
        uint __e = (x);                                                                                                     \
        edge_t * __edges = EDGE_IS_PTXN(__e) ? __disp->ptxn_pool[EDGE_SPECIAL_IDX(__e)].edges : __disp->pool[__e>>8].edges; \
        uint __w_cnt_1   = EDGE_IS_PTXN(__e) ? 0 : __disp->pool[__e>>8].edge_cnt_etc & 0x7FU;                               \
        uint __idx  = fd_uint_if( EDGE_IS_PTXN(__e), 0U, (__e & 0xFFU) );                                                   \
        (w) = (!EDGE_IS_PTXN(__e)) & (__idx<=__w_cnt_1);                                                                    \
        (void)(w);  /* not robust... */                                                                                     \
        __edges + __idx + 2*fd_int_max( 0, (int)(__idx)-(int)(__w_cnt_1)-1 ); }))
...
fd_rdisp_new( void * mem,
  ...
  for( ulong i=0UL; i<4UL; i++ ) {
    pending_prq_new( _pending, full_depth );
...
fd_rdisp_complete_txn( fd_rdisp_t * disp,
  ...
  fd_rdisp_txn_t * rtxn = (!ptxn) ? disp->pool + txn_idx :
                                    (fd_rdisp_txn_t *)fd_type_pun( (disp->ptxn_pool + EDGE_SPECIAL_IDX(txn_idx)) );
  fd_rdisp_blockinfo_t * block = NULL;

  if( FD_UNLIKELY( rtxn->in_degree==IN_DEGREE_UNSTAGED_DISPATCHED ) ) {
    /* Unstaged */
    if( FD_UNLIKELY( ptxn ) ) {
      fd_rdisp_ptxn_t  * ptxn_e    = disp->ptxn_pool + EDGE_SPECIAL_IDX(txn_idx); /* == rtxn */
      ...
      if( FD_UNLIKELY( EDGE_IS_LAST( e0 ) ) ) {
        ...
        if( FD_UNLIKELY( EDGE_IS_LAST( *child_edge ) ) ) {
          /* For example, either:
             ...
             to know if we have a pending LtHash pseudo-transaction to
...
fd_rdisp_add_pseudo_txn( fd_rdisp_t          *  disp,
  ...
  if( FD_LIKELY( block->staged ) ) {
    ...
    /* since we're kind of cheating with edge_cnt_etc, we need to fix it
       here.  Additionally, type punning ptxn like that is a bit
       fast-and-loose with the C spec, so add a compiler fence to make
       sure these writes can't be reordered. */
    FD_COMPILER_MFENCE();
