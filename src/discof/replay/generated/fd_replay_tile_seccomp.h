...
#endif
static const unsigned int sock_filter_policy_fd_replay_tile_instr_cnt = 25;

static void populate_sock_filter_policy_fd_replay_tile( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int accounts_fd ) {
  FD_TEST( out_cnt >= 25 );
  struct sock_filter filter[25] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_pwritev2, /* check_pwritev2 */ 9, 0 ),
    /* allow preadv2 based on expression */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_preadv2, /* check_preadv2 */ 10, 0 ),
