...
struct fd_sched {
  ...
  ulong               root_idx;
  fd_rdisp_t *        rdisp;
  ulong               active_block_idx; /* Index of the actively replayed block, or ULONG_MAX if no block is
...
fd_sched_new( void * mem, ulong block_cnt_max ) {
  ...
  sched->active_block_idx = ULONG_MAX;
...
fd_sched_fec_can_ingest( fd_sched_t * sched, fd_sched_fec_t * fec ) {
  FD_TEST( sched->canary==FD_SCHED_MAGIC );
  FD_TEST( fec->block_idx<sched->block_cnt_max );
  FD_TEST( fec->parent_block_idx<sched->block_cnt_max );
  ...
  fd_sched_block_t * block = sched->block_pool+fec->block_idx;
...
fd_sched_fec_ingest( fd_sched_t * sched, fd_sched_fec_t * fec ) {
  ...
  if( FD_UNLIKELY( fec->is_first_in_block ) ) {
    /* This is a new block. */
    add_block( sched, fec->block_idx, fec->parent_block_idx );
    block->slot        = fec->slot;
    ...
    int alloc_lane = 0;
    fd_sched_block_t * parent_block = sched->block_pool+fec->parent_block_idx;
    if( FD_LIKELY( parent_block->staged ) ) {
      ...
      if( FD_LIKELY( staging_lane!=FD_RDISP_UNSTAGED ) ) {
        ...
        fd_rdisp_add_block( sched->rdisp, fec->block_idx, staging_lane );
      ...
      if( block_is_stageable( parent_block ) ) {
        ...
        block->in_rdisp = 1;
        block->staged   = 0;
        fd_rdisp_add_block( sched->rdisp, fec->block_idx, FD_RDISP_UNSTAGED );
        sched->metrics->block_added_cnt++;
    ...
    if( FD_UNLIKELY( alloc_lane ) ) {
      ...
      if( FD_LIKELY( sched->staged_bitset!=fd_ulong_mask_lsb( FD_SCHED_MAX_STAGING_LANES ) ) ) { /* Optimize for lane available. */
        ...
        fd_rdisp_add_block( sched->rdisp, fec->block_idx, block->staging_lane );
        sched->metrics->block_added_cnt++;
        sched->metrics->block_added_staged_cnt++;
      } else {
        /* No lanes available. */
  ...
  if( FD_UNLIKELY( sched->active_block_idx==ULONG_MAX ) ) {
    try_activate_block( sched );
  } else {
    fd_sched_block_t * active_block = sched->block_pool+sched->active_block_idx;
    if( FD_UNLIKELY( !block_is_activatable( active_block ) ) ) {
      FD_LOG_CRIT(( "invariant violation: active_block_idx %lu is not activatable, txn_parsed_cnt %u, txn_done_cnt %u, fec_eos %u, dying %u, slot %lu, parent slot %lu",
                    sched->active_block_idx, active_block->txn_parsed_cnt, active_block->txn_done_cnt, (uint)active_block->fec_eos, (uint)active_block->dying, active_block->slot, active_block->parent_slot ));
    }
  }
...
fd_sched_txn_next_ready( fd_sched_t * sched, fd_sched_txn_ready_t * out_txn ) {
  ...
  ulong block_idx = sched->active_block_idx;
  ...
  if( FD_LIKELY( txn_queued_cnt>0 ) ) { /* Optimize for no fork switching. */
    out_txn->txn_idx          = fd_rdisp_get_next_ready( sched->rdisp, block_idx );
...
fd_sched_txn_done( fd_sched_t * sched, ulong txn_idx ) {
  ...
  ulong              block_idx = fd_ulong_if( txn_idx==FD_SCHED_TXN_IDX_BLOCK_START||txn_idx==FD_SCHED_TXN_IDX_BLOCK_END, sched->active_block_idx, sched->txn_to_block_idx[ txn_idx ] );
  fd_sched_block_t * block     = sched->block_pool+block_idx;
  ...
  if( FD_UNLIKELY( block->dying && block->txn_in_flight_cnt==0U ) ) {
    if( FD_UNLIKELY( sched->active_block_idx==block_idx ) ) {
      FD_LOG_CRIT(( "invariant violation: active block shouldn't be dying, block_idx %lu, slot %lu, parent slot %lu",
                    block_idx, block->slot, block->parent_slot ));
    }
  ...
  if( FD_UNLIKELY( !block->dying && sched->active_block_idx!=block_idx ) ) {
    ...
    fd_sched_block_t * active_block = sched->block_pool+sched->active_block_idx;
    FD_LOG_CRIT(( "invariant violation: sched->active_block_idx %lu, slot %lu, parent slot %lu, block_idx %lu, slot %lu, parent slot %lu",
                  sched->active_block_idx, active_block->slot, active_block->parent_slot,
                  block_idx, block->slot, block->parent_slot ));
  }
  ...
  if( FD_UNLIKELY( block_is_done( block ) ) ) {
    block->in_rdisp = 0;
    block->staged   = 0;
    fd_rdisp_remove_block( sched->rdisp, block_idx );
    ...
    sched->staged_head_block_idx[ block->staging_lane ] = ULONG_MAX;
...
void
fd_sched_block_abandon( fd_sched_t * sched, ulong block_idx ) {
  FD_TEST( sched->canary==FD_SCHED_MAGIC );
  ...
  if( FD_UNLIKELY( block_idx!=sched->active_block_idx ) ) {
    ...
    FD_LOG_CRIT(( "invariant violation: active_block_idx %lu, block_idx %lu, slot %lu, parent slot %lu",
                  sched->active_block_idx, block_idx, block->slot, block->parent_slot ));
  }
...
int
fd_sched_block_is_done( fd_sched_t * sched, ulong block_idx ) {
  FD_TEST( sched->canary==FD_SCHED_MAGIC );
  FD_TEST( block_idx<sched->block_cnt_max );
  return block_is_done( sched->block_pool+block_idx );
}

void
fd_sched_block_add_done( fd_sched_t * sched, ulong block_idx, ulong parent_block_idx ) {
  FD_TEST( sched->canary==FD_SCHED_MAGIC );
  FD_TEST( block_idx<sched->block_cnt_max );
  ...
  block->block_start_signaled = 1;
  block->block_end_signaled   = 1;
  if( FD_UNLIKELY( parent_block_idx==ULONG_MAX ) ) {
    /* Assumes that a NULL parent implies the snapshot slot. */
    sched->root_idx = block_idx;
    block->rooted = 1;
  }
}
...
fd_sched_root_advance( fd_sched_t * sched, ulong root_idx ) {
  ...
  fd_sched_block_t * new_root = sched->block_pool+root_idx;
  fd_sched_block_t * old_root = sched->block_pool+sched->root_idx;
  ...
  while( head ) {
    ...
    fd_sched_block_t * next = sched->block_pool+head->parent_idx;
    FD_TEST( next->in_sched );
    next->in_sched = 0;
    head = next;
  }
...
fd_sched_root_notify( fd_sched_t * sched, ulong root_idx ) {
  ...
  fd_sched_block_t * block = sched->block_pool+root_idx;
  fd_sched_block_t * old_root = sched->block_pool+sched->root_idx;
  if( FD_UNLIKELY( !old_root->rooted ) ) {
    FD_LOG_CRIT(( "invariant violation: old_root is not rooted, slot %lu, parent slot %lu",
                  old_root->slot, old_root->parent_slot ));
  ...
  while( curr ) {
    ...
    curr->rooted = 1;
    prev = curr;
    curr = sched->block_pool+curr->parent_idx;
  }
  ...
  if( sched->active_block_idx==ULONG_MAX ) {
    sched->metrics->deactivate_pruned_cnt += fd_uint_if( old_active_block_idx!=ULONG_MAX, 1U, 0U );
    try_activate_block( sched );
  }
}
...
fd_hash_t *
fd_sched_get_poh( fd_sched_t * sched, ulong block_idx ) {
  FD_TEST( sched->canary==FD_SCHED_MAGIC );
  FD_TEST( block_idx<sched->block_cnt_max );
  fd_sched_block_t * block = sched->block_pool+block_idx;
  return &block->poh;
...
uint
fd_sched_get_shred_cnt( fd_sched_t * sched, ulong block_idx ) {
  FD_TEST( sched->canary==FD_SCHED_MAGIC );
  FD_TEST( block_idx<sched->block_cnt_max );
  fd_sched_block_t * block = sched->block_pool+block_idx;
...
add_block( fd_sched_t * sched,
  ...
  /* node->parent link */
  fd_sched_block_t * parent_block = sched->block_pool+parent_block_idx;
  block->parent_idx = parent_block_idx;

  /* parent->node and sibling->node links */
  ulong child_idx = block_idx;
  if( FD_LIKELY( parent_block->child_idx==ULONG_MAX ) ) { /* Optimize for no forking. */
    parent_block->child_idx = child_idx;
  } else {
    fd_sched_block_t * curr_block = sched->block_pool+parent_block->child_idx;
    while( curr_block->sibling_idx!=ULONG_MAX ) {
      curr_block = sched->block_pool+curr_block->sibling_idx;
    }
...
try_activate_block( fd_sched_t * sched ) {
  ...
  while( staged_bitset ) {
    ...
    fd_sched_block_t * head_block   = sched->block_pool+head_idx;
    fd_sched_block_t * parent_block = sched->block_pool+head_block->parent_idx;
    if( FD_UNLIKELY( parent_block->dying ) ) {
      /* Invariant: no child of a dying block should be staged. */
      FD_LOG_CRIT(( "invariant violation: staged_head_block_idx %lu, slot %lu, parent slot %lu on lane %d has parent_block->dying set, slot %lu, parent slot %lu",
  ...
  if( FD_LIKELY( depth>0UL ) ) {
    ...
    if( FD_UNLIKELY( head_block_idx==ULONG_MAX ) ) {
      /* We found a promotable fork depth>0.  This should not happen. */
      FD_LOG_CRIT(( "invariant violation: head_block_idx==ULONG_MAX" ));
    }
...
subtree_abandon( fd_sched_t * sched, fd_sched_block_t * block ) {
  ...
  if( block->in_rdisp ) {
    fd_sched_block_t * parent = sched->block_pool+block->parent_idx;
...
find_and_stage_longest_unstaged_fork( fd_sched_t * sched, int lane_idx ) {
  ...
  if( FD_UNLIKELY( (depth>0UL && head_block_idx==ULONG_MAX) || (depth==0UL && head_block_idx!=ULONG_MAX) ) ) {
...
static ulong
compute_longest_unstaged_fork( fd_sched_t * sched, ulong block_idx ) {
  if( FD_UNLIKELY( block_idx==ULONG_MAX ) ) {
    FD_LOG_CRIT(( "invariant violation: block_idx==ULONG_MAX" ));
  }
...
stage_longest_unstaged_fork_helper( fd_sched_t * sched, ulong block_idx, int lane_idx ) {
  ...
  ulong rv       = fd_ulong_if( stage_it, block_idx, ULONG_MAX );
  if( FD_LIKELY( stage_it ) ) {
    block->staged = 1;
    block->staging_lane = (ulong)lane_idx;
    fd_rdisp_promote_block( sched->rdisp, block_idx, block->staging_lane );
  ...
  if( best_child_idx!=ULONG_MAX ) {
    ...
    rv = fd_ulong_if( rv!=ULONG_MAX, rv, head_block_idx );
  }
...
stage_longest_unstaged_fork( fd_sched_t * sched, ulong block_idx, int lane_idx ) {
  ...
  if( FD_LIKELY( head_block_idx!=ULONG_MAX ) ) {
