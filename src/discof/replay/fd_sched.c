...
ulong fd_sched_align( void ) { return alignof(fd_sched_t); }
...
fd_sched_parse( fd_sched_t * sched, fd_sched_block_t * block, fd_sched_alut_ctx_t * alut_ctx ) {
  while( 1 ) {
    ...
    if( block->txns_rem==0UL && block->mblks_rem>0UL ) {
      ...
      block->fec_buf_soff      += sizeof(fd_microblock_hdr_t);

      fd_memcpy( block->poh.hash, hdr->hash, sizeof(block->poh.hash) );
    ...
    if( block->txns_rem==0UL && block->mblks_rem==0UL && block->fec_sob ) {
      ...
      block->fec_buf_soff += sizeof(ulong);
...
fd_sched_parse_txn( fd_sched_t * sched, fd_sched_block_t * block, fd_sched_alut_ctx_t * alut_ctx ) {
  ...
  if( FD_UNLIKELY( !pay_sz || !txn_sz ) ) {
    ...
    FD_LOG_WARNING(("parse txn %lu, %lu txns remaining, %lu mblks remaining, fec_sob %d, fec_eob %d, pay_sz %lu, txn_sz %lu", (ulong)block->block_id.slot, block->txns_rem, block->mblks_rem, block->fec_sob, block->fec_eob, pay_sz, txn_sz ));
    return 0;
  }
  ...
  if( has_aluts ) {
    ...
    if( FD_LIKELY( slot_hashes_global ) ) {
      ...
      serializing = !!fd_runtime_load_txn_address_lookup_tables( txn, block->fec_buf+block->fec_buf_soff, alut_ctx->funk, alut_ctx->funk_txn, block->block_id.slot, slot_hash, block->aluts );
    } else {
      serializing = 1;
    }
    } FD_SPAD_FRAME_END;
