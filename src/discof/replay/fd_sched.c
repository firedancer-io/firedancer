...
#define FD_SCHED_MAX_DEPTH                 (FD_RDISP_MAX_DEPTH>>1)
...
struct fd_sched {
  ...
  ulong               active_block_idx; /* index of the actively replayed block, or null_idx if no block is
                                           actively replayed; has to be a non-empty block; staged blocks that
                                           are empty are not eligible for being active. */
...
block_is_done( fd_sched_block_t * block ) {
  return block->fec_eos && block->txn_done_cnt==block->txn_parsed_cnt;
}
...
block_is_activatable( fd_sched_block_t * block ) {
  return block_is_stageable( block ) && block->txn_parsed_cnt>block->txn_done_cnt && block->staged;
}
...
fd_sched_fec_can_ingest( fd_sched_t * sched, fd_sched_fec_t * fec ) {
  if( FD_UNLIKELY( fec->fec->data_sz>FD_SCHED_MAX_PAYLOAD_PER_FEC ) ) {
    FD_LOG_CRIT(( "invalid FEC set: fec->data_sz %lu, fec->cmr %s, slot %lu, prime %lu",
                  fec->fec->data_sz, FD_BASE58_ENC_32_ALLOCA( fec->fec->cmr.hash ), (ulong)fec->block_id.slot, (ulong)fec->block_id.prime ));
  }
...
fd_sched_fec_ingest( fd_sched_t * sched, fd_sched_fec_t * fec ) {
  ...
  if( FD_UNLIKELY( block->fec_eos ) ) {
    ...
    FD_LOG_CRIT(( "invariant violation: block->fec_eos set but getting more FEC sets fec->cmr %s, slot %lu, prime %lu",
                  FD_BASE58_ENC_32_ALLOCA( fec->fec->cmr.hash ), (ulong)fec->block_id.slot, (ulong)fec->block_id.prime ));
  }
  if( FD_UNLIKELY( block->fec_eob && fec->is_last_in_batch ) ) {
    ...
    FD_LOG_WARNING(( "invariant violation: block->fec_eob set but getting another FEC set that is last in batch fec->cmr %s, slot %lu, prime %lu",
                     FD_BASE58_ENC_32_ALLOCA( fec->fec->cmr.hash ), (ulong)fec->block_id.slot, (ulong)fec->block_id.prime ));
  ...
  if( FD_UNLIKELY( block->child_idx!=null_idx ) ) {
    ...
    FD_LOG_CRIT(( "invariant violation: block->child_idx %lu, fec->cmr %s, slot %lu, prime %lu",
                  block->child_idx, FD_BASE58_ENC_32_ALLOCA( fec->fec->cmr.hash ), (ulong)fec->block_id.slot, (ulong)fec->block_id.prime ));
  }
  ...
  if( FD_LIKELY( block->fec_buf_sz>block->fec_buf_soff ) ) {
    ...
    if( FD_UNLIKELY( block->fec_buf_sz-block->fec_buf_soff+fec->fec->data_sz>FD_SCHED_MAX_FEC_BUF_SZ ) ) {
      ...
      FD_LOG_WARNING(( "bad block: fec_buf_sz %u, fec_buf_soff %u, fec->data_sz %lu, fec->cmr %s, slot %lu, prime %lu",
                       block->fec_buf_sz, block->fec_buf_soff, fec->fec->data_sz, FD_BASE58_ENC_32_ALLOCA( fec->fec->cmr.hash ), (ulong)fec->block_id.slot, (ulong)fec->block_id.prime ));
      block->dying = 1;//FIXME inform replay/banks that it's dead.
      FD_LOG_WARNING(( "killing block2"));
      return 0;
    }
  ...
  /* This is an edge case where the last FEC set in a block has no
     transactions in it, and we've dispatched all transactions from the
     block before we saw the last empty FEC set. */
  uint txn_queued_cnt = block->txn_parsed_cnt-block->txn_in_flight_cnt-block->txn_done_cnt;
  if( FD_UNLIKELY( txn_queued_cnt==0 && fec->is_last_in_block ) ) {
    if( block_is_done( block ) ) {
      /* There isn't gonna be another txn_done() call to drive
         end-of-block state transitions.  So do that now. */
      block->in_rdisp = 0;
      block->staged   = 0;
      ...
      if( FD_UNLIKELY( block->child_idx!=null_idx ) ) {
        fd_sched_block_t * child = block_pool_ele( sched->block_pool, block->child_idx );
        FD_LOG_CRIT(( "invariant violation: block slot %lu, prime %lu still ingesting but already has child idx %lu, slot %lu, prime %lu",
                      (ulong)block->block_id.slot, (ulong)block->block_id.prime, block->child_idx, (ulong)child->block_id.slot, (ulong)child->block_id.prime ));
      }
      /* There isn't a child block down the same staging lane.  This is
         the last block in the staging lane.  Release the lane. */
    ...
    /* There isn't gonna be another successful next_ready() call to
       inform the caller of end-of-block.  So signal that now. */
    return 1;
  }
...
fd_sched_txn_next_ready( fd_sched_t * sched, fd_sched_txn_ready_t * out_txn ) {
  ...
  if( FD_LIKELY( txn_queued_cnt>0 ) ) { /* Optimize for no fork switching. */
    ...
    out_txn->is_last_in_block = 0;
    block->txn_in_flight_cnt++;
    txn_queued_cnt--;
    if( FD_UNLIKELY( block->fec_eos && txn_queued_cnt==0 ) ) {
      /* This is the last transaction in the block. */
      out_txn->is_last_in_block = 1;
    }
    return 1UL;
  }
...
fd_sched_txn_done( fd_sched_t * sched, ulong txn_id ) {
  ulong              null_idx  = block_pool_idx_null( sched->block_pool );
  ulong              block_idx = sched->txn_to_block_idx[ txn_id ];
  ...
  if( block_is_done( block ) ) {
    ...
    while( child_idx != null_idx ) {
      ...
      if( FD_LIKELY( child->staged && child->staging_lane==block->staging_lane ) ) {
        /* There is a child block down the same staging lane.  So switch
           the active block to it.  This is the common case. */
    ...
    sched->active_block_idx = null_idx;
    try_activate_block( sched );
  } else if( block->txn_parsed_cnt==block->txn_done_cnt ) {
