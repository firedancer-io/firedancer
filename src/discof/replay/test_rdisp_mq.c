...
FD_UNIT_TEST( basic ) {
  ...
  for( ulong outer=0UL; outer<20UL; outer++ ) {
    ...
    for( ulong inner=1UL; inner<10UL; inner++ ) {
      ...
      FD_TEST( fd_rdisp_mq_cnt( mq, 0UL )==inner );
    }
    for( ulong inner=1UL; inner<10UL; inner++ ) {
      ulong acct_idx = fd_ulong_if( outer+inner<=20UL, outer+inner, outer+inner-19UL );
...
FD_UNIT_TEST( full_heap ) {
  fd_rdisp_mq_t * mq = fd_rdisp_mq_join( fd_rdisp_mq_new( _footprint, 20UL, 10UL ) );
  for( ulong i=0UL; i<10UL; i++ ) fd_rdisp_mq_insert( mq, 1UL+i, 0UL, 0.0f );
  for( ulong i=0UL; i<10UL; i++ ) fd_rdisp_mq_adjust( mq, 1UL+i, 0UL, 1.0f );
  for( ulong i=0UL; i<10UL; i++ ) fd_rdisp_mq_adjust( mq, 1UL+i, 0UL, 0.0f );
  for( ulong i=0UL; i<10UL; i++ ) fd_rdisp_mq_adjust( mq, 1UL+i, 0UL, 0.0f );
  ulong popped = 0UL;
  for( ulong i=0UL; i<10UL; i++ ) popped |= 1UL<<fd_rdisp_mq_pop( mq, 0UL );
  FD_TEST( popped==fd_ulong_mask( 1, 10 ) );
}
