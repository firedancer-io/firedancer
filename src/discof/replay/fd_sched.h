...
#define FD_SCHED_TXN_ID_NULL        (ULONG_MAX)
#define FD_SCHED_TXN_ID_BLOCK_START (ULONG_MAX-1UL)
#define FD_SCHED_TXN_ID_BLOCK_END   (ULONG_MAX-2UL)
...
struct fd_sched_txn_ready {
  ...
  uint                block_end:1;   /* set for the final sentinel in the block; relevant because the
                                        caller might need to do end-of-block processing; special txn_id */
  uint                block_start:1; /* set for the first sentinel in the block; relevant because the
                                        caller might need to do start-of-block processing; special txn_id */
};
