...
struct fd_sched_fec {
  ulong            block_idx;           /* Index of the block.  Assumed to be in [0, block_cnt_max).  Caller
                                           is responsible for ensuring that block idx is in bounds and unique
                                           across equivocated blocks. */
  ulong            parent_block_idx;    /* Index of the parent block.  Assumed to be in [0, block_cnt_max).
                                           Caller is responsible for ensuring that block idx is in bounds and
