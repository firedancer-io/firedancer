...
metrics_write( fd_replay_tile_t * ctx ) {
  ...
  ulong store_fec_spill_read_failed= 0UL;
  ...
  if( FD_LIKELY( ctx->store ) ) {
    ...
    store_fec_spill_read_failed= FD_VOLATILE_CONST( ctx->store->fec_spill_read_fail_cnt );
  ...
  FD_MCNT_SET  ( REPLAY, STORE_FEC_SPILL_READ_FAILED, store_fec_spill_read_failed         );
...
# endif
  ...
  if( FD_UNLIKELY( fd_store_fec_data_view( ctx->store, ctx->store_disk_fd, store_fec, data_view ) ) ) {
    ctx->metrics.store_query_missing_cnt++;
    ctx->metrics.store_query_missing_mr = reasm_fec->key.ul[0];
    FD_BASE58_ENCODE_32_BYTES( reasm_fec->key.key, key_b58 );
...
privileged_init( fd_topo_t const *      topo,
  ...
  if( FD_LIKELY( store_obj_id!=ULONG_MAX ) ) {
    ...
    if( FD_UNLIKELY( ctx->store_disk_fd<0 ) ) {
      if( FD_UNLIKELY( store->cache_slot_cnt<store->fec_max ) ) {
        FD_LOG_ERR(( "open(%s) failed (%i-%s); bounded store payload cache requires spill reads",
                     store->db_path, errno, fd_io_strerror( errno ) ));
      }
...
populate_allowed_seccomp( fd_topo_t const *      topo,
  ...
  FD_SCRATCH_ALLOC_INIT( l, scratch );
  fd_replay_tile_t * ctx = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_replay_tile_t), sizeof(fd_replay_tile_t) );
  populate_sock_filter_policy_fd_replay_tile( out_cnt, out, (uint)fd_log_private_logfile_fd(), FD_ACCDB_FD_RW, (uint)ctx->store_disk_fd, FD_STAKE_DELEGATIONS_FD, FD_STAKE_REWARDS_FD, (uint)FD_TXNCACHE_FD, FD_ACCDB_IDX_FD_RW );
