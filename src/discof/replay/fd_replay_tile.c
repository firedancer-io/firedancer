...
/* compute_max_idle_window iterates leader slots from reset_slot
   forward across all available epoch schedules to find the maximum
   contiguous gap between consecutive leader slots (using the same
   distance convention as idle_gap: next_leader - prev_position).
   The terminal gap uses the current epoch end as a conservative
   upper bound.  Returns 0 if the leader schedule is unavailable. */
...
compute_max_idle_window( fd_multi_epoch_leaders_t const * mleaders,
  ...
  for(;;) {
    ...
    if( FD_UNLIKELY( next==ULONG_MAX || next>=epoch_end ) ) {
...
process_tower_slot_done( fd_replay_tile_t *           ctx,
  ...
  ulong prev_next_leader = ctx->next_leader_slot;
  ctx->next_leader_slot = fd_multi_epoch_leaders_get_next_slot( ctx->mleaders, min_leader_slot, ctx->identity_pubkey );
  /* Recompute max idle window only when we pass a leader slot, to avoid scanning the full epoch every slot. */
  if( FD_UNLIKELY( ctx->next_leader_slot!=prev_next_leader ) ) {
    ctx->max_idle_window_slots = compute_max_idle_window( ctx->mleaders, ctx->reset_slot, ctx->identity_pubkey );
  }
  if( FD_LIKELY( ctx->next_leader_slot != ULONG_MAX ) ) {
    double slot_duration_ticks = (double)bank->f.slot_params.ns_per_slot_adjusted*ctx->tick_per_ns;
