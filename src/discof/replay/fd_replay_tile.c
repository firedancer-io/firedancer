...
/* Polls the blockstore block info object for newly completed slices of
   executable slots on the frontier. Guaranteed good properties, like
   happiness, love, in order, executable immediately as long as the
   mcache wasn't overrun.  */
...
during_frag( fd_replay_tile_ctx_t * ctx,
  ...
  if( in_idx == PACK_IN_IDX ) {
    ...
    */
    ...
    FD_LOG_INFO(("reading seq from store_replay: %lu", seq ));
    return;
  }
...
prepare_new_block_execution( fd_replay_tile_ctx_t * ctx,
  ...
  child->end_idx = UINT_MAX;
  FD_LOG_WARNING(("prepare_new_block_execution Non-kosher new child fork, end_idx is %u", child->end_idx));
  // TODO: might reset the end_idx here instead of at the end of execution.
...
prepare_first_batch_execution( fd_replay_tile_ctx_t * ctx, fd_stem_context_t * stem ){
  ...
  if( fork == NULL ) {
    ...
    FD_LOG_WARNING(("Fork for slot %lu already exists. We don't make a new one.", curr_slot));
  }
...
after_frag( fd_replay_tile_ctx_t * ctx,
  ...
  if( FD_LIKELY( in_idx == REPAIR_IN_IDX || in_idx == STORE_IN_IDX ) ) {
    ...
    FD_LOG_WARNING(("REPAIR/STORE - slot: %lu, data_cnt: %u, parent_off: %u, slot_complete: %d", slot, data_cnt, parent_off, slot_complete));
...
unprivileged_init( fd_topo_t *      topo,
  ...
  *ctx->vote_authority = *ctx->validator_identity; /* FIXME */
  ...
  for( ulong i = 0UL; i < ctx->exec_cnt; i++ ) {
    ...
    FD_TEST( 0==strcmp( exec_out_link->name, "replay_exec" ) );
