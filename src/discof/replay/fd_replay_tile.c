...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  l = FD_LAYOUT_APPEND( l, alignof(fd_replay_tile_t),    sizeof(fd_replay_tile_t) * 2 );
...
publish_slot_completed( fd_replay_tile_t *  ctx,
  ...
  if( FD_UNLIKELY( !is_initial ) ) {
    parent_block_id = ctx->block_id_arr[ bank->data->parent_idx ].block_id;
  }
...
publish_slot_dead( fd_replay_tile_t *  ctx,
                   fd_stem_context_t * stem,
                   fd_bank_t *         bank ) {
  FD_TEST( ctx->block_id_arr[ bank->data->idx ].block_id_seen );
...
unprivileged_init( fd_topo_t *      topo,
  ...
  FD_SCRATCH_ALLOC_INIT( l, scratch );
  fd_replay_tile_t * ctx    = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_replay_tile_t),   sizeof(fd_replay_tile_t) * 2 );
