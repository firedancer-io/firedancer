...
on_snapshot_message( fd_replay_tile_ctx_t * ctx,
  ...
  switch( msg ) {
    ...
    case FD_SSMSG_MANIFEST_INCREMENTAL: {
      ...
      ctx->store->slot0 = fd_bank_slot_get( ctx->slot_ctx->bank );
      fd_rwlock_unwrite( &ctx->store->lock );
      break;
    }
    default: {
...
after_frag( fd_replay_tile_ctx_t *   ctx,
  ...
  if( FD_UNLIKELY( ctx->in_kind[ in_idx ]==IN_KIND_TOWER ) ) {
    ...
    FD_LOG_NOTICE(( "Rooting slot %lu with block_id %s", root, FD_BASE58_ENC_32_ALLOCA( block_id->block_id.uc ) ));
    ...
    /* If the internal slice buffer is full, there is nowhere for the
      ...
      FD_LOG_NOTICE(( "pushing slice %lu", slice->slot ));
...
handle_new_slice( fd_replay_tile_ctx_t * ctx, fd_stem_context_t * stem ) {
  ...
  FD_LOG_NOTICE(( "handling next slice %lu", slot ));
...
unprivileged_init( fd_topo_t *      topo,
  ...
  *ctx->vote_authority = *ctx->validator_identity; /* FIXME */
  ...
  FD_LOG_NOTICE(( "max_exec_slices %lu", max_exec_slices ));
