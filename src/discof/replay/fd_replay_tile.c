...
#include "fd_exec.h"
#include "../../discof/restore/stream/fd_frag_reader.h"
...
struct fd_replay_tile_ctx {
  ...
  fd_frag_reader_t * snapshot_messages_reader; /* Snapshot messages */
  uchar const *      manifest_dcache;          /* Dcache to receive decoded solana manifest */

  /* stem pointer
     TODO: get rid of this eventually.
     Replay should own its producers, not stem */
...
scratch_footprint( fd_topo_tile_t const * tile FD_PARAM_UNUSED ) {
  ...
  l = FD_LAYOUT_APPEND( l, fd_frag_reader_align(), fd_frag_reader_footprint() );
...
static void
fd_replay_restore_slot_ctx( fd_replay_tile_ctx_t * ctx,
  ...
  fd_solana_manifest_global_t * manifest_global
    = (fd_solana_manifest_global_t *)( ctx->manifest_dcache + frag->chunk );
...
static int
fd_replay_on_snapshot_message( void *                 _ctx,
  ...
  if( FD_UNLIKELY( fd_frag_meta_ctl_eom( frag->ctl) ) ) {
    ...
    /* Snapshot is being re-downloaded.  If replay has already
       rebuilt its bank off any snapshot manifest, it should verify that
       its bank is coherent with the incoming snapshot manifest
       messages. */
    return 1;
  } else if( FD_UNLIKELY( fd_frag_meta_ctl_som( frag->ctl ) ) ) {
    /* Snapshot loading encountered an error and is restarting.
       Replay should reset and rebuild its bank of the incoming snapshot
       messages. */
    return 1;
  }

  if( frag->sig == FD_FULL_SNAPSHOT_MANIFEST_MESSAGE_ID ) {
    ...
    return 1;
  } else if( frag->sig == FD_INCREMENTAL_SNAPSHOT_MANIFEST_MESSAGE_ID ) {
    FD_LOG_INFO(("Received incremental snapshot manifest message"));
    return 1;
  } else if( frag->sig == FD_FULL_SNAPSHOT_MANIFEST_EXTERNAL_MESSAGE_ID ) {
    /* TODO: currently we don't know if we will receive a full snapshot
       manifest message or both a full and incremental snapshot manifest
       message.  The snapshot tiles should communicate this over their
       snapshot messages. */
    /* Use the full snapshot manifest to initialize the slot context */
    fd_replay_restore_slot_ctx( ctx, frag );
    /* kick off repair orphans */
    kickoff_repair_orphans( ctx, ctx->stem );
    return 1;
  } else if( frag->sig == FD_INCREMENTAL_SNAPSHOT_MANIFEST_EXTERNAL_MESSAGE_ID ) {
    return 1;
  } else {
    FD_LOG_WARNING(("Received unknown snapshot message with sig %lu", frag->sig));
    return 0;
  }
...
after_credit( fd_replay_tile_ctx_t * ctx,
  ...
  if( !ctx->snapshot_init_done ) {
    ...
    if( strlen( ctx->genesis )>0 ) {
      ...
      /* Keep polling snapshot messages until snapshot is fully loaded. */
      fd_frag_reader_poll( ctx->snapshot_messages_reader,
                          fd_replay_on_snapshot_message,
                          ctx );

      if( ctx->snapshot_init_done ) {
        FD_LOG_WARNING(("Snapshot init starting, ctx->slot_ctx->slot is %lu", ctx->slot_ctx->slot));
...
unprivileged_init( fd_topo_t *      topo,
  ...
  void * snapshot_messages_reader_mem = FD_SCRATCH_ALLOC_APPEND( l, fd_frag_reader_align(), fd_frag_reader_footprint() );
  ...
  *ctx->vote_authority = *ctx->validator_identity; /* FIXME */
  ...
  /* set up snapshot messages reader */
  fd_frag_meta_t * mcache = topo->links[ tile->in_link_id[ SNAP_IN_IDX ] ].mcache;
  ulong *          fseq   = tile->in_link_fseq[ SNAP_IN_IDX ];
  ctx->snapshot_messages_reader = fd_frag_reader_new( snapshot_messages_reader_mem,
                                                       mcache,
                                                       fseq,
                                                       SNAP_IN_IDX );
