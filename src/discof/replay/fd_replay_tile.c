...
metrics_write( fd_replay_tile_t * ctx ) {
  ...
  FD_MGAUGE_SET( REPLAY, ALPENGLOW_VOTE_SLOT, ctx->voted_slot );
...
replay_reward_cert_voted( fd_replay_tile_t * ctx,
  ...
  *rank_out = replay_voter_rank( ctx, bank, fd_slot_to_epoch( &bank->f.epoch_schedule, bank->f.slot, NULL ) );
  ...
  if( FD_UNLIKELY( in_cert && ( ctx->voted_slot==ULONG_MAX || reward_slot>ctx->voted_slot ) ) ) {
    ctx->voted_slot = reward_slot;
  }
  return in_cert;
}
...
publish_slot_completed( fd_replay_tile_t *  ctx,
  ...
  slot_info->voted_slot = ctx->voted_slot;
...
privileged_init( fd_topo_t const *      topo,
  ...
  /* Only Alpenglow samples the vote account, so do not make a bad path a
     boot failure for Tower. */
