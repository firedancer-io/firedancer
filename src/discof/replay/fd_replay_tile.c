...
metrics_write( fd_replay_tile_t * ctx ) {
  /* Seqlock: odd seqno signals that a write is in progress.  Consumers
     ...
     gauges; a mismatch or odd value means the snapshot is torn. */
...
/* Agave defines a validator as delinquent when its last vote is more
   than 128 slots behind the current slot.
...
update_metric_delinquent_stake( fd_replay_tile_t * ctx,
  ...
  for( fd_vote_stakes_t_2_iter_t * iter = fd_vote_stakes_t_2_iter_init( vote_stakes, fork_id, iter_mem );
    ...
    uchar       is_valid       = 0;
    fd_vote_stakes_t_2_iter_ele( vote_stakes, fork_id, iter,
