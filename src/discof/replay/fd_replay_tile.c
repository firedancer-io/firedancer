...
fd_block_id_ele_query( fd_replay_tile_t * ctx, fd_hash_t * block_id, ulong slot ) {
  if( !ctx->alpenglow ) {
...
prepare_leader_bank( fd_replay_tile_t * ctx,
  ...
  if( FD_LIKELY( fd_block_id_ele_query( ctx, &block_id_ele->latest_mr, slot )==block_id_ele ) ) {
...
maybe_switch_identity( fd_replay_tile_t * ctx ) {
  ...
  if( FD_LIKELY( ctx->next_leader_slot != ULONG_MAX && ctx->is_booted ) ) {
    ...
    fd_block_id_ele_t * block_id_ele = fd_block_id_ele_query( ctx, &ctx->reset_block_id, ctx->reset_slot );
...
#define PROCESS_FEC_SKIP 3  /* Good to skip this FEC set */
...
can_process_rotor_fec( fd_replay_tile_t      * ctx,
  ...
  if( FD_UNLIKELY( fec->fec_set_idx==0 ) ) {
    ...
    if( FD_UNLIKELY( !parent ) ) {
      FD_LOG_INFO(( "parent bank not found for slot %lu fec set idx %u, parent slot %lu", fec->slot, fec->fec_set_idx, fec->parent_slot ));
      return PROCESS_FEC_DROP; // either pruned or bank evicted
  ...
  /* See can_process_fec. But:
     Rotor equivocating slots are guaranteed to be delivered starting
     from FEC 0, so even equivocations mid-slot are delivered from fec 0 */

  if( FD_UNLIKELY( !fd_banks_can_start_bank( ctx->banks ) ) ) {
    int is_new_block = fec->fec_set_idx==0U;
    int invalid_parent = !parent_fec_bank || parent_fec_bank->bank_seq!=parent->bank_seq;
    if( FD_UNLIKELY( is_new_block || invalid_parent ) ) {
      ctx->metrics.banks_full++;
      if( FD_UNLIKELY( fd_sched_is_drained( ctx->sched ) ) ) *evict_banks_out = 1;
      return PROCESS_FEC_WAIT;
    }
...
try_become_leader_ag( fd_replay_tile_t *        ctx,
  ...
  fd_block_id_ele_t * block_id_ele = fd_block_id_ele_query( ctx, (fd_hash_t *)parent_block_id, parent_slot );
  if( FD_UNLIKELY( !block_id_ele ) ) {
...
/* Essentially an inlined version of try_process_fec that reads off the
   ...
   state machine. Returns 0 if we want to look at this fec again, or 1
   if we are done processing. */
static int
process_rotor_fec( fd_replay_tile_t * ctx, fd_stem_context_t * stem, fd_rotor_replay_fec_t * fec ) {
  FD_BASE58_ENCODE_32_BYTES( fec->block_id.uc, block_id );
  FD_LOG_INFO(( "process fec: slot %lu, fec set idx %u, parent slot %lu. fec_complete %d, fec_block_id %s replay root is %lu/%lu", fec->slot, fec->fec_set_idx, fec->parent_slot, fec->slot_complete, block_id, ctx->consensus_root_slot, ctx->published_root_slot ));
  ...
  if( FD_UNLIKELY( fec->fec_set_idx==0 ) ) {
    ...
    if( FD_UNLIKELY( !parent ) ) {
      // pruned before delivery
      FD_LOG_DEBUG(( "parent block not found for slot %lu, fec set idx %u, parent slot %lu", fec->slot, fec->fec_set_idx, fec->parent_slot ));
      return 1;
    }
    parent_bank_idx = fd_block_id_ele_get_idx( ctx->block_id_arr, parent );
  } else {
    /* mid-slot FEC: rotor tile promises to populate block_id
       ...
       the last FEC set of a slot recieved through turbine though, the
    ...
    /* pruned mid-delivery, out-of-order, or duplicate */
    if( FD_UNLIKELY( !parent || parent->latest_fec_idx!=fec->fec_set_idx - FD_FEC_SHRED_CNT ) ) {
      FD_LOG_INFO(( "parent ref not found for slot %lu fec set idx %u, parent slot %lu. parent bank? %d, latest_fec_idx? %u", fec->slot, fec->fec_set_idx, fec->parent_slot, parent ? 1 : 0, parent ? parent->latest_fec_idx : UINT_MAX ));
      return 1;
    }
    parent_bank_idx = fd_block_id_ele_get_idx( ctx->block_id_arr, parent );
  }
  ...
  if( FD_UNLIKELY( !parent_bank ) ) {
    FD_LOG_DEBUG(( "parent bank pruned for bank idx %lu, slot %lu, fec set idx %u, parent slot %lu",
                    parent_bank_idx, fec->slot, fec->fec_set_idx, fec->parent_slot ));
    return 1;
  }
  ...
  if( FD_UNLIKELY( fec->slot_complete ) ) {
    ...
    /* block is complete and DMR block id is now known so re-key */
    FD_TEST( fd_ag_block_id_map_ele_remove( ctx->ag_block_id_map, &block_id_ele->block_info, NULL, ctx->block_id_arr )==block_id_ele );
    block_id_ele->latest_mr  = fec->block_id; /* set the latest mr to block_id - easier for tower/alpen querying */
    block_id_ele->block_info = ag_block_id( fec->slot, fec->block_id.uc );
    FD_TEST( fd_ag_block_id_map_ele_insert( ctx->ag_block_id_map, block_id_ele, ctx->block_id_arr ) );
  } else {
    block_id_ele->latest_mr = fec->mr;
  ...
  switch( ctx->in_kind[in_idx] ) {
    ...
    case IN_KIND_REPAIR: {
      ...
      if( FD_UNLIKELY( sig==REPAIR_SIG_FEC || sig==REPAIR_SIG_FEC_LEADER || sig==REPAIR_SIG_FEC_INVALID ) ) {
        ...
        /* 1 to keep frag for retry, returning 0 consumes it. */
        ...
        if( FD_LIKELY  ( res==PROCESS_FEC_OK ) )   return !process_rotor_fec( ctx, stem, fec );
        if( FD_UNLIKELY( res==PROCESS_FEC_DROP ) ) return  notify_rotor_fec( ctx, stem, fec );
        if( FD_UNLIKELY( res==PROCESS_FEC_SKIP ) ) return 0;

        /* PROCESS_FEC_WAIT: maybe queue an eviction, then retry the frag. */
        if( FD_UNLIKELY( evict_banks ) ) {
          ulong evictable_bank_idx = fd_banks_get_evictable_bank( ctx->banks, ctx->notified_root_bank );
          if( FD_UNLIKELY( evictable_bank_idx==ULONG_MAX ) ) {
            FD_LOG_DEBUG(( "replay has no banks to mark as prunable, it's possible that there is one bank already marked as prunable" ));
