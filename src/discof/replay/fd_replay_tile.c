...
/* Agave defines a validator as delinquent when its last vote is at
   ...
   T-2 is required: last_vote_slot and is_valid are only available
   for the T-2 vote-stake set (per-fork state in vacc_states).
...
vote_account_is_current( ulong cur_slot, ulong last_vote_slot ) {
  return last_vote_slot!=ULONG_MAX &&
         fd_int_if( cur_slot>=DELINQUENT_VALIDATOR_SLOT_DISTANCE,
