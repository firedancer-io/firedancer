...
init_after_snapshot( fd_replay_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( !snapshot_slot ) ) {
    ...
    FD_TEST( fd_runtime_block_execute_prepare( ctx->bank,
                                               ctx->funk,
                                               ctx->funk_txn,
                                               ctx->blockstore,
                                               ctx->runtime_spad ) == 0 );
...
init_from_snapshot( fd_replay_tile_ctx_t * ctx,
                    fd_stem_context_t *    stem ) {
  fd_features_restore( ctx->bank, ctx->funk, ctx->funk_txn, ctx->runtime_spad );
  fd_calculate_epoch_accounts_hash_values( ctx->bank );

  fd_slot_lthash_t const * lthash = fd_bank_lthash_query( ctx->bank );
  if( fd_lthash_is_zero( (fd_lthash_value_t * )lthash ) ) {
    ...
    fd_snapshot_hash( ctx->bank,
                      ctx->funk,
                      &accounts_hash,
                      ctx->runtime_spad,
                      &exec_para_ctx_snap,
                      &lthash_buf );
    fd_slot_lthash_t * lthash_val = fd_bank_lthash_modify( ctx->bank );
    fd_memcpy( (fd_lthash_value_t *)fd_type_pun(lthash_val->lthash), &lthash_buf, sizeof(lthash_buf) );
  }

  fd_runtime_update_leaders( ctx->bank, ctx->runtime_spad );

  fd_runtime_read_genesis( ctx->bank,
                           ctx->funk,
                           &ctx->funk_txn,
                           ctx->genesis,
                           0,
...
prepare_new_block_execution( fd_replay_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( fd_fork_frontier_ele_query(
      ctx->forks->frontier, &slot, NULL, ctx->forks->pool ) ) ) {
  ...
  FD_LOG_DEBUG(("TIMING: prepare_time - slot: %lu, elapsed: %6.6f ms", slot, (double)prepare_time_ns * 1e-6));
...
handle_slice( fd_replay_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( slot != ctx->bank->slot ) ) {
    ...
    prepare_first_batch_execution( ctx, stem, slot );
