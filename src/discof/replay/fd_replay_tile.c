...
maybe_switch_identity( fd_replay_tile_t * ctx ) {
  ...
  /* Resetting identity_vote_rooted above re-arms the leader gate.
     Suppress it only while WFS is still pending.  Once WFS completes,
     normal equivocation protection should resume. */
  ...
  if( FD_UNLIKELY( ctx->wait_for_vote_to_start_leader==FD_BOOLAU_AUTO &&
                   wfs_should_suppress_leader_gate( wfs_mode, ctx->wfs_complete ) ) ) {
    FD_LOG_NOTICE(( "auto-overriding wait_for_vote_to_start_leader to false "
                    "after identity rotation (WFS target slot %lu, boot slot %lu)",
                    ctx->wait_for_supermajority_at_slot, ctx->wfs_boot_slot ));
    ...
    if( FD_UNLIKELY( ctx->wfs_enabled ) ) {
      ...
      switch( wfs_mode ) {
        case FD_WFS_MODE_MATCH: {
          /* Extra check: snapin must have already verified this. */
