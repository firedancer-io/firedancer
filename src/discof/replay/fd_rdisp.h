...
/* fd_rdisp defines methods for building a DAG (directed acyclic graph)
   ...
   was created for it.  That's a little obtuse, but essentially creating
...
/* fd_rdisp_add_txn adds a transaction to the block with tag
   ...
   account address; otherwise bit i will be cleared.  If new_writable is
   NULL, the accounts in this transaction will be ignored for the
   purposes of LtHash tracking.  Obviously, if alts==NULL, accounts in
   alts will not be included in LtHash tracking.

   rdisp can track depth*FD_RDISP_MAX_ACCT_PER_TXN writable accounts,
   which is enough so it doesn't run out even if the whole depth is full
   with transactions that write to the maximum number of unique
   accounts.  However, it is possible to run out of state for tracking
   accounts by adding transactions, scheduling and completing them
   repeatedly.  If depth is less than
                        max CUs per block
                  -------------------------------,
                  300 * FD_RDISP_MAX_ACCT_PER_TXN

   the caller MUST create and schedule pseudo-transactions every depth
   transactions or depth*MAX_ACCT_PER_TXN writable accounts.  For
   example, at 75M CUs and MAX_ACCT_PER_TXN=128, care needs to be taken
   if depth is less than 2000.
...
/* fd_rdisp_complete_txn notifies the dispatcher that the specified
   ...
   index until a future invocation of fd_rdisp_complete_txn with the
   same txn_idx, which must have reclaim!=0.  This is useful when there
   is non-execution work to be done asynchronously for the transaction,
   but the caller would like to unblock the execution of transactions
   that depend on this one.  If txn_idx is an LtHash pseudo-transaction,
   this function will ignore the value of reclaim and behave like
   reclaim!=0. */
void
...
/* fd_rdisp_pseudo_txn_to_addr stores to out the account address
   ...
   txn_idx must have been returned from get_next_ready or
   fd_rdisp_add_pseudo_txn and must have FD_RDISP_LTHASH_PSEUDO_TXN set.
   Note that this means once you call complete_txn on a txn_idx, you
   cannot call this function on it any more.  disp must be a valid local
   join.  out must be non-NULL. */
void
...
/* fd_rdisp_add_pseudo_txn creates an LtHash addition pseudo-transaction
   ...
   staging lane.  You may also use fd_rdisp_add_all_pseudo_txn for this
   purpose.

   Note: If insert_block is unstaged, pseudo-transactions are always
   after normal transactions.  This behavior differs slightly from
   staged blocks (where the pseudo-transaction is inserted in a way that
   preserves serial fiction) and may change in the future.  This
   shouldn't cause problems, but it probably means that the best
   approach for unstaged blocks is to call add_all_pseudo_txn after the
   last transaction is received.  Anything else is trying to optimize
   for dispatching performance, but in a situation where that doesn't
   really apply.
