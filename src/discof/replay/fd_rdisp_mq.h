...
/* fd_rdisp_mq_{align,footprint} return the required alignment and
   ...
   max_writers_per_block in range [1, UINT_MAX-1].  If
   max_writers_per_block > acct_depth, the effective value used will be
   acct_depth instead. */
static inline ulong fd_rdisp_mq_align( void ) { return 8UL; }
static inline ulong fd_rdisp_mq_footprint( ulong acct_depth, ulong max_writers_per_block );
...
/* fd_rdisp_mq_new formats a region of memory with the required
   ...
   fd_rdisp_mq_join joins the caller to the multiprq, making it ready
   for use. */
static inline void *
...
static inline fd_rdisp_mq_t * fd_rdisp_mq_join( void * mem );
...
fd_rdisp_mq_footprint( ulong acct_depth,
                       ulong max_writers_per_block ) {
  /* The normal case is that max_writers_per_block < acct_depth, so this
     is normally a no-op. */
  max_writers_per_block = fd_ulong_min( max_writers_per_block, acct_depth );
...
static inline void * fd_rdisp_mq_leave ( fd_rdisp_mq_t * mq ) { return (void *)mq;  }
static inline void * fd_rdisp_mq_delete( void * mem )         { return (void *)mem; }
