...
handle_vinyl_lthash_request( fd_snaplv_t *             ctx,
  ...
  fd_stem_publish( stem, ctx->out_link[ OUT_LINK_LH ].idx, FD_SNAPSHOT_HASH_MSG_LTHASH_SUB_BATCH, o_link->chunk, data_sz, 0UL, 0UL, 0UL );
...
handle_data_frag( fd_snaplv_t *       ctx,
  ...
  if( sig!=FD_SNAPSHOT_HASH_MSG_LTHASH_SUB_BATCH ) {
  ...
  if( FD_UNLIKELY( batch_cnt>FD_SSPARSE_ACC_BATCH_MAX ) ) {
    FD_LOG_CRIT(( "batch count %lu exceeds FD_SSPARSE_ACC_BATCH_MAX %lu", batch_cnt, FD_SSPARSE_ACC_BATCH_MAX ));
  }
...
returnable_frag( fd_snaplv_t *       ctx,
  ...
  if( FD_LIKELY( sig==FD_SNAPSHOT_HASH_MSG_LTHASH_SUB_BATCH ) ) handle_data_frag( ctx, stem, sig, chunk, sz, tspub );
...
/* Maximum burst may contain one control message and one malformed
   message, in addition to VINYL_LTHASH_PENDING_MAX requests to be
   forwarded and FD_SSPARSE_ACC_BATCH_MAX incoming requests that may
   be directly sent by skipping the pending list. */
#define STEM_BURST (2UL+(VINYL_LTHASH_PENDING_MAX+FD_SSPARSE_ACC_BATCH_MAX))
