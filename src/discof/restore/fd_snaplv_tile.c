...
struct fd_snaplv_tile {
  ...
  ulong running_capitalization;
  ulong dup_capitalization;
  ...
  switch( sig ) {
    ...
    case FD_SNAPSHOT_MSG_CTRL_INIT_INCR: {
      ...
      if( sig==FD_SNAPSHOT_MSG_CTRL_INIT_FULL ) {
        ...
        ctx->running_capitalization = 0UL;
    ...
    case FD_SNAPSHOT_MSG_CTRL_DONE: {
      ...
      if( FD_UNLIKELY( ctx->running_capitalization!=ctx->manifest_capitalization ) ) {
        /* SnapshotError::MismatchedCapitalization
           TODO: use a tagged release when it becomes available
           https://github.com/anza-xyz/agave/blob/master/runtime/src/snapshot_bank_utils.rs#L217 */
        FD_LOG_WARNING(( "snapshot manifest capitalization %lu does not match computed capitalization %lu",
                         ctx->manifest_capitalization, ctx->running_capitalization ));
...
handle_hash_frag( fd_snaplv_t * ctx,
  ...
  switch( sig ) {
    case FD_SNAPSHOT_HASH_MSG_RESULT_ADD: {
      ...
      ctx->running_capitalization += result->capitalization;
    ...
    case FD_SNAPSHOT_HASH_MSG_RESULT_SUB: {
      ...
      ctx->dup_capitalization += result->capitalization;
      break;
    }
...
returnable_frag( fd_snaplv_t *       ctx,
  ...
  else if( FD_LIKELY( sig==FD_SNAPSHOT_MSG_EXP_CAPITALIZATION ) ) handle_expected_capitalization_message( ctx, chunk );
...
after_credit( fd_snaplv_t *        ctx,
  ...
  if( FD_UNLIKELY( ctx->hash_accum.awaiting_results && ctx->hash_accum.received_lthashes==ctx->num_hash_tiles ) ) {
    ...
    ctx->running_capitalization -= ctx->dup_capitalization;

    int test = memcmp( &ctx->hash_accum.expected_lthash, &ctx->hash_accum.calculated_lthash, sizeof(fd_lthash_value_t) );
    int capitalization_match = ctx->running_capitalization==ctx->manifest_capitalization;
