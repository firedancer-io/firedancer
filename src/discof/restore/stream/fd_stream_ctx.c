...
fd_stream_ctx_new( void *                 mem,
  ...
  ulong stream_in_cnt = 0UL;
  ulong frag_in_cnt   = 0UL;
  for( ulong i=0UL; i<in_cnt; i++ ) {
    if( FD_UNLIKELY( !tile->in_link_poll[ i ] ) ) continue;

    /* TODO: hack to get link type without adding to link definition.
       If we ever want to make stream links available outside of restore,
       we should add a stream link type to the link definition */
    if( strstr( topo->links[ tile->in_link_id[ i ] ].name, "snap" ) ) {
      stream_in_cnt++;
    } else {
      frag_in_cnt++;
    }
  ...
  self->stream_in      = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_stream_reader_t),   stream_in_cnt*sizeof(fd_stream_reader_t) );
  self->stream_in_ptrs = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_stream_reader_t *), stream_in_cnt*sizeof(fd_stream_reader_t *) );

  self->frag_in        = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_frag_reader_t),   frag_in_cnt*sizeof(fd_frag_reader_t) );
  self->frag_in_ptrs   = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_frag_reader_t *), frag_in_cnt*sizeof(fd_frag_reader_t *) );
  ...
  /* sanity check ins */
  FD_TEST( in_cnt == stream_in_cnt + frag_in_cnt );

  self->in_cnt        = in_cnt;
  self->stream_in_cnt = stream_in_cnt;
  self->frag_in_cnt   = frag_in_cnt;
  self->out_cnt       = out_cnt;

  self->event_map = fd_event_map_new( event_map_mem, in_cnt, stream_in_cnt, frag_in_cnt, out_cnt );
  self->in_seq = 0UL;

  /* init in */
  ulong in_idx        = 0UL;
  ulong stream_in_idx = 0UL;
  ulong frag_in_idx   = 0UL;
  for( ulong i=0UL; i<self->in_cnt; i++ ) {
    if( FD_UNLIKELY( !tile->in_link_poll[ i ] ) ) continue;

    /* TODO: hack to get link type without adding to link definition.
       If we ever want to make stream links available outside of restore,
       we should add a stream link type to the link definition... sadness */
    if( strstr( topo->links[ tile->in_link_id[ i ] ].name, "snap" ) ) {
      fd_stream_reader_init( &self->stream_in[ stream_in_idx ],
        fd_type_pun( topo->links[ tile->in_link_id[ i ] ].mcache ),
        tile->in_link_fseq[ i ],
        in_idx );
      stream_in_idx++;
    } else {
      fd_frag_reader_init( &self->frag_in[ frag_in_idx ],
        fd_type_pun( topo->links[ tile->in_link_id[ i ] ].mcache ),
        tile->in_link_fseq[ i ],
        in_idx );
      frag_in_idx++;
  ...
  /* init in_ptrs */
  for( ulong i=0UL; i<self->stream_in_cnt; i++ ) {
    self->stream_in_ptrs[ i ] = &self->stream_in[ i ];
  }

  for( ulong i=0UL; i<self->frag_in_cnt; i++ ) {
    self->frag_in_ptrs[ i ] = &self->frag_in[ i ];
  }
