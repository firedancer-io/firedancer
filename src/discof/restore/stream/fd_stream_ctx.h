...
struct fd_stream_ctx {
  fd_stream_reader_t *         stream_in;
  fd_stream_reader_t **        stream_in_ptrs;

  fd_frag_reader_t *           frag_in;
  fd_frag_reader_t **          frag_in_ptrs;
  ...
  fd_tile_on_frag_fn_t         tile_on_frag;
};
typedef struct fd_stream_ctx fd_stream_ctx_t;
...
fd_stream_ctx_init_run_loop( fd_stream_ctx_t *              ctx,
  ...
  if( ctx->stream_in_cnt && !tile_on_stream_frag ) {
    FD_LOG_ERR(( "tile_on_stream_frag function cannot be null if there are stream producers to this tile!" ));
  }

  if( ctx->frag_in_cnt && !tile_on_frag ) {
    FD_LOG_ERR(( "tile_on_frag function cannot be null if there are frag producers to this tile!" ));
  }
...
fd_stream_ctx_housekeeping_advance( fd_stream_ctx_t * ctx ) {
  ...
  fd_event_map_advance( ctx->event_map,
                        ctx->rng,
                        (void **)ctx->stream_in_ptrs,
                        ctx->stream_in_cnt,
                        (void **)ctx->frag_in_ptrs,
                        ctx->frag_in_cnt );
...
fd_stream_ctx_do_housekeeping( fd_stream_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( fd_stream_ticks_is_housekeeping_time( ctx->ticks ) ) ) {
    ...
    } else if( event_idx>ctx->out_cnt &&
               event_idx<=ctx->out_cnt + ctx->stream_in_cnt ) { /* send stream credits */

      ulong in_idx = event_idx - ctx->out_cnt - 1UL;
      ctx->tile_update_in( &ctx->stream_in[ in_idx ] );

    } else if( event_idx>ctx->out_cnt + ctx->stream_in_cnt ) { /* send frag credits */

      ulong in_idx = event_idx -ctx->out_cnt - ctx->stream_in_cnt - 1UL;
      fd_frag_reader_update_upstream( &ctx->frag_in[ in_idx ] );
...
static inline void
fd_stream_ctx_poll_stream( fd_stream_ctx_t * ctx,
                           void *            tile_ctx ) {
  if( FD_UNLIKELY( !ctx->stream_in_cnt ) ) {
  ...
  fd_stream_reader_t * this_in = &ctx->stream_in[ ctx->in_seq ];
  ctx->in_seq++;
...
static inline void
fd_stream_ctx_poll_frag( fd_stream_ctx_t * ctx,
                         void *            tile_ctx ) {
  if( FD_UNLIKELY( !ctx->frag_in_cnt ) ) {
    fd_stream_ctx_advance_poll_empty( ctx );
    return;
  }
  ...
  /* select input to poll */
  fd_frag_reader_t * this_in = &ctx->frag_in[ ctx->in_seq ];
  ctx->in_seq++;
  if( ctx->in_seq>=ctx->in_cnt ) {
    ctx->in_seq = 0UL; /* cmov */
  }
  ...
  fd_frag_reader_consume_ctx_t consume_ctx;
  long diff = fd_frag_reader_poll_frag( this_in,
                                          &consume_ctx );
  ...
  if( FD_UNLIKELY( diff<0L ) ) {
    ...
    fd_frag_reader_process_overrun( this_in,
                                      &consume_ctx,
                                      diff );
  }
  else if ( FD_UNLIKELY( diff ) ) {
    ...
    int consumed_frag = ctx->tile_on_frag( tile_ctx, this_in, frag );

    if( FD_LIKELY( consumed_frag ) ) {
      fd_frag_reader_consume_frag( this_in,
                                     &consume_ctx );
    }
...
fd_stream_ctx_run_loop( fd_stream_ctx_t * ctx,
  ...
  for(;;) {
    ...
    fd_stream_ctx_poll_stream( ctx, tile_ctx );
    fd_stream_ctx_poll_frag( ctx, tile_ctx );
  }
}
...
fd_stream_ctx_delete( fd_stream_ctx_t * ctx ) {
  for( ulong i=0UL; i<ctx->stream_in_cnt; i++ ) {
    fd_stream_reader_delete( &ctx->stream_in[ i ] );
    ctx->stream_in_ptrs[ i ] = NULL;
  }

  for( ulong i=0UL; i<ctx->frag_in_cnt; i++ ) {
    fd_frag_reader_delete( &ctx->frag_in[ i ] );
    ctx->frag_in_ptrs[ i ] = NULL;
  }
