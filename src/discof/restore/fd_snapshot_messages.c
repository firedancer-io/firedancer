...
fd_snapshot_manifest_init_epoch_stakes_elem( fd_snapshot_manifest_t * snapshot_manifest,
                                             ...
                                             ulong                    idx ) {
  /* TODO: Need to do all epoch stakes */
...
/* need to make sure the hash map is in order and put the epochs in the right order */
static void
fd_snapshot_manifest_init_epoch_stakes( fd_snapshot_manifest_t * snapshot_manifest,
  ...
  for( ulong i=0UL; i<solana_manifest->versioned_epoch_stakes_len; i++ ) {
    if( solana_manifest->versioned_epoch_stakes[i].epoch == epoch ) {
      fd_snapshot_manifest_init_epoch_stakes_elem( snapshot_manifest, solana_manifest, i );
    }
  }
}
...
fd_snapshot_manifest_init_from_solana_manifest( void *                 mem,
  ...
  /* TODO: need to verify if we even need epoch stakes for computing rewards since we seem to
     be getting it from funk anyways... might only be needed for leader schedule. */
