...
fd_snapshot_manifest_init_epoch_stakes_elem( fd_snapshot_manifest_t * snapshot_manifest,
                                             ...
                                             ulong                    idx ) {
  snapshot_manifest->epoch_stakes[idx].stakes_len = fd_stake_pair_t_map_size( solana_manifest->versioned_epoch_stakes[idx].val.inner.Current.stakes.stake_delegations_pool,
                                                                              solana_manifest->versioned_epoch_stakes[idx].val.inner.Current.stakes.stake_delegations_root );
  ...
  ulong i = 0UL;
  for ( fd_stake_pair_t_mapnode_t * n = fd_stake_pair_t_map_minimum(
    solana_manifest->versioned_epoch_stakes->val.inner.Current.stakes.stake_delegations_pool,
    solana_manifest->versioned_epoch_stakes->val.inner.Current.stakes.stake_delegations_root );
        n;
        n = fd_stake_pair_t_map_successor( solana_manifest->versioned_epoch_stakes->val.inner.Current.stakes.stake_delegations_pool, n ) ) {
...
fd_snapshot_manifest_init_epoch_stakes( fd_snapshot_manifest_t * snapshot_manifest,
  ...
  for( ulong i=0UL; i<solana_manifest->versioned_epoch_stakes_len; i++ ) {
    if( solana_manifest->versioned_epoch_stakes[i].epoch == epoch ) {
      fd_snapshot_manifest_init_epoch_stakes_elem( snapshot_manifest, solana_manifest, 0UL );
    }
    if( solana_manifest->versioned_epoch_stakes[i].epoch == epoch-1UL ) {
      fd_snapshot_manifest_init_epoch_stakes_elem( snapshot_manifest, solana_manifest, 1UL );
    }
    if( solana_manifest->versioned_epoch_stakes[i].epoch == epoch-2UL ) {
      fd_snapshot_manifest_init_epoch_stakes_elem( snapshot_manifest, solana_manifest, 2UL );
    }
  }
}
...
fd_snapshot_manifest_init_vote_accounts( fd_snapshot_manifest_t * snapshot_manifest,
  ...
  snapshot_manifest->vote_accounts_len 
    = fd_vote_accounts_pair_t_map_size( solana_manifest->bank.stakes.vote_accounts.vote_accounts_pool,
                                        solana_manifest->bank.stakes.vote_accounts.vote_accounts_root );
  ...
  for ( fd_vote_accounts_pair_t_mapnode_t * n = fd_vote_accounts_pair_t_map_minimum(
    solana_manifest->bank.stakes.vote_accounts.vote_accounts_pool,
    solana_manifest->bank.stakes.vote_accounts.vote_accounts_root );
        n;
        n = fd_vote_accounts_pair_t_map_successor( solana_manifest->bank.stakes.vote_accounts.vote_accounts_pool, n ) ) {
...
fd_snapshot_manifest_init_from_solana_manifest( void *                 mem,
  ...
  snapshot_manifest->has_accounts_lthash = !!solana_manifest->lthash;
  if( solana_manifest->lthash ) {
    fd_memcpy( snapshot_manifest->accounts_lthash,
               solana_manifest->lthash,
               sizeof( snapshot_manifest->accounts_lthash ) );
  }

  snapshot_manifest->has_epoch_account_hash = !!solana_manifest->epoch_account_hash;
  if( solana_manifest->epoch_account_hash ) {
    fd_memcpy( snapshot_manifest->epoch_account_hash,
               &solana_manifest->epoch_account_hash,
               sizeof( snapshot_manifest->epoch_account_hash ) );
  }

  snapshot_manifest->ticks_per_slot = solana_manifest->bank.ticks_per_slot;
  snapshot_manifest->has_hashes_per_tick = !!solana_manifest->bank.hashes_per_tick;
  if( solana_manifest->bank.hashes_per_tick ) {
    snapshot_manifest->hashes_per_tick = *solana_manifest->bank.hashes_per_tick;
  }
