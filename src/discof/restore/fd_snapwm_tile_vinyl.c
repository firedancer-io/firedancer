...
fd_snapwm_vinyl_update_admin( fd_snapwm_tile_t * ctx,
  ...
  for( ulong i=0UL; i<ctx->vinyl.wr_cnt; i++ ) {
    ...
    ulong curr = fd_vinyl_admin_ulong_query( &ctx->vinyl.admin->wr_seq[ i ] );
    ulong pres = ctx->vinyl.io_wd->seq_present;
    if( curr > pres ) FD_LOG_WARNING(( "regession in wr_seq[ %lu ] from %lu ... %lu", i, curr, pres ));
