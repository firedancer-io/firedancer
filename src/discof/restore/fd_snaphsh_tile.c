...
/* The snaphsh tile is a state machine that hashes accounts from an
   account input stream that it receives from the snapin tile.

   An account input stream starts with either a full or incremental
   SNAPSHOT_HASH_MSG message, which contains the full or incremental
   snapshot hash.  An account input stream ends with a
   SNAPSHOT_HASH_MSG_DONE message, indicating snaphsh should send
   its calculated hash to snapin with a SNAPSHOT_HASH_MSG_RESULT
   message. */

#define FD_SNAPHS_STATE_HASHING  (0)
#define FD_SNAPHS_STATE_DONE     (1)
#define FD_SNAPHS_STATE_SHUTDOWN (2)

/* Stolen from fd_runtime_public.h */
#define FD_ACC_SZ_MAX       (10UL<<20) /* 10MiB */

struct fd_snaphsh_tile {
  int state;
  ...
  ulong                 acc_data_sz;
  uchar                 acc_data[ FD_ACC_SZ_MAX ];

  struct {
    ulong full_accounts_hashed;
    ulong incremental_accounts_hashed;
  } metrics;
...
typedef struct fd_snaphsh_tile fd_snaphsh_tile_t;

static inline int
should_shutdown( fd_snaphsh_tile_t * ctx ) {
...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  l = FD_LAYOUT_APPEND( l, alignof(fd_snaphsh_tile_t), sizeof(fd_snaphsh_tile_t) );
  return FD_LAYOUT_FINI( l, alignof(fd_snaphsh_tile_t) );
}

static void
metrics_write( fd_snaphsh_tile_t * ctx ) {
  FD_MGAUGE_SET( SNAPHS, FULL_ACCOUNTS_HASHED, ctx->metrics.full_accounts_hashed );
  FD_MGAUGE_SET( SNAPHS, INCREMENTAL_ACCOUNTS_HASHED, ctx->metrics.incremental_accounts_hashed );
  FD_MGAUGE_SET( SNAPHS, STATE, (ulong)(ctx->state) );
}

static void
during_frag( fd_snaphsh_tile_t * ctx,
             ulong               in_idx,
             ulong               seq,
             ulong               sig,
             ulong               chunk,
  ...
  switch( sig ) {
    case FD_SNAPSHOT_HASH_MSG_SUB: {
      FD_LOG_WARNING(("chunk is %lu sz is %lu wmark is %lu", chunk, sz, ctx->in.wmark));
      FD_TEST( chunk>=ctx->in.chunk0 && chunk<=ctx->in.wmark && sz<=ctx->in.mtu );
      fd_lthash_value_t * prev_lthash = (fd_lthash_value_t *)fd_chunk_to_laddr_const( ctx->in.wksp, chunk );
      fd_lthash_sub( &ctx->running_lthash, prev_lthash );
      break;
    }
    case FD_SNAPSHOT_HASH_MSG_ACCOUNT_HDR: {
      FD_LOG_WARNING(("chunk is %lu sz is %lu", chunk, sz));
      FD_TEST( chunk>=ctx->in.chunk0 && chunk<=ctx->in.wmark && sz<=ctx->in.mtu );
    ...
    case FD_SNAPSHOT_HASH_MSG_ACCOUNT_DATA: {
      FD_LOG_WARNING(("chunk is %lu sz is %lu acc_data_sz is %lu ctx->account.data_len is %lu", chunk, sz, ctx->acc_data_sz, ctx->account.data_len));
      FD_TEST( chunk>=ctx->in.chunk0 && chunk<=ctx->in.wmark && sz<=ctx->in.mtu );
...
static void
after_frag( fd_snaphsh_tile_t *    ctx,
            ulong                  in_idx,
            ulong                  seq,
            ulong                  sig,
            ulong                  sz,
...
unprivileged_init( fd_topo_t *      topo,
  ...
  fd_snaphsh_tile_t * ctx = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_snaphsh_tile_t), sizeof(fd_snaphsh_tile_t) );
  ...
  ctx->metrics.full_accounts_hashed = 0UL;
  ctx->metrics.incremental_accounts_hashed = 0UL;
...
#define STEM_CALLBACK_CONTEXT_TYPE  fd_snaphsh_tile_t
#define STEM_CALLBACK_CONTEXT_ALIGN alignof(fd_snaphsh_tile_t)
