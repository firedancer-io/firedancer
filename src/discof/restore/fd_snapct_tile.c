...
init_load( fd_snapct_tile_t *  ctx,
  ...
  ulong ctl = (file && !(full ? ctx->local_in.full_snapshot_zstd : ctx->local_in.incremental_snapshot_zstd))
    ? FD_SNAPSHOT_CTL_RAW : FD_SNAPSHOT_CTL_ZSTD;
  fd_stem_publish( stem, ctx->out_ld.idx, sig, ctx->out_ld.chunk, sizeof(fd_ssctrl_init_t), ctl, 0UL, 0UL );
