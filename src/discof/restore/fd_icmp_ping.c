...
fd_icmp_send_ping( int                   sockfd,
  ...
  /* TODO: for some reason this identifier is not the same in the reply
     use the lower 16 bits of the ip addr as identifier */
  icmp_hdr.un.echo.id  = dest->addr & 0xFFFF;
  /* TODO: however the sequence number is the same in the reply */
...
fd_icmp_recv_ping_resp( int                   sockfd,
  ...
  FD_LOG_WARNING(("received ping response: type %d, code %d, id %u seq %u",
  ...
  // if( icmp_hdr.un.echo.id != (dest->addr & 0xFFFF) ) {
  //   FD_LOG_WARNING(( "Received ICMP packet with unexpected identifier: %u, expected: %u",
  //                icmp_hdr.un.echo.id, dest->addr & 0xFFFF ));
  //   return -1;
  // }
