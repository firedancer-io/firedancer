...
handle_data_frag( fd_snapdc_tile_t *  ctx,
  ...
  FD_TEST( chunk>=ctx->in.chunk0 && chunk<=ctx->in.wmark && sz<=ctx->in.mtu && sz>=ctx->data_pos );

  uchar const * data = fd_chunk_to_laddr_const( ctx->in.wksp, chunk );

  uchar const * in  = data+ctx->data_pos;
  ...
  ctx->data_pos += in_consumed;
  FD_TEST( ctx->data_pos<=sz );
  ...
  if( FD_UNLIKELY( !error ) ) {
    if( FD_UNLIKELY( ctx->data_pos!=sz ) ) {
      transition_malformed( ctx, stem );
      return 0;
    }
  ...
  int maybe_more_output = out_produced==USHORT_MAX || ctx->data_pos<sz;
  if( FD_LIKELY( !maybe_more_output ) ) ctx->data_pos = 0UL;
...
unprivileged_init( fd_topo_t *      topo,
  ...
  FD_TEST( writer_link->mtu==USHORT_MAX );
