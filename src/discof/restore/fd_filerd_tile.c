...
after_credit( fd_filerd_tile_t *      ctx,
  ...
  *charge_busy_after = 1;
  ...
  // FD_LOG_WARNING(("publishing %lu bytes into dcache!", frag_sz));
...
fd_filerd_run1(
  ...
  for(;;) {
      ...
      if( FD_LIKELY( event_idx<cons_cnt ) ) {
        ...
        if( FD_LIKELY( cr_byte_avail<cr_byte_max || cr_frag_avail<cr_frag_max ) ) {
          ...
          for( ulong cons_idx=0UL; cons_idx<cons_cnt; cons_idx++ ) {
            ...
            // FD_LOG_WARNING(("filerd: updating flow control credits: cr_byte_avail is %lu, cr_frag_avail: %lu", cr_byte_avail, cr_frag_avail));
          }
    ...
    if( FD_UNLIKELY( cr_byte_avail<burst_byte || cr_frag_avail<burst_frag ) ) {
      // FD_LOG_WARNING(("filerd: no more credits! cr_byte_avail is %lu cr_frag_avail is %lu", cr_byte_avail, cr_frag_avail));
