...
sync_ctx_init( fd_snapin_tile_t * ctx,
  ...
  FD_TEST( fd_snapin_shmem_footprint()<=sizeof(shmem_mem) );
...
test_cluster_new( ulong tile_cnt,
  ...
  cl->shmem_mem = aligned_alloc( fd_snapin_shmem_align(), fd_snapin_shmem_footprint() );
...
/* A worker holds data until tile 0 publishes the current attempt. */
static void
test_init_gate_holds_data( void ) {
  ...
  /* Keep a plausible fork ID in the stale attempt. */
  FD_TEST( cl->shmem->attempt.number==0UL );
  cl->shmem->attempt.fork_id = (ulong)USHORT_MAX;

  for( ulong lane=0UL; lane<cl->lane_cnt; lane++ ) send_control( ctx, lane, FD_SNAPSHOT_MSG_CTRL_INIT_FULL );
...
/* An interrupted tile 0 INIT must not strand other workers. */
static void
test_init_aborted_barrier_retries( void ) {
  ...
  for( ulong lane=0UL; lane<cl->lane_cnt; lane++ ) send_control( t1, lane, FD_SNAPSHOT_MSG_CTRL_INIT_FULL );
  FD_TEST( t1->attempt_number==1UL );
  FD_TEST( t1->waiting_for_tile0 );

  /* Interrupt tile 0 before its INIT barrier completes. */
  send_control( t0, 0UL, FD_SNAPSHOT_MSG_CTRL_INIT_FULL );
  FD_TEST( t0->attempt_number==1UL );
  FD_TEST( before_frag( t0, 0UL, 0UL, FD_SNAPSHOT_MSG_CTRL_ERROR )==0 );
  send_control( t0, 0UL, FD_SNAPSHOT_MSG_CTRL_ERROR );
  ...
  send_control( t1, 0UL, FD_SNAPSHOT_MSG_CTRL_ERROR );
  ...
  /* Tile 0 never initialized this attempt, so nothing rolls back. */
...
test_accumulator_fold( void ) {
  ...
  for( ulong t=0UL; t<n; t++ ) {
    fd_snapin_tile_t * ctx = &cl->ctx[ t ];
    ctx->worker.loaded             = 100UL+t;
    ctx->worker.duplicates         =  11UL+2UL*t;
    ctx->worker.input_lamports     = 1000000UL*(t+1UL);
    ctx->worker.duplicate_lamports =    5700UL*(t+1UL);

    exp_loaded             += ctx->worker.loaded;
    exp_duplicates         += ctx->worker.duplicates;
    exp_input              += ctx->worker.input_lamports;
    exp_duplicate_lamports += ctx->worker.duplicate_lamports;
  }

  cluster_barrier( cl, FD_SNAPSHOT_MSG_CTRL_FINI );

  FD_TEST( cl->shmem->totals.loaded             ==exp_loaded             );
  FD_TEST( cl->shmem->totals.duplicates         ==exp_duplicates         );
  FD_TEST( cl->shmem->totals.input_lamports     ==exp_input              );
  FD_TEST( cl->shmem->totals.duplicate_lamports ==exp_duplicate_lamports );
...
test_writer_flush( void ) {
  ...
  FD_TEST( !writer_append_batch( ctx, &batch ) );
  FD_TEST( ctx->writer.record_cnt==1UL );
  FD_TEST( !ctx->metrics.accounts_loaded );
  FD_TEST( ctx->metrics.total_accounts_processed==1UL );
  FD_TEST( ctx->metrics.total_account_batches_processed==1UL );
  FD_TEST( !writer_flush( ctx ) );
  ...
  FD_TEST( !ctx->writer.buf_used && !ctx->writer.record_cnt );
...
static void
test_writer_record_capacity_flush( void ) {
  fd_snapin_tile_t * ctx = test_ctx;
  sync_ctx_init( ctx, 1UL, FD_SNAPSHOT_STATE_IDLE );
  test_io_reset();

  FD_TEST( !(FD_SNAPIN_WRITE_RECORD_MAX%FD_SSPARSE_ACC_BATCH_MAX) );
  for( ulong i=0UL; i<FD_SNAPIN_WRITE_RECORD_MAX; i++ ) {
    ctx->writer.records[ i ].batch_cnt = (ushort)( i%FD_SSPARSE_ACC_BATCH_MAX ? 0UL : FD_SSPARSE_ACC_BATCH_MAX );
  }
  ctx->writer.record_cnt = FD_SNAPIN_WRITE_RECORD_MAX;
  ctx->writer.buf_used   = FD_SNAPIN_WRITE_RECORD_MAX*sizeof(fd_accdb_disk_meta_t);
  fd_memset( ctx->writer.buf, 0, ctx->writer.buf_used );
  ...
  FD_TEST( test_pwrite_call_cnt==1UL );
  FD_TEST( ctx->writer.record_cnt==1UL );
  FD_TEST( ctx->writer.buf_used==sizeof(fd_accdb_disk_meta_t) );
}
...
main( int     argc,
  ...
  test_writer_record_capacity_flush();
