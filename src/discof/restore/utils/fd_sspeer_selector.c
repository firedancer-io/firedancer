...
#define DEFAULT_SLOTS_BEHIND   (1000UL*1000UL)        /* 100_000 slots behind */
...
fd_sspeer_selector_best( fd_sspeer_selector_t * selector,
  ...
  for( score_treap_fwd_iter_t iter = score_treap_fwd_iter_init( selector->score_treap, selector->pool );
       !score_treap_fwd_iter_done( iter ); iter = score_treap_fwd_iter_next( iter, selector->pool ) ) {
...
fd_sspeer_selector_process_cluster_slot( fd_sspeer_selector_t * selector,
  ...
  /* merge score treap with shadow treap */
  score_treap_merge( selector->score_treap, selector->shadow_score_treap, selector->pool );

#ifdef FD_SSPEER_SELECTOR_USE_HANDHOLDING
