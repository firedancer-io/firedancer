...
fd_sspeer_selector_footprint( ulong max_peers ) {
  ...
  l = FD_LAYOUT_APPEND( l, score_treap_align(),           score_treap_footprint( max_peers ) );
  l = FD_LAYOUT_APPEND( l, alignof(ulong),                2UL * max_peers * sizeof(ulong) ); /* 2UL={full_slots,incr_slots} each max_peers */
...
fd_sspeer_selector_new( void * shmem,
  ...
  void * _peer_idx_list           = FD_SCRATCH_ALLOC_APPEND( l, alignof(ulong),           2UL * max_peers * sizeof(ulong) ); /* 2UL={full_slots,incr_slots} each max_peers */
...
fd_sspeer_selector_process_cluster_slot( fd_sspeer_selector_t * selector ) {
  ...
  /* Collect full_slot and incr_slot values in a single treap pass.
     full_slots occupies peer_idx_list[0..max_peers), incr_slots
     occupies peer_idx_list[max_peers..2*max_peers). */
  ulong full_cnt = 0UL;
  ulong incr_cnt = 0UL;
  ulong * full_slots = selector->peer_idx_list;
  ulong * incr_slots = selector->peer_idx_list + selector->max_peers;
  for( score_treap_fwd_iter_t iter = score_treap_fwd_iter_init( selector->score_treap, selector->pool );
       !score_treap_fwd_iter_done( iter );
       iter = score_treap_fwd_iter_next( iter, selector->pool ) ) {
    fd_sspeer_private_t const * peer = score_treap_fwd_iter_ele_const( iter, selector->pool );
    if( peer->full_slot!=FD_SSPEER_SLOT_UNKNOWN ) {
      full_slots[ full_cnt++ ] = peer->full_slot;
    }
    if( peer->incr_slot!=FD_SSPEER_SLOT_UNKNOWN ) {
      incr_slots[ incr_cnt++ ] = peer->incr_slot;
    }
  }
  ...
  /* Lower median is more conservative against a single malicious peer
     in small sets (e.g. 2 peers: picks the lower slot). */
  ulong mid_full = (full_cnt-1UL)/2UL; /* full_cnt > 0 */
  fd_sort_up_ulong_select( full_slots, full_cnt, mid_full );
  ulong median_full = full_slots[ mid_full ];

  ulong median_incr = FD_SSPEER_SLOT_UNKNOWN;
  if( FD_LIKELY( incr_cnt>0UL ) ) {
    ulong mid_incr = (incr_cnt-1UL)/2UL;
    fd_sort_up_ulong_select( incr_slots, incr_cnt, mid_incr );
    median_incr = incr_slots[ mid_incr ];
  }

  /* The full and incremental medians are computed from different peer
     subsets, so median_incr < median_full is possible.  Clamp to
     maintain the invariant that incremental >= full. */
  if( FD_UNLIKELY( median_incr!=FD_SSPEER_SLOT_UNKNOWN &&
                   median_full!=FD_SSPEER_SLOT_UNKNOWN &&
                   median_incr<median_full ) ) {
    median_incr = median_full;
  }

  /* If neither median changed, no rescore needed. */
  if( FD_LIKELY( median_full==selector->cluster_slot.full &&
                 median_incr==selector->cluster_slot.incremental ) ) return;

  selector->cluster_slot.full        = median_full;
  selector->cluster_slot.incremental = median_incr;
