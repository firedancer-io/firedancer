...
fd_ssload_recover_bank( fd_snapshot_manifest_t * manifest,
  ...
  bank->f.total_epoch_stake = manifest->epoch_stakes[2].total_stake;

  bank->txncache_fork_id = (fd_txncache_fork_id_t){ .val = manifest->txncache_fork_id };
}

void
fd_ssload_recover_epoch_stakes_init( fd_bank_t *          bank,
...
void
fd_ssload_recover_epoch_stakes_t_1_entry( fd_snapshot_manifest_vote_stakes_t const * entry,
                                          ulong                                      idx,
                                          ulong                                      epoch,
                                          fd_bank_t *                                bank,
                                          fd_runtime_stack_t *                       runtime_stack ) {
  ...
  vote_ele->commission = (uchar)entry->commission;
  ...
  if( FD_FEATURE_ACTIVE_BANK( bank, validator_admission_ticket ) ) {
    /* FIXME: at this point features aren't loaded in yet. */
...
void
fd_ssload_recover_epoch_stakes_t_2( fd_snapshot_epoch_stakes_slim_t const * entries,
                                    ulong                                   len,
                                    fd_bank_t *                             bank,
                                    fd_runtime_stack_t *                    runtime_stack ) {
  fd_vote_stakes_t * vote_stakes = fd_bank_vote_stakes( bank );
  ...
  for( ulong i=0UL; i<len; i++ ) {
    ...
    if( FD_LIKELY( vote_ele ) ) vote_ele->commission_delayed = (uchar)elem->commission;
...
void
fd_ssload_recover_epoch_stakes_t_3( fd_snapshot_epoch_stakes_slim_t const * entries,
                                    ulong                                   len,
                                    fd_bank_t *                             bank,
                                    fd_runtime_stack_t *                    runtime_stack ) {
  (void)bank;
