...
test_boundaries( void ) {
  /* Exactly at S is MATCH; one slot either side flips mode. */
  FD_TEST( fd_wfs_mode( S, H_SET, V, S-1UL )==FD_WFS_MODE_ERROR );
  FD_TEST( fd_wfs_mode( S, H_SET, V, S     )==FD_WFS_MODE_MATCH );
  FD_TEST( fd_wfs_mode( S, H_SET, V, S+1UL )==FD_WFS_MODE_NOOP  );

  /* boot_slot is the effective slot after the last snapshot is loaded,
     not necessarily the full base slot.  A full at 90 + incremental
     at 100 boots at effective slot 100, which matches S=100 -> MATCH
     (not ERROR, as it would be if the base slot 90 were used). */
  FD_TEST( fd_wfs_mode( S, H_SET, V, 100UL )==FD_WFS_MODE_MATCH );
