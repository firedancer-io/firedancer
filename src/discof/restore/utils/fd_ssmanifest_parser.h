...
/* Returns the epoch derived from the snapshot slot and epoch schedule.
   Available after state_validate has run (i.e. before any vote_stakes
   entry is signalled). */

ulong
fd_ssmanifest_parser_epoch( fd_ssmanifest_parser_t const * parser );

/* Returns the leader schedule epoch derived from the snapshot slot and
   epoch schedule.  Available after state_validate has run. */
