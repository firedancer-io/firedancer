...
struct fd_sshead_private {
  ...
  long             deadline;
  struct pollfd    pfd;
  char             hostname[ 16 ]; /* "xxx.xxx.xxx.xxx" + NUL */
  ulong            magic;
};
...
fd_sshead_start( fd_sshead_t * head,
  ...
  FD_TEST( fd_cstr_printf_check( head->hostname, sizeof(head->hostname), NULL,
           FD_IP4_ADDR_FMT, FD_IP4_ADDR_FMT_ARGS( addr.addr ) ) );
  fd_ssresolve_init( head->ssresolve, addr, sockfd, full, head->hostname );
