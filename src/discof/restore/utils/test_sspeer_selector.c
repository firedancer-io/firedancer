...
generate_rand_url( char *     url,
                   ulong      max_sz,
                   fd_rng_t * rng ) {
  ulong max_len = 5UL/*https*/ + 21UL/*fake_url*/ + 6UL/*:upd_port*/ + 1UL/*\0*/;
...
test_update_on_ping( fd_sspeer_selector_t * selector,
  ...
  fd_sspeer_selector_update_on_ping( selector, addr_AB, 5L*1000L*1000L );
  best = fd_sspeer_selector_best( selector, 0, ULONG_MAX);
  ...
  fd_sspeer_selector_update_on_ping( selector, addr_CD, 6L*1000L*1000L );
  best = fd_sspeer_selector_best( selector, 0, ULONG_MAX);
