...
test_basic_peer_selection( fd_sspeer_selector_t * selector,
  ...
  /* Add a peer and compute the cluster slot from the median of its
     slots.  With 1 peer at (1000, 1500), the median is (1000, 1500). */
...
test_add_clears_incremental( fd_sspeer_selector_t * selector,
  ...
  /* Establish the cluster slot via median computation. */
  fd_sspeer_selector_process_cluster_slot( selector );
...
test_on_resolve_clears_incremental( fd_sspeer_selector_t * selector,
  ...
  /* Establish the cluster slot via median. */
  fd_sspeer_selector_process_cluster_slot( selector );
...
test_cluster_slot_rescoring( fd_sspeer_selector_t * selector,
  ...
  /* Compute lower median.  Full slots: [9000, 9000] -> mid=0 -> 9000.
     Incr slots: sorted [9200, 9500] -> mid=0 -> 9200.
     Cluster = (9000, 9200). */
  fd_sspeer_selector_process_cluster_slot( selector );
  fd_sscluster_slot_t cs = fd_sspeer_selector_cluster_slot( selector );
  FD_TEST( cs.full==9000UL );
  FD_TEST( cs.incremental==9200UL );

  /* After rescore:
     Peer A: behind = max(0, 9200-9500) = 0. score = 2_000_000.
     Peer B: behind = max(0, 9200-9200) = 0. score = 3_000_000. */
  fd_sspeer_t best = fd_sspeer_selector_best( selector, 0, FD_SSPEER_SLOT_UNKNOWN );
  FD_TEST( best.addr.l==addr_A.l );
  FD_TEST( best.score==2UL*1000UL*1000UL );

  /* Add a third peer to shift the median. */
  FD_TEST( add_peer( selector, key_C, addr_C, 9000UL, 9800UL, 4UL*1000UL*1000UL )!=FD_SSPEER_SCORE_INVALID );

  /* Recompute lower median.  Incr slots: sorted [9200, 9500, 9800].
     mid=1 -> 9500.  Cluster = (9000, 9500). */
  fd_sspeer_selector_process_cluster_slot( selector );
  cs = fd_sspeer_selector_cluster_slot( selector );
  FD_TEST( cs.full==9000UL );
  FD_TEST( cs.incremental==9500UL );

  /* Remove peer A (the one at 9500) to shift the median.
     Remaining incr slots: sorted [9200, 9800]. mid=0 -> 9200.
     Cluster = (9000, 9200). */
  fd_sspeer_selector_remove( selector, key_A );
  fd_sspeer_selector_process_cluster_slot( selector );
  cs = fd_sspeer_selector_cluster_slot( selector );
  ...
  /* After rescore:
     Peer B: behind = max(0, 9200-9200) = 0.   score = 3_000_000.
     Peer C: behind = max(0, 9200-9800) = 0.   score = 4_000_000. */
  best = fd_sspeer_selector_best( selector, 0, FD_SSPEER_SLOT_UNKNOWN );
  FD_TEST( best.addr.l==addr_B.l );
  FD_TEST( best.score==3UL*1000UL*1000UL );
...
test_cluster_slot_regression( fd_sspeer_selector_t * selector,
  ...
  /* Verify that removing high-slot peers causes the median-based
  ...
  /* Add 5 peers: 3 at full=1000, 2 at full=500.
     Full slots sorted: [500, 500, 1000, 1000, 1000].
     Lower median = [(5-1)/2] = [2] = 1000. */
  ...
  /* Remove 2 of the 3 high-slot peers.  Remaining:
     ...
     Full slots sorted: [500, 500, 1000].
     Lower median = [(3-1)/2] = [1] = 500.
     Incr slots sorted: [800, 800, 1500].
     Lower median = [1] = 800.
  ...
  /* After rescore with cluster=(500, 800):
     Peer 2 (full=1000, incr=1500): ahead of cluster, behind=0.
       score = 3_000_000.
     Peer 3 (full=500, incr=800): at cluster, behind=0.
       score = 4_000_000.
     Peer 4 (full=500, incr=800): at cluster, behind=0.
       score = 5_000_000.
  ...
  /* Remove the last high-slot peer.  Remaining:
     ...
     Lower median = [(2-1)/2] = [0] = 500.
     Cluster stays at (500, 800). */
...
test_cluster_slot_recovery_after_poison( fd_sspeer_selector_t * selector,
  ...
  /* Step 1: Malicious is the first and only peer.
     Median of 1 = the malicious's slot.  Cluster slot is poisoned. */
  ...
  /* Step 4: process_cluster_slot recomputes the median from the
...
test_slot_zero( fd_sspeer_selector_t * selector,
  ...
  /* Compute cluster slot from median.  cluster = (0, 0). */
  fd_sspeer_selector_process_cluster_slot( selector );
  fd_sscluster_slot_t cs = fd_sspeer_selector_cluster_slot( selector );
  FD_TEST( cs.full==0UL );
...
test_stress_peer_count( fd_sspeer_selector_t * selector,
  ...
  /* Compute the cluster slot from the median of remaining peers. */
...
test_genesis_cluster_slot( fd_sspeer_selector_t * selector,
  ...
  /* Add peers to establish cluster = (0, 0) via median. */
  fd_sspeer_key_t key_A[1]; FD_TEST( generate_rand_sspeer_key( key_A, rng, fd_rng_uint( rng )&0x1 ) );
  fd_sspeer_key_t key_B[1]; FD_TEST( generate_rand_sspeer_key( key_B, rng, fd_rng_uint( rng )&0x1 ) );
  fd_sspeer_key_t key_C[1]; FD_TEST( generate_rand_sspeer_key( key_C, rng, fd_rng_uint( rng )&0x1 ) );
  fd_ip4_port_t addr_A; FD_TEST( generate_rand_addr_non_zero( &addr_A, rng ) );
  ...
  /* Add peer C at (0, 5).
     Incr slots: [0, 5] (peer B has UNKNOWN).  Lower median = [0] = 0.
     Cluster = (0, 0).  No change, no rescore. */
...
test_invalid_clear_and_best_sentinel( fd_sspeer_selector_t * selector,
  ...
  /* Establish cluster slot via median. */
  fd_sspeer_selector_process_cluster_slot( selector );
...
static void
test_median_outlier_robustness( fd_sspeer_selector_t * selector,
                                fd_rng_t *             rng ) {
  FD_LOG_NOTICE(( "testing median outlier robustness" ));
  ...
  /* Compute median.  6 peers total.
     Full slots sorted: [300, 300, 300, 300, 300, 1000].
     median = [(6-1)/2] = [2] = 300.
     Same for incremental.
     The single outlier cannot shift the median. */
  fd_sspeer_selector_process_cluster_slot( selector );
  fd_sscluster_slot_t cs = fd_sspeer_selector_cluster_slot( selector );
  FD_TEST( cs.full==300UL );
  FD_TEST( cs.incremental==300UL );

  /* Verify that the malicious slot did NOT become the cluster slot. */
  FD_TEST( cs.full!=1000UL );
  FD_TEST( cs.incremental!=1000UL );

  /* Best should be honest peer 0 (lat=1ms, at cluster slot, score=1M). */
  fd_sspeer_t best = fd_sspeer_selector_best( selector, 0, FD_SSPEER_SLOT_UNKNOWN );
  ...
  /* ... Two malicious: 5 honest + 2 malicious = 7 total.
     Sorted full slots: [300, 300, 300, 300, 300, 1000, 1000].
     median = [7/2] = [3] = 300.  Honest majority still wins. */
  ...
  /* ... N/2 malicious: 3 honest + 3 malicious = 6 total.
     Sorted full slots: [300, 300, 300, 1000, 1000, 1000].
     Lower median = [(6-1)/2] = [2] = 300.
     At exactly 50% the lower-median still picks the honest value. */
...
static void
test_median_low_slot_malicious( fd_sspeer_selector_t * selector,
                                fd_rng_t *             rng ) {
  FD_LOG_NOTICE(( "testing median low slot malicious" ));

  /* Verify that malicious peers reporting low slot values cannot drag
     the cluster slot down when honest peers hold a majority. */
  ...
  /* 6 peers total.  Full slots sorted: [0, 300, 300, 300, 300, 300].
     Lower median = [(6-1)/2] = [2] = 300.
     Single low outlier cannot shift the median. */
  fd_sspeer_selector_process_cluster_slot( selector );
  fd_sscluster_slot_t cs = fd_sspeer_selector_cluster_slot( selector );
  FD_TEST( cs.full==300UL );
  FD_TEST( cs.incremental==300UL );
  ...
  /* 2 malicious low-slot + 5 honest = 7 total.
     Sorted: [0, 0, 300, 300, 300, 300, 300].
     Lower median = [(7-1)/2] = [3] = 300. */
  for( ulong i=0; i<5UL; i++ ) {
    FD_TEST( generate_rand_sspeer_key( &honest_keys[i], rng, fd_rng_uint( rng )&0x1 ) );
    FD_TEST( generate_rand_addr_non_zero( &honest_addrs[i], rng ) );
    FD_TEST( add_peer( selector, &honest_keys[i], honest_addrs[i],
  ...
  /* 3 malicious low-slot + 3 honest = 6 total (50% split).
     Sorted: [0, 0, 0, 300, 300, 300].
     Lower median = [(6-1)/2] = [2] = 0.
     At exactly 50% malicious, the lower-median picks the malicious
     value.  This is a known property of median defenses. */
  for( ulong i=0; i<3UL; i++ ) {
    FD_TEST( generate_rand_sspeer_key( &honest_keys[i], rng, fd_rng_uint( rng )&0x1 ) );
    FD_TEST( generate_rand_addr_non_zero( &honest_addrs[i], rng ) );
    FD_TEST( add_peer( selector, &honest_keys[i], honest_addrs[i],
...
test_all_unknown_slots( fd_sspeer_selector_t * selector,
  ...
  /* When every tracked peer has full_slot==UNKNOWN, the median
     computation should preserve the existing cluster slot (no known
     slots to compute a median from). */
...
test_process_cluster_slot_idempotent( fd_sspeer_selector_t * selector,
  ...
  /* First call: computes median and rescores. */
  fd_sspeer_selector_process_cluster_slot( selector );
...
test_full_only_cluster_rescore( fd_sspeer_selector_t * selector,
  ...
  /* Median of full slots [1000, 2000, 3000] = 2000.
  ...
  /* Scoring falls back to full_slot comparison:
     Peer A: behind = 2000-1000 = 1000.
       score = 1_000_000 + 1000*1000 = 2_000_000.
     Peer B: behind = 2000-2000 = 0.
       score = 2_000_000.
     Peer C: behind = max(0, 2000-3000) = 0.
       score = 3_000_000.
     Best = Peer A or Peer B (both 2_000_000), treap ordering decides. */
  fd_sspeer_t best = fd_sspeer_selector_best( selector, 0, FD_SSPEER_SLOT_UNKNOWN );
  FD_TEST( best.score==2UL*1000UL*1000UL );

  /* Remove the highest-slot peer.  Remaining: [1000, 2000].
     Lower median = [(2-1)/2] = [0] = 1000.
     Incr: still UNKNOWN. */
  fd_sspeer_selector_remove( selector, key_C );
  fd_sspeer_selector_process_cluster_slot( selector );
  cs = fd_sspeer_selector_cluster_slot( selector );
  ...
  /* After rescore with cluster=(1000, UNKNOWN):
     Peer A: behind = 1000-1000 = 0.  score = 1_000_000.
     Peer B: behind = max(0, 1000-2000) = 0.  score = 2_000_000.
     Best = Peer A. */
  best = fd_sspeer_selector_best( selector, 0, FD_SSPEER_SLOT_UNKNOWN );
  FD_TEST( best.addr.l==addr_A.l );
  FD_TEST( best.score==1UL*1000UL*1000UL );
...
static void
test_median_incr_clamped_to_full( fd_sspeer_selector_t * selector,
                                   fd_rng_t *             rng ) {
  FD_LOG_NOTICE(( "testing median_incr < median_full clamping" ));

  /* A(full=2000,incr=UNKNOWN), B(full=3000,incr=UNKNOWN), C(full=500,incr=500).
     Full median = [500,2000,3000][1] = 2000.  Incr median = [500][0] = 500.
     500 < 2000 -> clamped to (2000, 2000). */
  ...
  fd_sscluster_slot_t cs = fd_sspeer_selector_cluster_slot( selector );
  FD_TEST( cs.full==2000UL );
  FD_TEST( cs.incremental==2000UL );  /* clamped from 500 */
...
static void
test_median_four_distinct( fd_sspeer_selector_t * selector,
                           fd_rng_t *             rng ) {
  FD_LOG_NOTICE(( "testing median four distinct" ));

  /* sorted: [100,200,300,400], lower median index (4-1)/2=1 -> 200 */
  ...
  fd_sspeer_selector_process_cluster_slot( selector );
  FD_TEST( fd_sspeer_selector_cluster_slot( selector ).full==200UL );
...
static void
test_dirty_but_median_unchanged( fd_sspeer_selector_t * selector,
                                 fd_rng_t *             rng ) {
  FD_LOG_NOTICE(( "testing dirty but median unchanged" ));
  ...
  /* Add and remove a peer with UNKNOWN slots, sets dirty but cannot change median. */
...
main( int     argc,
  ...
  test_wksp_reinit( &t_wksp_base );
  test_median_outlier_robustness( t_wksp_base.selector, rng );

  test_wksp_reinit( &t_wksp_base );
  test_median_low_slot_malicious( t_wksp_base.selector, rng );
  ...
  test_wksp_reinit( &t_wksp_base );
  test_median_incr_clamped_to_full( t_wksp_base.selector, rng );

  test_wksp_reinit( &t_wksp_base );
  test_median_four_distinct( t_wksp_base.selector, rng );

  test_wksp_reinit( &t_wksp_base );
  test_dirty_but_median_unchanged( t_wksp_base.selector, rng );
