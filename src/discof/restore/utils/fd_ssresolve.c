...
/* TODO: seriously consider refactoring the common http code in
   ssresolve and sshttp into a common library */

#define FD_SSRESOLVE_CONNECT             (0)
#define FD_SSRESOLVE_STATE_REQ           (1) /* sending request for snapshot */
#define FD_SSRESOLVE_STATE_RESP          (2) /* receiving snapshot response */
#define FD_SSRESOLVE_STATE_SHUTTING_DOWN (3)
...
fd_ssresolve_init_https( fd_ssresolve_t * ssresolve,
  ...
  if( FD_UNLIKELY( alpn_res!=0 ) ) {
    FD_LOG_ERR(( "SSL_set_alpn_protos failed (%d)", SSL_get_error( ssresolve->ssl, alpn_res ) ));
  }
  ...
  if( FD_UNLIKELY( !set1_host_res ) ) {
    FD_LOG_ERR(( "SSL_set1_host failed (%d)", SSL_get_error( ssresolve->ssl, set1_host_res ) ));
  }
}
#endif
...
#if FD_HAS_OPENSSL
  ssresolve->state = FD_SSRESOLVE_STATE_SHUTTING_DOWN;
#else
...
fd_ssresolve_cancel( fd_ssresolve_t * ssresolve ) {
  if( FD_UNLIKELY( ssresolve->sockfd==-1 ) ) return;
  if( FD_UNLIKELY( -1==close( ssresolve->sockfd ) ) ) FD_LOG_ERR(( "close() failed (%d-%s)", errno, fd_io_strerror( errno ) ));
