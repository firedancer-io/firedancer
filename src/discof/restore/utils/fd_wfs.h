...
/* Wait-for-supermajority (WFS) is the coordinated-restart boot mode.
   ...
   Specific rules:
     ...
     - In MATCH mode the leader gate (the requirement that the
       ...
       be suppressed until WFS completes.  Once completed, normal
    ...
    Regarding snapshot(s) (down)load:
     ...
     - When WFS uses full-only snapshot, the full snapshot's shred
       ...
       snapshot's shred version might be V_old, whereas the
       incremental's shred version must be V.
       ...
       The alternatives are to treat WFS configuration as:
         - strict:
             ...
             - Operational nuances: the operator must remove the WFS
               config right after the network restarts, otherwise a
               subsequent validator restart will fail.
     ...
     - When WFS is configured and the full snapshot is behind slot S,
       ...
       true, enabling incremental snapshot loading to reach S.  When
       the full snapshot is at or past S (MATCH or NOOP), the config
       flag is respected as-is. */
