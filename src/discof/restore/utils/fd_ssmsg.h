...
/* A streamlined version of epoch_stakes used for E and E-1 entries.
   These only need vote/identity/has_identity_bls/stake/commission
   (no epoch_credits). */
...
struct fd_snapshot_manifest {
  ...
  /* Epoch stakes represent the exact amount staked to each vote
     ...
     where <epoch> assumes these values:
       ...
       E-1 - represents the stakes at the beginning of epoch E-2 or the
             end of the E-3 epoch.  This is used for delayed commission
             values.

       E   - represents the stakes at the beginning of epoch E-1 or the
             end of the E-2 epoch, used to compute the leader schedule
             at E.  This corresponds to the T-2 epoch stakes.

       E+1 - represents the stakes at the beginning of epoch E and the
             end of the E-1 epoch.  This is used to compute the leader
             schedule at E+1.

     The epoch stakes are stored in an array:
       epoch_stakes[0] contains E-1 data (T-3, delayed commission)
       epoch_stakes[1] contains E   data (T-2, current leader schedule)
       epoch_stakes[2] contains E+1 data (T-1, next leader schedule) */
