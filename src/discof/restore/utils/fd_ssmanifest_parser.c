...
state_validate( fd_ssmanifest_parser_t * parser ) {
  ...
  switch( parser->state ) {
    ...
    case STATE_VERSIONED_EPOCH_STAKES_STAKES_VOTE_ACCOUNTS_VALUE_DATA_V4_INFLATION_REWARDS_COMMISSION_BPS:
      if( FD_UNLIKELY( parser->staging_vote_stakes.commission>10000 ) ) {
        FD_LOG_WARNING(( "invalid commission %u", parser->staging_vote_stakes.commission ));
        return -1;
      }
      break;
    ...
    case STATE_VERSIONED_EPOCH_STAKES_STAKES_VOTE_ACCOUNTS_VALUE_DATA_V3_COMMISSION:
      if( FD_UNLIKELY( (parser->staging_vote_stakes.commission & 0xFF) >100 ) ) {
        FD_LOG_WARNING(( "invalid commission %u", parser->staging_vote_stakes.commission ));
        return -1;
      }
      break;
    case STATE_STAKES_VOTE_ACCOUNTS_VALUE_DATA_V4_INFLATION_REWARDS_COMMISSION_BPS:
      if( FD_UNLIKELY( parser->va_commission>10000 ) ) {
        FD_LOG_WARNING(( "invalid commission %u", parser->va_commission ));
        return -1;
      }
      break;
    ...
    case STATE_STAKES_VOTE_ACCOUNTS_VALUE_DATA_V0235_COMMISSION:
      if( FD_UNLIKELY( (parser->va_commission & 0xFF) >100 ) ) {
        FD_LOG_WARNING(( "invalid commission %u", parser->va_commission ));
        return -1;
      }
      break;
...
state_process( fd_ssmanifest_parser_t * parser,
  ...
  if( FD_UNLIKELY( parser->state==STATE_EPOCH_SCHEDULE_FIRST_NORMAL_SLOT ) ) {
    ...
    if( FD_UNLIKELY( parser->leader_schedule_epoch-parser->epoch+1UL>=epoch_stakes_ele_cnt ) ) {
      /* We support storing epoch stakes for E-1, E, and the leader
         schedule epoch (E+1).  If the leader schedule epoch is further
         ahead we will hit this error. */
  ...
  if( FD_UNLIKELY( parser->state==STATE_VERSIONED_EPOCH_STAKES_EPOCH ) ) {
    ...
    ulong min_epoch = parser->epoch > 0UL ? parser->epoch - 1UL : 0UL;
    if( parser->epoch_stakes_epoch>=min_epoch && parser->epoch_stakes_epoch<=parser->leader_schedule_epoch ) {
      parser->epoch_idx = parser->epoch_stakes_epoch - min_epoch;
