...
fd_ssping_best( fd_ssping_t const * ssping,
  ...
  for( score_treap_fwd_iter_t iter = score_treap_fwd_iter_init( ssping->score_treap, ssping->pool );
    ...
    if( FD_LIKELY( ssping->incremental_snapshot_fetch ) ) {
      if( FD_UNLIKELY( best->snapshot_info.incremental.base_slot==ULONG_MAX ) ) {
        continue; /* incremental snapshot not available */
      }
    ...
    if( FD_LIKELY( peer_slot!=ULONG_MAX && peer_slot>=highest_slot ) ) {
      return (fd_sspeer_t){
        .addr = best->addr,
