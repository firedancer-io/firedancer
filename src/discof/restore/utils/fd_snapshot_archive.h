...
struct fd_full_snapshot_archive_entry {
   char      path[ PATH_MAX ];          /* Absolute path to snapshot file on disk. */
   ulong     slot;                      /* Slot of the full snapshot */
   uchar     hash[ FD_HASH_FOOTPRINT ]; /* Hash of all accounts in the snapshot  */
};
...
/* Given a snapshots archive directory, finds the most recent full
   snapshot file by slot number and populates the full_snapshot_entry
   accordingly.
...
/* Given a snapshots archive directory, finds the most recent
   incremental snapshot file by slot number and populates the
