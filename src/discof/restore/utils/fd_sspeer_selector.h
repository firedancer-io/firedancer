...
/* fd_sscluster_slot stores the median full and incremental slot
   computed from all tracked peers in the cluster. */
...
/* Recompute the selector's internal cluster slot from the median of
   all tracked peers' slots.  If the median changes, all peers are
...
/* Obtain the cluster slot from the selector.  It is the lower median
