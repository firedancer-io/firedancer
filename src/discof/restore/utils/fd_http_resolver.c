...
#define PEER_STATE_INVALID    (2)

#define PEER_DEADLINE_NANOS_RESOLVE (1L*1000L*1000L*1000L) /* 1 second */
#define PEER_DEADLINE_NANOS_INVALID (1L*60L*1000L*1000L*1000L) /* 1 minute */
...
fd_http_resolver_align( void ) {
  return alignof(fd_http_resolver_t);
}
...
fd_http_resolver_new( void * shmem,
  ...
  resolver->fds_len_max = 2UL*peers_cnt;
...
remove_peer( fd_http_resolver_t * resolver,
  ...
  resolver->fds_len--;
  resolver->fds[ idx ] = (struct pollfd) {
    .fd      = -1,
    .events  = 0,
    .revents = 0
  };
  resolver->fds_idx[ idx ] = ULONG_MAX;

  resolver->fds_len--;
  resolver->fds[ idx+1UL ] = (struct pollfd) {
    .fd      = -1,
    .events  = 0,
    .revents = 0
  };
  resolver->fds_idx[ idx+1UL ] = ULONG_MAX;
}
...
poll_advance( fd_http_resolver_t * resolver,
  ...
  ulong num_valid_fds = 0UL;
  for( ulong i=0UL; i<resolver->fds_len_max; i++) {
    if( num_valid_fds>=resolver->fds_len ) break;
    ...
    if( FD_LIKELY( fd_ssresolve_is_done( peer->full_ssresolve ) &&
      ...
      peer->resolve_latency_nanos = (peer->full_latency_nanos + peer->incremental_latency_nanos) / 2UL;
