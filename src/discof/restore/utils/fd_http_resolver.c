...
static void
fd_http_resolver_sign_fn( void * ctx, uchar sig[ static 64 ], uchar const payload[ static 130 ] ) {
  (void)ctx; (void)sig; (void)payload;
  FD_LOG_ERR(( "unexpected TLS sign request in http resolver" ));
}
...
fd_http_resolver_new( void *                           shmem,
...

  {
    ...
    tls->sign.ctx     = NULL;
    tls->sign.sign_fn = fd_http_resolver_sign_fn;
