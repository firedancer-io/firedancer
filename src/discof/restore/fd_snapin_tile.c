  ...
  for(;;) {
    ...
    switch( res ) {
      ...
      case FD_SSPARSE_ADVANCE_STATUS_CACHE: {
        fd_slot_delta_parser_advance_result_t advance[1];
        ...
        while( bytes_remaining ) {
          ...
          if( FD_UNLIKELY( res<0 ) ) {
            ...
            ctx->txncache_entries[ ctx->txncache_entries_len++ ] = *advance->entry;
          }

          bytes_remaining           -= advance->bytes_consumed;
          result->status_cache.data += advance->bytes_consumed;
        }
        break;
      }
