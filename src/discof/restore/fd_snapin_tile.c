...
process_account_header( fd_snapin_tile_t *            ctx,
  ...
  fd_funk_rec_t const * rec = fd_funk_rec_query_try( funk, ctx->xid, &id, query );
  ...
  if( FD_LIKELY( rec->val_sz<sizeof(fd_account_meta_t)+result->account_header.data_len ) ) {
    meta = fd_funk_val_truncate( (fd_funk_rec_t*)rec, funk->alloc, funk->wksp, 0UL, sizeof(fd_account_meta_t)+result->account_header.data_len, NULL );
    FD_TEST( meta );
  }
