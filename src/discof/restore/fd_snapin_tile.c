...
account_cb( void *                          _ctx,
  ...
  if( FD_LIKELY( !existing_rec ) ) {
    /* An existing record may exist in an ancestor transaction */
    existing_rec = fd_funk_rec_clone( ctx->funk, ctx->funk_txn, &id, prepare, NULL );
  }

  if( FD_LIKELY( existing_rec ) ) {
    /* If a record exists either in the current txn or in an ancestor
       txn, it may be a duplicate.  Duplicates need to be subtracted
       from the running lthash in the hashing tiles. */
    ...
    if( FD_UNLIKELY( meta ) ) {
      ...
      if( FD_LIKELY( ctx->hash_info.enabled ) ) {
        ...
        fd_snapshot_existing_account_t * existing_account = (fd_snapshot_existing_account_t *)fd_chunk_to_laddr( ctx->hash_out.wksp, ctx->hash_out.chunk );
...
handle_control_frag( fd_snapin_tile_t *  ctx,
  ...
  switch( sig ) {
    ...
    case FD_SNAPSHOT_HASH_MSG_RESULT: {
      ...
      fd_lthash_value_t const * calculated_lthash = (fd_lthash_value_t const *)fd_chunk_to_laddr_const( ctx->hash_in[ in_idx - 1UL ].wksp, chunk );
