...
struct fd_snapin_tile {
  ...
  ulong              replay_manifest_dcache_offset;
...
send_manifest( fd_snapshot_parser_t *        parser,
  ...
  if( ctx->state == FD_SNAPIN_STATE_LOADING_FULL ) {
    sig          = FD_FULL_SNAPSHOT_MANIFEST_MESSAGE_ID;
    external_sig = FD_FULL_SNAPSHOT_MANIFEST_EXTERNAL_MESSAGE_ID;
  } else if( ctx->state == FD_SNAPIN_STATE_LOADING_INCREMENTAL ) {
    sig          = FD_INCREMENTAL_SNAPSHOT_MANIFEST_MESSAGE_ID;
    external_sig = FD_INCREMENTAL_SNAPSHOT_MANIFEST_EXTERNAL_MESSAGE_ID;
  } else {
    FD_LOG_ERR(( "Unexpected state %d for sending manifest", ctx->state ));
  }

  /* Send snapshot manifest message over snap_replay link */
  ...
  fd_memcpy( ctx->replay_manifest_dcache + ctx->replay_manifest_dcache_offset,
             manifest,
             manifest_sz );
  fd_frag_writer_publish_external_dcache( ctx->manifest_writer,
                                          ctx->replay_manifest_dcache,
                                          ctx->replay_manifest_dcache_obj_id,
                                          ctx->replay_manifest_dcache_offset,
                                          manifest_sz,
                                          external_sig,
                                          0UL,
                                          0UL );
  ctx->replay_manifest_dcache_offset += manifest_sz;
}
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->replay_manifest_dcache_offset = 0UL;
