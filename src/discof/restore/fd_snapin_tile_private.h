...
struct fd_snapin_tile {
  ...
  /* vote_stakes buffers (removed from fd_snapshot_manifest_t struct):
       ...
       E   uses the slim struct (T-2 stakes / current leader schedule).
       E-1 uses the slim struct (T-3 stakes / delayed commission). */
  ulong                                vote_stakes_t_1_len; /* running counter for E+1 entries */
  fd_snapshot_epoch_stakes_slim_t    * vote_stakes_t_2;
  ulong                                vote_stakes_t_2_len;
  fd_snapshot_epoch_stakes_slim_t    * vote_stakes_t_3;
  ulong                                vote_stakes_t_3_len;
