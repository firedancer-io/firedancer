...
static void populate_sock_filter_policy_fd_snapwr_tile( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int accounts_fd ) {
  ...
  struct sock_filter filter[25] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_pwrite64, /* check_pwrite64 */ 9, 0 ),
    /* allow ftruncate based on expression */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_ftruncate, /* check_ftruncate */ 10, 0 ),
...
//  check_ftruncate:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, accounts_fd, /* lbl_2 */ 0, /* RET_KILL_PROCESS */ 4 ),
//  lbl_2:
