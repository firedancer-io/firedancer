...
#define FD_SNAPIN_VINYL_IMPL_VERSION (0)
...
fd_snapin_process_account_header_vinyl( fd_snapin_tile_t *            ctx,
  ...
  res.account_header.slot        = result->account_header.slot;
  res.account_header.data_len    = result->account_header.data_len;
  res.account_header.pubkey      = NULL;
  res.account_header.lamports    = result->account_header.lamports;
  res.account_header.rent_epoch  = result->account_header.rent_epoch;
  res.account_header.owner       = NULL;
  res.account_header.executable  = result->account_header.executable;
  res.account_header.hash        = NULL;
  fd_memcpy( dst, &res, sizeof(result->account_header) );
  dst += sizeof(result->account_header);

  fd_memcpy( dst, result->account_header.owner, sizeof(fd_pubkey_t) );
  dst += sizeof(fd_pubkey_t);

  fd_memcpy( dst, result->account_header.pubkey, sizeof(fd_pubkey_t) );
  dst += sizeof(fd_pubkey_t);

  dst += sizeof(ulong);

  FD_TEST( (((ulong)dst)-((ulong)out))==hdr_sz );

  ctx->vinyl_op.pair     = out;
  ctx->vinyl_op.pair_sz  = hdr_sz + data_len;
  ctx->vinyl_op.dst      = dst;
  ctx->vinyl_op.dst_rem  = data_len;
  ctx->vinyl_op.data_rem = data_len;
...
fd_snapin_process_account_header_vinyl( fd_snapin_tile_t *            ctx,
  ...
  uchar const * frame = result->account_header.pubkey - 0x10UL;
  fd_memcpy( dst, frame, 0x80UL );
  dst += 0x80UL;

  fd_memcpy( dst, &result->account_header.slot, 0x08UL );
  dst += 0x08UL;
...
fd_snapin_process_account_batch_vinyl( fd_snapin_tile_t *            ctx,
  ...
  for( ulong i=0UL; i<FD_SSPARSE_ACC_BATCH_MAX; i++ ) {
    ...
    fd_ssparse_advance_result_t res;
    res.account_header.slot        = result->account_batch.slot;
    res.account_header.data_len    = data_len;
    res.account_header.pubkey      = NULL;
    res.account_header.lamports    = lamports;
    res.account_header.rent_epoch  = 0; /*unused*/
    res.account_header.owner       = NULL;
    res.account_header.executable  = executable;
    res.account_header.hash        = NULL;
    fd_memcpy( dst, &res, sizeof(res.account_header) );
    dst += sizeof(res.account_header);

    fd_memcpy( dst, owner, sizeof(fd_pubkey_t) );
    dst += sizeof(fd_pubkey_t);
