  ...
  if( ctx->in_kind[ in_idx ]==IN_KIND_GOSSIP ) {
    ...
    switch( msg->tag ) {
      ...
      case FD_GOSSIP_UPDATE_TAG_SNAPSHOT_HASHES: {
        ...
        FD_LOG_DEBUG(("encountered pubkey %s with full slot %lu and incremental slot %lu",
  ...
  if( FD_UNLIKELY( !has_gossip_in ) ) {
    if( FD_LIKELY( !strcmp( tile->snaprd.cluster, "testnet" ) ) ) {
      fd_ip4_port_t initial_peers[ 2UL ] = {
        ...
        { .addr = FD_IP4_ADDR( 177, 54, 155, 187 ), .port = fd_ushort_bswap(8899) } /* A fast testnet peer from snapshot-finder script */
      };
