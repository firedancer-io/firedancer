  ...
  switch ( ctx->state ) {
    ...
    case FD_SNAPRD_STATE_FLUSHING_INCREMENTAL_FILE:
      ...
      if( FD_UNLIKELY( ctx->malformed ) ) {
        ctx->malformed = 0;
        fd_stem_publish( stem, ctx->out_ld.idx, FD_SNAPSHOT_MSG_CTRL_FAIL_INCR, 0UL, 0UL, 0UL, 0UL, 0UL );
        ctx->flush_ack = 0;
    ...
    case FD_SNAPRD_STATE_FLUSHING_FULL_FILE:
      ...
      if( FD_UNLIKELY( ctx->malformed ) ) {
        ctx->malformed = 0;
        fd_stem_publish( stem, ctx->out_ld.idx, FD_SNAPSHOT_MSG_CTRL_FAIL_FULL, 0UL, 0UL, 0UL, 0UL, 0UL );
        ctx->flush_ack = 0;
    ...
    case FD_SNAPRD_STATE_READING_FULL_FILE:
      ...
      if( FD_UNLIKELY( ctx->metrics.full.bytes_read == ctx->metrics.full.bytes_total ) ) {
        fd_stem_publish( stem, ctx->out_ld.idx, FD_SNAPSHOT_MSG_CTRL_DONE_FULL, 0UL, 0UL, 0UL, 0UL, 0UL );
    ...
    case FD_SNAPRD_STATE_READING_INCREMENTAL_FILE:
      ...
      if ( FD_UNLIKELY( ctx->metrics.incremental.bytes_read == ctx->metrics.incremental.bytes_total ) ) {
        fd_stem_publish( stem, ctx->out_ld.idx, FD_SNAPSHOT_MSG_CTRL_DONE_INCR, 0UL, 0UL, 0UL, 0UL, 0UL );
...
gossip_frag( fd_snaprd_tile_t *  ctx,
  ...
  FD_TEST( chunk>=ctx->gossip_in.chunk0 && chunk<=ctx->gossip_in.wmark && sz==sizeof(fd_gossip_update_message_t) );
...
snapin_frag( fd_snaprd_tile_t *  ctx,
  ...
  switch( sig ) {
    ...
    case FD_SNAPSHOT_MSG_CTRL_DONE_FULL:
    ...
    case FD_SNAPSHOT_MSG_CTRL_DONE_INCR:
      if( FD_LIKELY( ctx->state==FD_SNAPRD_STATE_FLUSHING_INCREMENTAL_HTTP ||
                     ctx->state==FD_SNAPRD_STATE_FLUSHING_INCREMENTAL_FILE ) ) {
        FD_TEST( !ctx->flush_ack );
        ctx->flush_ack = 1;
      } else FD_LOG_ERR(( "invalid control frag %lu in state %d", sig, ctx->state ));
    ...
    case FD_SNAPSHOT_MSG_CTRL_FAIL_FULL:
    ...
    case FD_SNAPSHOT_MSG_CTRL_FAIL_INCR:
      if( FD_LIKELY( ctx->state==FD_SNAPRD_STATE_FLUSHING_INCREMENTAL_HTTP_RESET ||
                     ctx->state==FD_SNAPRD_STATE_FLUSHING_INCREMENTAL_FILE_RESET ) ) {
        FD_TEST( !ctx->flush_ack );
        ctx->flush_ack = 1;
      } else FD_LOG_ERR(( "invalid control frag %lu in state %d", sig, ctx->state ));
...
returnable_frag( fd_snaprd_tile_t *  ctx,
  ...
  if( FD_LIKELY( ctx->in_kind[ in_idx ]==IN_KIND_GOSSIP ) ) {
    ...
    snapin_frag( ctx, sig );
  } else FD_LOG_ERR(( "invalid in_kind %u %hhu", in_idx, ctx->in_kind[ in_idx ] ));
  return 0;
}
...
unprivileged_init( fd_topo_t *      topo,
  ...
  FD_TEST( tile->in_cnt==NUM_IN_LINKS );
