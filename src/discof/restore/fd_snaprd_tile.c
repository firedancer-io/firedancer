...
/* Wait for peer ContactInfo and SnapshotHashes to arrive from the
   ...
   the fixed duration, we stay in the waiting state indefinitely.

   If there are elegible peers and there exists a local
   snapshot whose slot is recent enough compared to the collected
   SnapshotHashes slot numbers, we transition to loading a snapshot from
   disk.

  If there are elegible peers and no local snapshot is recent enough,
  we transition to peer selection. */
...
#define FD_SNAPRD_STATE_PINGING_PEERS                        ( 1)
...
#define FD_SNAPRD_STATE_COLLECTING_RESPONSES                 ( 2)
...
#define FD_SNAPRD_STATE_SELECTING_PEER                       ( 3)
...
#define FD_SNAPRD_STATE_SELECTING_PEER_INCREMENTAL           (10)
...
/* Once we have fully downloaded the full snapshot, and there is no
   ...
   shutdown. */
#define FD_SNAPRD_STATE_WAITING_FOR_FULL_LOAD                (11)
...
#define FD_SNAPRD_STATE_WAITING_FOR_INCREMENTAL_LOAD         (12)

/* The terminal state of the tile, snapshot load is fully completed and
   the tile has exited. */
#define FD_SNAPRD_STATE_DONE                                 (13)


#define FD_SNAPRD_STATE_WAITING_FOR_GOSSIP (0)
#define FD_SNAPRD_STATE_INIT_READER        (1)
#define FD_SNAPRD_STATE_SELECTING_PEERS     (2)
#define FD_SNAPRD_STATE_READING_SNAPSHOT   (3)
...
//   int state = 0;
//   switch( state ) {
//     case FD_SNAPRD_STATE_WAITING_FOR_PEER:
//       if( any_collected_peer_is_valid ) {
//         state = FD_SNAPRD_STATE_COLLECTING_PEERS;
//         break;
//       }
//       /* Nothing to do ... */
//       break;
//     case FD_SNAPRD_STATE_COLLECTING_PEERS: {
//       if( now<ctx->collect_until_deadline_nanos ) break;

//       ulong highest_file_snapshot_slot = /* some slot */;
//       ulong highest_valid_peer_snapshot_slot = /* some slot */;
//       if( highest_file_snapshot_slot>highest_valid_peer_snapshot_slot+age_config ) {
//         /* We have a local snapshot that is more recent than any
//            peer, so we can just load it. */
//         state = FD_SNAPRD_STATE_READING_FULL_FILE;
//         break;
//       }

//       /* Transition to COLLECTING_PING_RESPONSES ... so
//          we need to send out some pings. */
//       for( ulong i=0UL; i<ctx->peers_cnt; i++ ) {
//         // ... send ping
//       }
//       ctx->collect_ping_responses_until_deadline_nanos = now + 1000000000UL; // 1 second
//       state = FD_SNAPRD_STATE_COLLECTING_PING_RESPONSES;
//       break;
//     }
//     case FD_SNAPRD_STATE_COLLECTING_PING_RESPONSES: {
//       if( now<ctx->collect_ping_responses_until_deadline_nanos ) break;

//       /* Transition to SELECTING_PEER ... so
//          we need to select a peer */
//       peer * best_peer = /* ... some selection algo */
//       if( !peer ) {
//         state = FD_SNAPRD_STATE_WAITING_FOR_PEER;
//         break;
//       }

//       // have some valid peer ...
//       break;
//     }
//   }
// }
...
struct fd_snaprd_tile {
  ...
  struct {
    ...
    long response_deadline_nanos;
  } peer_selection;

  /* peers */
  /* TODO: peer type not yet finalized */
...
fd_snaprd_init( fd_snaprd_tile_t * ctx ) {
  fd_snaprd_download_pair_t download_pair = fd_snaprd_should_download( ctx );
  ctx->should_download = download_pair.full || download_pair.incremental;
  ...
  if( ctx->should_download ) {
    ctx->state = FD_SNAPRD_STATE_SELECTING_PEERS;
  } else {
    ctx->state = FD_SNAPRD_STATE_READING_SNAPSHOT;
  }
}
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->state = FD_SNAPRD_STATE_WAITING_FOR_GOSSIP;
...
static void
fd_snaprd_receive_gossip( fd_snaprd_tile_t * ctx ) {
  /* TODO: implement logic to receive snapshot hashes and peers from gossip receiver */

  if( ctx->should_download ) {
    ctx->state = FD_SNAPRD_STATE_SELECTING_PEERS;
  } else {
    ctx->state = FD_SNAPRD_STATE_INIT_READER;
  }

  /* TODO: update locally managed peers list */
}

static void
fd_snaprd_select_peers( fd_snaprd_tile_t * ctx ) {
  (void)ctx;
  ctx->state = FD_SNAPRD_STATE_READING_SNAPSHOT;

  /* TODO: after sorting peers by request latency and min download speed,
     send candidates to snapshot reader */
}
...
after_credit( void *            _ctx,
  ...
  switch ( ctx->state ) {
    case FD_SNAPRD_STATE_WAITING_FOR_PEERS: {
      // Wait until we get the first gossip contact info,
      // and then wait an additional 3 seconds ...
      fd_snaprd_receive_gossip( ctx );
      break;
    }
    case FD_SNAPRD_STATE_INIT_READER: {
      fd_snaprd_init( ctx );
      break;
    }
    case FD_SNAPRD_STATE_SELECTING_PEERS: {
      fd_snaprd_select_peers( ctx );
      break;
    }
    case FD_SNAPRD_STATE_READING_SNAPSHOT: {
