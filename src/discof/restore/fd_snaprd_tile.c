...
/* SnapRd waits until it sees a single valid gossip peer which we could
   ...
   PINGING_PEERS. */
#define FD_SNAPRD_STATE_COLLECTING_PEERS                     ( 2)
...
#define FD_SNAPRD_STATE_PINGING_PEERS                        ( 3)
...
#define FD_SNAPRD_STATE_COLLECTING_RESPONSES                 ( 4)
...
#define FD_SNAPRD_STATE_READING_FULL_FILE                    ( 5)
...
/* Optionally, after loading the full snapshot SnapRd loads the
   ...
   state. */
#define FD_SNAPRD_STATE_READING_INCREMENTAL_FILE             ( 6)
...
#define FD_SNAPRD_STATE_FULL_DOWNLOAD                        ( 7)
...
#define FD_SNAPRD_STATE_INCREMENTAL_DOWNLOAD                 ( 8)
...
#define FD_SNAPRD_STATE_PINGING_PEERS_INCREMENTAL            ( 9)
...
#define FD_SNAPRD_STATE_COLLECTING_RESPONSES_INCREMENTAL     (10)
...
fd_snaprd_should_download( fd_snaprd_tile_t * ctx ) {
  ...
  if( FD_UNLIKELY( res ) ) {
    FD_LOG_INFO(( "Unable to find any local full snapshots in the "
  ...
  if( ctx->config.incremental_snapshot_fetch ) {
    ...
    if( FD_UNLIKELY( res ) ) {
      ...
      FD_LOG_INFO(( "Unable to find any local incremental snapshots "
  ...
  if( highest_slot >= fd_ulong_sat_sub( CLUSTER_SNAPSHOT_SLOT, ctx->config.maximum_local_snapshot_age ) ) {
    FD_LOG_INFO(( "Re-using local full snapshot at slot %lu", ctx->full_snapshot_entry.slot ));
    if( ctx->config.incremental_snapshot_fetch ) {
      FD_LOG_INFO(( "Re-using local incremental snapshot at slot %lu", ctx->incremental_snapshot_entry.inner.slot ));
    }
    return ( fd_snaprd_download_pair_t ){ .full = 0,
                                            .incremental = 0 };
  } else {
  ...
  switch ( ctx->state ) {
    case FD_SNAPRD_STATE_WAITING_FOR_PEERS: {
      ...
      if( ctx->peers_manager->peers_cnt > 0 ) {
        ...
        fd_snaprd_set_state( ctx, FD_SNAPRD_STATE_COLLECTING_PEERS );
      }
      break;
    }
