...
read_http_data( fd_snaprd_tile_t *  ctx,
                fd_stem_context_t * stem,
                long                now ) {
  uchar * out = fd_chunk_to_laddr( (void *)ctx->out_snapctl.chunk0, ctx->out_snapctl.chunk );
  ...
  char snapshot_path[ PATH_MAX+30UL ]; /* 30 is fd_cstr_nlen( "https://255.255.255.255:12345/", ULONG_MAX ) */
...
after_credit( fd_snaprd_tile_t *  ctx,
  ...
  if( FD_UNLIKELY( ctx->local_in.full_snapshot_slot!=ULONG_MAX && !ctx->gui_incremental_path_published ) ) {
