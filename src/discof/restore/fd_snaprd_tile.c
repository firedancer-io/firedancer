...
read_http_data( fd_snaprd_tile_t *  ctx,
  ...
  if( FD_UNLIKELY( !ctx->metrics.full.path_sent ) && fd_cstr_nlen( full_snapshot_name, 1 ) ) {
    fd_restore_snapshot_update_t * out = fd_chunk_to_laddr( ctx->out[ OUT_KIND_PLUGIN ].wksp, ctx->out[ OUT_KIND_PLUGIN ].chunk );
    FD_TEST( fd_cstr_printf_check( out->read_path, PATH_MAX, NULL, "%s", full_snapshot_name ) );
    out->is_download = 1;
    out->type = FD_PLUGIN_MSG_SNAPSHOT_TYPE_FULL;
    fd_stem_publish( stem, ctx->out[ OUT_KIND_PLUGIN ].out_idx, 0UL, ctx->out[ OUT_KIND_PLUGIN ].chunk, sizeof(fd_restore_snapshot_update_t) , 0UL, 0UL, 0UL );
    ctx->out[ OUT_KIND_PLUGIN ].chunk = fd_dcache_compact_next( ctx->out[ OUT_KIND_PLUGIN ].chunk, data_len, ctx->out[ OUT_KIND_PLUGIN ].chunk0, ctx->out[ OUT_KIND_PLUGIN ].wmark );
  }

  if( FD_UNLIKELY( !ctx->metrics.incremental.path_sent && fd_cstr_nlen( incremental_snapshot_name, 1 ) ) ) {
    fd_restore_snapshot_update_t * out = fd_chunk_to_laddr( ctx->out[ OUT_KIND_PLUGIN ].wksp, ctx->out[ OUT_KIND_PLUGIN ].chunk );
    FD_TEST( fd_cstr_printf_check( out->read_path, PATH_MAX, NULL, "%s", incremental_snapshot_name ) );
    out->is_download = 1;
    out->type = FD_PLUGIN_MSG_SNAPSHOT_TYPE_INCREMENTAL;
    fd_stem_publish( stem, ctx->out[ OUT_KIND_PLUGIN ].out_idx, 0UL, ctx->out[ OUT_KIND_PLUGIN ].chunk, sizeof(fd_restore_snapshot_update_t) , 0UL, 0UL, 0UL );
    ctx->out[ OUT_KIND_PLUGIN ].chunk = fd_dcache_compact_next( ctx->out[ OUT_KIND_PLUGIN ].chunk, data_len, ctx->out[ OUT_KIND_PLUGIN ].chunk0, ctx->out[ OUT_KIND_PLUGIN ].wmark );
  }
