...
fd_ext_poh_get_leader_after_n_slots( ulong n,
  ...
  fd_pubkey_t const * leader = fd_multi_epoch_leaders_get_slot_leader( ctx->mleaders, slot ); /* AMANTODO: Verify safe to call from Rust?! */
...
during_frag( fd_poh_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( ctx->in_kind[ in_idx ]==IN_KIND_STAKE ) ) {
    ...
    fd_multi_epoch_leaders_stake_msg_init( ctx->mleaders, dcache_entry );
    return;
  }
