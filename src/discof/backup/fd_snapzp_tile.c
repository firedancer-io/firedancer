...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->batch_depth = topo->links[ tile->in_link_id[0] ].depth;
  FD_TEST( fd_ulong_is_pow2( ctx->batch_depth ) );
...
process_batch( fd_snapzp_t * ctx,
               ulong         seq ) {
  fd_funk_scan_batch_t const * batch = ctx->batch + (seq & (ctx->batch_depth-1UL));
