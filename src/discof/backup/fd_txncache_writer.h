...
/* FD_TXNCACHE_WRITER_MAX_GROUPS is the most rooted blockcaches a
   ...
   The root list holds the initial root plus one entry per retained
   root advancement, so it is FD_TXNCACHE_MAX_SLOT_DELTAS+1 long in
   steady state (see fd_txncache_advance_root). */

#define FD_TXNCACHE_WRITER_MAX_GROUPS (FD_TXNCACHE_MAX_SLOT_DELTAS+1UL)
...
/* FD_TXNCACHE_WRITER_BUF_MIN is the smallest output buffer
   ...
   does not fit the remaining buffer is retried on the next call, which
   the writer signals by returning fewer bytes than offered; the caller
   must then come back with an empty buffer.  A bucket that does not fit
   an empty buffer is fatal, so the caller's full buffer bounds the
   transactions per bucket (snapmk's 32 MiB fits 1.4M). */

#define FD_TXNCACHE_WRITER_BUF_MIN (4096UL)

/* FD_TXNCACHE_WRITER_HOLD_MAX bounds the hash chain entries walked
   under one txncache read lock hold (checked at bucket boundaries),
   after which the lock is released and reacquired so that replay's
   write lock operations are not blocked for the whole group. */

#define FD_TXNCACHE_WRITER_HOLD_MAX (1UL<<16)
...
/* fd_txncache_writer_init prepares writer to serialize the status
   ...
   the snapshot cannot be produced.  Replay never advances the root
   while a snapshot is in progress, so snapmk treats NULL as fatal. */
