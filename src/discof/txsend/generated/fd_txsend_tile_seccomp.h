...
#endif
static const unsigned int sock_filter_policy_fd_txsend_tile_instr_cnt = 18;

static void populate_sock_filter_policy_fd_txsend_tile( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd ) {
  FD_TEST( out_cnt >= 18 );
  struct sock_filter filter[18] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_getrandom, /* RET_ALLOW */ 11, 0 ),
    /* allow clock_nanosleep based on expression */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_clock_nanosleep, /* check_clock_nanosleep */ 7, 0 ),
