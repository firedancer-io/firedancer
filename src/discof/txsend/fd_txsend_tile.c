...
handle_contact_info_update( fd_txsend_tile_t *                 ctx,
  ...
  for( ulong i=0UL; i<2UL; i++ ) {
    if( FD_LIKELY( msg->contact_info.contact_info->sockets[ quic_socket_idx[ i ] ].addr ) ) {
      entry->quic_ip_addrs[ i ] = msg->contact_info.contact_info->sockets[ quic_socket_idx[ i ] ].addr;
    }
    if( FD_LIKELY( fd_ushort_bswap( msg->contact_info.contact_info->sockets[ quic_socket_idx[ i ] ].port ) ) ) {
      entry->quic_ports   [ i ] = fd_ushort_bswap( msg->contact_info.contact_info->sockets[ quic_socket_idx[ i ] ].port );
    }
  }

  for( ulong i=0UL; i<2UL; i++ ) {
    if( FD_LIKELY( msg->contact_info.contact_info->sockets[ udp_socket_idx[ i ] ].addr ) ) {
      entry->udp_ip_addrs[ i ] = msg->contact_info.contact_info->sockets[ udp_socket_idx[ i ] ].addr;
    }
    if( FD_LIKELY( fd_ushort_bswap( msg->contact_info.contact_info->sockets[ udp_socket_idx[ i ] ].port ) ) ) {
      entry->udp_ports   [ i ] = fd_ushort_bswap( msg->contact_info.contact_info->sockets[ udp_socket_idx[ i ] ].port );
    }
  }
