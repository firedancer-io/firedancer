...
struct fd_schedulor {
  ...
  ulong     seq;   /* seq of the next scheduler event */
  ulong     magic;
};
...
fd_schedulor_handle_chainer_event( fd_schedulor_t *           self,
  ...
  task_key_t key = task_key( event->slot, &event->block_id );
  switch( event->kind ) {
  case FD_EVENT_CHAINER_SLOT_ADDED:
  case FD_EVENT_CHAINER_FEC_EVICTED:
    schedule( self, &key, event->ts );
    return;
  case FD_EVENT_CHAINER_SLOT_RETIRED:
    cancel( self, &key );
    return;
  case FD_EVENT_CHAINER_FEC_ADDED:
  case FD_EVENT_CHAINER_SLOT_COMPLETE:
  case FD_EVENT_CHAINER_FEC_DELIVERED:
    return; /* nunya business */
  default:
    FD_LOG_CRIT(( "bad chainer event kind %d", event->kind ));
  }
}
...
fd_schedulor_handle_requestor_event( fd_schedulor_t *             self,
  ...
  task_key_t key = task_key( event->slot, &event->block_id );
  switch( event->kind ) {
  case FD_EVENT_REQUESTOR_REQUESTED_PARENT: schedule( self, &key, now + FD_SCHEDULOR_PARENT_TIMEOUT_NS  ); return;
  case FD_EVENT_REQUESTOR_REQUESTED:        schedule( self, &key, now + FD_SCHEDULOR_REQUEST_TIMEOUT_NS ); return;
  case FD_EVENT_REQUESTOR_DONE:             return; /* nothing to recheck; a chainer event will queue it again if that changes */
  default: FD_LOG_CRIT(( "bad requestor event kind %u", event->kind ));
  }
}
...
fd_schedulor_poll_schedulor_event( fd_schedulor_t *       self,
  ...
  event->seq      = self->seq++;
  event->ts       = now;
  event->kind     = FD_EVENT_SCHEDULER_CHECK_SLOT;
  event->slot     = task->key.slot;
  event->block_id = task->key.block_id;
