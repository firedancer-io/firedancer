...
static ulong tsms( void ) {
  return (ulong)fd_log_wallclock() / (ulong)10e6; /* ms */
}
...
uchar *
preimage_pong( fd_hash_t * ping_token, uchar preimage[ static FD_REPAIR_PREIMAGE_SZ ] ) {
...
fd_repair_keyguard_hash_sign_pong( fd_keyguard_client_t * client, fd_hash_t * ping_token, fd_repair_pong_t * pong ) {
  ...
  fd_keyguard_client_sign( client, pong->sig, preimage_pong( ping_token, buf ), FD_REPAIR_PREIMAGE_SZ, FD_KEYGUARD_SIGN_TYPE_SHA256_ED25519 );
  return pong;
}

/* fd_repair_ed25519_hash_sign_pong hashes, signs and returns the
   provided pointer to pong. */
...
fd_repair_ed25519_hash_sign_pong( uchar const public_key[ 32 ], uchar const private_key[ 32 ], fd_hash_t * ping_token, fd_repair_pong_t * pong ) {
  uchar buf[ FD_REPAIR_PREIMAGE_SZ ];
  fd_sha256_hash( preimage_pong( ping_token, buf ), FD_REPAIR_PREIMAGE_SZ, pong->hash.uc );
  fd_sha512_t sha[1];
