...
after_credit( fd_repair_tile_ctx_t * ctx,
  ...
  if( FD_LIKELY( !fd_fec_out_empty( ctx->fec_chainer->out ) ) ) {
    ...
    if( FD_UNLIKELY( out.err != FD_FEC_CHAINER_SUCCESS ) ) {
      FD_LOG_WARNING(( "chainer err %d %lu %u %s %s", out.err, out.slot, out.fec_set_idx, FD_BASE58_ENC_32_ALLOCA( &out.merkle_root ), FD_BASE58_ENC_32_ALLOCA( &out.chained_root ) ));
