...
after_credit( fd_repair_tile_ctx_t * ctx,
  ...
  if( FD_LIKELY( !fd_fec_out_empty( ctx->fec_chainer->out ) ) ) {
    ...
    /* This is a hack to handle the fact the `block_id` field is not
       available in the snapshot manifest, so there is nothing to chain
       the very first FEC set to the snapshot slot's last FEC set. */

    if( FD_UNLIKELY( ctx->store->slot0!=(out.slot - out.parent_off) ) ) {
      // FD_LOG_NOTICE(( "linking %lu %s to %lu %s", out.slot, FD_BASE58_ENC_32_ALLOCA( &out.merkle_root ), out.slot - out.parent_off, FD_BASE58_ENC_32_ALLOCA( &out.chained_root ) ));
      // ctx->store->slot0    = ULONG_MAX;
      // fd_hash_t root_mr    = { 0 };
      // fd_store_fec_t * fec = fd_store_map_ele_remove( fd_store_map( ctx->store ), &out.merkle_root, NULL, fd_store_pool( ctx->store ) );
      // fec->key             = out.merkle_root;
      // fd_store_map_ele_insert( fd_store_map( ctx->store ), fec, fd_store_pool( ctx->store ) );
      // FD_LOG_NOTICE(( "rekeyed slot %lu to %s", out.slot,  ))
      fd_rwlock_read( &ctx->store->lock );
      fd_store_link( ctx->store, &out.merkle_root, &out.chained_root );
    ...
    if( FD_UNLIKELY( ctx->shredcap_enabled ) ) {
      ...
      fd_rwlock_read( &ctx->store->lock );
      fd_store_fec_t * fec = fd_store_query( ctx->store, &out.merkle_root );
