...
should_force_complete( fd_forest_t *  forest, fd_fec_intra_t * fec ) {
  ...
  for( uint idx = fec->fec_set_idx + 1; idx < ele->buffered_idx + 1; idx++ ) { /* TODO iterate by word */
    if( FD_UNLIKELY( fd_forest_ele_idxs_test( ele->fecs, idx ) ) ) {
      fec->data_cnt = (ushort)(idx - fec->fec_set_idx);
      return 1;
    }
  }
  if( FD_UNLIKELY( !fec->data_cnt && ele->complete_idx != UINT_MAX && ele->buffered_idx == ele->complete_idx ) ) {
    fec->data_cnt = (ushort)(ele->complete_idx - fec->fec_set_idx + 1);
    return 1;
  }
  return 0;
}
...
after_frag( fd_repair_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( in_kind==IN_KIND_SHRED ) ) {
    ...
    if( FD_UNLIKELY( is_fec_completes_msg( sz ) ) ) {
      ...
      fec->data_cnt = (ushort)(shred->idx + 1 - shred->fec_set_idx);
      ...
      while ( !fd_fec_out_empty( ctx->fec_chainer->out ) ) {
        fd_fec_out_t out = fd_fec_out_pop_head( ctx->fec_chainer->out );
        FD_LOG_NOTICE(( "new chainer %lu %u", out.slot, out.fec_set_idx ));
        FD_TEST( out.err == FD_FEC_CHAINER_SUCCESS );
        if( FD_UNLIKELY( out.data_complete ) ) {
          FD_LOG_NOTICE(( "completed data %lu %u", out.slot, out.data_cnt ));
          ulong sig   = fd_disco_repair_replay_sig( out.slot, out.parent_off, out.data_cnt, out.slot_complete );
          ulong tspub = fd_frag_meta_ts_comp( fd_tickcount() );
          fd_stem_publish( stem, REPLAY_OUT_IDX, sig, ctx->replay_out_chunk, 0, 0, tsorig, tspub );
