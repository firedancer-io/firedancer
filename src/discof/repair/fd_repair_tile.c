...
typedef struct fd_pong_data fd_pong_data_t;

struct fd_repair_pending_sign {
  ulong       key;        /* map key, unique nonce */
  ulong       buflen;
  union {
    uchar           buf[sizeof(fd_repair_msg_t)];
    fd_repair_msg_t msg;
  };
  fd_pong_data_t  pong_data;
};
...
pending_sign_request_insert( ctx_t *                  ctx,
                             ...
                             fd_pong_data_t  const *  opt_pong_data ) {
...
fd_repair_send_sign_request( ctx_t                 * ctx,
                             ...
                             fd_pong_data_t  const * opt_pong_data ){
  /* New sign request */
  fd_repair_pending_sign_t * pending = pending_sign_request_insert( ctx, msg, opt_pong_data );
...
after_contact( ctx_t * ctx, fd_gossip_update_message_t const * msg ) {
  ...
  if( peer ) {
    ...
    fd_ping_signs_queue_push( ctx->ping_queue, (fd_signable_ping_t){ .msg = *init } );
  }
}
...
after_sign( ctx_t             * ctx,
  ...
  if( FD_UNLIKELY( !active ) ) {
    ...
    pending_sign_request_remove( ctx, pending_key );
    fd_ping_signs_queue_push( ctx->ping_queue, (fd_signable_ping_t){ .msg = pending->msg } );
    return;
  }
  ...
  if( ctx->repair_intake_addr.port == dport ) {
    ...
    switch( res->kind ) {
      case FD_REPAIR_KIND_PING: {
        ...
        fd_ping_signs_queue_push( ctx->ping_queue, (fd_signable_ping_t){ .msg = *pong, .pong_data = { .peer_addr = peer_addr, .hash = res->ping.hash, .daddr = ip4->daddr } } );
        break;
      }
