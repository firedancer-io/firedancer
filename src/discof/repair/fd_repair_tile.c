...
handle_new_cluster_contact_info( fd_repair_tile_ctx_t * ctx,
  ...
  for( ulong i=0UL; i<dest_cnt; i++ ) {
   if( FD_UNLIKELY( ctx->repair->peer_cnt >= FD_ACTIVE_KEY_MAX ) ) break;// FIXME: aiming to move all peer tracking out of lib into tile, leaving like this for now
    ...
    if( !dup ) {
      /*
      Peer warm up. Peers send ping-pong messages on first repair request. Due to a design flaw in the iterator, we wait 40ms between each peer during warm up.
      This is fix to workaround the iterator and decrease warm up time from ~40ms * num_peers to ~ time until first turbine. (about 50 seconds decrease in startup time)
      */
