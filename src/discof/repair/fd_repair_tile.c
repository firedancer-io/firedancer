...
after_credit( ctx_t *             ctx,
  ...
  if( fd_log_wallclock() < (ctx->ts + (long)1e9) ) return;
  ...
  *charge_busy = 1;
  ...
  if( req->kind == FD_REPAIR_KIND_ORPHAN_REQ ) {
    FD_LOG_NOTICE(( "repair orphan %lu to " FD_IP4_ADDR_FMT ":%u", req->orphan.slot, FD_IP4_ADDR_FMT_ARGS(peer->ip4), peer->port ));
  } else if( req->kind == FD_REPAIR_KIND_SHRED_REQ ) {
    FD_LOG_NOTICE(( "repair shred %lu %lu to " FD_IP4_ADDR_FMT ":%u", req->shred.slot, req->shred.shred_idx, FD_IP4_ADDR_FMT_ARGS(peer->ip4), peer->port ));
  } else if( req->kind == FD_REPAIR_KIND_HIGHEST_SHRED_REQ ) {
    FD_LOG_NOTICE(( "repair highest shred %lu to " FD_IP4_ADDR_FMT ":%u", req->highest_shred.slot, FD_IP4_ADDR_FMT_ARGS(peer->ip4), peer->port ));
  } else {
    FD_LOG_ERR(( "unhandled repair request kind %u", req->kind ));
  }

  send_packet( ctx, 1, peer->ip4, peer->port, 0U, buf, sz, tsorig );
}
...
after_net( ctx_t * ctx, ulong sz ) {
  ...
  case fd_repair_response_enum_ping: {
    ...
    ulong tsorig = fd_frag_meta_ts_comp( fd_tickcount() );
    send_packet( ctx, 1, ip4->saddr, udp->net_sport, 0U /* populated by net */, buf, sz, tsorig );
    break;
  }
