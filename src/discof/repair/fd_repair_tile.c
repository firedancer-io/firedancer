...
typedef struct {
  fd_shred_t last;                /* The last data shred in the FEC set. */
  fd_hash_t  merkle_root;         /* The merkle root of the FEC set. */
  fd_hash_t  chained_merkle_root; /* The chained merkle root of the FEC set. */
} fec_completes_t;
...
is_fec_completes_msg( ulong sz ) {
  return sz == FD_SHRED_DATA_HEADER_SZ + FD_SHRED_MERKLE_ROOT_SZ * 2;
}
...
after_frag( fd_repair_tile_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( in_kind==IN_KIND_SHRED ) ) {
    ...
    if( FD_UNLIKELY( is_fec_completes_msg( sz ) ) ) {
      ...
      fec_completes_t const * fec_completes = (fec_completes_t const * )fd_type_pun_const( ctx->buffer );
      ...
      FD_TEST( fd_fec_chainer_insert( ctx->fec_chainer, shred->slot, shred->fec_set_idx, (ushort)(shred->idx - shred->fec_set_idx + 1), data_complete, slot_complete, shred->data.parent_off, &fec_completes->merkle_root, &fec_completes->chained_merkle_root ) );
    }
