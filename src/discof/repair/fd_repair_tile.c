...
after_frag( ctx_t *             ctx,
  ...
  if( FD_UNLIKELY( in_kind==IN_KIND_GOSSIP ) ) {
    fd_gossip_update_message_t const * msg = (fd_gossip_update_message_t const *)fd_type_pun_const( src );
  ...
  if( FD_UNLIKELY( in_kind==IN_KIND_TOWER ) ) {
    if( FD_LIKELY( sig==FD_TOWER_SIG_SLOT_DONE ) ) {
      fd_tower_slot_done_t const * msg = (fd_tower_slot_done_t const *)fd_type_pun_const( src );
      ...
      fd_tower_slot_confirmed_t const * msg = (fd_tower_slot_confirmed_t const *)fd_type_pun_const( src );
...
#   endif
  ...
  if( FD_UNLIKELY( in_kind==IN_KIND_REPLAY ) ) {
    fd_replay_fec_evicted_t const * msg = (fd_replay_fec_evicted_t const *)fd_type_pun_const( src );
