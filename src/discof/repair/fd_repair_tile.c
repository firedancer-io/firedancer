...
after_credit( fd_repair_tile_ctx_t * ctx,
  ...
  if( FD_LIKELY( !fd_fec_out_empty( ctx->fec_chainer->out ) ) ) {
    ...
    ulong tspub = fd_frag_meta_ts_comp( fd_tickcount() );
    fd_stem_publish( ctx->stem, REPLAY_OUT_IDX, sig, ctx->replay_out_chunk, sizeof(fd_fec_out_t), 0, 0, tspub );
    ...
    //     .slot          = out.slot,
    //     .parent_off    = out.parent_off,
    //     .fec_set_idx   = out.fec_set_idx,
    //     .data_cnt      = (ushort)cnt,
    //     .slot_complete = out.slot_complete,
    //     .data_complete = out.data_complete,
    //   };
    //   memcpy( fd_chunk_to_laddr( ctx->replay_out_mem, ctx->replay_out_chunk ), &frag, sizeof(fd_fec_out_t) );
    //   sig = out.slot << 32 | reasm->cnt;
    //   fd_stem_publish( ctx->stem, REPLAY_OUT_IDX, sig, ctx->replay_out_chunk, sizeof(fd_fec_out_t), 0, 0, tspub );
    //   ctx->replay_out_chunk = fd_dcache_compact_next( ctx->replay_out_chunk, sizeof(fd_fec_out_t), ctx->replay_out_chunk0, ctx->replay_out_wmark );
