...
/* fd_reasm inserts a new FEC set into reasm.  Returns the newly
   inserted fd_reasm_fec_t, NULL on error.  Inserting this FEC set may
   make one or more FEC sets available for in-order delivery.
   Caller can consume these FEC sets via fd_reasm_out.
...
/* fd_reasm_out pops the head of the out queue of FEC sets that are
   ready to be delivered to the caller in order.  Returns NULL if the
   queue is empty. */

fd_reasm_fec_t *
fd_reasm_out( fd_reasm_t * reasm );

/* fd_reasm_publish prunes the fec tree when the wmk is updated. */

void
fd_reasm_publish( fd_reasm_t * reasm, ulong new_root );
