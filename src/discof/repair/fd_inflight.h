...
/* fd_inflight tracks repair requests that are inflight to other
   ...
   time bucketed by 60ms, which is the same as the retransmission
   timeout, it's highly unlikely that a retransmission request will have
   the same nonce.  The chances that an inflight request does not get a
   response are non-negligible due to shred tile upstream deduping
   duplicates. */
