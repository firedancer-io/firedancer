...
static const uint sock_filter_policy_fd_rserve_tile_instr_cnt = 27;

static void populate_sock_filter_policy_fd_rserve_tile( ulong out_cnt, struct sock_filter out[ static 27 ], uint logfile_fd, uint shredb_fd ) {
  FD_TEST( out_cnt >= 27 );
  struct sock_filter filter[27] = {
    ...
    /* check fallocate */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_fallocate, /* check_fallocate */ 10, 0 ),
    /* check madvise */
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_madvise, /* check_madvise */ 13, 0 ),
    /* check fsync */
