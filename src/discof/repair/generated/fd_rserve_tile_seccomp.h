...
#endif
static const unsigned int sock_filter_policy_fd_rserve_tile_instr_cnt = 17;

static void populate_sock_filter_policy_fd_rserve_tile( ulong out_cnt, struct sock_filter * out, unsigned int logfile_fd, unsigned int shredb_fd ) {
  FD_TEST( out_cnt >= 17 );
  struct sock_filter filter[17] = {
    ...
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, SYS_ftruncate, /* check_ftruncate */ 6, 0 ),
...
//  check_ftruncate:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
    BPF_JUMP( BPF_JMP | BPF_JEQ | BPF_K, shredb_fd, /* RET_ALLOW */ 3, /* RET_KILL_PROCESS */ 2 ),
//  check_fsync:
    /* load syscall argument 0 in accumulator */
    BPF_STMT( BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, args[0])),
