...
test_meta_requests( fd_repair_t * client ) {
  ...
  for( uint k=0U; k<FEC_CNT; k++ ) {
    ...
    FD_TEST( !ag_repair_fec_set_root_verify( &res->fec_set_root, &blk_id, k*FD_FEC_SHRED_CNT ) );
  }
...
/* Without a blockdb, metadata requests are dropped as unknown before
   any signature work. */
...
test_no_blockdb( fd_repair_t * client ) {
  ...
  msg = ag_repair_fec_set_root( client, &server_pub, now_ms(), 1U, blk_slot, &blk_id, 0U );
  FD_TEST( !request( ctx, (uchar const *)msg, sign( msg ) ) );
  FD_TEST( ctx->metrics->received_malformed_count[ FD_METRICS_ENUM_RSERVE_MALFORMED_TYPES_V_UNKNOWN_TAG_IDX ]==2UL );
