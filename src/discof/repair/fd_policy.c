...
dedup_evict( fd_policy_t * policy ) {
  ...
  if( FD_UNLIKELY( !fd_policy_dedup_pool_free( policy->dedup->pool ) ) ) {
    FD_LOG_WARNING(( "no dedup lru to evict" ));
    return;
  }
...
dedup_next( fd_policy_t * policy, ulong key ) {
  ...
  if( FD_LIKELY( now < ele->req_ts + (long)20e6 ) ) {
    // FD_LOG_NOTICE( ( "deduped request for kind %u slot %lu shred_idx %u, peer_idx %lu, req_ts %ld",
    //                  fd_policy_dedup_key_kind( key ),
    //                  fd_policy_dedup_key_slot( key ),
    //                  fd_policy_dedup_key_shred_idx( key ),
    //                  ele->peer_idx,
    //                  ele->req_ts ) );
    return 1;
  }
...
fd_policy_next( fd_policy_t * policy, fd_forest_t * forest, fd_repair_t * repair ) {
  ...
  for( fd_forest_orphaned_iter_t iter = fd_forest_orphaned_iter_init( orphaned, pool );
    ...
    if( FD_LIKELY( dedup_next( policy, key ) ) ) continue;
  ...
  /* Every so often we'll need to reset the frontier iterator to the
    ...
    long now = fd_log_wallclock();
    if( FD_UNLIKELY( policy->iterf.ele_idx == null || now - policy->tsref > policy->tsmax ) ) {
  ...
  if( FD_UNLIKELY( dedup_next( policy, key )                    ) ) --policy->peers->idx;
  return out;
}
