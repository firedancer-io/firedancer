...
/* FD_VOTOR_VALIDATOR_MAX is the most validators alpenglow consensus can
   have in one epoch -- the validator admission ticket caps the voting
   set.  It bounds the votor's ranked validator table AND the size of
   the QUIC all-to-all mesh, which is why the topology needs it too:
   sizing votor's QUIC from the TPU tile's connection count instead
   reserves tens of thousands of connections that can never be used. */

#define FD_VOTOR_VALIDATOR_MAX (2000UL)
...
struct ag_votor_slot_done {
  ...
  /* Neither of the following is used by consensus.  They are the
     alpenglow counterparts of the vote_slot / is_voting fields
     fd_tower_slot_done_t carries for TowerBFT, and exist so the gui can
     show a vote state and a vote distance; replay forwards them.

     vote_slot is the highest slot this validator has cast an alpenglow
     vote in, of ANY type -- a skip vote is participation in consensus
     just as a notarization vote is, and counting only the latter would
     make a healthy validator look delinquent across a run of skipped
     leader slots.  ULONG_MAX before the first vote.

     is_voting is 1 if our identity is in the active epoch's ranked
     validator set, i.e. we are structurally able to vote at all.  Like
     tower's, it is independent of whether a vote was actually cast for
     any particular slot. */
