...
handle_microblock( fd_bank_ctx_t *     ctx,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    fd_exec_txn_ctx_setup( txn_ctx, TXN( txn ), &raw_txn );

    fd_executor_setup_txn_account_keys( txn_ctx );

    fd_runtime_pre_execute_check( &task_info );
    if( FD_UNLIKELY( task_info.exec_res ) ) {
      ctx->metrics.txn_result[ -fd_bank_err_from_runtime_err( task_info.exec_res ) ]++;
      continue;
    }
    ...
    int transaction_err = fd_bank_err_from_runtime_err( fd_execute_txn( &task_info ) );
    ...
    txn->flags = (txn->flags & 0x00FFFFFFU) | ((uint)transaction_err<<24);

    ctx->metrics.txn_result[ -transaction_err ]++;

    uint actual_execution_cus = (uint)(task_info.txn_ctx->compute_unit_limit - task_info.txn_ctx->compute_meter);
    uint actual_acct_data_cus = (uint)(task_info.txn_ctx->loaded_accounts_data_size);
    ...
    /* The VM will stop executing and fail an instruction immediately if
       ...
       loading stage. */
    if( FD_UNLIKELY( !task_info.exec_res && actual_execution_cus+actual_acct_data_cus>requested_exec_plus_acct_data_cus ) ) {
    ...
    fd_runtime_finalize_txn( NULL, NULL, &task_info, NULL, NULL ); // TODO: ARGS
  }
...
handle_bundle( fd_bank_ctx_t *     ctx,
  ...
  fd_execute_txn_task_info_t task_info[ MAX_TXN_PER_MICROBLOCK ];
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ) {
    ...
    task_info[ i ] = (fd_execute_txn_task_info_t){
      .txn_ctx  = txn_ctx,
      .exec_res = 0,
      .txn      = txn,
    };
    ...
    fd_runtime_pre_execute_check( &task_info[ i ] );
    if( FD_UNLIKELY( task_info[ i ].exec_res ) ) {
      transaction_err[ i ] = fd_bank_err_from_runtime_err( task_info[ i ].exec_res );
      execution_success = 0;
      break;
    }
  ...
  if( FD_LIKELY( execution_success ) ) {
    for( ulong i=0UL; i<txn_cnt; i++ ) {
      int result = fd_bank_err_from_runtime_err( fd_execute_txn( &task_info[ i ] ) );
      actual_execution_cus[ i ] = (uint)(task_info[ i ].txn_ctx->compute_unit_limit - task_info[ i ].txn_ctx->compute_meter);
      actual_acct_data_cus[ i ] = (uint)(task_info[ i ].txn_ctx->loaded_accounts_data_size);
      (void)tips; // TODO: GUI, report tips
      (void)out_timestamps; // TODO: GUI, report timestamps
      transaction_err[ i ] = result;
