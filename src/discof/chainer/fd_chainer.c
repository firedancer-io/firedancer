...
fd_chainer_shred_insert( fd_chainer_t *        chainer,
  ...
  for( ulong _i =slotv_iter_init( chainer, slot );
    ...
    if( FD_UNLIKELY( rx_ts && !slotv->metrics.last_shred_ts && slotv->complete_idx!=UINT_MAX && slotv->buffered_idx==slotv->complete_idx ) ) {
      ...
      FD_LOG_NOTICE(( "slot %lu complete in %ld ms. turbine %u repair %u recovered %u code %u", slot, ( rx_ts - slotv->metrics.first_shred_ts )/1000000L, slotv->metrics.turbine_cnt, slotv->metrics.repair_cnt, slotv->metrics.recovered_cnt, slotv->metrics.parity_cnt ));
    }
