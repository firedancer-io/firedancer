...
```json
{
    ...
    "value": {
        ...
        "total_elapsed": 7.9,
        "joining_gossip_elapsed": 5,
        "loading_full_snapshot_elapsed_secs": 7.8,
        ...
        "loading_full_snapshot_read_throughput_compressed": 107384637.43,
        ...
        "loading_full_snapshot_decompress_throughput_compressed": 107381727.35,
        "loading_full_snapshot_insert_bytes_decompressed": "4864409599",
        "loading_full_snapshot_insert_throughput_decompressed": 714354268.39,
        "loading_full_snapshot_insert_accounts_throughput": 1994829.92,
        "loading_full_snapshot_insert_accounts_current": 10634591,
        ...
        "loading_incremental_snapshot_read_throughput_compressed": null,
        ...
        "loading_incremental_snapshot_decompress_throughput_compressed": null,
        "loading_incremental_snapshot_insert_bytes_decompressed": null,
        "loading_incremental_snapshot_insert_throughput_decompressed": null,
...
| loading_{full|incremental}_snapshot_read_throughput_compressed       | `number\|null`  | If the phase is at least `loading_{full|incremental}_snapshot`, this is the (compressed) current read throughput for the snapshot in bytes per second. Otherwise, `null`. |
...
| loading_{full|incremental}_snapshot_decompress_throughput_compressed | `number\|null`  | If the phase is at least `loading_{full|incremental}_snapshot`, this is the (compressed) throughput of decompress from the snapshot in bytes per second. Otherwise, `null`. |
...
| loading_{full|incremental}_snapshot_insert_throughput_decompressed   | `number\|null`  | If the phase is at least `loading_{full|incremental}_snapshot`, this is the (decompressed) insertion throughput in bytes per second for the snapshot. Otherwise, `null`. |
...
```json
{
    "health": {
        "push_rx_pct": 0.5,
        "pull_response_rx_pct": 0.5,
        "push_rx_dup_pct": 0.14,
        "pull_response_rx_dup_pct": null,
        "push_rx_msg_bad_pct": 0.3,
        "push_rx_entry_bad_pct": 0.5,
        "pull_response_rx_msg_bad_pct": 0,
        "pull_response_rx_entry_bad_pct": null,
        "pull_already_known_pct": null,
    ...
    "storage": {
        ...
        "count": [0, 10608, 95, ...],
        "eps_tx": [0, 3359.98, 0, ...],
        "bps_tx": [0, 1548950.29, 0, ...]
    },
    "messages": {
        "bytes_rx_total": [857419, 8839524, 43480758, ...],
        "bytes_tx_total": [28938, 2416123, 72351557, ...],
        "count_rx_total": [1364, 20477, 456094, ...],
        "count_tx_total": [26, 2490, 73599, ...],
        "bps_rx": [0, 0, 1856398.36, ...],
        "mps_rx": [0, 0, 3099.98, ...],
        "bps_tx": [11129.93, 0, 5176817.54, ...],
        "mps_tx": [10, 0, 5439.97, ...]
    }
}
```
...
| Field                          | Type     | Description |
|--------------------------------|----------|-------------|
| push_rx_pct                    | `number` | `100 * num_rx_push_messages / ( num_rx_push_messages + num_rx_pull_request_messages )`. Messages older than 400ms are not considered |
| pull_response_rx_pct           | `number` | `100 * num_rx_pull_request_messages / ( num_rx_push_messages + num_rx_pull_request_messages )`. Messages older than 400ms are not considered |
| push_rx_dup_pct                | `number` | The percentage of all Table entries (including dropped and evicted entries) that were discarded due to being a duplicate. A duplicate Table entry is identical to one we have received in the past. Only Table entries received via valid push message are considered. Messages older than 400ms are not considered |
| pull_response_rx_dup_pct       | `number` | The percentage of all Table entries (including dropped and evicted entries) that were discarded due to being a duplicate. Only Table entries received via valid pull response message are considered. Messages older than 400ms are not considered |
| push_rx_msg_bad_pct            | `number` | The percentage of all Gossip push messages that were discarded for any reason. Messages older than 400ms are not considered |
| push_rx_entry_bad_pct          | `number` | The percentage of all Table entries (including dropped and evicted entries) that were discarded for any reason. Examples include duplicates, invalid signatures, invalid shred version, etc. Only Table entries received via valid push message are considered. Messages older than 400ms are not considered |
| pull_response_rx_msg_bad_pct   | `number` | The percentage of all Gossip pull response messages that were discarded for any reason. Messages older than 400ms are not considered |
| pull_response_rx_entry_bad_pct | `number` | The percentage of all Table entries (including dropped and evicted entries) that were discarded for any reason. Only Table entries received via valid pull response message are considered. Messages older than 400ms are not considered |
