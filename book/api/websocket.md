...
| vote_latency_exact           | `number\|null` | Like `vote_latency`, but discounting slots that were skipped on the fork our vote landed on.  This is the effective vote latency ignoring gaps in the leader schedule.  `null` under the same conditions as `vote_latency` |
...
latency (`null` for runs whose slots were never voted for).
