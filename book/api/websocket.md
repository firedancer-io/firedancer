...
| Field    | Type               | Description |
|----------|--------------------|-------------|
| data     | `object`           | The tabular data in the clients view, as a 2D dictionary. The dictionary is keyed by row index (object keys are always strings). Each value is a dictionary that represents a table row. Each row is keyed by column name, and each row value is the value of the cell for the corresponding (rowIndex, column_name) |
...
The server maintains a copy of each client's sort key.  The message
updates two attributes for the included column.

- precedence: Indicates the order in which to sort the columns. Columns
with highest precedence will be sorted later, which increases the visual
impact of their ordering. Every time a sort key update is received, the
included column will be moved to the highest precedence. The
default/initial sort key column precedence is ordered by column name
sorted in descending order.
- sort direction: Indicates the direction of sort which rows will be
sorted.  This only makes sense as 'asc' (ascending) or 'desc'
descending. The default/initial sort direction for columns is 'desc'.

Since updating the sort key changes the client's view completely, the
response will be a fresh copy of all the data in the client's new view.
...
| frequency       | type                      | example     |
|-----------------|---------------------------|-------------|
| *Once* + *Live* | `GossipPeerTableRowCount` | below       |

Sent every time the total number of rows in the gossip peer table
changes.
...
**`GossipPeerTableRowCount`**
| Field   | Type       | Description |
|---------|------------|-------------|
| row_cnt | `number`   | The latest known count of the number of rows in the gossip peer table |
