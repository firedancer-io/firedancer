...
| loading_{full|incremental}_snapshot_elapsed                    | `number\|null`  | If the phase is at least `loading_{full|incremental}_snapshot`, this is the elapsed time, in seconds, spent loading the {full|incremental} snapshot since the last reset. Otherwise, `null`. |
...
| loading_{full|incremental}_snapshot_decompress_bytes           | `number\|null`  | If the phase is at least `loading_{full|incremental}_snapshot`, this is the (compressed) number of bytes processed by decompress from the {full|incremental} snapshot so far. Otherwise, `null`. |
