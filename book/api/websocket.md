...
Under Alpenglow, `vote_slot` is the latest target slot reflected in the
validator's on-chain vote account.  See `summary.vote_slot` for more
...
Under Alpenglow, this is the latest target slot reflected in this
validator's on-chain vote account.  The value advances when the
validator's ordinary notarize or skip vote is included in a reward
certificate, or when its signature is included in a direct fast or slow
finalization proof. It is the slot being voted on, not the later slot
whose block carries the certificate.
...
fully confirmed and irreversible, and the rooted slot never decreases.
The slot number may increase by more than one across updates as skipped
slots do update the root slot.
...
votes. It can also be greater than `summary.root_slot` or
`summary.completed_slot` while the validator is still acquiring or
replaying the finalized fork and never decreases.

This message is Alpenglow-only and replaces
`summary.optimistically_confirmed_slot`.
...
Under Alpenglow, this is the slot of the latest reset parent selected by
Votor's state machine for replay and leader production. A fork choice is
triggered when Votor emits a parent-ready event for the first slot of a
leader window. This occurs once a block has a regular or fallback
notarization certificate (or a stronger fast-finalization certificate)
and every intervening slot has a skip certificate. This is not derived
from an on-chain Tower vote or lockout.
...
| vote_slot               | `number\|null`  | Latest target slot reflected in this validator's on-chain vote account after executing this slot's block, when one was replayed. It advances through reward-certificate or direct-finalization participation and is not the slot which carried the certificate. It is `null` when no replay-bank snapshot exists |
...
for which this validator was structurally a voter but was not included
in either the ordinary notarize or ordinary skip reward certificate.
Notar-fallback, skip-fallback, and finalization votes are not rewarded
and do not remove a slot from this history. Participation in a
finalization certificate may advance `vote_slot` without changing this
reward result.
