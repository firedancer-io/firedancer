...
## `set-identity`
Changes the identity key of a running validator. The `<PATH>` argument
is required and must be the path to an Agave style `identity.json`
keypair file. If the path is specified as `-` the key will instead be
read from `stdin`.
...
to the new one, to ensure that vote lockouts are repected.
...
| Arguments         | Description |
|-------------------|-------------|
| `<PATH>`          | Path to a `identity.json` keypair file, or `-` to read the JSON formatted key from `stdin` |
