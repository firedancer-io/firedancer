...
FRONTEND_TARGET_CLIENT := frankendancer
ifeq ($(FRONTEND_TARGET_CLIENT),frankendancer)
override FRONTEND_DIST_MODULE := disco
else ifeq ($(FRONTEND_TARGET_CLIENT),firedancer)
override FRONTEND_DIST_MODULE := discof
else
$(error FRONTEND_TARGET_CLIENT must be either frankendancer or firedancer)
endif


frontend:
	cd frontend && npm ci && npm run build:$(if $(findstring frankendancer,$(FRONTEND_TARGET_CLIENT)),fr,fd)
	rm -rf src/$(FRONTEND_DIST_MODULE)/gui/dist
	mkdir -p src/$(FRONTEND_DIST_MODULE)/gui/dist
	cp -r frontend/dist/* src/$(FRONTEND_DIST_MODULE)/gui/dist
	> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
	echo "/* THIS FILE WAS GENERATED BY make frontend. DO NOT EDIT BY HAND! */" >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
	echo "#include \"http_import_dist.h\"" >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
	echo "" >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
	counter=0; \
	for file in $$(find src/$(FRONTEND_DIST_MODULE)/gui/dist -type f | sort); do \
		echo "FD_IMPORT_BINARY( file$$counter, \"$$file\" );" >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
		counter=$$((counter + 1)); \
	done; \
	echo "" >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
	echo "fd_http_static_file_t STATIC_FILES[] = {" >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
	counter=0; \
	for file in $$(find src/$(FRONTEND_DIST_MODULE)/gui/dist -type f | sort); do \
		stripped_file=$$(echo $$file | sed 's|^src/$(FRONTEND_DIST_MODULE)/gui/dist/||'); \
		echo "    {" >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
		echo "        .name = \"/$$stripped_file\"," >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
		echo "        .data = file$$counter," >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
		echo "        .data_len = &file$${counter}_sz," >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
		echo "    }," >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
		counter=$$((counter + 1)); \
	done; \
	echo "    {0}" >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
	echo "};" >> src/$(FRONTEND_DIST_MODULE)/gui/generated/http_import_dist.c; \
