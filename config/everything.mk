...
# FD_FIXED_BUILD_INFO makes the build info blob a constant so builds
# are deterministic and fd_log.o is compiler-cacheable (see
# config/extra/with-ccache.mk, which enables it by default).  The
# content must be a CONSTANT, not e.g. "$(MACHINE)/$(EXTRAS)": extras
# like rpath only alter LDFLAGS, so two different EXTRAS strings can
# produce identical compile command lines, and a compiler cache would
# then (correctly, per its key) serve an object embedding the other
# build's info text.  Constant content makes any cached fd_log.o
# correct by construction.
#
# Mode switches on a pre-existing OBJDIR must never leave stale
# content, and an unchanged file must keep its mtime so fd_log.o is
# not invalidated:
#   fixed:    force-run with a cmp guard (content is a constant)
#   volatile: rule unchanged from historical behavior; a leftover
#             fixed-mode file is removed at parse time so the normal
#             mtime logic regenerates it
ifdef FD_FIXED_BUILD_INFO
.PHONY: fd-info-force
$(OBJDIR)/info : fd-info-force
	$(Q)$(MKDIR) $(dir $@) && \
echo "# build info elided for deterministic builds (FD_FIXED_BUILD_INFO)" > $@.tmp && \
if cmp -s $@.tmp $@ ; then $(RM) $@.tmp ; else mv $@.tmp $@ ; fi
else
_:=$(shell grep -sq "FD_FIXED_BUILD_INFO" $(OBJDIR)/info && $(RM) $(OBJDIR)/info)
$(OBJDIR)/info :
	$(MKDIR) $(dir $@) && \
echo -e \
...
# FD_COMPILE_PREFIX is empty by default and set to ccache by
# EXTRAS=ccache (config/extra/with-ccache.mk).  Objects that embed
# build-time-GENERATED files via FD_IMPORT (assembler .incbin, which
# ccache cannot key on -- committed embeds are handled via
# CCACHE_EXTRAFILES instead) must never be served from cache, so they
# force the prefix empty via target-specific overrides:
#
#   fd_log.o embeds $(OBJDIR)/info (volatile: date/user/git status)
#   http_import_dist.o embeds dist_cmp/*.{gz,zst} (generated by
#     src/disco/gui/Local.mk pattern rules from the committed dist/)
#
# If you add an FD_IMPORT of a generated file, add its object here.
#
# fd_log.o is only exempted when the info blob is volatile; under
# FD_FIXED_BUILD_INFO the embedded content is a constant, so a cached
# fd_log.o is correct by construction and may be cached like any
# other object.
ifndef FD_FIXED_BUILD_INFO
$(OBJDIR)/obj/util/log/fd_log.o: FD_COMPILE_PREFIX=
endif
$(OBJDIR)/obj/disco/gui/generated/http_import_dist.o: FD_COMPILE_PREFIX=
$(OBJDIR)/obj/discoh/guih/generated/http_import_dist.o: FD_COMPILE_PREFIX=
...
$(OBJDIR)/obj/%.o : src/%.c
	@echo -e "CC\t$(notdir $@)"
	$(Q)$(MKDIR) $(dir $@) && \
$(FD_COMPILE_PREFIX) $(CC) $(CPPFLAGS) $(CFLAGS) $(DEPFLAGS) -c $< -o $@

$(OBJDIR)/obj/%.o : src/%.cxx
	@echo -e "CXX\t$(notdir $@)"
	$(Q)$(MKDIR) $(dir $@) && \
$(FD_COMPILE_PREFIX) $(CXX) $(CPPFLAGS) $(CXXFLAGS) $(DEPFLAGS) -c $< -o $@

$(OBJDIR)/obj/%.o : src/%.S
	@echo -e "AS\t$(notdir $@)"
	$(Q)$(MKDIR) $(dir $@) && \
$(FD_COMPILE_PREFIX) $(CC) $(CPPFLAGS) $(CFLAGS) -c $< -o $@
